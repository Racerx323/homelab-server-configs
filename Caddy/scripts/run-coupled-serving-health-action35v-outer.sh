...
readonly prefix=action_35_v_outer
readonly transaction_sha256=5b948fe56bc4cf5d5be07631fc4b5418214faf0390ed9534fe9b7f53e5c8d838
...
safe_capture() {
    local action35v_path=$1

    regular_file "$action35v_path"
    [[ "$(stat -c '%s' "$action35v_path")" -le "$max_stream_bytes" ]]
    iconv -f UTF-8 -t UTF-8 "$action35v_path" >/dev/null
    ! LC_ALL=C grep -Paq '[\x00-\x08\x0b\x0c\x0e-\x1f\x7f]' "$action35v_path"
}

capture() {
    local action35v_label=$1
    shift
    local action35v_stdout=$workstation_evidence/$action35v_label.stdout
    local action35v_stderr=$workstation_evidence/$action35v_label.stderr
    local action35v_status=$workstation_evidence/$action35v_label.status
    ...
    safe_capture "$action35v_stdout"
    safe_capture "$action35v_stderr"
    return "$action35v_rc"
}

build_payload() {
    local action35v_repository action35v_source action35v_target action35v_mode
    local action35v_hash action35v_lifecycle action35v_source_path action35v_destination
    ...
    install -m 0600 "$repository_root/Caddy/manifests/action35v-node-b-quarantine.tsv" \
        "$payload_stage/manifests/action35v-node-b-quarantine.tsv"
    while IFS=$'\t' read -r action35v_repository action35v_source action35v_target \
        action35v_mode action35v_hash action35v_lifecycle; do
        [[ "$action35v_repository" = '# repository' ]] && continue
        [[ "$action35v_mode" =~ ^0[0-7]{3}$ && "$action35v_lifecycle" = production-candidate ]]
        [[ "$action35v_target" = /* ]]
        action35v_source_path=${workspace_root}/$action35v_repository/$action35v_source
        regular_file "$action35v_source_path"
        [[ "$(sha256sum "$action35v_source_path" | awk '{ print $1 }')" = "$action35v_hash" ]]
        action35v_destination=$payload_stage/repositories/$action35v_repository/$action35v_source
        install -d -m 0700 "$(dirname -- "$action35v_destination")"
        install -m 0600 "$action35v_source_path" "$action35v_destination"
...
readonly archive=$2
case "$remote_root" in /tmp/caddy-action35v-*) ;; *) exit 64 ;; esac
case "$archive" in /tmp/caddy-action35v-*.tar) ;; *) exit 64 ;; esac
...
case "$evidence_root" in /tmp/caddy-action35v-*) ;; *) exit 64 ;; esac
...
ssh_stream() {
    local action35v_host=$1
    local action35v_program=$2
    shift 2
    "$ssh_command" "$action35v_host" \
        "cd / && sudo -n /bin/bash -s --$(printf ' %q' "$@")" <"$action35v_program"
}

upload_payload() {
    local action35v_role=$1
    local action35v_host=$2
    local action35v_remote_root=$3
    local action35v_remote_archive=$4
    local action35v_hash

    action35v_hash=$(awk '{ print $1 }' "$workstation_evidence/payload.sha256")
    capture "$action35v_role-upload-prepare" ssh_stream "$action35v_host" \
        "$prepare_program" "$action35v_remote_root" "$action35v_remote_archive"
    capture "$action35v_role-upload-copy" "$scp_command" -p -- \
        "$payload_archive" "$action35v_host:$action35v_remote_archive"
    capture "$action35v_role-upload-accept" ssh_stream "$action35v_host" \
        "$accept_program" "$action35v_remote_root" "$action35v_remote_archive" \
        "$action35v_hash"
}
...
remote_transaction() {
    ...
    capture "$action35v_label" ssh_stream "$action35v_host" "$transaction" "$@"
}

readback() {
    local action35v_role=$1
    local action35v_host=$2
    local action35v_remote_evidence=$3

    capture "$action35v_role-readback" ssh_stream "$action35v_host" \
        "$readback_program" "$action35v_remote_evidence"
}
...
cleanup_remote() {
    ...
    capture "$action35v_role-disposition" ssh_stream "$action35v_host" \
        "$disposition_program" "$action35v_remote_root" "$action35v_remote_archive"
}

run_live() {
    local action35v_node_b_mutated=false
    local action35v_node_a_mutated=false
    local action35v_failure=0
    local action35v_phase_status=0
    ...
    if upload_payload node-b "$node_b_host" "$node_b_payload" "$node_b_archive" &&
        ...
        return "$action35v_phase_status"
    fi

    action35v_node_b_mutated=true
    ...
    if [[ "$action35v_failure" -eq 0 ]]; then
    ...
    if ! remote_transaction node-a-journal-capture "$node_a_host" journal-capture node-a \
        "$node_a_payload" "$node_a_evidence"; then
        [[ "$action35v_failure" -ne 0 ]] || action35v_failure=1
    fi
    ...
    if [[ "$action35v_failure" -ne 0 ]]; then
        if [[ "$action35v_node_a_mutated" = true ]]; then
            remote_transaction node-a-rollback "$node_a_host" rollback node-a \
                "$node_a_payload" "$node_a_evidence" || exit 125
        fi
        if [[ "$action35v_node_b_mutated" = true ]]; then
...
production_path_test() {
    ...
    local action35v_remote_base action35v_raw action35v_decision action35v_observed
    local action35v_status action35v_role action35v_remote_evidence action35v_test_id
    local retained_root retained_candidate retained_release_hash retained_payload_hash
    local action35v_transaction_test_evidence
    ...
    action35v_remote_base=$(mktemp -d /tmp/caddy-action35v-production-path.XXXXXX)
    action35v_test_id=${action35v_remote_base##*.}
    test_remote_base=$action35v_remote_base
    export ACTION35V_TEST_REMOTE_BASE=$test_remote_base
...
[[ "$1" = -n ]]
shift
export ACTION35V_PRODUCTION_PATH_TEST=1
exec "$@"
SUDO
...
set -Eeuo pipefail
[[ "$1" = -u && "$3" = -- ]]
printf '%s\n' "$2" >>"$ACTION35V_TEST_REMOTE_BASE/runuser.identities"
shift 3
exec "$@"
RUNUSER
...
#!/usr/bin/env bash
set -Eeuo pipefail
printf '%s\n' "$*" >>"$ACTION35V_TEST_REMOTE_BASE/prohibited-keepalived-parser.calls"
exit 97
KEEPALIVED
...
#!/usr/bin/env bash
set -Eeuo pipefail
printf '%s\n' "$*" >>"$ACTION35V_TEST_REMOTE_BASE/systemctl.calls"
case "$1" in
    is-active) [[ "$2" = --quiet ]] ;;
    start | stop) [[ $# -eq 2 ]] ;;
...
set -Eeuo pipefail
case " $* " in
    *' --show-cursor '*) printf '%s\n' '-- cursor: s=action35v-production-path' ;;
    *' --after-cursor '*) printf '%s\n' 'test-host caddy.service: action35v production-path event' ;;
    *) exit 64 ;;
esac
JOURNALCTL
    cat >"$ssh_command" <<'SSH'
...
shift
command="$*"
printf '%s\t%s\n' "$host" "$command" >>"$ACTION35V_TEST_REMOTE_BASE/ssh.calls"
PATH="$ACTION35V_TEST_REMOTE_BASE/bin:/usr/bin:/bin" /bin/bash -c "$command"
SSH
    cat >"$scp_command" <<'SCP'
#!/usr/bin/env bash
set -Eeuo pipefail
...
source=$3
target=${4#*:}
printf '%s\t%s\n' "$source" "$target" >>"$ACTION35V_TEST_REMOTE_BASE/scp.calls"
...
SCP
    ...
    node_a_payload=/tmp/caddy-action35v-test-$action35v_test_id-node-a
    node_b_payload=/tmp/caddy-action35v-test-$action35v_test_id-node-b
    ...
    ACTION35V_INCOMING_ROOT=$retained_root/incoming \
        ACTION35V_OUTGOING_ROOT=$retained_root/outgoing \
        ACTION35V_QUARANTINE_ROOT=$retained_root/quarantine \
        ACTION35V_RELEASES_ROOT=$retained_root/releases \
        ACTION35V_RETAINED_RELEASE_MANIFEST_SHA256=$retained_release_hash \
        ACTION35V_RETAINED_PAYLOAD_MANIFEST_SHA256=$retained_payload_hash \
    ...
    ACTION35V_ENVIRONMENT_FILE=$test_remote_base/environment-node-b \
        ACTION35V_UNBOUND_CHECKCONF_COMMAND=$test_remote_base/bin/unbound-checkconf \
        ACTION35V_RUNUSER_COMMAND=$test_remote_base/bin/runuser \
        ACTION35V_SYSTEMCTL_COMMAND=$test_remote_base/bin/systemctl \
        ACTION35V_DNS_DIG_COMMAND=$test_remote_base/bin/dig \
        ACTION35V_CURL_COMMAND=$test_remote_base/bin/curl \
        ACTION35V_SS_COMMAND=$test_remote_base/bin/ss \
    ...
    ACTION35V_JOURNALCTL_COMMAND=$test_remote_base/bin/journalctl \
    ...
    ACTION35V_TARGET_ROOT=$test_remote_base/node-b-root \
        ACTION35V_SYSTEMCTL_COMMAND=$test_remote_base/bin/systemctl \
        ACTION35V_UNBOUND_CHECKCONF_COMMAND=$test_remote_base/bin/unbound-checkconf \
        remote_transaction node-b-install-producer "$node_b_host" install node-b \
        "$node_b_payload" "$node_b_evidence"
    action35v_transaction_test_evidence=$action35v_test_root/transaction-through-outer
    install -d -m 0700 "$action35v_transaction_test_evidence"
    CADDY_PRODUCTION_PATH_EVIDENCE_ROOT=$action35v_transaction_test_evidence \
        ACTION35V_TEST_REPOSITORY_ROOT=$repository_root \
        remote_transaction node-a-post-promote-production-path "$node_a_host" \
        --production-path-test
    ACTION35V_TARGET_ROOT=$test_remote_base/node-a-root \
        ACTION35V_SYSTEMCTL_COMMAND=$test_remote_base/bin/systemctl \
        ACTION35V_UNBOUND_CHECKCONF_COMMAND=$test_remote_base/bin/unbound-checkconf \
    ...
    action35v_raw=$action35v_test_root/raw/outer-preflight.txt
    action35v_decision=$action35v_test_root/decisions/outer-preflight.tsv
    cat "$test_remote_base/ssh.calls" "$test_remote_base/scp.calls" \
        ...
        >"$action35v_raw"
    action35v_observed=$(wc -l <"$action35v_raw")
    write_decision outer-preflight reach 0 "$action35v_observed" \
        "$action35v_observed" "$action35v_raw" "$action35v_decision"

    for action35v_role in node-a node-b; do
        if [[ "$action35v_role" = node-a ]]; then
            action35v_remote_evidence=$node_a_evidence
        else
            action35v_remote_evidence=$node_b_evidence
        fi
        action35v_raw=$action35v_test_root/raw/evidence-readback-$action35v_role-success.txt
        action35v_decision=$action35v_test_root/decisions/evidence-readback-$action35v_role-success.tsv
        ssh_stream "test-$action35v_role" "$readback_program" \
            "$action35v_remote_evidence" >"$action35v_raw"
        action35v_observed=$(sha256sum "$action35v_raw" | awk '{ print $1 }')
        write_decision "evidence-readback-$action35v_role-success" accept 0 \
            "$action35v_observed" "$action35v_observed" "$action35v_raw" "$action35v_decision"

        action35v_raw=$action35v_test_root/raw/evidence-readback-$action35v_role-failure.txt
        action35v_decision=$action35v_test_root/decisions/evidence-readback-$action35v_role-failure.tsv
        if ssh_stream "test-$action35v_role" "$readback_program" \
            "$action35v_remote_evidence-absent" >"$action35v_raw" 2>&1; then
            action35v_status=0
        else
            action35v_status=$?
        fi
        write_decision "evidence-readback-$action35v_role-failure" reject \
            "$action35v_status" present absent "$action35v_raw" "$action35v_decision"
    done
    cleanup_remote node-a "$node_a_host" "$node_a_payload" "$node_a_archive"
    cleanup_remote node-b "$node_b_host" "$node_b_payload" "$node_b_archive"
    chmod -R u+rwX -- "$action35v_remote_base"
    rm -rf -- "$action35v_remote_base"
...
transaction=$repository_root/Caddy/scripts/apply-coupled-serving-health-action35v.sh
...
if [[ "$invocation_mode" = --production-path-test ]]; then
    workstation_evidence=$(mktemp -d /tmp/caddy-action35v-outer-test.XXXXXX)
else
    workstation_evidence=$(mktemp -d /tmp/caddy-ssh-evidence-action35v.XXXXXX)
fi
readonly workstation_evidence
chmod 0700 "$workstation_evidence"
payload_stage=$workstation_evidence/payload
payload_archive=$workstation_evidence/action35v-payload.tar
...
readonly run_id
node_a_payload=/tmp/caddy-action35v-$run_id-node-a
node_b_payload=/tmp/caddy-action35v-$run_id-node-b
node_a_archive=$node_a_payload.tar
node_b_archive=$node_b_payload.tar
node_a_evidence=$node_a_payload/evidence
node_b_evidence=$node_b_payload/evidence
