...
readonly prefix=action_20d_retry9_readiness
readonly maximum_stream_bytes=1048576
readonly maximum_stream_lines=4096
readonly probe_sha256=0acc886c5317179571fdc0947e69f19fa57f3aa34f031ab5788727cdfee4750e
...
readonly probe="$script_directory/inspect-dual-node-caddy-readiness-action20d-retry9.sh"
readonly ssh_binary=${CADDY_ACTION20D_RETRY9_READINESS_SSH_BINARY:-/usr/bin/ssh}
...
validate_probe() {
    ...
    validation_observed_count=$(grep -Ec '^action_20d_retry9_readiness_probe_assertion_[a-z0-9_]+=(true|false)$' "$validation_stdout") || return 1
    [[ "$validation_observed_count" -eq "$validation_expected_count" ]] || return 1
    [[ "$(grep '^action_20d_retry9_readiness_probe_assertion_' "$validation_stdout" | cut -d= -f1 | LC_ALL=C sort | uniq -d | wc -l)" -eq 0 ]] || return 1
    while IFS= read -r validation_label; do
        require_one "action_20d_retry9_readiness_probe_assertion_${validation_label}=true" "$validation_stdout" || return 1
    done < <("$probe" --expected-assertions)
    require_one "action_20d_retry9_readiness_probe_value_node_role=$validation_role" "$validation_stdout" || return 1
    require_one "action_20d_retry9_readiness_probe_assertion_count=$validation_expected_count" "$validation_stdout" || return 1
    require_one 'action_20d_retry9_readiness_probe_failed_assertion_count=0' "$validation_stdout" || return 1
    require_one 'action_20d_retry9_readiness_probe_first_failure=none' "$validation_stdout" || return 1
    require_one 'action_20d_retry9_readiness_probe_notification_helper_invoked=false' "$validation_stdout" || return 1
    for validation_marker in filesystem_mutations service_mutations keepalived_mutations \
        ...
        require_one "action_20d_retry9_readiness_probe_${validation_marker}=false" "$validation_stdout" || return 1
    done
    require_one 'action_20d_retry9_readiness_probe_remote_complete=true' "$validation_stdout"
}
run_probe() {
    local probe_role=$1
...
write_probe_fixture() {
    ...
    fixture_dedupe_metadata=pi:pi:750
    {
        while IFS= read -r fixture_label; do
            printf 'action_20d_retry9_readiness_probe_assertion_%s=true\n' "$fixture_label"
        done < <("$probe" --expected-assertions)
        printf '%s\n' \
            "action_20d_retry9_readiness_probe_value_node_role=$fixture_role" \
            'action_20d_retry9_readiness_probe_value_inherited_execution_user=pi' \
            "action_20d_retry9_readiness_probe_value_state_directory_metadata=$fixture_state_metadata" \
            "action_20d_retry9_readiness_probe_value_dedupe_directory_metadata=$fixture_dedupe_metadata" \
            'action_20d_retry9_readiness_probe_value_caddy_ipv4_vip_count=0' \
            'action_20d_retry9_readiness_probe_value_caddy_ipv6_vip_count=0' \
            "action_20d_retry9_readiness_probe_value_dns_ipv4_vip_count=$fixture_dns_count" \
            "action_20d_retry9_readiness_probe_value_dns_ipv6_vip_count=$fixture_dns_count" \
            "action_20d_retry9_readiness_probe_value_before_snapshot_sha256=$fixture_hash" \
            "action_20d_retry9_readiness_probe_value_after_snapshot_sha256=$fixture_hash" \
            "action_20d_retry9_readiness_probe_assertion_count=$("$probe" --expected-assertions | wc -l)" \
            'action_20d_retry9_readiness_probe_failed_assertion_count=0' \
            'action_20d_retry9_readiness_probe_first_failure=none' \
            'action_20d_retry9_readiness_probe_notification_helper_invoked=false' \
            'action_20d_retry9_readiness_probe_filesystem_mutations=false' \
            'action_20d_retry9_readiness_probe_service_mutations=false' \
            'action_20d_retry9_readiness_probe_keepalived_mutations=false' \
            'action_20d_retry9_readiness_probe_vrrp_mutations=false' \
            'action_20d_retry9_readiness_probe_vip_mutations=false' \
            'action_20d_retry9_readiness_probe_network_mutations=false' \
            'action_20d_retry9_readiness_probe_persistent_mutations=false' \
            'action_20d_retry9_readiness_probe_remote_complete=true'
    } >"$fixture_path"
}
run_self_test() {
    local self_root
    ...
    printf '%s\n' \
        'action_20d_retry9_readiness_probe_assertion_state_directory_writable_as_pi=false' \
        >>"$self_root/node-a"
...
node_a_user=$(extract_one action_20d_retry9_readiness_probe_value_inherited_execution_user "$node_a_stdout" 2>/dev/null || true)
node_b_user=$(extract_one action_20d_retry9_readiness_probe_value_inherited_execution_user "$node_b_stdout" 2>/dev/null || true)
...
node_a_state_metadata=$(extract_one action_20d_retry9_readiness_probe_value_state_directory_metadata "$node_a_stdout" 2>/dev/null || true)
node_b_state_metadata=$(extract_one action_20d_retry9_readiness_probe_value_state_directory_metadata "$node_b_stdout" 2>/dev/null || true)
node_a_dedupe_metadata=$(extract_one action_20d_retry9_readiness_probe_value_dedupe_directory_metadata "$node_a_stdout" 2>/dev/null || true)
node_b_dedupe_metadata=$(extract_one action_20d_retry9_readiness_probe_value_dedupe_directory_metadata "$node_b_stdout" 2>/dev/null || true)
...
run_assertion node_a_caddy_ipv4_vip_absent require_one \
    'action_20d_retry9_readiness_probe_value_caddy_ipv4_vip_count=0' "$node_a_stdout"
run_assertion node_a_caddy_ipv6_vip_absent require_one \
    'action_20d_retry9_readiness_probe_value_caddy_ipv6_vip_count=0' "$node_a_stdout"
run_assertion node_b_caddy_ipv4_vip_absent require_one \
    'action_20d_retry9_readiness_probe_value_caddy_ipv4_vip_count=0' "$node_b_stdout"
run_assertion node_b_caddy_ipv6_vip_absent require_one \
    'action_20d_retry9_readiness_probe_value_caddy_ipv6_vip_count=0' "$node_b_stdout"
run_assertion node_a_dns_ipv4_vip_owner require_one \
    'action_20d_retry9_readiness_probe_value_dns_ipv4_vip_count=1' "$node_a_stdout"
run_assertion node_a_dns_ipv6_vip_owner require_one \
    'action_20d_retry9_readiness_probe_value_dns_ipv6_vip_count=1' "$node_a_stdout"
run_assertion node_b_dns_ipv4_vip_absent require_one \
    'action_20d_retry9_readiness_probe_value_dns_ipv4_vip_count=0' "$node_b_stdout"
run_assertion node_b_dns_ipv6_vip_absent require_one \
    'action_20d_retry9_readiness_probe_value_dns_ipv6_vip_count=0' "$node_b_stdout"
run_assertion node_a_notifier_not_invoked require_one \
    'action_20d_retry9_readiness_probe_notification_helper_invoked=false' "$node_a_stdout"
run_assertion node_b_notifier_not_invoked require_one \
    'action_20d_retry9_readiness_probe_notification_helper_invoked=false' "$node_b_stdout"
node_a_before_hash=$(extract_one action_20d_retry9_readiness_probe_value_before_snapshot_sha256 "$node_a_stdout" 2>/dev/null || true)
node_a_after_hash=$(extract_one action_20d_retry9_readiness_probe_value_after_snapshot_sha256 "$node_a_stdout" 2>/dev/null || true)
node_b_before_hash=$(extract_one action_20d_retry9_readiness_probe_value_before_snapshot_sha256 "$node_b_stdout" 2>/dev/null || true)
node_b_after_hash=$(extract_one action_20d_retry9_readiness_probe_value_after_snapshot_sha256 "$node_b_stdout" 2>/dev/null || true)
