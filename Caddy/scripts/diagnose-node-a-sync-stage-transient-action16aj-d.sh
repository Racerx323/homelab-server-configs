...
readonly diagnostic_stage=/var/tmp/caddy-sync-ssh-node-a-action16aj-d
...
if [[ "${1:-}" == --self-test ]]; then
    ...
    printf 'action_16aj_d_transient_diagnostic_self_test_complete=true\n'
    exit 0
elif (($#)); then
...
package_inventory() {
    dpkg-query -W \
        ...
        sort || printf 'package_inventory_unavailable\n'
}
...
preflight_fail() {
    ...
    printf 'action_16aj_d_preflight_valid=false\n'
    exit 1
}
...
printf 'action_16aj_d_remote_reached=true\n'
...
require_preflight_absent preflight_diagnostic_stage_absent "$diagnostic_stage"
staging_count=$(
...
if [[ "$staging_count" != 0 ]]; then
    preflight_fail preflight_action_staging_count_zero
fi
...
if [[ -z "$protected_before" ]]; then
    preflight_fail preflight_protected_state_capture
fi
printf 'preflight_protected_state_capture=true\n'
printf 'action_16aj_d_preflight_valid=true\n'
...
cleanup_on_exit() {
    ...
    [[ "$(protected_state)" == "$protected_before" ]] ||
        exit_cleanup_valid=false
    printf 'action_16aj_d_exit_cleanup_valid=%s\n' \
...
record_command failed_diagnostic_stage_still_absent \
    test ! -e "$failed_diagnostic_stage"
...
if [[ "$cleanup_valid" != true ]]; then
    printf 'manual_intervention_required=true\n' >&2
    exit 97
fi
printf 'action_16aj_d_transient_diagnostic_complete=true\n'
success=true
