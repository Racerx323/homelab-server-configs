...
readonly driver_name=apply-node-a-caddy-routing-correction-action16ar.sh
...
readonly -a success_markers=(
    action_16ar_remote_reached=true
    action_16ar_preflight_complete=true
    action_16ar_mutation_started=true
    ...
    action_16ar_routing_correction_complete=true
)
...
evaluate_success() {
    ...
    grep -Fxq 'previous_release=/etc/caddy/releases/bootstrap' "$output_file" ||
        return 97
    grep -Fxq \
        'selected_release=/etc/caddy/releases/action16ar-node-a-default-deny' \
    ...
    before_pid=$(numeric_value caddy_main_pid_before "$output_file")
    after_pid=$(numeric_value caddy_main_pid_after "$output_file")
    [[ "$before_pid" =~ ^[1-9][0-9]*$ ]] || return 97
    [[ "$after_pid" == "$before_pid" ]] || return 97
    if grep -Eq \
        'action_16ar_(rollback_complete|rollback_incomplete|rollback_not_required)=true|manual_intervention_required=true' \
...
evaluate_contract() {
    ...
    if [[ "$(grep -Fxc 'action_16ar_routing_correction_complete=true' \
        "$output_file")" -eq 1 ]]; then
        evaluate_success "$output_file" "$error_file" "$ssh_status"
        return
    fi
    [[ "$ssh_status" -ne 0 ]] || return 97
    if grep -Fxq 'action_16ar_rollback_incomplete=true' "$error_file" ||
        grep -Fxq 'manual_intervention_required=true' "$error_file"; then
        return 97
    fi
    if [[ "$(grep -Fxc 'action_16ar_rollback_complete=true' \
        "$error_file")" -eq 1 ]]; then
        return 1
    fi
    if [[ "$(grep -Fxc 'action_16ar_rollback_not_required=true' \
        "$error_file")" -eq 1 ]]; then
        return 1
    fi
    return 97
...
if [[ "${1:-}" == --self-test && $# -eq 1 ]]; then
    ...
    verify_artifacts
    printf 'action_16ar_routing_correction_runner_self_test_complete=true\n'
    exit 0
elif [[ "${1:-}" == --contract-test && $# -eq 1 ]]; then
    contract_dir=$(mktemp -d /tmp/caddy-action16ar-contract.XXXXXX)
    ...
    error_file="$contract_dir/error"
    {
        ...
        printf 'selected_release=/etc/caddy/releases/action16ar-node-a-default-deny\n'
    ...
    if evaluate_contract "$duplicate" "$error_file" 0; then
        printf 'Duplicate Action 16ar evidence was accepted.\n' >&2
        exit 1
    fi
    ...
    if evaluate_contract "$wrong_code" "$error_file" 0; then
        printf 'Incorrect Action 16ar rejection status was accepted.\n' >&2
        exit 1
    fi
    ...
    if evaluate_contract "$restarted" "$error_file" 0; then
        printf 'Action 16ar Caddy restart evidence was accepted.\n' >&2
        exit 1
    fi
    ...
    if evaluate_contract "$secret" "$error_file" 0; then
        printf 'Secret-bearing Action 16ar evidence was accepted.\n' >&2
        exit 1
    fi

    printf 'action_16ar_routing_correction_contract_test_complete=true\n'
    exit 0
elif (($#)); then
...
verify_artifacts
work_dir=$(mktemp -d /tmp/caddy-action16ar.XXXXXX)
readonly work_dir
...
finish() {
    ...
    if [[ -e "$work_dir" || -L "$work_dir" ]]; then
        printf 'action_16ar_local_cleanup_complete=false\n' >&2
        exit 97
    fi
    printf 'action_16ar_local_cleanup_complete=true\n'
    exit "$status"
}
