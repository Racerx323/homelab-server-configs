...
readonly regression_sha256=10bc4c15be20d846a73b869fdc5675b9b78f8db6750ed2210bab9cbba31a8b28
...
readonly runner="$script_directory/run-dual-node-caddy-vrrp-preactivation-action20a-retry.sh"
readonly regression="$caddy_root/tests/action20a-retry-dual-node-caddy-vrrp-preactivation-regression.sh"
...
run_local_gates() {
    ...
    require_quiet_bash_gate probe_self_test "$probe" --self-test || return 1
...
[[ -z "${CADDY_ACTION20ARETRY_INTERCEPTED_TEST:-}" ]]
work_directory=$(mktemp -d /tmp/caddy-action20a-retry-outer.XXXXXX)
