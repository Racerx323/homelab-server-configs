...
readonly expected_action19a_fragment_sha256=294d5ba44d903ba4ab3ab4330ae3ae05e0160e2b134d71e91a787b65dd96dc4d
...
expected_assertions() {
    printf '%s\n' \
        ...
        action20g_backup_count_one action20g_backup_directory_regular \
        action20g_backup_directory_not_symlink action20g_backup_directory_metadata_exact \
        action20g_backup_fragment_regular action20g_backup_fragment_not_symlink \
        action20g_backup_fragment_metadata_exact action20g_backup_fragment_hash_exact \
        action20g_backup_manifest_regular action20g_backup_manifest_not_symlink \
        action20g_backup_manifest_metadata_exact action20g_backup_manifest_line_count_exact \
        action20g_backup_manifest_action_exact action20g_backup_manifest_node_exact \
        action20g_backup_manifest_old_hash_exact action20g_backup_manifest_candidate_hash_exact \
        action20g_backup_manifest_main_hash_exact action20g_run_stage_count_zero \
        action20g_tmp_stage_count_zero action20g_install_stage_absent \
        ...
        dns_ipv4_vip_absent dns_ipv6_vip_absent \
...
safe_stream() {
    local action20ga_stream_path=$1

    [[ "$(wc -c <"$action20ga_stream_path")" -le "$maximum_stream_bytes" ]] || return 1
    [[ "$(line_count "$action20ga_stream_path")" -le "$maximum_stream_lines" ]] || return 1
    ! LC_ALL=C grep -n '[^[:print:][:space:]]' "$action20ga_stream_path" >/dev/null || return 1
    ! grep -Eqi \
        'BEGIN [A-Z ]*PRIVATE KEY|CADDY_TLS_PRIVATE_KEY_PEM|PRIVATE_KEY=|DOPPLER_TOKEN|Authorization:[[:space:]]*Bearer' \
        "$action20ga_stream_path" || return 1
}
emit_stream() {
    local action20ga_stream_label=$1
    local action20ga_stream_path=$2

    printf '%s_capture_%s_bytes=%s\n' "$prefix" "$action20ga_stream_label" \
        "$(wc -c <"$action20ga_stream_path")"
    printf '%s_capture_%s_lines=%s\n' "$prefix" "$action20ga_stream_label" \
        "$(line_count "$action20ga_stream_path")"
    printf '%s_capture_%s_sha256=%s\n' "$prefix" "$action20ga_stream_label" \
        "$(file_hash "$action20ga_stream_path")"
    printf '%s_capture_%s_classification=bounded_safe\n' "$prefix" "$action20ga_stream_label"
    if [[ ! -s "$action20ga_stream_path" ]]; then
        printf '%s_capture_%s_content_secured=empty\n' "$prefix" "$action20ga_stream_label"
        return 0
    fi
    printf '%s_capture_%s_begin\n' "$prefix" "$action20ga_stream_label"
    sed "s/^/${prefix}_capture_${action20ga_stream_label}_content=/" "$action20ga_stream_path"
    printf '%s_capture_%s_end\n' "$prefix" "$action20ga_stream_label"
}
run_probe() {
    local action20ga_probe_label=$1
    local action20ga_probe_root=$2
    local action20ga_probe_stdout=$action20ga_probe_root/$action20ga_probe_label.stdout
    local action20ga_probe_stderr=$action20ga_probe_root/$action20ga_probe_label.stderr
    local action20ga_probe_status=0

    shift 2
    : >"$action20ga_probe_stdout"
    : >"$action20ga_probe_stderr"
    chmod 0600 "$action20ga_probe_stdout" "$action20ga_probe_stderr"
    "$@" >"$action20ga_probe_stdout" 2>"$action20ga_probe_stderr" || action20ga_probe_status=$?
    safe_stream "$action20ga_probe_stdout" && safe_stream "$action20ga_probe_stderr" || return 97
    emit_stream "${action20ga_probe_label}_stdout" "$action20ga_probe_stdout"
    emit_stream "${action20ga_probe_label}_stderr" "$action20ga_probe_stderr"
    printf '%s_value_%s_status=%s\n' "$prefix" "$action20ga_probe_label" "$action20ga_probe_status"
    [[ "$action20ga_probe_status" -eq 0 ]]
}
...
case "${1:-}" in
    ...
    --self-test)
        [[ $# -eq 1 ]]
        expected_root=$(mktemp -d /tmp/caddy-action20g-a-expected.XXXXXX)
...
work_directory=$(mktemp -d /tmp/caddy-action20g-a-inspector.XXXXXX)
...
run_assertion keepalived_script_identity_exact test \
    ...
    'keepalived_script:x:992:988::/home/keepalived_script:/usr/sbin/nologin'
run_assertion keepalived_script_uid_exact test "$(id -u keepalived_script)" -eq 992
run_assertion caddy_tls_identity_exact test \
    "$(getent group caddy-tls)" = 'caddy-tls:x:990:caddy,caddy-sync,keepalived_script'
run_assertion caddy_tls_gid_exact test "$(getent group caddy-tls | cut -d: -f3)" -eq 990
run_assertion keepalived_caddy_tls_membership_exact test \
    "$(id -Gn keepalived_script | tr ' ' '\n' | grep -Fxc caddy-tls)" -eq 1

context_runtime_root=$(mktemp -d /tmp/caddy-action20g-a-context.XXXXXX)
readonly context_runtime_root
chown 992:990 "$context_runtime_root"
...
chown 992:990 "$context_runtime_root/home" "$context_runtime_root/config" \
    "$context_runtime_root/data"
run_assertion exact_context_runtime_metadata test \
    "$(stat -c '%u:%g:%a' "$context_runtime_root")" = 992:990:700
readonly -a exact_context=(setpriv --reuid 992 --regid 990 --clear-groups --)
...
run_assertion backup_manifest_candidate_hash_exact test \
    "$(grep -Fxc "fragment_candidate_sha256=$expected_action19a_fragment_sha256" "$backup_manifest" 2>/dev/null || true)" -eq 1
...
action20g_backup_count=$(find "$rollback_root" -mindepth 1 -maxdepth 1 -type d \
    -name 'action20g-node-b-health-group.*' -printf '.' 2>/dev/null | wc -c)
readonly action20g_backup_count
readonly action20g_backup_manifest=$expected_group_backup_path/manifest
readonly action20g_backup_fragment=$expected_group_backup_path/caddy-ha.conf
run_assertion action20g_backup_count_one test "$action20g_backup_count" -eq 1
run_assertion action20g_backup_directory_regular test -d "$expected_group_backup_path"
run_assertion action20g_backup_directory_not_symlink test ! -L "$expected_group_backup_path"
run_assertion action20g_backup_directory_metadata_exact test \
    "$(stat -c '%U:%G:%a' "$expected_group_backup_path" 2>/dev/null || true)" = root:root:700
run_assertion action20g_backup_fragment_regular test -f "$action20g_backup_fragment"
run_assertion action20g_backup_fragment_not_symlink test ! -L "$action20g_backup_fragment"
run_assertion action20g_backup_fragment_metadata_exact test \
    "$(stat -c '%U:%G:%a' "$action20g_backup_fragment" 2>/dev/null || true)" = root:root:600
run_assertion action20g_backup_fragment_hash_exact test \
    "$(file_hash "$action20g_backup_fragment" 2>/dev/null || true)" = \
    "$expected_action19a_fragment_sha256"
run_assertion action20g_backup_manifest_regular test -f "$action20g_backup_manifest"
run_assertion action20g_backup_manifest_not_symlink test ! -L "$action20g_backup_manifest"
run_assertion action20g_backup_manifest_metadata_exact test \
    "$(stat -c '%U:%G:%a' "$action20g_backup_manifest" 2>/dev/null || true)" = root:root:600
run_assertion action20g_backup_manifest_line_count_exact test \
    "$(awk 'END { print NR }' "$action20g_backup_manifest" 2>/dev/null || true)" -eq 5
run_assertion action20g_backup_manifest_action_exact test \
    "$(grep -Fxc 'action=action20g' "$action20g_backup_manifest" 2>/dev/null || true)" -eq 1
run_assertion action20g_backup_manifest_node_exact test \
    "$(grep -Fxc 'node=node-b' "$action20g_backup_manifest" 2>/dev/null || true)" -eq 1
run_assertion action20g_backup_manifest_old_hash_exact test \
    "$(grep -Fxc "old_fragment_sha256=$expected_action19a_fragment_sha256" "$action20g_backup_manifest" 2>/dev/null || true)" -eq 1
run_assertion action20g_backup_manifest_candidate_hash_exact test \
    "$(grep -Fxc "candidate_fragment_sha256=$expected_fragment_sha256" "$action20g_backup_manifest" 2>/dev/null || true)" -eq 1
run_assertion action20g_backup_manifest_main_hash_exact test \
    "$(grep -Fxc "main_sha256=$expected_main_sha256" "$action20g_backup_manifest" 2>/dev/null || true)" -eq 1
action20g_run_stage_count=$(find /run -mindepth 1 -maxdepth 1 -type d \
    -name 'caddy-action20g-stage.*' -printf '.' 2>/dev/null | wc -c)
readonly action20g_run_stage_count
action20g_tmp_stage_count=$(find /tmp -mindepth 1 -maxdepth 1 -type d \
    -name 'caddy-action20g-*' ! -path "$work_directory" -printf '.' 2>/dev/null | wc -c)
readonly action20g_tmp_stage_count
run_assertion action20g_run_stage_count_zero test "$action20g_run_stage_count" -eq 0
run_assertion action20g_tmp_stage_count_zero test "$action20g_tmp_stage_count" -eq 0
run_assertion action20g_install_stage_absent test -z \
    "$(find /etc/keepalived/conf.d -mindepth 1 -maxdepth 1 \
        -name '.caddy-ha.action20g.*' -print -quit 2>/dev/null)"
...
run_assertion dns_ipv4_vip_absent test "$dns_ipv4_vip_count" -eq 0
run_assertion dns_ipv6_vip_absent test "$dns_ipv6_vip_count" -eq 0
...
printf '%s_exact_context_uid=992\n' "$prefix"
printf '%s_exact_context_gid=990\n' "$prefix"
