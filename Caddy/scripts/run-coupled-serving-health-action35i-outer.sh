...
readonly prefix=action_35_i_outer
readonly transaction_sha256=658a53d92cbccd50af9d1f906e513fe2e55ead60b79fb749c76ec4036e5924d0
...
safe_capture() {
    local action35i_path=$1

    regular_file "$action35i_path"
    [[ "$(stat -c '%s' "$action35i_path")" -le "$max_stream_bytes" ]]
    iconv -f UTF-8 -t UTF-8 "$action35i_path" >/dev/null
    ! LC_ALL=C grep -Paq '[\x00-\x08\x0b\x0c\x0e-\x1f\x7f]' "$action35i_path"
}

capture() {
    local action35i_label=$1
    shift
    local action35i_stdout=$workstation_evidence/$action35i_label.stdout
    local action35i_stderr=$workstation_evidence/$action35i_label.stderr
    local action35i_status=$workstation_evidence/$action35i_label.status
    ...
    safe_capture "$action35i_stdout"
    safe_capture "$action35i_stderr"
    return "$action35i_rc"
}

build_payload() {
    local action35i_repository action35i_source action35i_target action35i_mode
    local action35i_hash action35i_lifecycle action35i_source_path action35i_destination
    ...
    while IFS=$'\t' read -r action35i_repository action35i_source action35i_target \
        action35i_mode action35i_hash action35i_lifecycle; do
        [[ "$action35i_repository" = '# repository' ]] && continue
        [[ "$action35i_mode" =~ ^0[0-7]{3}$ && "$action35i_lifecycle" = production-candidate ]]
        [[ "$action35i_target" = /* ]]
        action35i_source_path=${workspace_root}/$action35i_repository/$action35i_source
        regular_file "$action35i_source_path"
        [[ "$(sha256sum "$action35i_source_path" | awk '{ print $1 }')" = "$action35i_hash" ]]
        action35i_destination=$payload_stage/repositories/$action35i_repository/$action35i_source
        install -d -m 0700 "$(dirname -- "$action35i_destination")"
        install -m 0600 "$action35i_source_path" "$action35i_destination"
...
readonly archive=$2
case "$remote_root" in /tmp/caddy-action35i-*) ;; *) exit 64 ;; esac
case "$archive" in /tmp/caddy-action35i-*.tar) ;; *) exit 64 ;; esac
...
case "$evidence_root" in /tmp/caddy-action35i-*) ;; *) exit 64 ;; esac
...
ssh_stream() {
    local action35i_host=$1
    local action35i_program=$2
    shift 2
    "$ssh_command" "$action35i_host" \
        "cd / && sudo -n /bin/bash -s --$(printf ' %q' "$@")" <"$action35i_program"
}

upload_payload() {
    local action35i_role=$1
    local action35i_host=$2
    local action35i_remote_root=$3
    local action35i_remote_archive=$4
    local action35i_hash

    action35i_hash=$(awk '{ print $1 }' "$workstation_evidence/payload.sha256")
    capture "$action35i_role-upload-prepare" ssh_stream "$action35i_host" \
        "$prepare_program" "$action35i_remote_root" "$action35i_remote_archive"
    capture "$action35i_role-upload-copy" "$scp_command" -p -- \
        "$payload_archive" "$action35i_host:$action35i_remote_archive"
    capture "$action35i_role-upload-accept" ssh_stream "$action35i_host" \
        "$accept_program" "$action35i_remote_root" "$action35i_remote_archive" \
        "$action35i_hash"
}
...
remote_transaction() {
    ...
    capture "$action35i_label" ssh_stream "$action35i_host" "$transaction" "$@"
}

readback() {
    local action35i_role=$1
    local action35i_host=$2
    local action35i_remote_evidence=$3

    capture "$action35i_role-readback" ssh_stream "$action35i_host" \
        "$readback_program" "$action35i_remote_evidence"
}
...
cleanup_remote() {
    ...
    capture "$action35i_role-disposition" ssh_stream "$action35i_host" \
        "$disposition_program" "$action35i_remote_root" "$action35i_remote_archive"
}

run_live() {
    local action35i_node_b_mutated=false
    local action35i_node_a_mutated=false
    local action35i_failure=0
    local action35i_phase_status=0
    ...
    action35i_node_b_mutated=true
    if [[ "$action35i_failure" -eq 0 ]]; then
    ...
    if ! remote_transaction node-a-journal-capture "$node_a_host" journal-capture node-a \
        "$node_a_payload" "$node_a_evidence"; then
        [[ "$action35i_failure" -ne 0 ]] || action35i_failure=1
    fi
    ...
    if [[ "$action35i_failure" -ne 0 ]]; then
        if [[ "$action35i_node_a_mutated" = true ]]; then
            remote_transaction node-a-rollback "$node_a_host" rollback node-a \
                "$node_a_payload" "$node_a_evidence" || exit 125
        fi
        if [[ "$action35i_node_b_mutated" = true ]]; then
...
production_path_test() {
    ...
    local action35i_remote_base action35i_raw action35i_decision action35i_observed
    local action35i_status action35i_role action35i_remote_evidence action35i_test_id
    ...
    action35i_remote_base=$(mktemp -d /tmp/caddy-action35i-production-path.XXXXXX)
    action35i_test_id=${action35i_remote_base##*.}
    test_remote_base=$action35i_remote_base
    export ACTION35I_TEST_REMOTE_BASE=$test_remote_base
...
[[ "$1" = -n ]]
shift
export ACTION35I_PRODUCTION_PATH_TEST=1
exec "$@"
SUDO
...
set -Eeuo pipefail
[[ "$1" = -u && "$3" = -- ]]
printf '%s\n' "$2" >>"$ACTION35I_TEST_REMOTE_BASE/runuser.identities"
shift 3
exec "$@"
RUNUSER
...
set -Eeuo pipefail
case " $* " in
    *' --show-cursor '*) printf '%s\n' '-- cursor: s=action35i-production-path' ;;
    *' --after-cursor '*) printf '%s\n' 'test-host caddy.service: action35i production-path event' ;;
    *) exit 64 ;;
esac
JOURNALCTL
    cat >"$ssh_command" <<'SSH'
...
shift
command="$*"
printf '%s\t%s\n' "$host" "$command" >>"$ACTION35I_TEST_REMOTE_BASE/ssh.calls"
PATH="$ACTION35I_TEST_REMOTE_BASE/bin:/usr/bin:/bin" /bin/bash -c "$command"
SSH
    cat >"$scp_command" <<'SCP'
#!/usr/bin/env bash
set -Eeuo pipefail
...
source=$3
target=${4#*:}
printf '%s\t%s\n' "$source" "$target" >>"$ACTION35I_TEST_REMOTE_BASE/scp.calls"
...
SCP
    ...
    node_a_payload=/tmp/caddy-action35i-test-$action35i_test_id-node-a
    node_b_payload=/tmp/caddy-action35i-test-$action35i_test_id-node-b
    ...
    ACTION35I_ENVIRONMENT_FILE=$test_remote_base/environment-node-a \
        ACTION35I_KEEPALIVED_COMMAND=$test_remote_base/bin/keepalived \
        ACTION35I_RUNUSER_COMMAND=$test_remote_base/bin/runuser \
        ACTION35I_SYSTEMCTL_COMMAND=$test_remote_base/bin/systemctl \
        ACTION35I_DNS_DIG_COMMAND=$test_remote_base/bin/dig \
        ACTION35I_CURL_COMMAND=$test_remote_base/bin/curl \
        ACTION35I_SS_COMMAND=$test_remote_base/bin/ss \
        remote_transaction node-a-candidate-producer "$node_a_host" candidate-check \
        node-a "$node_a_payload" "$node_a_evidence"
    ACTION35I_ENVIRONMENT_FILE=$test_remote_base/environment-node-b \
        ACTION35I_KEEPALIVED_COMMAND=$test_remote_base/bin/keepalived \
        ACTION35I_RUNUSER_COMMAND=$test_remote_base/bin/runuser \
        ACTION35I_SYSTEMCTL_COMMAND=$test_remote_base/bin/systemctl \
        ACTION35I_DNS_DIG_COMMAND=$test_remote_base/bin/dig \
    ...
    ACTION35I_JOURNALCTL_COMMAND=$test_remote_base/bin/journalctl \
    ...
    ACTION35I_TARGET_ROOT=$test_remote_base/node-b-root \
        ACTION35I_SYSTEMCTL_COMMAND=$test_remote_base/bin/systemctl \
        remote_transaction node-b-install-producer "$node_b_host" install node-b \
        "$node_b_payload" "$node_b_evidence"
    ACTION35I_TARGET_ROOT=$test_remote_base/node-a-root \
        ACTION35I_SYSTEMCTL_COMMAND=$test_remote_base/bin/systemctl \
    ...
    action35i_raw=$action35i_test_root/raw/outer-preflight.txt
    action35i_decision=$action35i_test_root/decisions/outer-preflight.tsv
    cat "$test_remote_base/ssh.calls" "$test_remote_base/scp.calls" \
        "$test_remote_base/runuser.identities" >"$action35i_raw"
    action35i_observed=$(wc -l <"$action35i_raw")
    write_decision outer-preflight reach 0 "$action35i_observed" \
        "$action35i_observed" "$action35i_raw" "$action35i_decision"

    for action35i_role in node-a node-b; do
        if [[ "$action35i_role" = node-a ]]; then
            action35i_remote_evidence=$node_a_evidence
        else
            action35i_remote_evidence=$node_b_evidence
        fi
        action35i_raw=$action35i_test_root/raw/evidence-readback-$action35i_role-success.txt
        action35i_decision=$action35i_test_root/decisions/evidence-readback-$action35i_role-success.tsv
        ssh_stream "test-$action35i_role" "$readback_program" \
            "$action35i_remote_evidence" >"$action35i_raw"
        action35i_observed=$(sha256sum "$action35i_raw" | awk '{ print $1 }')
        write_decision "evidence-readback-$action35i_role-success" accept 0 \
            "$action35i_observed" "$action35i_observed" "$action35i_raw" "$action35i_decision"

        action35i_raw=$action35i_test_root/raw/evidence-readback-$action35i_role-failure.txt
        action35i_decision=$action35i_test_root/decisions/evidence-readback-$action35i_role-failure.tsv
        if ssh_stream "test-$action35i_role" "$readback_program" \
            "$action35i_remote_evidence-absent" >"$action35i_raw" 2>&1; then
            action35i_status=0
        else
            action35i_status=$?
        fi
        write_decision "evidence-readback-$action35i_role-failure" reject \
            "$action35i_status" present absent "$action35i_raw" "$action35i_decision"
    done
    cleanup_remote node-a "$node_a_host" "$node_a_payload" "$node_a_archive"
    cleanup_remote node-b "$node_b_host" "$node_b_payload" "$node_b_archive"
    rm -rf -- "$action35i_remote_base"
...
transaction=$repository_root/Caddy/scripts/apply-coupled-serving-health-action35i.sh
...
if [[ "$invocation_mode" = --production-path-test ]]; then
    workstation_evidence=$(mktemp -d /tmp/caddy-action35i-outer-test.XXXXXX)
else
    workstation_evidence=$(mktemp -d /tmp/caddy-ssh-evidence-action35i.XXXXXX)
fi
readonly workstation_evidence
chmod 0700 "$workstation_evidence"
payload_stage=$workstation_evidence/payload
payload_archive=$workstation_evidence/action35i-payload.tar
...
readonly run_id
node_a_payload=/tmp/caddy-action35i-$run_id-node-a
node_b_payload=/tmp/caddy-action35i-$run_id-node-b
node_a_archive=$node_a_payload.tar
node_b_archive=$node_b_payload.tar
node_a_evidence=$node_a_payload/evidence
node_b_evidence=$node_b_payload/evidence
