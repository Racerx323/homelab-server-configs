...
validate_fragment_parser() {
    ...
    parser_root=$(mktemp -d /tmp/caddy-action20c-a-parser.XXXXXX) || return 1
...
case "${1:-}" in
    ...
    --self-test)
        [[ $# -eq 1 ]]
        expected_root=$(mktemp -d /tmp/caddy-action20c-a-expected.XXXXXX)
...
work_directory=$(mktemp -d /tmp/caddy-action20c-a-inspector.XXXXXX)
