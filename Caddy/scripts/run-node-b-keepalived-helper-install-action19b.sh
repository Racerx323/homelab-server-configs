...
verify_sources() {
    ...
    "$collision_checker" "$health_source" "$notification_source" \
        "$inspector" "$installer" >/dev/null || return 1
...
validate_success() {
    ...
    [[ "$(grep -Ec \
        "^${prefix}_backup_path=/var/backups/caddy-ha/action19b-node-b-keepalived-helpers\.[A-Za-z0-9]+$" \
...
write_remote_bundle() {
    ...
    # shellcheck disable=SC1003,SC2016
    {
        printf '%s\n' \
            ...
            'export PATH' \
            'cd /' \
            'bundle_stage=$(mktemp -d /run/caddy-action19b-stage.XXXXXX)' \
        ...
        printf '%s\n' \
            ...
            'cd /' \
            '/bin/bash \' \
            '    "$bundle_stage/payload/install-node-b-keepalived-helpers-action19b.sh" \' \
...
case "${1:-}" in
    ...
    --contract-test)
        [[ $# -eq 1 ]] || exit 64
        verify_sources
        contract_root=$(mktemp -d /tmp/caddy-action19b-contract.XXXXXX)
...
verify_sources
working_directory_approved
work_directory=$(mktemp -d /tmp/caddy-action19b-runner.XXXXXX)
...
tar --create --file "$archive_path" --directory "$script_directory" \
    ...
    install-node-b-keepalived-helpers-action19b.sh
