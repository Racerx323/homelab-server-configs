...
readonly prefix=action_20o_retry2
...
address_count() {
    ...
    local action20o_address_output

    action20o_address_output=$(ip -o "-$action20o_address_family" address show dev eth0) || return 1
    printf '%s\n' "$action20o_address_output" |
        awk -v address="$action20o_address_cidr" '$4 == address { count++ } END { print count + 0 }'
}
address_absent() {
    local action20o_address_family=$1
    local action20o_address_cidr=$2
    local action20o_observed_count

    action20o_observed_count=$(address_count "$action20o_address_family" "$action20o_address_cidr") || return 1
    [[ "$action20o_observed_count" -eq 0 ]]
}
global_defs_contains_enable_dbus() {
    local action20o_configuration_path=$1
    ...
    ' "$action20o_configuration_path"
}
build_candidate() {
    local action20o_source_path=$1
    local action20o_candidate_path=$2

    {
        cat "$action20o_source_path"
        printf '\ninclude /etc/keepalived/conf.d/caddy-ha.conf\n'
    } >"$action20o_candidate_path"
}
expected_checks() {
    printf '%s\n' \
        identity_root working_directory_root hostname_exact \
        main_regular main_not_symlink main_metadata main_hash_exact \
        main_enable_dbus_absent main_include_once \
        ...
        source_stage_directory source_regular source_not_symlink source_metadata \
        source_hash_exact source_terminal_newline source_global_defs_once \
        source_enable_dbus_once source_enable_dbus_in_global_defs \
        source_dbus_service_name_absent source_include_absent candidate_regular \
        candidate_metadata candidate_hash_exact candidate_global_defs_once \
        candidate_enable_dbus_once candidate_enable_dbus_in_global_defs \
        candidate_dbus_service_name_absent candidate_include_once candidate_include_terminal \
        ...
        caddy_vip_https_before journal_cursor_captured candidate_stage_created \
        candidate_installed candidate_renamed reload_status \
        ...
        main_hash_candidate_after fragment_hash_unchanged health_hash_unchanged \
...
wait_for_runtime() {
    for _ in $(seq 1 20); do
        if systemctl is-active --quiet keepalived.service &&
            ...
            address_absent 4 "$caddy_ipv4_cidr" &&
            address_absent 6 "$caddy_ipv6_cidr" &&
...
dbus_object_present() {
    local action20o_tree_path=$1
    local action20o_object_path=$2

    grep -Fqx "$action20o_object_path" "$action20o_tree_path"
}
rollback_transaction() {
    ...
    for _ in $(seq 1 20); do
        if systemctl is-active --quiet keepalived.service &&
            [[ "$(sed -n '1p' /run/caddy-ha/vrrp-state 2>/dev/null || true)" = BACKUP ]] &&
            address_absent 4 "$caddy_ipv4_cidr" &&
            address_absent 6 "$caddy_ipv6_cidr"; then
            break
        fi
        sleep 1
    done
    ...
    address_absent 4 "$caddy_ipv4_cidr" || action20o_rollback_ok=false
    address_absent 6 "$caddy_ipv6_cidr" || action20o_rollback_ok=false
    address_absent 4 "$dns_ipv4_cidr" || action20o_rollback_ok=false
    address_absent 6 "$dns_ipv6_cidr" || action20o_rollback_ok=false
...
self_test() {
    [[ "$(expected_checks | wc -l)" -eq 99 ]] || return 1
    [[ "$(expected_checks | LC_ALL=C sort -u | wc -l)" -eq 99 ]] || return 1
    ...
    dbus_object_present <(printf '%s\n' "$dbus_ipv4_object" "$dbus_ipv6_object") "$dbus_ipv4_object" || return 1
    if dbus_object_present <(printf '└─%s\n' "$dbus_ipv4_object") "$dbus_ipv4_object"; then return 1; fi
}
...
case "${1:-}" in
    ...
    --stage)
        [[ $# -eq 2 && "$2" = /* ]] || exit 64
        source_stage=$2
        source_configuration=$source_stage/keepalived-pihole00.conf
        ;;
    *) exit 64 ;;
esac

transaction_root=$(mktemp -d /run/caddy-action20o-retry2.XXXXXX)
...
record_check main_enable_dbus_absent test "$(grep -Ec '^[[:space:]]*enable_dbus([[:space:]]|$)' "$main_configuration" 2>/dev/null || true)" -eq 0 || exit 1
...
record_check source_stage_directory test "$(stat -c '%U:%G:%a' "$source_stage" 2>/dev/null || true)" = root:root:700 || exit 1
...
record_check source_hash_exact test "$(file_hash "$source_configuration" 2>/dev/null || true)" = "$expected_source_sha256" || exit 1
record_check source_terminal_newline test "$(tail -c 1 "$source_configuration" | od -An -tuC | tr -d ' ')" = 10 || exit 1
record_check source_global_defs_once test "$(grep -Ec '^[[:space:]]*global_defs[[:space:]]*\{' "$source_configuration")" -eq 1 || exit 1
record_check source_enable_dbus_once test "$(grep -Ec '^[[:space:]]*enable_dbus([[:space:]]|$)' "$source_configuration")" -eq 1 || exit 1
record_check source_enable_dbus_in_global_defs global_defs_contains_enable_dbus "$source_configuration" || exit 1
record_check source_dbus_service_name_absent test "$(grep -Ec '^[[:space:]]*dbus_service_name([[:space:]]|$)' "$source_configuration" || true)" -eq 0 || exit 1
record_check source_include_absent test "$(grep -Fxc 'include /etc/keepalived/conf.d/caddy-ha.conf' "$source_configuration" || true)" -eq 0 || exit 1
build_candidate "$source_configuration" "$candidate_configuration"
chmod 0600 "$candidate_configuration"
record_check candidate_regular test -f "$candidate_configuration" || exit 1
record_check candidate_metadata test "$(stat -c '%U:%G:%a' "$candidate_configuration")" = root:root:600 || exit 1
record_check candidate_hash_exact test "$(file_hash "$candidate_configuration")" = "$expected_candidate_sha256" || exit 1
record_check candidate_global_defs_once test "$(grep -Ec '^[[:space:]]*global_defs[[:space:]]*\{' "$candidate_configuration")" -eq 1 || exit 1
record_check candidate_enable_dbus_once test "$(grep -Ec '^[[:space:]]*enable_dbus([[:space:]]|$)' "$candidate_configuration")" -eq 1 || exit 1
record_check candidate_enable_dbus_in_global_defs global_defs_contains_enable_dbus "$candidate_configuration" || exit 1
record_check candidate_dbus_service_name_absent test "$(grep -Ec '^[[:space:]]*dbus_service_name([[:space:]]|$)' "$candidate_configuration" || true)" -eq 0 || exit 1
record_check candidate_include_once test "$(grep -Fxc 'include /etc/keepalived/conf.d/caddy-ha.conf' "$candidate_configuration")" -eq 1 || exit 1
record_check candidate_include_terminal test "$(tail -n 1 "$candidate_configuration")" = 'include /etc/keepalived/conf.d/caddy-ha.conf' || exit 1
...
record_check rollback_manifest_candidate grep -Fqx "candidate_sha256=$expected_candidate_sha256" "$rollback_manifest" || exit 1
...
record_check caddy_ipv4_absent_before address_absent 4 "$caddy_ipv4_cidr" || exit 1
record_check caddy_ipv6_absent_before address_absent 6 "$caddy_ipv6_cidr" || exit 1
record_check dns_ipv4_absent_before address_absent 4 "$dns_ipv4_cidr" || exit 1
record_check dns_ipv6_absent_before address_absent 6 "$dns_ipv6_cidr" || exit 1
...
install_stage=$(mktemp "$(dirname "$main_configuration")/.keepalived.conf.action20o-retry2.XXXXXX")
readonly install_stage
record_check candidate_stage_created test -f "$install_stage" || exit 1
record_check candidate_installed run_captured candidate_install install -o root -g root -m 0644 "$candidate_configuration" "$install_stage" || exit 1
record_check candidate_renamed run_captured candidate_rename mv -fT "$install_stage" "$main_configuration" || exit 1
...
record_check dbus_tree_ipv4_object dbus_object_present "$transaction_root/dbus_tree.stdout" "$dbus_ipv4_object" || exit 1
record_check dbus_tree_ipv6_object dbus_object_present "$transaction_root/dbus_tree.stdout" "$dbus_ipv6_object" || exit 1
...
record_check caddy_ipv4_absent_after address_absent 4 "$caddy_ipv4_cidr" || exit 1
record_check caddy_ipv6_absent_after address_absent 6 "$caddy_ipv6_cidr" || exit 1
record_check dns_ipv4_absent_after address_absent 4 "$dns_ipv4_cidr" || exit 1
record_check dns_ipv6_absent_after address_absent 6 "$dns_ipv6_cidr" || exit 1
record_check main_hash_candidate_after test "$(file_hash "$main_configuration")" = "$expected_candidate_sha256" || exit 1
...
printf '%s_filesystem_mutation=true\n' "$prefix"
