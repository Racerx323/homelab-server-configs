...
readonly runner="$script_directory/run-node-a-caddy-health-action20d-retry7-a-retry.sh"
readonly regression="$caddy_root/tests/action20d-retry7-a-retry-ancestry-regression.sh"
...
source_exact() {
    ...
    [[ "$(file_hash "$outer_source_path")" = "$outer_expected_hash" ]]
}
# Invoked indirectly through require_gate.
# shellcheck disable=SC2317
...
run_local_gates() {
    ...
    require_gate collision_policy /bin/bash "$collision_checker" "$diagnostic" "$runner" "$regression" || return 1
    ...
    require_gate ancestry_regression /bin/bash "$regression" || return 1
...
case "${1:-}" in
    --expected-local-gates)
        ...
        printf '%s\n' \
            ...
            transaction_output_evidence_policy diagnostic_self_test \
            runner_self_test runner_contract_test ancestry_regression \
            working_directory
        exit 0
        ;;
...
run_local_gates
[[ -z "${CADDY_ACTION20D_RETRY7_A_RETRY_SSH_BINARY:-}" ]]
work_directory=$(mktemp -d /tmp/caddy-action20d-retry7-a-retry-outer.XXXXXX)
