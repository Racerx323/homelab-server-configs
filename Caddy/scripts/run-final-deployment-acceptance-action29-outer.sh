...
readonly prefix=action_29_outer
readonly schema_version=action29-final-acceptance-v1
...
readonly inspector=$script_directory/inspect-final-deployment-action29.sh
readonly dns_probe=$script_directory/run-dual-node-dns-record-families-action24-retry-outer.sh
readonly web_probe=$script_directory/run-dual-node-pihole-web-access-action25-retry2-outer.sh
...
readonly regression=$caddy_root/tests/action29-final-deployment-acceptance-regression.sh
readonly manifest=$caddy_root/manifests/caddy-final-deployment-acceptance-action29.yaml
readonly expected_inspector_sha256=1e21e9366b7531c9ccc218d378b91b7288233fbe6589b9f3aa3967dd53d84d86
readonly expected_dns_probe_sha256=daaa1904cab02dbf9a83aa6f8d4479582d6d571bc3fd008f4cd1393878fdc6f6
readonly expected_web_probe_sha256=e9eb4e88f02939778e42f7da3fc10135bdc3023e1cef675d875f59a1c27dd2af
...
readonly expected_regression_sha256=b2e312c56b09aae7d4d3ff5b8a04348a1ca8a89d9e581df25083fc4b458f5a48
evidence_root=${CADDY_ACTION29_EVIDENCE_ROOT:-/tmp/caddy-ssh-evidence/action29}
readonly evidence_root
ssh_binary=${CADDY_ACTION29_SSH_BIN:-/usr/bin/ssh}
readonly ssh_binary
run_id=
work_root=
self_test_root=
...
validate_node_transcript() {
    ...
    while IFS= read -r action29_outer_label; do
        require_one "action_29_remote_${action29_outer_token}_check_${action29_outer_label}=true" "$action29_outer_stdout" || return 1
    done < <(/bin/bash "$inspector" --expected-checks "$action29_outer_role")
    ! grep -Eq '^action_29_remote_.*_check_.*=false$' "$action29_outer_stdout" || return 1
    require_one "action_29_remote_${action29_outer_token}_schema=action29-node-snapshot-v1" "$action29_outer_stdout" || return 1
    require_one "action_29_remote_${action29_outer_token}_first_failure=none" "$action29_outer_stdout" || return 1
    require_one "action_29_remote_${action29_outer_token}_read_only=true" "$action29_outer_stdout" || return 1
    require_one "action_29_remote_${action29_outer_token}_complete=true" "$action29_outer_stdout" || return 1
    # conditional-validator-explicit-failures-end
}
component_marker() {
    case "$1" in
        dns) printf '%s\n' action_24_retry_outer_complete=true ;;
        web) printf '%s\n' action_25_retry2_outer_complete=true ;;
...
run_local_gates() {
    ...
    gate web_probe_hash test "$(file_hash "$web_probe")" = "$expected_web_probe_sha256" || return 1
...
run_action() {
    ...
    run_component web "$web_probe" || return 1
