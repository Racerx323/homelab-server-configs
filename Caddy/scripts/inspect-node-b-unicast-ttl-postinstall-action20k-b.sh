...
readonly source_fragment_sha256=7b42e1296bd06768b96e1097fe1e6546fcdc9fb4e249eaa7b0bbaffa85b3b270
...
readonly backup_directory=/var/backups/caddy-ha/action20k-node-b-unicast-ttl.JcpMAl
...
action20kb_failed_check_count=0
action20kb_first_failure=none

file_hash() { sha256sum "$1" | awk '{ print $1 }'; }
address_count() {
    local action20kb_address_family=$1
    local action20kb_expected_cidr=$2

    ip -o "-$action20kb_address_family" address show dev eth0 2>/dev/null |
        awk -v expected="$action20kb_expected_cidr" '$4 == expected { count++ }
            END { print count + 0 }'
}
expected_checks() {
    printf '%s\n' \
        ...
        vrrp_state_backup_before caddy_ipv4_count_before \
        ...
        vrrp_state_backup_after caddy_ipv4_count_after caddy_ipv6_count_after \
...
record_check() {
    local action20kb_check_label=$1

    shift
    if "$@"; then
        printf '%s_check_%s=true\n' "$prefix" "$action20kb_check_label"
    else
        printf '%s_check_%s=false\n' "$prefix" "$action20kb_check_label"
        action20kb_failed_check_count=$((action20kb_failed_check_count + 1))
        if [[ "$action20kb_first_failure" = none ]]; then
            action20kb_first_failure=$action20kb_check_label
        fi
    fi
    return 0
}
...
matching_path_count() {
    local action20kb_parent=$1
    local action20kb_pattern=$2

    find "$action20kb_parent" -mindepth 1 -maxdepth 1 -name "$action20kb_pattern" \
...
self_test() {
    local action20kb_fixture_root
    local action20kb_fixture_fragment
    ...
    action20kb_fixture_root=$(mktemp -d /tmp/caddy-action20k-a-self-test.XXXXXX) || return 1
    trap 'rm -rf -- "$action20kb_fixture_root"' RETURN
    action20kb_fixture_fragment=$action20kb_fixture_root/fragment
    printf '%s\n' \
        ...
        '    unicast_ttl 255' \
        '}' >"$action20kb_fixture_fragment" || return 1
    [[ "$(grep -Fxc '    unicast_ttl 255' "$action20kb_fixture_fragment")" -eq 2 ]] || return 1
...
action20kb_keepalived_pid_before=$(systemctl show keepalived.service --property MainPID --value 2>/dev/null || true)
readonly action20kb_keepalived_pid_before
action20kb_keepalived_restarts_before=$(systemctl show keepalived.service --property NRestarts --value 2>/dev/null || true)
readonly action20kb_keepalived_restarts_before
action20kb_state_before=$(state_snapshot | sha256sum | awk '{ print $1 }')
readonly action20kb_state_before
...
record_check backup_count_exact test "$(matching_path_count /var/backups/caddy-ha 'action20k-node-b-unicast-ttl.*')" -eq 1
...
record_check keepalived_pid_numeric_before test "$action20kb_keepalived_pid_before" -gt 0
record_check keepalived_restarts_numeric_before test "$action20kb_keepalived_restarts_before" -ge 0
...
record_check keepalived_pid_unchanged test "$(systemctl show keepalived.service --property MainPID --value 2>/dev/null || true)" = "$action20kb_keepalived_pid_before"
record_check keepalived_restarts_unchanged test "$(systemctl show keepalived.service --property NRestarts --value 2>/dev/null || true)" = "$action20kb_keepalived_restarts_before"
...
action20kb_state_after=$(state_snapshot | sha256sum | awk '{ print $1 }')
readonly action20kb_state_after
record_check state_snapshot_unchanged test "$action20kb_state_after" = "$action20kb_state_before"
...
printf '%s_value_before_state_sha256=%s\n' "$prefix" "$action20kb_state_before"
printf '%s_value_after_state_sha256=%s\n' "$prefix" "$action20kb_state_after"
printf '%s_check_count=%s\n' "$prefix" "$expected_check_count"
printf '%s_failed_check_count=%s\n' "$prefix" "$action20kb_failed_check_count"
printf '%s_first_failure=%s\n' "$prefix" "$action20kb_first_failure"
...
[[ "$action20kb_failed_check_count" -eq 0 ]]
