...
pass_check() {
    local action23a_pass_label=$1

    if [[ -n "${seen_checks[$action23a_pass_label]+set}" ]]; then
        printf 'action_23a_duplicate_check=%s\n' "$action23a_pass_label" >&2
        return 1
    fi
    seen_checks[$action23a_pass_label]=true
    printf 'action_23a_check_%s=true\n' "$action23a_pass_label"
}

fail_check() {
    local action23a_fail_label=$1
    local action23a_fail_observed=${2:-unavailable}

    printf 'action_23a_check_%s=false\n' "$action23a_fail_label" >&2
    printf 'action_23a_failed_check=%s\n' "$action23a_fail_label" >&2
    printf 'action_23a_failed_observed=%s\n' "$action23a_fail_observed" >&2
    return 1
}

assert_equal() {
    local action23a_equal_label=$1
    local action23a_equal_observed=$2
    local action23a_equal_expected=$3

    if [[ "$action23a_equal_observed" == "$action23a_equal_expected" ]]; then
        pass_check "$action23a_equal_label"
    else
        fail_check "$action23a_equal_label" "$action23a_equal_observed"
    fi
}

assert_file_shape() {
    local action23a_shape_label=$1
    local action23a_shape_path=$2
    local action23a_shape_regular
    local action23a_shape_not_symlink

    action23a_shape_regular=$(
        if [[ -f "$action23a_shape_path" ]]; then printf true; else printf false; fi
    )
    action23a_shape_not_symlink=$(
        if [[ ! -L "$action23a_shape_path" ]]; then printf true; else printf false; fi
    )

    # conditional-validator-explicit-failures-begin
    assert_equal "${action23a_shape_label}_regular" \
        "$action23a_shape_regular" true || return 1
    assert_equal "${action23a_shape_label}_not_symlink" \
        "$action23a_shape_not_symlink" true || return 1
    # conditional-validator-explicit-failures-end
}

assert_path_absent() {
    local action23a_absent_label=$1
    local action23a_absent_path=$2
    local action23a_entry_absent
    local action23a_symlink_absent

    action23a_entry_absent=$(
        if [[ ! -e "$action23a_absent_path" ]]; then printf true; else printf false; fi
    )
    action23a_symlink_absent=$(
        if [[ ! -L "$action23a_absent_path" ]]; then printf true; else printf false; fi
    )

    # conditional-validator-explicit-failures-begin
    assert_equal "${action23a_absent_label}_entry_absent" \
        "$action23a_entry_absent" true || return 1
    assert_equal "${action23a_absent_label}_symlink_absent" \
        "$action23a_symlink_absent" true || return 1
    # conditional-validator-explicit-failures-end
}

run_check() {
    local action23a_run_label=$1

    shift
    if "$@"; then
        pass_check "$action23a_run_label"
    else
        fail_check "$action23a_run_label"
    fi
}

set_boundary() {
    current_boundary=$1
    printf 'action_23a_boundary=%s\n' "$current_boundary"
}
...
https_probe() {
    local action23a_https_name=$1
    local action23a_https_address=$2

    curl -kfsS -o /dev/null --connect-timeout 3 --max-time 5 \
        --resolve "${action23a_https_name}:443:${action23a_https_address}" \
        "https://${action23a_https_name}/"
}

validate_candidate_records() {
    local action23a_candidate=$1
    local action23a_live_records
    local action23a_candidate_records
    local action23a_existing_records_unchanged

    # conditional-validator-explicit-failures-begin
    assert_equal candidate_admin_a_exact_once \
        "$(grep -Fxc '    local-data: "pihole-admin.local.theama.co. IN A 10.1.0.56"' \
            "$action23a_candidate" || true)" 1 || return 1
    ...
    assert_equal candidate_caddy_aaaa_absent \
        "$(grep -Ec '(pihole-admin|proxy)[.]local[.]theama[.]co[.].* IN AAAA ' \
            "$action23a_candidate" || true)" 0 || return 1
    ...
    assert_equal candidate_homeassistant_absent \
        "$(grep -Fc 'homeassistant.local.theama.co' \
            "$action23a_candidate" || true)" 0 || return 1
    # conditional-validator-explicit-failures-end
    action23a_live_records=$(mktemp)
    action23a_candidate_records=$(mktemp)
    grep -E '^[[:space:]]+local-data(-ptr)?: ' "$live_local_zone" |
        LC_ALL=C sort >"$action23a_live_records" || return 1
    grep -E '^[[:space:]]+local-data(-ptr)?: ' "$action23a_candidate" |
        grep -Fv 'pihole-admin.local.theama.co. IN A 10.1.0.56' |
        grep -Fv 'proxy.local.theama.co. IN A 10.1.0.56' |
        LC_ALL=C sort >"$action23a_candidate_records" || return 1
    action23a_existing_records_unchanged=$(
        if cmp -s "$action23a_live_records" "$action23a_candidate_records"; then
            printf true
        else
            printf false
        fi
    ...
    assert_equal candidate_existing_records_unchanged \
        "$action23a_existing_records_unchanged" true || return 1
    rm -f -- "$action23a_live_records" || return 1
    rm -f -- "$action23a_candidate_records" || return 1
    # conditional-validator-explicit-failures-end
}

validate_shadow_parser() {
    local action23a_parser_root=$1
    local action23a_parser_candidate=$2
...
capture_dns_probe() {
    local action23a_probe_server=$1
    local action23a_probe_port=$2
    local action23a_probe_name=$3
    local action23a_probe_type=$4
    local action23a_probe_output
    local action23a_probe_status
    local action23a_probe_canonical

    set +e
    if [[ "$action23a_probe_type" == PTR ]]; then
        action23a_probe_output=$(timeout 2 dig +time=1 +tries=1 +short \
            -p "$action23a_probe_port" "@$action23a_probe_server" \
            -x "$action23a_probe_name" 2>/dev/null)
        action23a_probe_status=$?
    else
        action23a_probe_output=$(timeout 2 dig +time=1 +tries=1 +short \
            -p "$action23a_probe_port" "@$action23a_probe_server" \
            "$action23a_probe_name" "$action23a_probe_type" 2>/dev/null)
        action23a_probe_status=$?
    fi
    set -e

    action23a_probe_canonical=$(printf '%s\n' "$action23a_probe_output" |
    ...
    if [[ -z "$action23a_probe_canonical" ]]; then
        action23a_probe_canonical=none
    fi

    probe_status_value=$action23a_probe_status
    if [[ ${#action23a_probe_canonical} -le 512 &&
        "$action23a_probe_canonical" =~ ^[A-Za-z0-9:.,_-]+$ ]]; then
        probe_safe_value=true
        probe_answer_value=$action23a_probe_canonical
    else
        probe_safe_value=false
        probe_answer_value="unsafe_sha256_$(printf '%s' "$action23a_probe_canonical" |
            sha256sum | awk '{ print $1 }')"
    fi
}

record_readiness_equal() {
    local action23a_ready_label=$1
    local action23a_ready_observed=$2
    local action23a_ready_expected=$3

    if [[ "$action23a_ready_observed" == "$action23a_ready_expected" ]]; then
        pass_check "$action23a_ready_label"
    else
        printf 'action_23a_check_%s=false\n' "$action23a_ready_label" >&2
        printf 'action_23a_failed_check=%s\n' "$action23a_ready_label" >&2
        printf 'action_23a_failed_observed=%s\n' "$action23a_ready_observed" >&2
        readiness_failure_count=$((readiness_failure_count + 1))
    fi
}

validate_readiness_results() {
    local action23a_ready_key
    local -n action23a_status_map=$1
    local -n action23a_answer_map=$2
    local -n action23a_safe_map=$3
    local -n action23a_expected_map=$4
    local -n action23a_iteration_map=$5

    # conditional-validator-explicit-failures-begin
    readiness_failure_count=0
    for action23a_ready_key in "${readiness_keys[@]}"; do
        record_readiness_equal \
            "readiness_${action23a_ready_key}_command_status" \
            "${action23a_status_map[$action23a_ready_key]}" 0 || return 1
        record_readiness_equal \
            "readiness_${action23a_ready_key}_answer_safe" \
            "${action23a_safe_map[$action23a_ready_key]}" true || return 1
        record_readiness_equal \
            "readiness_${action23a_ready_key}_answer_exact" \
            "${action23a_answer_map[$action23a_ready_key]}" \
            "${action23a_expected_map[$action23a_ready_key]}" || return 1
        printf 'action_23a_value_readiness_%s_answer=%s\n' \
            "$action23a_ready_key" "${action23a_answer_map[$action23a_ready_key]}"
        printf 'action_23a_value_readiness_%s_iteration=%s\n' \
            "$action23a_ready_key" "${action23a_iteration_map[$action23a_ready_key]}"
    done
...
emit_expected_labels() {
    local action23a_expected_command
    local action23a_expected_key

    for action23a_expected_command in "${required_commands[@]}"; do
        printf 'command_%s_available\n' "${action23a_expected_command//-/_}"
    done
    printf '%s\n' \
    ...
    for action23a_expected_key in "${readiness_keys[@]}"; do
        printf 'readiness_%s_command_status\n' "$action23a_expected_key"
        printf 'readiness_%s_answer_safe\n' "$action23a_expected_key"
        printf 'readiness_%s_answer_exact\n' "$action23a_expected_key"
    done
    printf '%s\n' \
        node_a_management_https caddy_vip_https \
...
emit_contract_transcript() {
    local action23a_contract_label
    local action23a_contract_count=0

    while IFS= read -r action23a_contract_label; do
        [[ -n "$action23a_contract_label" ]] || continue
        printf 'action_23a_check_%s=true\n' "$action23a_contract_label"
        action23a_contract_count=$((action23a_contract_count + 1))
    done < <(emit_expected_labels)
    printf 'action_23a_value_check_count=%s\n' "$action23a_contract_count"
    printf 'action_23a_backup_dir=%s\n' "$backup_dir"
    printf 'action_23a_manifest_action=23a\n'
    printf 'action_23a_record_family=A\n'
    printf 'action_23a_local_zone_sha256=%s\n' "$candidate_local_zone_sha256"
    printf 'action_23a_dns_configuration_mutation=true\n'
    printf 'action_23a_pihole_cache_reset=true\n'
    printf 'action_23a_unbound_reload=true\n'
    printf 'action_23a_peer_ssh=false\n'
    printf 'action_23a_synchronization_executed=false\n'
    printf 'action_23a_acceptance=true\n'
}

rollback_record_status() {
    local action23a_rollback_label=$1
    local action23a_rollback_status=$2

    printf 'action_23a_rollback_check_%s_status=%s\n' \
        "$action23a_rollback_label" "$action23a_rollback_status" >&2
    if [[ "$action23a_rollback_status" -ne 0 ]]; then
        rollback_failed=true
    fi
}

rollback_record_equal() {
    local action23a_rollback_equal_label=$1
    local action23a_rollback_observed=$2
    local action23a_rollback_expected=$3

    if [[ "$action23a_rollback_observed" == "$action23a_rollback_expected" ]]; then
        printf 'action_23a_rollback_check_%s=true\n' \
            "$action23a_rollback_equal_label" >&2
    else
        printf 'action_23a_rollback_check_%s=false\n' \
            "$action23a_rollback_equal_label" >&2
        rollback_failed=true
    fi
}
...
rollback() {
    local action23a_original_status=$?
    local action23a_rollback_status=0

    trap - EXIT
    if [[ "$transaction_complete" == true ]]; then
        exit "$action23a_original_status"
    fi

    set +e
    rollback_failed=false
    printf 'action_23a_rollback_started=true\n' >&2
    if [[ "$mutation_started" == true ]]; then
        install -o root -g root -m 0644 \
            "$backup_dir/pihole-local-zone.conf.before" "$live_local_zone"
        action23a_rollback_status=$?
        rollback_record_status local_zone_restore "$action23a_rollback_status"
        unbound-control reload >/dev/null 2>&1
        action23a_rollback_status=$?
        rollback_record_status unbound_reload "$action23a_rollback_status"
        perform_pihole_reset >/dev/null 2>&1
        action23a_rollback_status=$?
        rollback_record_status pihole_restartdns "$action23a_rollback_status"
        systemctl is-active --quiet unbound.service
        action23a_rollback_status=$?
        rollback_record_status unbound_active "$action23a_rollback_status"
        systemctl is-active --quiet pihole-FTL.service
        action23a_rollback_status=$?
        rollback_record_status pihole_ftl_active "$action23a_rollback_status"
    elif [[ "$backup_started" == true ]]; then
        rm -rf -- "$backup_dir"
        action23a_rollback_status=$?
        rollback_record_status unused_backup_cleanup "$action23a_rollback_status"
    fi
    rm -f -- "$transaction_file"
    action23a_rollback_status=$?
    rollback_record_status transaction_cleanup "$action23a_rollback_status"
    ...
    if [[ "$rollback_failed" == true ]]; then
        printf 'action_23a_rollback_complete=false\n' >&2
        printf 'manual_intervention_required=true\n' >&2
        exit 125
    fi
    printf 'action_23a_rollback_complete=true\n' >&2
    exit "$action23a_original_status"
}

unhandled_error() {
    local action23a_error_status=$?
    local action23a_error_line=$1
    local action23a_error_command=$2

    printf 'action_23a_unhandled_error=true\n' >&2
    printf 'action_23a_unhandled_status=%s\n' "$action23a_error_status" >&2
    printf 'action_23a_unhandled_boundary=%s\n' "$current_boundary" >&2
    printf 'action_23a_unhandled_line=%s\n' "$action23a_error_line" >&2
    printf 'action_23a_unhandled_command_sha256=%s\n' \
        "$(printf '%s' "$action23a_error_command" | sha256sum | awk '{ print $1 }')" >&2
    return "$action23a_error_status"
}
...
case "${1:-}" in
    --self-test)
        ...
        printf 'action_23a_driver_self_test_complete=true\n'
        exit 0
        ;;
    --expected-labels)
        [[ $# -eq 1 ]]
    ...
    --candidate)
        ...
        ;;
    *)
        printf 'Usage: %s --candidate /run/caddy-action23a.*/pihole-local-zone.conf\n' \
            "${0##*/}" >&2
        exit 2
        ;;
esac
...
parser_root="$candidate_parent/unbound-action23a.conf"
readonly parser_root

trap 'unhandled_error "$LINENO" "$BASH_COMMAND"' ERR
trap rollback EXIT

printf 'action_23a_remote_reached=true\n'
...
assert_equal candidate_parent_pattern \
    "$(if [[ "$candidate_parent" =~ ^/run/caddy-action23a\.[A-Za-z0-9]+$ ]]; then
        printf valid
    else
        printf invalid
    fi)" valid
...
assert_equal backup_manifest_action \
    "$(awk -F= '$1 == "action" { print $2 }' "$backup_dir/manifest")" 23a
