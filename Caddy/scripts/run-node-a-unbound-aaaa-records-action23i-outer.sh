...
readonly driver="$script_dir/apply-node-a-unbound-aaaa-records-action23i.sh"
...
readonly regression="$caddy_root/tests/action23i-node-a-unbound-aaaa-records-regression.sh"
...
gate_pass() {
    local action23i_gate_label=$1

    printf 'action_23i_outer_gate_%s=true\n' "$action23i_gate_label"
}
...
require_gate() {
    ...
    shift
    if "$@"; then
        gate_pass "$action23i_gate_label"
    else
        printf 'action_23i_outer_gate_%s=false\n' "$action23i_gate_label" >&2
        return 1
    fi
}
...
render_candidate() {
    ...
    ' "$source_local_zone" >"$action23i_render_path"
}

reconstruct_live_parent() {
    local action23i_candidate_path=$1
    local action23i_parent_path=$2
    ...
    ' "$action23i_candidate_path" >"$action23i_parent_path"
}

verify_source_and_candidate() {
    local action23i_verify_dir
    local action23i_verify_candidate
    local action23i_verify_parent
    ...
    action23i_verify_dir=$(mktemp -d)
    action23i_verify_candidate="$action23i_verify_dir/pihole-local-zone.conf"
    action23i_verify_parent="$action23i_verify_dir/live-parent.conf"
    render_candidate "$action23i_verify_candidate" || return 1
    reconstruct_live_parent \
        "$action23i_verify_candidate" "$action23i_verify_parent" || return 1
    require_gate candidate_hash has_hash \
        "$action23i_verify_candidate" "$candidate_local_zone_sha256" || return 1
    require_gate candidate_homeassistant_absent test \
        "$(grep -Fc 'homeassistant.local.theama.co' \
            "$action23i_verify_candidate" || true)" -eq 0 || return 1
    require_gate candidate_admin_aaaa_exact test \
        "$(grep -Fxc '    local-data: "pihole-admin.local.theama.co. IN AAAA fd36:5aa8:6971:1::56"' \
            "$action23i_verify_candidate" || true)" -eq 1 || return 1
    ...
    require_gate candidate_exact_live_parent has_hash \
        "$action23i_verify_parent" "$accepted_local_zone_sha256" || return 1
    rm -rf -- "$action23i_verify_dir" || return 1
    # conditional-validator-explicit-failures-end
}

verify_expected_labels() {
    local action23i_labels
    local action23i_label_count
    local action23i_unique_count

    action23i_labels=$("$driver" --expected-labels)
    action23i_label_count=$(printf '%s\n' "$action23i_labels" | sed '/^$/d' | wc -l)
    action23i_unique_count=$(printf '%s\n' "$action23i_labels" | sed '/^$/d' |
        LC_ALL=C sort -u | wc -l)
    require_gate driver_expected_labels_nonempty test \
        "$action23i_label_count" -gt 0 || return 1
    require_gate driver_expected_labels_unique test \
        "$action23i_label_count" -eq "$action23i_unique_count" || return 1
}
...
capture_is_safe() {
    local action23i_capture_path=$1
    local action23i_capture_bytes=$2
    local action23i_capture_lines=$3

    [[ "$action23i_capture_bytes" -le "$max_capture_bytes" ]] || return 1
    [[ "$action23i_capture_lines" -le "$max_capture_lines" ]] || return 1
    if LC_ALL=C grep -Eq '[^[:print:][:space:]]' "$action23i_capture_path"; then
        return 1
    fi
    if grep -Eqi 'PRIVATE KEY|Authorization:|api[_-]?key|token=' \
        "$action23i_capture_path"; then
        return 1
    fi
}

emit_capture() {
    local action23i_capture_label=$1
    local action23i_capture_path=$2
    local action23i_capture_bytes
    local action23i_capture_lines
    local action23i_capture_hash

    action23i_capture_bytes=$(wc -c <"$action23i_capture_path")
    action23i_capture_lines=$(wc -l <"$action23i_capture_path")
    action23i_capture_hash=$(file_hash "$action23i_capture_path")
    printf 'action_23i_outer_%s_bytes=%s\n' \
        "$action23i_capture_label" "$action23i_capture_bytes"
    printf 'action_23i_outer_%s_lines=%s\n' \
        "$action23i_capture_label" "$action23i_capture_lines"
    printf 'action_23i_outer_%s_sha256=%s\n' \
        "$action23i_capture_label" "$action23i_capture_hash"
    if capture_is_safe "$action23i_capture_path" \
        "$action23i_capture_bytes" "$action23i_capture_lines"; then
        printf 'action_23i_outer_%s_classification=bounded_safe\n' \
            "$action23i_capture_label"
        if [[ "$action23i_capture_bytes" -eq 0 ]]; then
            printf 'action_23i_outer_%s_content=empty\n' "$action23i_capture_label"
        else
            printf 'action_23i_outer_%s_begin\n' "$action23i_capture_label"
            sed -n '1,2000p' "$action23i_capture_path"
            printf 'action_23i_outer_%s_end\n' "$action23i_capture_label"
        fi
        return 0
    fi
    printf 'action_23i_outer_%s_classification=unsafe_or_unbounded\n' \
        "$action23i_capture_label"
    return 1
}

validate_success_transcript() {
    local action23i_transcript=$1
    local action23i_expected_file=$2
    local action23i_actual_file=$3
    local action23i_actual_count
    local action23i_unique_count
    local action23i_false_count

    action23i_false_count=$(grep -Ec \
        '^action_23i_check_[a-z0-9_]+=false$' "$action23i_transcript" || true)

    # conditional-validator-explicit-failures-begin
    "$driver" --expected-labels | LC_ALL=C sort >"$action23i_expected_file" || return 1
    sed -n 's/^action_23i_check_\([a-z0-9_]*\)=\(true\|false\)$/\1/p' \
        "$action23i_transcript" | LC_ALL=C sort >"$action23i_actual_file" || return 1
    action23i_actual_count=$(wc -l <"$action23i_actual_file")
    action23i_unique_count=$(LC_ALL=C sort -u "$action23i_actual_file" | wc -l)
    [[ "$action23i_actual_count" -eq "$action23i_unique_count" ]] || return 1
    cmp -s "$action23i_expected_file" "$action23i_actual_file" || return 1
    [[ "$action23i_false_count" -eq 0 ]] || return 1
    [[ "$(grep -Fxc 'action_23i_acceptance=true' "$action23i_transcript")" -eq 1 ]] || return 1
    [[ "$(grep -Fxc 'action_23i_manifest_action=23i' "$action23i_transcript")" -eq 1 ]] || return 1
    [[ "$(grep -Fxc 'action_23i_record_family=AAAA' "$action23i_transcript")" -eq 1 ]] || return 1
    [[ "$(grep -Fxc 'action_23i_pihole_ftl_sha256=c77de6654c575e12fa1661f8ec901de67d9a623c3e9b965d4e32b550c132a7aa' "$action23i_transcript")" -eq 1 ]] || return 1
    [[ "$(grep -Fxc 'action_23i_pihole_domain_sha256=a8305acbc27a9133d6e68e8b1a0fe9a462a975919233d71866d54b2e98810f96' "$action23i_transcript")" -eq 1 ]] || return 1
    [[ "$(grep -Fxc 'action_23i_pihole_cache_reset=true' "$action23i_transcript")" -eq 1 ]] || return 1
    [[ "$(grep -Fxc 'action_23i_unbound_reload=true' "$action23i_transcript")" -eq 1 ]] || return 1
    [[ "$(grep -Fxc 'action_23i_peer_ssh=false' "$action23i_transcript")" -eq 1 ]] || return 1
    [[ "$(grep -Fxc 'action_23i_synchronization_executed=false' "$action23i_transcript")" -eq 1 ]] || return 1
    # conditional-validator-explicit-failures-end
}

self_test() {
    run_local_gates
    printf 'action_23i_outer_regression_skipped_for_self_test=true\n'
    printf 'action_23i_outer_node_contact=false\n'
    printf 'action_23i_outer_self_test_complete=true\n'
}
...
contract_test() {
    ...
    grep -Fq '/bin/bash "$remote_stage/apply-node-a-unbound-aaaa-records-action23i.sh"' "$0"
    grep -Fq 'validate_success_transcript' "$0"
    grep -Fq 'action_23i_outer_stdout_classification' "$0"
    grep -Fq 'action_23i_outer_stderr_classification' "$0"
    if grep -Eq '(^|[[:space:]])(rsync|scp|sftp)([[:space:]]|$)' "$0"; then
        return 1
    fi
    printf 'action_23i_outer_contract_test_complete=true\n'
}
...
case "${1:-}" in
    ...
    --source-test)
        [[ $# -eq 1 ]]
        verify_source_and_candidate
        printf 'action_23i_outer_source_test_complete=true\n'
        exit 0
        ;;
    --contract-test)
        [[ $# -eq 1 ]]
...
if [[ "$transport_test" == false ]]; then
    "$regression"
    printf 'action_23i_outer_regression=true\n'
else
    printf 'action_23i_outer_regression_skipped_for_transport_test=true\n'
fi

work_dir=$(mktemp -d)
readonly work_dir
candidate="$work_dir/pihole-local-zone.conf"
archive="$work_dir/action23i.tar"
...
tar -C "$script_dir" -cf "$archive" \
    apply-node-a-unbound-aaaa-records-action23i.sh \
    -C "$work_dir" pihole-local-zone.conf

ssh_bin=ssh
if [[ "$transport_test" == true ]]; then
    ssh_bin=${ACTION23I_TEST_SSH_BIN:?ACTION23I_TEST_SSH_BIN is required}
fi
readonly ssh_bin
...
"$ssh_bin" -T \
    ...
    "pi@$host_address" \
    "cd / && sudo -n /bin/bash -c 'remote_stage=\$(mktemp -d /run/caddy-action23i.XXXXXX) && trap '\''rm -rf -- \"\$remote_stage\"'\'' EXIT && tar -C \"\$remote_stage\" -xf - && chown root:root \"\$remote_stage\"/* && chmod 0700 \"\$remote_stage\" && chmod 0700 \"\$remote_stage/apply-node-a-unbound-aaaa-records-action23i.sh\" && chmod 0600 \"\$remote_stage/pihole-local-zone.conf\" && /bin/bash \"\$remote_stage/apply-node-a-unbound-aaaa-records-action23i.sh\" --candidate \"\$remote_stage/pihole-local-zone.conf\"'" \
...
if [[ "$stdout_safe" != true || "$stderr_safe" != true ]]; then
    ...
    printf 'action_23i_outer_protected_evidence_path=%s\n' "$work_dir" >&2
    exit 1
fi
...
if [[ "$ssh_status" -eq 0 ]] &&
    ...
    printf 'action_23i_outer_ssh_status=0\n'
    printf 'action_23i_outer_node_a_contacted=true\n'
    printf 'action_23i_outer_dns_configuration_mutation=true\n'
    printf 'action_23i_outer_unbound_reload=true\n'
    printf 'action_23i_outer_pihole_cache_reset=true\n'
    printf 'action_23i_outer_acceptance=true\n'
    exit 0
fi

printf 'action_23i_outer_ssh_status=%s\n' "$ssh_status" >&2
if grep -Fqx 'manual_intervention_required=true' "$stderr_capture"; then
    printf 'action_23i_outer_manual_intervention_required=true\n' >&2
    exit 125
fi
if grep -Fqx 'action_23i_rollback_complete=true' "$stderr_capture"; then
    printf 'action_23i_outer_rollback_complete=true\n' >&2
fi
printf 'action_23i_outer_acceptance=false\n' >&2
exit 1
