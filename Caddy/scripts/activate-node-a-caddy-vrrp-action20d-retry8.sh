...
readonly prefix=action_20d_retry8_node
...
case "${1:-}" in
    ...
    --inspect)
        ...
        configure_role
        [[ "$node_role" = node-a ]] || exit 64
        transaction_root=$(mktemp -d /run/caddy-action20d-retry8-inspect.XXXXXX)
    ...
    --rollback)
        ...
        configure_role
        [[ "$node_role" = node-a ]] || exit 64
        transaction_root=$(mktemp -d /run/caddy-action20d-retry8-rollback.XXXXXX)
        readonly transaction_root
...
transaction_root=$(mktemp -d /run/caddy-action20d-retry8-node.XXXXXX)
readonly transaction_root
...
backup_directory=$(mktemp -d "$backup_root/action20d-retry8-${node_role}-caddy-vrrp.XXXXXX")
