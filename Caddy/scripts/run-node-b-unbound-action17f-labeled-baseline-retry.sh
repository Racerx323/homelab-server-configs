...
readonly tracer_sha256=1513aa2a559259965a1c6730e79dc3bfb953f4f5aca17590f4d39d71cc73de10
readonly regression_sha256=99e5b2bed4dc023157b3a0dcff0b2dd1fb665d09a8d88a8ff6a4daf424d0bf6b
readonly collision_checker_sha256=78b46fcdc6c4ca6b4cfe0121c347e0e101c472d723a891aac2dea7f23f085cd8
readonly failed_runner_sha256=9abfa890b93b5bb3e8ac3f509cf27b9c391c137a56180202f99c8c90c3c88e5d
...
readonly tracer_path="$script_dir/trace-node-b-unbound-action17f-baseline-retry.sh"
readonly regression_path="$caddy_root/tests/action17f-b-labeled-baseline-retry-regression.sh"
readonly collision_checker_path="$caddy_root/tests/check-shell-readonly-local-collisions.sh"
readonly failed_runner_path="$script_dir/run-node-b-unbound-action17f-transition-diagnostic.sh"
...
verify_sources() {
    ...
    verify_file "$failed_runner_path" "$failed_runner_sha256"
    bash -n \
        "$inspector_path" "$tracer_path" "$regression_path" \
        "$collision_checker_path" "$failed_runner_path"
}
...
verify_live_sources() {
    ...
    for source_path in \
        "$inspector_path" "$tracer_path" "$regression_path" \
        "$collision_checker_path" "$failed_runner_path"; do
...
validate_inspector_evidence() {
    ...
    require_value action_17f_a_node_b_unbound_prewrite_diagnostic_complete \
        true "$evidence_path"
...
validate_trace_evidence() {
    local evidence_path=$1
    local exact_status failed_label failure_count completion_count

    require_value action_17f_b_retry_trace_remote_reached true "$evidence_path"
    require_value remote_paths_created false "$evidence_path"
    require_value dns_queries_performed false "$evidence_path"
    require_value dns_configuration_mutations false "$evidence_path"
    require_value service_mutations false "$evidence_path"
    require_value persistent_mutations false "$evidence_path"
    require_value action_17f_b_retry_labeled_trace_complete true "$evidence_path"

    exact_status=$(
        unique_value action_17f_b_retry_exact_baseline_status "$evidence_path"
    )
    [[ "$exact_status" =~ ^[0-9]+$ ]]
    failure_count=$(
    ...
    completion_count=$(
        grep -Ec '^action_17f_b_retry_exact_baseline_complete=true$' \
            "$evidence_path" || true
    )
...
run_contract_test() {
    ...
    printf '%s\n' \
        ...
        action_17f_a_node_b_unbound_prewrite_diagnostic_complete=true \
        >"$inspector_fixture"
    printf '%s\n' \
        action_17f_b_retry_trace_remote_reached=true \
        exact_assertion_baseline_live_state_hash=false \
        exact_assertion_baseline_live_state_hash_status=1 \
        action_17f_b_retry_exact_baseline_status=1 \
        remote_paths_created=false \
        dns_queries_performed=false \
        dns_configuration_mutations=false \
        service_mutations=false \
        persistent_mutations=false \
        action_17f_b_retry_labeled_trace_complete=true \
        >"$trace_fixture"

    (
        readonly remote_inspector_evidence="$inspector_fixture"
        readonly remote_trace_evidence="$trace_fixture"
        validate_inspector_evidence "$remote_inspector_evidence"
        validate_trace_evidence "$remote_trace_evidence"
    )
    printf 'action_17f_b_retry_runner_contract_test_complete=true\n'
}
...
case "${1:-}" in
    --self-test)
        ...
        printf 'action_17f_b_retry_runner_self_test_complete=true\n'
        exit 0
        ;;
    --source-test)
        (($# == 1))
        ...
        printf 'action_17f_b_retry_runner_source_test_complete=true\n'
        exit 0
        ;;
    --contract-test)
        (($# == 1))
        verify_sources
        run_contract_test
        printf 'action_17f_b_retry_runner_contract_complete=true\n'
        exit 0
        ;;
    "") ;;
    *)
...
cleanup() {
    ...
    printf 'action_17f_b_retry_local_cleanup_complete=true\n'
}
trap cleanup EXIT
...
printf '%s\n' action_17f_b_retry_inspector_transcript_begin
cat "$remote_inspector_evidence"
printf '%s\n' action_17f_b_retry_inspector_transcript_end
printf '%s\n' action_17f_b_retry_trace_transcript_begin
cat "$remote_trace_evidence"
printf '%s\n' action_17f_b_retry_trace_transcript_end
...
validate_inspector_evidence "$remote_inspector_evidence"
validate_trace_evidence "$remote_trace_evidence"
printf 'action_17f_b_retry_read_only_diagnostic_accepted=true\n'
