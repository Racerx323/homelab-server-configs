...
readonly prefix=action_20e_node
...
expected_checks() {
    printf '%s\n' \
        ...
        tmpfiles_parent_regular tmpfiles_parent_not_symlink pi_user_exists pi_group_exists \
        ...
        candidate_content_exact parser_status_zero parser_stdout_safe parser_stderr_safe \
        backup_directory_regular backup_directory_not_symlink backup_directory_metadata_exact \
...
case "${1:-}" in
    ...
    --rollback)
        ...
        ;;
    *)
        printf 'Usage: %s --node node-a|node-b | --rollback --node node-a|node-b BACKUP | --expected-checks | --expected-rollback-checks | --render-config | --self-test\n' "${0##*/}" >&2
        exit 64
        ;;
esac
...
transaction_root=$(mktemp -d /run/caddy-action20e-node.XXXXXX)
...
: >"$create_stdout"
: >"$create_stderr"
chmod 0600 "$candidate" "$parser_stdout" "$parser_stderr" "$create_stdout" "$create_stderr"
...
parser_status=0
systemd-tmpfiles --dry-run --create "$candidate" >"$parser_stdout" 2>"$parser_stderr" || parser_status=$?
