...
validate_probe() {
    ...
    observed_count=$(grep -Ec '^action_20e_retry_a_probe_assertion_[a-z0-9_]+=true$' "$output_path" || true)
    unique_count=$(sed -n 's/^\(action_20e_retry_a_probe_assertion_[a-z0-9_]*\)=true$/\1/p' "$output_path" | LC_ALL=C sort -u | wc -l)
    [[ "$observed_count" -eq "$expected_count" ]] || return 1
    [[ "$unique_count" -eq "$expected_count" ]] || return 1
    diff -u \
        <(/bin/bash "$inspector" --expected-assertions | sed 's/^/action_20e_retry_a_probe_assertion_/' | LC_ALL=C sort) \
        <(sed -n 's/^\(action_20e_retry_a_probe_assertion_[a-z0-9_]*\)=true$/\1/p' "$output_path" | LC_ALL=C sort) >/dev/null || return 1
    [[ "$(grep -Ec '^action_20e_retry_a_probe_assertion_[a-z0-9_]+=false$' "$output_path" || true)" -eq 0 ]] || return 1
    grep -Fxq 'action_20e_retry_a_probe_persistent_mutations=false' "$output_path" || return 1
    grep -Fxq 'action_20e_retry_a_probe_inspection_complete=true' "$output_path" || return 1
}
run_node() {
    local node_role=$1
    local node_target=$2
...
verify_source
working_directory_approved
work_directory=$(mktemp -d /tmp/caddy-action20e-retry-a-runner.XXXXXX)
