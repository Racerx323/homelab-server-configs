...
readonly driver_sha256=ff413bb040f4a6d318d27f87bb9cef198cd2ba7b8f653d3f1a9834b689838b9e
readonly driver_name=cutover-node-a-lighttpd-caddy-action16ap-retry.sh
readonly -a success_markers=(
    action_16ap_retry_remote_reached=true
    action_16ap_retry_preflight_complete=true
    action_16ap_retry_mutation_started=true
...
evaluate_success() {
    ...
    grep -Eq '^caddy_reset_before_start=(true|false)$' \
        "$output_file" || return 97
    if grep -Eq \
        'action_16ap_retry_rollback_(complete|incomplete|not_required)=true|manual_intervention_required=true' \
...
evaluate_contract() {
    ...
    if [[ "$(grep -Fxc 'action_16ap_retry_cutover_complete=true' \
        "$output_file")" -eq 1 ]]; then
        evaluate_success "$output_file" "$error_file" "$ssh_status"
        return
    fi
    [[ "$ssh_status" -ne 0 ]] || return 97
    if grep -Fxq 'action_16ap_retry_rollback_incomplete=true' "$error_file" ||
        grep -Fxq 'manual_intervention_required=true' "$error_file"; then
        return 97
    fi
    if [[ "$(grep -Fxc 'action_16ap_retry_rollback_complete=true' \
        "$error_file")" -eq 1 ]]; then
        return 1
    fi
    if [[ "$(grep -Fxc 'action_16ap_retry_rollback_not_required=true' \
        "$error_file")" -eq 1 ]]; then
        return 1
    fi
    return 97
...
if [[ "${1:-}" == --self-test && $# -eq 1 ]]; then
    ...
    verify_driver
    printf 'action_16ap_retry_cutover_runner_self_test_complete=true\n'
    exit 0
    ...
    error_file="$contract_dir/error"
    {
        ...
        printf 'caddy_reset_before_start=false\n'
    } >"$success"
    : >"$error_file"
    ...
    printf 'action_16ap_retry_rollback_complete=true\n' >"$rollback_error"
    set +e
    ...
    printf 'action_16ap_retry_rollback_not_required=true\n' >"$no_mutation_error"
    set +e
    ...
    {
        printf 'action_16ap_retry_rollback_incomplete=true\n'
    ...
    printf 'action_16ap_retry_contract_test_complete=true\n'
    exit 0
elif (($#)); then
...
verify_driver
work_dir=$(mktemp -d /tmp/caddy-action16ap-retry.XXXXXX)
readonly work_dir
...
finish() {
    ...
    if [[ -e "$work_dir" || -L "$work_dir" ]]; then
        printf 'action_16ap_retry_local_cleanup_complete=false\n' >&2
        exit 97
    fi
    printf 'action_16ap_retry_local_cleanup_complete=true\n'
    exit "$status"
}
