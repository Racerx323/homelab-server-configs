...
validate_baseline_transcript() {
    ...
    record_command baseline_state_unchanged grep -Fxq \
        "${baseline_prefix}_assertion_state_unchanged=true" "$transcript_path"
    record_command baseline_helper_invoked_false grep -Fxq \
        "${baseline_prefix}_helper_execution=false" "$transcript_path"
    record_command baseline_persistent_mutation_false grep -Fxq \
        "${baseline_prefix}_persistent_mutations=false" "$transcript_path"
...
write_contract_fixture() {
    ...
    printf '%s\n' \
        ...
        "${baseline_prefix}_helper_execution=false" \
        "${baseline_prefix}_persistent_mutations=false" >>"$fixture_path"
}
...
case "${1:-}" in
    --self-test)
        ...
        expected_root=$(mktemp -d /tmp/caddy-action19b-a-retry2-self.XXXXXX)
        readonly expected_root
        trap 'rm -rf -- "$expected_root"' EXIT
        /bin/bash "$baseline_inspector" --expected-assertions \
    ...
    --contract-test)
        [[ $# -eq 2 ]] || exit 64
        baseline_inspector=$2
        contract_root=$(mktemp -d /tmp/caddy-action19b-a-retry2-contract.XXXXXX)
        readonly contract_root
        trap 'rm -rf -- "$contract_root"' EXIT
        /bin/bash "$baseline_inspector" --expected-assertions >"$contract_root/labels"
...
record_command historical_action19b_a_remote_bundle_stage_absent \
    test -z "$(find /run -mindepth 1 -maxdepth 1 \
        -name 'caddy-action19b-a-stage.*' -print -quit 2>/dev/null)"

work_directory=$(mktemp -d /run/caddy-action19b-a-retry2-inspector.XXXXXX)
...
baseline_status=0
/bin/bash "$baseline_inspector" >"$work_directory/baseline.stdout" \
