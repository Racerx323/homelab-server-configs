...
readonly prefix=action_34m_outer
...
readonly action_transaction=$script_directory/apply-durable-apprise-action34m.sh
readonly regression=$caddy_root/tests/durable-apprise-deployment-regression.sh
readonly artifact_manifest=$caddy_root/manifests/durable-apprise-action34m.tsv
readonly runtime_baseline_manifest=$caddy_root/manifests/caddy-runtime-lifecycle-action32g.tsv
readonly action_manifest=$caddy_root/manifests/durable-apprise-action34m.yaml
readonly expected_upload_transaction_sha256=34d43af6e72bcb40826eb8510cfe0a2d11369f8b7ec94529772e47217505675f
readonly expected_action_transaction_sha256=bbe08dfab49e71a1a1297ca0f1cc164529a30df343ca1820898989fb85307ca6
readonly expected_regression_sha256=8a52ea31ea824d6099f21c9c1731e6a8256bf49ac69a569ad66f4d25625432aa
...
readonly evidence_root=/tmp/caddy-ssh-evidence/action34m
...
build_payload() {
    ...
    install -m 0600 "$artifact_manifest" \
        "$action34i_outer_payload_root/Caddy/manifests/durable-apprise-action34m.tsv" || return 1
    ...
    payload_archive=$work_root/caddy-action34m-payload.tar
...
run_action() {
    ...
    run_streamed node-b-action34l-retained-upload "$node_b_target" \
        pihole00.local.theama.co "$action_transaction" \
        --dispose-action34l-retained-upload node-b \
        /tmp/caddy-action34h-payload-node-b-1786840719-615179.tar \
        8653c5379bd86226ec1d0a8d03da21e2f7071b223c3ba42c3ac31b9907fd349f \
        1786840719-615179 || return 1
...
production_path_helper_call() {
    local action34m_outer_helper_role=$1
    local action34m_outer_helper_path=$2
    local action34m_outer_helper_operation=$3
    local action34m_outer_helper_function

    case "$action34m_outer_helper_operation" in
        prepare) action34m_outer_helper_function=prepare_upload ;;
        accept) action34m_outer_helper_function=accept_upload ;;
        disposition) action34m_outer_helper_function=dispose_upload ;;
        *) return 64 ;;
    esac
    CADDY_ACTION34I_TEST_TMP=/tmp \
        CADDY_ACTION34I_TEST_RUN=$work_root/helper-run \
        /bin/bash -c '
            action34m_helper_source=$6
            action34m_helper_function=$7
            set -- --library-test "$1" "$2" "$3" "$4" "$5"
            source "$action34m_helper_source"
            "$action34m_helper_function"
        ' _ "$action34m_outer_helper_role" "$action34m_outer_helper_path" \
        "$payload_sha256" "$payload_size" "$run_token" \
        "$upload_transaction" "$action34m_outer_helper_function"
}

production_path_test() {
    local action34m_outer_evidence_root=${CADDY_PRODUCTION_PATH_EVIDENCE_ROOT:-}
    local action34m_outer_transaction_stdout action34m_outer_transaction_stderr
    local action34m_outer_transaction_status=0 action34m_outer_streamed_status=0
    local action34m_outer_node_a_path action34m_outer_node_b_path
    local action34m_outer_streamed_root action34m_outer_streamed_stdout
    local action34m_outer_streamed_stderr
    local action34m_outer_streamed_fd

    [[ "$action34m_outer_evidence_root" = /tmp/* ]] || return 64
    [[ -d "$action34m_outer_evidence_root" && ! -L "$action34m_outer_evidence_root" ]] || return 64
    [[ "$(stat -c '%a' "$action34m_outer_evidence_root")" = 700 ]] || return 64
    [[ -z "$(find "$action34m_outer_evidence_root" -mindepth 1 -maxdepth 1 -print -quit)" ]] || return 64

    work_root=$(mktemp -d /tmp/caddy-action34m-outer-production.XXXXXX) || return 1
    chmod 0700 "$work_root" || return 1
    run_token=$(date +%s)-$$ || return 1
    action34m_outer_node_b_path=/tmp/caddy-action34h-payload-node-b-$run_token.tar
    action34m_outer_node_a_path=/tmp/caddy-action34h-payload-node-a-$run_token.tar
    action34m_outer_transaction_stdout=$work_root/transaction.stdout
    action34m_outer_transaction_stderr=$work_root/transaction.stderr
    action34m_outer_streamed_root=$(mktemp -d \
        /tmp/caddy-action34m-production-path.XXXXXX) || return 1
    chmod 0700 "$action34m_outer_streamed_root" || return 1
    # Expand the validated temporary paths now; function locals are unavailable at EXIT.
    # shellcheck disable=SC2064
    trap "rm -f -- '$action34m_outer_node_a_path' '$action34m_outer_node_b_path'; rm -rf -- '/tmp/caddy-action34i/$run_token-node-a' '/tmp/caddy-action34i/$run_token-node-b' '$action34m_outer_streamed_root' '$work_root'" EXIT
    ...
    production_path_helper_call node-b "$action34m_outer_node_b_path" prepare || return 1
    printf 'production_path_outer_upload_prepare=true\n'
    install -m 0600 "$payload_archive" "$action34m_outer_node_b_path" || return 1
    production_path_helper_call node-b "$action34m_outer_node_b_path" accept || return 1
    printf 'production_path_outer_upload_accept=true\n'

    production_path_helper_call node-a "$action34m_outer_node_a_path" prepare || return 1
    install -m 0600 "$payload_archive" "$action34m_outer_node_a_path" || return 1
    production_path_helper_call node-a "$action34m_outer_node_a_path" disposition || return 1
    printf 'production_path_outer_upload_disposition=true\n'

    printf '%s\n' \
        "cd / && sudo -n /bin/bash -s -- --apply node-b $action34m_outer_node_b_path $payload_sha256 $run_token" \
        >"$action34m_outer_evidence_root/remote-command.argv" || return 1
    printf '%s\n' "$action34m_outer_node_b_path" \
        >"$action34m_outer_evidence_root/remote-path" || return 1
    printf 'production_path_outer_remote_command_constructed=true\n'

    /bin/bash "$action_transaction" --production-path-test \
        >"$action34m_outer_transaction_stdout" \
        2>"$action34m_outer_transaction_stderr" || action34m_outer_transaction_status=$?
    [[ "$action34m_outer_transaction_status" -eq 0 ]] || return 1
    [[ ! -s "$action34m_outer_transaction_stderr" ]] || return 1
    printf 'production_path_outer_transaction_dispatched=true\n'

    action34m_outer_streamed_stdout=$work_root/streamed-transaction.stdout
    action34m_outer_streamed_stderr=$work_root/streamed-transaction.stderr
    exec {action34m_outer_streamed_fd}<"$action_transaction" || return 1
    (
        cd /
        /bin/bash -s -- --production-path-case node-b-queue-absent \
            "$action34m_outer_streamed_root" node-b <&"$action34m_outer_streamed_fd"
    ) >"$action34m_outer_streamed_stdout" \
        2>"$action34m_outer_streamed_stderr" || action34m_outer_streamed_status=$?
    exec {action34m_outer_streamed_fd}<&- || return 1
    [[ "$action34m_outer_streamed_status" -eq 0 ]] || return 1
    [[ ! -s "$action34m_outer_streamed_stderr" ]] || return 1
    grep -Fq 'production_payload_validation=reached' \
        "$action34m_outer_streamed_stdout" || return 1
    ...
    printf '%s\n' "$payload_sha256" \
        >"$action34m_outer_evidence_root/payload.sha256" || return 1
    printf '%s\n' "$action34m_outer_transaction_status" \
        >"$action34m_outer_evidence_root/transaction.status" || return 1
    printf '0\n' >"$action34m_outer_evidence_root/mutation-count" || return 1
    printf 'prepare\t0\naccept\t0\ndisposition\t0\n' \
        >"$action34m_outer_evidence_root/upload-events.tsv" || return 1
    chmod 0600 "$action34m_outer_evidence_root"/* || return 1

    rm -f -- "$action34m_outer_node_a_path" "$action34m_outer_node_b_path" || return 1
