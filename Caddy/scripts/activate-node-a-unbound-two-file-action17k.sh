...
readonly accepted_legacy_primary_sha256=dd5af7ccbbac11324921e1d447d753dbab33b633bc4cb1db248ee0574825d3ae
...
pass_step() {
    ...
    printf 'action_17k_check_%s=true\n' "$passed_label"
}
...
fail_step() {
    ...
    printf 'action_17k_check_%s=false\n' "$failed_label" >&2
    printf 'action_17k_failed_step=%s\n' "$failed_label" >&2
    printf 'action_17k_failed_observed=%s\n' "$observed_value" >&2
    return 1
}
...
unhandled_error() {
    ...
    printf 'action_17k_unhandled_boundary=%s\n' "$current_boundary" >&2
    printf 'action_17k_unhandled_status=%s\n' "$error_status" >&2
    printf 'action_17k_unhandled_line=%s\n' "$error_line" >&2
    return "$error_status"
}
...
protected_tree_hash() {
    find "$live_conf_dir" -mindepth 1 -maxdepth 1 \
        ...
        ! -name '.pihole.conf.action17k.new' \
        ! -name '.pihole-local-zone.conf.action17k.new' \
        ! -name '.pihole.conf.action17k.before' \
...
validate_pihole_path() {
    ...
    pihole_query_result=$(
        dig +time=2 +tries=1 +short \
            @127.0.0.1 -p 53 pihole0.local.theama.co A |
            LC_ALL=C sort
    )
    assert_equal pihole_path_query "$pihole_query_result" 10.1.0.53
}
    ...
    for ((readiness_iteration = 1; readiness_iteration <= 20; readiness_iteration += 1)); do
        ...
        readiness_answer=$(
            dig +time=1 +tries=1 +short \
                @127.0.0.1 -p 5335 pihole0.local.theama.co A 2>/dev/null |
                LC_ALL=C sort
        ) || readiness_answer=
        if [[ "$(systemctl is-active unbound.service 2>/dev/null)" == active ]] &&
            [[ "$readiness_addresses" == *127.0.0.1:5335* ]] &&
            [[ "$readiness_addresses" == *"[::1]:5335"* ]] &&
            [[ "$readiness_answer" == 10.1.0.53 ]]; then
            printf 'action_17k_readiness_iterations=%s\n' \
                "$readiness_iteration"
...
reload_legacy_after_rollback() {
    ...
    for ((rollback_iteration = 1; rollback_iteration <= 20; rollback_iteration += 1)); do
        ...
        if [[ "$(systemctl is-active unbound.service 2>/dev/null)" == active ]] &&
            [[ "$rollback_answer" == 10.1.0.53 ]]; then
            return 0
        fi
        sleep 1
    done
...
rollback() {
    ...
    if [[ "$mutation_started" == false ]]; then
        printf 'action_17k_prewrite_failure_before_mutation=true\n' >&2
        exit "$original_status"
    fi

    set +e
    printf 'action_17k_rollback_started=true\n' >&2
    ...
    if [[ "$rollback_failed" == true ]]; then
        printf 'action_17k_rollback_complete=false\n' >&2
        printf 'manual_intervention_required=true\n' >&2
        exit 125
    fi
    printf 'action_17k_rollback_complete=true\n' >&2
    exit "$original_status"
}
...
if [[ "${1:-}" == --self-test && $# -eq 1 ]]; then
    ...
    [[ "$local_zone_stage_file" == */pihole-local-zone.conf ]]
    [[ "$live_local_zone" != "$local_zone_stage_file" ]]
    printf 'action_17k_node_a_unbound_activation_self_test_complete=true\n'
    exit 0
elif (($#)); then
...
current_boundary=command_preflight
...
current_boundary=accepted_live_baseline
...
assert_equal legacy_primary_metadata \
    "$(stat -c '%U:%G:%a:%s' "$live_primary")" root:root:644:33211
...
current_boundary=protected_stage_validation
...
current_boundary=parser_and_service_preflight
...
printf 'action_17k_preflight_complete=true\n'
printf 'action_17k_live_primary_name=pihole.conf\n'
printf 'action_17k_live_local_zone_name=pihole-local-zone.conf\n'

current_boundary=protected_backup_creation
run_step backup_directory_created \
    install -d -o root -g root -m 0700 "$backup_dir"
mutation_started=true
printf 'action_17k_mutation_started=true\n'
...
current_boundary=transaction_file_creation
...
current_boundary=bounded_reload
reload_attempted=true
printf 'action_17k_reload_attempted=true\n'
run_step unbound_reload systemctl reload unbound.service
printf 'action_17k_reload_status=0\n'
wait_for_readiness

current_boundary=post_reload_continuity
...
current_boundary=stage_and_transaction_continuity
assert_equal primary_stage_preserved \
    "$(file_hash "$primary_stage_file")" "$candidate_primary_sha256"
assert_equal local_zone_stage_preserved \
    "$(file_hash "$local_zone_stage_file")" "$candidate_local_zone_sha256"
...
current_boundary=transaction_commit
transaction_complete=true
trap - EXIT
printf 'action_17k_backup_path=%s\n' "$backup_dir"
printf 'action_17k_primary_sha256=%s\n' "$candidate_primary_sha256"
printf 'action_17k_local_zone_sha256=%s\n' "$candidate_local_zone_sha256"
printf 'action_17k_unbound_pid_preserved=true\n'
printf 'action_17k_ftl_pid_preserved=true\n'
printf 'action_17k_live_names_correct=true\n'
printf 'action_17k_legacy_live_name_absent=true\n'
printf 'action_17k_stages_preserved=true\n'
printf 'action_17k_dns_queries_performed=true\n'
printf 'action_17k_service_mutation=unbound_reload_only\n'
printf 'action_17k_persistent_mutation_scope=two_live_files_and_backup\n'
printf 'action_17k_node_a_unbound_activation_complete=true\n'
