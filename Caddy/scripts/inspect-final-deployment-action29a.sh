...
readonly prefix=action_29a_remote
readonly schema_version=action29a-node-snapshot-v2
...
first_failure=none
action29a_remote_versions=
action29a_remote_web_body=
...
probe_web_endpoint() {
    local action29a_remote_label=$1
    local action29a_remote_fqdn=$2
    local action29a_remote_resolve=$3
    local action29a_remote_expected_ip=$4
    local action29a_remote_status=0
    local action29a_remote_metadata
    local action29a_remote_http_code
    local action29a_remote_effective_url
    local action29a_remote_remote_ip
    local action29a_remote_redirects
    local action29a_remote_body_bytes
    local action29a_remote_body_sha256

    : >"$action29a_remote_web_body" || return 1
    action29a_remote_metadata=$(/usr/bin/curl --noproxy '*' --insecure --silent --show-error \
        --http1.1 --location --max-redirs 3 --proto-redir '=https' \
        --connect-timeout 3 --max-time 10 \
        --resolve "${action29a_remote_fqdn}:443:${action29a_remote_resolve}" \
        --output "$action29a_remote_web_body" \
        --write-out '%{http_code}|%{url_effective}|%{remote_ip}|%{num_redirects}' \
        "https://${action29a_remote_fqdn}/admin/") || action29a_remote_status=$?
    IFS='|' read -r action29a_remote_http_code action29a_remote_effective_url \
        action29a_remote_remote_ip action29a_remote_redirects <<<"$action29a_remote_metadata"
    action29a_remote_body_bytes=$(wc -c <"$action29a_remote_web_body") || return 1
    action29a_remote_body_sha256=$(file_hash "$action29a_remote_web_body") || return 1
    printf '%s_%s_observed_web_%s_status=%s\n' "$prefix" "$node_token" "$action29a_remote_label" "$action29a_remote_status"
    printf '%s_%s_observed_web_%s_http_code=%s\n' "$prefix" "$node_token" "$action29a_remote_label" "$action29a_remote_http_code"
    printf '%s_%s_observed_web_%s_effective_url=%s\n' "$prefix" "$node_token" "$action29a_remote_label" "$action29a_remote_effective_url"
    printf '%s_%s_observed_web_%s_remote_ip=%s\n' "$prefix" "$node_token" "$action29a_remote_label" "$action29a_remote_remote_ip"
    printf '%s_%s_observed_web_%s_redirects=%s\n' "$prefix" "$node_token" "$action29a_remote_label" "$action29a_remote_redirects"
    printf '%s_%s_observed_web_%s_body_bytes=%s\n' "$prefix" "$node_token" "$action29a_remote_label" "$action29a_remote_body_bytes"
    printf '%s_%s_observed_web_%s_body_sha256=%s\n' "$prefix" "$node_token" "$action29a_remote_label" "$action29a_remote_body_sha256"
    check "web_${action29a_remote_label}_status" test "$action29a_remote_status" -eq 0 || return 1
    check "web_${action29a_remote_label}_http_200" test "$action29a_remote_http_code" = 200 || return 1
    check "web_${action29a_remote_label}_effective_url" test "$action29a_remote_effective_url" = \
        "https://${action29a_remote_fqdn}/admin/login.php" || return 1
    check "web_${action29a_remote_label}_remote_ip" test "$action29a_remote_remote_ip" = \
        "$action29a_remote_expected_ip" || return 1
    check "web_${action29a_remote_label}_redirect_once" test "$action29a_remote_redirects" = 1 || return 1
    check "web_${action29a_remote_label}_body_nonempty" test "$action29a_remote_body_bytes" -gt 0 || return 1
    check "web_${action29a_remote_label}_body_bounded" test "$action29a_remote_body_bytes" -le \
        "$maximum_web_body_bytes" || return 1
    check "web_${action29a_remote_label}_body_pihole" grep -Fqi Pi-hole \
        "$action29a_remote_web_body" || return 1
}
manifest_valid() {
...
expected_checks() {
    printf '%s\n' \
        ...
        pihole_ftl_regular pihole_ftl_not_symlink pihole_ftl_hash_exact \
        pihole_ftl_v5_semantics_exact pihole_domain_regular pihole_domain_not_symlink \
        pihole_domain_hash_exact pihole_domain_v5_semantics_exact unbound_zone_hash_exact \
    ...
    while IFS='|' read -r action29a_remote_endpoint _ _ _; do
        printf '%s\n' \
            "web_${action29a_remote_endpoint}_status" \
            "web_${action29a_remote_endpoint}_http_200" \
            "web_${action29a_remote_endpoint}_effective_url" \
            "web_${action29a_remote_endpoint}_remote_ip" \
            "web_${action29a_remote_endpoint}_redirect_once" \
            "web_${action29a_remote_endpoint}_body_nonempty" \
            "web_${action29a_remote_endpoint}_body_bounded" \
            "web_${action29a_remote_endpoint}_body_pihole"
    done < <(endpoint_specs)
}
state_snapshot() {
...
cleanup() {
    local action29a_remote_cleanup_status=$?

    trap - EXIT INT TERM
    if [[ -n ${action29a_remote_versions:-} ]]; then
        rm -f -- "$action29a_remote_versions"
    fi
    if [[ -n ${action29a_remote_web_body:-} ]]; then
        rm -f -- "$action29a_remote_web_body"
    fi
    exit "$action29a_remote_cleanup_status"
}
run_inspection() {
    ...
    local action29a_remote_observed_ftl_sha256
    local action29a_remote_observed_domain_sha256
    local action29a_remote_endpoint
    local action29a_remote_fqdn
    local action29a_remote_resolve
    local action29a_remote_expected_ip

    trap cleanup EXIT INT TERM
    action29_remote_started_ns=$(date +%s%N) || return 1
    action29a_remote_versions=$(mktemp /tmp/action29a-versions.XXXXXX) || return 1
    action29a_remote_web_body=$(mktemp /tmp/action29a-web-body.XXXXXX) || return 1
    ...
    action29a_remote_observed_ftl_sha256=$(file_hash "$pihole_ftl") || return 1
    printf '%s_%s_expected_pihole_ftl_sha256=%s\n' "$prefix" "$node_token" "$pihole_ftl_target_sha256"
    printf '%s_%s_observed_pihole_ftl_sha256=%s\n' "$prefix" "$node_token" "$action29a_remote_observed_ftl_sha256"
    check pihole_ftl_hash_exact test "$action29a_remote_observed_ftl_sha256" = "$pihole_ftl_target_sha256" || return 1
    ...
    action29a_remote_observed_domain_sha256=$(file_hash "$pihole_domain") || return 1
    printf '%s_%s_expected_pihole_domain_sha256=%s\n' "$prefix" "$node_token" "$pihole_domain_target_sha256"
    printf '%s_%s_observed_pihole_domain_sha256=%s\n' "$prefix" "$node_token" "$action29a_remote_observed_domain_sha256"
    check pihole_domain_hash_exact test "$action29a_remote_observed_domain_sha256" = "$pihole_domain_target_sha256" || return 1
    ...
    while IFS='|' read -r action29a_remote_endpoint action29a_remote_fqdn \
        action29a_remote_resolve action29a_remote_expected_ip; do
        probe_web_endpoint "$action29a_remote_endpoint" "$action29a_remote_fqdn" \
            "$action29a_remote_resolve" "$action29a_remote_expected_ip" || return 1
    done < <(endpoint_specs)
    version_inventory >"$action29a_remote_versions" 2>&1 || action29_remote_version_status=$?
    check version_inventory_status test "$action29_remote_version_status" -eq 0 || return 1
    check version_inventory_nonempty test -s "$action29a_remote_versions" || return 1
    check version_inventory_safe safe_version_inventory "$action29a_remote_versions" || return 1
    ...
    sed "s/^/${prefix}_${node_token}_observed_version=/" "$action29a_remote_versions"
