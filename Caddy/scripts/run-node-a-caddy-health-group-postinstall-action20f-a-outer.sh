...
readonly runner="$script_directory/run-node-a-caddy-health-group-postinstall-action20f-a.sh"
readonly regression="$caddy_root/tests/action20f-a-node-a-caddy-health-group-postinstall-regression.sh"
...
case "${1:-}" in
    ...
    --self-test | --source-test | --contract-test)
        [[ $# -eq 1 ]] || exit 64
        run_local_gates
        action20fa_outer_test_kind=${1#--}
        action20fa_outer_test_kind=${action20fa_outer_test_kind//-/_}
        printf '%s_outer_%s_complete=true\n' "$prefix" "$action20fa_outer_test_kind"
        exit 0
        ;;
    "") [[ $# -eq 0 ]] || exit 64 ;;
    *)
...
run_local_gates
working_directory_approved
work_directory=$(mktemp -d /tmp/caddy-action20f-a-outer.XXXXXX)
