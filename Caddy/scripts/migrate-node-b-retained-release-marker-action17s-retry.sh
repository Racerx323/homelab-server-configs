...
expected_check_labels() {
    ...
    printf '%s\n' \
        ...
        action17s_retry_backup_count_zero
...
validate_prestate() {
    ...
    backup_count=$(
        find "$rollback_root" -mindepth 1 -maxdepth 1 \
            -name 'action17s-retry-node-b-marker-migration.*' -printf '.' 2>/dev/null |
            wc -c
    )
    require_check action17s_retry_backup_count_zero test "$backup_count" -eq 0
}
...
validate_prestate
capture_service_state
work_directory=$(mktemp -d /run/caddy-action17s-retry-node-b.XXXXXX)
...
rollback_directory=$(
    mktemp -d "$rollback_root/action17s-retry-node-b-marker-migration.XXXXXX"
)
...
printf '%s_finalizer_invocation_started=true\n' "$prefix"
finalizer_status=0
runuser -u caddy-sync -- "$finalizer" --source-role node-a \
    >"$finalizer_output" 2>"$finalizer_error" || finalizer_status=$?
readonly finalizer_status
emit_stream_evidence finalizer_stdout "$finalizer_output"
emit_stream_evidence finalizer_stderr "$finalizer_error"
...
require_check finalizer_status_zero test "$finalizer_status" -eq 0
...
printf '%s_value_finalizer_status=%s\n' "$prefix" "$finalizer_status"
