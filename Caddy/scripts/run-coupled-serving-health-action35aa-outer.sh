...
readonly prefix=action_35_aa_outer
readonly transaction_sha256=4011f7a3edc27491d603bcec28587824a16a3f202555aa915e129e98f7209415
...
safe_capture() {
    local action35aa_path=$1

    regular_file "$action35aa_path"
    [[ "$(stat -c '%s' "$action35aa_path")" -le "$max_stream_bytes" ]]
    iconv -f UTF-8 -t UTF-8 "$action35aa_path" >/dev/null
    ! LC_ALL=C grep -Paq '[\x00-\x08\x0b\x0c\x0e-\x1f\x7f]' "$action35aa_path"
}

capture() {
    local action35aa_label=$1
    shift
    local action35aa_stdout=$workstation_evidence/$action35aa_label.stdout
    local action35aa_stderr=$workstation_evidence/$action35aa_label.stderr
    local action35aa_status=$workstation_evidence/$action35aa_label.status
    ...
    safe_capture "$action35aa_stdout"
    safe_capture "$action35aa_stderr"
    return "$action35aa_rc"
}

build_payload() {
    local action35aa_repository action35aa_source action35aa_target action35aa_mode
    local action35aa_hash action35aa_lifecycle action35aa_source_path action35aa_destination
    ...
    install -m 0600 "$repository_root/Caddy/manifests/action35aa-node-b-quarantine.tsv" \
        "$payload_stage/manifests/action35aa-node-b-quarantine.tsv"
    while IFS=$'\t' read -r action35aa_repository action35aa_source action35aa_target \
        action35aa_mode action35aa_hash action35aa_lifecycle; do
        [[ "$action35aa_repository" = '# repository' ]] && continue
        [[ "$action35aa_mode" =~ ^0[0-7]{3}$ && "$action35aa_lifecycle" = production-candidate ]]
        [[ "$action35aa_target" = /* ]]
        action35aa_source_path=${workspace_root}/$action35aa_repository/$action35aa_source
        regular_file "$action35aa_source_path"
        [[ "$(sha256sum "$action35aa_source_path" | awk '{ print $1 }')" = "$action35aa_hash" ]]
        action35aa_destination=$payload_stage/repositories/$action35aa_repository/$action35aa_source
        install -d -m 0700 "$(dirname -- "$action35aa_destination")"
        install -m 0600 "$action35aa_source_path" "$action35aa_destination"
...
readonly archive=$2
case "$remote_root" in /tmp/caddy-action35aa-*) ;; *) exit 64 ;; esac
case "$archive" in /tmp/caddy-action35aa-*.tar) ;; *) exit 64 ;; esac
...
case "$evidence_root" in /tmp/caddy-action35aa-*) ;; *) exit 64 ;; esac
...
ssh_stream() {
    local action35aa_host=$1
    local action35aa_program=$2
    shift 2
    "$ssh_command" "$action35aa_host" \
        "cd / && sudo -n /bin/bash -s --$(printf ' %q' "$@")" <"$action35aa_program"
}

upload_payload() {
    local action35aa_role=$1
    local action35aa_host=$2
    local action35aa_remote_root=$3
    local action35aa_remote_archive=$4
    local action35aa_hash

    action35aa_hash=$(awk '{ print $1 }' "$workstation_evidence/payload.sha256")
    capture "$action35aa_role-upload-prepare" ssh_stream "$action35aa_host" \
        "$prepare_program" "$action35aa_remote_root" "$action35aa_remote_archive"
    capture "$action35aa_role-upload-copy" "$scp_command" -p -- \
        "$payload_archive" "$action35aa_host:$action35aa_remote_archive"
    capture "$action35aa_role-upload-accept" ssh_stream "$action35aa_host" \
        "$accept_program" "$action35aa_remote_root" "$action35aa_remote_archive" \
        "$action35aa_hash"
}
...
remote_transaction() {
    ...
    capture "$action35aa_label" ssh_stream "$action35aa_host" "$transaction" "$@"
}

readback() {
    local action35aa_role=$1
    local action35aa_host=$2
    local action35aa_remote_evidence=$3

    capture "$action35aa_role-readback" ssh_stream "$action35aa_host" \
        "$readback_program" "$action35aa_remote_evidence"
}
...
cleanup_remote() {
    ...
    capture "$action35aa_role-disposition" ssh_stream "$action35aa_host" \
        "$disposition_program" "$action35aa_remote_root" "$action35aa_remote_archive"
}

run_live() {
    local action35aa_node_b_mutated=false
    local action35aa_node_a_mutated=false
    local action35aa_failure=0
    local action35aa_phase_status=0
    ...
    if upload_payload node-b "$node_b_host" "$node_b_payload" "$node_b_archive" &&
        ...
        return "$action35aa_phase_status"
    fi

    action35aa_node_b_mutated=true
    ...
    if [[ "$action35aa_failure" -eq 0 ]]; then
    ...
    if [[ "$action35aa_failure" -eq 0 ]]; then
        action35aa_node_a_mutated=true
    ...
    if ! remote_transaction node-a-journal-capture "$node_a_host" journal-capture node-a \
        "$node_a_payload" "$node_a_evidence"; then
        [[ "$action35aa_failure" -ne 0 ]] || action35aa_failure=1
    fi
    ...
    if [[ "$action35aa_failure" -ne 0 ]]; then
        if [[ "$action35aa_node_a_mutated" = true ]]; then
            remote_transaction node-a-rollback "$node_a_host" rollback node-a \
                "$node_a_payload" "$node_a_evidence" || exit 125
        fi
        if [[ "$action35aa_node_b_mutated" = true ]]; then
...
production_path_test() {
    ...
    local action35aa_remote_base action35aa_raw action35aa_decision action35aa_observed
    local action35aa_status action35aa_role action35aa_remote_evidence action35aa_test_id
    local action35aa_tmpfiles_line action35aa_dns_line action35aa_proxy_line
    local action35aa_keepalived_line action35aa_dns_last action35aa_proxy_last
    local retained_root retained_candidate retained_release_hash retained_payload_hash
    local action35aa_transaction_test_evidence
    ...
    action35aa_remote_base=$(mktemp -d /tmp/caddy-action35aa-production-path.XXXXXX)
    action35aa_test_id=${action35aa_remote_base##*.}
    test_remote_base=$action35aa_remote_base
    export ACTION35AA_TEST_REMOTE_BASE=$test_remote_base
...
[[ "$1" = -n ]]
shift
export ACTION35AA_PRODUCTION_PATH_TEST=1
exec "$@"
SUDO
...
printf '%s:%s\n' "$runuser_identity" "$runuser_group" \
    >>"$ACTION35AA_TEST_REMOTE_BASE/runuser.identities"
printf 'runuser %s:%s\n' "$runuser_identity" "$runuser_group" \
    >>"$ACTION35AA_TEST_REMOTE_BASE/ordering.calls"
if [[ "${1:-}" = env && "${2:-}" = -i ]]; then
    shift 2
    exec env -i ACTION35AA_TEST_REMOTE_BASE="$ACTION35AA_TEST_REMOTE_BASE" "$@"
fi
exec "$@"
RUNUSER
...
#!/usr/bin/env bash
set -Eeuo pipefail
printf '%s\n' "$*" >>"$ACTION35AA_TEST_REMOTE_BASE/prohibited-keepalived-parser.calls"
exit 97
KEEPALIVED
...
#!/usr/bin/env bash
set -Eeuo pipefail
printf '%s\n' "$*" >>"$ACTION35AA_TEST_REMOTE_BASE/systemctl.calls"
printf 'systemctl %s\n' "$*" >>"$ACTION35AA_TEST_REMOTE_BASE/ordering.calls"
case "$1" in
    is-active) [[ "$2" = --quiet ]] ;;
    start | stop) [[ $# -eq 2 ]] ;;
...
#!/usr/bin/env bash
set -Eeuo pipefail
[[ "$1" = --create && "$2" = "${ACTION35AA_TARGET_ROOT:?}/etc/tmpfiles.d/caddy-ha.conf" ]]
install -d -m 0755 "$ACTION35AA_TARGET_ROOT/run/caddy-serving-health"
install -d -m 0755 "$ACTION35AA_TARGET_ROOT/run/caddy-serving-health/dns"
install -d -m 0755 "$ACTION35AA_TARGET_ROOT/run/caddy-serving-health/proxy"
install -d -m 0755 "$ACTION35AA_TARGET_ROOT/run/caddy-serving-health/keepalived"
printf '%s\n' "$*" >>"$ACTION35AA_TEST_REMOTE_BASE/tmpfiles.calls"
printf 'tmpfiles %s\n' "$*" >>"$ACTION35AA_TEST_REMOTE_BASE/ordering.calls"
TMPFILES
...
set -Eeuo pipefail
case " $* " in
    *' --show-cursor '*) printf '%s\n' '-- cursor: s=action35aa-production-path' ;;
    *' --after-cursor '*) printf '%s\n' 'test-host caddy.service: action35aa production-path event' ;;
    *) exit 64 ;;
esac
JOURNALCTL
...
shift
command="$*"
printf '%s\t%s\n' "$host" "$command" >>"$ACTION35AA_TEST_REMOTE_BASE/ssh.calls"
PATH="$ACTION35AA_TEST_REMOTE_BASE/bin:/usr/bin:/bin" /bin/bash -c "$command"
SSH
    cat >"$scp_command" <<'SCP'
#!/usr/bin/env bash
set -Eeuo pipefail
...
source=$3
target=${4#*:}
printf '%s\t%s\n' "$source" "$target" >>"$ACTION35AA_TEST_REMOTE_BASE/scp.calls"
install -o "$(id -un)" -g "$(id -gn)" -m 0600 "$source" "$target"
SCP
    chmod 0700 "$ssh_command" "$scp_command" "$test_remote_base/bin/"*
    export ACTION35AA_SLEEP_COMMAND=$test_remote_base/bin/sleep
    ...
    node_a_payload=/tmp/caddy-action35aa-test-$action35aa_test_id-node-a
    node_b_payload=/tmp/caddy-action35aa-test-$action35aa_test_id-node-b
    ...
    ACTION35AA_INCOMING_ROOT=$retained_root/incoming \
        ACTION35AA_OUTGOING_ROOT=$retained_root/outgoing \
        ACTION35AA_QUARANTINE_ROOT=$retained_root/quarantine \
        ACTION35AA_RELEASES_ROOT=$retained_root/releases \
        ACTION35AA_RETAINED_RELEASE_MANIFEST_SHA256=$retained_release_hash \
        ACTION35AA_RETAINED_PAYLOAD_MANIFEST_SHA256=$retained_payload_hash \
    ...
    ACTION35AA_ENVIRONMENT_FILE=$test_remote_base/environment-node-b \
        ACTION35AA_UNBOUND_CHECKCONF_COMMAND=$test_remote_base/bin/unbound-checkconf \
        ACTION35AA_RUNUSER_COMMAND=$test_remote_base/bin/runuser \
        ACTION35AA_SYSTEMCTL_COMMAND=$test_remote_base/bin/systemctl \
        ACTION35AA_DNS_DIG_COMMAND=$test_remote_base/bin/dig \
        ACTION35AA_CURL_COMMAND=$test_remote_base/bin/curl \
        ACTION35AA_SS_COMMAND=$test_remote_base/bin/ss \
    ...
    ACTION35AA_JOURNALCTL_COMMAND=$test_remote_base/bin/journalctl \
    ...
    ACTION35AA_TARGET_ROOT=$test_remote_base/node-b-root \
        ACTION35AA_SYSTEMCTL_COMMAND=$test_remote_base/bin/systemctl \
        ACTION35AA_SYSTEMD_TMPFILES_COMMAND=$test_remote_base/bin/systemd-tmpfiles \
        ACTION35AA_RUNUSER_COMMAND=$test_remote_base/bin/runuser \
        ACTION35AA_DNS_DIG_COMMAND=$test_remote_base/bin/dig \
        ACTION35AA_CURL_COMMAND=$test_remote_base/bin/curl \
        ACTION35AA_SS_COMMAND=$test_remote_base/bin/ss \
        ...
        "$node_b_payload" "$node_b_evidence"
    action35aa_raw=$action35aa_test_root/raw/scheduled-helper-acceptance-before-keepalived-reload.txt
    action35aa_decision=$action35aa_test_root/decisions/scheduled-helper-acceptance-before-keepalived-reload.tsv
    install -m 0600 "$test_remote_base/ordering.calls" "$action35aa_raw"
    action35aa_tmpfiles_line=$(grep -n '^tmpfiles --create ' "$action35aa_raw" | cut -d: -f1)
    [[ "$(grep -c '^runuser pi:default$' "$action35aa_raw")" -eq 5 ]]
    [[ "$(grep -c '^runuser keepalived_script:caddy-tls$' "$action35aa_raw")" -eq 5 ]]
    action35aa_dns_line=$(grep -n '^runuser pi:default$' "$action35aa_raw" | head -n 1 | cut -d: -f1)
    action35aa_proxy_line=$(grep -n '^runuser keepalived_script:caddy-tls$' \
        "$action35aa_raw" | head -n 1 | cut -d: -f1)
    action35aa_dns_last=$(grep -n '^runuser pi:default$' "$action35aa_raw" | tail -n 1 | cut -d: -f1)
    action35aa_proxy_last=$(grep -n '^runuser keepalived_script:caddy-tls$' \
        "$action35aa_raw" | tail -n 1 | cut -d: -f1)
    action35aa_keepalived_line=$(grep -n '^systemctl reload keepalived.service$' \
        "$action35aa_raw" | cut -d: -f1)
    [[ "$action35aa_tmpfiles_line" -lt "$action35aa_dns_line" &&
        "$action35aa_tmpfiles_line" -lt "$action35aa_proxy_line" &&
        "$action35aa_dns_last" -lt "$action35aa_keepalived_line" &&
        "$action35aa_proxy_last" -lt "$action35aa_keepalived_line" ]]
    ...
    write_decision scheduled-helper-acceptance-before-keepalived-reload accept 0 \
        five-overlapping-dns-and-caddy-cycles-before-keepalived \
        five-overlapping-dns-and-caddy-cycles-before-keepalived \
        "$action35aa_raw" "$action35aa_decision"
    action35aa_transaction_test_evidence=$action35aa_test_root/transaction-through-outer
    install -d -m 0700 "$action35aa_transaction_test_evidence"
    CADDY_PRODUCTION_PATH_EVIDENCE_ROOT=$action35aa_transaction_test_evidence \
        ACTION35AA_TEST_REPOSITORY_ROOT=$repository_root \
        remote_transaction node-a-post-promote-production-path "$node_a_host" \
        --production-path-test
    ACTION35AA_TARGET_ROOT=$test_remote_base/node-a-root \
        ACTION35AA_SYSTEMCTL_COMMAND=$test_remote_base/bin/systemctl \
        ACTION35AA_SYSTEMD_TMPFILES_COMMAND=$test_remote_base/bin/systemd-tmpfiles \
        ACTION35AA_RUNUSER_COMMAND=$test_remote_base/bin/runuser \
        ACTION35AA_DNS_DIG_COMMAND=$test_remote_base/bin/dig \
    ...
    action35aa_raw=$action35aa_test_root/raw/outer-preflight.txt
    action35aa_decision=$action35aa_test_root/decisions/outer-preflight.tsv
    cat "$test_remote_base/ssh.calls" "$test_remote_base/scp.calls" \
        ...
        >"$action35aa_raw"
    action35aa_observed=$(wc -l <"$action35aa_raw")
    write_decision outer-preflight reach 0 "$action35aa_observed" \
        "$action35aa_observed" "$action35aa_raw" "$action35aa_decision"

    for action35aa_role in node-a node-b; do
        if [[ "$action35aa_role" = node-a ]]; then
            action35aa_remote_evidence=$node_a_evidence
        else
            action35aa_remote_evidence=$node_b_evidence
        fi
        action35aa_raw=$action35aa_test_root/raw/evidence-readback-$action35aa_role-success.txt
        action35aa_decision=$action35aa_test_root/decisions/evidence-readback-$action35aa_role-success.tsv
        ssh_stream "test-$action35aa_role" "$readback_program" \
            "$action35aa_remote_evidence" >"$action35aa_raw"
        action35aa_observed=$(sha256sum "$action35aa_raw" | awk '{ print $1 }')
        write_decision "evidence-readback-$action35aa_role-success" accept 0 \
            "$action35aa_observed" "$action35aa_observed" "$action35aa_raw" "$action35aa_decision"

        action35aa_raw=$action35aa_test_root/raw/evidence-readback-$action35aa_role-failure.txt
        action35aa_decision=$action35aa_test_root/decisions/evidence-readback-$action35aa_role-failure.tsv
        if ssh_stream "test-$action35aa_role" "$readback_program" \
            "$action35aa_remote_evidence-absent" >"$action35aa_raw" 2>&1; then
            action35aa_status=0
        else
            action35aa_status=$?
        fi
        write_decision "evidence-readback-$action35aa_role-failure" reject \
            "$action35aa_status" present absent "$action35aa_raw" "$action35aa_decision"
    done
    cleanup_remote node-a "$node_a_host" "$node_a_payload" "$node_a_archive"
    cleanup_remote node-b "$node_b_host" "$node_b_payload" "$node_b_archive"
    chmod -R u+rwX -- "$action35aa_remote_base"
    rm -rf -- "$action35aa_remote_base"
...
transaction=$repository_root/Caddy/scripts/apply-coupled-serving-health-action35aa.sh
...
if [[ "$invocation_mode" = --production-path-test ]]; then
    workstation_evidence=$(mktemp -d /tmp/caddy-action35aa-outer-test.XXXXXX)
else
    workstation_evidence=$(mktemp -d /tmp/caddy-ssh-evidence-action35aa.XXXXXX)
fi
readonly workstation_evidence
chmod 0700 "$workstation_evidence"
payload_stage=$workstation_evidence/payload
payload_archive=$workstation_evidence/action35aa-payload.tar
...
readonly run_id
node_a_payload=/tmp/caddy-action35aa-$run_id-node-a
node_b_payload=/tmp/caddy-action35aa-$run_id-node-b
node_a_archive=$node_a_payload.tar
node_b_archive=$node_b_payload.tar
node_a_evidence=$node_a_payload/evidence
node_b_evidence=$node_b_payload/evidence
