...
readonly prefix=action_20d_retry6
readonly transaction_sha256=5ab20bc49a8bd62913388b38b8ddd5f9bdd36d64ad7df046dec2ff8dec38bdce
...
readonly transaction="$script_directory/activate-node-a-caddy-vrrp-action20d-retry6.sh"
...
validate_assertion_inventory() {
    ...
    while IFS= read -r inventory_label; do
        printf '%s_check_%s\n' action_20d_retry6_node "$inventory_label"
    done < <(/bin/bash "$transaction" "$inventory_mode") |
        LC_ALL=C sort >"$inventory_expected"
    sed -n 's/^\(action_20d_retry6_node_check_[a-z0-9_]*\)=true$/\1/p' \
    ...
    ! grep -Eq '^action_20d_retry6_node_check_[a-z0-9_]+=false$' \
...
validate_activation() {
    ...
    validate_assertion_inventory --expected-checks "$activation_transcript" ||
        return 1
    if ! require_one "action_20d_retry6_node_value_node_role=$activation_role" \
        "$activation_transcript"; then return 1; fi
    if ! require_one "action_20d_retry6_node_value_vrrp_state=$activation_expected_state" \
        "$activation_transcript"; then return 1; fi
    if ! require_one "action_20d_retry6_node_value_caddy_ipv4_count=$activation_expected_count" \
        "$activation_transcript"; then return 1; fi
    if ! require_one "action_20d_retry6_node_value_caddy_ipv6_count=$activation_expected_count" \
        "$activation_transcript"; then return 1; fi
    if ! require_one 'action_20d_retry6_node_notification_helper_transition_invocation_expected=true' \
        "$activation_transcript"; then return 1; fi
    if ! require_one 'action_20d_retry6_node_validation_scope=static_candidates_plus_bounded_live_reload' \
        "$activation_transcript"; then return 1; fi
    if ! require_one 'action_20d_retry6_node_production_fragment_installed_unchanged=true' \
        "$activation_transcript"; then return 1; fi
    if ! require_one 'action_20d_retry6_node_health_helper_execution_context=keepalived_script' \
        "$activation_transcript"; then return 1; fi
    if ! require_one 'action_20d_retry6_node_notification_helper_preflight_invoked=false' \
        "$activation_transcript"; then return 1; fi
    if ! require_one 'action_20d_retry6_node_persistent_mutation_scope=main_include,rollback_backup' \
        "$activation_transcript"; then return 1; fi
    if ! require_one 'action_20d_retry6_node_activation_complete=true' \
        "$activation_transcript"; then return 1; fi
    if ! [[ "$(grep -Ec '^action_20d_retry6_node_backup_path=/var/backups/caddy-ha/action20d-retry6-node-a-caddy-vrrp\.[A-Za-z0-9]+$' \
...
validate_inspection() {
    ...
    if ! require_one "action_20d_retry6_node_value_node_role=$inspection_role" \
        "$inspection_transcript"; then return 1; fi
    if ! require_one "action_20d_retry6_node_value_vrrp_state=$inspection_expected_state" \
        "$inspection_transcript"; then return 1; fi
    if ! require_one "action_20d_retry6_node_value_caddy_ipv4_count=$inspection_expected_count" \
        "$inspection_transcript"; then return 1; fi
    if ! require_one "action_20d_retry6_node_value_caddy_ipv6_count=$inspection_expected_count" \
        "$inspection_transcript"; then return 1; fi
    if ! require_one 'action_20d_retry6_node_filesystem_mutations=false' \
        "$inspection_transcript"; then return 1; fi
    if ! require_one 'action_20d_retry6_node_service_mutations=false' \
        "$inspection_transcript"; then return 1; fi
    if ! require_one 'action_20d_retry6_node_vrrp_mutations=false' \
        "$inspection_transcript"; then return 1; fi
    if ! require_one 'action_20d_retry6_node_vip_mutations=false' \
        "$inspection_transcript"; then return 1; fi
    if ! require_one 'action_20d_retry6_node_inspection_complete=true' \
        "$inspection_transcript"; then return 1; fi
    # conditional-validator-explicit-failures-end
}
extract_backup() {
    ...
    [[ "$(grep -c '^action_20d_retry6_node_backup_path=' "$backup_transcript")" -eq 1 ]] ||
        return 1
    backup_value=$(sed -n 's/^action_20d_retry6_node_backup_path=//p' \
        "$backup_transcript") || return 1
    [[ "$backup_value" =~ ^/var/backups/caddy-ha/action20d-retry6-node-a-caddy-vrrp\.[A-Za-z0-9]+$ ]] ||
        return 1
    printf '%s\n' "$backup_value"
}
verify_sources() {
...
rollback_nodes() {
    ...
    if [[ "$node_a_activated" = true && -n "$node_a_backup" ]]; then
        if invoke_remote node_a_rollback node-a --rollback "$node_a_backup"; then
            require_one 'action_20d_retry6_node_explicit_rollback_complete=true' \
...
case "${1:-}" in
    ...
    --contract-test)
        [[ $# -eq 1 ]] || exit 64
        verify_sources
        contract_root=$(mktemp -d /tmp/caddy-action20d-retry6-contract.XXXXXX)
        readonly contract_root
        trap 'rm -rf -- "$contract_root"' EXIT
        : >"$contract_root/empty.stderr"
        while IFS= read -r contract_label; do
            printf 'action_20d_retry6_node_check_%s=true\n' "$contract_label"
        done < <(/bin/bash "$transaction" --expected-checks) \
        >"$contract_root/valid.stdout"
        printf '%s\n' \
            'action_20d_retry6_node_value_node_role=node-a' \
            'action_20d_retry6_node_value_vrrp_state=MASTER' \
            'action_20d_retry6_node_value_caddy_ipv4_count=1' \
            'action_20d_retry6_node_value_caddy_ipv6_count=1' \
            'action_20d_retry6_node_notification_helper_transition_invocation_expected=true' \
            'action_20d_retry6_node_validation_scope=static_candidates_plus_bounded_live_reload' \
            'action_20d_retry6_node_production_fragment_installed_unchanged=true' \
            'action_20d_retry6_node_health_helper_execution_context=keepalived_script' \
            'action_20d_retry6_node_notification_helper_preflight_invoked=false' \
            'action_20d_retry6_node_persistent_mutation_scope=main_include,rollback_backup' \
            'action_20d_retry6_node_backup_path=/var/backups/caddy-ha/action20d-retry6-node-a-caddy-vrrp.FIXTURE' \
            'action_20d_retry6_node_activation_complete=true' \
            >>"$contract_root/valid.stdout"
        work_directory=$contract_root
        validate_activation node-a "$contract_root/valid.stdout" \
            "$contract_root/empty.stderr"
        sed -i '/action_20d_retry6_node_check_root_user=true/d' \
            "$contract_root/valid.stdout"
...
verify_sources
working_directory_approved
work_directory=$(mktemp -d /tmp/caddy-action20d-retry6-runner.XXXXXX)
