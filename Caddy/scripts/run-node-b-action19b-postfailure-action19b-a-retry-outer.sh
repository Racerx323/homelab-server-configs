...
readonly regression_sha256=8414636b0778aad073823282065470edbc0d2922d825822394b9ab09a24411b5
...
readonly derivation="$script_directory/derive-node-b-action19b-postfailure-action19b-a-retry.sh"
readonly regression="$caddy_root/tests/action19b-a-retry-node-b-postfailure-regression.sh"
...
render_stage() {
    ...
    local staged_inspector="$stage_scripts/inspect-node-b-action19b-postfailure-action19b-a-retry.sh"
    local staged_runner="$stage_scripts/run-node-b-action19b-postfailure-action19b-a-retry.sh"
...
case "${1:-}" in
    --self-test | --source-test | --contract-test)
        [[ $# -eq 1 ]] || exit 64
        verify_sources
        test_root=$(mktemp -d /tmp/caddy-action19b-a-retry-outer-test.XXXXXX)
        readonly test_root
...
work_directory=$(mktemp -d /tmp/caddy-action19b-a-retry-outer.XXXXXX)
