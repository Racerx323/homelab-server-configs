...
readonly ssh_binary=${CADDY_ACTION20CA_SSH_BINARY:-/usr/bin/ssh}
...
validate_transcript() {
    ...
    contract_root=$(mktemp -d /tmp/caddy-action20c-a-contract.XXXXXX) || return 97
...
case "${1:-}" in
    ...
    --contract-test)
        [[ $# -eq 1 ]]
        verify_sources
        contract_root=$(mktemp -d /tmp/caddy-action20c-a-runner-contract.XXXXXX)
...
work_directory=$(mktemp -d /tmp/caddy-action20c-a-runner.XXXXXX)
...
remote_status=0
if [[ "${CADDY_ACTION20CA_INTERCEPTED_TEST:-}" = 1 ]]; then
