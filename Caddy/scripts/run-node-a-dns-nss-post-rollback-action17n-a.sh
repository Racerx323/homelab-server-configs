...
readonly inspector_sha256=8a6c8b9608ef691ee56de891bf5359d87192beb07f261e4b7d1f945b3e8f917c
...
readonly expected_dns_value_count=6

script_dir=$(cd -- "$(dirname -- "${BASH_SOURCE[0]}")" && pwd)
readonly script_dir
caddy_root=$(cd -- "$script_dir/.." && pwd)
readonly caddy_root
readonly inspector="$script_dir/inspect-node-a-dns-nss-post-rollback-action17n-a.sh"
...
validate_line_grammar() {
    ...
    ! grep -Ev \
        '^(action_17n_a_(remote_reached|assertion_[a-z0-9_]+|observed_[a-z0-9_]+|value_[a-z0-9_]+|assertion_count|failed_assertion_count|first_failure|before_state_sha256|after_state_sha256|conclusion|remote_complete)|remote_paths_created|dns_queries_performed|peer_connections|synchronization_commands_executed|dns_configuration_mutations|nss_configuration_mutations|service_mutations|persistent_mutations)=' \
        "$grammar_transcript" |
        grep -q .
}
...
validate_structure() {
    ...
    local required_dns_value

    validate_secret_free "$transcript_path" || return 1
    validate_line_grammar "$transcript_path" || return 1
    total_assertions=$(
        grep -Ec '^action_17n_a_assertion_[a-z0-9_]+=(true|false)$' \
            "$transcript_path"
    )
    unique_assertions=$(
        grep -E '^action_17n_a_assertion_[a-z0-9_]+=(true|false)$' \
            "$transcript_path" |
            cut -d= -f1 |
            LC_ALL=C sort -u |
            wc -l
    ...
    false_assertions=$(
        grep -Ec '^action_17n_a_assertion_[a-z0-9_]+=false$' \
            "$transcript_path" || true
    )
    observed_assertions=$(
        grep -Ec '^action_17n_a_observed_[a-z0-9_]+=' \
            "$transcript_path" || true
    )
    ...
    [[ "$(grep -Ec '^action_17n_a_value_[a-z0-9_]+_answer=' \
        "$transcript_path")" -eq "$expected_dns_value_count" ]] || return 1
    for required_dns_value in \
        direct_unbound_peer_aaaa direct_unbound_node_a_aaaa \
        direct_unbound_peer_ptr6 local_pihole_peer_aaaa \
        local_pihole_node_a_aaaa local_pihole_peer_ptr6; do
        value_for \
            "action_17n_a_value_${required_dns_value}_answer" \
            "$transcript_path" |
            grep -Eq '^[0-9A-Za-z:._,-]+$' || return 1
    done

    reported_assertions=$(value_for action_17n_a_assertion_count "$transcript_path")
    reported_failures=$(
        value_for action_17n_a_failed_assertion_count "$transcript_path"
    )
    reported_first_failure=$(
        value_for action_17n_a_first_failure "$transcript_path"
    )
    ...
    if [[ "$false_assertions" -eq 0 ]]; then
        ...
        require_value action_17n_a_conclusion \
            post_rollback_state_verified "$transcript_path" || return 1
    else
        expected_first_failure=$(
            grep -E '^action_17n_a_assertion_[a-z0-9_]+=false$' \
                "$transcript_path" |
                head -n 1 |
                sed 's/^action_17n_a_assertion_//; s/=false$//'
        )
        [[ "$reported_first_failure" == "$expected_first_failure" ]] ||
            return 1
        require_value action_17n_a_conclusion \
            post_rollback_state_mismatch "$transcript_path" || return 1
    fi

    require_value action_17n_a_remote_reached true "$transcript_path" ||
        return 1
    require_value action_17n_a_remote_complete true "$transcript_path" ||
        return 1
    ...
    before_state=$(value_for action_17n_a_before_state_sha256 "$transcript_path")
    after_state=$(value_for action_17n_a_after_state_sha256 "$transcript_path")
...
write_fixture() {
    ...
    local conclusion=post_rollback_state_verified
    ...
    for ((fixture_index = 1; fixture_index <= expected_assertion_count; fixture_index += 1)); do
        ...
        if [[ "$false_label" == "fixture_${fixture_index}" ]]; then
            ...
            conclusion=post_rollback_state_mismatch
        fi
        printf 'action_17n_a_assertion_fixture_%02d=%s\n' \
            "$fixture_index" "$fixture_value"
        if [[ "$fixture_value" == false ]]; then
            printf 'action_17n_a_observed_fixture_%02d=mismatch\n' \
                "$fixture_index"
        fi
    done >>"$fixture_path"
    printf '%s\n' \
        action_17n_a_value_direct_unbound_peer_aaaa_answer=none \
        action_17n_a_value_direct_unbound_node_a_aaaa_answer=none \
        action_17n_a_value_direct_unbound_peer_ptr6_answer=none \
        action_17n_a_value_local_pihole_peer_aaaa_answer=none \
        action_17n_a_value_local_pihole_node_a_aaaa_answer=none \
        action_17n_a_value_local_pihole_peer_ptr6_answer=pi.hole \
        "action_17n_a_assertion_count=$expected_assertion_count" \
        "action_17n_a_failed_assertion_count=$failed_count" \
        "action_17n_a_first_failure=$first_failure" \
        action_17n_a_before_state_sha256=aaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaa \
        action_17n_a_after_state_sha256=aaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaa \
        ...
        "action_17n_a_conclusion=$conclusion" \
        action_17n_a_remote_complete=true \
        >>"$fixture_path"
}
...
if [[ "${1:-}" == --self-test && $# -eq 1 ]]; then
    ...
    printf 'action_17n_a_runner_self_test_complete=true\n'
    exit 0
elif [[ "${1:-}" == --source-test && $# -eq 1 ]]; then
    verify_live_sources
    printf 'action_17n_a_runner_source_test_complete=true\n'
    exit 0
elif [[ "${1:-}" == --contract-test && $# -eq 1 ]]; then
    contract_directory=$(mktemp -d /tmp/caddy-action17n-a-contract.XXXXXX)
    ...
    printf 'action_17n_a_assertion_fixture_01=true\n' >>"$duplicate_fixture"
    set +e
    ...
    printf 'action_17n_a_runner_contract_test_complete=true\n'
    exit 0
elif (($#)); then
...
verify_live_sources
work_directory=$(mktemp -d /tmp/caddy-action17n-a.XXXXXX)
...
if [[ "$classification_status" -eq 0 ||
    ...
    printf 'action_17n_a_ssh_status=%s\n' "$ssh_status"
    printf 'action_17n_a_runner_classification=%s\n' \
        "$(if [[ "$classification_status" -eq 0 ]]; then
            printf verified
        else
            printf semantic_mismatch
        fi)"
    printf 'action_17n_a_workstation_cleanup=true\n'
    exit "$classification_status"
fi

printf 'action_17n_a_ssh_status=%s\n' "$ssh_status" >&2
printf 'action_17n_a_remote_stdout_sha256=%s\n' \
    "$(file_hash "$remote_output_path")" >&2
printf 'action_17n_a_remote_stderr_sha256=%s\n' \
    "$(file_hash "$remote_error_path")" >&2
printf 'action_17n_a_runner_classification=evidence_failure\n' >&2
printf 'action_17n_a_workstation_cleanup=true\n' >&2
exit 97
