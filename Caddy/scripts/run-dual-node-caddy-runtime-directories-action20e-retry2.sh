...
validate_success() {
    ...
    observed_count=$(grep -Ec '^action_20e_retry2_node_check_[a-z0-9_]+=true$' "$output_path" || true)
    unique_count=$(sed -n 's/^\(action_20e_retry2_node_check_[a-z0-9_]*\)=true$/\1/p' "$output_path" | LC_ALL=C sort -u | wc -l)
    [[ "$observed_count" -eq "$expected_count" ]] || return 1
    [[ "$unique_count" -eq "$expected_count" ]] || return 1
    diff -u \
        <(/bin/bash "$installer" --expected-checks | sed 's/^/action_20e_retry2_node_check_/' | LC_ALL=C sort) \
        <(sed -n 's/^\(action_20e_retry2_node_check_[a-z0-9_]*\)=true$/\1/p' "$output_path" | LC_ALL=C sort) >/dev/null || return 1
    [[ "$(grep -Ec '^action_20e_retry2_node_check_[a-z0-9_]+=false$' "$output_path" || true)" -eq 0 ]] || return 1
    grep -Fxq 'action_20e_retry2_node_install_complete=true' "$output_path" || return 1
    grep -Fxq 'action_20e_retry2_node_keepalived_mutated=false' "$output_path" || return 1
    grep -Fxq 'action_20e_retry2_node_service_mutations=false' "$output_path" || return 1
    grep -Fxq 'action_20e_retry2_node_vrrp_mutated=false' "$output_path" || return 1
    grep -Fxq 'action_20e_retry2_node_vip_mutated=false' "$output_path" || return 1
    grep -Fxq 'action_20e_retry2_node_notifier_invoked=false' "$output_path" || return 1
    [[ "$(grep -Ec '^action_20e_retry2_node_backup_path=/var/backups/caddy-ha/action20e-node-[ab]-runtime-directories\.[A-Za-z0-9]+$' "$output_path" || true)" -eq 1 ]]
}
extract_backup_path() {
    local output_path=$1

    sed -n 's/^action_20e_retry2_node_backup_path=//p' "$output_path"
}
run_node() {
    local node_role=$1
    local node_target=$2
...
verify_sources
working_directory_approved
work_directory=$(mktemp -d /tmp/caddy-action20e-retry2-runner.XXXXXX)
...
if ! validate_success "$node_a_stdout" "$node_a_status"; then
    ...
    grep -Fxq 'action_20e_retry2_node_rollback_complete=true' "$rollback_stdout"
