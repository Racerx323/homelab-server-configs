...
readonly prefix=action_28j_c_route
...
safe_text() {
    local action28j_c_route_text=$1

    [[ ${#action28j_c_route_text} -le 131072 ]] || return 1
    ! printf '%s' "$action28j_c_route_text" | LC_ALL=C grep -q '[^[:print:][:space:]]'
}
record_check() {
    local action28j_c_route_label=$1

    shift
    if "$@" >/dev/null 2>&1; then
        printf '%s_check_%s=true\n' "$prefix" "$action28j_c_route_label"
        return 0
    fi
    printf '%s_check_%s=false\n' "$prefix" "$action28j_c_route_label"
    failed_check_count=$((failed_check_count + 1))
    [[ "$first_failure" != none ]] || first_failure=$action28j_c_route_label
    return 0
}
run_query() {
    local action28j_c_route_type=$1

    timeout 10 dig @127.0.0.1 "$fqdn" "$action28j_c_route_type" +short
}
run_https() {
    local action28j_c_route_family=$1
    local action28j_c_route_resolve_entry
    local action28j_c_route_curl_program=curl

    case "$action28j_c_route_family" in
        --ipv4) action28j_c_route_resolve_entry="$fqdn:443:$expected_ipv4" ;;
        --ipv6) action28j_c_route_resolve_entry="$fqdn:443:[$expected_ipv6]" ;;
        *) return 64 ;;
    esac
    if [[ "${CADDY_ACTION28J_C_TEST_MODE:-}" = 1 ]]; then
        action28j_c_route_curl_program=${CADDY_ACTION28J_C_CURL_PROGRAM:-curl}
    fi
    timeout 15 "$action28j_c_route_curl_program" --silent --show-error \
        --location --max-redirs 3 --resolve "$action28j_c_route_resolve_entry" \
        --proto '=https' --proto-redir '=https' --http1.1 "$action28j_c_route_family" \
        --connect-timeout 3 --max-time 10 --output /dev/null \
        --write-out '%{http_code}\t%{url_effective}\t%{remote_ip}\t%{num_redirects}\n' \
        "https://$fqdn/admin/"
}
parse_field() {
    local action28j_c_route_value=$1
    local action28j_c_route_field=$2

    printf '%s\n' "$action28j_c_route_value" | awk -F '\t' -v field="$action28j_c_route_field" 'NR == 1 { print $field }'
}
...
case "${1:-}" in
    ...
    --self-test)
        ...
        ;;
    --test-run-https)
        [[ $# -eq 2 && "${CADDY_ACTION28J_C_TEST_MODE:-}" = 1 ]] || exit 64
        case "$2" in
            ipv4) run_https --ipv4 ;;
            ipv6) run_https --ipv6 ;;
            *) exit 64 ;;
