...
readonly caddy_root
readonly installer="$script_directory/install-node-b-protocol-v2-action17q-retry.sh"
...
write_remote_bundle() {
    ...
    # shellcheck disable=SC1003,SC2016
    {
        printf '%s\n' \
            ...
            'export PATH' \
            'cd /' \
            'bundle_stage=$(mktemp -d /run/caddy-action17q-retry-stage.XXXXXX)' \
            'cleanup_bundle_stage() {' \
            '    rm -rf -- "$bundle_stage"' \
            '}' \
            'trap cleanup_bundle_stage EXIT' \
            'install -d -o root -g root -m 0700 "$bundle_stage/payload"' \
            'base64 -d >"$bundle_stage/payload.tar" <<'\''ACTION17Q_RETRY_ARCHIVE'\'''
        base64 "$archive_source"
        printf '%s\n' \
            'ACTION17Q_RETRY_ARCHIVE' \
            ...
            'chmod 0700 \' \
            '    "$bundle_stage/payload/install-node-b-protocol-v2-action17q-retry.sh"' \
            'cd /' \
            '/bin/bash \' \
            '    "$bundle_stage/payload/install-node-b-protocol-v2-action17q-retry.sh" \' \
...
validate_success() {
    ...
    [[ ! -s "$error_transcript" ]]
    if grep -Eq \
        '^action_17q_retry_check_[^=]+=false$|^action_17q_retry_(receiver_invoked|finalizer_invoked|release_mutated|lsyncd_enabled|reconciliation_enabled|service_mutations)=true$' \
    ...
    for success_marker in \
        ...
        action_17q_retry_node_b_protocol_v2_install_complete=true; do
        require_exact_line "$success_marker" "$output_transcript" || return 1
    done
    [[ "$(grep -Ec \
        '^action_17q_retry_backup_path=/var/backups/caddy-ha/action17q-retry-node-b-protocol-v2\.[A-Za-z0-9]+$' \
        "$output_transcript")" -eq 1 ]]
    if grep -Eq \
        'action_17q_retry_rollback_|manual_intervention_required=true' \
...
validate_failure() {
    ...
    if grep -Fq 'manual_intervention_required=true' \
        "$output_transcript" "$error_transcript" ||
        grep -Fq 'action_17q_retry_rollback_complete=false' \
            "$output_transcript" "$error_transcript"; then
        return 97
    fi
    if grep -Fq 'action_17q_retry_mutation_started=true' "$output_transcript"; then
        require_exact_line \
            action_17q_retry_rollback_started=true "$error_transcript" || return 97
        require_exact_line \
            action_17q_retry_rollback_complete=true "$error_transcript" || return 97
    elif grep -Eq \
        'action_17q_retry_rollback_' "$output_transcript" "$error_transcript"; then
        return 97
    fi
}
...
case "${1:-}" in
    --self-test)
        ...
        printf 'action_17q_retry_runner_self_test_complete=true\n'
        exit 0
        ;;
    --source-test)
        [[ $# -eq 1 ]]
        verify_sources
        printf 'action_17q_retry_runner_source_test_complete=true\n'
        exit 0
        ;;
    --contract-test)
        [[ $# -eq 1 ]]
        contract_directory=$(mktemp -d /tmp/caddy-action17q-retry-contract.XXXXXX)
        ...
        printf 'action_17q_retry_finalizer_invoked=true\n' \
        ...
        printf '%s\n' \
            action_17q_retry_rollback_started=true \
            action_17q_retry_rollback_complete=true \
            >"$contract_directory/rollback-error"
        validate_failure \
            "$contract_directory/rollback-error" \
            "$contract_directory/rollback-output" 1
        printf 'action_17q_retry_runner_contract_test_complete=true\n'
        exit 0
        ;;
    "") ;;
    *)
...
verify_sources
work_directory=$(mktemp -d /tmp/caddy-action17q-retry.XXXXXX)
...
finish() {
    ...
    if [[ -e "$work_directory" || -L "$work_directory" ]]; then
        printf 'action_17q_retry_workstation_cleanup_complete=false\n' >&2
        exit 97
    fi
    printf 'action_17q_retry_workstation_cleanup_complete=true\n'
    exit "$finish_status"
}

install -d -m 0700 "$payload_directory"
install -m 0700 "$installer" \
    "$payload_directory/install-node-b-protocol-v2-action17q-retry.sh"
...
tar --create --file "$archive_path" --directory "$payload_directory" \
    install-node-b-protocol-v2-action17q-retry.sh \
...
cat "$output_path"
cat "$error_path" >&2
printf 'action_17q_retry_ssh_status=%s\n' "$ssh_status"
if ! validate_secret_free "$error_path" "$output_path"; then
    printf 'Unsafe Action 17q retry output detected.\n' >&2
    finish 97
fi
if [[ "$ssh_status" -eq 0 ]]; then
    if ! validate_success "$error_path" "$output_path" "$ssh_status"; then
        printf 'Action 17q retry success contract failed.\n' >&2
        finish 97
    fi
    printf 'action_17q_retry_runner_acceptance=true\n'
    finish 0
fi
...
if [[ "$failure_validation_status" -eq 97 ]]; then
    printf 'Action 17q retry rollback evidence is incomplete.\n' >&2
    finish 97
fi
printf 'action_17q_retry_runner_acceptance=false\n'
finish "$ssh_status"
