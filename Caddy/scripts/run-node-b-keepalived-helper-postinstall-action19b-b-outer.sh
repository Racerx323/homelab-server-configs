...
readonly prefix=action_19b_b
readonly derivation_sha256=7ba7ca096db09c48b57c32ae62bf300b3aa16aa6b6f8d4cf033d83624395dd1b
readonly regression_sha256=daa584d9af999fe7bfd839dde58f190b75a44b1e9b445f7132369e3406b980fc
...
readonly derivation="$script_directory/derive-node-b-keepalived-helper-postinstall-action19b-b.sh"
readonly regression="$caddy_root/tests/action19b-b-node-b-keepalived-helper-postinstall-regression.sh"
...
render_stage() {
    ...
    local staged_inspector=$stage_scripts/inspect-node-b-keepalived-helper-postinstall-action19b-b.sh
    local staged_runner=$stage_scripts/run-node-b-keepalived-helper-postinstall-action19b-b.sh
    ...
    [[ "$(file_hash "$staged_inspector")" = "$rendered_inspector_sha256" ]] || return 1
...
case "${1:-}" in
    --self-test | --source-test | --contract-test)
        [[ $# -eq 1 ]] || exit 64
        verify_sources
        test_root=$(mktemp -d /tmp/caddy-action19b-b-outer-test.XXXXXX)
        readonly test_root
...
work_directory=$(mktemp -d /tmp/caddy-action19b-b-outer.XXXXXX)
