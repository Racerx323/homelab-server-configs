...
readonly prefix=action_20h_outer
...
readonly installer=$script_directory/install-node-a-caddy-health-helper-action20h.sh
readonly stager=$script_directory/stage-node-a-caddy-health-helper-action20h.sh
readonly runner=$script_directory/run-node-a-caddy-health-helper-action20h.sh
readonly regression=$caddy_root/tests/action20h-health-helper-regression.sh
...
source_exact() {
    local action20h_outer_expected_hash=$1
    local action20h_outer_source_path=$2

    [[ -f "$action20h_outer_source_path" && ! -L "$action20h_outer_source_path" ]] || return 1
    [[ "$(stat -c '%a' "$action20h_outer_source_path")" = 755 ]] || return 1
    [[ "$(file_hash "$action20h_outer_source_path")" = "$action20h_outer_expected_hash" ]] || return 1
}
gate() {
    local action20h_outer_gate_label=$1

    shift
    if "$@" >/dev/null; then
        printf '%s_outer_gate_%s=true\n' "$prefix" "$action20h_outer_gate_label"
        return 0
    fi
    printf '%s_outer_gate_%s=false\n' "$prefix" "$action20h_outer_gate_label" >&2
    return 1
}
safe_stream() {
    local action20h_outer_stream_path=$1

    [[ "$(wc -c <"$action20h_outer_stream_path")" -le "$maximum_stream_bytes" ]] || return 1
    [[ "$(line_count "$action20h_outer_stream_path")" -le "$maximum_stream_lines" ]] || return 1
    ! LC_ALL=C grep -n '[^[:print:][:space:]]' "$action20h_outer_stream_path" >/dev/null || return 1
    ! grep -Eqi \
        'BEGIN [A-Z ]*PRIVATE KEY|CADDY_TLS_PRIVATE_KEY_PEM|PRIVATE_KEY=|DOPPLER_TOKEN|Authorization:[[:space:]]*Bearer' \
        "$action20h_outer_stream_path" || return 1
}
emit_stream() {
    local action20h_outer_stream_label=$1
    local action20h_outer_stream_path=$2

    printf '%s_%s_bytes=%s\n' "$prefix" "$action20h_outer_stream_label" \
        "$(wc -c <"$action20h_outer_stream_path")"
    printf '%s_%s_lines=%s\n' "$prefix" "$action20h_outer_stream_label" \
        "$(line_count "$action20h_outer_stream_path")"
    printf '%s_%s_sha256=%s\n' "$prefix" "$action20h_outer_stream_label" \
        "$(file_hash "$action20h_outer_stream_path")"
    if ! safe_stream "$action20h_outer_stream_path"; then
        printf '%s_%s_classification=unsafe_retained\n' "$prefix" \
            "$action20h_outer_stream_label" >&2
        return 97
    fi
    printf '%s_%s_classification=bounded_safe\n' "$prefix" "$action20h_outer_stream_label"
    if [[ -s "$action20h_outer_stream_path" ]]; then
        printf '%s_%s_begin\n' "$prefix" "$action20h_outer_stream_label"
        cat "$action20h_outer_stream_path"
        printf '%s_%s_end\n' "$prefix" "$action20h_outer_stream_label"
    else
...
run_local_gates
work_root=$(mktemp -d /tmp/caddy-action20h-outer.XXXXXX)
readonly work_root
