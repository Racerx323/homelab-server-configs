...
readonly validation_outer_sha256=30830a62bbb03e048b9edd79eb20a5d3b3229c1284b9776aa8966cbc8f82913a
readonly readiness_outer_sha256=b7e1db77b4889a62d782a0331922f326edd73c87e13a42952441ad7fe9ce9f20
readonly activation_outer_sha256=fda34fd7c00ea88539132a29d39d553e4c203bc08cfb0067e7c6c111a130361a
readonly regression_sha256=755e34e3c5742dfb3b893792972f7df690a8389c742cbc76feae47dc45e17b33
...
readonly validation_outer=$script_directory/run-node-a-caddy-health-action20d-retry7-a-retry-outer.sh
readonly readiness_outer=$script_directory/run-dual-node-caddy-notifier-context-action20d-c-retry-outer.sh
readonly activation_outer=$script_directory/run-node-a-caddy-vrrp-activation-action20d-retry8.sh
readonly regression=$caddy_root/tests/action20d-retry8-activation-boundary-regression.sh
...
run_local_gates() {
    ...
    run_gate validation_outer_hash require_hash "$validation_outer_sha256" "$validation_outer" || return 1
    ...
    run_gate syntax /bin/bash -n "$validation_outer" "$readiness_outer" "$activation_outer" \
        "$regression" "$0" || return 1
    run_gate collision_policy /bin/bash "$collision" "$validation_outer" "$readiness_outer" \
    ...
    run_gate validation_outer_self_test /bin/bash "$validation_outer" --self-test || return 1
...
run_boundary() {
    ...
    boundary_root=$(mktemp -d /tmp/caddy-action20d-retry8-boundary.XXXXXX)
    ...
    run_captured_gate validation "$validation_command" "$boundary_root" || validation_status=$?
    ...
    printf '%s_validation_accepted=true\n' "$prefix"
...
case "${1:-}" in
    --expected-local-gates)
        ...
        printf '%s\n' working_directory readiness_outer_hash \
            validation_outer_hash activation_outer_hash regression_hash collision_hash conditional_hash \
            output_evidence_hash syntax collision_policy conditional_policy \
            output_evidence_policy validation_outer_self_test readiness_outer_self_test \
    ...
    "")
        [[ $# -eq 0 ]] || exit 64
        run_local_gates
        run_boundary "$validation_outer" "$readiness_outer" "$activation_outer"
        ;;
    *)
        printf 'Usage: %s [--expected-local-gates|--production-path-test VALIDATION READINESS ACTIVATION|--self-test|--source-test|--contract-test]\n' \
            "${0##*/}" >&2
        exit 64
        ;;
esac
