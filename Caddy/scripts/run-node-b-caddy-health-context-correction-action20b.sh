...
readonly installer="$script_directory/install-node-b-caddy-health-context-action20b.sh"
...
verify_sources() {
    ...
    [[ "$(/bin/bash "$installer" --expected-check-count)" -eq 65 ]] || return 1
}
safe_stream() {
...
validate_success() {
    ...
    [[ "$(grep -Ec \
        "^${prefix}_backup_path=/var/backups/caddy-ha/action20b-node-b-health-context\.[A-Za-z0-9]+$" \
...
write_remote_bundle() {
    ...
    # Remote variables expand only on Node B.
    # shellcheck disable=SC2016
    {
        ...
        printf '%s\n' \
            ...
            'cd /' \
            '/bin/bash "$bundle_stage/payload/install-node-b-caddy-health-context-action20b.sh" --stage "$bundle_stage/payload"'
    } >"$remote_bundle_path"
...
write_success_fixture() {
    ...
    printf '%s\n' \
        ...
        "${prefix}_vrrp_mutated=false" \
        "${prefix}_vip_mutated=false" \
        "${prefix}_backup_path=/var/backups/caddy-ha/action20b-node-b-health-context.ABC123" \
        "${prefix}_persistent_mutation_scope=health_helper,rollback_backup" \
...
case "${1:-}" in
    ...
    --contract-test)
        [[ $# -eq 1 ]] || exit 64
        verify_sources
        contract_root=$(mktemp -d /tmp/caddy-action20b-contract.XXXXXX)
...
verify_sources
working_directory_approved
work_directory=$(mktemp -d /tmp/caddy-action20b-runner.XXXXXX)
...
tar --create --file "$archive_path" --directory "$script_directory" \
    ...
    install-node-b-caddy-health-context-action20b.sh
