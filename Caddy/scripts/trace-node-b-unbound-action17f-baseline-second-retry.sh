...
report_exact_failure() {
    local failure_status=$?

    printf 'exact_assertion_%s=false\n' "$current_assertion" >&2
    printf 'exact_assertion_%s_status=%s\n' \
        "$current_assertion" "$failure_status" >&2
}
...
run_labeled_validation() {
    ...
    (
        set -Eeuo pipefail
        trap report_exact_failure ERR
        "$validation_function"
        trap - ERR
        printf 'action_17f_b_second_retry_exact_baseline_complete=true\n'
    )
}
...
validate_baseline_labeled() {
    ...
    mark_passed
    printf 'observed_live_state_sha256=%s\n' "$state_hash"
    printf 'expected_live_state_sha256=%s\n' "$accepted_live_state_sha256"
    current_assertion=baseline_live_state_hash
    [[ "$state_hash" == "$accepted_live_state_sha256" ]]
    mark_passed
}

failure_regression_probe() {
    current_assertion=regression_function_internal_failure
    false
    mark_passed
}
...
case "${1:-}" in
    --self-test)
        ...
        printf 'action_17f_b_second_retry_trace_self_test_complete=true\n'
        exit 0
        ;;
    --failure-regression)
        (($# == 1))
        run_labeled_validation failure_regression_probe
        regression_status=$?
        printf 'action_17f_b_second_retry_failure_regression_status=%s\n' \
            "$regression_status"
        exit "$regression_status"
        ;;
    "") ;;
    *)
        printf 'Usage: %s [--self-test|--failure-regression]\n' \
            "${0##*/}" >&2
        exit 2
        ;;
esac

printf 'action_17f_b_second_retry_trace_remote_reached=true\n'
run_labeled_validation validate_baseline_labeled
exact_status=$?

printf 'action_17f_b_second_retry_exact_baseline_status=%s\n' "$exact_status"
...
printf 'action_17f_b_second_retry_labeled_trace_complete=true\n'
