...
readonly prefix=action_30c_remote
readonly backup_directory=/var/backups/caddy-ha/action30c-systemd-boot-persistence
readonly evidence_root=/tmp/caddy-action30c
...
readonly lsyncd_status_file=${CADDY_ACTION30C_STATUS_FILE:-/run/caddy-lsyncd/status}
...
safe_stream() {
    local action30c_stream=$1

    [[ "$(wc -c <"$action30c_stream")" -le 1048576 ]] || return 1
    [[ "$(line_count "$action30c_stream")" -le 8192 ]] || return 1
    iconv -f UTF-8 -t UTF-8 "$action30c_stream" >/dev/null 2>&1 || return 1
    ! LC_ALL=C.UTF-8 grep -n '[^[:print:][:space:]]' "$action30c_stream" >/dev/null || return 1
    ! grep -Eqi 'BEGIN [A-Z ]*PRIVATE KEY|Authorization:[[:space:]]*Bearer|WEBPASSWORD' "$action30c_stream"
}
emit_stream() {
    local action30c_label=$1
    local action30c_stream=$2

    printf '%s_%s_%s_bytes=%s\n' "$prefix" "$node_token" "$action30c_label" "$(wc -c <"$action30c_stream")"
    printf '%s_%s_%s_lines=%s\n' "$prefix" "$node_token" "$action30c_label" "$(line_count "$action30c_stream")"
    printf '%s_%s_%s_sha256=%s\n' "$prefix" "$node_token" "$action30c_label" "$(file_hash "$action30c_stream")"
    safe_stream "$action30c_stream" || return 97
    printf '%s_%s_%s_classification=bounded_safe\n' "$prefix" "$node_token" "$action30c_label"
    if [[ -s "$action30c_stream" ]]; then
        printf '%s_%s_%s_begin\n' "$prefix" "$node_token" "$action30c_label"
        cat "$action30c_stream"
        printf '%s_%s_%s_end\n' "$prefix" "$node_token" "$action30c_label"
    else
...
check() {
    ...
    printf '%s_%s_check_%s=false\n' "$prefix" "$node_token" "$action30c_label" >&2
    return 1
}
run_captured() {
    local action30c_label=$1
    local action30c_stdout=$capture_directory/$action30c_label.stdout
    local action30c_stderr=$capture_directory/$action30c_label.stderr
    local action30c_status_file=$capture_directory/$action30c_label.status
    local action30c_status=0

    shift
    install -m 0600 /dev/null "$action30c_stdout" || return 1
    install -m 0600 /dev/null "$action30c_stderr" || return 1
    install -m 0600 /dev/null "$action30c_status_file" || return 1
    "$@" >"$action30c_stdout" 2>"$action30c_stderr" || action30c_status=$?
    printf '%s\n' "$action30c_status" >"$action30c_status_file"
    printf '%s_%s_%s_status=%s\n' "$prefix" "$node_token" "$action30c_label" "$action30c_status"
    emit_stream "${action30c_label}_stdout" "$action30c_stdout" || return $?
    emit_stream "${action30c_label}_stderr" "$action30c_stderr" || return $?
    [[ "$action30c_status" -eq 0 ]]
}
cursor_from_capture() {
    local action30c_label=$1
    local action30c_capture=$capture_directory/$action30c_label.stdout
...
capture_cursor() {
    ...
    run_captured "$action30c_label" journalctl --show-cursor -n 0 --no-pager || return 1
    cursor_from_capture "$action30c_label" >/dev/null
}
manifest_paths_safe() {
    local action30c_manifest=$1
...
manifest_file_set_matches() {
    local action30c_release=$1
    local action30c_expected
    local action30c_observed
    local action30c_status=0

    action30c_expected=$(mktemp /tmp/action30c-manifest-expected.XXXXXX) || return 1
    action30c_observed=$(mktemp /tmp/action30c-manifest-observed.XXXXXX) || {
        rm -f -- "$action30c_expected"
        return 1
    }
    awk '{ print substr($0, 67) }' "$action30c_release/manifest.sha256" |
        LC_ALL=C sort -u >"$action30c_expected"
    (
        cd "$action30c_release"
        find . -type f \
    ...
    cmp -s "$action30c_expected" "$action30c_observed" || action30c_status=$?
    rm -f -- "$action30c_expected" "$action30c_observed"
    [[ "$action30c_status" -eq 0 ]]
}
manifest_valid() {
    local action30c_release=$1

    [[ -d "$action30c_release" && ! -L "$action30c_release" ]] || return 1
    [[ -f "$action30c_release/release-manifest.json" &&
        ! -L "$action30c_release/release-manifest.json" ]] || return 1
    [[ -f "$action30c_release/manifest.sha256" &&
        ! -L "$action30c_release/manifest.sha256" ]] || return 1
    [[ -z "$(find "$action30c_release" -type l -print -quit)" ]] || return 1
    [[ -z "$(find "$action30c_release" ! -type d ! -type f -print -quit)" ]] || return 1
    [[ -z "$(find "$action30c_release" -type f -links +1 -print -quit)" ]] || return 1
    jq -e '
    ...
    ' "$action30c_release/release-manifest.json" >/dev/null || return 1
    manifest_paths_safe "$action30c_release/manifest.sha256" || return 1
    manifest_file_set_matches "$action30c_release" || return 1
    (cd "$action30c_release" &&
        sha256sum --strict --check manifest.sha256 >/dev/null 2>&1)
}
release_identity_exact() {
    local action30c_release=$1
    local action30c_revision=$2
    local action30c_parent=$3
    local action30c_source=$4

    manifest_valid "$action30c_release" || return 1
    [[ "$(jq -r '.revision // empty' "$action30c_release/release-manifest.json")" = "$action30c_revision" ]] || return 1
    [[ "$(jq -r '.parent_revision // empty' "$action30c_release/release-manifest.json")" = "$action30c_parent" ]] || return 1
    [[ "$(jq -r '.source_node // empty' "$action30c_release/release-manifest.json")" = "$action30c_source" ]]
}
current_release_inventory() {
    local action30c_resolved
    local action30c_revision
    local action30c_parent
    local action30c_source

    [[ -L "$current_link" ]] || return 1
    action30c_resolved=$(readlink -f -- "$current_link") || return 1
    [[ "$action30c_resolved" = "$releases_root/$accepted_current_revision" ]] || return 1
    manifest_valid "$action30c_resolved" || return 1
    action30c_revision=$(jq -r '.revision // empty' "$action30c_resolved/release-manifest.json") || return 1
    action30c_parent=$(jq -r '.parent_revision // empty' "$action30c_resolved/release-manifest.json") || return 1
    action30c_source=$(jq -r '.source_node // empty' "$action30c_resolved/release-manifest.json") || return 1
    [[ "$action30c_revision" = "$accepted_current_revision" ]] || return 1
    [[ "$action30c_source" = node-a ]] || return 1
    printf 'current_path=%s\n' "$action30c_resolved"
    printf 'current_revision=%s\n' "$action30c_revision"
    printf 'current_parent=%s\n' "$action30c_parent"
    printf 'current_source=%s\n' "$action30c_source"
    printf 'current_release_manifest_sha256=%s\n' \
        "$(file_hash "$action30c_resolved/release-manifest.json")"
    printf 'current_payload_manifest_sha256=%s\n' \
        "$(file_hash "$action30c_resolved/manifest.sha256")"
}
...
outbound_entry_type() {
    local action30c_entry=$1

    if [[ -L "$action30c_entry" ]]; then
        printf 'symlink\n'
    elif [[ -d "$action30c_entry" ]]; then
        printf 'directory\n'
    elif [[ -f "$action30c_entry" ]]; then
        printf 'regular\n'
    elif [[ -b "$action30c_entry" ]]; then
        printf 'block\n'
    elif [[ -c "$action30c_entry" ]]; then
        printf 'character\n'
    elif [[ -p "$action30c_entry" ]]; then
        printf 'fifo\n'
    elif [[ -S "$action30c_entry" ]]; then
        printf 'socket\n'
    else
        printf 'other\n'
    fi
...
write_outbound_snapshot() {
    local action30c_snapshot=$1
    local action30c_snapshot_root=${2:-$outbound_root}

    install -m 0600 /dev/null "$action30c_snapshot" || return 1
    find "$action30c_snapshot_root" -mindepth 1 -maxdepth 1 -print0 >"$action30c_snapshot" || return 1
    sort -z -o "$action30c_snapshot" "$action30c_snapshot" || return 1
}
candidate_disposition() {
    local action30c_candidate=$1
    local action30c_revision=$2
    local action30c_source=$3
    local action30c_current_candidate=${4:-$releases_root/$accepted_current_revision}

    [[ "${action30c_candidate##*/}" = "$action30c_revision" ]] || return 1
    case "$role" in
        node-a)
            [[ "$action30c_source" = node-a ]] || return 1
            if [[ "$action30c_revision" = "$accepted_current_revision" ]]; then
                manifest_valid "$action30c_current_candidate" || return 1
                [[ "$(file_hash "$action30c_candidate/release-manifest.json")" = "$(file_hash "$action30c_current_candidate/release-manifest.json")" ]] || return 1
                [[ "$(file_hash "$action30c_candidate/manifest.sha256")" = "$(file_hash "$action30c_current_candidate/manifest.sha256")" ]] || return 1
                printf 'retain\n'
            else
                printf 'quarantine\n'
            fi
            ;;
        node-b)
            [[ "$action30c_source" = node-b ]] || return 1
            printf 'quarantine\n'
            ;;
        *) return 1 ;;
    esac
...
outbound_inventory() {
    local action30c_inventory_root=${1:-$outbound_root}
    local action30c_candidate
    local action30c_candidate_count=0
    local action30c_disposition
    local action30c_entry_count=0
    local action30c_entry_name
    local action30c_entry_type
    local action30c_invalid_entry=false
    local action30c_parent
    local action30c_quarantine_count=0
    local action30c_retain_count=0
    local action30c_revision
    local action30c_source

    [[ -d "$action30c_inventory_root" && ! -L "$action30c_inventory_root" ]] || return 1
    ...
    write_outbound_snapshot "$outbound_inventory_snapshot" "$action30c_inventory_root" || return 1
    install -m 0600 /dev/null "$outbound_quarantine_plan" || return 1
    while IFS= read -r -d '' action30c_candidate; do
        action30c_entry_count=$((action30c_entry_count + 1))
        action30c_entry_name=${action30c_candidate##*/}
        action30c_entry_type=$(outbound_entry_type "$action30c_candidate") || return 1
        printf 'outbound_entry_%s_name_q=%q\n' "$action30c_entry_count" "$action30c_entry_name"
        printf 'outbound_entry_%s_type=%s\n' "$action30c_entry_count" "$action30c_entry_type"
        if [[ "$action30c_entry_type" != directory ||
            "$action30c_entry_name" = .* ]] ||
            ! valid_revision "$action30c_entry_name"; then
            action30c_invalid_entry=true
        fi
    done <"$outbound_inventory_snapshot"
    printf 'outbound_entry_count=%s\n' "$action30c_entry_count"
    [[ "$action30c_invalid_entry" = false ]] || return 1
    while IFS= read -r -d '' action30c_candidate; do
        [[ -n "$action30c_candidate" ]] || continue
        action30c_candidate_count=$((action30c_candidate_count + 1))
        manifest_valid "$action30c_candidate" || return 1
        action30c_revision=$(jq -r '.revision // empty' "$action30c_candidate/release-manifest.json") || return 1
        action30c_parent=$(jq -r '.parent_revision // empty' "$action30c_candidate/release-manifest.json") || return 1
        action30c_source=$(jq -r '.source_node // empty' "$action30c_candidate/release-manifest.json") || return 1
        valid_revision "$action30c_revision" || return 1
        action30c_disposition=$(candidate_disposition "$action30c_candidate" \
            "$action30c_revision" "$action30c_source") || return 1
        printf 'outbound_candidate_%s=%s\n' "$action30c_candidate_count" \
            "${action30c_candidate##*/}"
        ...
        printf 'outbound_candidate_%s_disposition=%s\n' "$action30c_candidate_count" \
            "$action30c_disposition"
        if [[ "$action30c_disposition" = quarantine ]]; then
            printf '%s\0' "$action30c_candidate" >>"$outbound_quarantine_plan" || return 1
            action30c_quarantine_count=$((action30c_quarantine_count + 1))
        else
            action30c_retain_count=$((action30c_retain_count + 1))
        fi
    done <"$outbound_inventory_snapshot"
    [[ "$action30c_retain_count" -le 1 ]] || return 1
    printf 'outbound_candidate_count=%s\n' "$action30c_candidate_count"
    printf 'outbound_retain_count=%s\n' "$action30c_retain_count"
    printf 'outbound_quarantine_count=%s\n' "$action30c_quarantine_count"
    [[ "$role" != node-b || "$action30c_retain_count" -eq 0 ]] || return 1
...
outbound_snapshot_unchanged() {
    local action30c_observed=$capture_directory/outbound-inventory.after-stop

    write_outbound_snapshot "$action30c_observed" || return 1
    cmp -s "$outbound_inventory_snapshot" "$action30c_observed"
}
protect_ineligible_outbound() {
    local action30c_candidate
    local action30c_destination
    local action30c_parent
    local action30c_revision
    local action30c_source
    ...
    while IFS= read -r -d '' action30c_candidate; do
        ...
        action30c_parent=$(jq -r '.parent_revision // empty' "$action30c_candidate/release-manifest.json") || return 1
        action30c_source=$(jq -r '.source_node // empty' "$action30c_candidate/release-manifest.json") || return 1
        [[ "$(candidate_disposition "$action30c_candidate" "$action30c_revision" "$action30c_source")" = quarantine ]] || return 1
        action30c_destination=$quarantine_root/${node_token}-outbound-${action30c_revision}-action30c
        [[ ! -e "$action30c_destination" && ! -L "$action30c_destination" ]] || return 1
        mv -- "$action30c_candidate" "$action30c_destination" || return 1
        release_identity_exact "$action30c_destination" \
            "$action30c_revision" "$action30c_parent" "$action30c_source" || return 1
        [[ ! -e "$action30c_candidate" && ! -L "$action30c_candidate" ]] || return 1
        quarantined_outbound_count=$((quarantined_outbound_count + 1))
        printf 'protected_outbound_%s_revision=%s\n' "$quarantined_outbound_count" "$action30c_revision"
        printf 'protected_outbound_%s_source=%s\n' "$quarantined_outbound_count" "$action30c_source"
        printf 'protected_outbound_%s_path=%s\n' "$quarantined_outbound_count" "$action30c_destination"
...
node_b_outbound_clear() {
    ...
    action30c_observed=$(find "$outbound_root" -mindepth 1 -maxdepth 1 \
        -type d ! -name '.*' -print -quit) || return 1
    [[ -z "$action30c_observed" ]]
}
role_state_exact() {
    local action30c_expected

    case "$role" in
        node-a) action30c_expected='(us) 2 "Master"' ;;
        node-b) action30c_expected='(us) 1 "Backup"' ;;
        *) return 1 ;;
    esac
    [[ "$(timeout 3 busctl get-property org.keepalived.Vrrp1 \
        "$ipv4_object" org.keepalived.Vrrp1.Instance State)" = "$action30c_expected" ]] || return 1
    [[ "$(timeout 3 busctl get-property org.keepalived.Vrrp1 \
        "$ipv6_object" org.keepalived.Vrrp1.Instance State)" = "$action30c_expected" ]]
}
lsyncd_stable() {
    local action30c_initial_pid
    local action30c_initial_restarts
    local action30c_sample
    local action30c_observed_pid
    local action30c_observed_restarts

    action30c_initial_pid=$(systemctl show caddy-lsyncd.service -p MainPID --value) || return 1
    action30c_initial_restarts=$(systemctl show caddy-lsyncd.service -p NRestarts --value) || return 1
    [[ "$action30c_initial_pid" =~ ^[1-9][0-9]*$ ]] || return 1
    [[ "$action30c_initial_restarts" =~ ^[0-9]+$ ]] || return 1
    printf 'lsyncd_stability_initial_pid=%s\n' "$action30c_initial_pid"
    printf 'lsyncd_stability_initial_restarts=%s\n' "$action30c_initial_restarts"
    for ((action30c_sample = 1; action30c_sample <= lsyncd_stability_samples; action30c_sample++)); do
        ...
        action30c_observed_pid=$(systemctl show caddy-lsyncd.service -p MainPID --value) || return 1
        action30c_observed_restarts=$(systemctl show caddy-lsyncd.service -p NRestarts --value) || return 1
        [[ "$action30c_observed_pid" = "$action30c_initial_pid" ]] || return 1
        [[ "$action30c_observed_restarts" = "$action30c_initial_restarts" ]] || return 1
        lsyncd_status_fresh || return 1
        printf 'lsyncd_stability_sample_%s=true\n' "$action30c_sample"
        if [[ "$action30c_sample" -lt "$lsyncd_stability_samples" ]]; then
            sleep "$lsyncd_stability_delay_seconds"
        fi
    done
}
post_cursor_transport_clean() {
    local action30c_journal=$capture_directory/sync_post_cursor_journal.stdout

    [[ -f "$action30c_journal" ]] || return 1
    ! grep -Eqi \
...
static_enablement_contract() {
    local action30c_unit

    for action30c_unit in "${static_units[@]}"; do
        enabled_exact "$action30c_unit" static || return 1
    done
}
lsyncd_status_age() {
    local action30c_now
    local action30c_modified

    [[ -f "$lsyncd_status_file" && ! -L "$lsyncd_status_file" && -s "$lsyncd_status_file" ]] || return 1
    action30c_now=$(date +%s) || return 1
    action30c_modified=$(stat -c %Y "$lsyncd_status_file") || return 1
    [[ "$action30c_modified" -le "$action30c_now" ]] || return 1
    printf '%s\n' "$((action30c_now - action30c_modified))"
}
lsyncd_status_fresh() {
    local action30c_age

    action30c_age=$(lsyncd_status_age) || return 1
    [[ "$action30c_age" -le "$maximum_lsyncd_status_age" ]]
}
wait_for_fresh_lsyncd_status() {
    local action30c_waited=0

    while [[ "$action30c_waited" -lt "$lsyncd_status_wait_seconds" ]]; do
        if lsyncd_status_fresh; then
            printf '%s_%s_lsyncd_status_wait_seconds=%s\n' "$prefix" "$node_token" "$action30c_waited"
            return 0
        fi
        sleep 1
        action30c_waited=$((action30c_waited + 1))
    done
    return 1
}

wait_for_lsyncd_status_advance() {
    local action30c_initial_mtime
    local action30c_current_mtime
    local action30c_waited=0

    action30c_initial_mtime=$(stat -c %Y "$lsyncd_status_file") || return 1
    printf '%s_%s_lsyncd_status_mtime_initial=%s\n' "$prefix" "$node_token" "$action30c_initial_mtime"
    while [[ "$action30c_waited" -lt "$lsyncd_status_wait_seconds" ]]; do
        sleep 1
        action30c_waited=$((action30c_waited + 1))
        action30c_current_mtime=$(stat -c %Y "$lsyncd_status_file" 2>/dev/null || true)
        if [[ "$action30c_current_mtime" =~ ^[0-9]+$ &&
            "$action30c_current_mtime" -gt "$action30c_initial_mtime" ]]; then
            printf '%s_%s_lsyncd_status_mtime_advanced=%s\n' "$prefix" "$node_token" "$action30c_current_mtime"
...
emit_unit_properties() {
    local action30c_label=$1
    local action30c_unit=$2

    run_captured "$action30c_label" systemctl show "$action30c_unit" \
        -p LoadState -p UnitFileState -p ActiveState -p SubState -p Result -p ExecMainStatus
}
successful_static_worker() {
    local action30c_unit=$1

    enabled_exact "$action30c_unit" static || return 1
    unit_not_failed "$action30c_unit" || return 1
    [[ "$(systemctl show "$action30c_unit" -p Result --value 2>/dev/null || true)" = success ]] || return 1
    [[ "$(systemctl show "$action30c_unit" -p ExecMainStatus --value 2>/dev/null || true)" = 0 ]]
}
continuity() {
...
target_contract() {
    ...
    continuity || return 1
    for action30c_unit in "${persistent_units[@]}"; do
        check "${action30c_unit//[^a-zA-Z0-9]/_}_enabled" enabled_exact "$action30c_unit" enabled || return 1
        check "${action30c_unit//[^a-zA-Z0-9]/_}_active" active_exact "$action30c_unit" active || return 1
    done
...
manifest_value() {
    local action30c_key=$1

    awk -F= -v key="$action30c_key" '$1 == key { print $2; found++ } END { if (found != 1) exit 1 }' \
        "$backup_directory/manifest"
}
record_backup() {
    local action30c_unit
    local action30c_backup_staging=${backup_directory}.staging.$$

    if ! install -d -o root -g root -m 0700 "$action30c_backup_staging"; then
        return 1
    fi
    if ! install -o root -g root -m 0600 "$obsolete_path" \
        "$action30c_backup_staging/caddy-validate-reload.path" ||
        ! install -o root -g root -m 0600 "$obsolete_service" \
            "$action30c_backup_staging/caddy-validate-reload.service"; then
        rm -rf -- "$action30c_backup_staging"
        return 1
    fi
    {
        printf 'action=30c\nrole=%s\n' "$role"
        for action30c_unit in "${persistent_units[@]}" caddy-api.service "$distribution_lsyncd_unit" caddy-validate-reload.path; do
            printf 'enabled_%s=%s\n' "${action30c_unit//[^a-zA-Z0-9]/_}" \
                "$(systemctl is-enabled "$action30c_unit" 2>/dev/null || true)"
            printf 'active_%s=%s\n' "${action30c_unit//[^a-zA-Z0-9]/_}" \
                "$(systemctl is-active "$action30c_unit" 2>/dev/null || true)"
        done
    } >"$action30c_backup_staging/manifest" || {
        rm -rf -- "$action30c_backup_staging"
        return 1
    }
    if ! chmod 0600 "$action30c_backup_staging/manifest" ||
        ! mv -- "$action30c_backup_staging" "$backup_directory"; then
        rm -rf -- "$action30c_backup_staging"
        return 1
    fi
}
restore_enablement() {
    local action30c_unit=$1
    local action30c_token=${action30c_unit//[^a-zA-Z0-9]/_}
    local action30c_state
    local action30c_active

    action30c_state=$(manifest_value "enabled_$action30c_token") || return 1
    action30c_active=$(manifest_value "active_$action30c_token") || return 1
    case "$action30c_state" in
        enabled) systemctl enable "$action30c_unit" >/dev/null 2>&1 || return 1 ;;
        disabled) systemctl disable "$action30c_unit" >/dev/null 2>&1 || return 1 ;;
        masked) systemctl mask "$action30c_unit" >/dev/null 2>&1 || return 1 ;;
        static | indirect | generated | transient | '') ;;
        *) return 1 ;;
    esac
    case "$action30c_active" in
        active) systemctl start "$action30c_unit" >/dev/null 2>&1 || return 1 ;;
        inactive) systemctl stop "$action30c_unit" >/dev/null 2>&1 || return 1 ;;
        failed) return 1 ;;
        *) ;;
    esac
}
rollback() {
    ...
    for action30c_unit in "${persistent_units[@]}" caddy-api.service "$distribution_lsyncd_unit" caddy-validate-reload.path; do
        restore_enablement "$action30c_unit" || return 1
    done
    continuity || return 1
    printf '%s_%s_rollback_complete=true\n' "$prefix" "$node_token"
}
apply() {
    local action30c_cert_journal_cursor
    local action30c_health_journal_cursor
    local action30c_status_age
    ...
    if action30c_status_age=$(lsyncd_status_age 2>/dev/null); then
        printf '%s_%s_lsyncd_status_age_before=%s\n' "$prefix" "$node_token" "$action30c_status_age"
    else
    ...
    action30c_cert_journal_cursor=$(cursor_from_capture cert_worker_journal_cursor) || return 1
    ...
    run_captured cert_worker_journal journalctl -u caddy-cert-expiry.service \
        --after-cursor "$action30c_cert_journal_cursor" \
    ...
    action30c_health_journal_cursor=$(cursor_from_capture health_worker_journal_cursor) || return 1
    ...
    run_captured health_worker_journal journalctl -u caddy-sync-health.service \
        --after-cursor "$action30c_health_journal_cursor" \
        --no-pager --no-hostname -o short-iso || return 1
...
create_semantic_fixture_release() {
    local action30c_fixture_release=$1
    local action30c_fixture_revision=$2
    local action30c_fixture_parent=$3
    local action30c_fixture_source=$4

    mkdir -p "$action30c_fixture_release"
    printf 'fixture\n' >"$action30c_fixture_release/Caddyfile"
    jq -cn --arg revision "$action30c_fixture_revision" \
        --arg parent "$action30c_fixture_parent" \
        --arg source "$action30c_fixture_source" \
        '{revision:$revision,parent_revision:$parent,source_node:$source,created_at:"2026-08-11T00:00:00Z"}' \
        >"$action30c_fixture_release/release-manifest.json"
    (
        cd "$action30c_fixture_release"
        sha256sum ./Caddyfile ./release-manifest.json >manifest.sha256
    )
}
semantic_self_test() {
    local action30c_fixture_node_a
    local action30c_fixture_node_b
    local action30c_fixture_node_b_older
    local action30c_fixture_wrong_source
    local action30c_observed

    semantic_fixture_root=$(mktemp -d /tmp/action30c-semantic.XXXXXX) || return 1
    trap 'rm -rf -- "${semantic_fixture_root:-}"' EXIT INT TERM
    capture_directory=$semantic_fixture_root/captures
    mkdir -p "$capture_directory"
    action30c_fixture_node_a=$semantic_fixture_root/$accepted_current_revision
    action30c_fixture_node_b=$semantic_fixture_root/$historical_revision
    action30c_fixture_node_b_older=$semantic_fixture_root/20260811T174240Z-31d43261-5cd7-44ce-83e5-947927184d29
    action30c_fixture_wrong_source=$semantic_fixture_root/wrong-source
    create_semantic_fixture_release "$action30c_fixture_node_a" \
        "$accepted_current_revision" "$historical_revision" node-a
    create_semantic_fixture_release "$action30c_fixture_node_b" \
        "$historical_revision" "$historical_parent" node-b
    create_semantic_fixture_release "$action30c_fixture_node_b_older" \
        "${action30c_fixture_node_b_older##*/}" "$historical_parent" node-b
    create_semantic_fixture_release "$action30c_fixture_wrong_source" \
        wrong-source "$historical_parent" node-a
    configure_role node-a || return 1
    [[ "$(candidate_disposition "$action30c_fixture_node_a" \
        "$accepted_current_revision" node-a "$action30c_fixture_node_a")" = retain ]] || return 1
    configure_role node-b || return 1
    [[ "$(candidate_disposition "$action30c_fixture_node_b" \
        "$historical_revision" node-b)" = quarantine ]] || return 1
    [[ "$(candidate_disposition "$action30c_fixture_node_b_older" \
        "${action30c_fixture_node_b_older##*/}" node-b)" = quarantine ]] || return 1
    if candidate_disposition "$action30c_fixture_wrong_source" wrong-source node-a >/dev/null; then
        return 1
    fi
    ...
    mv "$action30c_fixture_node_b" "$semantic_fixture_root/outbound/"
    mv "$action30c_fixture_node_b_older" "$semantic_fixture_root/outbound/"
    ...
    [[ "$(awk -F= '$1 == "outbound_quarantine_count" { print $2 }' \
        "$semantic_fixture_root/inventory.stdout")" = 2 ]] || return 1
    action30c_observed=$(grep -c '^outbound_entry_[0-9][0-9]*_name_q=' \
        "$semantic_fixture_root/inventory.stdout") || return 1
    [[ "$action30c_observed" -eq 2 ]] || return 1
    ...
    [[ "$action30c_observed" -eq 3 ]] || return 1
...
main() {
    local action30c_mode=${1:-}
    local action30c_role=${2:-}

    configure_role "$action30c_role" || return $?
    install -d -o root -g root -m 0700 "$capture_directory" || return 1
    case "$action30c_mode" in
        --apply)
