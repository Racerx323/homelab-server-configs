...
readonly inspector_sha256=533278646571fe5aecea3428d7045ea50557d291fcb2e2fa62e752c403336415
...
readonly caddy_root
readonly inspector="$script_dir/inspect-node-a-pihole-response-path-action17n-b.sh"
...
validate_structure() {
    ...
    ! grep -Ev \
        '^(action_17n_b_(remote_reached|assertion_[a-z0-9_]+|observed_[a-z0-9_]+|value_[a-z0-9_]+|assertion_count|failed_assertion_count|first_failure|before_state_sha256|after_state_sha256|conclusion|remote_complete)|remote_paths_created|dns_queries_performed|peer_connections|synchronization_commands_executed|dns_configuration_mutations|nss_configuration_mutations|pihole_cache_reset_performed|service_mutations|persistent_mutations)=' \
        "$transcript_path" |
        grep -q . || return 1
    total_assertions=$(
        grep -Ec '^action_17n_b_assertion_[a-z0-9_]+=(true|false)$' \
            "$transcript_path"
    )
    unique_assertions=$(
        grep -E '^action_17n_b_assertion_[a-z0-9_]+=(true|false)$' \
            "$transcript_path" |
            cut -d= -f1 |
            LC_ALL=C sort -u |
            wc -l
    ...
    false_assertions=$(
        grep -Ec '^action_17n_b_assertion_[a-z0-9_]+=false$' \
    ...
    reported_assertions=$(value_for action_17n_b_assertion_count "$transcript_path")
    reported_failures=$(
        value_for action_17n_b_failed_assertion_count "$transcript_path"
    )
    [[ "$reported_assertions" -eq "$expected_assertion_count" ]] || return 1
    [[ "$reported_failures" -eq "$false_assertions" ]] || return 1
    dns_value_count=$(
        grep -Ec '^action_17n_b_value_(direct_unbound|local_pihole)_[a-z0-9_]+_(rcode|answer|ttl)=' \
            "$transcript_path"
    )
    [[ "$dns_value_count" -eq $((expected_dns_query_count * 3)) ]] || return 1
    [[ "$(value_for action_17n_b_remote_reached "$transcript_path")" == true ]]
    [[ "$(value_for action_17n_b_remote_complete "$transcript_path")" == true ]]
    ...
    before_state=$(value_for action_17n_b_before_state_sha256 "$transcript_path")
    after_state=$(value_for action_17n_b_after_state_sha256 "$transcript_path")
...
if [[ "${1:-}" == --self-test && $# -eq 1 ]]; then
    ...
    printf 'action_17n_b_runner_self_test_complete=true\n'
    exit 0
elif [[ "${1:-}" == --source-test && $# -eq 1 ]]; then
    verify_live_sources
    printf 'action_17n_b_runner_source_test_complete=true\n'
    exit 0
elif (($#)); then
...
verify_live_sources
work_directory=$(mktemp -d /tmp/caddy-action17n-b.XXXXXX)
...
if [[ "$classification_status" -eq 0 ||
    ...
    printf 'action_17n_b_ssh_status=%s\n' "$ssh_status"
    if [[ "$classification_status" -eq 0 ]]; then
        printf 'action_17n_b_runner_classification=observation_complete\n'
    else
        printf 'action_17n_b_runner_classification=semantic_mismatch\n'
    fi
    printf 'action_17n_b_workstation_cleanup=true\n'
    exit "$classification_status"
fi

printf 'action_17n_b_ssh_status=%s\n' "$ssh_status" >&2
printf 'action_17n_b_remote_stdout_sha256=%s\n' \
    "$(file_hash "$remote_output_path")" >&2
printf 'action_17n_b_remote_stderr_sha256=%s\n' \
    "$(file_hash "$remote_error_path")" >&2
printf 'action_17n_b_runner_classification=evidence_failure\n' >&2
printf 'action_17n_b_workstation_cleanup=true\n' >&2
exit 97
