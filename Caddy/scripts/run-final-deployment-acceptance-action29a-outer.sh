...
readonly prefix=action_29a_outer
readonly schema_version=action29a-final-acceptance-v2
...
readonly inspector=$script_directory/inspect-final-deployment-action29a.sh
...
readonly regression=$caddy_root/tests/action29a-final-deployment-acceptance-regression.sh
readonly manifest=$caddy_root/manifests/caddy-final-deployment-acceptance-action29a.yaml
readonly expected_inspector_sha256=2a9348e1352cfcb515233beaf415cd4fb0aa81a6e2343733255d22ac19dc17ea
...
readonly expected_regression_sha256=c462f04442d21f206b72da7e170a18af0ca22448029d0cfe0df26b3b5b15d997
evidence_root=${CADDY_ACTION29A_EVIDENCE_ROOT:-/tmp/caddy-ssh-evidence/action29a}
readonly evidence_root
ssh_binary=${CADDY_ACTION29A_SSH_BIN:-/usr/bin/ssh}
readonly ssh_binary
run_id=
work_root=
self_test_root=
...
validate_node_transcript() {
    ...
    while IFS= read -r action29_outer_label; do
        require_one "action_29a_remote_${action29_outer_token}_check_${action29_outer_label}=true" "$action29_outer_stdout" || return 1
    done < <(/bin/bash "$inspector" --expected-checks "$action29_outer_role")
    ! grep -Eq '^action_29a_remote_.*_check_.*=false$' "$action29_outer_stdout" || return 1
    require_one "action_29a_remote_${action29_outer_token}_schema=action29a-node-snapshot-v2" "$action29_outer_stdout" || return 1
    require_one "action_29a_remote_${action29_outer_token}_expected_pihole_ftl_sha256=a1dc88b1f696a6870e38025be113ff8664750fca6265c79a2aee12f80898cfa3" "$action29_outer_stdout" || return 1
    require_one "action_29a_remote_${action29_outer_token}_observed_pihole_ftl_sha256=a1dc88b1f696a6870e38025be113ff8664750fca6265c79a2aee12f80898cfa3" "$action29_outer_stdout" || return 1
    require_one "action_29a_remote_${action29_outer_token}_expected_pihole_domain_sha256=39fa219a7d1c81c7fb36d89bc17ba1caae26f3472eb861a750a8ba03ae55b026" "$action29_outer_stdout" || return 1
    require_one "action_29a_remote_${action29_outer_token}_observed_pihole_domain_sha256=39fa219a7d1c81c7fb36d89bc17ba1caae26f3472eb861a750a8ba03ae55b026" "$action29_outer_stdout" || return 1
    require_one "action_29a_remote_${action29_outer_token}_first_failure=none" "$action29_outer_stdout" || return 1
    require_one "action_29a_remote_${action29_outer_token}_read_only=true" "$action29_outer_stdout" || return 1
    require_one "action_29a_remote_${action29_outer_token}_complete=true" "$action29_outer_stdout" || return 1
