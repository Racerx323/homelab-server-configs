...
readonly health_baseline_outer_sha256=a4defbeab49958ecaadf1c2e34c259a847ecf0cc5b9a86d359da2c210d8b68c9
readonly readiness_outer_sha256=ea5b99cb764ff444500bea43efac7c783e20c5d626ed8b594c02f5c71ea328c3
readonly activation_outer_sha256=30c297807ed7f0fb48e41c5c653f3c4168b2aa5a05053162c9ece58ef87a88d4
readonly regression_sha256=5247599be6f8b60f81e89e78f33e2397c6da2738f77ac2df8a94277610d82f61
...
readonly health_baseline_outer=$script_directory/run-node-a-caddy-health-group-postinstall-action20f-a-outer.sh
readonly readiness_outer=$script_directory/run-dual-node-caddy-readiness-action20d-retry10.sh
readonly activation_outer=$script_directory/run-node-a-caddy-vrrp-activation-action20d-retry10.sh
readonly regression=$caddy_root/tests/action20d-retry10-activation-boundary-regression.sh
...
run_local_gates() {
    ...
    run_gate health_baseline_outer_hash require_hash "$health_baseline_outer_sha256" \
        "$health_baseline_outer" || return 1
    ...
    run_gate syntax /bin/bash -n "$health_baseline_outer" "$readiness_outer" \
        "$activation_outer" \
        "$regression" "$0" || return 1
    run_gate collision_policy /bin/bash "$collision" "$health_baseline_outer" \
        "$readiness_outer" "$activation_outer" \
        "$regression" "$0" || return 1
...
run_boundary() {
    ...
    boundary_root=$(mktemp -d /tmp/caddy-action20d-retry10-boundary.XXXXXX)
...
case "${1:-}" in
    --expected-local-gates)
        [[ $# -eq 1 ]] || exit 64
        printf '%s\n' working_directory health_baseline_outer_hash \
            readiness_outer_hash activation_outer_hash \
            regression_hash collision_hash conditional_hash \
