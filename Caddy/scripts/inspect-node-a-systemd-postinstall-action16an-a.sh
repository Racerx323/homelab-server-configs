...
readonly -a custom_units=(
    ...
    caddy-sync-failure@action16an-a-inspection.service
...
readonly -a expected_sync_stage_files=(
    caddy-sync-rsync-receiver
...
readonly -a expected_sync_stage_checksums=(
...
if [[ "${1:-}" == --self-test && $# -eq 1 ]]; then
    ...
    [[ "${#disabled_units[@]}" -eq 5 ]]
    [[ "${#static_units[@]}" -eq 5 ]]
    [[ "${#expected_libexec_files[@]}" -eq 7 ]]
    [[ "${#expected_sync_stage_files[@]}" -eq 5 ]]
    [[ "${#expected_sync_stage_checksums[@]}" -eq 5 ]]
    [[ "$stage_digest_sha256" =~ ^[0-9a-f]{64}$ ]]
    [[ "$expected_node_a_fingerprint" =~ ^SHA256:[A-Za-z0-9+/]{43}$ ]]
    printf 'action_16an_a_systemd_postinstall_inspector_self_test_complete=true\n'
    exit 0
elif (($#)); then
...
verify_custom_units() {
    # shellcheck disable=SC2317 # Invoked indirectly by record_command.
...
printf 'action_16an_a_remote_reached=true\n'
...
for command_name in \
    ...
    record_command "command_${command_name//-/_}_present" \
        test -n "$command_path"
done
...
record_equal protected_package_inventory \
    "$observed_inventory" "$package_inventory_sha256"
dpkg_audit_status=0
...
record_command stage_not_symlink test ! -L "$stage"
record_equal stage_meta \
    "$(stat -c '%U:%G:%a' "$stage" 2>/dev/null || true)" root:root:750
mapfile -t stage_files < <(
    find "$stage" -mindepth 1 -type f -printf '%P\n' 2>/dev/null | sort
)
record_equal stage_file_set "${stage_files[*]}" "${relative_files[*]}"
mapfile -t stage_directories < <(
    find "$stage" -mindepth 1 -type d -printf '%P\n' 2>/dev/null | sort
)
record_equal stage_directory_set "${stage_directories[*]}" \
    'scripts systemd systemd/caddy.service.d systemd/lighttpd.service.d'
record_equal stage_symlink_count \
    "$(find "$stage" -type l -printf x 2>/dev/null | wc -c)" 0
record_equal stage_digest \
    "$(stage_digest 2>/dev/null || true)" "$stage_digest_sha256"
...
for index in "${!targets[@]}"; do
    ...
    label=${label,,}
    stage_expected_mode=root:root:644
    if [[ "$relative_path" == scripts/* ]]; then
        stage_expected_mode=root:root:755
    fi
    record_command "stage_${label}_regular" \
        test -f "$stage/$relative_path"
    record_command "stage_${label}_not_symlink" \
        test ! -L "$stage/$relative_path"
    record_equal "stage_${label}_hash" \
        "$(sha256sum "$stage/$relative_path" 2>/dev/null |
            awk '{ print $1 }' || true)" \
        "$expected_hash"
    record_equal "stage_${label}_meta" \
        "$(stat -c '%U:%G:%a' "$stage/$relative_path" \
            2>/dev/null || true)" \
        "$stage_expected_mode"
    record_command "target_${label}_regular" test -f "$target"
    record_command "target_${label}_not_symlink" test ! -L "$target"
    record_equal "target_${label}_hash" \
        "$(sha256sum "$target" 2>/dev/null | awk '{ print $1 }' || true)" \
        "$expected_hash"
    case "$target" in
        /usr/local/libexec/*)
            expected_mode=root:root:755
            record_command "stage_${label}_syntax" \
                bash -n "$stage/$relative_path"
            ;;
        *)
            expected_mode=root:root:644
            ;;
    esac
    record_equal "target_${label}_meta" \
        "$(stat -c '%U:%G:%a' "$target" 2>/dev/null || true)" \
        "$expected_mode"
done

record_equal caddy_dropin_directory_meta \
    "$(stat -c '%U:%G:%a' /etc/systemd/system/caddy.service.d \
        2>/dev/null || true)" root:root:755
record_equal lighttpd_dropin_directory_meta \
    "$(stat -c '%U:%G:%a' /etc/systemd/system/lighttpd.service.d \
        2>/dev/null || true)" root:root:755
record_command systemd_analyze_verify verify_custom_units

for unit in "${custom_units[@]}"; do
    label=${unit//[^A-Za-z0-9]/_}
    label=${label,,}
    record_equal "custom_unit_${label}_loaded" \
        "$(unit_property "$unit" LoadState)" loaded
    record_equal "custom_unit_${label}_inactive" \
        "$(unit_property "$unit" ActiveState)" inactive
done
for unit in "${disabled_units[@]}"; do
    label=${unit//[^A-Za-z0-9]/_}
    label=${label,,}
    record_equal "custom_unit_${label}_disabled" \
        "$(unit_property "$unit" UnitFileState)" disabled
done
for unit in "${static_units[@]}"; do
    label=${unit//[^A-Za-z0-9]/_}
    label=${label,,}
    record_equal "custom_unit_${label}_static" \
        "$(unit_property "$unit" UnitFileState)" static
done
record_equal caddy_dropin_recognized \
    "$(unit_property caddy.service DropInPaths)" \
    /etc/systemd/system/caddy.service.d/override.conf
record_equal lighttpd_dropin_recognized \
    "$(unit_property lighttpd.service DropInPaths)" \
    /etc/systemd/system/lighttpd.service.d/caddy-ha.conf
record_command caddy_timeout_override \
...
record_equal sysctl_file_hash \
    "$(sha256sum /etc/sysctl.d/70-caddy-ha.conf 2>/dev/null |
        awk '{ print $1 }' || true)" \
    "$sysctl_sha256"
record_equal sysctl_ipv4_nonlocal_bind \
    "$(sysctl -n net.ipv4.ip_nonlocal_bind 2>/dev/null || true)" 1
record_equal sysctl_ipv6_nonlocal_bind \
...
record_command retained_sync_stage_not_symlink \
    test ! -L "$retained_sync_stage"
record_equal retained_sync_stage_meta \
    "$(stat -c '%U:%G:%a' "$retained_sync_stage" 2>/dev/null || true)" \
    root:root:750
mapfile -t actual_sync_stage_files < <(
    find "$retained_sync_stage" -mindepth 1 -maxdepth 1 -type f \
        -printf '%f\n' 2>/dev/null |
        sort
)
record_equal retained_sync_stage_file_set \
    "${actual_sync_stage_files[*]}" "${expected_sync_stage_files[*]}"
for checksum in "${expected_sync_stage_checksums[@]}"; do
    expected_hash=${checksum%% *}
    relative_path=${checksum#*  }
    ...
    case "$relative_path" in
        *.sh | caddy-sync-rsync-receiver)
            expected_mode=root:root:750
            ;;
        *)
            expected_mode=root:root:640
            ;;
    esac
    record_command "sync_stage_${label}_regular" \
        test -f "$retained_sync_stage/$relative_path"
    record_command "sync_stage_${label}_not_symlink" \
        test ! -L "$retained_sync_stage/$relative_path"
    record_equal "sync_stage_${label}_hash" \
        "$(sha256sum "$retained_sync_stage/$relative_path" 2>/dev/null |
            awk '{ print $1 }' || true)" \
        "$expected_hash"
    record_equal "sync_stage_${label}_meta" \
        "$(stat -c '%U:%G:%a' "$retained_sync_stage/$relative_path" \
            2>/dev/null || true)" \
        "$expected_mode"
done
...
record_equal restricted_authorization_count \
    "$(wc -l </var/lib/caddy-sync/.ssh/authorized_keys 2>/dev/null || true)" 1
...
printf 'journal_disk_usage=%s\n' \
    "$(journalctl --disk-usage 2>/dev/null |
        sed 's/^Archived and active journals take up //')"
printf 'stage_file_count=%s\n' "${#stage_files[@]}"
printf 'stage_digest=%s\n' "$(stage_digest 2>/dev/null || true)"
printf 'installed_target_count=%s\n' "${#targets[@]}"
printf 'acceptance_assertion_count=%s\n' "$assertion_count"
printf 'acceptance_mismatch_count=%s\n' "$mismatch_count"
...
if [[ "$mismatch_count" -eq 0 ]]; then
    ...
    printf 'action_16an_a_inspection_complete=true\n'
    exit 0
fi
printf 'action_16an_a_postinstall_valid=false\n'
printf 'action_16an_a_inspection_complete=true\n'
exit 1
