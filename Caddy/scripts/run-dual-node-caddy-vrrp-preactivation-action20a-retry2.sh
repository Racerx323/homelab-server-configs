...
readonly prefix=action_20a_retry2
...
readonly probe="$script_directory/inspect-dual-node-caddy-vrrp-preactivation-action20a-retry2.sh"
readonly normalized_diagnostic_outer="$script_directory/run-node-b-caddy-state-difference-action20a-retry-a-outer.sh"
...
validate_probe() {
    ...
    while IFS= read -r expected_label; do
        require_one "action_20a_retry2_probe_assertion_${expected_label}=true" \
            "$probe_stdout" || return 1
    done < <("$probe" --expected-assertions)
    [[ "$(grep -Ec '^action_20a_retry2_probe_assertion_[a-z0-9_]+=(true|false)$' \
        "$probe_stdout")" -eq "$probe_expected_count" ]] || return 1
    [[ "$(grep '^action_20a_retry2_probe_assertion_' "$probe_stdout" |
        cut -d= -f1 | LC_ALL=C sort | uniq -d | wc -l)" -eq 0 ]] || return 1
    require_one "action_20a_retry2_probe_assertion_count=$probe_expected_count" \
        "$probe_stdout" || return 1
    require_one 'action_20a_retry2_probe_failed_assertion_count=0' "$probe_stdout" ||
        return 1
    require_one 'action_20a_retry2_probe_first_failure=none' "$probe_stdout" || return 1
    require_one "action_20a_retry2_probe_value_node_role=$probe_role" \
        "$probe_stdout" || return 1
    require_one \
        'action_20a_retry2_probe_value_state_address_normalization=interface_cidr_scope' \
        "$probe_stdout" || return 1
    require_one 'action_20a_retry2_probe_notification_helper_invoked=false' \
        "$probe_stdout" || return 1
    for probe_marker in filesystem_mutations service_mutations vrrp_mutations \
        vip_mutations persistent_mutations; do
        require_one "action_20a_retry2_probe_${probe_marker}=false" \
            "$probe_stdout" || return 1
    done
    require_one 'action_20a_retry2_probe_remote_complete=true' "$probe_stdout"
}
verify_sources() {
    ...
    [[ -f "$normalized_diagnostic_outer" &&
        ! -L "$normalized_diagnostic_outer" ]] || return 1
    ...
    [[ "$(file_hash "$normalized_diagnostic_outer")" = "$normalized_diagnostic_outer_sha256" ]] || return 1
    bash -n "$node_a_baseline" "$node_b_baseline" "$node_a_producer" \
        "$node_b_producer" "$probe" "$normalized_diagnostic_outer" || return 1
...
run_contract_test() {
    ...
    contract_root=$(mktemp -d /tmp/caddy-action20a-retry2-contract.XXXXXX) || return 1
    : >"$contract_root/empty"
    for contract_role in node-a node-b; do
        ...
        printf 'action_20a_retry2_probe_assertion_identity_root=true\n' \
            >>"$contract_root/probe"
        if validate_probe "$contract_role" "$contract_root/probe" \
            "$contract_root/empty" 0; then
...
verify_sources
if [[ "${CADDY_ACTION20ARETRY2_INTERCEPTED_TEST:-}" = 1 ]]; then
    case "$PWD" in
...
work_directory=$(mktemp -d /tmp/caddy-action20a-retry2.XXXXXX)
...
node_a_health=$(extract_one action_20a_retry2_probe_value_health_status "$node_a_probe_path" || true)
node_b_health=$(extract_one action_20a_retry2_probe_value_health_status "$node_b_probe_path" || true)
node_a_before=$(extract_one action_20a_retry2_probe_value_before_state_sha256 "$node_a_probe_path" || true)
node_a_after=$(extract_one action_20a_retry2_probe_value_after_state_sha256 "$node_a_probe_path" || true)
node_b_before=$(extract_one action_20a_retry2_probe_value_before_state_sha256 "$node_b_probe_path" || true)
node_b_after=$(extract_one action_20a_retry2_probe_value_after_state_sha256 "$node_b_probe_path" || true)
node_a_dns4=$(extract_one action_20a_retry2_probe_value_dns_ipv4_vip_count "$node_a_probe_path" || true)
node_a_dns6=$(extract_one action_20a_retry2_probe_value_dns_ipv6_vip_count "$node_a_probe_path" || true)
node_b_dns4=$(extract_one action_20a_retry2_probe_value_dns_ipv4_vip_count "$node_b_probe_path" || true)
node_b_dns6=$(extract_one action_20a_retry2_probe_value_dns_ipv6_vip_count "$node_b_probe_path" || true)
node_a_caddy4=$(extract_one action_20a_retry2_probe_value_caddy_ipv4_vip_count "$node_a_probe_path" || true)
node_a_caddy6=$(extract_one action_20a_retry2_probe_value_caddy_ipv6_vip_count "$node_a_probe_path" || true)
node_b_caddy4=$(extract_one action_20a_retry2_probe_value_caddy_ipv4_vip_count "$node_b_probe_path" || true)
node_b_caddy6=$(extract_one action_20a_retry2_probe_value_caddy_ipv6_vip_count "$node_b_probe_path" || true)
...
run_assertion caddy_ipv6_absent_both test "$node_a_caddy6" = 0 -a \
    "$node_b_caddy6" = 0
node_a_priority=$(extract_one action_20a_retry2_probe_value_priority "$node_a_probe_path" || true)
node_b_priority=$(extract_one action_20a_retry2_probe_value_priority "$node_b_probe_path" || true)
...
run_assertion vrids_exact bash -c \
    ...
    _ \
    "$(extract_one action_20a_retry2_probe_value_ipv4_vrid "$node_a_probe_path" || true)" \
    "$(extract_one action_20a_retry2_probe_value_ipv6_vrid "$node_a_probe_path" || true)" \
    "$(extract_one action_20a_retry2_probe_value_ipv4_vrid "$node_b_probe_path" || true)" \
    "$(extract_one action_20a_retry2_probe_value_ipv6_vrid "$node_b_probe_path" || true)"
# shellcheck disable=SC2016
run_assertion ipv4_peers_reciprocal bash -c \
    '[[ "$1" = "$4" && "$2" = "$3" ]]' _ \
    "$(extract_one action_20a_retry2_probe_value_ipv4_source "$node_a_probe_path" || true)" \
    "$(extract_one action_20a_retry2_probe_value_ipv4_peer "$node_a_probe_path" || true)" \
    "$(extract_one action_20a_retry2_probe_value_ipv4_source "$node_b_probe_path" || true)" \
    "$(extract_one action_20a_retry2_probe_value_ipv4_peer "$node_b_probe_path" || true)"
# shellcheck disable=SC2016
run_assertion ipv6_peers_reciprocal bash -c \
    '[[ "$1" = "$4" && "$2" = "$3" ]]' _ \
    "$(extract_one action_20a_retry2_probe_value_ipv6_source "$node_a_probe_path" || true)" \
    "$(extract_one action_20a_retry2_probe_value_ipv6_peer "$node_a_probe_path" || true)" \
    "$(extract_one action_20a_retry2_probe_value_ipv6_source "$node_b_probe_path" || true)" \
    "$(extract_one action_20a_retry2_probe_value_ipv6_peer "$node_b_probe_path" || true)"
# shellcheck disable=SC2016
run_assertion main_inclusion_prerequisites bash -c \
    'for transcript in "$1" "$2"; do for label in main_configuration_excludes_fragment main_configuration_caddy_names_clear main_configuration_vrids_clear main_configuration_terminal_newline main_hash_matches_backup; do grep -Fqx "action_20a_retry2_probe_assertion_${label}=true" "$transcript" || exit 1; done; done' \
    _ "$node_a_probe_path" "$node_b_probe_path"
# shellcheck disable=SC2016
run_assertion notification_helpers_not_invoked bash -c \
    'grep -Fqx action_20a_retry2_probe_notification_helper_invoked=false "$1" && grep -Fqx action_20a_retry2_probe_notification_helper_invoked=false "$2"' \
    _ "$node_a_probe_path" "$node_b_probe_path"
