...
readonly prefix=action_20e_retry_a
readonly inspector_sha256=6587b6a9b5aad28a9e4f2e4b31773f61017dc449ebfcc05d009991421eb2367d
readonly runner_sha256=de66cc0c43524728f2bbdc645cf5439fb87f4edd5c53addbef204c59ddab6871
readonly regression_sha256=0c1e609c9514a36e3e7c5004a15b3df0baa78e9a984c21a3ebe544d7fc681272
...
readonly inspector=$script_directory/inspect-caddy-runtime-directories-action20e-retry-a.sh
readonly runner=$script_directory/run-dual-node-caddy-runtime-postinstall-action20e-retry-a.sh
readonly regression=$caddy_root/tests/action20e-retry-runtime-directories-regression.sh
...
run_local_gates() {
    ...
    run_gate inspector_hash require_hash "$inspector_sha256" "$inspector" || return 1
    ...
    run_gate syntax /bin/bash -n "$inspector" "$runner" "$regression" || return 1
    run_gate collision_policy /bin/bash "$collision" "$inspector" "$runner" "$regression" || return 1
...
case "${1:-}" in
    --expected-local-gates)
        printf '%s\n' working_directory inspector_hash runner_hash regression_hash \
...
run_local_gates
capture_root=$(mktemp -d /tmp/caddy-action20e-retry-a-outer.XXXXXX)
