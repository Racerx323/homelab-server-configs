...
readonly prefix=action_20d_retry2
readonly complete_suite_sha256=7393ef594f00839d0366b4d9415b04ed76378046e89cb50c2828877ae2b1a21d
readonly integration_suite_sha256=18acefaef2da1f0cbcff01b1c598344c11aa6a3caf12d017c1847842c74a2e73
...
readonly regression_sha256=9cc6adbb6cd5ff739f00d35eacfff6583ff90bbc08f8897b1b4322f7e2a1b26f
...
readonly regression=$caddy_root/tests/action20d-retry2-activation-boundary-regression.sh
...
run_local_gates() {
    ...
    run_gate syntax /bin/bash -n "$complete_suite" "$integration_suite" "$readiness_outer" "$activation_outer" "$regression" "$0" || return 1
    run_gate collision_policy /bin/bash "$collision" "$readiness_outer" "$activation_outer" "$regression" "$0" || return 1
...
run_boundary() {
    ...
    boundary_root=$(mktemp -d /tmp/caddy-action20d-retry2-boundary.XXXXXX)
...
case "${1:-}" in
    --expected-local-gates)
        ...
        printf '%s\n' working_directory complete_suite_hash integration_suite_hash \
            readiness_outer_hash activation_outer_hash regression_hash collision_hash conditional_hash \
