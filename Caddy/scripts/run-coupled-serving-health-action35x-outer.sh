...
readonly prefix=action_35_x_outer
readonly transaction_sha256=a592ab73a4e54591ae968be0fe6be7462233932c02fced4506e5378d279b9dd8
...
safe_capture() {
    local action35x_path=$1

    regular_file "$action35x_path"
    [[ "$(stat -c '%s' "$action35x_path")" -le "$max_stream_bytes" ]]
    iconv -f UTF-8 -t UTF-8 "$action35x_path" >/dev/null
    ! LC_ALL=C grep -Paq '[\x00-\x08\x0b\x0c\x0e-\x1f\x7f]' "$action35x_path"
}

capture() {
    local action35x_label=$1
    shift
    local action35x_stdout=$workstation_evidence/$action35x_label.stdout
    local action35x_stderr=$workstation_evidence/$action35x_label.stderr
    local action35x_status=$workstation_evidence/$action35x_label.status
    ...
    safe_capture "$action35x_stdout"
    safe_capture "$action35x_stderr"
    return "$action35x_rc"
}

build_payload() {
    local action35x_repository action35x_source action35x_target action35x_mode
    local action35x_hash action35x_lifecycle action35x_source_path action35x_destination
    ...
    install -m 0600 "$repository_root/Caddy/manifests/action35x-node-b-quarantine.tsv" \
        "$payload_stage/manifests/action35x-node-b-quarantine.tsv"
    while IFS=$'\t' read -r action35x_repository action35x_source action35x_target \
        action35x_mode action35x_hash action35x_lifecycle; do
        [[ "$action35x_repository" = '# repository' ]] && continue
        [[ "$action35x_mode" =~ ^0[0-7]{3}$ && "$action35x_lifecycle" = production-candidate ]]
        [[ "$action35x_target" = /* ]]
        action35x_source_path=${workspace_root}/$action35x_repository/$action35x_source
        regular_file "$action35x_source_path"
        [[ "$(sha256sum "$action35x_source_path" | awk '{ print $1 }')" = "$action35x_hash" ]]
        action35x_destination=$payload_stage/repositories/$action35x_repository/$action35x_source
        install -d -m 0700 "$(dirname -- "$action35x_destination")"
        install -m 0600 "$action35x_source_path" "$action35x_destination"
...
readonly archive=$2
case "$remote_root" in /tmp/caddy-action35x-*) ;; *) exit 64 ;; esac
case "$archive" in /tmp/caddy-action35x-*.tar) ;; *) exit 64 ;; esac
...
case "$evidence_root" in /tmp/caddy-action35x-*) ;; *) exit 64 ;; esac
...
ssh_stream() {
    local action35x_host=$1
    local action35x_program=$2
    shift 2
    "$ssh_command" "$action35x_host" \
        "cd / && sudo -n /bin/bash -s --$(printf ' %q' "$@")" <"$action35x_program"
}

upload_payload() {
    local action35x_role=$1
    local action35x_host=$2
    local action35x_remote_root=$3
    local action35x_remote_archive=$4
    local action35x_hash

    action35x_hash=$(awk '{ print $1 }' "$workstation_evidence/payload.sha256")
    capture "$action35x_role-upload-prepare" ssh_stream "$action35x_host" \
        "$prepare_program" "$action35x_remote_root" "$action35x_remote_archive"
    capture "$action35x_role-upload-copy" "$scp_command" -p -- \
        "$payload_archive" "$action35x_host:$action35x_remote_archive"
    capture "$action35x_role-upload-accept" ssh_stream "$action35x_host" \
        "$accept_program" "$action35x_remote_root" "$action35x_remote_archive" \
        "$action35x_hash"
}
...
remote_transaction() {
    ...
    capture "$action35x_label" ssh_stream "$action35x_host" "$transaction" "$@"
}

readback() {
    local action35x_role=$1
    local action35x_host=$2
    local action35x_remote_evidence=$3

    capture "$action35x_role-readback" ssh_stream "$action35x_host" \
        "$readback_program" "$action35x_remote_evidence"
}
...
cleanup_remote() {
    ...
    capture "$action35x_role-disposition" ssh_stream "$action35x_host" \
        "$disposition_program" "$action35x_remote_root" "$action35x_remote_archive"
}

run_live() {
    local action35x_node_b_mutated=false
    local action35x_node_a_mutated=false
    local action35x_failure=0
    local action35x_phase_status=0
    ...
    if upload_payload node-b "$node_b_host" "$node_b_payload" "$node_b_archive" &&
        ...
        return "$action35x_phase_status"
    fi

    action35x_node_b_mutated=true
    ...
    if [[ "$action35x_failure" -eq 0 ]]; then
    ...
    if ! remote_transaction node-a-journal-capture "$node_a_host" journal-capture node-a \
        "$node_a_payload" "$node_a_evidence"; then
        [[ "$action35x_failure" -ne 0 ]] || action35x_failure=1
    fi
    ...
    if [[ "$action35x_failure" -ne 0 ]]; then
        if [[ "$action35x_node_a_mutated" = true ]]; then
            remote_transaction node-a-rollback "$node_a_host" rollback node-a \
                "$node_a_payload" "$node_a_evidence" || exit 125
        fi
        if [[ "$action35x_node_b_mutated" = true ]]; then
...
production_path_test() {
    ...
    local action35x_remote_base action35x_raw action35x_decision action35x_observed
    local action35x_status action35x_role action35x_remote_evidence action35x_test_id
    local action35x_tmpfiles_line action35x_dns_line action35x_proxy_line
    local action35x_keepalived_line
    local retained_root retained_candidate retained_release_hash retained_payload_hash
    local action35x_transaction_test_evidence
    ...
    action35x_remote_base=$(mktemp -d /tmp/caddy-action35x-production-path.XXXXXX)
    action35x_test_id=${action35x_remote_base##*.}
    test_remote_base=$action35x_remote_base
    export ACTION35X_TEST_REMOTE_BASE=$test_remote_base
...
[[ "$1" = -n ]]
shift
export ACTION35X_PRODUCTION_PATH_TEST=1
exec "$@"
SUDO
...
set -Eeuo pipefail
[[ "$1" = -u && "$3" = -- ]]
printf '%s\n' "$2" >>"$ACTION35X_TEST_REMOTE_BASE/runuser.identities"
printf 'runuser %s\n' "$2" >>"$ACTION35X_TEST_REMOTE_BASE/ordering.calls"
shift 3
exec "$@"
RUNUSER
...
#!/usr/bin/env bash
set -Eeuo pipefail
printf '%s\n' "$*" >>"$ACTION35X_TEST_REMOTE_BASE/prohibited-keepalived-parser.calls"
exit 97
KEEPALIVED
...
#!/usr/bin/env bash
set -Eeuo pipefail
printf '%s\n' "$*" >>"$ACTION35X_TEST_REMOTE_BASE/systemctl.calls"
printf 'systemctl %s\n' "$*" >>"$ACTION35X_TEST_REMOTE_BASE/ordering.calls"
case "$1" in
    is-active) [[ "$2" = --quiet ]] ;;
    start | stop) [[ $# -eq 2 ]] ;;
...
#!/usr/bin/env bash
set -Eeuo pipefail
[[ "$1" = --create && "$2" = "${ACTION35X_TARGET_ROOT:?}/etc/tmpfiles.d/caddy-ha.conf" ]]
install -d -m 0755 "$ACTION35X_TARGET_ROOT/run/caddy-serving-health"
install -d -m 0755 "$ACTION35X_TARGET_ROOT/run/caddy-serving-health/dns"
install -d -m 0755 "$ACTION35X_TARGET_ROOT/run/caddy-serving-health/proxy"
install -d -m 0755 "$ACTION35X_TARGET_ROOT/run/caddy-serving-health/keepalived"
printf '%s\n' "$*" >>"$ACTION35X_TEST_REMOTE_BASE/tmpfiles.calls"
printf 'tmpfiles %s\n' "$*" >>"$ACTION35X_TEST_REMOTE_BASE/ordering.calls"
TMPFILES
...
set -Eeuo pipefail
case " $* " in
    *' --show-cursor '*) printf '%s\n' '-- cursor: s=action35x-production-path' ;;
    *' --after-cursor '*) printf '%s\n' 'test-host caddy.service: action35x production-path event' ;;
    *) exit 64 ;;
esac
JOURNALCTL
    cat >"$ssh_command" <<'SSH'
...
shift
command="$*"
printf '%s\t%s\n' "$host" "$command" >>"$ACTION35X_TEST_REMOTE_BASE/ssh.calls"
PATH="$ACTION35X_TEST_REMOTE_BASE/bin:/usr/bin:/bin" /bin/bash -c "$command"
SSH
    cat >"$scp_command" <<'SCP'
#!/usr/bin/env bash
set -Eeuo pipefail
...
source=$3
target=${4#*:}
printf '%s\t%s\n' "$source" "$target" >>"$ACTION35X_TEST_REMOTE_BASE/scp.calls"
...
SCP
    ...
    node_a_payload=/tmp/caddy-action35x-test-$action35x_test_id-node-a
    node_b_payload=/tmp/caddy-action35x-test-$action35x_test_id-node-b
    ...
    ACTION35X_INCOMING_ROOT=$retained_root/incoming \
        ACTION35X_OUTGOING_ROOT=$retained_root/outgoing \
        ACTION35X_QUARANTINE_ROOT=$retained_root/quarantine \
        ACTION35X_RELEASES_ROOT=$retained_root/releases \
        ACTION35X_RETAINED_RELEASE_MANIFEST_SHA256=$retained_release_hash \
        ACTION35X_RETAINED_PAYLOAD_MANIFEST_SHA256=$retained_payload_hash \
    ...
    ACTION35X_ENVIRONMENT_FILE=$test_remote_base/environment-node-b \
        ACTION35X_UNBOUND_CHECKCONF_COMMAND=$test_remote_base/bin/unbound-checkconf \
        ACTION35X_RUNUSER_COMMAND=$test_remote_base/bin/runuser \
        ACTION35X_SYSTEMCTL_COMMAND=$test_remote_base/bin/systemctl \
        ACTION35X_DNS_DIG_COMMAND=$test_remote_base/bin/dig \
        ACTION35X_CURL_COMMAND=$test_remote_base/bin/curl \
        ACTION35X_SS_COMMAND=$test_remote_base/bin/ss \
    ...
    ACTION35X_JOURNALCTL_COMMAND=$test_remote_base/bin/journalctl \
    ...
    ACTION35X_TARGET_ROOT=$test_remote_base/node-b-root \
        ACTION35X_SYSTEMCTL_COMMAND=$test_remote_base/bin/systemctl \
        ACTION35X_SYSTEMD_TMPFILES_COMMAND=$test_remote_base/bin/systemd-tmpfiles \
        ACTION35X_RUNUSER_COMMAND=$test_remote_base/bin/runuser \
        ACTION35X_DNS_DIG_COMMAND=$test_remote_base/bin/dig \
        ACTION35X_CURL_COMMAND=$test_remote_base/bin/curl \
        ACTION35X_SS_COMMAND=$test_remote_base/bin/ss \
        ...
        "$node_b_payload" "$node_b_evidence"
    action35x_raw=$action35x_test_root/raw/status-snapshot-initialization-before-keepalived-reload.txt
    action35x_decision=$action35x_test_root/decisions/status-snapshot-initialization-before-keepalived-reload.tsv
    install -m 0600 "$test_remote_base/ordering.calls" "$action35x_raw"
    action35x_tmpfiles_line=$(grep -n '^tmpfiles --create ' "$action35x_raw" | cut -d: -f1)
    action35x_dns_line=$(grep -n '^runuser pi$' "$action35x_raw" | cut -d: -f1)
    action35x_proxy_line=$(grep -n '^runuser keepalived_script$' "$action35x_raw" | cut -d: -f1)
    action35x_keepalived_line=$(grep -n '^systemctl reload keepalived.service$' \
        "$action35x_raw" | cut -d: -f1)
    [[ "$action35x_tmpfiles_line" -lt "$action35x_dns_line" &&
        "$action35x_dns_line" -lt "$action35x_proxy_line" &&
        "$action35x_proxy_line" -lt "$action35x_keepalived_line" ]]
    ...
    action35x_transaction_test_evidence=$action35x_test_root/transaction-through-outer
    install -d -m 0700 "$action35x_transaction_test_evidence"
    CADDY_PRODUCTION_PATH_EVIDENCE_ROOT=$action35x_transaction_test_evidence \
        ACTION35X_TEST_REPOSITORY_ROOT=$repository_root \
        remote_transaction node-a-post-promote-production-path "$node_a_host" \
        --production-path-test
    ACTION35X_TARGET_ROOT=$test_remote_base/node-a-root \
        ACTION35X_SYSTEMCTL_COMMAND=$test_remote_base/bin/systemctl \
        ACTION35X_SYSTEMD_TMPFILES_COMMAND=$test_remote_base/bin/systemd-tmpfiles \
        ACTION35X_RUNUSER_COMMAND=$test_remote_base/bin/runuser \
        ACTION35X_DNS_DIG_COMMAND=$test_remote_base/bin/dig \
    ...
    action35x_raw=$action35x_test_root/raw/outer-preflight.txt
    action35x_decision=$action35x_test_root/decisions/outer-preflight.tsv
    cat "$test_remote_base/ssh.calls" "$test_remote_base/scp.calls" \
        ...
        >"$action35x_raw"
    action35x_observed=$(wc -l <"$action35x_raw")
    write_decision outer-preflight reach 0 "$action35x_observed" \
        "$action35x_observed" "$action35x_raw" "$action35x_decision"

    for action35x_role in node-a node-b; do
        if [[ "$action35x_role" = node-a ]]; then
            action35x_remote_evidence=$node_a_evidence
        else
            action35x_remote_evidence=$node_b_evidence
        fi
        action35x_raw=$action35x_test_root/raw/evidence-readback-$action35x_role-success.txt
        action35x_decision=$action35x_test_root/decisions/evidence-readback-$action35x_role-success.tsv
        ssh_stream "test-$action35x_role" "$readback_program" \
            "$action35x_remote_evidence" >"$action35x_raw"
        action35x_observed=$(sha256sum "$action35x_raw" | awk '{ print $1 }')
        write_decision "evidence-readback-$action35x_role-success" accept 0 \
            "$action35x_observed" "$action35x_observed" "$action35x_raw" "$action35x_decision"

        action35x_raw=$action35x_test_root/raw/evidence-readback-$action35x_role-failure.txt
        action35x_decision=$action35x_test_root/decisions/evidence-readback-$action35x_role-failure.tsv
        if ssh_stream "test-$action35x_role" "$readback_program" \
            "$action35x_remote_evidence-absent" >"$action35x_raw" 2>&1; then
            action35x_status=0
        else
            action35x_status=$?
        fi
        write_decision "evidence-readback-$action35x_role-failure" reject \
            "$action35x_status" present absent "$action35x_raw" "$action35x_decision"
    done
    cleanup_remote node-a "$node_a_host" "$node_a_payload" "$node_a_archive"
    cleanup_remote node-b "$node_b_host" "$node_b_payload" "$node_b_archive"
    chmod -R u+rwX -- "$action35x_remote_base"
    rm -rf -- "$action35x_remote_base"
...
transaction=$repository_root/Caddy/scripts/apply-coupled-serving-health-action35x.sh
...
if [[ "$invocation_mode" = --production-path-test ]]; then
    workstation_evidence=$(mktemp -d /tmp/caddy-action35x-outer-test.XXXXXX)
else
    workstation_evidence=$(mktemp -d /tmp/caddy-ssh-evidence-action35x.XXXXXX)
fi
readonly workstation_evidence
chmod 0700 "$workstation_evidence"
payload_stage=$workstation_evidence/payload
payload_archive=$workstation_evidence/action35x-payload.tar
...
readonly run_id
node_a_payload=/tmp/caddy-action35x-$run_id-node-a
node_b_payload=/tmp/caddy-action35x-$run_id-node-b
node_a_archive=$node_a_payload.tar
node_b_archive=$node_b_payload.tar
node_a_evidence=$node_a_payload/evidence
node_b_evidence=$node_b_payload/evidence
