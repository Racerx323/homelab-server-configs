...
record_check() {
    local action28e_a_label=$1

    shift
    if [[ -n "${seen_checks[$action28e_a_label]+set}" ]]; then
        printf '%s_duplicate_check=%s\n' "$prefix" "$action28e_a_label" >&2
        return 1
    fi
    seen_checks[$action28e_a_label]=true
    if "$@"; then
        printf '%s_check_%s=true\n' "$prefix" "$action28e_a_label"
        return 0
    fi
...
marker_state() {
    local action28e_a_marker=$1

    if [[ ! -e "$action28e_a_marker" && ! -L "$action28e_a_marker" ]]; then
        printf 'absent\n'
    elif [[ -f "$action28e_a_marker" && ! -L "$action28e_a_marker" &&
        ! -s "$action28e_a_marker" ]]; then
...
manifest_file_set_matches() {
    local action28e_a_release=$1
    local action28e_a_expected
    local action28e_a_observed

    action28e_a_expected=$(mktemp /tmp/action28e-a-expected.XXXXXX) || return 1
    action28e_a_observed=$(mktemp /tmp/action28e-a-observed.XXXXXX) || {
        rm -f -- "$action28e_a_expected"
        return 1
    }
    awk '{ print substr($0, 67) }' "$action28e_a_release/manifest.sha256" |
        LC_ALL=C sort -u >"$action28e_a_expected"
    (
        cd "$action28e_a_release"
        find . -type f \
            ...
            -print | LC_ALL=C sort
    ) >"$action28e_a_observed"
    local action28e_a_status=0
    cmp -s "$action28e_a_expected" "$action28e_a_observed" || action28e_a_status=$?
    rm -f -- "$action28e_a_expected" "$action28e_a_observed"
    return "$action28e_a_status"
}
manifest_hashes_valid() {
    local action28e_a_release=$1

    (cd "$action28e_a_release" && sha256sum --strict --check manifest.sha256 >/dev/null)
}
current_tree_accepted() {
    local action28e_a_observed_tree

    action28e_a_observed_tree=$(tree_digest "$(root_path /etc/caddy/current)") || return 1
    if [[ "$role" == node-a ]]; then
        [[ "$action28e_a_observed_tree" = "$accepted_current_tree_sha256" ]]
    else
        valid_sha256 "$action28e_a_observed_tree"
    fi
}
unit_state() {
    local action28e_a_unit=$1

    if [[ "$test_mode" == true ]]; then
        case "$action28e_a_unit" in
            caddy.service | lighttpd.service | keepalived.service) printf 'active\n' ;;
            *) printf 'inactive\n' ;;
        esac
    else
        systemctl show "$action28e_a_unit" --no-pager --property ActiveState --value
    fi
}
address_count() {
    local action28e_a_family=$1
    local action28e_a_cidr=$2

    if [[ "$test_mode" == true ]]; then
        if [[ "$action28e_a_cidr" == "$physical_ipv4_cidr" ||
            "$action28e_a_cidr" == "$physical_ipv6_cidr" ]]; then
            printf '1\n'
    ...
    case "$action28e_a_family" in
        4) ip -o -4 addr show dev "$interface" | awk -v cidr="$action28e_a_cidr" '$4 == cidr { count++ } END { print count + 0 }' ;;
        6) ip -o -6 addr show dev "$interface" | awk -v cidr="$action28e_a_cidr" '$4 == cidr { count++ } END { print count + 0 }' ;;
        *) return 1 ;;
    esac
}
vrrp_state() {
...
snapshot() {
    local action28e_a_unit
    local action28e_a_current

    action28e_a_current=$(root_path /etc/caddy/current)
    {
        printf 'hostname=%s\n' "$expected_hostname"
        printf 'vrrp=%s\n' "$(vrrp_state)"
        printf 'current_link=%s\n' "$(readlink -- "$action28e_a_current")"
        printf 'current_target=%s\n' "$(readlink -f -- "$action28e_a_current")"
        printf 'current_tree=%s\n' "$(tree_digest "$action28e_a_current")"
        for action28e_a_unit in "${continuity_units[@]}"; do
            printf 'unit=%s|%s\n' "$action28e_a_unit" "$(unit_state "$action28e_a_unit")"
        done
...
historical_integrity() {
    if [[ "$role" == node-a ]]; then
        [[ "$(tree_digest "$historical_release")" == "$historical_outbound_tree_sha256" ]]
    else
        [[ "$(file_hash "$historical_release/release-manifest.json")" == "$historical_manifest_sha256" ]]
    fi
}
...
candidate_role_marker_state() {
    ...
    if [[ "$role" == node-a ]]; then
        [[ ! -e "$action28e_a_release/.complete" ]]
    else
        [[ -f "$action28e_a_release/.complete" &&
            ! -L "$action28e_a_release/.complete" &&
            ! -s "$action28e_a_release/.complete" ]]
    fi
}
candidate_optional_check() {
    ...
    if [[ "$candidate_name" == absent ]]; then
        record_check "$action28e_a_label" true
    else
        record_check "$action28e_a_label" "$@"
    fi
}
...
case "${1:-}" in
    ...
    node-a | node-b)
        ...
        ;;
    --test-root)
        [[ $# -eq 3 && "${CADDY_ACTION28E_A_TEST_MODE:-}" == 1 ]]
        test_mode=true
        root_prefix=$2
        role=$3
        ;;
