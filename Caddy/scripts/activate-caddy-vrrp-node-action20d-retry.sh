...
readonly prefix=action_20d_retry_node
...
expected_checks() {
    printf '%s\n' \
        ...
        fragment_owner fragment_mode fragment_hash fragment_health_command_exact \
        fragment_health_user_exact fragment_notification_command_exact \
        ...
        sanitized_candidate_config_valid \
        candidate_regular candidate_include_once candidate_config_valid \
        production_fragment_hash_preinstall \
        backup_created \
        backup_file_exact backup_manifest_exact main_installed \
        main_installed_exact reload_status \
...
case "${1:-}" in
    ...
    --self-test)
        [[ $# -eq 1 ]] || exit 64
        [[ "$(expected_checks | wc -l)" -eq 93 ]]
        [[ "$(expected_checks | LC_ALL=C sort -u | wc -l)" -eq 93 ]]
    ...
    --inspect)
        ...
        readonly node_role
        configure_role
        transaction_root=$(mktemp -d /run/caddy-action20d-retry-inspect.XXXXXX)
    ...
    --rollback)
        ...
        configure_role
        transaction_root=$(mktemp -d /run/caddy-action20d-retry-rollback.XXXXXX)
        readonly transaction_root
...
transaction_root=$(mktemp -d /run/caddy-action20d-retry-node.XXXXXX)
readonly transaction_root
...
record_captured_check sanitized_candidate_config_valid sanitized_candidate_config \
    "$transaction_root" \
    keepalived --config-test="$transaction_root/keepalived.sanitized.log" \
...
record_check candidate_include_once test \
    "$(grep -Fxc "$include_record" "$production_configuration")" -eq 1
# Compatibility label: execution reaches this point only after the independently
# labeled sanitized_candidate_config_valid gate returned success.
record_check candidate_config_valid true
...
printf '%s_validation_scope=sanitized_ephemeral_candidate_only\n' "$prefix"
