...
readonly -a continuity_properties=(
    ActiveState
    SubState
    MainPID
    NRestarts
...
require_check() {
    ...
    shift
    if "$@"; then
        printf 'action_17q_check_%s=true\n' "$check_label"
        return 0
    fi
    printf 'action_17q_check_%s=false\n' "$check_label" >&2
    return 1
}
...
capture_service_state() {
    ...
    for unit in "${continuity_units[@]}"; do
        unit_label=${unit//[-.]/_}
        for property in "${continuity_properties[@]}"; do
            property_value=$(
...
rollback() {
    ...
    set +e
    printf 'action_17q_rollback_started=true\n' >&2
    ...
    if [[ "$rollback_failed" == true ]]; then
        printf 'action_17q_rollback_complete=false\n' >&2
        printf 'manual_intervention_required=true\n' >&2
        exit 125
    fi
    printf 'action_17q_rollback_complete=true\n' >&2
    exit "$original_status"
}
...
if [[ "${1:-}" == --self-test && $# -eq 1 ]]; then
    ...
    printf 'action_17q_installer_self_test_complete=true\n'
    exit 0
fi
...
printf 'action_17q_preflight_complete=true\n'

trap rollback EXIT
mutation_started=true
printf 'action_17q_mutation_started=true\n'
install -d -o root -g root -m 0700 "$rollback_root"
rollback_directory=$(
    mktemp -d "$rollback_root/action17q-node-b-protocol-v2.XXXXXX"
)
...
printf 'action_17q_receiver_invoked=false\n'
printf 'action_17q_finalizer_invoked=false\n'
printf 'action_17q_release_mutated=false\n'
printf 'action_17q_lsyncd_enabled=false\n'
printf 'action_17q_reconciliation_enabled=false\n'
printf 'action_17q_service_mutations=false\n'
printf 'action_17q_backup_path=%s\n' "$rollback_directory"
printf 'action_17q_persistent_mutation_scope=receiver_v2,finalizer_v2,authorized_keys,rollback_backup\n'

transaction_complete=true
trap - EXIT
printf 'action_17q_node_b_protocol_v2_install_complete=true\n'
