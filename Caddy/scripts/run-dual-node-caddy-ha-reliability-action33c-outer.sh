...
readonly prefix=action_33c_outer
...
readonly evidence_root=/tmp/caddy-ssh-evidence/action33c
readonly node_evidence_root=/tmp/caddy-action33c
readonly maximum_stream_bytes=8388608
readonly maximum_stream_lines=65536
script_directory=$(cd -- "$(dirname -- "${BASH_SOURCE[0]}")" && pwd)
readonly script_directory
readonly caddy_root=${script_directory%/scripts}
readonly transaction=$script_directory/transact-caddy-ha-reliability-action33c.sh
...
readonly regression=$caddy_root/tests/caddy-ha-reliability-action33c-regression.sh
ssh_binary=${CADDY_ACTION33C_SSH_BIN:-/usr/bin/ssh}
readonly ssh_binary
scp_binary=${CADDY_ACTION33C_SCP_BIN:-/usr/bin/scp}
readonly scp_binary
...
safe_stream() {
    local action33c_outer_stream=$1
    [[ "$(wc -c <"$action33c_outer_stream")" -le "$maximum_stream_bytes" ]] || return 1
    [[ "$(line_count "$action33c_outer_stream")" -le "$maximum_stream_lines" ]] || return 1
    iconv -f UTF-8 -t UTF-8 "$action33c_outer_stream" >/dev/null 2>&1 || return 1
    ! LC_ALL=C.UTF-8 grep -n '[^[:print:][:space:]]' "$action33c_outer_stream" >/dev/null || return 1
    ! grep -Eqi 'BEGIN [A-Z ]*PRIVATE KEY|Authorization:[[:space:]]*Bearer|WEBPASSWORD' "$action33c_outer_stream"
}
capture_files() {
    local action33c_outer_path
    for action33c_outer_path in "$@"; do
        install -m 0600 /dev/null "$action33c_outer_path"
        chmod 0600 "$action33c_outer_path"
    done
}
emit_stream() {
    local action33c_outer_label=$1
    local action33c_outer_path=$2
    printf '%s_%s_bytes=%s\n' "$prefix" "$action33c_outer_label" "$(wc -c <"$action33c_outer_path")"
    printf '%s_%s_lines=%s\n' "$prefix" "$action33c_outer_label" "$(line_count "$action33c_outer_path")"
    printf '%s_%s_sha256=%s\n' "$prefix" "$action33c_outer_label" "$(file_hash "$action33c_outer_path")"
    safe_stream "$action33c_outer_path" || return 97
    printf '%s_%s_classification=bounded_safe\n' "$prefix" "$action33c_outer_label"
    if [[ -s "$action33c_outer_path" ]]; then
        printf '%s_%s_begin\n' "$prefix" "$action33c_outer_label"
        sed "s/^/${prefix}_${action33c_outer_label}_content=/" \
            "$action33c_outer_path"
        printf '%s_%s_end\n' "$prefix" "$action33c_outer_label"
    else
...
gate() {
    ...
    printf '%s_gate_%s=false\n' "$prefix" "$action33c_outer_label" >&2
    return 1
}
run_remote() {
    local action33c_outer_label=$1 target=$2 alias=$3 role=$4 mode=$5 scenario=$6 argument=${7:-}
    local action33c_outer_stdout=$work_root/$action33c_outer_label.stdout
    local action33c_outer_stderr=$work_root/$action33c_outer_label.stderr
    local action33c_outer_status_file=$work_root/$action33c_outer_label.status
    local action33c_outer_status=0
    capture_files "$action33c_outer_stdout" "$action33c_outer_stderr" "$action33c_outer_status_file"
    "$ssh_binary" -T -o BatchMode=yes -o StrictHostKeyChecking=yes -o "HostKeyAlias=$alias" -o ConnectTimeout=10 -o ConnectionAttempts=1 "$target" \
        "cd / && sudo -n /bin/bash -s -- $mode $role $run_id $scenario $argument" <"$transaction" >"$action33c_outer_stdout" 2>"$action33c_outer_stderr" || action33c_outer_status=$?
    printf '%s\n' "$action33c_outer_status" >"$action33c_outer_status_file"
    printf '%s_%s_status=%s\n' "$prefix" "$action33c_outer_label" "$action33c_outer_status"
    emit_stream "remote_stdout_$action33c_outer_label" "$action33c_outer_stdout"
    emit_stream "remote_stderr_$action33c_outer_label" "$action33c_outer_stderr"
    [[ "$action33c_outer_status" -eq 0 ]]
}
run_local() {
    local action33c_outer_label=$1
    local action33c_outer_stdout=$work_root/$action33c_outer_label.stdout
    ...
    shift
    capture_files "$action33c_outer_stdout" "$action33c_outer_stderr" \
        "$action33c_outer_status_file"
    "$@" >"$action33c_outer_stdout" 2>"$action33c_outer_stderr" ||
        action33c_outer_status=$?
    printf '%s\n' "$action33c_outer_status" >"$action33c_outer_status_file"
    printf '%s_%s_status=%s\n' "$prefix" "$action33c_outer_label" \
        "$action33c_outer_status"
    emit_stream "local_stdout_$action33c_outer_label" "$action33c_outer_stdout"
    emit_stream "local_stderr_$action33c_outer_label" "$action33c_outer_stderr"
    [[ "$action33c_outer_status" -eq 0 ]]
}
upload_registry() {
    local action33c_outer_label=$1
    local action33c_outer_target=$2
    local action33c_outer_alias=$3
    local action33c_outer_stdout=$work_root/$action33c_outer_label.stdout
    local action33c_outer_stderr=$work_root/$action33c_outer_label.stderr
    local action33c_outer_status_file=$work_root/$action33c_outer_label.status
    local action33c_outer_status=0
    ...
    "$scp_binary" -q -p -o BatchMode=yes -o StrictHostKeyChecking=yes \
        -o "HostKeyAlias=$action33c_outer_alias" -o ConnectTimeout=10 \
        -o ConnectionAttempts=1 "$registry" \
        "$action33c_outer_target:/tmp/caddy-action33c-registry-$run_id.tsv" \
        >"$action33c_outer_stdout" 2>"$action33c_outer_stderr" ||
        action33c_outer_status=$?
    printf '%s\n' "$action33c_outer_status" >"$action33c_outer_status_file"
    printf '%s_%s_status=%s\n' "$prefix" "$action33c_outer_label" \
...
wait_ssh_state() {
    local action33c_outer_label=$1 target=$2 alias=$3 expected=$4 limit=$5 elapsed=0 status
    while ((elapsed < limit)); do
        ...
        if { [[ "$expected" = up && "$status" -eq 0 ]]; } || { [[ "$expected" = down && "$status" -ne 0 ]]; }; then
            printf '%s_%s_elapsed_seconds=%s\n' "$prefix" "$action33c_outer_label" "$elapsed"
            return 0
        fi
        sleep 1
        elapsed=$((elapsed + 1))
...
start_availability() {
    local action33c_outer_scenario=$1
    local action33c_outer_output=$work_root/$action33c_outer_scenario.availability
    capture_files "$action33c_outer_output"
    (
        while :; do
            printf '%s\t' "$(date +%s%N)" >>"$action33c_outer_output"
            dig +time=1 +tries=1 @10.1.0.55 proxy.local.theama.co A +short >>"$action33c_outer_output" 2>&1 &&
                curl --fail --silent --show-error --max-time 2 https://proxy.local.theama.co/ -o /dev/null >>"$action33c_outer_output" 2>&1 &&
                curl --fail --silent --show-error --max-time 2 https://pihole-admin.local.theama.co/admin/login.php -o /dev/null >>"$action33c_outer_output" 2>&1 && printf 'ok\n' >>"$action33c_outer_output" || printf 'failed\n' >>"$action33c_outer_output"
            sleep 1
        done
    ) &
    availability_pid=$!
    availability_scenario=$action33c_outer_scenario
}
stop_availability() {
    local action33c_outer_scenario=$1
    ...
    availability_pid=
    availability_scenario=
    gate "${action33c_outer_scenario}_availability_no_failure" test -z "$(grep -F 'failed' "$work_root/$action33c_outer_scenario.availability" || true)"
    emit_stream "${action33c_outer_scenario}_availability" "$work_root/$action33c_outer_scenario.availability"
}
baseline() {
    local action33c_outer_scenario=$1

    run_remote "$action33c_outer_scenario-baseline-a" "$node_a_target" \
        "$node_a_alias" node-a --baseline "$action33c_outer_scenario"
    run_remote "$action33c_outer_scenario-baseline-b" "$node_b_target" \
        "$node_b_alias" node-b --baseline "$action33c_outer_scenario"
    run_remote "$action33c_outer_scenario-master" "$node_a_target" \
        "$node_a_alias" node-a --assert-master "$action33c_outer_scenario"
    run_remote "$action33c_outer_scenario-backup" "$node_b_target" \
        "$node_b_alias" node-b --assert-backup "$action33c_outer_scenario"
    gate "$action33c_outer_scenario-release_identity_equal" test \
        "$(grep -Eo 'observed_current_revision=[A-Za-z0-9._-]+' \
            "$work_root/$action33c_outer_scenario-baseline-a.stdout" | tail -n 1)" = \
        "$(grep -Eo 'observed_current_revision=[A-Za-z0-9._-]+' \
            "$work_root/$action33c_outer_scenario-baseline-b.stdout" | tail -n 1)"
    gate "$action33c_outer_scenario-release_manifest_equal" test \
...
restore_baseline() {
    local action33c_outer_scenario=$1
    run_remote "$action33c_outer_scenario-prepare-b" "$node_b_target" \
        "$node_b_alias" node-b --prepare-cleanup "$action33c_outer_scenario"
    run_remote "$action33c_outer_scenario-prepare-a" "$node_a_target" \
        "$node_a_alias" node-a --prepare-cleanup "$action33c_outer_scenario"
    run_remote "$action33c_outer_scenario-freeze-b" "$node_b_target" \
        "$node_b_alias" node-b --freeze "$action33c_outer_scenario"
    run_remote "$action33c_outer_scenario-freeze-a" "$node_a_target" \
        "$node_a_alias" node-a --freeze "$action33c_outer_scenario"
    run_remote "$action33c_outer_scenario-cleanup-b" "$node_b_target" \
        "$node_b_alias" node-b --cleanup "$action33c_outer_scenario"
    run_remote "$action33c_outer_scenario-cleanup-a" "$node_a_target" \
        "$node_a_alias" node-a --cleanup "$action33c_outer_scenario"
    run_remote "$action33c_outer_scenario-restore-b" "$node_b_target" \
        "$node_b_alias" node-b --restore-services "$action33c_outer_scenario"
    run_remote "$action33c_outer_scenario-restore-a" "$node_a_target" \
        "$node_a_alias" node-a --restore-services "$action33c_outer_scenario"
    run_remote "$action33c_outer_scenario-final-a" "$node_a_target" \
        "$node_a_alias" node-a --final "$action33c_outer_scenario"
    run_remote "$action33c_outer_scenario-final-b" "$node_b_target" \
        "$node_b_alias" node-b --final "$action33c_outer_scenario"
    gate "$action33c_outer_scenario-final_release_identity_equal" test \
        "$(grep -Eo 'observed_current_revision=[A-Za-z0-9._-]+' \
            "$work_root/$action33c_outer_scenario-final-a.stdout" | tail -n 1)" = \
        "$(grep -Eo 'observed_current_revision=[A-Za-z0-9._-]+' \
            "$work_root/$action33c_outer_scenario-final-b.stdout" | tail -n 1)"
    gate "$action33c_outer_scenario-final_release_manifest_equal" test \
        "$(grep -Eo 'observed_release_manifest_sha256=[0-9a-f]{64}' \
            "$work_root/$action33c_outer_scenario-final-a.stdout" | tail -n 1)" = \
        "$(grep -Eo 'observed_release_manifest_sha256=[0-9a-f]{64}' \
            "$work_root/$action33c_outer_scenario-final-b.stdout" | tail -n 1)"
...
normalize_emergency_ancestry() {
    local action33c_outer_scenario=$1
    local action33c_outer_revision=action33c-$run_id-$action33c_outer_scenario-normalized

    run_remote "$action33c_outer_scenario-owner-a" "$node_a_target" \
        "$node_a_alias" node-a --restore-services "$action33c_outer_scenario"
    run_remote "$action33c_outer_scenario-master-a" "$node_a_target" \
        "$node_a_alias" node-a --assert-master "$action33c_outer_scenario"
    run_remote "$action33c_outer_scenario-backup-b" "$node_b_target" \
        "$node_b_alias" node-b --assert-backup "$action33c_outer_scenario"
    run_remote "$action33c_outer_scenario-normal-publish-a" "$node_a_target" \
        "$node_a_alias" node-a --publish-normal "$action33c_outer_scenario" \
        "$action33c_outer_revision"
    run_remote "$action33c_outer_scenario-normal-promote-a" "$node_a_target" \
        "$node_a_alias" node-a --promote-outgoing "$action33c_outer_scenario" \
        "$action33c_outer_revision"
    run_remote "$action33c_outer_scenario-normal-wait-b" "$node_b_target" \
        "$node_b_alias" node-b --wait-current "$action33c_outer_scenario" \
        "$action33c_outer_revision"
}
recover() {
    local action33c_outer_failed=false
    if [[ -n "$availability_pid" ]]; then
        stop_availability "$availability_scenario" || true
    fi
    if [[ "$mutated" = true ]]; then
        restore_baseline "$current_scenario" || action33c_outer_failed=true
    fi
    ...
    [[ "$action33c_outer_failed" = false ]] || exit 125
}
...
run_remote invalid-stage-a "$node_a_target" "$node_a_alias" node-a \
    --stage-invalid "$scenario" "action33c-$run_id-$scenario"
sleep 8
run_remote invalid-accept-b "$node_b_target" "$node_b_alias" node-b \
    --assert-invalid "$scenario" "action33c-$run_id-$scenario"
...
current_scenario=$scenario
conflict_one=action33c-$run_id-conflict-one
conflict_two=action33c-$run_id-conflict-two
baseline "$scenario"
...
run_remote emergency-b "$node_b_target" "$node_b_alias" node-b --publish-emergency emergency-online "action33c-$run_id-emergency-online"
run_remote emergency-promote-b "$node_b_target" "$node_b_alias" node-b \
    --promote-outgoing emergency-online "action33c-$run_id-emergency-online"
...
run_remote emergency-wait-a "$node_a_target" "$node_a_alias" node-a \
    --wait-current emergency-online "action33c-$run_id-emergency-online"
...
for scenario in node-a-controlled node-a-reboot node-b-controlled node-b-reboot; do
    ...
    case "$scenario" in
        node-a-controlled)
            ...
            run_remote publish-b "$node_b_target" "$node_b_alias" node-b --publish-emergency "$scenario" "action33c-$run_id-$scenario"
            run_remote promote-b "$node_b_target" "$node_b_alias" node-b \
                --promote-outgoing "$scenario" "action33c-$run_id-$scenario"
            run_remote queued-b "$node_b_target" "$node_b_alias" node-b \
                --assert-queued "$scenario" "action33c-$run_id-$scenario"
            sleep 12
            ...
            run_remote wait-a "$node_a_target" "$node_a_alias" node-a \
                --wait-current "$scenario" "action33c-$run_id-$scenario"
            ;;
        node-a-reboot)
        ...
        node-b-controlled)
            ...
            run_remote publish-a "$node_a_target" "$node_a_alias" node-a --publish-normal "$scenario" "action33c-$run_id-$scenario"
