...
readonly prefix=action_17r_c_node_a
...
if [[ "${1:-}" == --self-test && $# -eq 1 ]]; then
    ...
    printf 'action_17r_c_node_a_inspector_self_test_complete=true\n'
    exit 0
elif (($#)); then
...
work_directory=$(mktemp -d /tmp/caddy-action17r-c-node-a.XXXXXX)
...
ipv4_status=0
runuser -u caddy-sync -- ssh -G -T -F /dev/null -4 \
...
ipv6_status=0
runuser -u caddy-sync -- ssh -G -T -F /dev/null -6 \
