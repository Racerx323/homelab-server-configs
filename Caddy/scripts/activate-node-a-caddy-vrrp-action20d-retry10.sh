...
readonly prefix=action_20d_retry10_node
...
configure_role() {
    case "$node_role" in
        node-a)
            ...
            expected_health_user_line='    user keepalived_script caddy-tls'
        ...
        node-b)
            ...
            expected_health_user_line='    user keepalived_script'
    ...
    readonly expected_health_user_line
...
build_production_candidate() {
    local production_source_path=$1
    local production_output_path=$2

    cp --preserve=mode,ownership,timestamps \
        "$production_source_path" "$production_output_path" || return 1
    printf '\n%s\n' "$include_record" >>"$production_output_path" || return 1
    chmod 0600 "$production_output_path" || return 1
}
candidate_contract_check() {
    local candidate_contract_label=$1

    shift
    if "$@"; then
        printf '%s_candidate_contract_%s=true\n' "$prefix" \
            "$candidate_contract_label"
        return 0
    fi
    printf '%s_candidate_contract_%s=false\n' "$prefix" \
        "$candidate_contract_label" >&2
    return 1
}
run_candidate_contract_test() {
    local candidate_contract_root
    local candidate_contract_source
    local candidate_contract_actual
    local candidate_contract_expected
    local candidate_contract_missing_blank
    local candidate_contract_extra_blank
    local candidate_contract_missing_terminal_newline
    local candidate_contract_reordered
    local candidate_contract_embedded_fragment

    candidate_contract_root=$(mktemp -d /tmp/caddy-action20d-retry10-candidate.XXXXXX) ||
        return 1
    trap 'rm -rf -- "$candidate_contract_root"' RETURN
    candidate_contract_source=$candidate_contract_root/main.conf
    candidate_contract_actual=$candidate_contract_root/actual.conf
    candidate_contract_expected=$candidate_contract_root/expected.conf
    candidate_contract_missing_blank=$candidate_contract_root/missing-blank.conf
    candidate_contract_extra_blank=$candidate_contract_root/extra-blank.conf
    candidate_contract_missing_terminal_newline=$candidate_contract_root/missing-terminal-newline.conf
    candidate_contract_reordered=$candidate_contract_root/reordered.conf
    candidate_contract_embedded_fragment=$candidate_contract_root/embedded-fragment.conf

    printf '%s\n' 'global_defs {' '    router_id fixture' '}' \
        >"$candidate_contract_source"
    printf '%s\n' 'global_defs {' '    router_id fixture' '}' '' \
        "$include_record" >"$candidate_contract_expected"
    printf '%s\n' 'global_defs {' '    router_id fixture' '}' \
        "$include_record" >"$candidate_contract_missing_blank"
    printf '%s\n' 'global_defs {' '    router_id fixture' '}' '' '' \
        "$include_record" >"$candidate_contract_extra_blank"
    printf '%s\n' 'global_defs {' '    router_id fixture' '}' '' \
        >"$candidate_contract_missing_terminal_newline"
    printf '%s' "$include_record" >>"$candidate_contract_missing_terminal_newline"
    printf '%s\n' "$include_record" '' 'global_defs {' \
        '    router_id fixture' '}' >"$candidate_contract_reordered"
    printf '%s\n' 'global_defs {' '    router_id fixture' '}' '' \
        'vrrp_instance CADDY_IPV4 {' '}' \
        >"$candidate_contract_embedded_fragment"

    build_production_candidate "$candidate_contract_source" \
        "$candidate_contract_actual"
    candidate_contract_check exact_bytes cmp -s \
        "$candidate_contract_expected" "$candidate_contract_actual"
    candidate_contract_check mode_exact test \
        "$(stat -c '%a' "$candidate_contract_actual")" = 600
    candidate_contract_check missing_blank_bytes_differ test \
        "$(file_hash "$candidate_contract_missing_blank")" != \
        "$(file_hash "$candidate_contract_actual")"
    candidate_contract_check extra_blank_rejected test \
        "$(file_hash "$candidate_contract_extra_blank")" != \
        "$(file_hash "$candidate_contract_actual")"
    candidate_contract_check missing_terminal_newline_rejected test \
        "$(file_hash "$candidate_contract_missing_terminal_newline")" != \
        "$(file_hash "$candidate_contract_actual")"
    candidate_contract_check reordered_include_rejected test \
        "$(file_hash "$candidate_contract_reordered")" != \
        "$(file_hash "$candidate_contract_actual")"
    candidate_contract_check embedded_fragment_rejected test \
        "$(file_hash "$candidate_contract_embedded_fragment")" != \
        "$(file_hash "$candidate_contract_actual")"
    candidate_contract_check include_exactly_once test \
        "$(grep -Fxc "$include_record" "$candidate_contract_actual")" -eq 1
    candidate_contract_check line_count_exact test \
        "$(line_count "$candidate_contract_actual")" -eq 5
    rm -rf -- "$candidate_contract_root"
    trap - RETURN
    printf '%s_candidate_contract_complete=true\n' "$prefix"
}
rollback_check() {
...
case "${1:-}" in
    ...
    --inspect)
        ...
        configure_role
        [[ "$node_role" = node-a ]] || exit 64
        transaction_root=$(mktemp -d /run/caddy-action20d-retry10-inspect.XXXXXX)
    ...
    --rollback)
        ...
        configure_role
        [[ "$node_role" = node-a ]] || exit 64
        transaction_root=$(mktemp -d /run/caddy-action20d-retry10-rollback.XXXXXX)
        readonly transaction_root
        ...
        ;;
    *)
        printf 'Usage: %s --activate node-a | --inspect node-a | --rollback node-a BACKUP | --expected-checks | --expected-inspection-checks | --self-test | --candidate-contract-test\n' \
            "${0##*/}" >&2
        exit 64
        ;;
esac

transaction_root=$(mktemp -d /run/caddy-action20d-retry10-node.XXXXXX)
readonly transaction_root
...
record_check fragment_health_user_exact test \
    "$(grep -Fxc "$expected_health_user_line" "$fragment")" -eq 1
...
build_production_candidate "$main_configuration" "$production_configuration"
...

{
    ...
    sed -e '/^[[:space:]]*notify "/d' \
        -e 's#^[[:space:]]*script "[^"]*"#    script "/bin/true"#' \
        -e 's/^[[:space:]]*user keepalived_script caddy-tls[[:space:]]*$/    user root/' \
        -e 's/^[[:space:]]*user keepalived_script[[:space:]]*$/    user root/' \
        "$fragment"
...
backup_directory=$(mktemp -d "$backup_root/action20d-retry10-${node_role}-caddy-vrrp.XXXXXX")
