...
readonly caddy_root
readonly inspector="$script_directory/inspect-node-b-protocol-v2-postfailure-action17q-a.sh"
...
transcript_grammar_valid() {
    ...
    awk '
        index($0, "action_17q_a_") != 1 { invalid++ }
        $0 !~ /^action_17q_a_[a-z0-9_]+=[A-Za-z0-9_.\/:-]+$/ {
            invalid++
        }
...
evaluate_contract() {
    ...
    for contract_assertion in "${expected_assertions[@]}"; do
        [[ "$(grep -Ec \
            "^action_17q_a_assertion_${contract_assertion}=(true|false)$" \
            "$contract_output")" -eq 1 ]] || return 97
    done
    contract_observed_assertions=$(
        grep -Ec '^action_17q_a_assertion_[a-z0-9_]+=(true|false)$' \
            "$contract_output"
    )
    [[ "$contract_observed_assertions" -eq "${#expected_assertions[@]}" ]] || return 97

    for contract_value_key in \
        action_17q_a_value_reconcile_path_property_count \
        action_17q_a_value_reconcile_path_property_names_sha256 \
        action_17q_a_value_reconcile_path_load_state \
        action_17q_a_value_reconcile_path_active_state \
        action_17q_a_value_reconcile_path_sub_state \
        action_17q_a_value_reconcile_path_unit_file_state \
        action_17q_a_value_reconcile_path_fragment_path \
        action_17q_a_value_reconcile_path_mainpid_present \
        action_17q_a_value_reconcile_path_nrestarts_present \
        action_17q_a_value_action17q_backup_count \
        action_17q_a_value_action17q_stage_count \
        action_17q_a_value_before_state_sha256 \
        action_17q_a_value_after_state_sha256 \
        action_17q_a_value_conclusion \
        action_17q_a_assertion_count \
        action_17q_a_failed_assertion_count \
        action_17q_a_first_failure; do
    ...
    contract_before_hash=$(
        value_for action_17q_a_value_before_state_sha256 "$contract_output"
    ) || return 97
    is_sha256 "$contract_before_hash" || return 97
    require_one \
        "action_17q_a_value_after_state_sha256=$contract_before_hash" \
        "$contract_output" || return 97
    contract_conclusion=$(
        value_for action_17q_a_value_conclusion "$contract_output"
    ) || return 97
    conclusion_matches_presence \
        "$contract_conclusion" \
        "$contract_mainpid_present" \
    ...
    contract_assertion_count=$(
        value_for action_17q_a_assertion_count "$contract_output"
    ) || return 97
    contract_failed_count=$(
        value_for action_17q_a_failed_assertion_count "$contract_output"
    ) || return 97
    contract_first_failure=$(
        value_for action_17q_a_first_failure "$contract_output"
    ) || return 97
    ...
    [[ "$contract_failed_count" -eq "$(grep -Ec '^action_17q_a_assertion_[a-z0-9_]+=false$' \
        "$contract_output")" ]] || return 97
    ...
    if [[ "$contract_failed_count" -eq 0 ]]; then
        ...
        [[ "$contract_first_failure" != none ]] || return 97
        require_one \
            "action_17q_a_assertion_${contract_first_failure}=false" \
            "$contract_output" || return 97
...
case "${1:-}" in
    --self-test)
        ...
        [[ "${#fixed_markers[@]}" -eq 7 ]]
        printf 'action_17q_a_runner_self_test_complete=true\n'
        exit 0
        ;;
    --source-test)
        [[ $# -eq 1 ]]
        verify_inspector
        printf 'action_17q_a_runner_source_test_complete=true\n'
        exit 0
        ;;
    --contract-test)
        [[ $# -eq 1 ]]
        ...
        contract_directory=$(mktemp -d /tmp/caddy-action17q-a-contract.XXXXXX)
        ...
        printf 'action_17q_a_runner_contract_test_complete=true\n'
        exit 0
        ;;
    "") ;;
    *)
...
verify_inspector
work_directory=$(mktemp -d /tmp/caddy-action17q-a-runner.XXXXXX)
...
finish() {
    ...
    if [[ -e "$work_directory" || -L "$work_directory" ]]; then
        printf 'action_17q_a_workstation_cleanup_complete=false\n' >&2
        exit 97
    fi
    printf 'action_17q_a_workstation_cleanup_complete=true\n'
    exit "$finish_status"
}
...
cat "$output_path"
cat "$error_path" >&2
printf 'action_17q_a_ssh_status=%s\n' "$ssh_status"
set +e
evaluate_contract "$error_path" "$output_path" "$ssh_status"
contract_status=$?
set -e
if [[ "$contract_status" -eq 97 ]]; then
    printf 'Action 17q-a transcript contract failed.\n' >&2
    printf 'action_17q_a_runner_contract_valid=false\n' >&2
    finish 97
fi
printf 'action_17q_a_runner_contract_valid=true\n'
if [[ "$contract_status" -eq 0 ]]; then
    printf 'action_17q_a_runner_acceptance=true\n'
else
    printf 'action_17q_a_runner_acceptance=false\n'
fi
