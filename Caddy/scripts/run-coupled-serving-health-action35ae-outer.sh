...
readonly prefix=action_35_ae_outer
readonly transaction_sha256=3d0dac4e42fadf3196ed4cc44e730ecd06c5bcf90d0526d4d294623640e2c33b
...
safe_capture() {
    local action35ae_path=$1

    regular_file "$action35ae_path"
    [[ "$(stat -c '%s' "$action35ae_path")" -le "$max_stream_bytes" ]]
    iconv -f UTF-8 -t UTF-8 "$action35ae_path" >/dev/null
    ! LC_ALL=C grep -Paq '[\x00-\x08\x0b\x0c\x0e-\x1f\x7f]' "$action35ae_path"
}

capture() {
    local action35ae_label=$1
    shift
    local action35ae_stdout=$workstation_evidence/$action35ae_label.stdout
    local action35ae_stderr=$workstation_evidence/$action35ae_label.stderr
    local action35ae_status=$workstation_evidence/$action35ae_label.status
    ...
    safe_capture "$action35ae_stdout"
    safe_capture "$action35ae_stderr"
    return "$action35ae_rc"
}

build_payload() {
    local action35ae_repository action35ae_source action35ae_target action35ae_mode
    local action35ae_hash action35ae_lifecycle action35ae_source_path action35ae_destination
    ...
    install -m 0600 "$repository_root/Caddy/manifests/action35ae-node-b-quarantine.tsv" \
        "$payload_stage/manifests/action35ae-node-b-quarantine.tsv"
    while IFS=$'\t' read -r action35ae_repository action35ae_source action35ae_target \
        action35ae_mode action35ae_hash action35ae_lifecycle; do
        [[ "$action35ae_repository" = '# repository' ]] && continue
        [[ "$action35ae_mode" =~ ^0[0-7]{3}$ && "$action35ae_lifecycle" = production-candidate ]]
        [[ "$action35ae_target" = /* ]]
        action35ae_source_path=${workspace_root}/$action35ae_repository/$action35ae_source
        regular_file "$action35ae_source_path"
        [[ "$(sha256sum "$action35ae_source_path" | awk '{ print $1 }')" = "$action35ae_hash" ]]
        action35ae_destination=$payload_stage/repositories/$action35ae_repository/$action35ae_source
        install -d -m 0700 "$(dirname -- "$action35ae_destination")"
        install -m 0600 "$action35ae_source_path" "$action35ae_destination"
...
readonly archive=$2
case "$remote_root" in /tmp/caddy-action35ae-*) ;; *) exit 64 ;; esac
case "$archive" in /tmp/caddy-action35ae-*.tar) ;; *) exit 64 ;; esac
...
case "$evidence_root" in /tmp/caddy-action35ae-*) ;; *) exit 64 ;; esac
...
ssh_stream() {
    local action35ae_host=$1
    local action35ae_program=$2
    shift 2
    "$ssh_command" "$action35ae_host" \
        "cd / && sudo -n /bin/bash -s --$(printf ' %q' "$@")" <"$action35ae_program"
}

upload_payload() {
    local action35ae_role=$1
    local action35ae_host=$2
    local action35ae_remote_root=$3
    local action35ae_remote_archive=$4
    local action35ae_hash

    action35ae_hash=$(awk '{ print $1 }' "$workstation_evidence/payload.sha256")
    capture "$action35ae_role-upload-prepare" ssh_stream "$action35ae_host" \
        "$prepare_program" "$action35ae_remote_root" "$action35ae_remote_archive"
    capture "$action35ae_role-upload-copy" "$scp_command" -p -- \
        "$payload_archive" "$action35ae_host:$action35ae_remote_archive"
    capture "$action35ae_role-upload-accept" ssh_stream "$action35ae_host" \
        "$accept_program" "$action35ae_remote_root" "$action35ae_remote_archive" \
        "$action35ae_hash"
}
...
remote_transaction() {
    ...
    capture "$action35ae_label" ssh_stream "$action35ae_host" "$transaction" "$@"
}

readback() {
    local action35ae_role=$1
    local action35ae_host=$2
    local action35ae_remote_evidence=$3

    capture "$action35ae_role-readback" ssh_stream "$action35ae_host" \
        "$readback_program" "$action35ae_remote_evidence"
}
...
cleanup_remote() {
    ...
    capture "$action35ae_role-disposition" ssh_stream "$action35ae_host" \
        "$disposition_program" "$action35ae_remote_root" "$action35ae_remote_archive"
}

run_live() {
    local action35ae_node_b_mutated=false
    local action35ae_node_a_mutated=false
    local action35ae_failure=0
    local action35ae_phase_status=0
    ...
    if upload_payload node-b "$node_b_host" "$node_b_payload" "$node_b_archive" &&
        ...
        return "$action35ae_phase_status"
    fi

    action35ae_node_b_mutated=true
    ...
    if [[ "$action35ae_failure" -eq 0 ]]; then
    ...
    if [[ "$action35ae_failure" -eq 0 ]]; then
        action35ae_node_a_mutated=true
    ...
    if ! remote_transaction node-a-journal-capture "$node_a_host" journal-capture node-a \
        "$node_a_payload" "$node_a_evidence"; then
        [[ "$action35ae_failure" -ne 0 ]] || action35ae_failure=1
    fi
    ...
    if [[ "$action35ae_failure" -ne 0 ]]; then
        if [[ "$action35ae_node_a_mutated" = true ]]; then
            remote_transaction node-a-rollback "$node_a_host" rollback node-a \
                "$node_a_payload" "$node_a_evidence" || exit 125
        fi
        if [[ "$action35ae_node_b_mutated" = true ]]; then
...
production_path_test() {
    ...
    local action35ae_remote_base action35ae_raw action35ae_decision action35ae_observed
    local action35ae_status action35ae_role action35ae_remote_evidence action35ae_test_id
    local action35ae_tmpfiles_line action35ae_keepalived_stop_line
    local action35ae_keepalived_line
    local action35ae_mode action35ae_variant_root action35ae_variant_evidence
    local retained_root retained_candidate retained_release_hash retained_payload_hash
    local action35ae_transaction_test_evidence
    ...
    action35ae_remote_base=$(mktemp -d /tmp/caddy-action35ae-production-path.XXXXXX)
    action35ae_test_id=${action35ae_remote_base##*.}
    test_remote_base=$action35ae_remote_base
    export ACTION35AE_TEST_REMOTE_BASE=$test_remote_base
...
[[ "$1" = -n ]]
shift
export ACTION35AE_PRODUCTION_PATH_TEST=1
exec "$@"
SUDO
...
printf '%s:%s\n' "$runuser_identity" "$runuser_group" \
    >>"$ACTION35AE_TEST_REMOTE_BASE/runuser.identities"
printf 'runuser %s:%s\n' "$runuser_identity" "$runuser_group" \
    >>"$ACTION35AE_TEST_REMOTE_BASE/ordering.calls"
if [[ "${1:-}" = env && "${2:-}" = -i ]]; then
    shift 2
    exec env -i ACTION35AE_TEST_REMOTE_BASE="$ACTION35AE_TEST_REMOTE_BASE" "$@"
fi
exec "$@"
RUNUSER
...
#!/usr/bin/env bash
set -Eeuo pipefail
printf '%s\n' "$*" >>"$ACTION35AE_TEST_REMOTE_BASE/prohibited-keepalived-parser.calls"
exit 97
KEEPALIVED
...
#!/usr/bin/env bash
set -Eeuo pipefail
printf '%s\n' "$*" >>"$ACTION35AE_TEST_REMOTE_BASE/systemctl.calls"
printf 'systemctl %s\n' "$*" >>"$ACTION35AE_TEST_REMOTE_BASE/ordering.calls"
state_root=${ACTION35AE_TARGET_ROOT:-$ACTION35AE_TEST_REMOTE_BASE/default-root}
state_file=$state_root/run/action35ae-keepalived-stopped
case "$1" in
    is-active)
        [[ "$2" = --quiet ]]
    ...
    start)
        ...
        if [[ "$2" = keepalived.service ]]; then
            ...
            printf '%s\n' \
                ...
                >>"$ACTION35AE_TEST_REMOTE_BASE/keepalived.journal"
            if [[ "${ACTION35AE_TEST_DAEMON_MODE:-success}" = journal-failure ]]; then
                printf '%s\n' \
                    'test-host Keepalived_vrrp[100]: VRRP_Script(check-caddy) failed (exited with status 1)' \
                    >>"$ACTION35AE_TEST_REMOTE_BASE/keepalived.journal"
            fi
            if [[ -x "$ACTION35AE_TARGET_ROOT/etc/scripts/check-dns.sh" &&
                -x "$ACTION35AE_TARGET_ROOT/usr/local/libexec/check-caddy.sh" ]]; then
                /usr/bin/setsid -f "$ACTION35AE_TEST_REMOTE_BASE/bin/keepalived-daemon-producer"
            fi
        fi
        ;;
...
#!/usr/bin/env bash
set -Eeuo pipefail
mode=${ACTION35AE_TEST_DAEMON_MODE:-success}
cycles=5
[[ "$mode" != stale ]] || cycles=1
for ((sequence = 1; sequence <= cycles; sequence++)); do
    ...
    if ! env DNS_CHECK_STATUS_FILE="$ACTION35AE_TARGET_ROOT/run/caddy-serving-health/dns/status" \
        DNS_CHECK_DIG_COMMAND="$ACTION35AE_TEST_REMOTE_BASE/bin/dig" \
        DNS_CHECK_SYSTEMCTL_COMMAND="$ACTION35AE_TEST_REMOTE_BASE/bin/systemctl" \
        ACTION35AE_TEST_DIG_FAIL="$dns_fail" \
        "$ACTION35AE_TEST_REMOTE_BASE/bin/runuser" -u pi -- \
        "$ACTION35AE_TARGET_ROOT/etc/scripts/check-dns.sh" \
        >>"$ACTION35AE_TEST_REMOTE_BASE/daemon-dns.stdout" \
        2>>"$ACTION35AE_TEST_REMOTE_BASE/daemon-dns.stderr"; then
        printf '%s\n' \
    ...
    if [[ "$caddy_mode" = timeout || "$caddy_mode" = signal ]]; then
        env CADDY_SERVING_HEALTH_STATUS_FILE="$ACTION35AE_TARGET_ROOT/run/caddy-serving-health/proxy/status" \
            CADDY_SERVING_HEALTH_ENVIRONMENT_FILE="$ACTION35AE_TARGET_ROOT/etc/default/caddy-ha" \
            CADDY_SERVING_HEALTH_CURL_COMMAND="$ACTION35AE_TEST_REMOTE_BASE/bin/curl" \
            CADDY_SERVING_HEALTH_SS_COMMAND="$ACTION35AE_TEST_REMOTE_BASE/bin/ss" \
            CADDY_SERVING_HEALTH_SYSTEMCTL_COMMAND="$ACTION35AE_TEST_REMOTE_BASE/bin/systemctl" \
            ACTION35AE_TEST_CURL_MODE="$caddy_mode" \
            /usr/bin/timeout --preserve-status \
            --signal="$([[ "$caddy_mode" = signal ]] && printf KILL || printf TERM)" \
            --kill-after=0.05 0.05 \
            "$ACTION35AE_TEST_REMOTE_BASE/bin/runuser" -u keepalived_script \
            -g caddy-tls -- "$ACTION35AE_TARGET_ROOT/usr/local/libexec/check-caddy.sh" \
            >>"$ACTION35AE_TEST_REMOTE_BASE/daemon-caddy.stdout" \
            2>>"$ACTION35AE_TEST_REMOTE_BASE/daemon-caddy.stderr" || caddy_status=$?
    else
        env CADDY_SERVING_HEALTH_STATUS_FILE="$ACTION35AE_TARGET_ROOT/run/caddy-serving-health/proxy/status" \
            CADDY_SERVING_HEALTH_ENVIRONMENT_FILE="$ACTION35AE_TARGET_ROOT/etc/default/caddy-ha" \
            CADDY_SERVING_HEALTH_CURL_COMMAND="$ACTION35AE_TEST_REMOTE_BASE/bin/curl" \
...
#!/usr/bin/env bash
set -Eeuo pipefail
[[ "$1" = --create && "$2" = "${ACTION35AE_TARGET_ROOT:?}/etc/tmpfiles.d/caddy-ha.conf" ]]
install -d -m 0755 "$ACTION35AE_TARGET_ROOT/run/caddy-serving-health"
install -d -m 0755 "$ACTION35AE_TARGET_ROOT/run/caddy-serving-health/dns"
install -d -m 0755 "$ACTION35AE_TARGET_ROOT/run/caddy-serving-health/proxy"
install -d -m 0755 "$ACTION35AE_TARGET_ROOT/run/caddy-serving-health/keepalived"
printf '%s\n' "$*" >>"$ACTION35AE_TEST_REMOTE_BASE/tmpfiles.calls"
printf 'tmpfiles %s\n' "$*" >>"$ACTION35AE_TEST_REMOTE_BASE/ordering.calls"
TMPFILES
    cat >"$test_remote_base/bin/dig" <<'DIG'
#!/usr/bin/env bash
if [[ "${ACTION35AE_TEST_DIG_FAIL:-0}" = 1 ]]; then
    exit 9
fi
case " $* " in
    *' A '*) printf '10.1.0.55\n' ;;
...
#!/usr/bin/env bash
case "${ACTION35AE_TEST_CURL_MODE:-success}" in
    failure) exit 7 ;;
...
set -Eeuo pipefail
case " $* " in
    *' --show-cursor '*) printf '%s\n' '-- cursor: s=action35ae-production-path' ;;
    *' --after-cursor '*)
        if [[ -s "$ACTION35AE_TEST_REMOTE_BASE/keepalived.journal" ]]; then
            cat "$ACTION35AE_TEST_REMOTE_BASE/keepalived.journal"
        else
            printf '%s\n' 'test-host caddy.service: action35ae production-path event'
        fi
        ;;
    *) exit 64 ;;
esac
...
shift
command="$*"
printf '%s\t%s\n' "$host" "$command" >>"$ACTION35AE_TEST_REMOTE_BASE/ssh.calls"
PATH="$ACTION35AE_TEST_REMOTE_BASE/bin:/usr/bin:/bin" /bin/bash -c "$command"
SSH
    cat >"$scp_command" <<'SCP'
#!/usr/bin/env bash
set -Eeuo pipefail
...
source=$3
target=${4#*:}
printf '%s\t%s\n' "$source" "$target" >>"$ACTION35AE_TEST_REMOTE_BASE/scp.calls"
install -o "$(id -un)" -g "$(id -gn)" -m 0600 "$source" "$target"
SCP
    chmod 0700 "$ssh_command" "$scp_command" "$test_remote_base/bin/"*
    export ACTION35AE_SLEEP_COMMAND=$test_remote_base/bin/sleep
    ...
    node_a_payload=/tmp/caddy-action35ae-test-$action35ae_test_id-node-a
    node_b_payload=/tmp/caddy-action35ae-test-$action35ae_test_id-node-b
    ...
    ACTION35AE_INCOMING_ROOT=$retained_root/incoming \
        ACTION35AE_OUTGOING_ROOT=$retained_root/outgoing \
        ACTION35AE_QUARANTINE_ROOT=$retained_root/quarantine \
        ACTION35AE_RELEASES_ROOT=$retained_root/releases \
        ACTION35AE_RETAINED_RELEASE_MANIFEST_SHA256=$retained_release_hash \
        ACTION35AE_RETAINED_PAYLOAD_MANIFEST_SHA256=$retained_payload_hash \
    ...
    ACTION35AE_ENVIRONMENT_FILE=$test_remote_base/environment-node-b \
        ACTION35AE_UNBOUND_CHECKCONF_COMMAND=$test_remote_base/bin/unbound-checkconf \
        ACTION35AE_RUNUSER_COMMAND=$test_remote_base/bin/runuser \
        ACTION35AE_SYSTEMCTL_COMMAND=$test_remote_base/bin/systemctl \
        ACTION35AE_DNS_DIG_COMMAND=$test_remote_base/bin/dig \
        ACTION35AE_CURL_COMMAND=$test_remote_base/bin/curl \
        ACTION35AE_SS_COMMAND=$test_remote_base/bin/ss \
    ...
    ACTION35AE_JOURNALCTL_COMMAND=$test_remote_base/bin/journalctl \
    ...
    ACTION35AE_TARGET_ROOT=$test_remote_base/node-b-root \
        ACTION35AE_SYSTEMCTL_COMMAND=$test_remote_base/bin/systemctl \
        ACTION35AE_JOURNALCTL_COMMAND=$test_remote_base/bin/journalctl \
        ACTION35AE_SYSTEMD_TMPFILES_COMMAND=$test_remote_base/bin/systemd-tmpfiles \
        ACTION35AE_RUNUSER_COMMAND=$test_remote_base/bin/runuser \
        ACTION35AE_DNS_DIG_COMMAND=$test_remote_base/bin/dig \
        ACTION35AE_CURL_COMMAND=$test_remote_base/bin/curl \
        ACTION35AE_SS_COMMAND=$test_remote_base/bin/ss \
        ...
        "$node_b_payload" "$node_b_evidence"
    action35ae_raw=$action35ae_test_root/raw/keepalived-daemon-owned-acceptance.txt
    action35ae_decision=$action35ae_test_root/decisions/keepalived-daemon-owned-acceptance.tsv
    install -m 0600 "$test_remote_base/ordering.calls" "$action35ae_raw"
    action35ae_tmpfiles_line=$(grep -n '^tmpfiles --create ' "$action35ae_raw" | cut -d: -f1)
    action35ae_keepalived_stop_line=$(grep -n '^systemctl stop keepalived.service$' \
        "$action35ae_raw" | cut -d: -f1)
    action35ae_keepalived_line=$(grep -n '^systemctl start keepalived.service$' \
        "$action35ae_raw" | cut -d: -f1)
    [[ "$action35ae_keepalived_stop_line" -lt "$action35ae_tmpfiles_line" &&
        "$action35ae_tmpfiles_line" -lt "$action35ae_keepalived_line" ]]
    [[ "$(grep -c '^runuser pi:default$' "$action35ae_raw")" -eq 5 ]]
    [[ "$(grep -c '^runuser keepalived_script:caddy-tls$' "$action35ae_raw")" -eq 5 ]]
    grep -Fxq 'action_35_ae_check_keepalived_daemon_dns_transition_count=true' \
        "$workstation_evidence/node-b-install-producer.stdout"
    grep -Fxq 'action_35_ae_check_keepalived_daemon_proxy_transition_count=true' \
        "$workstation_evidence/node-b-install-producer.stdout"
    ...
    action35ae_transaction_test_evidence=$action35ae_test_root/transaction-through-outer
    install -d -m 0700 "$action35ae_transaction_test_evidence"
    CADDY_PRODUCTION_PATH_EVIDENCE_ROOT=$action35ae_transaction_test_evidence \
        ACTION35AE_TEST_REPOSITORY_ROOT=$repository_root \
        remote_transaction node-a-post-promote-production-path "$node_a_host" \
        --production-path-test
    ACTION35AE_TARGET_ROOT=$test_remote_base/node-a-root \
        ACTION35AE_SYSTEMCTL_COMMAND=$test_remote_base/bin/systemctl \
        ACTION35AE_JOURNALCTL_COMMAND=$test_remote_base/bin/journalctl \
        ACTION35AE_SYSTEMD_TMPFILES_COMMAND=$test_remote_base/bin/systemd-tmpfiles \
        ACTION35AE_RUNUSER_COMMAND=$test_remote_base/bin/runuser \
    ...
    for action35ae_mode in dns-intermittent caddy-intermittent stale journal-failure timeout signal; do
        action35ae_variant_root=$test_remote_base/rejection-$action35ae_mode-root
        action35ae_variant_evidence=$node_b_payload/evidence-$action35ae_mode
        install -d -m 0700 "$action35ae_variant_root" "$action35ae_variant_evidence"
        install -d -m 0755 "$action35ae_variant_root/etc/default" \
            "$action35ae_variant_root/usr/local/libexec"
        install -m 0640 "$test_remote_base/environment-node-b" \
            "$action35ae_variant_root/etc/default/caddy-ha"
        install -m 0755 "$repository_root/Caddy/scripts/prepare-lighttpd-config.sh" \
            "$action35ae_variant_root/usr/local/libexec/prepare-lighttpd-config.sh"
        : >"$test_remote_base/keepalived.journal"
        if ACTION35AE_TEST_DAEMON_MODE=$action35ae_mode \
            ACTION35AE_TARGET_ROOT=$action35ae_variant_root \
            ACTION35AE_SYSTEMCTL_COMMAND=$test_remote_base/bin/systemctl \
            ACTION35AE_JOURNALCTL_COMMAND=$test_remote_base/bin/journalctl \
            ACTION35AE_SYSTEMD_TMPFILES_COMMAND=$test_remote_base/bin/systemd-tmpfiles \
            ACTION35AE_RUNUSER_COMMAND=$test_remote_base/bin/runuser \
            ...
            ACTION35AE_SS_COMMAND=$test_remote_base/bin/ss \
            ACTION35AE_UNBOUND_CHECKCONF_COMMAND=$test_remote_base/bin/unbound-checkconf \
            remote_transaction "daemon-$action35ae_mode-rejection-producer" \
            "$node_b_host" install node-b "$node_b_payload" \
            "$action35ae_variant_evidence"; then
            action35ae_status=0
        else
            action35ae_status=$?
        fi
        [[ "$action35ae_status" -ne 0 ]]
        action35ae_raw=$action35ae_test_root/raw/daemon-$action35ae_mode-rejection.txt
        action35ae_decision=$action35ae_test_root/decisions/daemon-$action35ae_mode-rejection.tsv
        {
            cat "$workstation_evidence/daemon-$action35ae_mode-rejection-producer.stdout"
            cat "$workstation_evidence/daemon-$action35ae_mode-rejection-producer.stderr"
            cat "$test_remote_base/keepalived.journal"
            find "$action35ae_variant_evidence" -maxdepth 1 -type f \
                -name '*status-record' -exec sh -c 'printf "file=%s\n" "$1"; cat "$1"' sh {} \;
        } >"$action35ae_raw"
        write_decision "daemon-$action35ae_mode-rejection" reject \
            "$action35ae_status" accepted rejected "$action35ae_raw" "$action35ae_decision"
        ACTION35AE_TARGET_ROOT=$action35ae_variant_root \
            ACTION35AE_SYSTEMCTL_COMMAND=$test_remote_base/bin/systemctl \
            ACTION35AE_UNBOUND_CHECKCONF_COMMAND=$test_remote_base/bin/unbound-checkconf \
            remote_transaction "daemon-$action35ae_mode-rollback-producer" \
            "$node_b_host" rollback node-b "$node_b_payload" \
            "$action35ae_variant_evidence"
        [[ "$(find "$action35ae_variant_root" -type f -printf '%P\n' | LC_ALL=C sort)" = $'etc/default/caddy-ha\nusr/local/libexec/prepare-lighttpd-config.sh' ]]
    done
    ...
    action35ae_raw=$action35ae_test_root/raw/outer-preflight.txt
    action35ae_decision=$action35ae_test_root/decisions/outer-preflight.tsv
    cat "$test_remote_base/ssh.calls" "$test_remote_base/scp.calls" \
        ...
        >"$action35ae_raw"
    action35ae_observed=$(wc -l <"$action35ae_raw")
    write_decision outer-preflight reach 0 "$action35ae_observed" \
        "$action35ae_observed" "$action35ae_raw" "$action35ae_decision"

    for action35ae_role in node-a node-b; do
        if [[ "$action35ae_role" = node-a ]]; then
            action35ae_remote_evidence=$node_a_evidence
        else
            action35ae_remote_evidence=$node_b_evidence
        fi
        action35ae_raw=$action35ae_test_root/raw/evidence-readback-$action35ae_role-success.txt
        action35ae_decision=$action35ae_test_root/decisions/evidence-readback-$action35ae_role-success.tsv
        ssh_stream "test-$action35ae_role" "$readback_program" \
            "$action35ae_remote_evidence" >"$action35ae_raw"
        action35ae_observed=$(sha256sum "$action35ae_raw" | awk '{ print $1 }')
        write_decision "evidence-readback-$action35ae_role-success" accept 0 \
            "$action35ae_observed" "$action35ae_observed" "$action35ae_raw" "$action35ae_decision"

        action35ae_raw=$action35ae_test_root/raw/evidence-readback-$action35ae_role-failure.txt
        action35ae_decision=$action35ae_test_root/decisions/evidence-readback-$action35ae_role-failure.tsv
        if ssh_stream "test-$action35ae_role" "$readback_program" \
            "$action35ae_remote_evidence-absent" >"$action35ae_raw" 2>&1; then
            action35ae_status=0
        else
            action35ae_status=$?
        fi
        write_decision "evidence-readback-$action35ae_role-failure" reject \
            "$action35ae_status" present absent "$action35ae_raw" "$action35ae_decision"
    done
    cleanup_remote node-a "$node_a_host" "$node_a_payload" "$node_a_archive"
    cleanup_remote node-b "$node_b_host" "$node_b_payload" "$node_b_archive"
    chmod -R u+rwX -- "$action35ae_remote_base"
    rm -rf -- "$action35ae_remote_base"
...
transaction=$repository_root/Caddy/scripts/apply-coupled-serving-health-action35ae.sh
...
if [[ "$invocation_mode" = --production-path-test ]]; then
    workstation_evidence=$(mktemp -d /tmp/caddy-action35ae-outer-test.XXXXXX)
else
    workstation_evidence=$(mktemp -d /tmp/caddy-ssh-evidence-action35ae.XXXXXX)
fi
readonly workstation_evidence
chmod 0700 "$workstation_evidence"
payload_stage=$workstation_evidence/payload
payload_archive=$workstation_evidence/action35ae-payload.tar
...
readonly run_id
node_a_payload=/tmp/caddy-action35ae-$run_id-node-a
node_b_payload=/tmp/caddy-action35ae-$run_id-node-b
node_a_archive=$node_a_payload.tar
node_b_archive=$node_b_payload.tar
node_a_evidence=$node_a_payload/evidence
node_b_evidence=$node_b_payload/evidence
