...
readonly prefix=action_30d_remote
readonly backup_directory=/var/backups/caddy-ha/action30d-systemd-boot-persistence
readonly evidence_root=/tmp/caddy-action30d
...
readonly lsyncd_status_file=${CADDY_ACTION30D_STATUS_FILE:-/run/caddy-lsyncd/status}
...
safe_stream() {
    local action30d_stream=$1

    [[ "$(wc -c <"$action30d_stream")" -le 1048576 ]] || return 1
    [[ "$(line_count "$action30d_stream")" -le 8192 ]] || return 1
    iconv -f UTF-8 -t UTF-8 "$action30d_stream" >/dev/null 2>&1 || return 1
    ! LC_ALL=C.UTF-8 grep -n '[^[:print:][:space:]]' "$action30d_stream" >/dev/null || return 1
    ! grep -Eqi 'BEGIN [A-Z ]*PRIVATE KEY|Authorization:[[:space:]]*Bearer|WEBPASSWORD' "$action30d_stream"
}
emit_stream() {
    local action30d_label=$1
    local action30d_stream=$2

    printf '%s_%s_%s_bytes=%s\n' "$prefix" "$node_token" "$action30d_label" "$(wc -c <"$action30d_stream")"
    printf '%s_%s_%s_lines=%s\n' "$prefix" "$node_token" "$action30d_label" "$(line_count "$action30d_stream")"
    printf '%s_%s_%s_sha256=%s\n' "$prefix" "$node_token" "$action30d_label" "$(file_hash "$action30d_stream")"
    safe_stream "$action30d_stream" || return 97
    printf '%s_%s_%s_classification=bounded_safe\n' "$prefix" "$node_token" "$action30d_label"
    if [[ -s "$action30d_stream" ]]; then
        printf '%s_%s_%s_begin\n' "$prefix" "$node_token" "$action30d_label"
        cat "$action30d_stream"
        printf '%s_%s_%s_end\n' "$prefix" "$node_token" "$action30d_label"
    else
...
check() {
    ...
    printf '%s_%s_check_%s=false\n' "$prefix" "$node_token" "$action30d_label" >&2
    return 1
}
run_captured() {
    local action30d_label=$1
    local action30d_stdout=$capture_directory/$action30d_label.stdout
    local action30d_stderr=$capture_directory/$action30d_label.stderr
    local action30d_status_file=$capture_directory/$action30d_label.status
    local action30d_status=0

    shift
    install -m 0600 /dev/null "$action30d_stdout" || return 1
    install -m 0600 /dev/null "$action30d_stderr" || return 1
    install -m 0600 /dev/null "$action30d_status_file" || return 1
    "$@" >"$action30d_stdout" 2>"$action30d_stderr" || action30d_status=$?
    printf '%s\n' "$action30d_status" >"$action30d_status_file"
    printf '%s_%s_%s_status=%s\n' "$prefix" "$node_token" "$action30d_label" "$action30d_status"
    emit_stream "${action30d_label}_stdout" "$action30d_stdout" || return $?
    emit_stream "${action30d_label}_stderr" "$action30d_stderr" || return $?
    [[ "$action30d_status" -eq 0 ]]
}
cursor_from_capture() {
    local action30d_label=$1
    local action30d_capture=$capture_directory/$action30d_label.stdout
...
capture_cursor() {
    ...
    run_captured "$action30d_label" journalctl --show-cursor -n 0 --no-pager || return 1
    cursor_from_capture "$action30d_label" >/dev/null
}
manifest_paths_safe() {
    local action30d_manifest=$1
...
manifest_file_set_matches() {
    local action30d_release=$1
    local action30d_expected
    local action30d_observed
    local action30d_status=0

    action30d_expected=$(mktemp /tmp/action30d-manifest-expected.XXXXXX) || return 1
    action30d_observed=$(mktemp /tmp/action30d-manifest-observed.XXXXXX) || {
        rm -f -- "$action30d_expected"
        return 1
    }
    awk '{ print substr($0, 67) }' "$action30d_release/manifest.sha256" |
        LC_ALL=C sort -u >"$action30d_expected"
    (
        cd "$action30d_release"
        find . -type f \
    ...
    cmp -s "$action30d_expected" "$action30d_observed" || action30d_status=$?
    rm -f -- "$action30d_expected" "$action30d_observed"
    [[ "$action30d_status" -eq 0 ]]
}
manifest_valid() {
    local action30d_release=$1

    [[ -d "$action30d_release" && ! -L "$action30d_release" ]] || return 1
    [[ -f "$action30d_release/release-manifest.json" &&
        ! -L "$action30d_release/release-manifest.json" ]] || return 1
    [[ -f "$action30d_release/manifest.sha256" &&
        ! -L "$action30d_release/manifest.sha256" ]] || return 1
    [[ -z "$(find "$action30d_release" -type l -print -quit)" ]] || return 1
    [[ -z "$(find "$action30d_release" ! -type d ! -type f -print -quit)" ]] || return 1
    [[ -z "$(find "$action30d_release" -type f -links +1 -print -quit)" ]] || return 1
    jq -e '
    ...
    ' "$action30d_release/release-manifest.json" >/dev/null || return 1
    manifest_paths_safe "$action30d_release/manifest.sha256" || return 1
    manifest_file_set_matches "$action30d_release" || return 1
    (cd "$action30d_release" &&
        sha256sum --strict --check manifest.sha256 >/dev/null 2>&1)
}
release_identity_exact() {
    local action30d_release=$1
    local action30d_revision=$2
    local action30d_parent=$3
    local action30d_source=$4

    manifest_valid "$action30d_release" || return 1
    [[ "$(jq -r '.revision // empty' "$action30d_release/release-manifest.json")" = "$action30d_revision" ]] || return 1
    [[ "$(jq -r '.parent_revision // empty' "$action30d_release/release-manifest.json")" = "$action30d_parent" ]] || return 1
    [[ "$(jq -r '.source_node // empty' "$action30d_release/release-manifest.json")" = "$action30d_source" ]]
}
current_release_inventory() {
    local action30d_resolved
    local action30d_revision
    local action30d_parent
    local action30d_source

    [[ -L "$current_link" ]] || return 1
    action30d_resolved=$(readlink -f -- "$current_link") || return 1
    [[ "$action30d_resolved" = "$releases_root/$accepted_current_revision" ]] || return 1
    manifest_valid "$action30d_resolved" || return 1
    action30d_revision=$(jq -r '.revision // empty' "$action30d_resolved/release-manifest.json") || return 1
    action30d_parent=$(jq -r '.parent_revision // empty' "$action30d_resolved/release-manifest.json") || return 1
    action30d_source=$(jq -r '.source_node // empty' "$action30d_resolved/release-manifest.json") || return 1
    [[ "$action30d_revision" = "$accepted_current_revision" ]] || return 1
    [[ "$action30d_source" = node-a ]] || return 1
    printf 'current_path=%s\n' "$action30d_resolved"
    printf 'current_revision=%s\n' "$action30d_revision"
    printf 'current_parent=%s\n' "$action30d_parent"
    printf 'current_source=%s\n' "$action30d_source"
    printf 'current_release_manifest_sha256=%s\n' \
        "$(file_hash "$action30d_resolved/release-manifest.json")"
    printf 'current_payload_manifest_sha256=%s\n' \
        "$(file_hash "$action30d_resolved/manifest.sha256")"
}
...
outbound_entry_type() {
    local action30d_entry=$1

    if [[ -L "$action30d_entry" ]]; then
        printf 'symlink\n'
    elif [[ -d "$action30d_entry" ]]; then
        printf 'directory\n'
    elif [[ -f "$action30d_entry" ]]; then
        printf 'regular\n'
    elif [[ -b "$action30d_entry" ]]; then
        printf 'block\n'
    elif [[ -c "$action30d_entry" ]]; then
        printf 'character\n'
    elif [[ -p "$action30d_entry" ]]; then
        printf 'fifo\n'
    elif [[ -S "$action30d_entry" ]]; then
        printf 'socket\n'
    else
        printf 'other\n'
    fi
...
write_outbound_snapshot() {
    local action30d_snapshot=$1
    local action30d_snapshot_root=${2:-$outbound_root}

    install -m 0600 /dev/null "$action30d_snapshot" || return 1
    find "$action30d_snapshot_root" -mindepth 1 -maxdepth 1 -print0 >"$action30d_snapshot" || return 1
    sort -z -o "$action30d_snapshot" "$action30d_snapshot" || return 1
}
candidate_disposition() {
    local action30d_candidate=$1
    local action30d_revision=$2
    local action30d_source=$3
    local action30d_current_candidate=${4:-$releases_root/$accepted_current_revision}

    [[ "${action30d_candidate##*/}" = "$action30d_revision" ]] || return 1
    case "$role" in
        node-a)
            [[ "$action30d_source" = node-a ]] || return 1
            if [[ "$action30d_revision" = "$accepted_current_revision" ]]; then
                manifest_valid "$action30d_current_candidate" || return 1
                [[ "$(file_hash "$action30d_candidate/release-manifest.json")" = "$(file_hash "$action30d_current_candidate/release-manifest.json")" ]] || return 1
                [[ "$(file_hash "$action30d_candidate/manifest.sha256")" = "$(file_hash "$action30d_current_candidate/manifest.sha256")" ]] || return 1
                printf 'retain\n'
            else
                printf 'quarantine\n'
            fi
            ;;
        node-b)
            [[ "$action30d_source" = node-b ]] || return 1
            printf 'quarantine\n'
            ;;
        *) return 1 ;;
    esac
...
outbound_inventory() {
    local action30d_inventory_root=${1:-$outbound_root}
    local action30d_candidate
    local action30d_candidate_count=0
    local action30d_disposition
    local action30d_entry_count=0
    local action30d_entry_name
    local action30d_entry_type
    local action30d_invalid_entry=false
    local action30d_parent
    local action30d_quarantine_count=0
    local action30d_retain_count=0
    local action30d_revision
    local action30d_source

    [[ -d "$action30d_inventory_root" && ! -L "$action30d_inventory_root" ]] || return 1
    ...
    write_outbound_snapshot "$outbound_inventory_snapshot" "$action30d_inventory_root" || return 1
    install -m 0600 /dev/null "$outbound_quarantine_plan" || return 1
    while IFS= read -r -d '' action30d_candidate; do
        action30d_entry_count=$((action30d_entry_count + 1))
        action30d_entry_name=${action30d_candidate##*/}
        action30d_entry_type=$(outbound_entry_type "$action30d_candidate") || return 1
        printf 'outbound_entry_%s_name_q=%q\n' "$action30d_entry_count" "$action30d_entry_name"
        printf 'outbound_entry_%s_type=%s\n' "$action30d_entry_count" "$action30d_entry_type"
        if [[ "$action30d_entry_type" != directory ||
            "$action30d_entry_name" = .* ]] ||
            ! valid_revision "$action30d_entry_name"; then
            action30d_invalid_entry=true
        fi
    done <"$outbound_inventory_snapshot"
    printf 'outbound_entry_count=%s\n' "$action30d_entry_count"
    [[ "$action30d_invalid_entry" = false ]] || return 1
    while IFS= read -r -d '' action30d_candidate; do
        [[ -n "$action30d_candidate" ]] || continue
        action30d_candidate_count=$((action30d_candidate_count + 1))
        manifest_valid "$action30d_candidate" || return 1
        action30d_revision=$(jq -r '.revision // empty' "$action30d_candidate/release-manifest.json") || return 1
        action30d_parent=$(jq -r '.parent_revision // empty' "$action30d_candidate/release-manifest.json") || return 1
        action30d_source=$(jq -r '.source_node // empty' "$action30d_candidate/release-manifest.json") || return 1
        valid_revision "$action30d_revision" || return 1
        action30d_disposition=$(candidate_disposition "$action30d_candidate" \
            "$action30d_revision" "$action30d_source") || return 1
        printf 'outbound_candidate_%s=%s\n' "$action30d_candidate_count" \
            "${action30d_candidate##*/}"
        ...
        printf 'outbound_candidate_%s_disposition=%s\n' "$action30d_candidate_count" \
            "$action30d_disposition"
        if [[ "$action30d_disposition" = quarantine ]]; then
            printf '%s\0' "$action30d_candidate" >>"$outbound_quarantine_plan" || return 1
            action30d_quarantine_count=$((action30d_quarantine_count + 1))
        else
            action30d_retain_count=$((action30d_retain_count + 1))
        fi
    done <"$outbound_inventory_snapshot"
    [[ "$action30d_retain_count" -le 1 ]] || return 1
    printf 'outbound_candidate_count=%s\n' "$action30d_candidate_count"
    printf 'outbound_retain_count=%s\n' "$action30d_retain_count"
    printf 'outbound_quarantine_count=%s\n' "$action30d_quarantine_count"
    [[ "$role" != node-b || "$action30d_retain_count" -eq 0 ]] || return 1
...
outbound_snapshot_unchanged() {
    local action30d_observed=$capture_directory/outbound-inventory.after-stop

    write_outbound_snapshot "$action30d_observed" || return 1
    cmp -s "$outbound_inventory_snapshot" "$action30d_observed"
}
protect_ineligible_outbound() {
    local action30d_candidate
    local action30d_destination
    local action30d_parent
    local action30d_revision
    local action30d_source
    ...
    while IFS= read -r -d '' action30d_candidate; do
        ...
        action30d_parent=$(jq -r '.parent_revision // empty' "$action30d_candidate/release-manifest.json") || return 1
        action30d_source=$(jq -r '.source_node // empty' "$action30d_candidate/release-manifest.json") || return 1
        [[ "$(candidate_disposition "$action30d_candidate" "$action30d_revision" "$action30d_source")" = quarantine ]] || return 1
        action30d_destination=$quarantine_root/${node_token}-outbound-${action30d_revision}-action30d
        [[ ! -e "$action30d_destination" && ! -L "$action30d_destination" ]] || return 1
        mv -- "$action30d_candidate" "$action30d_destination" || return 1
        release_identity_exact "$action30d_destination" \
            "$action30d_revision" "$action30d_parent" "$action30d_source" || return 1
        [[ ! -e "$action30d_candidate" && ! -L "$action30d_candidate" ]] || return 1
        quarantined_outbound_count=$((quarantined_outbound_count + 1))
        printf 'protected_outbound_%s_revision=%s\n' "$quarantined_outbound_count" "$action30d_revision"
        printf 'protected_outbound_%s_source=%s\n' "$quarantined_outbound_count" "$action30d_source"
        printf 'protected_outbound_%s_path=%s\n' "$quarantined_outbound_count" "$action30d_destination"
...
node_b_outbound_clear() {
    ...
    [[ "$role" = node-b ]] || return 0
    action30d_observed=$(find "$outbound_root" -mindepth 1 -maxdepth 1 \
        -type d ! -name '.*' -print -quit) || return 1
    [[ -z "$action30d_observed" ]]
}
role_state_exact() {
    local action30d_expected

    case "$role" in
        node-a) action30d_expected='(us) 2 "Master"' ;;
        node-b) action30d_expected='(us) 1 "Backup"' ;;
        *) return 1 ;;
    esac
    [[ "$(timeout 3 busctl get-property org.keepalived.Vrrp1 \
        "$ipv4_object" org.keepalived.Vrrp1.Instance State)" = "$action30d_expected" ]] || return 1
    [[ "$(timeout 3 busctl get-property org.keepalived.Vrrp1 \
        "$ipv6_object" org.keepalived.Vrrp1.Instance State)" = "$action30d_expected" ]]
}
lsyncd_stable() {
    local action30d_initial_pid
    local action30d_initial_restarts
    local action30d_sample
    local action30d_observed_pid
    local action30d_observed_restarts

    action30d_initial_pid=$(systemctl show caddy-lsyncd.service -p MainPID --value) || return 1
    action30d_initial_restarts=$(systemctl show caddy-lsyncd.service -p NRestarts --value) || return 1
    [[ "$action30d_initial_pid" =~ ^[1-9][0-9]*$ ]] || return 1
    [[ "$action30d_initial_restarts" =~ ^[0-9]+$ ]] || return 1
    printf 'lsyncd_stability_initial_pid=%s\n' "$action30d_initial_pid"
    printf 'lsyncd_stability_initial_restarts=%s\n' "$action30d_initial_restarts"
    for ((action30d_sample = 1; action30d_sample <= lsyncd_stability_samples; action30d_sample++)); do
        ...
        action30d_observed_pid=$(systemctl show caddy-lsyncd.service -p MainPID --value) || return 1
        action30d_observed_restarts=$(systemctl show caddy-lsyncd.service -p NRestarts --value) || return 1
        [[ "$action30d_observed_pid" = "$action30d_initial_pid" ]] || return 1
        [[ "$action30d_observed_restarts" = "$action30d_initial_restarts" ]] || return 1
        lsyncd_status_snapshot_valid || return 1
        printf 'lsyncd_stability_sample_%s=true\n' "$action30d_sample"
        if [[ "$action30d_sample" -lt "$lsyncd_stability_samples" ]]; then
            sleep "$lsyncd_stability_delay_seconds"
        fi
    done
}
post_cursor_transport_clean() {
    local action30d_journal=$capture_directory/sync_post_cursor_journal.stdout

    [[ -f "$action30d_journal" ]] || return 1
    ! grep -Eqi \
...
static_enablement_contract() {
    local action30d_unit

    for action30d_unit in "${static_units[@]}"; do
        enabled_exact "$action30d_unit" static || return 1
    done
}
lsyncd_status_snapshot_valid() {
    local action30d_validator=${health_worker_candidate:-$health_worker}

    [[ -f "$action30d_validator" && ! -L "$action30d_validator" ]] || return 1
    /bin/bash "$action30d_validator" --validate-status-file "$lsyncd_status_file"
}
wait_for_lsyncd_status_snapshot() {
    local action30d_waited=0

    while [[ "$action30d_waited" -lt "$lsyncd_status_wait_seconds" ]]; do
        if lsyncd_status_snapshot_valid; then
            printf '%s_%s_lsyncd_status_snapshot_wait_seconds=%s\n' "$prefix" "$node_token" "$action30d_waited"
            return 0
        fi
        sleep 1
        action30d_waited=$((action30d_waited + 1))
    done
    return 1
}
create_health_worker_candidate() {
    health_worker_stage=$(mktemp -d /run/caddy-action30d-health.XXXXXX) || return 1
    ...
    cat >"$health_worker_candidate" <<'ACTION30D_HEALTH_WORKER'
#!/usr/bin/env bash
...
ACTION30D_HEALTH_WORKER
    ...
    /bin/bash -n "$health_worker_candidate"
}
...
emit_unit_properties() {
    local action30d_label=$1
    local action30d_unit=$2

    run_captured "$action30d_label" systemctl show "$action30d_unit" \
        -p LoadState -p UnitFileState -p ActiveState -p SubState -p Result -p ExecMainStatus
}
successful_static_worker() {
    local action30d_unit=$1

    enabled_exact "$action30d_unit" static || return 1
    unit_not_failed "$action30d_unit" || return 1
    [[ "$(systemctl show "$action30d_unit" -p Result --value 2>/dev/null || true)" = success ]] || return 1
    [[ "$(systemctl show "$action30d_unit" -p ExecMainStatus --value 2>/dev/null || true)" = 0 ]]
}
continuity() {
...
target_contract() {
    ...
    continuity || return 1
    for action30d_unit in "${persistent_units[@]}"; do
        check "${action30d_unit//[^a-zA-Z0-9]/_}_enabled" enabled_exact "$action30d_unit" enabled || return 1
        check "${action30d_unit//[^a-zA-Z0-9]/_}_active" active_exact "$action30d_unit" active || return 1
    done
...
manifest_value() {
    local action30d_key=$1

    awk -F= -v key="$action30d_key" '$1 == key { print $2; found++ } END { if (found != 1) exit 1 }' \
        "$backup_directory/manifest"
}
record_backup() {
    local action30d_unit
    local action30d_backup_staging=${backup_directory}.staging.$$

    if ! install -d -o root -g root -m 0700 "$action30d_backup_staging"; then
        return 1
    fi
    if ! install -o root -g root -m 0600 "$obsolete_path" \
        "$action30d_backup_staging/caddy-validate-reload.path" ||
        ! install -o root -g root -m 0600 "$obsolete_service" \
            "$action30d_backup_staging/caddy-validate-reload.service" ||
        ! install -o root -g root -m 0700 "$health_worker" \
            "$action30d_backup_staging/validate-sync-health.sh"; then
        rm -rf -- "$action30d_backup_staging"
        return 1
    fi
    {
        printf 'action=30d\nrole=%s\n' "$role"
        for action30d_unit in "${persistent_units[@]}" caddy-api.service "$distribution_lsyncd_unit" caddy-validate-reload.path; do
            printf 'enabled_%s=%s\n' "${action30d_unit//[^a-zA-Z0-9]/_}" \
                "$(systemctl is-enabled "$action30d_unit" 2>/dev/null || true)"
            printf 'active_%s=%s\n' "${action30d_unit//[^a-zA-Z0-9]/_}" \
                "$(systemctl is-active "$action30d_unit" 2>/dev/null || true)"
        done
    } >"$action30d_backup_staging/manifest" || {
        rm -rf -- "$action30d_backup_staging"
        return 1
    }
    if ! chmod 0600 "$action30d_backup_staging/manifest" ||
        ! mv -- "$action30d_backup_staging" "$backup_directory"; then
        rm -rf -- "$action30d_backup_staging"
        return 1
    fi
}
restore_enablement() {
    local action30d_unit=$1
    local action30d_token=${action30d_unit//[^a-zA-Z0-9]/_}
    local action30d_state
    local action30d_active

    action30d_state=$(manifest_value "enabled_$action30d_token") || return 1
    action30d_active=$(manifest_value "active_$action30d_token") || return 1
    case "$action30d_state" in
        enabled) systemctl enable "$action30d_unit" >/dev/null 2>&1 || return 1 ;;
        disabled) systemctl disable "$action30d_unit" >/dev/null 2>&1 || return 1 ;;
        masked) systemctl mask "$action30d_unit" >/dev/null 2>&1 || return 1 ;;
        static | indirect | generated | transient | '') ;;
        *) return 1 ;;
    esac
    case "$action30d_active" in
        active) systemctl start "$action30d_unit" >/dev/null 2>&1 || return 1 ;;
        inactive) systemctl stop "$action30d_unit" >/dev/null 2>&1 || return 1 ;;
        failed) return 1 ;;
        *) ;;
    esac
}
rollback() {
    ...
    for action30d_unit in "${persistent_units[@]}" caddy-api.service "$distribution_lsyncd_unit" caddy-validate-reload.path; do
        restore_enablement "$action30d_unit" || return 1
    done
...
apply() {
    local action30d_cert_journal_cursor
    local action30d_health_journal_cursor
    ...
    action30d_cert_journal_cursor=$(cursor_from_capture cert_worker_journal_cursor) || return 1
    ...
    run_captured cert_worker_journal journalctl -u caddy-cert-expiry.service \
        --after-cursor "$action30d_cert_journal_cursor" \
    ...
    action30d_health_journal_cursor=$(cursor_from_capture health_worker_journal_cursor) || return 1
    ...
    run_captured health_worker_journal journalctl -u caddy-sync-health.service \
        --after-cursor "$action30d_health_journal_cursor" \
        --no-pager --no-hostname -o short-iso || return 1
...
create_semantic_fixture_release() {
    local action30d_fixture_release=$1
    local action30d_fixture_revision=$2
    local action30d_fixture_parent=$3
    local action30d_fixture_source=$4

    mkdir -p "$action30d_fixture_release"
    printf 'fixture\n' >"$action30d_fixture_release/Caddyfile"
    jq -cn --arg revision "$action30d_fixture_revision" \
        --arg parent "$action30d_fixture_parent" \
        --arg source "$action30d_fixture_source" \
        '{revision:$revision,parent_revision:$parent,source_node:$source,created_at:"2026-08-11T00:00:00Z"}' \
        >"$action30d_fixture_release/release-manifest.json"
    (
        cd "$action30d_fixture_release"
        sha256sum ./Caddyfile ./release-manifest.json >manifest.sha256
    )
}
semantic_self_test() {
    local action30d_fixture_node_a
    local action30d_fixture_node_b
    local action30d_fixture_node_b_older
    local action30d_fixture_wrong_source
    local action30d_observed

    semantic_fixture_root=$(mktemp -d /tmp/action30d-semantic.XXXXXX) || return 1
    trap 'rm -rf -- "${semantic_fixture_root:-}"' EXIT INT TERM
    capture_directory=$semantic_fixture_root/captures
    mkdir -p "$capture_directory"
    action30d_fixture_node_a=$semantic_fixture_root/$accepted_current_revision
    action30d_fixture_node_b=$semantic_fixture_root/$historical_revision
    action30d_fixture_node_b_older=$semantic_fixture_root/20260811T174240Z-31d43261-5cd7-44ce-83e5-947927184d29
    action30d_fixture_wrong_source=$semantic_fixture_root/wrong-source
    create_semantic_fixture_release "$action30d_fixture_node_a" \
        "$accepted_current_revision" "$historical_revision" node-a
    create_semantic_fixture_release "$action30d_fixture_node_b" \
        "$historical_revision" "$historical_parent" node-b
    create_semantic_fixture_release "$action30d_fixture_node_b_older" \
        "${action30d_fixture_node_b_older##*/}" "$historical_parent" node-b
    create_semantic_fixture_release "$action30d_fixture_wrong_source" \
        wrong-source "$historical_parent" node-a
    configure_role node-a || return 1
    [[ "$(candidate_disposition "$action30d_fixture_node_a" \
        "$accepted_current_revision" node-a "$action30d_fixture_node_a")" = retain ]] || return 1
    configure_role node-b || return 1
    [[ "$(candidate_disposition "$action30d_fixture_node_b" \
        "$historical_revision" node-b)" = quarantine ]] || return 1
    [[ "$(candidate_disposition "$action30d_fixture_node_b_older" \
        "${action30d_fixture_node_b_older##*/}" node-b)" = quarantine ]] || return 1
    if candidate_disposition "$action30d_fixture_wrong_source" wrong-source node-a >/dev/null; then
        return 1
    fi
    ...
    mv "$action30d_fixture_node_b" "$semantic_fixture_root/outbound/"
    mv "$action30d_fixture_node_b_older" "$semantic_fixture_root/outbound/"
    ...
    [[ "$(awk -F= '$1 == "outbound_quarantine_count" { print $2 }' \
        "$semantic_fixture_root/inventory.stdout")" = 2 ]] || return 1
    action30d_observed=$(grep -c '^outbound_entry_[0-9][0-9]*_name_q=' \
        "$semantic_fixture_root/inventory.stdout") || return 1
    [[ "$action30d_observed" -eq 2 ]] || return 1
    ...
    [[ "$action30d_observed" -eq 3 ]] || return 1
...
main() {
    local action30d_mode=${1:-}
    local action30d_role=${2:-}

    configure_role "$action30d_role" || return $?
    install -d -o root -g root -m 0700 "$capture_directory" || return 1
    case "$action30d_mode" in
        --apply)
