...
readonly prefix=action_25_retry2
...
expected_checks() {
    ...
    while IFS='|' read -r action25_endpoint_label _ _ _; do
        printf '%s\n' \
            ...
            "${action25_endpoint_label}_redirect_count_exact" \
...
probe_endpoint() {
    ...
    local action25_redirect_count
    local action25_expected_url="https://${action25_endpoint_fqdn}/admin/"
    local action25_expected_final_url="https://${action25_endpoint_fqdn}/admin/login.php"

    action25_curl_output=$("${CADDY_ACTION25_RETRY2_CURL_BIN:-/usr/bin/curl}" \
        --noproxy '*' --insecure --silent --show-error --http1.1 \
        --location --max-redirs 3 --proto-redir '=https' \
        --connect-timeout 3 --max-time 10 \
        --resolve "${action25_endpoint_fqdn}:443:${action25_resolve_address}" \
        --write-out $'\naction25-metadata|%{http_code}|%{url_effective}|%{remote_ip}|%{num_redirects}' \
    ...
    IFS='|' read -r _ action25_http_code action25_effective_url action25_remote_ip \
        action25_redirect_count <<<"$action25_metadata"
    ...
    printf '%s_%s_observed_%s_redirect_count=%s\n' "$prefix" "$node_token" \
        "$action25_endpoint_label" "$action25_redirect_count"
    ...
    check "${action25_endpoint_label}_redirect_count_exact" \
        test "$action25_redirect_count" = 1 || return 1
    check "${action25_endpoint_label}_effective_url_exact" \
        test "$action25_effective_url" = "$action25_expected_final_url" || return 1
    ...
    printf '%s_%s_value_%s_remote_ip=%s\n' "$prefix" "$node_token" \
        "$action25_endpoint_label" "$action25_remote_ip"
    printf '%s_%s_value_%s_redirect_count=%s\n' "$prefix" "$node_token" \
        "$action25_endpoint_label" "$action25_redirect_count"
...
self_test() {
    ...
    while IFS='|' read -r action25_self_endpoint action25_self_fqdn _ action25_self_remote; do
        ...
        printf '%s_%s_value_%s_effective_url=https://%s/admin/login.php\n' "$prefix" "$node_token" "$action25_self_endpoint" "$action25_self_fqdn"
        printf '%s_%s_value_%s_remote_ip=%s\n' "$prefix" "$node_token" "$action25_self_endpoint" "$action25_self_remote"
        printf '%s_%s_value_%s_redirect_count=1\n' "$prefix" "$node_token" "$action25_self_endpoint"
