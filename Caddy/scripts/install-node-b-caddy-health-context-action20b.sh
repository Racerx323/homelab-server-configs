...
expected_checks() {
    printf '%s\n' \
        ...
        baseline_role_node_b baseline_state_unchanged \
        ...
        keepalived_tree_hash_unchanged current_target_unchanged caddy_pid_unchanged \
...
validate_baseline() {
    ...
    /bin/bash "$stage_directory/inspect-dual-node-caddy-postfailure-continuity-action20a-b.sh" \
        --node node-b >"$baseline_stdout" 2>"$baseline_stderr" || baseline_status=$?
    ...
    require_check baseline_role_node_b grep -Fxq \
        'action_20a_b_probe_value_node_role=node-b' "$baseline_stdout" || return 1
...
validate_prestate() {
    ...
    candidate_execution_stage=$(mktemp /tmp/.check-caddy.action20b-validate.XXXXXX) ||
        return 1
    ...
    require_check candidate_transient_residue_absent /bin/bash -c \
        '! find /tmp -mindepth 1 -maxdepth 1 \( -name '\''caddy-health.*'\'' -o -name '\''.check-caddy.action20b-validate.*'\'' \) -print -quit | grep -q .' ||
        return 1
    ...
    require_check prior_backup_absent test -z \
        "$(find "$rollback_root" -mindepth 1 -maxdepth 1 \
            -name 'action20b-node-b-health-context.*' -print -quit)" || return 1
    require_check prior_install_stage_absent test -z \
        "$(find /usr/local/libexec -mindepth 1 -maxdepth 1 \
            -name '.check-caddy.action20b.*' -print -quit)" || return 1
...
rollback_directory=$(mktemp -d "$rollback_root/action20b-node-b-health-context.XXXXXX")
...
install_stage=$(mktemp /usr/local/libexec/.check-caddy.action20b.XXXXXX)
