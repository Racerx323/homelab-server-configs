...
readonly prefix=action_28ah_remote
...
readonly ownership_maximum_samples=45
readonly ownership_required_stable_samples=5
readonly release_maximum_samples=60
readonly release_required_stable_samples=3
...
timing_self_test=false
timing_fixture_count=0
timing_fixture_ready_after=0
expected_hostname=
...
usage() {
    printf 'Usage: %s --mode MODE --role node-a|node-b --stage DIR --run-id ID [--revision ID] [--source node-a|node-b] | --self-test | --stream-classifier-self-test | --timing-self-test\n' \
        "${0##*/}" >&2
}
...
safe_stream() {
    local action28ah_remote_stream=$1

    [[ "$(wc -c <"$action28ah_remote_stream")" -le "$maximum_stream_bytes" ]] || return 1
    [[ "$(line_count "$action28ah_remote_stream")" -le "$maximum_stream_lines" ]] || return 1
    iconv -f UTF-8 -t UTF-8 "$action28ah_remote_stream" >/dev/null 2>&1 || return 1
    ! LC_ALL=C.UTF-8 grep -n '[^[:print:][:space:]]' \
        "$action28ah_remote_stream" >/dev/null || return 1
    ! grep -Eqi 'BEGIN [A-Z ]*PRIVATE KEY|Authorization:[[:space:]]*Bearer|WEBPASSWORD' \
        "$action28ah_remote_stream"
}
run_stream_classifier_self_test() {
    local action28ah_remote_fixture

    classifier_fixture_root=$(mktemp -d /tmp/action28ah-remote-stream.XXXXXX)
    ...
    for action28ah_remote_fixture in control invalid secret bytes lines; do
        if safe_stream "$classifier_fixture_root/$action28ah_remote_fixture"; then
            return 1
        fi
    done
...
emit_stream() {
    local action28ah_remote_label=$1
    local action28ah_remote_stream=$2

    printf '%s_%s_%s_bytes=%s\n' "$prefix" "$mode" "$action28ah_remote_label" \
        "$(wc -c <"$action28ah_remote_stream")"
    printf '%s_%s_%s_lines=%s\n' "$prefix" "$mode" "$action28ah_remote_label" \
        "$(line_count "$action28ah_remote_stream")"
    printf '%s_%s_%s_sha256=%s\n' "$prefix" "$mode" "$action28ah_remote_label" \
        "$(file_hash "$action28ah_remote_stream")"
    if ! safe_stream "$action28ah_remote_stream"; then
        printf '%s_%s_%s_classification=unsafe_retained\n' \
            "$prefix" "$mode" "$action28ah_remote_label" >&2
        return 97
    fi
    printf '%s_%s_%s_classification=bounded_safe\n' \
        "$prefix" "$mode" "$action28ah_remote_label"
    if [[ -s "$action28ah_remote_stream" ]]; then
        printf '%s_%s_%s_begin\n' "$prefix" "$mode" "$action28ah_remote_label"
        sed "s/^/${prefix}_${mode}_${action28ah_remote_label}_content=/" \
            "$action28ah_remote_stream"
        printf '%s_%s_%s_end\n' "$prefix" "$mode" "$action28ah_remote_label"
    else
...
run_captured() {
    local action28ah_remote_label=$1
    local action28ah_remote_status=0

    shift
    install -m 0600 /dev/null "$evidence_directory/$action28ah_remote_label.stdout"
    install -m 0600 /dev/null "$evidence_directory/$action28ah_remote_label.stderr"
    "$@" >"$evidence_directory/$action28ah_remote_label.stdout" \
        2>"$evidence_directory/$action28ah_remote_label.stderr" || action28ah_remote_status=$?
    printf '%s_%s_%s_status=%s\n' "$prefix" "$mode" "$action28ah_remote_label" \
        "$action28ah_remote_status"
    emit_stream "${action28ah_remote_label}_stdout" \
        "$evidence_directory/$action28ah_remote_label.stdout" || return 97
    emit_stream "${action28ah_remote_label}_stderr" \
        "$evidence_directory/$action28ah_remote_label.stderr" || return 97
    [[ "$action28ah_remote_status" -eq 0 ]]
}
lsyncd_stability() {
    local action28ah_remote_sample
    local action28ah_remote_active
    local action28ah_remote_sub
    local action28ah_remote_result
    local action28ah_remote_pid
    local action28ah_remote_restarts

    for action28ah_remote_sample in 1 2 3 4 5; do
        action28ah_remote_active=$(systemctl show caddy-lsyncd.service \
            --property ActiveState --value) || return 1
        action28ah_remote_sub=$(systemctl show caddy-lsyncd.service \
            --property SubState --value) || return 1
        action28ah_remote_result=$(systemctl show caddy-lsyncd.service \
            --property Result --value) || return 1
        action28ah_remote_pid=$(systemctl show caddy-lsyncd.service \
            --property MainPID --value) || return 1
        action28ah_remote_restarts=$(systemctl show caddy-lsyncd.service \
            --property NRestarts --value) || return 1
        printf 'sample=%s active=%s sub=%s result=%s pid=%s restarts=%s\n' \
            "$action28ah_remote_sample" "$action28ah_remote_active" \
            "$action28ah_remote_sub" "$action28ah_remote_result" \
            "$action28ah_remote_pid" "$action28ah_remote_restarts"
        [[ "$action28ah_remote_active" = active ]] || return 1
        [[ "$action28ah_remote_sub" = running ]] || return 1
        [[ "$action28ah_remote_result" = success ]] || return 1
        [[ "$action28ah_remote_pid" =~ ^[1-9][0-9]*$ ]] || return 1
        [[ "$action28ah_remote_restarts" =~ ^[0-9]+$ ]] || return 1
        [[ "$action28ah_remote_sample" -eq 5 ]] || sleep 1
    done
}
check() {
    local action28ah_remote_label=$1
    ...
    first_failure=$action28ah_remote_label
    printf '%s_%s_check_%s=false\n' "$prefix" "$mode" "$action28ah_remote_label" >&2
    return 1
}
address_count() {
    local action28ah_remote_family=$1
    local action28ah_remote_cidr=$2

    ip -o "-$action28ah_remote_family" address show dev eth0 |
        awk -v expected="$action28ah_remote_cidr" \
...
ownership_exact() {
    local action28ah_remote_state=$1
    local action28ah_remote_count=0
    local action28ah_remote_dbus='(us) 1 "Backup"'

    case "$action28ah_remote_state" in
        master)
            action28ah_remote_count=1
            action28ah_remote_dbus='(us) 2 "Master"'
            ;;
        backup) ;;
        absent)
    ...
    [[ "$(dbus_state "$ipv4_object")" = "$action28ah_remote_dbus" ]] || return 1
    [[ "$(dbus_state "$ipv6_object")" = "$action28ah_remote_dbus" ]] || return 1
    [[ "$(address_count 4 "$dns_ipv4_cidr")" -eq "$action28ah_remote_count" ]] || return 1
    [[ "$(address_count 6 "$dns_ipv6_cidr")" -eq "$action28ah_remote_count" ]] || return 1
    [[ "$(address_count 4 "$caddy_ipv4_cidr")" -eq "$action28ah_remote_count" ]] || return 1
    [[ "$(address_count 6 "$caddy_ipv6_cidr")" -eq "$action28ah_remote_count" ]]
}
bounded_until() {
    local action28ah_remote_label=$1
    local action28ah_remote_maximum=$2
    local action28ah_remote_required=$3
    local action28ah_remote_delay=$4
    local action28ah_remote_sample
    local action28ah_remote_consecutive=0

    shift 4
    for ((action28ah_remote_sample = 1; action28ah_remote_sample <= action28ah_remote_maximum; action28ah_remote_sample++)); do
        if "$@"; then
            action28ah_remote_consecutive=$((action28ah_remote_consecutive + 1))
            printf '%s_%s_%s_sample_%s=true\n' \
                "$prefix" "$mode" "$action28ah_remote_label" \
                "$action28ah_remote_sample"
            printf '%s_%s_%s_sample_%s_consecutive=%s\n' \
                "$prefix" "$mode" "$action28ah_remote_label" \
                "$action28ah_remote_sample" "$action28ah_remote_consecutive"
            if [[ "$action28ah_remote_consecutive" -ge "$action28ah_remote_required" ]]; then
                printf '%s_%s_%s_converged=true\n' \
                    "$prefix" "$mode" "$action28ah_remote_label"
                printf '%s_%s_%s_convergence_samples=%s\n' \
                    "$prefix" "$mode" "$action28ah_remote_label" \
                    "$action28ah_remote_sample"
                return 0
            fi
        else
            action28ah_remote_consecutive=0
            printf '%s_%s_%s_sample_%s=false\n' \
                "$prefix" "$mode" "$action28ah_remote_label" \
                "$action28ah_remote_sample"
            printf '%s_%s_%s_sample_%s_consecutive=0\n' \
                "$prefix" "$mode" "$action28ah_remote_label" \
                "$action28ah_remote_sample"
        fi
        if [[ "$action28ah_remote_sample" -lt "$action28ah_remote_maximum" &&
            "$action28ah_remote_delay" -gt 0 ]]; then
            sleep "$action28ah_remote_delay"
        fi
    done
    printf '%s_%s_%s_converged=false\n' \
        "$prefix" "$mode" "$action28ah_remote_label" >&2
    printf '%s_%s_%s_convergence_samples=%s\n' \
        "$prefix" "$mode" "$action28ah_remote_label" \
        "$action28ah_remote_maximum" >&2
    return 1
}
stable_ownership() {
    bounded_until ownership "$ownership_maximum_samples" \
        "$ownership_required_stable_samples" 1 ownership_exact "$expected_state"
}
manifest_valid() {
    local action28ah_remote_release=$1

    # conditional-validator-explicit-failures-begin
    [[ -d "$action28ah_remote_release" && ! -L "$action28ah_remote_release" ]] || return 1
    [[ -f "$action28ah_remote_release/release-manifest.json" ]] || return 1
    [[ -f "$action28ah_remote_release/manifest.sha256" ]] || return 1
    [[ ! -e "$action28ah_remote_release/.complete.pending" && ! -L "$action28ah_remote_release/.complete.pending" ]] || return 1
    manifest_paths_safe "$action28ah_remote_release/manifest.sha256" || return 1
    manifest_file_set_matches "$action28ah_remote_release" || return 1
    (cd "$action28ah_remote_release" &&
        sha256sum --strict --check manifest.sha256 >/dev/null 2>&1) || return 1
    # conditional-validator-explicit-failures-end
}
resolve_current_release_path() {
    local action28ah_remote_current=$1
    local action28ah_remote_release_root=$2
    local action28ah_remote_revision=$3
    local action28ah_remote_resolved

    # conditional-validator-explicit-failures-begin
    valid_revision "$action28ah_remote_revision" || return 1
    [[ -d "$action28ah_remote_release_root" && ! -L "$action28ah_remote_release_root" ]] || return 1
    action28ah_remote_resolved=$(readlink -f -- "$action28ah_remote_current") || return 1
    [[ "$action28ah_remote_resolved" = "$action28ah_remote_release_root/$action28ah_remote_revision" ]] || return 1
    [[ -d "$action28ah_remote_resolved" && ! -L "$action28ah_remote_resolved" ]] || return 1
    printf '%s\n' "$action28ah_remote_resolved"
    # conditional-validator-explicit-failures-end
}
manifest_paths_safe() {
    local action28ah_remote_manifest=$1
    ...
    awk '
        ...
        END { exit bad ? 1 : 0 }
    ' "$action28ah_remote_manifest" || return 1
    # conditional-validator-explicit-failures-end
}
manifest_file_set_matches() {
    local action28ah_remote_release=$1
    local action28ah_remote_expected
    local action28ah_remote_observed
    local action28ah_remote_status=0

    # conditional-validator-explicit-failures-begin
    action28ah_remote_expected=$(mktemp /tmp/action28ah-manifest-expected.XXXXXX) || return 1
    action28ah_remote_observed=$(mktemp /tmp/action28ah-manifest-observed.XXXXXX) || {
        rm -f -- "$action28ah_remote_expected"
        return 1
    }
    awk '{ print substr($0, 67) }' \
        "$action28ah_remote_release/manifest.sha256" |
        LC_ALL=C sort -u >"$action28ah_remote_expected"
    (
        cd "$action28ah_remote_release"
        find . -type f \
            ...
            -print |
            LC_ALL=C sort
    ) >"$action28ah_remote_observed"
    if ! cmp -s "$action28ah_remote_expected" "$action28ah_remote_observed"; then
        action28ah_remote_status=1
    fi
    rm -f -- "$action28ah_remote_expected" "$action28ah_remote_observed"
    [[ "$action28ah_remote_status" -eq 0 ]] || return 1
    # conditional-validator-explicit-failures-end
}
release_payload_equal() {
    local action28ah_remote_left=$1
    local action28ah_remote_right=$2

    # conditional-validator-explicit-failures-begin
    manifest_valid "$action28ah_remote_left" || return 1
    manifest_valid "$action28ah_remote_right" || return 1
    cmp -s "$action28ah_remote_left/release-manifest.json" "$action28ah_remote_right/release-manifest.json" || return 1
    cmp -s "$action28ah_remote_left/manifest.sha256" "$action28ah_remote_right/manifest.sha256" || return 1
    # conditional-validator-explicit-failures-end
}
lsyncd_baseline_state_valid() {
    ...
    # conditional-validator-explicit-failures-begin
    [[ "$action28ah_remote_state" = inactive || "$action28ah_remote_state" = failed ]] || return 1
    # conditional-validator-explicit-failures-end
}
retained_destination_restored() {
    local action28ah_remote_state=$1
    local action28ah_remote_destination=$2

    # conditional-validator-explicit-failures-begin
    case "$action28ah_remote_state" in
        present)
            [[ -d "$action28ah_remote_destination" && ! -L "$action28ah_remote_destination" ]] || return 1
            ;;
        absent)
            [[ ! -e "$action28ah_remote_destination" && ! -L "$action28ah_remote_destination" ]] || return 1
            ;;
        *) return 1 ;;
    esac
    # conditional-validator-explicit-failures-end
...
run_manifest_boundary_self_test() {
    local action28ah_remote_source
    local action28ah_remote_equal
    local action28ah_remote_drift
    local action28ah_remote_extra
    local action28ah_remote_current
    local action28ah_remote_resolved
    local action28ah_remote_releases

    manifest_fixture_root=$(mktemp -d /tmp/action28ah-manifest-boundary.XXXXXX)
    trap 'rm -rf -- "$manifest_fixture_root"' EXIT INT TERM
    action28ah_remote_releases=$manifest_fixture_root/releases
    action28ah_remote_source=$action28ah_remote_releases/fixture
    action28ah_remote_equal=$manifest_fixture_root/equal
    action28ah_remote_drift=$manifest_fixture_root/drift
    action28ah_remote_extra=$manifest_fixture_root/extra
    mkdir -p "$action28ah_remote_source/conf.d"
    printf 'fixture\n' >"$action28ah_remote_source/conf.d/site.caddy"
    printf '{"revision":"fixture","source_node":"node-a"}\n' \
        >"$action28ah_remote_source/release-manifest.json"
    (
        cd "$action28ah_remote_source"
        find . -type f ! -path ./manifest.sha256 -print0 |
            LC_ALL=C sort -z |
            xargs -0 sha256sum
    ) >"$action28ah_remote_source/manifest.sha256"
    : >"$action28ah_remote_source/.finalize-request"
    cp -a "$action28ah_remote_source" "$action28ah_remote_equal"
    cp -a "$action28ah_remote_source" "$action28ah_remote_drift"
    cp -a "$action28ah_remote_source" "$action28ah_remote_extra"
    printf 'drift\n' >"$action28ah_remote_drift/conf.d/site.caddy"
    printf 'extra\n' >"$action28ah_remote_extra/unmanifested"
    manifest_valid "$action28ah_remote_source"
    printf '%s_manifest_boundary_valid_source_accepted=true\n' "$prefix"
    release_payload_equal "$action28ah_remote_source" "$action28ah_remote_equal"
    printf '%s_manifest_boundary_equal_payload_accepted=true\n' "$prefix"
    if manifest_valid "$action28ah_remote_drift"; then
        return 1
    fi
    printf '%s_manifest_boundary_hash_drift_rejected=true\n' "$prefix"
    if manifest_valid "$action28ah_remote_extra"; then
        return 1
    fi
    printf '%s_manifest_boundary_unmanifested_file_rejected=true\n' "$prefix"
    if release_payload_equal "$action28ah_remote_source" "$action28ah_remote_drift"; then
        return 1
    fi
    ...
    action28ah_remote_current=$manifest_fixture_root/current
    ln -s -- "$action28ah_remote_source" "$action28ah_remote_current"
    action28ah_remote_resolved=$(resolve_current_release_path \
        "$action28ah_remote_current" "$action28ah_remote_releases" fixture)
    [[ "$action28ah_remote_resolved" = "$action28ah_remote_source" ]]
    manifest_valid "$action28ah_remote_resolved"
    if manifest_valid "$action28ah_remote_current"; then
        return 1
    fi
    if resolve_current_release_path \
        "$action28ah_remote_current" "$action28ah_remote_releases" wrong-revision \
        >/dev/null 2>&1; then
        return 1
    fi
...
historical_quarantine_valid() {
    local action28ah_remote_incoming=$1
    local action28ah_remote_quarantine=$2
    local action28ah_remote_release_manifest_sha256=$3
    local action28ah_remote_payload_manifest_sha256=$4

    manifest_valid "$action28ah_remote_quarantine" || return 1
    [[ "$(file_hash "$action28ah_remote_quarantine/release-manifest.json")" = "$action28ah_remote_release_manifest_sha256" ]] || return 1
    [[ "$(file_hash "$action28ah_remote_quarantine/manifest.sha256")" = "$action28ah_remote_payload_manifest_sha256" ]] || return 1
    [[ "$(jq -r '.revision // empty' \
        "$action28ah_remote_quarantine/release-manifest.json")" = "$historical_revision" ]] || return 1
    [[ "$(jq -r '.source_node // empty' \
        "$action28ah_remote_quarantine/release-manifest.json")" = node-a ]] || return 1
    [[ -f "$action28ah_remote_quarantine/.finalize-request" &&
        ! -L "$action28ah_remote_quarantine/.finalize-request" &&
        ! -s "$action28ah_remote_quarantine/.finalize-request" ]] || return 1
    [[ -f "$action28ah_remote_quarantine/.complete" &&
        ! -L "$action28ah_remote_quarantine/.complete" &&
        ! -s "$action28ah_remote_quarantine/.complete" ]] || return 1
    [[ ! -e "$action28ah_remote_quarantine/.complete.pending" &&
        ! -L "$action28ah_remote_quarantine/.complete.pending" ]] || return 1
    [[ -z "$(find "$action28ah_remote_quarantine" -type l -print -quit)" ]] || return 1
    [[ -z "$(find "$action28ah_remote_quarantine" ! -type d ! -type f -print -quit)" ]] || return 1
    [[ -z "$(find "$action28ah_remote_quarantine" -type f -links +1 -print -quit)" ]] || return 1
    [[ -z "$(find "$action28ah_remote_quarantine" -type d ! -perm 0550 -print -quit)" ]] || return 1
    [[ -z "$(find "$action28ah_remote_quarantine" -type f ! -perm 0440 -print -quit)" ]] || return 1
    if [[ -e "$action28ah_remote_incoming" || -L "$action28ah_remote_incoming" ]]; then
        [[ -d "$action28ah_remote_incoming" && ! -L "$action28ah_remote_incoming" ]] || return 1
        release_payload_equal "$action28ah_remote_incoming" \
            "$action28ah_remote_quarantine" || return 1
    fi
}
run_historical_quarantine_self_test() {
    local action28ah_remote_fixture_incoming
    local action28ah_remote_fixture_quarantine
    local action28ah_remote_fixture_release_hash
    local action28ah_remote_fixture_payload_hash

    quarantine_fixture_root=$(mktemp -d /tmp/action28ah-quarantine.XXXXXX)
    trap 'chmod -R u+rwX -- "$quarantine_fixture_root" 2>/dev/null || :; rm -rf -- "$quarantine_fixture_root"' EXIT INT TERM
    action28ah_remote_fixture_incoming=$quarantine_fixture_root/incoming/$historical_revision
    action28ah_remote_fixture_quarantine=$quarantine_fixture_root/quarantine/node-a-$historical_revision
    mkdir -p "$action28ah_remote_fixture_quarantine"
    printf '{"revision":"%s","parent_revision":"obsolete","source_node":"node-a"}\n' \
        "$historical_revision" >"$action28ah_remote_fixture_quarantine/release-manifest.json"
    printf 'fixture\n' >"$action28ah_remote_fixture_quarantine/Caddyfile"
    : >"$action28ah_remote_fixture_quarantine/.finalize-request"
    : >"$action28ah_remote_fixture_quarantine/.complete"
    (
        cd "$action28ah_remote_fixture_quarantine"
        sha256sum ./Caddyfile ./release-manifest.json >manifest.sha256
    )
    chmod 0440 "$action28ah_remote_fixture_quarantine"/* \
        "$action28ah_remote_fixture_quarantine"/.finalize-request \
        "$action28ah_remote_fixture_quarantine"/.complete
    chmod 0550 "$action28ah_remote_fixture_quarantine"
    action28ah_remote_fixture_release_hash=$(file_hash \
        "$action28ah_remote_fixture_quarantine/release-manifest.json")
    action28ah_remote_fixture_payload_hash=$(file_hash \
        "$action28ah_remote_fixture_quarantine/manifest.sha256")
    historical_quarantine_valid "$action28ah_remote_fixture_incoming" \
        "$action28ah_remote_fixture_quarantine" \
        "$action28ah_remote_fixture_release_hash" \
        "$action28ah_remote_fixture_payload_hash"
    mkdir -p "$(dirname -- "$action28ah_remote_fixture_incoming")"
    cp -a -- "$action28ah_remote_fixture_quarantine" \
        "$action28ah_remote_fixture_incoming"
    historical_quarantine_valid "$action28ah_remote_fixture_incoming" \
        "$action28ah_remote_fixture_quarantine" \
        "$action28ah_remote_fixture_release_hash" \
        "$action28ah_remote_fixture_payload_hash"
    chmod 0640 "$action28ah_remote_fixture_incoming/Caddyfile"
    printf 'drift\n' >>"$action28ah_remote_fixture_incoming/Caddyfile"
    chmod 0440 "$action28ah_remote_fixture_incoming/Caddyfile"
    if historical_quarantine_valid "$action28ah_remote_fixture_incoming" \
        "$action28ah_remote_fixture_quarantine" \
        "$action28ah_remote_fixture_release_hash" \
        "$action28ah_remote_fixture_payload_hash"; then
        return 1
    fi
    chmod -R u+rwX -- "$action28ah_remote_fixture_incoming"
    rm -rf -- "$action28ah_remote_fixture_incoming"
    chmod 0644 "$action28ah_remote_fixture_quarantine/Caddyfile"
    if historical_quarantine_valid "$action28ah_remote_fixture_incoming" \
        "$action28ah_remote_fixture_quarantine" \
        "$action28ah_remote_fixture_release_hash" \
        "$action28ah_remote_fixture_payload_hash"; then
    ...
    printf '%s_historical_exact_replay_accepted=true\n' "$prefix"
    printf '%s_historical_divergent_replay_rejected=true\n' "$prefix"
...
configure_role() {
    ...
    backup_directory=/var/backups/caddy-ha/action28ah-$role-go-live
    evidence_directory=/tmp/caddy-action28ah/$run_id/$role
...
current_release_exact() {
    local action28ah_remote_revision=$1
    local action28ah_remote_source=$2
    local action28ah_remote_release
    local action28ah_remote_observed_source

    action28ah_remote_release=$(resolve_current_release_path \
        /etc/caddy/current "$releases_root" "$action28ah_remote_revision" \
        2>/dev/null) || return 1
    action28ah_remote_observed_source=$(jq -r '.source_node // empty' \
        "$action28ah_remote_release/release-manifest.json") || return 1
    [[ "$action28ah_remote_observed_source" = "$action28ah_remote_source" ]] || return 1
    manifest_valid "$action28ah_remote_release" || return 1
    systemctl is-active --quiet caddy.service || return 1
    systemctl is-active --quiet caddy-lsyncd.service || return 1
    systemctl is-active --quiet caddy-sync-reconcile.path || return 1
    [[ "$(systemctl is-failed caddy-sync-reconcile.service || true)" != failed ]]
}
current_payload_accepted() {
    local action28ah_remote_revision
    local action28ah_remote_release
    local action28ah_remote_source

    action28ah_remote_revision=$(current_revision) || return 1
    valid_revision "$action28ah_remote_revision" || return 1
    action28ah_remote_release=$(resolve_current_release_path \
        /etc/caddy/current "$releases_root" "$action28ah_remote_revision" \
        2>/dev/null) || return 1
    action28ah_remote_source=$(jq -r '.source_node // empty' \
        "$action28ah_remote_release/release-manifest.json") || return 1
    [[ "$action28ah_remote_source" = node-a ||
        "$action28ah_remote_source" = node-b ]] || return 1
    manifest_valid "$action28ah_remote_release" || return 1
    [[ "$(file_hash "$action28ah_remote_release/manifest.sha256")" = "$retained_payload_sha256" ]] || return 1
    systemctl is-active --quiet caddy.service || return 1
    systemctl is-active --quiet caddy-lsyncd.service || return 1
    systemctl is-active --quiet caddy-sync-reconcile.path || return 1
    [[ "$(systemctl is-failed caddy-sync-reconcile.service || true)" != failed ]]
...
timing_fixture_probe() {
    timing_fixture_count=$((timing_fixture_count + 1))
    [[ "$timing_fixture_count" -ge "$timing_fixture_ready_after" ]]
}
timing_fixture_never() {
    timing_fixture_count=$((timing_fixture_count + 1))
    return 1
}
run_timing_self_test() {
    mode=timing-self-test
    timing_fixture_count=0
    timing_fixture_ready_after=3
    bounded_until delayed 6 2 0 timing_fixture_probe
    [[ "$timing_fixture_count" -eq 4 ]] || return 1
    timing_fixture_count=0
    if bounded_until never 3 1 0 timing_fixture_never; then
        return 1
    fi
    [[ "$timing_fixture_count" -eq 3 ]] || return 1
    printf '%s_timing_delayed_convergence_accepted=true\n' "$prefix"
    printf '%s_timing_unconverged_rejected=true\n' "$prefix"
    printf '%s_timing_self_test_complete=true\n' "$prefix"
}
record_backup() {
    local action28ah_remote_retained_destination=$releases_root/$retained_revision
    ...
    if [[ -e "$action28ah_remote_retained_destination" ||
        -L "$action28ah_remote_retained_destination" ]]; then
        [[ -d "$action28ah_remote_retained_destination" &&
            ! -L "$action28ah_remote_retained_destination" ]] || return 1
...
install_atomic() {
    local action28ah_remote_source=$1
    local action28ah_remote_target=$2
    local action28ah_remote_mode=$3
    local action28ah_remote_directory=${action28ah_remote_target%/*}
    local action28ah_remote_stage

    action28ah_remote_stage=$(mktemp "$action28ah_remote_directory/.action28ah.XXXXXX")
    install -o root -g root -m "$action28ah_remote_mode" \
        "$action28ah_remote_source" "$action28ah_remote_stage"
    mv -fT -- "$action28ah_remote_stage" "$action28ah_remote_target"
}
reset_failed_if_needed() {
    local action28ah_remote_unit=$1
    local action28ah_remote_failed_state

    action28ah_remote_failed_state=$(systemctl is-failed "$action28ah_remote_unit" 2>/dev/null || true)
    printf 'unit=%s before=%s\n' "$action28ah_remote_unit" \
        "${action28ah_remote_failed_state:-unknown}"
    if [[ "$action28ah_remote_failed_state" = failed ]]; then
        systemctl reset-failed "$action28ah_remote_unit" || return 1
    fi
    action28ah_remote_failed_state=$(systemctl is-failed "$action28ah_remote_unit" 2>/dev/null || true)
    printf 'unit=%s after=%s\n' "$action28ah_remote_unit" \
        "${action28ah_remote_failed_state:-unknown}"
    [[ "$action28ah_remote_failed_state" != failed ]]
}
...
promote_release() {
    local action28ah_remote_revision=$1
    local action28ah_remote_source=/var/lib/caddy-sync/outbound/$action28ah_remote_revision
    local action28ah_remote_destination=/etc/caddy/releases/$action28ah_remote_revision
    local action28ah_remote_stage

    if [[ "$role" = node-b && "$action28ah_remote_revision" = "$retained_revision" ]]; then
        action28ah_remote_source=/var/lib/caddy-sync/incoming/node-a/$action28ah_remote_revision
    fi
    manifest_valid "$action28ah_remote_source" || return 1
    [[ "$(jq -r '.revision // empty' "$action28ah_remote_source/release-manifest.json")" = "$action28ah_remote_revision" ]] || return 1
    action28ah_remote_stage=$(mktemp -d \
        "/etc/caddy/releases/.action28ah-promote.XXXXXX")
    cp -a -- "$action28ah_remote_source/." "$action28ah_remote_stage/" || {
        rm -rf -- "$action28ah_remote_stage"
        return 1
    }
    manifest_valid "$action28ah_remote_stage" || {
        rm -rf -- "$action28ah_remote_stage"
        return 1
    }
    if release_payload_equal "$action28ah_remote_destination" \
        "$action28ah_remote_stage"; then
        rm -rf -- "$action28ah_remote_stage"
    else
        [[ ! -e "$backup_directory/retained-destination.displaced" &&
            ! -L "$backup_directory/retained-destination.displaced" ]] || return 1
        if [[ -e "$action28ah_remote_destination" ||
            -L "$action28ah_remote_destination" ]]; then
            mv -- "$action28ah_remote_destination" \
                "$backup_directory/retained-destination.displaced" || return 1
        fi
        mv -- "$action28ah_remote_stage" "$action28ah_remote_destination" || return 1
    fi
    chown -R root:caddy-tls "$action28ah_remote_destination"
    find "$action28ah_remote_destination" -type d -exec chmod 0550 {} +
    find "$action28ah_remote_destination" -type f -exec chmod 0440 {} +
    set -a
    # shellcheck disable=SC1091
    source /etc/default/caddy-ha
    set +a
    env CADDY_CONFIG_ROOT="$action28ah_remote_destination" \
        caddy validate --config "$action28ah_remote_destination/Caddyfile" \
        --adapter caddyfile >/dev/null
    ln -sfn "$action28ah_remote_destination" /etc/caddy/current.new
    mv -Tf /etc/caddy/current.new /etc/caddy/current
    systemctl reload caddy.service
}
rollback_node() {
    local action28ah_remote_before
    local action28ah_remote_retained_destination=$releases_root/$retained_revision
    local action28ah_remote_retained_state
    ...
    action28ah_remote_before=$(<"$backup_directory/current.before")
    ln -sfn "$action28ah_remote_before" /etc/caddy/current.new
    mv -Tf /etc/caddy/current.new /etc/caddy/current
    action28ah_remote_retained_state=$(
    ...
    if [[ -d "$backup_directory/retained-destination.displaced" ]]; then
        if [[ -e "$action28ah_remote_retained_destination" ||
            ...
            mv -- "$action28ah_remote_retained_destination" \
                "$backup_directory/retained-destination.failed" || return 1
        fi
        mv -- "$backup_directory/retained-destination.displaced" \
            "$action28ah_remote_retained_destination" || return 1
    elif [[ "$action28ah_remote_retained_state" = absent &&
        -d "$action28ah_remote_retained_destination" ]]; then
        mv -- "$action28ah_remote_retained_destination" \
...
install -d -o root -g root -m 0700 /tmp/caddy-action28ah "$evidence_directory"
cd /
...
case "$mode" in
    preflight)
        ...
        action28ah_remote_lsyncd_baseline_state=$(
        ...
        if [[ "$role" = node-a ]]; then
            ...
            if [[ -e "$historical_incoming" || -L "$historical_incoming" ]]; then
                printf '%s_%s_observed_historical_incoming_state=exact_replay_required\n' \
                    "$prefix" "$mode"
            else
                printf '%s_%s_observed_historical_incoming_state=absent\n' \
                    "$prefix" "$mode"
            fi
            check historical_quarantine_state_exact historical_quarantine_valid \
    ...
    activate)
        ...
        action28ah_remote_enable_lsyncd_status=0
        run_captured enable_lsyncd systemctl enable --now caddy-lsyncd.service ||
            action28ah_remote_enable_lsyncd_status=$?
        action28ah_remote_lsyncd_stability_status=0
        run_captured lsyncd_stability lsyncd_stability ||
            action28ah_remote_lsyncd_stability_status=$?
        action28ah_remote_lsyncd_show_status=0
        run_captured lsyncd_show systemctl show caddy-lsyncd.service --no-pager \
            ...
            action28ah_remote_lsyncd_show_status=$?
        action28ah_remote_lsyncd_status_status=0
        run_captured lsyncd_status env SYSTEMD_COLORS=0 systemctl status \
            caddy-lsyncd.service --no-pager --full --lines=80 ||
            action28ah_remote_lsyncd_status_status=$?
        action28ah_remote_lsyncd_journal_status=0
        run_captured lsyncd_journal journalctl -u caddy-lsyncd.service \
            --after-cursor "$lsyncd_journal_cursor" --no-pager \
            --output=short-iso || action28ah_remote_lsyncd_journal_status=$?
        ...
        printf '%s_%s_lsyncd_journal_observed_status=%s\n' "$prefix" "$mode" \
            "$action28ah_remote_lsyncd_journal_status"
        check lsyncd_enable_status_zero test \
            "$action28ah_remote_enable_lsyncd_status" -eq 0
        check lsyncd_stability_status_zero test \
            "$action28ah_remote_lsyncd_stability_status" -eq 0
        check lsyncd_show_status_zero test \
            "$action28ah_remote_lsyncd_show_status" -eq 0
        check lsyncd_status_status_zero test \
            "$action28ah_remote_lsyncd_status_status" -eq 0
        check lsyncd_journal_status_zero test \
            "$action28ah_remote_lsyncd_journal_status" -eq 0
    ...
    publish)
        ...
        ;;
    accept-payload)
        action28ah_remote_current_revision=$(current_revision 2>/dev/null || true)
        action28ah_remote_current_release=$(resolve_current_release_path \
            /etc/caddy/current "$releases_root" \
            "$action28ah_remote_current_revision" 2>/dev/null || true)
        action28ah_remote_current_source=$(jq -r '.source_node // empty' \
            "$action28ah_remote_current_release/release-manifest.json" \
            2>/dev/null || true)
        action28ah_remote_current_payload_sha256=$(file_hash \
            "$action28ah_remote_current_release/manifest.sha256" \
            2>/dev/null || true)
        printf '%s_%s_observed_revision=%s\n' "$prefix" "$mode" \
            "$action28ah_remote_current_revision"
        printf '%s_%s_observed_source=%s\n' "$prefix" "$mode" \
            "$action28ah_remote_current_source"
        printf '%s_%s_observed_payload_sha256=%s\n' "$prefix" "$mode" \
            "$action28ah_remote_current_payload_sha256"
        check current_payload_accepted current_payload_accepted
        ;;
    wait-release)
        valid_revision "$expected_revision"
        [[ "$expected_source" =~ ^node-[ab]$ ]]
        printf '%s_%s_expected_revision=%s\n' "$prefix" "$mode" \
            "$expected_revision"
        printf '%s_%s_expected_source=%s\n' "$prefix" "$mode" \
            "$expected_source"
        check receiver_reconciliation_bounded bounded_until receiver-release \
            "$release_maximum_samples" "$release_required_stable_samples" 1 \
            current_release_exact "$expected_revision" "$expected_source"
        printf '%s_%s_observed_revision=%s\n' "$prefix" "$mode" \
            "$(current_revision)"
        printf '%s_%s_observed_source=%s\n' "$prefix" "$mode" \
            "$(jq -r '.source_node // empty' \
    ...
    accept-release)
        ...
        action28ah_remote_current_source=$(jq -r '.source_node // empty' \
            "$action28ah_remote_current_release/release-manifest.json" \
            2>/dev/null || true)
        check current_release_resolved test -n "$action28ah_remote_current_release"
        check current_revision_exact test "$(current_revision)" = "$expected_revision"
        check current_source_exact test \
            "$action28ah_remote_current_source" = "$expected_source"
        check current_manifest_valid manifest_valid "$action28ah_remote_current_release"
    ...
    commit)
        ...
        check transient_stage_absent test \
            -z "$(find /run -maxdepth 1 -name 'caddy-action28ah-bundle.*' \
