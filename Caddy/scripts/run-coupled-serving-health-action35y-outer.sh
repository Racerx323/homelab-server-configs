...
readonly prefix=action_35_y_outer
readonly transaction_sha256=70ef7928deee004748fcb36fd12f403a0b46e3b96c869edd3cc4dd201957f3dd
...
safe_capture() {
    local action35y_path=$1

    regular_file "$action35y_path"
    [[ "$(stat -c '%s' "$action35y_path")" -le "$max_stream_bytes" ]]
    iconv -f UTF-8 -t UTF-8 "$action35y_path" >/dev/null
    ! LC_ALL=C grep -Paq '[\x00-\x08\x0b\x0c\x0e-\x1f\x7f]' "$action35y_path"
}

capture() {
    local action35y_label=$1
    shift
    local action35y_stdout=$workstation_evidence/$action35y_label.stdout
    local action35y_stderr=$workstation_evidence/$action35y_label.stderr
    local action35y_status=$workstation_evidence/$action35y_label.status
    ...
    safe_capture "$action35y_stdout"
    safe_capture "$action35y_stderr"
    return "$action35y_rc"
}

build_payload() {
    local action35y_repository action35y_source action35y_target action35y_mode
    local action35y_hash action35y_lifecycle action35y_source_path action35y_destination
    ...
    install -m 0600 "$repository_root/Caddy/manifests/action35y-node-b-quarantine.tsv" \
        "$payload_stage/manifests/action35y-node-b-quarantine.tsv"
    while IFS=$'\t' read -r action35y_repository action35y_source action35y_target \
        action35y_mode action35y_hash action35y_lifecycle; do
        [[ "$action35y_repository" = '# repository' ]] && continue
        [[ "$action35y_mode" =~ ^0[0-7]{3}$ && "$action35y_lifecycle" = production-candidate ]]
        [[ "$action35y_target" = /* ]]
        action35y_source_path=${workspace_root}/$action35y_repository/$action35y_source
        regular_file "$action35y_source_path"
        [[ "$(sha256sum "$action35y_source_path" | awk '{ print $1 }')" = "$action35y_hash" ]]
        action35y_destination=$payload_stage/repositories/$action35y_repository/$action35y_source
        install -d -m 0700 "$(dirname -- "$action35y_destination")"
        install -m 0600 "$action35y_source_path" "$action35y_destination"
...
readonly archive=$2
case "$remote_root" in /tmp/caddy-action35y-*) ;; *) exit 64 ;; esac
case "$archive" in /tmp/caddy-action35y-*.tar) ;; *) exit 64 ;; esac
...
case "$evidence_root" in /tmp/caddy-action35y-*) ;; *) exit 64 ;; esac
...
ssh_stream() {
    local action35y_host=$1
    local action35y_program=$2
    shift 2
    "$ssh_command" "$action35y_host" \
        "cd / && sudo -n /bin/bash -s --$(printf ' %q' "$@")" <"$action35y_program"
}

upload_payload() {
    local action35y_role=$1
    local action35y_host=$2
    local action35y_remote_root=$3
    local action35y_remote_archive=$4
    local action35y_hash

    action35y_hash=$(awk '{ print $1 }' "$workstation_evidence/payload.sha256")
    capture "$action35y_role-upload-prepare" ssh_stream "$action35y_host" \
        "$prepare_program" "$action35y_remote_root" "$action35y_remote_archive"
    capture "$action35y_role-upload-copy" "$scp_command" -p -- \
        "$payload_archive" "$action35y_host:$action35y_remote_archive"
    capture "$action35y_role-upload-accept" ssh_stream "$action35y_host" \
        "$accept_program" "$action35y_remote_root" "$action35y_remote_archive" \
        "$action35y_hash"
}
...
remote_transaction() {
    ...
    capture "$action35y_label" ssh_stream "$action35y_host" "$transaction" "$@"
}

readback() {
    local action35y_role=$1
    local action35y_host=$2
    local action35y_remote_evidence=$3

    capture "$action35y_role-readback" ssh_stream "$action35y_host" \
        "$readback_program" "$action35y_remote_evidence"
}
...
cleanup_remote() {
    ...
    capture "$action35y_role-disposition" ssh_stream "$action35y_host" \
        "$disposition_program" "$action35y_remote_root" "$action35y_remote_archive"
}

run_live() {
    local action35y_node_b_mutated=false
    local action35y_node_a_mutated=false
    local action35y_failure=0
    local action35y_phase_status=0
    ...
    if upload_payload node-b "$node_b_host" "$node_b_payload" "$node_b_archive" &&
        ...
        return "$action35y_phase_status"
    fi

    action35y_node_b_mutated=true
    ...
    if [[ "$action35y_failure" -eq 0 ]]; then
    ...
    if ! remote_transaction node-a-journal-capture "$node_a_host" journal-capture node-a \
        "$node_a_payload" "$node_a_evidence"; then
        [[ "$action35y_failure" -ne 0 ]] || action35y_failure=1
    fi
    ...
    if [[ "$action35y_failure" -ne 0 ]]; then
        if [[ "$action35y_node_a_mutated" = true ]]; then
            remote_transaction node-a-rollback "$node_a_host" rollback node-a \
                "$node_a_payload" "$node_a_evidence" || exit 125
        fi
        if [[ "$action35y_node_b_mutated" = true ]]; then
...
production_path_test() {
    ...
    local action35y_remote_base action35y_raw action35y_decision action35y_observed
    local action35y_status action35y_role action35y_remote_evidence action35y_test_id
    local action35y_tmpfiles_line action35y_dns_line action35y_proxy_line
    local action35y_keepalived_line
    local retained_root retained_candidate retained_release_hash retained_payload_hash
    local action35y_transaction_test_evidence
    ...
    action35y_remote_base=$(mktemp -d /tmp/caddy-action35y-production-path.XXXXXX)
    action35y_test_id=${action35y_remote_base##*.}
    test_remote_base=$action35y_remote_base
    export ACTION35Y_TEST_REMOTE_BASE=$test_remote_base
...
[[ "$1" = -n ]]
shift
export ACTION35Y_PRODUCTION_PATH_TEST=1
exec "$@"
SUDO
...
printf '%s:%s\n' "$runuser_identity" "$runuser_group" \
    >>"$ACTION35Y_TEST_REMOTE_BASE/runuser.identities"
printf 'runuser %s:%s\n' "$runuser_identity" "$runuser_group" \
    >>"$ACTION35Y_TEST_REMOTE_BASE/ordering.calls"
exec "$@"
RUNUSER
    cat >"$test_remote_base/bin/keepalived" <<'KEEPALIVED'
#!/usr/bin/env bash
set -Eeuo pipefail
printf '%s\n' "$*" >>"$ACTION35Y_TEST_REMOTE_BASE/prohibited-keepalived-parser.calls"
exit 97
KEEPALIVED
...
#!/usr/bin/env bash
set -Eeuo pipefail
printf '%s\n' "$*" >>"$ACTION35Y_TEST_REMOTE_BASE/systemctl.calls"
printf 'systemctl %s\n' "$*" >>"$ACTION35Y_TEST_REMOTE_BASE/ordering.calls"
case "$1" in
    is-active) [[ "$2" = --quiet ]] ;;
    start | stop) [[ $# -eq 2 ]] ;;
...
#!/usr/bin/env bash
set -Eeuo pipefail
[[ "$1" = --create && "$2" = "${ACTION35Y_TARGET_ROOT:?}/etc/tmpfiles.d/caddy-ha.conf" ]]
install -d -m 0755 "$ACTION35Y_TARGET_ROOT/run/caddy-serving-health"
install -d -m 0755 "$ACTION35Y_TARGET_ROOT/run/caddy-serving-health/dns"
install -d -m 0755 "$ACTION35Y_TARGET_ROOT/run/caddy-serving-health/proxy"
install -d -m 0755 "$ACTION35Y_TARGET_ROOT/run/caddy-serving-health/keepalived"
printf '%s\n' "$*" >>"$ACTION35Y_TEST_REMOTE_BASE/tmpfiles.calls"
printf 'tmpfiles %s\n' "$*" >>"$ACTION35Y_TEST_REMOTE_BASE/ordering.calls"
TMPFILES
...
set -Eeuo pipefail
case " $* " in
    *' --show-cursor '*) printf '%s\n' '-- cursor: s=action35y-production-path' ;;
    *' --after-cursor '*) printf '%s\n' 'test-host caddy.service: action35y production-path event' ;;
    *) exit 64 ;;
esac
JOURNALCTL
    cat >"$ssh_command" <<'SSH'
...
shift
command="$*"
printf '%s\t%s\n' "$host" "$command" >>"$ACTION35Y_TEST_REMOTE_BASE/ssh.calls"
PATH="$ACTION35Y_TEST_REMOTE_BASE/bin:/usr/bin:/bin" /bin/bash -c "$command"
SSH
    cat >"$scp_command" <<'SCP'
#!/usr/bin/env bash
set -Eeuo pipefail
...
source=$3
target=${4#*:}
printf '%s\t%s\n' "$source" "$target" >>"$ACTION35Y_TEST_REMOTE_BASE/scp.calls"
...
SCP
    ...
    node_a_payload=/tmp/caddy-action35y-test-$action35y_test_id-node-a
    node_b_payload=/tmp/caddy-action35y-test-$action35y_test_id-node-b
    ...
    ACTION35Y_INCOMING_ROOT=$retained_root/incoming \
        ACTION35Y_OUTGOING_ROOT=$retained_root/outgoing \
        ACTION35Y_QUARANTINE_ROOT=$retained_root/quarantine \
        ACTION35Y_RELEASES_ROOT=$retained_root/releases \
        ACTION35Y_RETAINED_RELEASE_MANIFEST_SHA256=$retained_release_hash \
        ACTION35Y_RETAINED_PAYLOAD_MANIFEST_SHA256=$retained_payload_hash \
    ...
    ACTION35Y_ENVIRONMENT_FILE=$test_remote_base/environment-node-b \
        ACTION35Y_UNBOUND_CHECKCONF_COMMAND=$test_remote_base/bin/unbound-checkconf \
        ACTION35Y_RUNUSER_COMMAND=$test_remote_base/bin/runuser \
        ACTION35Y_SYSTEMCTL_COMMAND=$test_remote_base/bin/systemctl \
        ACTION35Y_DNS_DIG_COMMAND=$test_remote_base/bin/dig \
        ACTION35Y_CURL_COMMAND=$test_remote_base/bin/curl \
        ACTION35Y_SS_COMMAND=$test_remote_base/bin/ss \
    ...
    ACTION35Y_JOURNALCTL_COMMAND=$test_remote_base/bin/journalctl \
    ...
    ACTION35Y_TARGET_ROOT=$test_remote_base/node-b-root \
        ACTION35Y_SYSTEMCTL_COMMAND=$test_remote_base/bin/systemctl \
        ACTION35Y_SYSTEMD_TMPFILES_COMMAND=$test_remote_base/bin/systemd-tmpfiles \
        ACTION35Y_RUNUSER_COMMAND=$test_remote_base/bin/runuser \
        ACTION35Y_DNS_DIG_COMMAND=$test_remote_base/bin/dig \
        ACTION35Y_CURL_COMMAND=$test_remote_base/bin/curl \
        ACTION35Y_SS_COMMAND=$test_remote_base/bin/ss \
        ...
        "$node_b_payload" "$node_b_evidence"
    action35y_raw=$action35y_test_root/raw/status-snapshot-initialization-before-keepalived-reload.txt
    action35y_decision=$action35y_test_root/decisions/status-snapshot-initialization-before-keepalived-reload.tsv
    install -m 0600 "$test_remote_base/ordering.calls" "$action35y_raw"
    action35y_tmpfiles_line=$(grep -n '^tmpfiles --create ' "$action35y_raw" | cut -d: -f1)
    action35y_dns_line=$(grep -n '^runuser pi:default$' "$action35y_raw" | cut -d: -f1)
    action35y_proxy_line=$(grep -n '^runuser keepalived_script:caddy-tls$' \
        "$action35y_raw" | cut -d: -f1)
    action35y_keepalived_line=$(grep -n '^systemctl reload keepalived.service$' \
        "$action35y_raw" | cut -d: -f1)
    [[ "$action35y_tmpfiles_line" -lt "$action35y_dns_line" &&
        "$action35y_dns_line" -lt "$action35y_proxy_line" &&
        "$action35y_proxy_line" -lt "$action35y_keepalived_line" ]]
    ...
    action35y_transaction_test_evidence=$action35y_test_root/transaction-through-outer
    install -d -m 0700 "$action35y_transaction_test_evidence"
    CADDY_PRODUCTION_PATH_EVIDENCE_ROOT=$action35y_transaction_test_evidence \
        ACTION35Y_TEST_REPOSITORY_ROOT=$repository_root \
        remote_transaction node-a-post-promote-production-path "$node_a_host" \
        --production-path-test
    ACTION35Y_TARGET_ROOT=$test_remote_base/node-a-root \
        ACTION35Y_SYSTEMCTL_COMMAND=$test_remote_base/bin/systemctl \
        ACTION35Y_SYSTEMD_TMPFILES_COMMAND=$test_remote_base/bin/systemd-tmpfiles \
        ACTION35Y_RUNUSER_COMMAND=$test_remote_base/bin/runuser \
        ACTION35Y_DNS_DIG_COMMAND=$test_remote_base/bin/dig \
    ...
    action35y_raw=$action35y_test_root/raw/outer-preflight.txt
    action35y_decision=$action35y_test_root/decisions/outer-preflight.tsv
    cat "$test_remote_base/ssh.calls" "$test_remote_base/scp.calls" \
        ...
        >"$action35y_raw"
    action35y_observed=$(wc -l <"$action35y_raw")
    write_decision outer-preflight reach 0 "$action35y_observed" \
        "$action35y_observed" "$action35y_raw" "$action35y_decision"

    for action35y_role in node-a node-b; do
        if [[ "$action35y_role" = node-a ]]; then
            action35y_remote_evidence=$node_a_evidence
        else
            action35y_remote_evidence=$node_b_evidence
        fi
        action35y_raw=$action35y_test_root/raw/evidence-readback-$action35y_role-success.txt
        action35y_decision=$action35y_test_root/decisions/evidence-readback-$action35y_role-success.tsv
        ssh_stream "test-$action35y_role" "$readback_program" \
            "$action35y_remote_evidence" >"$action35y_raw"
        action35y_observed=$(sha256sum "$action35y_raw" | awk '{ print $1 }')
        write_decision "evidence-readback-$action35y_role-success" accept 0 \
            "$action35y_observed" "$action35y_observed" "$action35y_raw" "$action35y_decision"

        action35y_raw=$action35y_test_root/raw/evidence-readback-$action35y_role-failure.txt
        action35y_decision=$action35y_test_root/decisions/evidence-readback-$action35y_role-failure.tsv
        if ssh_stream "test-$action35y_role" "$readback_program" \
            "$action35y_remote_evidence-absent" >"$action35y_raw" 2>&1; then
            action35y_status=0
        else
            action35y_status=$?
        fi
        write_decision "evidence-readback-$action35y_role-failure" reject \
            "$action35y_status" present absent "$action35y_raw" "$action35y_decision"
    done
    cleanup_remote node-a "$node_a_host" "$node_a_payload" "$node_a_archive"
    cleanup_remote node-b "$node_b_host" "$node_b_payload" "$node_b_archive"
    chmod -R u+rwX -- "$action35y_remote_base"
    rm -rf -- "$action35y_remote_base"
...
transaction=$repository_root/Caddy/scripts/apply-coupled-serving-health-action35y.sh
...
if [[ "$invocation_mode" = --production-path-test ]]; then
    workstation_evidence=$(mktemp -d /tmp/caddy-action35y-outer-test.XXXXXX)
else
    workstation_evidence=$(mktemp -d /tmp/caddy-ssh-evidence-action35y.XXXXXX)
fi
readonly workstation_evidence
chmod 0700 "$workstation_evidence"
payload_stage=$workstation_evidence/payload
payload_archive=$workstation_evidence/action35y-payload.tar
...
readonly run_id
node_a_payload=/tmp/caddy-action35y-$run_id-node-a
node_b_payload=/tmp/caddy-action35y-$run_id-node-b
node_a_archive=$node_a_payload.tar
node_b_archive=$node_b_payload.tar
node_a_evidence=$node_a_payload/evidence
node_b_evidence=$node_b_payload/evidence
