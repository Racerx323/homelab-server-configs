...
readonly prefix=action_34h_remote
readonly artifact_manifest_relative=Caddy/manifests/durable-apprise-action34h.tsv
...
readonly node_evidence_root=/tmp/caddy-action34h
...
readonly backup_root=$backup_parent/action34h-$role-$run_token
...
dispose_action34g_residue_at() {
    ...
    if [[ "$action34h_residue_role" = node-a ]]; then
        gate action34g_residue_node_a_absent test ! -e "$action34h_residue_queue" || return 1
        return 0
    fi
    ...
    install -o root -g root -m 0600 /dev/null "$action34h_residue_evidence/action34g-retained-records" || return 1
    ...
    gate action34g_residue_evidence_metadata test "$(stat -c '%U:%G:%a' "$action34h_residue_evidence/action34g-retained-records")" = root:root:600 || return 1
...
exercise_queue() {
    local action34h_before action34h_after action34h_record action34h_temporary action34h_now
    ...
    while IFS= read -r action34h_controlled_name; do
        ...
        action34h_record=$queue_root/pending/$action34h_controlled_name
        [[ -f "$action34h_record" && ! -L "$action34h_record" ]] || return 1
        action34h_temporary=$(mktemp "$queue_root/pending/.action34h.XXXXXX") || return 1
        chmod 0600 "$action34h_temporary" || return 1
        jq --argjson now "$action34h_now" '.retry.next_attempt_epoch = $now' \
            "$action34h_record" >"$action34h_temporary" || return 1
        mv -f -- "$action34h_temporary" "$action34h_record" || return 1
    done <"$action34h_controlled_evidence"
...
apply_action() {
    ...
    baseline || return 1
    dispose_action34g_residue_at "$queue_root" "$evidence_root" "$role" "$(id -u pi)" "$(id -g pi)" || return 1
    gate queue_baseline_absent test ! -e "$queue_root" || return 1
    [[ ! -e "$backup_root" ]] || return 1
    action34h_stage=$(mktemp -d /run/caddy-action34h-stage.XXXXXX) || return 1
...
cleanup_unmutated_backup() {
    [[ "$backup_root" =~ ^/var/backups/caddy-ha/action34h-node-[ab]-[0-9]{10,20}-[0-9]+$ ]] || return 1
