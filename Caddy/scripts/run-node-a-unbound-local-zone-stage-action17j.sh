...
readonly regression_sha256=fb0015c718667ec0dea1a5362b9f94e43753b0d77cd7d2c7c3077634e370a8eb
...
readonly expected_stage=/var/tmp/caddy-unbound-node-a-action17j-local-zone
...
readonly driver="$script_dir/stage-node-a-unbound-local-zone-action17j.sh"
readonly regression="$caddy_root/tests/action17j-node-a-unbound-local-zone-stage-regression.sh"
...
verify_live_sources() {
    ...
    if git -C "$dns_repo" ls-files --error-unmatch \
        ...
        printf 'Private Unbound local-zone source unexpectedly became tracked.\n' \
            >&2
        return 1
    fi
}
...
validate_success() {
    ...
    local -a required_records=(
        action_17j_remote_reached=true
        action_17j_preflight_complete=true
        primary_stage_previously_accepted=true
        local_zone_stage_previously_absent=true
        action_17j_mutation_started=true
        ...
        action_17j_conclusion=local_zone_stage_retained
        action_17j_remote_complete=true
        action_17j_node_a_unbound_local_zone_stage_complete=true
        action_17j_remote_source_cleanup_complete=true
    )
    ...
    [[ ! -s "$error_path" ]]
    true_assertions=$(
        grep -Ec '^action_17j_assertion_[a-z0-9_]+=true$' "$transcript_path"
    )
    unique_assertions=$(
        grep -E '^action_17j_assertion_[a-z0-9_]+=true$' "$transcript_path" |
            cut -d= -f1 |
            LC_ALL=C sort -u |
            wc -l
    )
    ...
    if grep -Eq \
        '^action_17j_assertion_[a-z0-9_]+=false$|^action_17j_observed_' \
        "$transcript_path"; then
        return 1
    fi
    require_value action_17j_assertion_count \
        "$expected_assertion_count" "$transcript_path" || return 1
    require_value action_17j_failed_assertion_count 0 "$transcript_path" ||
        return 1
    require_value action_17j_first_failure none "$transcript_path" || return 1
    ...
    if grep -Eq 'action_17j_rollback_|manual_intervention_required=true' \
...
validate_failure() {
    ...
    if grep -Fq 'manual_intervention_required=true' \
        ...
        grep -Fq 'action_17j_rollback_complete=false' \
            "$transcript_path" "$error_path"; then
        return 97
    fi
    if grep -Fq 'action_17j_mutation_started=true' "$transcript_path"; then
        [[ "$(grep -Fxc action_17j_rollback_started=true "$error_path")" -eq 1 ]] ||
            return 97
        [[ "$(grep -Fxc action_17j_rollback_complete=true "$error_path")" -eq 1 ]] ||
            return 97
        [[ "$(grep -Fxc action_17j_rollback_primary_stage_preserved=true \
            "$error_path")" -eq 1 ]] || return 97
    elif grep -Eq 'action_17j_rollback_' "$transcript_path" "$error_path"; then
        return 97
    fi
}
...
write_success_fixture() {
    ...
    for ((fixture_index = 1; fixture_index <= expected_assertion_count; fixture_index += 1)); do
        printf 'action_17j_assertion_fixture_%02d=true\n' "$fixture_index"
    done >>"$fixture_path"
    printf '%s\n' \
        action_17j_preflight_complete=true \
        ...
        local_zone_stage_previously_absent=true \
        action_17j_mutation_started=true \
        "action_17j_assertion_count=$expected_assertion_count" \
        action_17j_failed_assertion_count=0 \
        action_17j_first_failure=none \
        "local_zone_candidate_sha256=$candidate_local_zone_sha256" \
        "local_zone_stage_path=$expected_stage" \
        local_zone_stage_owner_mode=root:root:700 \
        local_zone_file_owner_mode=root:root:600 \
...
if [[ "${1:-}" == --self-test && $# -eq 1 ]]; then
    verify_sources
    grep -Fq "'cd /'" "$0"
    printf 'action_17j_node_a_unbound_local_zone_stage_runner_self_test_complete=true\n'
    exit 0
elif [[ "${1:-}" == --source-test && $# -eq 1 ]]; then
    verify_live_sources
    printf 'action_17j_node_a_unbound_local_zone_stage_runner_source_test_complete=true\n'
    exit 0
elif [[ "${1:-}" == --contract-test && $# -eq 1 ]]; then
    contract_directory=$(mktemp -d /tmp/caddy-action17j-contract.XXXXXX)
    ...
    sed -i 's/action_17j_assertion_fixture_17=true/action_17j_assertion_fixture_17=false/' \
        "$false_fixture"
    if validate_success "$false_fixture" "$empty_error" 0; then
        printf 'False Action 17j assertion was accepted.\n' >&2
        exit 1
    fi

    cp -- "$success_fixture" "$duplicate_fixture"
    printf 'action_17j_assertion_fixture_01=true\n' >>"$duplicate_fixture"
    if validate_success "$duplicate_fixture" "$empty_error" 0; then
        printf 'Duplicate Action 17j assertion was accepted.\n' >&2
        exit 1
    fi
    ...
    printf '%s\n' \
        action_17j_rollback_started=true \
        action_17j_rollback_primary_stage_preserved=true \
        action_17j_rollback_complete=true >"$rollback_error"
    ...
    printf 'action_17j_node_a_unbound_local_zone_stage_runner_contract_test_complete=true\n'
    exit 0
elif (($#)); then
...
verify_live_sources
work_directory=$(mktemp -d /tmp/caddy-action17j.XXXXXX)
...
finish() {
    ...
    if [[ -e "$work_directory" || -L "$work_directory" ]]; then
        printf 'action_17j_local_cleanup_complete=false\n' >&2
        exit 97
    fi
    printf 'action_17j_local_cleanup_complete=true\n'
    exit "$finish_status"
}

install -d -m 0700 "$payload_directory"
install -m 0700 "$driver" \
    "$payload_directory/stage-node-a-unbound-local-zone-action17j.sh"
...
remote_script=$(
    printf '%s\n' \
        ...
        'export PATH' \
        'cd /' \
        'source_stage=$(mktemp -d /run/caddy-action17j.XXXXXX)' \
        'cleanup_source_stage() { rm -rf -- "$source_stage"; }' \
        'trap cleanup_source_stage EXIT' \
        'tar -C "$source_stage" -xf -' \
        'chown -R root:root "$source_stage"' \
        'chmod 0700 "$source_stage" "$source_stage/stage-node-a-unbound-local-zone-action17j.sh"' \
        'chmod 0600 "$source_stage/pihole-local-zone.conf"' \
        'driver_status=0' \
        '/bin/bash "$source_stage/stage-node-a-unbound-local-zone-action17j.sh" --source-stage "$source_stage" || driver_status=$?' \
        'cleanup_source_stage' \
        'trap - EXIT' \
        '[[ ! -e "$source_stage" && ! -L "$source_stage" ]]' \
        'printf "action_17j_remote_source_cleanup_complete=true\n"' \
        'exit "$driver_status"'
)
...
if ! validate_safe_output "$remote_output" "$remote_error"; then
    printf 'Unsafe Action 17j output detected.\n' >&2
    finish 97
fi
if [[ "$ssh_status" -eq 0 ]]; then
    if ! validate_success "$remote_output" "$remote_error" "$ssh_status"; then
        printf 'Action 17j success contract failed.\n' >&2
        finish 97
    fi
    printf 'action_17j_node_a_unbound_local_zone_stage_accepted=true\n'
    finish 0
fi
...
if [[ "$failure_validation_status" -eq 97 ]]; then
    printf 'Action 17j rollback evidence is incomplete.\n' >&2
    finish 97
fi
printf 'action_17j_node_a_unbound_local_zone_stage_accepted=false\n'
finish "$ssh_status"
