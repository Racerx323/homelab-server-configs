...
probe_endpoint() {
    ...
    action25_curl_output=$("${CADDY_ACTION25_CURL_BIN:-/usr/bin/curl}" \
    ...
    check "${action25_endpoint_label}_output_safe" safe_text "$action25_curl_output" || return 1
    ...
    check "${action25_endpoint_label}_body_pihole_marker" \
        grep -Fqi 'Pi-hole' <<<"$action25_body" || return 1
    printf '%s_%s_value_%s_http_code=%s\n' "$prefix" "$node_token" "$action25_endpoint_label" "$action25_http_code"
    printf '%s_%s_value_%s_effective_url=%s\n' "$prefix" "$node_token" "$action25_endpoint_label" "$action25_effective_url"
    printf '%s_%s_value_%s_remote_ip=%s\n' "$prefix" "$node_token" "$action25_endpoint_label" "$action25_remote_ip"
    printf '%s_%s_value_%s_body_bytes=%s\n' "$prefix" "$node_token" "$action25_endpoint_label" "${#action25_body}"
    printf '%s_%s_value_%s_body_sha256=%s\n' "$prefix" "$node_token" "$action25_endpoint_label" \
        "$(printf '%s' "$action25_body" | sha256sum | awk '{ print $1 }')"
}
run_inspection() {
