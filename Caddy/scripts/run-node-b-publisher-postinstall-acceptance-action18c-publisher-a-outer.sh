...
readonly prefix=action_18c_publisher_a
readonly derivation_sha256=cb0339649b2d6c12b8565bf893fab65d6d4b1adba1038c082559c0ad6c958959
readonly regression_sha256=17137126c1ba5343a9b54cab1488fd9781d465915569731fb3c7b9b7c875a619
...
readonly derivation="$script_directory/derive-node-b-publisher-postinstall-acceptance-action18c-publisher-a.sh"
readonly regression="$caddy_root/tests/action18c-publisher-a-node-b-postinstall-acceptance-regression.sh"
...
verify_sources() {
    ...
    "$collision_checker" "$derivation" "$regression" >/dev/null ||
        return 1
}
...
render_stage() {
    ...
    local staged_inspector="$stage_scripts/inspect-node-b-publisher-postinstall-acceptance-action18c-publisher-a.sh"
    local staged_runner="$stage_scripts/run-node-b-publisher-postinstall-acceptance-action18c-publisher-a.sh"

    install -d -m 0700 "$stage_scripts" "$stage_tests" || return 1
    "$derivation" --output-directory "$stage_scripts" || return 1
    install -m 0755 "$inner_collision_checker" "$stage_tests/" || return 1
...
case "${1:-}" in
    --self-test | --source-test | --contract-test)
        [[ $# -eq 1 ]] || exit 64
        verify_sources
        test_root=$(mktemp -d /tmp/caddy-action18c-publisher-a-outer-test.XXXXXX)
        readonly test_root
...
work_directory=$(mktemp -d /tmp/caddy-action18c-publisher-a-outer.XXXXXX)
