...
readonly rendered_inspector_sha256=7a5cd81e0b1aad8ed110ed6220319d47a3cce4ea4f7eea9b7bf5102723b6d018
readonly rendered_core_sha256=645e36c40dbdb47f1cdefbbcebc50eb948e26397c87ce5212fd616867d4b03c8
...
SHIM
        ...
        -e "s/^readonly accepted_local_zone_sha256=.*/readonly accepted_local_zone_sha256=$accepted_local_zone_sha256/" \
        -e "s/^readonly accepted_pihole_ftl_sha256=.*/readonly accepted_pihole_ftl_sha256=$accepted_pihole_ftl_sha256/" \
        -e "s/^readonly accepted_pihole_domain_sha256=.*/readonly accepted_pihole_domain_sha256=$accepted_pihole_domain_sha256/" \
        -e 's/ vrrp_state_exact physical/ physical/' \
        -e 's/ vrrp_state_exact_after physical/ physical/' \
        -e '/^[[:space:]]*check vrrp_state_exact /d' \
        -e '/^[[:space:]]*check vrrp_state_exact_after /d' \
        -e '/value_vrrp_state=/d' \
        -e '/^[[:space:]]*expected_vrrp=/d' \
        -e 's/ expected_vrrp expected_vip_count/ expected_vip_count/' \
...
render_core() {
    ...
    sed \
        ...
        -e '/local action24_outer_expected_vrrp=\$3/d' \
        -e '/validate_assert.*_vrrp_state"/{N;d;}' \
