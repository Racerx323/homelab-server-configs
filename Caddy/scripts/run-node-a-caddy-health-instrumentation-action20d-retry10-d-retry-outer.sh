...
readonly prefix=action_20d_retry10_d_retry_outer
...
readonly builder=$script_directory/build-node-a-caddy-health-instrumentation-action20d-retry10-d-retry.sh
readonly regression=$caddy_root/tests/action20d-retry10-d-retry-candidate-stage-regression.sh
generated_root=
...
source_exact() {
    local action20d_d_retry_outer_expected_hash=$1
    local action20d_d_retry_outer_source_path=$2

    [[ -f "$action20d_d_retry_outer_source_path" && ! -L "$action20d_d_retry_outer_source_path" ]] || return 1
    [[ "$(stat -c '%a' "$action20d_d_retry_outer_source_path")" = 755 ]] || return 1
    [[ "$(file_hash "$action20d_d_retry_outer_source_path")" = "$action20d_d_retry_outer_expected_hash" ]] || return 1
}
gate() {
    local action20d_d_retry_outer_gate_label=$1

    shift
    if "$@" >/dev/null; then
        printf '%s_outer_gate_%s=true\n' "$prefix" "$action20d_d_retry_outer_gate_label"
        return 0
    fi
    printf '%s_outer_gate_%s=false\n' "$prefix" "$action20d_d_retry_outer_gate_label" >&2
    return 1
}
safe_stream() {
    local action20d_d_retry_outer_stream_path=$1

    [[ "$(wc -c <"$action20d_d_retry_outer_stream_path")" -le "$maximum_stream_bytes" ]] || return 1
    [[ "$(line_count "$action20d_d_retry_outer_stream_path")" -le "$maximum_stream_lines" ]] || return 1
    ! LC_ALL=C grep -n '[^[:print:][:space:]]' "$action20d_d_retry_outer_stream_path" \
        >/dev/null || return 1
    ! grep -Eqi \
        'BEGIN [A-Z ]*PRIVATE KEY|CADDY_TLS_PRIVATE_KEY_PEM|PRIVATE_KEY=|DOPPLER_TOKEN|Authorization:[[:space:]]*Bearer' \
        "$action20d_d_retry_outer_stream_path"
}
emit_stream() {
    local action20d_d_retry_outer_stream_label=$1
    local action20d_d_retry_outer_stream_path=$2

    printf '%s_%s_bytes=%s\n' "$prefix" "$action20d_d_retry_outer_stream_label" \
        "$(wc -c <"$action20d_d_retry_outer_stream_path")"
    printf '%s_%s_lines=%s\n' "$prefix" "$action20d_d_retry_outer_stream_label" \
        "$(line_count "$action20d_d_retry_outer_stream_path")"
    printf '%s_%s_sha256=%s\n' "$prefix" "$action20d_d_retry_outer_stream_label" \
        "$(file_hash "$action20d_d_retry_outer_stream_path")"
    if ! safe_stream "$action20d_d_retry_outer_stream_path"; then
        printf '%s_%s_classification=unsafe_retained\n' \
            "$prefix" "$action20d_d_retry_outer_stream_label" >&2
        return 97
    fi
    printf '%s_%s_classification=bounded_safe\n' \
        "$prefix" "$action20d_d_retry_outer_stream_label"
    if [[ -s "$action20d_d_retry_outer_stream_path" ]]; then
        printf '%s_%s_begin\n' "$prefix" "$action20d_d_retry_outer_stream_label"
        cat "$action20d_d_retry_outer_stream_path"
        printf '%s_%s_end\n' "$prefix" "$action20d_d_retry_outer_stream_label"
    else
...
generated_sources_exact() {
    local action20d_d_retry_outer_generated_root=$1

    source_exact "$candidate_sha256" \
        "$action20d_d_retry_outer_generated_root/check-caddy-instrumented-action20d-retry10-d.sh" || return 1
    source_exact "$installer_sha256" \
        "$action20d_d_retry_outer_generated_root/install-node-a-caddy-health-instrumentation-action20d-retry10-d-retry.sh" || return 1
    source_exact "$runner_sha256" \
        "$action20d_d_retry_outer_generated_root/run-node-a-caddy-health-instrumentation-action20d-retry10-d-retry.sh" || return 1
    source_exact "$stager_sha256" \
        "$action20d_d_retry_outer_generated_root/stage-node-a-caddy-health-instrumentation-action20d-retry10-d-retry.sh" || return 1
}
run_local_gates() {
    local action20d_d_retry_outer_gate_root

    action20d_d_retry_outer_gate_root=$(mktemp -d \
        /tmp/caddy-action20d-retry10-d-retry-outer-gates.XXXXXX) || return 1
    trap 'rm -rf -- "$action20d_d_retry_outer_gate_root"; trap - RETURN' RETURN
    ...
    gate generated_sources build_generated_sources "$action20d_d_retry_outer_gate_root/generated" || return 1
    gate generated_hashes generated_sources_exact \
        "$action20d_d_retry_outer_gate_root/generated" || return 1
...
case "${1:-}" in
    --expected-local-gates)
        ...
        printf '%s\n' \
            ...
            output_policy builder_self_test generated_sources generated_hashes \
...
run_local_gates
work_root=$(mktemp -d /tmp/caddy-action20d-retry10-d-retry-outer.XXXXXX)
readonly work_root
...
runner_status=0
/bin/bash \
    "$work_root/generated/run-node-a-caddy-health-instrumentation-action20d-retry10-d-retry.sh" \
