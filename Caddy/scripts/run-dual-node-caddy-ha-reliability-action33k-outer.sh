...
readonly prefix=action_33k_outer
...
readonly evidence_root=/tmp/caddy-ssh-evidence/action33k
readonly node_evidence_root=/tmp/caddy-action33k
readonly maximum_stream_bytes=8388608
readonly maximum_stream_lines=65536
script_directory=$(cd -- "$(dirname -- "${BASH_SOURCE[0]}")" && pwd)
readonly script_directory
readonly caddy_root=${script_directory%/scripts}
readonly transaction=$script_directory/transact-caddy-ha-reliability-action33k.sh
...
readonly regression=$caddy_root/tests/caddy-ha-reliability-action33k-regression.sh
...
ssh_binary=${CADDY_ACTION33K_SSH_BIN:-/usr/bin/ssh}
readonly ssh_binary
scp_binary=${CADDY_ACTION33K_SCP_BIN:-/usr/bin/scp}
readonly scp_binary
...
safe_stream() {
    local action33k_outer_stream=$1
    [[ "$(wc -c <"$action33k_outer_stream")" -le "$maximum_stream_bytes" ]] || return 1
    [[ "$(line_count "$action33k_outer_stream")" -le "$maximum_stream_lines" ]] || return 1
    iconv -f UTF-8 -t UTF-8 "$action33k_outer_stream" >/dev/null 2>&1 || return 1
    ! LC_ALL=C.UTF-8 grep -n '[^[:print:][:space:]]' "$action33k_outer_stream" >/dev/null || return 1
    ! grep -Eqi 'BEGIN [A-Z ]*PRIVATE KEY|Authorization:[[:space:]]*Bearer|WEBPASSWORD' "$action33k_outer_stream"
}
capture_files() {
    local action33k_outer_path
    for action33k_outer_path in "$@"; do
        install -m 0600 /dev/null "$action33k_outer_path"
        chmod 0600 "$action33k_outer_path"
    done
}
emit_stream() {
    local action33k_outer_label=$1
    local action33k_outer_path=$2
    printf '%s_%s_bytes=%s\n' "$prefix" "$action33k_outer_label" "$(wc -c <"$action33k_outer_path")"
    printf '%s_%s_lines=%s\n' "$prefix" "$action33k_outer_label" "$(line_count "$action33k_outer_path")"
    printf '%s_%s_sha256=%s\n' "$prefix" "$action33k_outer_label" "$(file_hash "$action33k_outer_path")"
    safe_stream "$action33k_outer_path" || return 97
    printf '%s_%s_classification=bounded_safe\n' "$prefix" "$action33k_outer_label"
    if [[ -s "$action33k_outer_path" ]]; then
        printf '%s_%s_begin\n' "$prefix" "$action33k_outer_label"
        sed "s/^/${prefix}_${action33k_outer_label}_content=/" \
            "$action33k_outer_path"
        printf '%s_%s_end\n' "$prefix" "$action33k_outer_label"
    else
...
gate() {
    ...
    printf '%s_gate_%s=false\n' "$prefix" "$action33k_outer_label" >&2
    return 1
}
run_remote() {
    local action33k_outer_label=$1 target=$2 alias=$3 role=$4 mode=$5 scenario=$6 argument=${7:-}
    local action33k_outer_stdout=$work_root/$action33k_outer_label.stdout
    local action33k_outer_stderr=$work_root/$action33k_outer_label.stderr
    local action33k_outer_status_file=$work_root/$action33k_outer_label.status
    local action33k_outer_status=0
    capture_files "$action33k_outer_stdout" "$action33k_outer_stderr" "$action33k_outer_status_file"
    "$ssh_binary" -T -o BatchMode=yes -o StrictHostKeyChecking=yes -o "HostKeyAlias=$alias" -o ConnectTimeout=10 -o ConnectionAttempts=1 "$target" \
        "cd / && sudo -n /bin/bash -s -- $mode $role $run_id $scenario $argument" <"$transaction" >"$action33k_outer_stdout" 2>"$action33k_outer_stderr" || action33k_outer_status=$?
    printf '%s\n' "$action33k_outer_status" >"$action33k_outer_status_file"
    printf '%s_%s_status=%s\n' "$prefix" "$action33k_outer_label" "$action33k_outer_status"
    emit_stream "remote_stdout_$action33k_outer_label" "$action33k_outer_stdout"
    emit_stream "remote_stderr_$action33k_outer_label" "$action33k_outer_stderr"
    [[ "$action33k_outer_status" -eq 0 ]]
}
run_local() {
    local action33k_outer_label=$1
    local action33k_outer_stdout=$work_root/$action33k_outer_label.stdout
    ...
    shift
    capture_files "$action33k_outer_stdout" "$action33k_outer_stderr" \
        "$action33k_outer_status_file"
    "$@" >"$action33k_outer_stdout" 2>"$action33k_outer_stderr" ||
        action33k_outer_status=$?
    printf '%s\n' "$action33k_outer_status" >"$action33k_outer_status_file"
    printf '%s_%s_status=%s\n' "$prefix" "$action33k_outer_label" \
        "$action33k_outer_status"
    emit_stream "local_stdout_$action33k_outer_label" "$action33k_outer_stdout"
    emit_stream "local_stderr_$action33k_outer_label" "$action33k_outer_stderr"
    [[ "$action33k_outer_status" -eq 0 ]]
}
upload_registry() {
    local action33k_outer_label=$1
    local action33k_outer_target=$2
    local action33k_outer_alias=$3
    local action33k_outer_stdout=$work_root/$action33k_outer_label.stdout
    local action33k_outer_stderr=$work_root/$action33k_outer_label.stderr
    local action33k_outer_status_file=$work_root/$action33k_outer_label.status
    local action33k_outer_status=0
    ...
    "$scp_binary" -q -p -o BatchMode=yes -o StrictHostKeyChecking=yes \
        -o "HostKeyAlias=$action33k_outer_alias" -o ConnectTimeout=10 \
        -o ConnectionAttempts=1 "$registry" \
        "$action33k_outer_target:/tmp/caddy-action33k-registry-$run_id.tsv" \
        >"$action33k_outer_stdout" 2>"$action33k_outer_stderr" ||
        action33k_outer_status=$?
    printf '%s\n' "$action33k_outer_status" >"$action33k_outer_status_file"
    printf '%s_%s_status=%s\n' "$prefix" "$action33k_outer_label" \
...
wait_ssh_state() {
    local action33k_outer_label=$1 target=$2 alias=$3 expected=$4 limit=$5 elapsed=0 status
    while ((elapsed < limit)); do
        ...
        if { [[ "$expected" = up && "$status" -eq 0 ]]; } || { [[ "$expected" = down && "$status" -ne 0 ]]; }; then
            printf '%s_%s_elapsed_seconds=%s\n' "$prefix" "$action33k_outer_label" "$elapsed"
            return 0
        fi
        sleep 1
        elapsed=$((elapsed + 1))
...
start_availability() {
    local action33k_outer_scenario=$1
    local action33k_outer_output=$work_root/$action33k_outer_scenario.availability
    availability_handoffs=$work_root/$action33k_outer_scenario.handoffs
    availability_stop_request=$work_root/$action33k_outer_scenario.stop-request
    capture_files "$action33k_outer_output" "$availability_handoffs"
    [[ ! -e "$availability_stop_request" ]]
    (
        while [[ ! -e "$availability_stop_request" ]]; do
            printf '%s\t' "$(date +%s%N)" >>"$action33k_outer_output"
            if availability_probe >>"$action33k_outer_output" 2>&1; then
                printf 'ok\n' >>"$action33k_outer_output"
            else
                printf 'failed\n' >>"$action33k_outer_output"
            fi
            sleep 1
    ...
    availability_pid=$!
    availability_scenario=$action33k_outer_scenario
}
begin_handoff() {
    local action33k_outer_label=$1
    [[ -z "$handoff_label" ]] || return 1
    handoff_label=$action33k_outer_label
...
end_handoff() {
    local action33k_outer_label=$1
    local action33k_outer_end_ns
    [[ "$handoff_label" = "$action33k_outer_label" ]] || return 1
    action33k_outer_end_ns=$(date +%s%N)
    printf '%s\t%s\t%s\n' "$handoff_label" "$handoff_start_ns" \
        "$action33k_outer_end_ns" >>"$availability_handoffs"
    printf '%s_handoff_%s_end_ns=%s\n' "$prefix" "$handoff_label" \
        "$action33k_outer_end_ns"
    handoff_label=
    handoff_start_ns=
}
build_availability_summary() {
    local action33k_outer_availability=$1
    local action33k_outer_summary=$2
    ...
    ' "$action33k_outer_availability" >"$action33k_outer_summary"
}
validate_availability_windows() {
    local action33k_outer_windows=$1
    local action33k_outer_summary=$2

    awk -F '\t' -v maximum_window="$maximum_handoff_window_ns" \
        -v windows_file="$action33k_outer_windows" '
    ...
    ' "$action33k_outer_windows" "$action33k_outer_summary"
}
validate_availability_recovery() {
    local action33k_outer_summary=$1
    ...
    ' "$action33k_outer_summary"
}
validate_availability_file() {
    local action33k_outer_label=$1
    local action33k_outer_availability=$2
    local action33k_outer_windows=$3
    local action33k_outer_summary=$action33k_outer_availability.summary
    local action33k_outer_sample_count
    local action33k_outer_failure_count

    # conditional-validator-explicit-failures-begin
    gate "${action33k_outer_label}_availability_regular" test -f \
        "$action33k_outer_availability" || return
    gate "${action33k_outer_label}_availability_not_symlink" test ! -L \
        "$action33k_outer_availability" || return
    gate "${action33k_outer_label}_handoffs_regular" test -f \
        "$action33k_outer_windows" || return
    gate "${action33k_outer_label}_handoffs_not_symlink" test ! -L \
        "$action33k_outer_windows" || return
    capture_files "$action33k_outer_summary"
    gate "${action33k_outer_label}_availability_grammar" \
        build_availability_summary "$action33k_outer_availability" \
        "$action33k_outer_summary" || return
    action33k_outer_sample_count=$(awk 'END { print NR }' \
        "$action33k_outer_summary")
    action33k_outer_failure_count=$(awk -F '\t' '$2 == "failed" { count++ } END { print count + 0 }' \
        "$action33k_outer_summary")
    printf '%s_%s_observed_sample_count=%s\n' "$prefix" \
        "$action33k_outer_label" "$action33k_outer_sample_count"
    printf '%s_%s_observed_failure_count=%s\n' "$prefix" \
        "$action33k_outer_label" "$action33k_outer_failure_count"
    gate "${action33k_outer_label}_availability_nonempty" test \
        "$action33k_outer_sample_count" -gt 0 || return
    gate "${action33k_outer_label}_failures_inside_handoffs" \
        validate_availability_windows "$action33k_outer_windows" \
        "$action33k_outer_summary" || return
    gate "${action33k_outer_label}_failures_isolated_and_bounded" \
        validate_availability_recovery "$action33k_outer_summary" || return
    # conditional-validator-explicit-failures-end
}
consume_action33g_rolling_evidence() {
    local action33k_outer_observed_hash
    local action33k_outer_observed_samples
    local action33k_outer_observed_ok
    local action33k_outer_observed_failed
    local action33k_outer_observed_failure_index
    local action33k_outer_observed_pre_ns
    local action33k_outer_observed_failure_ns
    local action33k_outer_observed_post_ns
    local action33k_outer_observed_window_ns
    ...
    gate consumed_rolling_evidence_not_symlink test ! -L \
        "$consumed_rolling_evidence" || return
    # conditional-validator-explicit-failures-end
    action33k_outer_observed_hash=$(file_hash "$consumed_rolling_evidence")
    read -r action33k_outer_observed_samples action33k_outer_observed_ok \
        action33k_outer_observed_failed action33k_outer_observed_failure_index \
        action33k_outer_observed_pre_ns action33k_outer_observed_failure_ns \
        action33k_outer_observed_post_ns < <(
            awk -F '\t' '
            /^[0-9][0-9]*\t/ {
                sample++
                timestamp[sample] = $1
...
stop_availability() {
    local action33k_outer_scenario=$1
    local action33k_outer_wait_samples=0

    capture_files "$availability_stop_request"
    while kill -0 "$availability_pid" >/dev/null 2>&1 &&
        ((action33k_outer_wait_samples < 12)); do
        sleep 1
        action33k_outer_wait_samples=$((action33k_outer_wait_samples + 1))
    done
    ...
    availability_pid=
    availability_scenario=
    emit_stream "${action33k_outer_scenario}_availability" "$work_root/$action33k_outer_scenario.availability"
    emit_stream "${action33k_outer_scenario}_handoffs" \
        "$work_root/$action33k_outer_scenario.handoffs"
    [[ -z "$handoff_label" ]] || return 1
    validate_availability_file "$action33k_outer_scenario" \
        "$work_root/$action33k_outer_scenario.availability" \
        "$work_root/$action33k_outer_scenario.handoffs"
    availability_handoffs=
    availability_stop_request=
}
baseline() {
    ...
    run_remote "$action33k_outer_scenario-baseline-a" "$node_a_target" \
        "$node_a_alias" node-a --baseline "$action33k_outer_scenario"
    run_remote "$action33k_outer_scenario-baseline-b" "$node_b_target" \
        "$node_b_alias" node-b --baseline "$action33k_outer_scenario"
    run_remote "$action33k_outer_scenario-master" "$node_a_target" \
        "$node_a_alias" node-a --assert-master "$action33k_outer_scenario"
    run_remote "$action33k_outer_scenario-backup" "$node_b_target" \
        "$node_b_alias" node-b --assert-backup "$action33k_outer_scenario"
    gate "$action33k_outer_scenario-release_identity_equal" test \
        "$(grep -Eo 'observed_current_revision=[A-Za-z0-9._-]+' \
            "$work_root/$action33k_outer_scenario-baseline-a.stdout" | tail -n 1)" = \
        "$(grep -Eo 'observed_current_revision=[A-Za-z0-9._-]+' \
            "$work_root/$action33k_outer_scenario-baseline-b.stdout" | tail -n 1)"
    gate "$action33k_outer_scenario-release_manifest_equal" test \
...
restore_baseline() {
    local action33k_outer_scenario=$1
    run_remote "$action33k_outer_scenario-prepare-b" "$node_b_target" \
        "$node_b_alias" node-b --prepare-cleanup "$action33k_outer_scenario"
    run_remote "$action33k_outer_scenario-prepare-a" "$node_a_target" \
        "$node_a_alias" node-a --prepare-cleanup "$action33k_outer_scenario"
    run_remote "$action33k_outer_scenario-freeze-b" "$node_b_target" \
        "$node_b_alias" node-b --freeze "$action33k_outer_scenario"
    run_remote "$action33k_outer_scenario-freeze-a" "$node_a_target" \
        "$node_a_alias" node-a --freeze "$action33k_outer_scenario"
    run_remote "$action33k_outer_scenario-cleanup-b" "$node_b_target" \
        "$node_b_alias" node-b --cleanup "$action33k_outer_scenario"
    run_remote "$action33k_outer_scenario-cleanup-a" "$node_a_target" \
        "$node_a_alias" node-a --cleanup "$action33k_outer_scenario"
    run_remote "$action33k_outer_scenario-restore-b" "$node_b_target" \
        "$node_b_alias" node-b --restore-services "$action33k_outer_scenario"
    run_remote "$action33k_outer_scenario-restore-a" "$node_a_target" \
        "$node_a_alias" node-a --restore-services "$action33k_outer_scenario"
    run_remote "$action33k_outer_scenario-final-a" "$node_a_target" \
        "$node_a_alias" node-a --final "$action33k_outer_scenario"
    run_remote "$action33k_outer_scenario-final-b" "$node_b_target" \
        "$node_b_alias" node-b --final "$action33k_outer_scenario"
    gate "$action33k_outer_scenario-final_release_identity_equal" test \
        "$(grep -Eo 'observed_current_revision=[A-Za-z0-9._-]+' \
            "$work_root/$action33k_outer_scenario-final-a.stdout" | tail -n 1)" = \
        "$(grep -Eo 'observed_current_revision=[A-Za-z0-9._-]+' \
            "$work_root/$action33k_outer_scenario-final-b.stdout" | tail -n 1)"
    gate "$action33k_outer_scenario-final_release_manifest_equal" test \
        "$(grep -Eo 'observed_release_manifest_sha256=[0-9a-f]{64}' \
            "$work_root/$action33k_outer_scenario-final-a.stdout" | tail -n 1)" = \
        "$(grep -Eo 'observed_release_manifest_sha256=[0-9a-f]{64}' \
            "$work_root/$action33k_outer_scenario-final-b.stdout" | tail -n 1)"
...
normalize_emergency_ancestry() {
    local action33k_outer_scenario=$1
    local action33k_outer_revision=action33k-$run_id-$action33k_outer_scenario-normalized

    begin_handoff "$action33k_outer_scenario-b-to-a"
    run_remote "$action33k_outer_scenario-owner-a" "$node_a_target" \
        "$node_a_alias" node-a --restore-services "$action33k_outer_scenario"
    run_remote "$action33k_outer_scenario-master-a" "$node_a_target" \
        "$node_a_alias" node-a --assert-master "$action33k_outer_scenario"
    run_remote "$action33k_outer_scenario-backup-b" "$node_b_target" \
        "$node_b_alias" node-b --assert-backup "$action33k_outer_scenario"
    end_handoff "$action33k_outer_scenario-b-to-a"
    run_remote "$action33k_outer_scenario-normal-publish-a" "$node_a_target" \
        "$node_a_alias" node-a --publish-normal "$action33k_outer_scenario" \
        "$action33k_outer_revision"
    run_remote "$action33k_outer_scenario-normal-promote-a" "$node_a_target" \
        "$node_a_alias" node-a --promote-outgoing "$action33k_outer_scenario" \
        "$action33k_outer_revision"
    run_remote "$action33k_outer_scenario-normal-wait-b" "$node_b_target" \
        "$node_b_alias" node-b --wait-current "$action33k_outer_scenario" \
        "$action33k_outer_revision"
}
recover() {
    local action33k_outer_failed=false
    if [[ -n "$availability_pid" ]]; then
        stop_availability "$availability_scenario" || true
    fi
    if [[ "$mutated" = true ]]; then
        restore_baseline "$current_scenario" || action33k_outer_failed=true
    fi
    ...
    [[ "$action33k_outer_failed" = false ]] || exit 125
}
...
if [[ "${1:-}" = --availability-stop-self-test ]]; then
    [[ $# -eq 1 ]] || exit 64
    work_root=$(mktemp -d /tmp/action33k-stop-self-test.XXXXXX)
...
# producer between its timestamp and terminal state. Action 33k consumes that
# completed scenario and begins at the first unexecuted reboot case.
...
for scenario in node-a-reboot node-b-controlled node-b-reboot; do
    ...
    case "$scenario" in
        node-a-controlled)
            ...
            run_remote publish-b "$node_b_target" "$node_b_alias" node-b --publish-emergency "$scenario" "action33k-$run_id-$scenario"
            run_remote promote-b "$node_b_target" "$node_b_alias" node-b \
                --promote-outgoing "$scenario" "action33k-$run_id-$scenario"
            run_remote queued-b "$node_b_target" "$node_b_alias" node-b \
                --assert-queued "$scenario" "action33k-$run_id-$scenario"
            sleep 12
            ...
            run_remote wait-a "$node_a_target" "$node_a_alias" node-a \
                --wait-current "$scenario" "action33k-$run_id-$scenario"
            ;;
        node-a-reboot)
        ...
        node-b-controlled)
            ...
            run_remote publish-a "$node_a_target" "$node_a_alias" node-a --publish-normal "$scenario" "action33k-$run_id-$scenario"
