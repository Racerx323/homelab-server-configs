...
readonly prefix=action_28aa_remote
...
readonly node_a_lsyncd_sha256=db0dd1cc9f297052fb4d627822696c9685218f442c5a1282e62186f9e61bad6d
readonly node_b_lsyncd_sha256=b9b525a2a46bab73062cfa5e2e98fab617d137887c889e092bf981504332caf1
...
safe_stream() {
    local action28aa_remote_stream=$1

    [[ "$(wc -c <"$action28aa_remote_stream")" -le "$maximum_stream_bytes" ]] || return 1
    [[ "$(line_count "$action28aa_remote_stream")" -le "$maximum_stream_lines" ]] || return 1
    iconv -f UTF-8 -t UTF-8 "$action28aa_remote_stream" >/dev/null 2>&1 || return 1
    ! LC_ALL=C.UTF-8 grep -n '[^[:print:][:space:]]' \
        "$action28aa_remote_stream" >/dev/null || return 1
    ! grep -Eqi 'BEGIN [A-Z ]*PRIVATE KEY|Authorization:[[:space:]]*Bearer|WEBPASSWORD' \
        "$action28aa_remote_stream"
}
run_stream_classifier_self_test() {
    local action28aa_remote_fixture

    classifier_fixture_root=$(mktemp -d /tmp/action28aa-remote-stream.XXXXXX)
    ...
    for action28aa_remote_fixture in control invalid secret bytes lines; do
        if safe_stream "$classifier_fixture_root/$action28aa_remote_fixture"; then
            return 1
        fi
    done
...
emit_stream() {
    local action28aa_remote_label=$1
    local action28aa_remote_stream=$2

    printf '%s_%s_%s_bytes=%s\n' "$prefix" "$mode" "$action28aa_remote_label" \
        "$(wc -c <"$action28aa_remote_stream")"
    printf '%s_%s_%s_lines=%s\n' "$prefix" "$mode" "$action28aa_remote_label" \
        "$(line_count "$action28aa_remote_stream")"
    printf '%s_%s_%s_sha256=%s\n' "$prefix" "$mode" "$action28aa_remote_label" \
        "$(file_hash "$action28aa_remote_stream")"
    if ! safe_stream "$action28aa_remote_stream"; then
        printf '%s_%s_%s_classification=unsafe_retained\n' \
            "$prefix" "$mode" "$action28aa_remote_label" >&2
        return 97
    fi
    printf '%s_%s_%s_classification=bounded_safe\n' \
        "$prefix" "$mode" "$action28aa_remote_label"
    if [[ -s "$action28aa_remote_stream" ]]; then
        printf '%s_%s_%s_begin\n' "$prefix" "$mode" "$action28aa_remote_label"
        sed "s/^/${prefix}_${mode}_${action28aa_remote_label}_content=/" \
            "$action28aa_remote_stream"
        printf '%s_%s_%s_end\n' "$prefix" "$mode" "$action28aa_remote_label"
    else
...
run_captured() {
    local action28aa_remote_label=$1
    local action28aa_remote_status=0

    shift
    install -m 0600 /dev/null "$evidence_directory/$action28aa_remote_label.stdout"
    install -m 0600 /dev/null "$evidence_directory/$action28aa_remote_label.stderr"
    "$@" >"$evidence_directory/$action28aa_remote_label.stdout" \
        2>"$evidence_directory/$action28aa_remote_label.stderr" || action28aa_remote_status=$?
    printf '%s_%s_%s_status=%s\n' "$prefix" "$mode" "$action28aa_remote_label" \
        "$action28aa_remote_status"
    emit_stream "${action28aa_remote_label}_stdout" \
        "$evidence_directory/$action28aa_remote_label.stdout" || return 97
    emit_stream "${action28aa_remote_label}_stderr" \
        "$evidence_directory/$action28aa_remote_label.stderr" || return 97
    [[ "$action28aa_remote_status" -eq 0 ]]
}
lsyncd_stability() {
    local action28aa_remote_sample
    local action28aa_remote_active
    local action28aa_remote_sub
    local action28aa_remote_result
    local action28aa_remote_pid
    local action28aa_remote_restarts

    for action28aa_remote_sample in 1 2 3 4 5; do
        action28aa_remote_active=$(systemctl show caddy-lsyncd.service \
            --property ActiveState --value) || return 1
        action28aa_remote_sub=$(systemctl show caddy-lsyncd.service \
            --property SubState --value) || return 1
        action28aa_remote_result=$(systemctl show caddy-lsyncd.service \
            --property Result --value) || return 1
        action28aa_remote_pid=$(systemctl show caddy-lsyncd.service \
            --property MainPID --value) || return 1
        action28aa_remote_restarts=$(systemctl show caddy-lsyncd.service \
            --property NRestarts --value) || return 1
        printf 'sample=%s active=%s sub=%s result=%s pid=%s restarts=%s\n' \
            "$action28aa_remote_sample" "$action28aa_remote_active" \
            "$action28aa_remote_sub" "$action28aa_remote_result" \
            "$action28aa_remote_pid" "$action28aa_remote_restarts"
        [[ "$action28aa_remote_active" = active ]] || return 1
        [[ "$action28aa_remote_sub" = running ]] || return 1
        [[ "$action28aa_remote_result" = success ]] || return 1
        [[ "$action28aa_remote_pid" =~ ^[1-9][0-9]*$ ]] || return 1
        [[ "$action28aa_remote_restarts" =~ ^[0-9]+$ ]] || return 1
        [[ "$action28aa_remote_sample" -eq 5 ]] || sleep 1
    done
}
check() {
    local action28aa_remote_label=$1
    ...
    first_failure=$action28aa_remote_label
    printf '%s_%s_check_%s=false\n' "$prefix" "$mode" "$action28aa_remote_label" >&2
    return 1
}
address_count() {
    local action28aa_remote_family=$1
    local action28aa_remote_cidr=$2

    ip -o "-$action28aa_remote_family" address show dev eth0 |
        awk -v expected="$action28aa_remote_cidr" \
...
ownership_exact() {
    local action28aa_remote_state=$1
    local action28aa_remote_count=0
    local action28aa_remote_dbus='(us) 1 "Backup"'

    case "$action28aa_remote_state" in
        master)
            action28aa_remote_count=1
            action28aa_remote_dbus='(us) 2 "Master"'
            ;;
        backup) ;;
        absent)
    ...
    [[ "$(dbus_state "$ipv4_object")" = "$action28aa_remote_dbus" ]] || return 1
    [[ "$(dbus_state "$ipv6_object")" = "$action28aa_remote_dbus" ]] || return 1
    [[ "$(address_count 4 "$dns_ipv4_cidr")" -eq "$action28aa_remote_count" ]] || return 1
    [[ "$(address_count 6 "$dns_ipv6_cidr")" -eq "$action28aa_remote_count" ]] || return 1
    [[ "$(address_count 4 "$caddy_ipv4_cidr")" -eq "$action28aa_remote_count" ]] || return 1
    [[ "$(address_count 6 "$caddy_ipv6_cidr")" -eq "$action28aa_remote_count" ]]
}
stable_ownership() {
    local action28aa_remote_sample
    ...
    for action28aa_remote_sample in 1 2 3 4 5; do
        ...
        printf '%s_%s_stable_sample_%s=true\n' \
            "$prefix" "$mode" "$action28aa_remote_sample"
        sleep 1
    done
}
manifest_valid() {
    local action28aa_remote_release=$1

    # conditional-validator-explicit-failures-begin
    [[ -d "$action28aa_remote_release" && ! -L "$action28aa_remote_release" ]] || return 1
    [[ -f "$action28aa_remote_release/release-manifest.json" ]] || return 1
    [[ -f "$action28aa_remote_release/manifest.sha256" ]] || return 1
    [[ ! -e "$action28aa_remote_release/.complete.pending" && ! -L "$action28aa_remote_release/.complete.pending" ]] || return 1
    manifest_paths_safe "$action28aa_remote_release/manifest.sha256" || return 1
    manifest_file_set_matches "$action28aa_remote_release" || return 1
    (cd "$action28aa_remote_release" &&
        sha256sum --strict --check manifest.sha256 >/dev/null 2>&1) || return 1
    # conditional-validator-explicit-failures-end
}
manifest_paths_safe() {
    local action28aa_remote_manifest=$1
    ...
    awk '
        ...
        END { exit bad ? 1 : 0 }
    ' "$action28aa_remote_manifest" || return 1
    # conditional-validator-explicit-failures-end
}
manifest_file_set_matches() {
    local action28aa_remote_release=$1
    local action28aa_remote_expected
    local action28aa_remote_observed
    local action28aa_remote_status=0

    # conditional-validator-explicit-failures-begin
    action28aa_remote_expected=$(mktemp /tmp/action28aa-manifest-expected.XXXXXX) || return 1
    action28aa_remote_observed=$(mktemp /tmp/action28aa-manifest-observed.XXXXXX) || {
        rm -f -- "$action28aa_remote_expected"
        return 1
    }
    awk '{ print substr($0, 67) }' \
        "$action28aa_remote_release/manifest.sha256" |
        LC_ALL=C sort -u >"$action28aa_remote_expected"
    (
        cd "$action28aa_remote_release"
        find . -type f \
            ...
            -print |
            LC_ALL=C sort
    ) >"$action28aa_remote_observed"
    if ! cmp -s "$action28aa_remote_expected" "$action28aa_remote_observed"; then
        action28aa_remote_status=1
    fi
    rm -f -- "$action28aa_remote_expected" "$action28aa_remote_observed"
    [[ "$action28aa_remote_status" -eq 0 ]] || return 1
    # conditional-validator-explicit-failures-end
}
release_payload_equal() {
    local action28aa_remote_left=$1
    local action28aa_remote_right=$2

    # conditional-validator-explicit-failures-begin
    manifest_valid "$action28aa_remote_left" || return 1
    manifest_valid "$action28aa_remote_right" || return 1
    cmp -s "$action28aa_remote_left/release-manifest.json" "$action28aa_remote_right/release-manifest.json" || return 1
    cmp -s "$action28aa_remote_left/manifest.sha256" "$action28aa_remote_right/manifest.sha256" || return 1
    # conditional-validator-explicit-failures-end
}
lsyncd_baseline_state_valid() {
    ...
    # conditional-validator-explicit-failures-begin
    [[ "$action28aa_remote_state" = inactive || "$action28aa_remote_state" = failed ]] || return 1
    # conditional-validator-explicit-failures-end
}
retained_destination_restored() {
    local action28aa_remote_state=$1
    local action28aa_remote_destination=$2

    # conditional-validator-explicit-failures-begin
    case "$action28aa_remote_state" in
        present)
            [[ -d "$action28aa_remote_destination" && ! -L "$action28aa_remote_destination" ]] || return 1
            ;;
        absent)
            [[ ! -e "$action28aa_remote_destination" && ! -L "$action28aa_remote_destination" ]] || return 1
            ;;
        *) return 1 ;;
    esac
    # conditional-validator-explicit-failures-end
...
run_manifest_boundary_self_test() {
    local action28aa_remote_source
    local action28aa_remote_equal
    local action28aa_remote_drift
    local action28aa_remote_extra

    manifest_fixture_root=$(mktemp -d /tmp/action28aa-manifest-boundary.XXXXXX)
    trap 'rm -rf -- "$manifest_fixture_root"' EXIT INT TERM
    action28aa_remote_source=$manifest_fixture_root/source
    action28aa_remote_equal=$manifest_fixture_root/equal
    action28aa_remote_drift=$manifest_fixture_root/drift
    action28aa_remote_extra=$manifest_fixture_root/extra
    mkdir -p "$action28aa_remote_source/conf.d"
    printf 'fixture\n' >"$action28aa_remote_source/conf.d/site.caddy"
    printf '{"revision":"fixture","source_node":"node-a"}\n' \
        >"$action28aa_remote_source/release-manifest.json"
    (
        cd "$action28aa_remote_source"
        find . -type f ! -path ./manifest.sha256 -print0 |
            LC_ALL=C sort -z |
            xargs -0 sha256sum
    ) >"$action28aa_remote_source/manifest.sha256"
    : >"$action28aa_remote_source/.finalize-request"
    cp -a "$action28aa_remote_source" "$action28aa_remote_equal"
    cp -a "$action28aa_remote_source" "$action28aa_remote_drift"
    cp -a "$action28aa_remote_source" "$action28aa_remote_extra"
    printf 'drift\n' >"$action28aa_remote_drift/conf.d/site.caddy"
    printf 'extra\n' >"$action28aa_remote_extra/unmanifested"
    manifest_valid "$action28aa_remote_source"
    printf '%s_manifest_boundary_valid_source_accepted=true\n' "$prefix"
    release_payload_equal "$action28aa_remote_source" "$action28aa_remote_equal"
    printf '%s_manifest_boundary_equal_payload_accepted=true\n' "$prefix"
    if manifest_valid "$action28aa_remote_drift"; then
        return 1
    fi
    printf '%s_manifest_boundary_hash_drift_rejected=true\n' "$prefix"
    if manifest_valid "$action28aa_remote_extra"; then
        return 1
    fi
    printf '%s_manifest_boundary_unmanifested_file_rejected=true\n' "$prefix"
    if release_payload_equal "$action28aa_remote_source" "$action28aa_remote_drift"; then
        return 1
    fi
...
historical_quarantine_valid() {
    local action28aa_remote_incoming=$1
    local action28aa_remote_quarantine=$2
    local action28aa_remote_release_manifest_sha256=$3
    local action28aa_remote_payload_manifest_sha256=$4

    [[ ! -e "$action28aa_remote_incoming" && ! -L "$action28aa_remote_incoming" ]] || return 1
    manifest_valid "$action28aa_remote_quarantine" || return 1
    [[ "$(file_hash "$action28aa_remote_quarantine/release-manifest.json")" = "$action28aa_remote_release_manifest_sha256" ]] || return 1
    [[ "$(file_hash "$action28aa_remote_quarantine/manifest.sha256")" = "$action28aa_remote_payload_manifest_sha256" ]] || return 1
    [[ "$(jq -r '.revision // empty' \
        "$action28aa_remote_quarantine/release-manifest.json")" = "$historical_revision" ]] || return 1
    [[ "$(jq -r '.source_node // empty' \
        "$action28aa_remote_quarantine/release-manifest.json")" = node-a ]] || return 1
    [[ -f "$action28aa_remote_quarantine/.finalize-request" &&
        ! -L "$action28aa_remote_quarantine/.finalize-request" &&
        ! -s "$action28aa_remote_quarantine/.finalize-request" ]] || return 1
    [[ -f "$action28aa_remote_quarantine/.complete" &&
        ! -L "$action28aa_remote_quarantine/.complete" &&
        ! -s "$action28aa_remote_quarantine/.complete" ]] || return 1
    [[ ! -e "$action28aa_remote_quarantine/.complete.pending" &&
        ! -L "$action28aa_remote_quarantine/.complete.pending" ]] || return 1
    [[ -z "$(find "$action28aa_remote_quarantine" -type l -print -quit)" ]] || return 1
    [[ -z "$(find "$action28aa_remote_quarantine" ! -type d ! -type f -print -quit)" ]] || return 1
    [[ -z "$(find "$action28aa_remote_quarantine" -type f -links +1 -print -quit)" ]] || return 1
    [[ -z "$(find "$action28aa_remote_quarantine" -type d ! -perm 0550 -print -quit)" ]] || return 1
    [[ -z "$(find "$action28aa_remote_quarantine" -type f ! -perm 0440 -print -quit)" ]]
}
run_historical_quarantine_self_test() {
    local action28aa_remote_fixture_incoming
    local action28aa_remote_fixture_quarantine
    local action28aa_remote_fixture_release_hash
    local action28aa_remote_fixture_payload_hash

    quarantine_fixture_root=$(mktemp -d /tmp/action28aa-quarantine.XXXXXX)
    trap 'chmod -R u+rwX -- "$quarantine_fixture_root" 2>/dev/null || :; rm -rf -- "$quarantine_fixture_root"' EXIT INT TERM
    action28aa_remote_fixture_incoming=$quarantine_fixture_root/incoming/$historical_revision
    action28aa_remote_fixture_quarantine=$quarantine_fixture_root/quarantine/node-a-$historical_revision
    mkdir -p "$action28aa_remote_fixture_quarantine"
    printf '{"revision":"%s","parent_revision":"obsolete","source_node":"node-a"}\n' \
        "$historical_revision" >"$action28aa_remote_fixture_quarantine/release-manifest.json"
    printf 'fixture\n' >"$action28aa_remote_fixture_quarantine/Caddyfile"
    : >"$action28aa_remote_fixture_quarantine/.finalize-request"
    : >"$action28aa_remote_fixture_quarantine/.complete"
    (
        cd "$action28aa_remote_fixture_quarantine"
        sha256sum ./Caddyfile ./release-manifest.json >manifest.sha256
    )
    chmod 0440 "$action28aa_remote_fixture_quarantine"/* \
        "$action28aa_remote_fixture_quarantine"/.finalize-request \
        "$action28aa_remote_fixture_quarantine"/.complete
    chmod 0550 "$action28aa_remote_fixture_quarantine"
    action28aa_remote_fixture_release_hash=$(file_hash \
        "$action28aa_remote_fixture_quarantine/release-manifest.json")
    action28aa_remote_fixture_payload_hash=$(file_hash \
        "$action28aa_remote_fixture_quarantine/manifest.sha256")
    historical_quarantine_valid "$action28aa_remote_fixture_incoming" \
        "$action28aa_remote_fixture_quarantine" \
        "$action28aa_remote_fixture_release_hash" \
        "$action28aa_remote_fixture_payload_hash"
    mkdir -p "$action28aa_remote_fixture_incoming"
    if historical_quarantine_valid "$action28aa_remote_fixture_incoming" \
        "$action28aa_remote_fixture_quarantine" \
        "$action28aa_remote_fixture_release_hash" \
        "$action28aa_remote_fixture_payload_hash"; then
        return 1
    fi
    rmdir "$action28aa_remote_fixture_incoming"
    chmod 0644 "$action28aa_remote_fixture_quarantine/Caddyfile"
    if historical_quarantine_valid "$action28aa_remote_fixture_incoming" \
        "$action28aa_remote_fixture_quarantine" \
        "$action28aa_remote_fixture_release_hash" \
        "$action28aa_remote_fixture_payload_hash"; then
...
configure_role() {
    ...
    backup_directory=/var/backups/caddy-ha/action28aa-$role-go-live
    evidence_directory=/tmp/caddy-action28aa/$run_id/$role
...
record_backup() {
    local action28aa_remote_retained_destination=$releases_root/$retained_revision
    ...
    if [[ -e "$action28aa_remote_retained_destination" ||
        -L "$action28aa_remote_retained_destination" ]]; then
        [[ -d "$action28aa_remote_retained_destination" &&
            ! -L "$action28aa_remote_retained_destination" ]] || return 1
...
install_atomic() {
    local action28aa_remote_source=$1
    local action28aa_remote_target=$2
    local action28aa_remote_mode=$3
    local action28aa_remote_directory=${action28aa_remote_target%/*}
    local action28aa_remote_stage

    action28aa_remote_stage=$(mktemp "$action28aa_remote_directory/.action28aa.XXXXXX")
    install -o root -g root -m "$action28aa_remote_mode" \
        "$action28aa_remote_source" "$action28aa_remote_stage"
    mv -fT -- "$action28aa_remote_stage" "$action28aa_remote_target"
}
promote_release() {
    local action28aa_remote_revision=$1
    local action28aa_remote_source=/var/lib/caddy-sync/outbound/$action28aa_remote_revision
    local action28aa_remote_destination=/etc/caddy/releases/$action28aa_remote_revision
    local action28aa_remote_stage

    if [[ "$role" = node-b && "$action28aa_remote_revision" = "$retained_revision" ]]; then
        action28aa_remote_source=/var/lib/caddy-sync/incoming/node-a/$action28aa_remote_revision
    fi
    manifest_valid "$action28aa_remote_source" || return 1
    [[ "$(jq -r '.revision // empty' "$action28aa_remote_source/release-manifest.json")" = "$action28aa_remote_revision" ]] || return 1
    action28aa_remote_stage=$(mktemp -d \
        "/etc/caddy/releases/.action28aa-promote.XXXXXX")
    cp -a -- "$action28aa_remote_source/." "$action28aa_remote_stage/" || {
        rm -rf -- "$action28aa_remote_stage"
        return 1
    }
    manifest_valid "$action28aa_remote_stage" || {
        rm -rf -- "$action28aa_remote_stage"
        return 1
    }
    if release_payload_equal "$action28aa_remote_destination" \
        "$action28aa_remote_stage"; then
        rm -rf -- "$action28aa_remote_stage"
    else
        [[ ! -e "$backup_directory/retained-destination.displaced" &&
            ! -L "$backup_directory/retained-destination.displaced" ]] || return 1
        if [[ -e "$action28aa_remote_destination" ||
            -L "$action28aa_remote_destination" ]]; then
            mv -- "$action28aa_remote_destination" \
                "$backup_directory/retained-destination.displaced" || return 1
        fi
        mv -- "$action28aa_remote_stage" "$action28aa_remote_destination" || return 1
    fi
    chown -R root:caddy-tls "$action28aa_remote_destination"
    find "$action28aa_remote_destination" -type d -exec chmod 0550 {} +
    find "$action28aa_remote_destination" -type f -exec chmod 0440 {} +
    set -a
    # shellcheck disable=SC1091
    source /etc/default/caddy-ha
    set +a
    env CADDY_CONFIG_ROOT="$action28aa_remote_destination" \
        caddy validate --config "$action28aa_remote_destination/Caddyfile" \
        --adapter caddyfile >/dev/null
    ln -sfn "$action28aa_remote_destination" /etc/caddy/current.new
    mv -Tf /etc/caddy/current.new /etc/caddy/current
    systemctl reload caddy.service
}
rollback_node() {
    local action28aa_remote_before
    local action28aa_remote_retained_destination=$releases_root/$retained_revision
    local action28aa_remote_retained_state
    ...
    action28aa_remote_before=$(<"$backup_directory/current.before")
    ln -sfn "$action28aa_remote_before" /etc/caddy/current.new
    mv -Tf /etc/caddy/current.new /etc/caddy/current
    action28aa_remote_retained_state=$(
    ...
    if [[ -d "$backup_directory/retained-destination.displaced" ]]; then
        if [[ -e "$action28aa_remote_retained_destination" ||
            ...
            mv -- "$action28aa_remote_retained_destination" \
                "$backup_directory/retained-destination.failed" || return 1
        fi
        mv -- "$backup_directory/retained-destination.displaced" \
            "$action28aa_remote_retained_destination" || return 1
    elif [[ "$action28aa_remote_retained_state" = absent &&
        -d "$action28aa_remote_retained_destination" ]]; then
        mv -- "$action28aa_remote_retained_destination" \
    ...
    systemctl reset-failed caddy-lsyncd.service caddy-sync-reconcile.service || return 1
...
install -d -o root -g root -m 0700 /tmp/caddy-action28aa "$evidence_directory"
cd /
...
case "$mode" in
    preflight)
        ...
        action28aa_remote_lsyncd_baseline_state=$(
    ...
    install)
        ...
        run_captured reset_lsyncd_failed systemctl reset-failed \
            caddy-lsyncd.service caddy-sync-reconcile.service
    ...
    activate)
        ...
        action28aa_remote_enable_lsyncd_status=0
        run_captured enable_lsyncd systemctl enable --now caddy-lsyncd.service ||
            action28aa_remote_enable_lsyncd_status=$?
        action28aa_remote_lsyncd_stability_status=0
        run_captured lsyncd_stability lsyncd_stability ||
            action28aa_remote_lsyncd_stability_status=$?
        action28aa_remote_lsyncd_show_status=0
        run_captured lsyncd_show systemctl show caddy-lsyncd.service --no-pager \
            ...
            action28aa_remote_lsyncd_show_status=$?
        action28aa_remote_lsyncd_status_status=0
        run_captured lsyncd_status env SYSTEMD_COLORS=0 systemctl status \
            caddy-lsyncd.service --no-pager --full --lines=80 ||
            action28aa_remote_lsyncd_status_status=$?
        action28aa_remote_lsyncd_journal_status=0
        run_captured lsyncd_journal journalctl -u caddy-lsyncd.service \
            --after-cursor "$lsyncd_journal_cursor" --no-pager \
            --output=short-iso || action28aa_remote_lsyncd_journal_status=$?
        ...
        printf '%s_%s_lsyncd_journal_observed_status=%s\n' "$prefix" "$mode" \
            "$action28aa_remote_lsyncd_journal_status"
        check lsyncd_enable_status_zero test \
            "$action28aa_remote_enable_lsyncd_status" -eq 0
        check lsyncd_stability_status_zero test \
            "$action28aa_remote_lsyncd_stability_status" -eq 0
        check lsyncd_show_status_zero test \
            "$action28aa_remote_lsyncd_show_status" -eq 0
        check lsyncd_status_status_zero test \
            "$action28aa_remote_lsyncd_status_status" -eq 0
        check lsyncd_journal_status_zero test \
            "$action28aa_remote_lsyncd_journal_status" -eq 0
    ...
    commit)
        ...
        check transient_stage_absent test \
            -z "$(find /run -maxdepth 1 -name 'caddy-action28aa-bundle.*' \
