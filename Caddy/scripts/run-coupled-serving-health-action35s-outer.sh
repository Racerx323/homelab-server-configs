...
readonly prefix=action_35_s_outer
readonly transaction_sha256=100e584cdaed7a53bcdd44868f9061275af3226f80e0b338c465330b7a6b5574
...
safe_capture() {
    local action35s_path=$1

    regular_file "$action35s_path"
    [[ "$(stat -c '%s' "$action35s_path")" -le "$max_stream_bytes" ]]
    iconv -f UTF-8 -t UTF-8 "$action35s_path" >/dev/null
    ! LC_ALL=C grep -Paq '[\x00-\x08\x0b\x0c\x0e-\x1f\x7f]' "$action35s_path"
}

capture() {
    local action35s_label=$1
    shift
    local action35s_stdout=$workstation_evidence/$action35s_label.stdout
    local action35s_stderr=$workstation_evidence/$action35s_label.stderr
    local action35s_status=$workstation_evidence/$action35s_label.status
    ...
    safe_capture "$action35s_stdout"
    safe_capture "$action35s_stderr"
    return "$action35s_rc"
}

build_payload() {
    local action35s_repository action35s_source action35s_target action35s_mode
    local action35s_hash action35s_lifecycle action35s_source_path action35s_destination
    ...
    install -m 0600 "$repository_root/Caddy/manifests/action35s-node-b-quarantine.tsv" \
        "$payload_stage/manifests/action35s-node-b-quarantine.tsv"
    while IFS=$'\t' read -r action35s_repository action35s_source action35s_target \
        action35s_mode action35s_hash action35s_lifecycle; do
        [[ "$action35s_repository" = '# repository' ]] && continue
        [[ "$action35s_mode" =~ ^0[0-7]{3}$ && "$action35s_lifecycle" = production-candidate ]]
        [[ "$action35s_target" = /* ]]
        action35s_source_path=${workspace_root}/$action35s_repository/$action35s_source
        regular_file "$action35s_source_path"
        [[ "$(sha256sum "$action35s_source_path" | awk '{ print $1 }')" = "$action35s_hash" ]]
        action35s_destination=$payload_stage/repositories/$action35s_repository/$action35s_source
        install -d -m 0700 "$(dirname -- "$action35s_destination")"
        install -m 0600 "$action35s_source_path" "$action35s_destination"
...
readonly archive=$2
case "$remote_root" in /tmp/caddy-action35s-*) ;; *) exit 64 ;; esac
case "$archive" in /tmp/caddy-action35s-*.tar) ;; *) exit 64 ;; esac
...
case "$evidence_root" in /tmp/caddy-action35s-*) ;; *) exit 64 ;; esac
...
ssh_stream() {
    local action35s_host=$1
    local action35s_program=$2
    shift 2
    "$ssh_command" "$action35s_host" \
        "cd / && sudo -n /bin/bash -s --$(printf ' %q' "$@")" <"$action35s_program"
}

upload_payload() {
    local action35s_role=$1
    local action35s_host=$2
    local action35s_remote_root=$3
    local action35s_remote_archive=$4
    local action35s_hash

    action35s_hash=$(awk '{ print $1 }' "$workstation_evidence/payload.sha256")
    capture "$action35s_role-upload-prepare" ssh_stream "$action35s_host" \
        "$prepare_program" "$action35s_remote_root" "$action35s_remote_archive"
    capture "$action35s_role-upload-copy" "$scp_command" -p -- \
        "$payload_archive" "$action35s_host:$action35s_remote_archive"
    capture "$action35s_role-upload-accept" ssh_stream "$action35s_host" \
        "$accept_program" "$action35s_remote_root" "$action35s_remote_archive" \
        "$action35s_hash"
}
...
remote_transaction() {
    ...
    capture "$action35s_label" ssh_stream "$action35s_host" "$transaction" "$@"
}

readback() {
    local action35s_role=$1
    local action35s_host=$2
    local action35s_remote_evidence=$3

    capture "$action35s_role-readback" ssh_stream "$action35s_host" \
        "$readback_program" "$action35s_remote_evidence"
}
...
cleanup_remote() {
    ...
    capture "$action35s_role-disposition" ssh_stream "$action35s_host" \
        "$disposition_program" "$action35s_remote_root" "$action35s_remote_archive"
}

run_live() {
    local action35s_node_b_mutated=false
    local action35s_node_a_mutated=false
    local action35s_failure=0
    local action35s_phase_status=0
    ...
    if upload_payload node-b "$node_b_host" "$node_b_payload" "$node_b_archive" &&
        ...
        return "$action35s_phase_status"
    fi

    action35s_node_b_mutated=true
    ...
    if [[ "$action35s_failure" -eq 0 ]]; then
    ...
    if ! remote_transaction node-a-journal-capture "$node_a_host" journal-capture node-a \
        "$node_a_payload" "$node_a_evidence"; then
        [[ "$action35s_failure" -ne 0 ]] || action35s_failure=1
    fi
    ...
    if [[ "$action35s_failure" -ne 0 ]]; then
        if [[ "$action35s_node_a_mutated" = true ]]; then
            remote_transaction node-a-rollback "$node_a_host" rollback node-a \
                "$node_a_payload" "$node_a_evidence" || exit 125
        fi
        if [[ "$action35s_node_b_mutated" = true ]]; then
...
production_path_test() {
    ...
    local action35s_remote_base action35s_raw action35s_decision action35s_observed
    local action35s_status action35s_role action35s_remote_evidence action35s_test_id
    local retained_root retained_candidate retained_release_hash retained_payload_hash
    local action35s_transaction_test_evidence
    ...
    action35s_remote_base=$(mktemp -d /tmp/caddy-action35s-production-path.XXXXXX)
    action35s_test_id=${action35s_remote_base##*.}
    test_remote_base=$action35s_remote_base
    export ACTION35S_TEST_REMOTE_BASE=$test_remote_base
...
[[ "$1" = -n ]]
shift
export ACTION35S_PRODUCTION_PATH_TEST=1
exec "$@"
SUDO
...
set -Eeuo pipefail
[[ "$1" = -u && "$3" = -- ]]
printf '%s\n' "$2" >>"$ACTION35S_TEST_REMOTE_BASE/runuser.identities"
shift 3
exec "$@"
RUNUSER
...
#!/usr/bin/env bash
set -Eeuo pipefail
printf '%s\n' "$*" >>"$ACTION35S_TEST_REMOTE_BASE/systemctl.calls"
case "$1" in
    is-active) [[ "$2" = --quiet ]] ;;
    start | stop) [[ $# -eq 2 ]] ;;
...
set -Eeuo pipefail
case " $* " in
    *' --show-cursor '*) printf '%s\n' '-- cursor: s=action35s-production-path' ;;
    *' --after-cursor '*) printf '%s\n' 'test-host caddy.service: action35s production-path event' ;;
    *) exit 64 ;;
esac
JOURNALCTL
    cat >"$ssh_command" <<'SSH'
...
shift
command="$*"
printf '%s\t%s\n' "$host" "$command" >>"$ACTION35S_TEST_REMOTE_BASE/ssh.calls"
PATH="$ACTION35S_TEST_REMOTE_BASE/bin:/usr/bin:/bin" /bin/bash -c "$command"
SSH
    cat >"$scp_command" <<'SCP'
#!/usr/bin/env bash
set -Eeuo pipefail
...
source=$3
target=${4#*:}
printf '%s\t%s\n' "$source" "$target" >>"$ACTION35S_TEST_REMOTE_BASE/scp.calls"
...
SCP
    ...
    node_a_payload=/tmp/caddy-action35s-test-$action35s_test_id-node-a
    node_b_payload=/tmp/caddy-action35s-test-$action35s_test_id-node-b
    ...
    action35s_transaction_test_evidence=$action35s_test_root/transaction-through-outer
    install -d -m 0700 "$action35s_transaction_test_evidence"
    CADDY_PRODUCTION_PATH_EVIDENCE_ROOT=$action35s_transaction_test_evidence \
        ACTION35S_TEST_REPOSITORY_ROOT=$repository_root \
    ...
    ACTION35S_INCOMING_ROOT=$retained_root/incoming \
        ACTION35S_OUTGOING_ROOT=$retained_root/outgoing \
        ACTION35S_QUARANTINE_ROOT=$retained_root/quarantine \
        ACTION35S_RELEASES_ROOT=$retained_root/releases \
        ACTION35S_RETAINED_RELEASE_MANIFEST_SHA256=$retained_release_hash \
        ACTION35S_RETAINED_PAYLOAD_MANIFEST_SHA256=$retained_payload_hash \
    ...
    ACTION35S_ENVIRONMENT_FILE=$test_remote_base/environment-node-a \
        ACTION35S_KEEPALIVED_COMMAND=$test_remote_base/bin/keepalived \
        ACTION35S_UNBOUND_CHECKCONF_COMMAND=$test_remote_base/bin/unbound-checkconf \
        ACTION35S_RUNUSER_COMMAND=$test_remote_base/bin/runuser \
        ACTION35S_SYSTEMCTL_COMMAND=$test_remote_base/bin/systemctl \
        ACTION35S_DNS_DIG_COMMAND=$test_remote_base/bin/dig \
        ACTION35S_CURL_COMMAND=$test_remote_base/bin/curl \
        ACTION35S_SS_COMMAND=$test_remote_base/bin/ss \
        remote_transaction node-a-candidate-producer "$node_a_host" candidate-check \
        node-a "$node_a_payload" "$node_a_evidence"
    ACTION35S_ENVIRONMENT_FILE=$test_remote_base/environment-node-b \
        ACTION35S_KEEPALIVED_COMMAND=$test_remote_base/bin/keepalived \
        ACTION35S_UNBOUND_CHECKCONF_COMMAND=$test_remote_base/bin/unbound-checkconf \
        ACTION35S_RUNUSER_COMMAND=$test_remote_base/bin/runuser \
        ACTION35S_SYSTEMCTL_COMMAND=$test_remote_base/bin/systemctl \
        ...
        node-b "$node_b_payload" "$node_b_evidence"
    ACTION35S_JOURNALCTL_COMMAND=$test_remote_base/bin/journalctl \
    ...
    ACTION35S_TARGET_ROOT=$test_remote_base/node-b-root \
        ACTION35S_SYSTEMCTL_COMMAND=$test_remote_base/bin/systemctl \
        ACTION35S_UNBOUND_CHECKCONF_COMMAND=$test_remote_base/bin/unbound-checkconf \
        remote_transaction node-b-install-producer "$node_b_host" install node-b \
        "$node_b_payload" "$node_b_evidence"
    ACTION35S_TARGET_ROOT=$test_remote_base/node-a-root \
        ACTION35S_SYSTEMCTL_COMMAND=$test_remote_base/bin/systemctl \
        ACTION35S_UNBOUND_CHECKCONF_COMMAND=$test_remote_base/bin/unbound-checkconf \
    ...
    action35s_raw=$action35s_test_root/raw/outer-preflight.txt
    action35s_decision=$action35s_test_root/decisions/outer-preflight.tsv
    cat "$test_remote_base/ssh.calls" "$test_remote_base/scp.calls" \
        ...
        >"$action35s_raw"
    action35s_observed=$(wc -l <"$action35s_raw")
    write_decision outer-preflight reach 0 "$action35s_observed" \
        "$action35s_observed" "$action35s_raw" "$action35s_decision"

    for action35s_role in node-a node-b; do
        if [[ "$action35s_role" = node-a ]]; then
            action35s_remote_evidence=$node_a_evidence
        else
            action35s_remote_evidence=$node_b_evidence
        fi
        action35s_raw=$action35s_test_root/raw/evidence-readback-$action35s_role-success.txt
        action35s_decision=$action35s_test_root/decisions/evidence-readback-$action35s_role-success.tsv
        ssh_stream "test-$action35s_role" "$readback_program" \
            "$action35s_remote_evidence" >"$action35s_raw"
        action35s_observed=$(sha256sum "$action35s_raw" | awk '{ print $1 }')
        write_decision "evidence-readback-$action35s_role-success" accept 0 \
            "$action35s_observed" "$action35s_observed" "$action35s_raw" "$action35s_decision"

        action35s_raw=$action35s_test_root/raw/evidence-readback-$action35s_role-failure.txt
        action35s_decision=$action35s_test_root/decisions/evidence-readback-$action35s_role-failure.tsv
        if ssh_stream "test-$action35s_role" "$readback_program" \
            "$action35s_remote_evidence-absent" >"$action35s_raw" 2>&1; then
            action35s_status=0
        else
            action35s_status=$?
        fi
        write_decision "evidence-readback-$action35s_role-failure" reject \
            "$action35s_status" present absent "$action35s_raw" "$action35s_decision"
    done
    cleanup_remote node-a "$node_a_host" "$node_a_payload" "$node_a_archive"
    cleanup_remote node-b "$node_b_host" "$node_b_payload" "$node_b_archive"
    chmod -R u+rwX -- "$action35s_remote_base"
    rm -rf -- "$action35s_remote_base"
...
transaction=$repository_root/Caddy/scripts/apply-coupled-serving-health-action35s.sh
...
if [[ "$invocation_mode" = --production-path-test ]]; then
    workstation_evidence=$(mktemp -d /tmp/caddy-action35s-outer-test.XXXXXX)
else
    workstation_evidence=$(mktemp -d /tmp/caddy-ssh-evidence-action35s.XXXXXX)
fi
readonly workstation_evidence
chmod 0700 "$workstation_evidence"
payload_stage=$workstation_evidence/payload
payload_archive=$workstation_evidence/action35s-payload.tar
...
readonly run_id
node_a_payload=/tmp/caddy-action35s-$run_id-node-a
node_b_payload=/tmp/caddy-action35s-$run_id-node-b
node_a_archive=$node_a_payload.tar
node_b_archive=$node_b_payload.tar
node_a_evidence=$node_a_payload/evidence
node_b_evidence=$node_b_payload/evidence
