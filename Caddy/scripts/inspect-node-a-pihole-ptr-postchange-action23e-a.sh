...
record_check() {
    local action23ea_check_label=$1

    shift
    if [[ -n "${seen_checks[$action23ea_check_label]+set}" ]]; then
        printf '%s_duplicate_check=%s\n' "$prefix" "$action23ea_check_label" >&2
        return 1
    fi
    seen_checks[$action23ea_check_label]=true
    if "$@"; then
        printf '%s_check_%s=true\n' "$prefix" "$action23ea_check_label"
        return 0
    fi
...
address_count() {
    local action23ea_family=$1
    local action23ea_cidr=$2

    ip -o "-$action23ea_family" address show dev "$interface" |
        awk -v wanted="$action23ea_cidr" '$4 == wanted { count++ } END { print count + 0 }'
}
https_probe() {
    local action23ea_name=$1
    local action23ea_address=$2

    curl -kfsS -o /dev/null --connect-timeout 3 --max-time 5 \
        --resolve "${action23ea_name}:443:${action23ea_address}" \
        "https://${action23ea_name}/"
}
run_dns_query() (
    trap - ERR

    local action23ea_server=$1
    local action23ea_port=$2
    local action23ea_name=$3
    local action23ea_type=$4

    if [[ "$action23ea_type" == PTR ]]; then
        timeout 4 dig +time=2 +tries=1 +short -p "$action23ea_port" \
            "@$action23ea_server" -x "$action23ea_name" 2>/dev/null
    else
        timeout 4 dig +time=2 +tries=1 +short -p "$action23ea_port" \
            "@$action23ea_server" "$action23ea_name" "$action23ea_type" 2>/dev/null
    fi
)
query_and_emit() {
    local action23ea_label=$1
    local action23ea_server=$2
    local action23ea_port=$3
    local action23ea_name=$4
    local action23ea_type=$5
    local action23ea_expected=$6
    local action23ea_answer=
    local action23ea_status=0

    if action23ea_answer=$(run_dns_query "$action23ea_server" "$action23ea_port" \
        "$action23ea_name" "$action23ea_type"); then
        action23ea_status=0
    else
        action23ea_status=$?
    fi
    action23ea_answer=$(printf '%s\n' "$action23ea_answer" | sed '/^$/d' | LC_ALL=C sort -u | paste -sd, -)
    action23ea_answer=${action23ea_answer:-none}
    record_check "${action23ea_label}_status" test "$action23ea_status" -eq 0 || return 1
    record_check "${action23ea_label}_safe" safe_single_value "$action23ea_answer" || return 1
    printf '%s_value_%s=%s\n' "$prefix" "$action23ea_label" "$action23ea_answer"
    if [[ "$action23ea_expected" != classify ]]; then
        record_check "${action23ea_label}_exact" test "$action23ea_answer" = "$action23ea_expected" || return 1
    fi
}
capture_safe() {
    local action23ea_capture=$1

    [[ $(printf '%s' "$action23ea_capture" | wc -c) -le "$maximum_capture_bytes" ]] || return 1
    [[ $(printf '%s\n' "$action23ea_capture" | wc -l) -le "$maximum_capture_lines" ]] || return 1
    ! grep -Eqi 'WEBPASSWORD|PRIVATE KEY|Authorization:|api[_-]?key|token=' \
        <<<"$action23ea_capture"
}
emit_capture() {
    local action23ea_capture_label=$1
    local action23ea_capture_text=$2
    local action23ea_capture_sha256

    action23ea_capture_sha256=$(printf '%s' "$action23ea_capture_text" | sha256sum | awk '{ print $1 }')
    record_check "${action23ea_capture_label}_safe" capture_safe "$action23ea_capture_text" || return 1
    printf '%s_value_%s_sha256=%s\n' "$prefix" "$action23ea_capture_label" "$action23ea_capture_sha256"
    printf '%s_value_%s_begin\n' "$prefix" "$action23ea_capture_label"
    if [[ -n "$action23ea_capture_text" ]]; then
        printf '%s\n' "$action23ea_capture_text"
    else
        printf 'none\n'
    fi
...
residue_count() {
    local action23ea_residue_root=$1
    local action23ea_residue_pattern=$2

    find "$action23ea_residue_root" -maxdepth 1 -mindepth 1 \
        -name "$action23ea_residue_pattern" -print | wc -l
}
state_snapshot() {
    printf 'ftl_hash=%s\n' "$(file_hash "$live_ftl")"
...
emit_expected_checks() {
    local action23ea_expected_command

    for action23ea_expected_command in "${required_commands[@]}"; do
        printf 'command_%s_available\n' "${action23ea_expected_command//-/_}"
    done
    printf '%s\n' \
...
emit_contract_transcript() {
    local action23ea_contract_label
    local action23ea_contract_count=0
    local action23ea_fixture_hash=aaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaa

    while IFS= read -r action23ea_contract_label; do
        printf '%s_check_%s=true\n' "$prefix" "$action23ea_contract_label"
        action23ea_contract_count=$((action23ea_contract_count + 1))
    ...
    printf '%s_value_ftl_sha256=%s\n' "$prefix" "$action23ea_fixture_hash"
    ...
    printf '%s_check_count=%s\n' "$prefix" "$action23ea_contract_count"
...
for action23ea_required_command in "${required_commands[@]}"; do
    record_check "command_${action23ea_required_command//-/_}_available" \
        command_available "$action23ea_required_command" || exit 1
done
