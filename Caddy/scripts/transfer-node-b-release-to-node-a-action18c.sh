...
readonly prefix=action_18c_node_b
...
if [[ "${1:-}" == --self-test && $# -eq 1 ]]; then
    ...
    [[ "$(remote_shell_value)" == *"-b $node_b_ipv6"* ]]
...
if [[ "$phase" == transfer && "$checks_failed" -eq 0 ]]; then
    capture_directory=$(mktemp -d /tmp/caddy-action18c-node-b.XXXXXX)
    ...
    if [[ "$checks_failed" -eq 0 ]]; then
        ...
        runuser -u caddy-sync -- timeout --signal=TERM --kill-after=5s 60s \
            ...
            "$release_dir" "caddy-sync@$node_a_fqdn:/" \
