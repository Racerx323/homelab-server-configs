...
readonly prefix=action_20d_retry9_node
...
configure_role() {
    case "$node_role" in
        node-a)
            ...
            expected_production_candidate_sha256=49ab7ef6d2b7c988f192b79e64175ac57f309c0fd639132e34858eda400cc460
...
case "${1:-}" in
    ...
    --inspect)
        ...
        configure_role
        [[ "$node_role" = node-a ]] || exit 64
        transaction_root=$(mktemp -d /run/caddy-action20d-retry9-inspect.XXXXXX)
    ...
    --rollback)
        ...
        configure_role
        [[ "$node_role" = node-a ]] || exit 64
        transaction_root=$(mktemp -d /run/caddy-action20d-retry9-rollback.XXXXXX)
        readonly transaction_root
...
transaction_root=$(mktemp -d /run/caddy-action20d-retry9-node.XXXXXX)
readonly transaction_root
...
backup_directory=$(mktemp -d "$backup_root/action20d-retry9-${node_role}-caddy-vrrp.XXXXXX")
