...
readonly prefix=action_20a_retry2_probe
...
normalized_addresses() {
    ip -o address show dev eth0 2>/dev/null | normalize_address_stream
}
normalized_files_differ() {
    # Invoked indirectly by self_test_assertion.
...
snapshot_state() {
    ...
    printf 'addresses_normalized_sha256=%s\n' \
        "$(normalized_addresses | sha256sum | awk '{ print $1 }')"
...
case "${1:-}" in
    ...
    --self-test)
        [[ $# -eq 1 ]] || exit 64
        self_test_root=$(mktemp -d /tmp/caddy-action20a-retry2-probe-self.XXXXXX)
        readonly self_test_root
        trap 'rm -rf -- "$self_test_root"' EXIT
        expected_assertions >"$self_test_root/labels"
        self_test_assertion assertion_inventory_count test \
            "$(wc -l <"$self_test_root/labels")" -eq 46
        self_test_assertion assertion_inventory_unique test \
            "$(LC_ALL=C sort -u "$self_test_root/labels" | wc -l)" -eq 46
        self_test_assertion assertion_labels_valid test \
            "$(grep -Evc '^[a-z0-9_]+$' "$self_test_root/labels")" -eq 0
        printf '%s\n' \
            '2: eth0 inet 10.1.0.54/22 brd 10.1.3.255 scope global eth0\ valid_lft forever preferred_lft forever' \
            '2: eth0 inet6 fd36:5aa8:6971:1::54/64 scope global dynamic\ valid_lft 100sec preferred_lft 100sec' \
            >"$self_test_root/addresses-before"
        printf '%s\n' \
            '2: eth0 inet 10.1.0.54/22 brd 10.1.3.255 scope global eth0\ valid_lft forever preferred_lft forever' \
            '2: eth0 inet6 fd36:5aa8:6971:1::54/64 scope global dynamic\ valid_lft 98sec preferred_lft 98sec' \
            >"$self_test_root/addresses-lifetime-only"
        printf '%s\n' \
            '2: eth0 inet 10.1.0.54/22 brd 10.1.3.255 scope global eth0\ valid_lft forever preferred_lft forever' \
            '2: eth0 inet6 fd36:5aa8:6971:1::99/64 scope global dynamic\ valid_lft 98sec preferred_lft 98sec' \
            >"$self_test_root/addresses-changed"
        normalize_address_stream <"$self_test_root/addresses-before" \
            >"$self_test_root/normalized-before"
        normalize_address_stream <"$self_test_root/addresses-lifetime-only" \
            >"$self_test_root/normalized-lifetime-only"
        normalize_address_stream <"$self_test_root/addresses-changed" \
            >"$self_test_root/normalized-changed"
        self_test_assertion lifetime_only_drift_ignored cmp -s \
            "$self_test_root/normalized-before" \
            "$self_test_root/normalized-lifetime-only"
        self_test_assertion actual_address_change_detected \
            normalized_files_differ "$self_test_root/normalized-before" \
            "$self_test_root/normalized-changed"
...
work_directory=$(mktemp -d /tmp/caddy-action20a-retry2-probe.XXXXXX)
...
printf '%s_value_state_address_normalization=interface_cidr_scope\n' "$prefix"
