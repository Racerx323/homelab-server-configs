...
readonly publisher=/usr/local/sbin/caddy-publish-release
...
while (($#)); do
    case "$1" in
        ...
        --self-test)
            ...
            printf 'action_28i_inspector_self_test_complete=true\n'
            exit 0
            ;;
        *) exit 64 ;;
    esac
...
case "$node_role:$phase" in
    node-a:baseline)
        readonly prefix=action_28i_node_a_baseline
        ...
        ;;
    node-b:baseline)
        readonly prefix=action_28i_node_b_baseline
        ...
        ;;
    node-a:failed)
        readonly prefix=action_28i_node_a_failed
        readonly expected_hostname=j1-svpihole0
        readonly expected_main_sha256=$node_a_main_sha256
        readonly expected_health_sha256=$node_a_health_sha256
        readonly fragment_sha256=8117510587c79fa597ebd5e48da6ab4be8d8dfef6740d57b9b327037b70830be
        ...
        ;;
    node-b:failed)
        readonly prefix=action_28i_node_b_failed
        readonly expected_hostname=j1-svpihole00
        readonly expected_main_sha256=$node_b_main_sha256
        readonly expected_health_sha256=$node_b_health_sha256
        readonly fragment_sha256=0dd8ec0a5410bcd3c41db0d3fed74cf8bd75978f09b01a797468dd6266b4b518
...
safe_text() {
    local action28i_text=$1
    [[ ${#action28i_text} -le 131072 ]] || return 1
    ! printf '%s' "$action28i_text" | LC_ALL=C grep -q '[^[:print:][:space:]]' || return 1
}
record_check() {
    local action28i_check_label=$1
    shift
    if "$@" >/dev/null 2>&1; then
        printf '%s_check_%s=true\n' "$prefix" "$action28i_check_label"
    else
        printf '%s_check_%s=false\n' "$prefix" "$action28i_check_label"
        failed_check_count=$((failed_check_count + 1))
        [[ "$first_failure" != none ]] || first_failure=$action28i_check_label
    fi
    return 0
}
address_count() {
    local action28i_output=$1
    local action28i_cidr=$2
    printf '%s\n' "$action28i_output" | awk -v expected="$action28i_cidr" \
        '$4 == expected { count++ } END { print count + 0 }'
}
status_matches() {
    local action28i_status=$1
    local action28i_expectation=$2

    if [[ "$action28i_expectation" = zero ]]; then
        [[ "$action28i_status" -eq 0 ]]
    else
        [[ "$action28i_status" -ne 0 ]]
    fi
}
