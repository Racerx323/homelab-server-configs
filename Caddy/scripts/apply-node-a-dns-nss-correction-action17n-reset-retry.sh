...
readonly backup_dir=/var/backups/caddy-ha/action17n-reset-retry-node-a-dns-nss
readonly prior_backup_dir=/var/backups/caddy-ha/action17n-retry-node-a-dns-nss
readonly local_zone_transaction=/etc/unbound/unbound.conf.d/.pihole-local-zone.conf.action17n-reset-retry.new
readonly hosts_transaction=/etc/.hosts.action17n-reset-retry.new
...
readonly prior_driver_sha256=0d56be5b31f141d7b6ab4d92164450d66675dbd9fe05f12a0903644915a91620
...
fail_check() {
    ...
    printf 'action_17n_reset_retry_check_%s=false\n' "$check_label" >&2
    printf 'action_17n_reset_retry_failed_check=%s\n' "$check_label" >&2
    printf 'action_17n_reset_retry_failed_observed=%s\n' "$observed_value" >&2
    return 1
}
...
set_boundary() {
    current_boundary=$1
    printf 'action_17n_reset_retry_boundary=%s\n' "$current_boundary"
}
...
validate_readiness_results() {
    ...
    for readiness_key in "${validation_keys[@]}"; do
        ...
        printf 'action_17n_reset_retry_value_readiness_%s_answer=%s\n' \
            "$readiness_key" "${validation_answer[$readiness_key]}"
        printf 'action_17n_reset_retry_value_readiness_%s_iteration=%s\n' \
...
rollback_record_status() {
    ...
    printf 'action_17n_reset_retry_rollback_check_%s_status=%s\n' \
...
rollback_record_equal() {
    ...
    if [[ "$rollback_observed" == "$rollback_expected" ]]; then
        printf 'action_17n_reset_retry_rollback_check_%s=true\n' \
            "$rollback_label" >&2
    else
        printf 'action_17n_reset_retry_rollback_check_%s=false\n' \
            "$rollback_label" >&2
        rollback_failed=true
    fi
}
...
rollback() {
    ...
    printf 'action_17n_reset_retry_rollback_started=true\n' >&2
    if [[ "$mutation_started" == true ]]; then
        ...
        rollback_status=$?
        rollback_record_status pihole_restartdns "$rollback_status"

        systemctl is-active --quiet pihole-FTL.service
        rollback_status=$?
        rollback_record_status pihole_ftl_active "$rollback_status"
    ...
    if [[ "$mutation_started" == true ]]; then
        ...
        rollback_record_equal pihole_ftl_pid_nonzero \
            "$(if [[ "$(service_pid pihole-FTL.service)" =~ ^[1-9][0-9]*$ ]]; then
                printf true
            else
                printf false
            fi)" true
        rollback_record_equal pihole_ftl_restarts_numeric \
            "$(if [[ "$(service_restarts pihole-FTL.service)" =~ ^[0-9]+$ ]]; then
                printf true
            else
                printf false
            fi)" true
    ...
    if [[ "$rollback_failed" == true ]]; then
        printf 'action_17n_reset_retry_rollback_complete=false\n' >&2
        printf 'manual_intervention_required=true\n' >&2
        exit 125
    fi
    printf 'action_17n_reset_retry_rollback_complete=true\n' >&2
    exit "$original_status"
}
...
unhandled_error() {
    ...
    printf 'action_17n_reset_retry_unhandled_error=true\n' >&2
    printf 'action_17n_reset_retry_unhandled_status=%s\n' "$error_status" >&2
    printf 'action_17n_reset_retry_unhandled_boundary=%s\n' "$current_boundary" >&2
    printf 'action_17n_reset_retry_unhandled_line=%s\n' "$error_line" >&2
    printf 'action_17n_reset_retry_unhandled_command_sha256=%s\n' \
...
if [[ "${1:-}" == --self-test && $# -eq 1 ]]; then
    ...
    printf 'action_17n_reset_retry_driver_self_test_complete=true\n'
    exit 0
elif [[ "${1:-}" != --candidate || $# -ne 2 ]]; then
    printf 'Usage: %s --candidate /run/caddy-action17n-reset-retry.*/pihole-local-zone.conf\n' \
        "${0##*/}" >&2
    exit 2
fi
...
parser_root="$candidate_parent/unbound-action17n-reset-retry.conf"
readonly parser_root
prior_driver_artifact="$candidate_parent/apply-node-a-dns-nss-correction-action17n-retry.sh"
readonly prior_driver_artifact

trap 'unhandled_error "$LINENO" "$BASH_COMMAND"' ERR
trap rollback EXIT

printf 'action_17n_reset_retry_remote_reached=true\n'
...
assert_equal candidate_parent_pattern \
    "$(if [[ "$candidate_parent" =~ ^/run/caddy-action17n-reset-retry\.[A-Za-z0-9]+$ ]]; then
        printf valid
    else
        printf invalid
    fi)" valid
...
assert_equal prior_backup_manifest_action \
    "$(awk -F= '$1 == "action" { print $2 }' "$prior_backup_dir/manifest")" 17n
...
assert_equal backup_manifest_action \
    "$(awk -F= '$1 == "action" { print $2 }' "$backup_dir/manifest")" \
    17n-reset-retry
...
assert_equal pihole_ftl_pid_after_reset_nonzero \
    "$(if [[ "$post_reset_ftl_pid" =~ ^[1-9][0-9]*$ ]]; then
        printf true
    else
        printf false
    fi)" true
assert_equal pihole_ftl_pid_changed_after_reset \
    "$(if [[ "$post_reset_ftl_pid" != "$pre_ftl_pid" ]]; then
        printf true
    else
        printf false
    fi)" true
assert_equal pihole_ftl_restarts_after_reset_numeric \
    "$(if [[ "$post_reset_ftl_restarts" =~ ^[0-9]+$ ]]; then
        printf true
    else
        printf false
    fi)" true
...
printf 'action_17n_reset_retry_backup_dir=%s\n' "$backup_dir"
printf 'action_17n_reset_retry_manifest_action=17n-reset-retry\n'
printf 'action_17n_reset_retry_local_zone_sha256=%s\n' \
    "$candidate_local_zone_sha256"
printf 'action_17n_reset_retry_dns_configuration_mutation=true\n'
printf 'action_17n_reset_retry_nss_configuration_mutation=true\n'
printf 'action_17n_reset_retry_resolv_conf_mutation=false\n'
printf 'action_17n_reset_retry_peer_connections=false\n'
printf 'action_17n_reset_retry_synchronization_executed=false\n'
printf 'action_17n_reset_retry_pihole_cache_reset=true\n'
printf 'action_17n_reset_retry_service_restart=true\n'
printf 'action_17n_reset_retry_acceptance=true\n'
