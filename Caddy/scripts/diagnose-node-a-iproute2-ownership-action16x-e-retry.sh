...
self_test() {
    ...
    if command -v dpkg-query >/dev/null &&
        dpkg-query -W bash >/dev/null 2>&1; then
        grammar_path=$(
            dpkg-query --control-path bash "$control_anchor_name"
        )
        [[ "$grammar_path" == *.md5sums ]]
        [[ -f "$grammar_path" ]]
    fi
    printf 'action_16x_e_retry_self_test_complete=true\n'
}
...
package_binary_name=$(
    dpkg-query -W -f='${binary:Package}' "$expected_package"
)
package_files=$(dpkg-query -L "$expected_package")
package_control_anchor=$(
    dpkg-query --control-path "$expected_package" "$control_anchor_name"
)
...
printf 'package_binary_name=%q\n' "$package_binary_name"
printf 'package_control_anchor_name=%q\n' "$control_anchor_name"
printf 'package_control_anchor=%q\n' "$package_control_anchor"
printf 'package_info_directory=%q\n' "$package_info_dir"
...
printf 'iproute2_ownership_diagnostic_16x_e_retry_complete=true\n'
