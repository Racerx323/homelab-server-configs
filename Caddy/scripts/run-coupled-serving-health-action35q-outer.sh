...
readonly prefix=action_35_q_outer
readonly transaction_sha256=d07bc39db0316a52e82fb3aa94bfc5354b8576b1e487527aca0ad5692597764a
...
safe_capture() {
    local action35q_path=$1

    regular_file "$action35q_path"
    [[ "$(stat -c '%s' "$action35q_path")" -le "$max_stream_bytes" ]]
    iconv -f UTF-8 -t UTF-8 "$action35q_path" >/dev/null
    ! LC_ALL=C grep -Paq '[\x00-\x08\x0b\x0c\x0e-\x1f\x7f]' "$action35q_path"
}

capture() {
    local action35q_label=$1
    shift
    local action35q_stdout=$workstation_evidence/$action35q_label.stdout
    local action35q_stderr=$workstation_evidence/$action35q_label.stderr
    local action35q_status=$workstation_evidence/$action35q_label.status
    ...
    safe_capture "$action35q_stdout"
    safe_capture "$action35q_stderr"
    return "$action35q_rc"
}

build_payload() {
    local action35q_repository action35q_source action35q_target action35q_mode
    local action35q_hash action35q_lifecycle action35q_source_path action35q_destination
    ...
    install -m 0600 "$repository_root/Caddy/manifests/action35q-node-b-quarantine.tsv" \
        "$payload_stage/manifests/action35q-node-b-quarantine.tsv"
    while IFS=$'\t' read -r action35q_repository action35q_source action35q_target \
        action35q_mode action35q_hash action35q_lifecycle; do
        [[ "$action35q_repository" = '# repository' ]] && continue
        [[ "$action35q_mode" =~ ^0[0-7]{3}$ && "$action35q_lifecycle" = production-candidate ]]
        [[ "$action35q_target" = /* ]]
        action35q_source_path=${workspace_root}/$action35q_repository/$action35q_source
        regular_file "$action35q_source_path"
        [[ "$(sha256sum "$action35q_source_path" | awk '{ print $1 }')" = "$action35q_hash" ]]
        action35q_destination=$payload_stage/repositories/$action35q_repository/$action35q_source
        install -d -m 0700 "$(dirname -- "$action35q_destination")"
        install -m 0600 "$action35q_source_path" "$action35q_destination"
...
readonly archive=$2
case "$remote_root" in /tmp/caddy-action35q-*) ;; *) exit 64 ;; esac
case "$archive" in /tmp/caddy-action35q-*.tar) ;; *) exit 64 ;; esac
...
case "$evidence_root" in /tmp/caddy-action35q-*) ;; *) exit 64 ;; esac
...
ssh_stream() {
    local action35q_host=$1
    local action35q_program=$2
    shift 2
    "$ssh_command" "$action35q_host" \
        "cd / && sudo -n /bin/bash -s --$(printf ' %q' "$@")" <"$action35q_program"
}

upload_payload() {
    local action35q_role=$1
    local action35q_host=$2
    local action35q_remote_root=$3
    local action35q_remote_archive=$4
    local action35q_hash

    action35q_hash=$(awk '{ print $1 }' "$workstation_evidence/payload.sha256")
    capture "$action35q_role-upload-prepare" ssh_stream "$action35q_host" \
        "$prepare_program" "$action35q_remote_root" "$action35q_remote_archive"
    capture "$action35q_role-upload-copy" "$scp_command" -p -- \
        "$payload_archive" "$action35q_host:$action35q_remote_archive"
    capture "$action35q_role-upload-accept" ssh_stream "$action35q_host" \
        "$accept_program" "$action35q_remote_root" "$action35q_remote_archive" \
        "$action35q_hash"
}
...
remote_transaction() {
    ...
    capture "$action35q_label" ssh_stream "$action35q_host" "$transaction" "$@"
}

readback() {
    local action35q_role=$1
    local action35q_host=$2
    local action35q_remote_evidence=$3

    capture "$action35q_role-readback" ssh_stream "$action35q_host" \
        "$readback_program" "$action35q_remote_evidence"
}
...
cleanup_remote() {
    ...
    capture "$action35q_role-disposition" ssh_stream "$action35q_host" \
        "$disposition_program" "$action35q_remote_root" "$action35q_remote_archive"
}

run_live() {
    local action35q_node_b_mutated=false
    local action35q_node_a_mutated=false
    local action35q_failure=0
    local action35q_phase_status=0
    ...
    if upload_payload node-b "$node_b_host" "$node_b_payload" "$node_b_archive" &&
        ...
        return "$action35q_phase_status"
    fi

    action35q_node_b_mutated=true
    ...
    if [[ "$action35q_failure" -eq 0 ]]; then
    ...
    if ! remote_transaction node-a-journal-capture "$node_a_host" journal-capture node-a \
        "$node_a_payload" "$node_a_evidence"; then
        [[ "$action35q_failure" -ne 0 ]] || action35q_failure=1
    fi
    ...
    if [[ "$action35q_failure" -ne 0 ]]; then
        if [[ "$action35q_node_a_mutated" = true ]]; then
            remote_transaction node-a-rollback "$node_a_host" rollback node-a \
                "$node_a_payload" "$node_a_evidence" || exit 125
        fi
        if [[ "$action35q_node_b_mutated" = true ]]; then
...
production_path_test() {
    ...
    local action35q_remote_base action35q_raw action35q_decision action35q_observed
    local action35q_status action35q_role action35q_remote_evidence action35q_test_id
    local retained_root retained_candidate retained_release_hash retained_payload_hash
    local action35q_transaction_test_evidence
    ...
    action35q_remote_base=$(mktemp -d /tmp/caddy-action35q-production-path.XXXXXX)
    action35q_test_id=${action35q_remote_base##*.}
    test_remote_base=$action35q_remote_base
    export ACTION35Q_TEST_REMOTE_BASE=$test_remote_base
...
[[ "$1" = -n ]]
shift
export ACTION35Q_PRODUCTION_PATH_TEST=1
exec "$@"
SUDO
...
set -Eeuo pipefail
[[ "$1" = -u && "$3" = -- ]]
printf '%s\n' "$2" >>"$ACTION35Q_TEST_REMOTE_BASE/runuser.identities"
shift 3
exec "$@"
RUNUSER
...
#!/usr/bin/env bash
set -Eeuo pipefail
printf '%s\n' "$*" >>"$ACTION35Q_TEST_REMOTE_BASE/systemctl.calls"
case "$1" in
    is-active) [[ "$2" = --quiet ]] ;;
    start | stop) [[ $# -eq 2 ]] ;;
...
set -Eeuo pipefail
case " $* " in
    *' --show-cursor '*) printf '%s\n' '-- cursor: s=action35q-production-path' ;;
    *' --after-cursor '*) printf '%s\n' 'test-host caddy.service: action35q production-path event' ;;
    *) exit 64 ;;
esac
JOURNALCTL
    cat >"$ssh_command" <<'SSH'
...
shift
command="$*"
printf '%s\t%s\n' "$host" "$command" >>"$ACTION35Q_TEST_REMOTE_BASE/ssh.calls"
PATH="$ACTION35Q_TEST_REMOTE_BASE/bin:/usr/bin:/bin" /bin/bash -c "$command"
SSH
    cat >"$scp_command" <<'SCP'
#!/usr/bin/env bash
set -Eeuo pipefail
...
source=$3
target=${4#*:}
printf '%s\t%s\n' "$source" "$target" >>"$ACTION35Q_TEST_REMOTE_BASE/scp.calls"
...
SCP
    ...
    node_a_payload=/tmp/caddy-action35q-test-$action35q_test_id-node-a
    node_b_payload=/tmp/caddy-action35q-test-$action35q_test_id-node-b
    ...
    action35q_transaction_test_evidence=$action35q_test_root/transaction-through-outer
    install -d -m 0700 "$action35q_transaction_test_evidence"
    CADDY_PRODUCTION_PATH_EVIDENCE_ROOT=$action35q_transaction_test_evidence \
        ACTION35Q_TEST_REPOSITORY_ROOT=$repository_root \
    ...
    ACTION35Q_INCOMING_ROOT=$retained_root/incoming \
        ACTION35Q_OUTGOING_ROOT=$retained_root/outgoing \
        ACTION35Q_QUARANTINE_ROOT=$retained_root/quarantine \
        ACTION35Q_RELEASES_ROOT=$retained_root/releases \
        ACTION35Q_RETAINED_RELEASE_MANIFEST_SHA256=$retained_release_hash \
        ACTION35Q_RETAINED_PAYLOAD_MANIFEST_SHA256=$retained_payload_hash \
    ...
    ACTION35Q_ENVIRONMENT_FILE=$test_remote_base/environment-node-a \
        ACTION35Q_KEEPALIVED_COMMAND=$test_remote_base/bin/keepalived \
        ACTION35Q_UNBOUND_CHECKCONF_COMMAND=$test_remote_base/bin/unbound-checkconf \
        ACTION35Q_RUNUSER_COMMAND=$test_remote_base/bin/runuser \
        ACTION35Q_SYSTEMCTL_COMMAND=$test_remote_base/bin/systemctl \
        ACTION35Q_DNS_DIG_COMMAND=$test_remote_base/bin/dig \
        ACTION35Q_CURL_COMMAND=$test_remote_base/bin/curl \
        ACTION35Q_SS_COMMAND=$test_remote_base/bin/ss \
        remote_transaction node-a-candidate-producer "$node_a_host" candidate-check \
        node-a "$node_a_payload" "$node_a_evidence"
    ACTION35Q_ENVIRONMENT_FILE=$test_remote_base/environment-node-b \
        ACTION35Q_KEEPALIVED_COMMAND=$test_remote_base/bin/keepalived \
        ACTION35Q_UNBOUND_CHECKCONF_COMMAND=$test_remote_base/bin/unbound-checkconf \
        ACTION35Q_RUNUSER_COMMAND=$test_remote_base/bin/runuser \
        ACTION35Q_SYSTEMCTL_COMMAND=$test_remote_base/bin/systemctl \
        ...
        node-b "$node_b_payload" "$node_b_evidence"
    ACTION35Q_JOURNALCTL_COMMAND=$test_remote_base/bin/journalctl \
    ...
    ACTION35Q_TARGET_ROOT=$test_remote_base/node-b-root \
        ACTION35Q_SYSTEMCTL_COMMAND=$test_remote_base/bin/systemctl \
        ACTION35Q_UNBOUND_CHECKCONF_COMMAND=$test_remote_base/bin/unbound-checkconf \
        remote_transaction node-b-install-producer "$node_b_host" install node-b \
        "$node_b_payload" "$node_b_evidence"
    ACTION35Q_TARGET_ROOT=$test_remote_base/node-a-root \
        ACTION35Q_SYSTEMCTL_COMMAND=$test_remote_base/bin/systemctl \
        ACTION35Q_UNBOUND_CHECKCONF_COMMAND=$test_remote_base/bin/unbound-checkconf \
    ...
    action35q_raw=$action35q_test_root/raw/outer-preflight.txt
    action35q_decision=$action35q_test_root/decisions/outer-preflight.tsv
    cat "$test_remote_base/ssh.calls" "$test_remote_base/scp.calls" \
        ...
        >"$action35q_raw"
    action35q_observed=$(wc -l <"$action35q_raw")
    write_decision outer-preflight reach 0 "$action35q_observed" \
        "$action35q_observed" "$action35q_raw" "$action35q_decision"

    for action35q_role in node-a node-b; do
        if [[ "$action35q_role" = node-a ]]; then
            action35q_remote_evidence=$node_a_evidence
        else
            action35q_remote_evidence=$node_b_evidence
        fi
        action35q_raw=$action35q_test_root/raw/evidence-readback-$action35q_role-success.txt
        action35q_decision=$action35q_test_root/decisions/evidence-readback-$action35q_role-success.tsv
        ssh_stream "test-$action35q_role" "$readback_program" \
            "$action35q_remote_evidence" >"$action35q_raw"
        action35q_observed=$(sha256sum "$action35q_raw" | awk '{ print $1 }')
        write_decision "evidence-readback-$action35q_role-success" accept 0 \
            "$action35q_observed" "$action35q_observed" "$action35q_raw" "$action35q_decision"

        action35q_raw=$action35q_test_root/raw/evidence-readback-$action35q_role-failure.txt
        action35q_decision=$action35q_test_root/decisions/evidence-readback-$action35q_role-failure.tsv
        if ssh_stream "test-$action35q_role" "$readback_program" \
            "$action35q_remote_evidence-absent" >"$action35q_raw" 2>&1; then
            action35q_status=0
        else
            action35q_status=$?
        fi
        write_decision "evidence-readback-$action35q_role-failure" reject \
            "$action35q_status" present absent "$action35q_raw" "$action35q_decision"
    done
    cleanup_remote node-a "$node_a_host" "$node_a_payload" "$node_a_archive"
    cleanup_remote node-b "$node_b_host" "$node_b_payload" "$node_b_archive"
    chmod -R u+rwX -- "$action35q_remote_base"
    rm -rf -- "$action35q_remote_base"
...
transaction=$repository_root/Caddy/scripts/apply-coupled-serving-health-action35q.sh
...
if [[ "$invocation_mode" = --production-path-test ]]; then
    workstation_evidence=$(mktemp -d /tmp/caddy-action35q-outer-test.XXXXXX)
else
    workstation_evidence=$(mktemp -d /tmp/caddy-ssh-evidence-action35q.XXXXXX)
fi
readonly workstation_evidence
chmod 0700 "$workstation_evidence"
payload_stage=$workstation_evidence/payload
payload_archive=$workstation_evidence/action35q-payload.tar
...
readonly run_id
node_a_payload=/tmp/caddy-action35q-$run_id-node-a
node_b_payload=/tmp/caddy-action35q-$run_id-node-b
node_a_archive=$node_a_payload.tar
node_b_archive=$node_b_payload.tar
node_a_evidence=$node_a_payload/evidence
node_b_evidence=$node_b_payload/evidence
