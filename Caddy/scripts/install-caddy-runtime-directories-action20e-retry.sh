...
readonly prefix=action_20e_retry_node
...
expected_checks() {
    printf '%s\n' \
        ...
        shadow_root_metadata_exact shadow_config_regular shadow_config_not_symlink \
...
case "${1:-}" in
    ...
    --rollback)
        ...
        ;;
    *)
        printf 'Usage: %s --node node-a|node-b | --rollback --node node-a|node-b BACKUP | --expected-checks | --expected-rollback-checks | --render-config | --render-shadow-config UID PI_GID SYNC_GID | --self-test\n' "${0##*/}" >&2
        exit 64
        ;;
esac
...
transaction_root=$(mktemp -d /run/caddy-action20e-retry-node.XXXXXX)
...
install -d -o root -g root -m 0700 "$shadow_root/etc/tmpfiles.d"
