...
readonly prefix=action_35_w_outer
readonly transaction_sha256=d6009f031c756652819979d9516967428431559d7884889cbbf965cadac60c01
...
safe_capture() {
    local action35w_path=$1

    regular_file "$action35w_path"
    [[ "$(stat -c '%s' "$action35w_path")" -le "$max_stream_bytes" ]]
    iconv -f UTF-8 -t UTF-8 "$action35w_path" >/dev/null
    ! LC_ALL=C grep -Paq '[\x00-\x08\x0b\x0c\x0e-\x1f\x7f]' "$action35w_path"
}

capture() {
    local action35w_label=$1
    shift
    local action35w_stdout=$workstation_evidence/$action35w_label.stdout
    local action35w_stderr=$workstation_evidence/$action35w_label.stderr
    local action35w_status=$workstation_evidence/$action35w_label.status
    ...
    safe_capture "$action35w_stdout"
    safe_capture "$action35w_stderr"
    return "$action35w_rc"
}

build_payload() {
    local action35w_repository action35w_source action35w_target action35w_mode
    local action35w_hash action35w_lifecycle action35w_source_path action35w_destination
    ...
    install -m 0600 "$repository_root/Caddy/manifests/action35w-node-b-quarantine.tsv" \
        "$payload_stage/manifests/action35w-node-b-quarantine.tsv"
    while IFS=$'\t' read -r action35w_repository action35w_source action35w_target \
        action35w_mode action35w_hash action35w_lifecycle; do
        [[ "$action35w_repository" = '# repository' ]] && continue
        [[ "$action35w_mode" =~ ^0[0-7]{3}$ && "$action35w_lifecycle" = production-candidate ]]
        [[ "$action35w_target" = /* ]]
        action35w_source_path=${workspace_root}/$action35w_repository/$action35w_source
        regular_file "$action35w_source_path"
        [[ "$(sha256sum "$action35w_source_path" | awk '{ print $1 }')" = "$action35w_hash" ]]
        action35w_destination=$payload_stage/repositories/$action35w_repository/$action35w_source
        install -d -m 0700 "$(dirname -- "$action35w_destination")"
        install -m 0600 "$action35w_source_path" "$action35w_destination"
...
readonly archive=$2
case "$remote_root" in /tmp/caddy-action35w-*) ;; *) exit 64 ;; esac
case "$archive" in /tmp/caddy-action35w-*.tar) ;; *) exit 64 ;; esac
...
case "$evidence_root" in /tmp/caddy-action35w-*) ;; *) exit 64 ;; esac
...
ssh_stream() {
    local action35w_host=$1
    local action35w_program=$2
    shift 2
    "$ssh_command" "$action35w_host" \
        "cd / && sudo -n /bin/bash -s --$(printf ' %q' "$@")" <"$action35w_program"
}

upload_payload() {
    local action35w_role=$1
    local action35w_host=$2
    local action35w_remote_root=$3
    local action35w_remote_archive=$4
    local action35w_hash

    action35w_hash=$(awk '{ print $1 }' "$workstation_evidence/payload.sha256")
    capture "$action35w_role-upload-prepare" ssh_stream "$action35w_host" \
        "$prepare_program" "$action35w_remote_root" "$action35w_remote_archive"
    capture "$action35w_role-upload-copy" "$scp_command" -p -- \
        "$payload_archive" "$action35w_host:$action35w_remote_archive"
    capture "$action35w_role-upload-accept" ssh_stream "$action35w_host" \
        "$accept_program" "$action35w_remote_root" "$action35w_remote_archive" \
        "$action35w_hash"
}
...
remote_transaction() {
    ...
    capture "$action35w_label" ssh_stream "$action35w_host" "$transaction" "$@"
}

readback() {
    local action35w_role=$1
    local action35w_host=$2
    local action35w_remote_evidence=$3

    capture "$action35w_role-readback" ssh_stream "$action35w_host" \
        "$readback_program" "$action35w_remote_evidence"
}
...
cleanup_remote() {
    ...
    capture "$action35w_role-disposition" ssh_stream "$action35w_host" \
        "$disposition_program" "$action35w_remote_root" "$action35w_remote_archive"
}

run_live() {
    local action35w_node_b_mutated=false
    local action35w_node_a_mutated=false
    local action35w_failure=0
    local action35w_phase_status=0
    ...
    if upload_payload node-b "$node_b_host" "$node_b_payload" "$node_b_archive" &&
        ...
        return "$action35w_phase_status"
    fi

    action35w_node_b_mutated=true
    ...
    if [[ "$action35w_failure" -eq 0 ]]; then
    ...
    if ! remote_transaction node-a-journal-capture "$node_a_host" journal-capture node-a \
        "$node_a_payload" "$node_a_evidence"; then
        [[ "$action35w_failure" -ne 0 ]] || action35w_failure=1
    fi
    ...
    if [[ "$action35w_failure" -ne 0 ]]; then
        if [[ "$action35w_node_a_mutated" = true ]]; then
            remote_transaction node-a-rollback "$node_a_host" rollback node-a \
                "$node_a_payload" "$node_a_evidence" || exit 125
        fi
        if [[ "$action35w_node_b_mutated" = true ]]; then
...
production_path_test() {
    ...
    local action35w_remote_base action35w_raw action35w_decision action35w_observed
    local action35w_status action35w_role action35w_remote_evidence action35w_test_id
    local retained_root retained_candidate retained_release_hash retained_payload_hash
    local action35w_transaction_test_evidence
    ...
    action35w_remote_base=$(mktemp -d /tmp/caddy-action35w-production-path.XXXXXX)
    action35w_test_id=${action35w_remote_base##*.}
    test_remote_base=$action35w_remote_base
    export ACTION35W_TEST_REMOTE_BASE=$test_remote_base
...
[[ "$1" = -n ]]
shift
export ACTION35W_PRODUCTION_PATH_TEST=1
exec "$@"
SUDO
...
set -Eeuo pipefail
[[ "$1" = -u && "$3" = -- ]]
printf '%s\n' "$2" >>"$ACTION35W_TEST_REMOTE_BASE/runuser.identities"
shift 3
exec "$@"
RUNUSER
...
#!/usr/bin/env bash
set -Eeuo pipefail
printf '%s\n' "$*" >>"$ACTION35W_TEST_REMOTE_BASE/prohibited-keepalived-parser.calls"
exit 97
KEEPALIVED
...
#!/usr/bin/env bash
set -Eeuo pipefail
printf '%s\n' "$*" >>"$ACTION35W_TEST_REMOTE_BASE/systemctl.calls"
case "$1" in
    is-active) [[ "$2" = --quiet ]] ;;
    start | stop) [[ $# -eq 2 ]] ;;
...
set -Eeuo pipefail
case " $* " in
    *' --show-cursor '*) printf '%s\n' '-- cursor: s=action35w-production-path' ;;
    *' --after-cursor '*) printf '%s\n' 'test-host caddy.service: action35w production-path event' ;;
    *) exit 64 ;;
esac
JOURNALCTL
    cat >"$ssh_command" <<'SSH'
...
shift
command="$*"
printf '%s\t%s\n' "$host" "$command" >>"$ACTION35W_TEST_REMOTE_BASE/ssh.calls"
PATH="$ACTION35W_TEST_REMOTE_BASE/bin:/usr/bin:/bin" /bin/bash -c "$command"
SSH
    cat >"$scp_command" <<'SCP'
#!/usr/bin/env bash
set -Eeuo pipefail
...
source=$3
target=${4#*:}
printf '%s\t%s\n' "$source" "$target" >>"$ACTION35W_TEST_REMOTE_BASE/scp.calls"
...
SCP
    ...
    node_a_payload=/tmp/caddy-action35w-test-$action35w_test_id-node-a
    node_b_payload=/tmp/caddy-action35w-test-$action35w_test_id-node-b
    ...
    ACTION35W_INCOMING_ROOT=$retained_root/incoming \
        ACTION35W_OUTGOING_ROOT=$retained_root/outgoing \
        ACTION35W_QUARANTINE_ROOT=$retained_root/quarantine \
        ACTION35W_RELEASES_ROOT=$retained_root/releases \
        ACTION35W_RETAINED_RELEASE_MANIFEST_SHA256=$retained_release_hash \
        ACTION35W_RETAINED_PAYLOAD_MANIFEST_SHA256=$retained_payload_hash \
    ...
    ACTION35W_ENVIRONMENT_FILE=$test_remote_base/environment-node-b \
        ACTION35W_UNBOUND_CHECKCONF_COMMAND=$test_remote_base/bin/unbound-checkconf \
        ACTION35W_RUNUSER_COMMAND=$test_remote_base/bin/runuser \
        ACTION35W_SYSTEMCTL_COMMAND=$test_remote_base/bin/systemctl \
        ACTION35W_DNS_DIG_COMMAND=$test_remote_base/bin/dig \
        ACTION35W_CURL_COMMAND=$test_remote_base/bin/curl \
        ACTION35W_SS_COMMAND=$test_remote_base/bin/ss \
    ...
    ACTION35W_JOURNALCTL_COMMAND=$test_remote_base/bin/journalctl \
    ...
    ACTION35W_TARGET_ROOT=$test_remote_base/node-b-root \
        ACTION35W_SYSTEMCTL_COMMAND=$test_remote_base/bin/systemctl \
        ACTION35W_UNBOUND_CHECKCONF_COMMAND=$test_remote_base/bin/unbound-checkconf \
        remote_transaction node-b-install-producer "$node_b_host" install node-b \
        "$node_b_payload" "$node_b_evidence"
    action35w_transaction_test_evidence=$action35w_test_root/transaction-through-outer
    install -d -m 0700 "$action35w_transaction_test_evidence"
    CADDY_PRODUCTION_PATH_EVIDENCE_ROOT=$action35w_transaction_test_evidence \
        ACTION35W_TEST_REPOSITORY_ROOT=$repository_root \
        remote_transaction node-a-post-promote-production-path "$node_a_host" \
        --production-path-test
    ACTION35W_TARGET_ROOT=$test_remote_base/node-a-root \
        ACTION35W_SYSTEMCTL_COMMAND=$test_remote_base/bin/systemctl \
        ACTION35W_UNBOUND_CHECKCONF_COMMAND=$test_remote_base/bin/unbound-checkconf \
    ...
    action35w_raw=$action35w_test_root/raw/outer-preflight.txt
    action35w_decision=$action35w_test_root/decisions/outer-preflight.tsv
    cat "$test_remote_base/ssh.calls" "$test_remote_base/scp.calls" \
        ...
        >"$action35w_raw"
    action35w_observed=$(wc -l <"$action35w_raw")
    write_decision outer-preflight reach 0 "$action35w_observed" \
        "$action35w_observed" "$action35w_raw" "$action35w_decision"

    for action35w_role in node-a node-b; do
        if [[ "$action35w_role" = node-a ]]; then
            action35w_remote_evidence=$node_a_evidence
        else
            action35w_remote_evidence=$node_b_evidence
        fi
        action35w_raw=$action35w_test_root/raw/evidence-readback-$action35w_role-success.txt
        action35w_decision=$action35w_test_root/decisions/evidence-readback-$action35w_role-success.tsv
        ssh_stream "test-$action35w_role" "$readback_program" \
            "$action35w_remote_evidence" >"$action35w_raw"
        action35w_observed=$(sha256sum "$action35w_raw" | awk '{ print $1 }')
        write_decision "evidence-readback-$action35w_role-success" accept 0 \
            "$action35w_observed" "$action35w_observed" "$action35w_raw" "$action35w_decision"

        action35w_raw=$action35w_test_root/raw/evidence-readback-$action35w_role-failure.txt
        action35w_decision=$action35w_test_root/decisions/evidence-readback-$action35w_role-failure.tsv
        if ssh_stream "test-$action35w_role" "$readback_program" \
            "$action35w_remote_evidence-absent" >"$action35w_raw" 2>&1; then
            action35w_status=0
        else
            action35w_status=$?
        fi
        write_decision "evidence-readback-$action35w_role-failure" reject \
            "$action35w_status" present absent "$action35w_raw" "$action35w_decision"
    done
    cleanup_remote node-a "$node_a_host" "$node_a_payload" "$node_a_archive"
    cleanup_remote node-b "$node_b_host" "$node_b_payload" "$node_b_archive"
    chmod -R u+rwX -- "$action35w_remote_base"
    rm -rf -- "$action35w_remote_base"
...
transaction=$repository_root/Caddy/scripts/apply-coupled-serving-health-action35w.sh
...
if [[ "$invocation_mode" = --production-path-test ]]; then
    workstation_evidence=$(mktemp -d /tmp/caddy-action35w-outer-test.XXXXXX)
else
    workstation_evidence=$(mktemp -d /tmp/caddy-ssh-evidence-action35w.XXXXXX)
fi
readonly workstation_evidence
chmod 0700 "$workstation_evidence"
payload_stage=$workstation_evidence/payload
payload_archive=$workstation_evidence/action35w-payload.tar
...
readonly run_id
node_a_payload=/tmp/caddy-action35w-$run_id-node-a
node_b_payload=/tmp/caddy-action35w-$run_id-node-b
node_a_archive=$node_a_payload.tar
node_b_archive=$node_b_payload.tar
node_a_evidence=$node_a_payload/evidence
node_b_evidence=$node_b_payload/evidence
