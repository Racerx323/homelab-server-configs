...
readonly prefix=action_35_o_outer
readonly transaction_sha256=a87579d6a2d74992b8af60cbf3c952bf7356d6eb667667920ed2c49a3ab119d4
...
safe_capture() {
    local action35o_path=$1

    regular_file "$action35o_path"
    [[ "$(stat -c '%s' "$action35o_path")" -le "$max_stream_bytes" ]]
    iconv -f UTF-8 -t UTF-8 "$action35o_path" >/dev/null
    ! LC_ALL=C grep -Paq '[\x00-\x08\x0b\x0c\x0e-\x1f\x7f]' "$action35o_path"
}

capture() {
    local action35o_label=$1
    shift
    local action35o_stdout=$workstation_evidence/$action35o_label.stdout
    local action35o_stderr=$workstation_evidence/$action35o_label.stderr
    local action35o_status=$workstation_evidence/$action35o_label.status
    ...
    safe_capture "$action35o_stdout"
    safe_capture "$action35o_stderr"
    return "$action35o_rc"
}

build_payload() {
    local action35o_repository action35o_source action35o_target action35o_mode
    local action35o_hash action35o_lifecycle action35o_source_path action35o_destination
    ...
    while IFS=$'\t' read -r action35o_repository action35o_source action35o_target \
        action35o_mode action35o_hash action35o_lifecycle; do
        [[ "$action35o_repository" = '# repository' ]] && continue
        [[ "$action35o_mode" =~ ^0[0-7]{3}$ && "$action35o_lifecycle" = production-candidate ]]
        [[ "$action35o_target" = /* ]]
        action35o_source_path=${workspace_root}/$action35o_repository/$action35o_source
        regular_file "$action35o_source_path"
        [[ "$(sha256sum "$action35o_source_path" | awk '{ print $1 }')" = "$action35o_hash" ]]
        action35o_destination=$payload_stage/repositories/$action35o_repository/$action35o_source
        install -d -m 0700 "$(dirname -- "$action35o_destination")"
        install -m 0600 "$action35o_source_path" "$action35o_destination"
...
readonly archive=$2
case "$remote_root" in /tmp/caddy-action35o-*) ;; *) exit 64 ;; esac
case "$archive" in /tmp/caddy-action35o-*.tar) ;; *) exit 64 ;; esac
...
case "$evidence_root" in /tmp/caddy-action35o-*) ;; *) exit 64 ;; esac
...
ssh_stream() {
    local action35o_host=$1
    local action35o_program=$2
    shift 2
    "$ssh_command" "$action35o_host" \
        "cd / && sudo -n /bin/bash -s --$(printf ' %q' "$@")" <"$action35o_program"
}

upload_payload() {
    local action35o_role=$1
    local action35o_host=$2
    local action35o_remote_root=$3
    local action35o_remote_archive=$4
    local action35o_hash

    action35o_hash=$(awk '{ print $1 }' "$workstation_evidence/payload.sha256")
    capture "$action35o_role-upload-prepare" ssh_stream "$action35o_host" \
        "$prepare_program" "$action35o_remote_root" "$action35o_remote_archive"
    capture "$action35o_role-upload-copy" "$scp_command" -p -- \
        "$payload_archive" "$action35o_host:$action35o_remote_archive"
    capture "$action35o_role-upload-accept" ssh_stream "$action35o_host" \
        "$accept_program" "$action35o_remote_root" "$action35o_remote_archive" \
        "$action35o_hash"
}
...
remote_transaction() {
    ...
    capture "$action35o_label" ssh_stream "$action35o_host" "$transaction" "$@"
}

readback() {
    local action35o_role=$1
    local action35o_host=$2
    local action35o_remote_evidence=$3

    capture "$action35o_role-readback" ssh_stream "$action35o_host" \
        "$readback_program" "$action35o_remote_evidence"
}
...
cleanup_remote() {
    ...
    capture "$action35o_role-disposition" ssh_stream "$action35o_host" \
        "$disposition_program" "$action35o_remote_root" "$action35o_remote_archive"
}

run_live() {
    local action35o_node_b_mutated=false
    local action35o_node_a_mutated=false
    local action35o_failure=0
    local action35o_phase_status=0
    ...
    if upload_payload node-b "$node_b_host" "$node_b_payload" "$node_b_archive" &&
        ...
        return "$action35o_phase_status"
    fi

    action35o_node_b_mutated=true
    ...
    if [[ "$action35o_failure" -eq 0 ]]; then
    ...
    if ! remote_transaction node-a-journal-capture "$node_a_host" journal-capture node-a \
        "$node_a_payload" "$node_a_evidence"; then
        [[ "$action35o_failure" -ne 0 ]] || action35o_failure=1
    fi
    ...
    if [[ "$action35o_failure" -ne 0 ]]; then
        if [[ "$action35o_node_a_mutated" = true ]]; then
            remote_transaction node-a-rollback "$node_a_host" rollback node-a \
                "$node_a_payload" "$node_a_evidence" || exit 125
        fi
        if [[ "$action35o_node_b_mutated" = true ]]; then
...
production_path_test() {
    ...
    local action35o_remote_base action35o_raw action35o_decision action35o_observed
    local action35o_status action35o_role action35o_remote_evidence action35o_test_id
    ...
    action35o_remote_base=$(mktemp -d /tmp/caddy-action35o-production-path.XXXXXX)
    action35o_test_id=${action35o_remote_base##*.}
    test_remote_base=$action35o_remote_base
    export ACTION35O_TEST_REMOTE_BASE=$test_remote_base
...
[[ "$1" = -n ]]
shift
export ACTION35O_PRODUCTION_PATH_TEST=1
exec "$@"
SUDO
...
set -Eeuo pipefail
[[ "$1" = -u && "$3" = -- ]]
printf '%s\n' "$2" >>"$ACTION35O_TEST_REMOTE_BASE/runuser.identities"
shift 3
exec "$@"
RUNUSER
...
#!/usr/bin/env bash
set -Eeuo pipefail
printf '%s\n' "$*" >>"$ACTION35O_TEST_REMOTE_BASE/systemctl.calls"
case "$1" in
    is-active) [[ "$2" = --quiet ]] ;;
    start | stop) [[ $# -eq 2 ]] ;;
...
set -Eeuo pipefail
case " $* " in
    *' --show-cursor '*) printf '%s\n' '-- cursor: s=action35o-production-path' ;;
    *' --after-cursor '*) printf '%s\n' 'test-host caddy.service: action35o production-path event' ;;
    *) exit 64 ;;
esac
JOURNALCTL
    cat >"$ssh_command" <<'SSH'
...
shift
command="$*"
printf '%s\t%s\n' "$host" "$command" >>"$ACTION35O_TEST_REMOTE_BASE/ssh.calls"
PATH="$ACTION35O_TEST_REMOTE_BASE/bin:/usr/bin:/bin" /bin/bash -c "$command"
SSH
    cat >"$scp_command" <<'SCP'
#!/usr/bin/env bash
set -Eeuo pipefail
...
source=$3
target=${4#*:}
printf '%s\t%s\n' "$source" "$target" >>"$ACTION35O_TEST_REMOTE_BASE/scp.calls"
...
SCP
    ...
    node_a_payload=/tmp/caddy-action35o-test-$action35o_test_id-node-a
    node_b_payload=/tmp/caddy-action35o-test-$action35o_test_id-node-b
    ...
    ACTION35O_INCOMING_ROOT=$retained_root/incoming \
        ACTION35O_OUTGOING_ROOT=$retained_root/outgoing \
        ACTION35O_QUARANTINE_ROOT=$retained_root/quarantine \
        ACTION35O_RELEASES_ROOT=$retained_root/releases \
        ACTION35O_RETAINED_RELEASE_MANIFEST_SHA256=$retained_release_hash \
        ACTION35O_RETAINED_PAYLOAD_MANIFEST_SHA256=$retained_payload_hash \
    ...
    ACTION35O_INCOMING_ROOT=$retained_root/incoming \
        ...
        ACTION35O_RETAINED_RELEASE_MANIFEST_SHA256=$retained_release_hash \
        ACTION35O_RETAINED_PAYLOAD_MANIFEST_SHA256=$retained_payload_hash \
        ACTION35O_SYSTEMCTL_COMMAND=$test_remote_base/bin/systemctl \
    ...
    ACTION35O_ENVIRONMENT_FILE=$test_remote_base/environment-node-a \
        ACTION35O_KEEPALIVED_COMMAND=$test_remote_base/bin/keepalived \
        ACTION35O_UNBOUND_CHECKCONF_COMMAND=$test_remote_base/bin/unbound-checkconf \
        ACTION35O_RUNUSER_COMMAND=$test_remote_base/bin/runuser \
        ACTION35O_SYSTEMCTL_COMMAND=$test_remote_base/bin/systemctl \
        ACTION35O_DNS_DIG_COMMAND=$test_remote_base/bin/dig \
        ACTION35O_CURL_COMMAND=$test_remote_base/bin/curl \
        ACTION35O_SS_COMMAND=$test_remote_base/bin/ss \
        remote_transaction node-a-candidate-producer "$node_a_host" candidate-check \
        node-a "$node_a_payload" "$node_a_evidence"
    ACTION35O_ENVIRONMENT_FILE=$test_remote_base/environment-node-b \
        ACTION35O_KEEPALIVED_COMMAND=$test_remote_base/bin/keepalived \
        ACTION35O_UNBOUND_CHECKCONF_COMMAND=$test_remote_base/bin/unbound-checkconf \
        ACTION35O_RUNUSER_COMMAND=$test_remote_base/bin/runuser \
        ACTION35O_SYSTEMCTL_COMMAND=$test_remote_base/bin/systemctl \
        ...
        node-b "$node_b_payload" "$node_b_evidence"
    ACTION35O_JOURNALCTL_COMMAND=$test_remote_base/bin/journalctl \
    ...
    ACTION35O_TARGET_ROOT=$test_remote_base/node-b-root \
        ACTION35O_SYSTEMCTL_COMMAND=$test_remote_base/bin/systemctl \
        ACTION35O_UNBOUND_CHECKCONF_COMMAND=$test_remote_base/bin/unbound-checkconf \
        remote_transaction node-b-install-producer "$node_b_host" install node-b \
        "$node_b_payload" "$node_b_evidence"
    ACTION35O_TARGET_ROOT=$test_remote_base/node-a-root \
        ACTION35O_SYSTEMCTL_COMMAND=$test_remote_base/bin/systemctl \
        ACTION35O_UNBOUND_CHECKCONF_COMMAND=$test_remote_base/bin/unbound-checkconf \
    ...
    action35o_raw=$action35o_test_root/raw/outer-preflight.txt
    action35o_decision=$action35o_test_root/decisions/outer-preflight.tsv
    cat "$test_remote_base/ssh.calls" "$test_remote_base/scp.calls" \
        ...
        >"$action35o_raw"
    action35o_observed=$(wc -l <"$action35o_raw")
    write_decision outer-preflight reach 0 "$action35o_observed" \
        "$action35o_observed" "$action35o_raw" "$action35o_decision"

    for action35o_role in node-a node-b; do
        if [[ "$action35o_role" = node-a ]]; then
            action35o_remote_evidence=$node_a_evidence
        else
            action35o_remote_evidence=$node_b_evidence
        fi
        action35o_raw=$action35o_test_root/raw/evidence-readback-$action35o_role-success.txt
        action35o_decision=$action35o_test_root/decisions/evidence-readback-$action35o_role-success.tsv
        ssh_stream "test-$action35o_role" "$readback_program" \
            "$action35o_remote_evidence" >"$action35o_raw"
        action35o_observed=$(sha256sum "$action35o_raw" | awk '{ print $1 }')
        write_decision "evidence-readback-$action35o_role-success" accept 0 \
            "$action35o_observed" "$action35o_observed" "$action35o_raw" "$action35o_decision"

        action35o_raw=$action35o_test_root/raw/evidence-readback-$action35o_role-failure.txt
        action35o_decision=$action35o_test_root/decisions/evidence-readback-$action35o_role-failure.tsv
        if ssh_stream "test-$action35o_role" "$readback_program" \
            "$action35o_remote_evidence-absent" >"$action35o_raw" 2>&1; then
            action35o_status=0
        else
            action35o_status=$?
        fi
        write_decision "evidence-readback-$action35o_role-failure" reject \
            "$action35o_status" present absent "$action35o_raw" "$action35o_decision"
    done
    cleanup_remote node-a "$node_a_host" "$node_a_payload" "$node_a_archive"
    cleanup_remote node-b "$node_b_host" "$node_b_payload" "$node_b_archive"
    chmod -R u+rwX -- "$action35o_remote_base"
    rm -rf -- "$action35o_remote_base"
...
transaction=$repository_root/Caddy/scripts/apply-coupled-serving-health-action35o.sh
...
if [[ "$invocation_mode" = --production-path-test ]]; then
    workstation_evidence=$(mktemp -d /tmp/caddy-action35o-outer-test.XXXXXX)
else
    workstation_evidence=$(mktemp -d /tmp/caddy-ssh-evidence-action35o.XXXXXX)
fi
readonly workstation_evidence
chmod 0700 "$workstation_evidence"
payload_stage=$workstation_evidence/payload
payload_archive=$workstation_evidence/action35o-payload.tar
...
readonly run_id
node_a_payload=/tmp/caddy-action35o-$run_id-node-a
node_b_payload=/tmp/caddy-action35o-$run_id-node-b
node_a_archive=$node_a_payload.tar
node_b_archive=$node_b_payload.tar
node_a_evidence=$node_a_payload/evidence
node_b_evidence=$node_b_payload/evidence
