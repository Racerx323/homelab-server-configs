...
readonly prefix=action_28u_outer
...
readonly regression=$caddy_root/tests/action28u-coupled-go-live-regression.sh
readonly manifest=$caddy_root/manifests/caddy-coupled-go-live-action28u.yaml
...
evidence_root=${CADDY_ACTION28U_EVIDENCE_ROOT:-/tmp/caddy-ssh-evidence/action28u}
readonly evidence_root
ssh_binary=${CADDY_ACTION28U_SSH_BIN:-/usr/bin/ssh}
readonly ssh_binary
run_id=
outer_root=
last_stdout=
...
write_remote() {
    ...
    # shellcheck disable=SC2016
    {
        ...
        printf '"$stage/transact-coupled-go-live-action28u.sh" --mode %q --role %q --stage "$stage" --run-id %q' \
