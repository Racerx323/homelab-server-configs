...
pass_check() {
    ...
    printf 'action_17n_check_%s=true\n' "$check_label"
}
...
fail_check() {
    ...
    printf 'action_17n_check_%s=false\n' "$check_label" >&2
    printf 'action_17n_failed_check=%s\n' "$check_label" >&2
    printf 'action_17n_failed_observed=%s\n' "$observed_value" >&2
    return 1
}
...
rollback() {
    ...
    set +e
    printf 'action_17n_rollback_started=true\n' >&2
    if [[ "$mutation_started" == true ]]; then
        install -o root -g root -m 0644 \
            "$backup_dir/pihole-local-zone.conf.before" "$live_local_zone"
        rollback_status=$?
        printf 'action_17n_rollback_local_zone_restore_status=%s\n' \
            "$rollback_status" >&2
        [[ "$rollback_status" -eq 0 ]] || rollback_failed=true

        cp -a -- "$backup_dir/hosts.before" "$live_hosts"
        rollback_status=$?
        printf 'action_17n_rollback_hosts_restore_status=%s\n' \
            "$rollback_status" >&2
        [[ "$rollback_status" -eq 0 ]] || rollback_failed=true

        unbound-control reload >/dev/null 2>&1
        rollback_status=$?
        printf 'action_17n_rollback_unbound_reload_status=%s\n' \
            "$rollback_status" >&2
        [[ "$rollback_status" -eq 0 ]] || rollback_failed=true
    fi
    ...
    rollback_status=$?
    printf 'action_17n_rollback_transaction_cleanup_status=%s\n' \
        "$rollback_status" >&2
    [[ "$rollback_status" -eq 0 ]] || rollback_failed=true
    ...
    if [[ "$rollback_failed" == true ]]; then
        printf 'action_17n_rollback_complete=false\n' >&2
        printf 'manual_intervention_required=true\n' >&2
        exit 125
    fi
    printf 'action_17n_rollback_complete=true\n' >&2
    exit "$original_status"
}
...
unhandled_error() {
    ...
    printf 'action_17n_unhandled_error=true\n' >&2
    printf 'action_17n_unhandled_status=%s\n' "$error_status" >&2
    printf 'action_17n_unhandled_boundary=%s\n' "$current_boundary" >&2
    printf 'action_17n_unhandled_line=%s\n' "$error_line" >&2
    printf 'action_17n_unhandled_command_sha256=%s\n' \
...
if [[ "${1:-}" == --self-test && $# -eq 1 ]]; then
    ...
    printf 'action_17n_driver_self_test_complete=true\n'
    exit 0
elif [[ "${1:-}" != --candidate || $# -ne 2 ]]; then
    printf 'Usage: %s --candidate /run/caddy-action17n.*/pihole-local-zone.conf\n' \
        "${0##*/}" >&2
    exit 2
fi
...
parser_root="$candidate_parent/unbound-action17n.conf"
readonly parser_root

trap 'unhandled_error "$LINENO" "$BASH_COMMAND"' ERR
trap rollback EXIT

printf 'action_17n_remote_reached=true\n'
...
assert_equal candidate_parent_pattern \
    "$(if [[ "$candidate_parent" =~ ^/run/caddy-action17n\.[A-Za-z0-9]+$ ]]; then
        printf valid
    else
        printf invalid
    fi)" valid
...
for readiness_iteration in {1..20}; do
    if query_exact 127.0.0.1 pihole00.local.theama.co AAAA "$peer_ipv6" &&
        query_exact 127.0.0.1 pihole0.local.theama.co AAAA \
            fd36:5aa8:6971:1::53 &&
        query_reverse_exact 127.0.0.1 "$peer_ipv6" \
            pihole00.local.theama.co.; then
        readiness_passed=true
        printf 'action_17n_readiness_iteration=%s\n' "$readiness_iteration"
        break
    fi
    sleep 1
done
...
printf 'action_17n_backup_dir=%s\n' "$backup_dir"
printf 'action_17n_local_zone_sha256=%s\n' "$candidate_local_zone_sha256"
printf 'action_17n_dns_configuration_mutation=true\n'
printf 'action_17n_nss_configuration_mutation=true\n'
printf 'action_17n_resolv_conf_mutation=false\n'
printf 'action_17n_peer_connections=false\n'
printf 'action_17n_synchronization_executed=false\n'
printf 'action_17n_service_restart=false\n'
printf 'action_17n_acceptance=true\n'
