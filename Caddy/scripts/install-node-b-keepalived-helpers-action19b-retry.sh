...
readonly expected_continuity_inspector_sha256=a9a92b4007e7b6a0798a76fd57bdd23771970e23d1e486e76b66f6408eb92c55
...
validate_action19b_a_retry2_baseline() {
    local continuity_error=$stage_directory/action19b-a-retry2.stderr
    local continuity_output=$stage_directory/action19b-a-retry2.stdout
    local continuity_status=0
    local assertion_count
    local unique_count

    : >"$continuity_output"
    : >"$continuity_error"
    chmod 0600 "$continuity_output" "$continuity_error"
    /bin/bash \
        "$stage_directory/inspect-node-b-action19b-postfailure-action19b-a-retry2.sh" \
        --baseline-inspector \
        "$stage_directory/inspect-node-b-keepalived-helper-prerequisite-action19a-a.sh" \
        >"$continuity_output" 2>"$continuity_error" || continuity_status=$?
    require_check action19b_a_retry2_status_zero \
        test "$continuity_status" -eq 0 || return 1
    require_check action19b_a_retry2_stderr_empty \
        test ! -s "$continuity_error" || return 1
    assertion_count=$(grep -Ec \
        '^action_19b_a_retry2_assertion_[a-z0-9_]+=true$' \
        "$continuity_output" || true)
    require_check action19b_a_retry2_assertion_count_exact \
        test "$assertion_count" -eq 89 || return 1
    unique_count=$(sed -n \
        's/^\(action_19b_a_retry2_assertion_[a-z0-9_]*\)=true$/\1/p' \
        "$continuity_output" | LC_ALL=C sort -u | wc -l)
    require_check action19b_a_retry2_assertion_labels_unique \
        test "$unique_count" -eq 89 || return 1
    require_check action19b_a_retry2_false_assertions_absent \
        test "$(grep -Ec \
            '^action_19b_a_retry2_assertion_[a-z0-9_]+=false$' \
            "$continuity_output" || true)" -eq 0 || return 1
    require_check action19b_a_retry2_assertion_summary_exact grep -Fxq \
        'action_19b_a_retry2_assertion_count=89' "$continuity_output" ||
        return 1
    require_check action19b_a_retry2_failed_summary_zero grep -Fxq \
        'action_19b_a_retry2_failed_assertion_count=0' "$continuity_output" ||
        return 1
    require_check action19b_a_retry2_first_failure_none grep -Fxq \
        'action_19b_a_retry2_first_failure=none' "$continuity_output" ||
        return 1
    require_check action19b_a_retry2_inspection_complete grep -Fxq \
        'action_19b_a_retry2_inspection_complete=true' "$continuity_output" ||
        return 1
    require_check action19b_a_retry2_helper_invocation_false grep -Fxq \
        'action_19b_a_retry2_helper_invoked=false' "$continuity_output" ||
        return 1
    require_check action19b_a_retry2_filesystem_mutation_false grep -Fxq \
        'action_19b_a_retry2_filesystem_mutation=false' "$continuity_output" ||
        return 1
    require_check action19b_a_retry2_service_mutation_false grep -Fxq \
        'action_19b_a_retry2_service_mutation=false' "$continuity_output" ||
        return 1
    require_check action19b_a_retry2_keepalived_mutation_false grep -Fxq \
        'action_19b_a_retry2_keepalived_mutation=false' "$continuity_output" ||
        return 1
    require_check action19b_a_retry2_vrrp_vip_mutation_false grep -Fxq \
        'action_19b_a_retry2_vrrp_vip_mutation=false' "$continuity_output" ||
        return 1
    require_check action19b_a_retry2_persistent_mutation_false grep -Fxq \
        'action_19b_a_retry2_persistent_mutation=false' "$continuity_output" ||
        return 1
}
...
validate_prestate() {
    ...
    validate_source "$expected_inspector_sha256" action19a_a_inspector \
        "$stage_directory/inspect-node-b-keepalived-helper-prerequisite-action19a-a.sh" ||
        return 1
    validate_source "$expected_continuity_inspector_sha256" \
        action19b_a_retry2_inspector \
        "$stage_directory/inspect-node-b-action19b-postfailure-action19b-a-retry2.sh" ||
        return 1
    validate_action19b_a_retry2_baseline || return 1
    ...
    require_check rollback_root_metadata \
        ...
        return 1
    require_check prior_action19b_retry_backup_absent \
        test -z "$(find "$rollback_root" -mindepth 1 -maxdepth 1 \
            -name 'action19b-retry-node-b-keepalived-helpers.*' -print -quit)" ||
        return 1
    require_check prior_action19b_retry_health_install_stage_absent \
        test -z "$(find /usr/local/libexec -mindepth 1 -maxdepth 1 \
            -name '.check-caddy.action19b-retry.*' -print -quit)" || return 1
    require_check prior_action19b_retry_notification_install_stage_absent \
        test -z "$(find /usr/local/libexec -mindepth 1 -maxdepth 1 \
            -name '.lsyncd-ha-failover-notify.action19b-retry.*' -print -quit)" ||
        return 1
    capture_continuity_state
}
...
case "${1:-}" in
    --self-test)
        ...
        [[ "$expected_continuity_inspector_sha256" =~ ^[0-9a-f]{64}$ ]]
...
rollback_directory=$(mktemp -d \
    "$rollback_root/action19b-retry-node-b-keepalived-helpers.XXXXXX")
...
health_install_stage=$(mktemp /usr/local/libexec/.check-caddy.action19b-retry.XXXXXX)
notification_install_stage=$(mktemp \
    /usr/local/libexec/.lsyncd-ha-failover-notify.action19b-retry.XXXXXX)
