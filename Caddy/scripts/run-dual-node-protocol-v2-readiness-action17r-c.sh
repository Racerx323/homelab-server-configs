...
readonly caddy_root
readonly node_a_inspector="$script_directory/inspect-node-a-protocol-v2-readiness-action17r-c.sh"
...
record_runner_assertion() {
    ...
    printf 'action_17r_c_runner_assertion_%s=%s\n' "$runner_label" "$runner_value"
...
if [[ "${1:-}" = --self-test && $# -eq 1 ]]; then
    verify_inspectors
    printf 'action_17r_c_runner_self_test_complete=true\n'
    exit 0
elif [[ "${1:-}" = --source-test && $# -eq 1 ]]; then
    verify_inspectors
    printf 'action_17r_c_runner_source_test_complete=true\n'
    exit 0
elif [[ "${1:-}" = --contract-test && $# -eq 1 ]]; then
    work_directory=$(mktemp -d /tmp/caddy-action17r-c-contract.XXXXXX)
    readonly work_directory
    trap 'rm -rf -- "$work_directory"' EXIT
    write_contract_fixture "$node_a_inspector" action_17r_c_node_a node-a \
    ...
    validate_remote_transcript node_a action_17r_c_node_a "$node_a_inspector" \
    ...
    printf 'action_17r_c_runner_contract_test_complete=true\n'
    exit 0
elif (($#)); then
...
verify_inspectors
work_directory=$(mktemp -d /tmp/caddy-action17r-c-runner.XXXXXX)
readonly work_directory
cleanup() {
...
node_a_payload=$(value_for action_17r_c_node_a_value_payload_sha256 \
...
node_a_manifest=$(value_for action_17r_c_node_a_value_manifest_sha256 \
...
record_semantic_command node_a_source_state_exact \
    require_one \
    action_17r_c_node_a_value_source_state=legacy_complete_requires_v2_finalize_request \
    "$work_directory/node-a.out"
...
printf 'action_17r_c_node_a_ssh_status=%s\n' "$node_a_status"
printf 'action_17r_node_b_after_ssh_status=%s\n' "$node_b_after_status"
printf 'action_17r_c_runner_assertion_count=%s\n' "$runner_assertion_count"
printf 'action_17r_c_runner_failed_assertion_count=%s\n' \
    "$runner_failed_assertion_count"
printf 'action_17r_c_runner_first_failure=%s\n' "$runner_first_failure"
printf 'action_17r_c_finalization_readiness=legacy_release_requires_transactional_marker_migration\n'
printf 'action_17r_c_peer_connection_executed=false\n'
printf 'action_17r_c_restricted_command_executed=false\n'
printf 'action_17r_c_release_transfer_executed=false\n'
printf 'action_17r_c_marker_mutation=false\n'
printf 'action_17r_c_helper_invocation=false\n'
printf 'action_17r_c_service_mutations=false\n'
printf 'action_17r_c_persistent_mutations=false\n'

if [[ "$runner_contract_invalid" = true ]]; then
    printf 'action_17r_c_runner_acceptance=false\n'
    exit 97
fi
...
printf 'action_17r_c_runner_acceptance=true\n'
cleanup
trap - EXIT
if [[ -e "$work_directory" ]]; then
    printf 'action_17r_c_workstation_cleanup_complete=false\n' >&2
    exit 97
fi
printf 'action_17r_c_workstation_cleanup_complete=true\n'
