...
readonly prefix=action_28x_remote
...
safe_stream() {
    local action28x_remote_stream=$1

    [[ "$(wc -c <"$action28x_remote_stream")" -le "$maximum_stream_bytes" ]] || return 1
    [[ "$(line_count "$action28x_remote_stream")" -le "$maximum_stream_lines" ]] || return 1
    iconv -f UTF-8 -t UTF-8 "$action28x_remote_stream" >/dev/null 2>&1 || return 1
    ! LC_ALL=C.UTF-8 grep -n '[^[:print:][:space:]]' \
        "$action28x_remote_stream" >/dev/null || return 1
    ! grep -Eqi 'BEGIN [A-Z ]*PRIVATE KEY|Authorization:[[:space:]]*Bearer|WEBPASSWORD' \
        "$action28x_remote_stream"
}
run_stream_classifier_self_test() {
    local action28x_remote_fixture

    classifier_fixture_root=$(mktemp -d /tmp/action28x-remote-stream.XXXXXX)
    ...
    for action28x_remote_fixture in control invalid secret bytes lines; do
        if safe_stream "$classifier_fixture_root/$action28x_remote_fixture"; then
            return 1
        fi
    done
...
emit_stream() {
    local action28x_remote_label=$1
    local action28x_remote_stream=$2

    printf '%s_%s_%s_bytes=%s\n' "$prefix" "$mode" "$action28x_remote_label" \
        "$(wc -c <"$action28x_remote_stream")"
    printf '%s_%s_%s_lines=%s\n' "$prefix" "$mode" "$action28x_remote_label" \
        "$(line_count "$action28x_remote_stream")"
    printf '%s_%s_%s_sha256=%s\n' "$prefix" "$mode" "$action28x_remote_label" \
        "$(file_hash "$action28x_remote_stream")"
    if ! safe_stream "$action28x_remote_stream"; then
        printf '%s_%s_%s_classification=unsafe_retained\n' \
            "$prefix" "$mode" "$action28x_remote_label" >&2
        return 97
    fi
    printf '%s_%s_%s_classification=bounded_safe\n' \
        "$prefix" "$mode" "$action28x_remote_label"
    if [[ -s "$action28x_remote_stream" ]]; then
        printf '%s_%s_%s_begin\n' "$prefix" "$mode" "$action28x_remote_label"
        sed "s/^/${prefix}_${mode}_${action28x_remote_label}_content=/" \
            "$action28x_remote_stream"
        printf '%s_%s_%s_end\n' "$prefix" "$mode" "$action28x_remote_label"
    else
...
run_captured() {
    local action28x_remote_label=$1
    local action28x_remote_status=0

    shift
    install -m 0600 /dev/null "$evidence_directory/$action28x_remote_label.stdout"
    install -m 0600 /dev/null "$evidence_directory/$action28x_remote_label.stderr"
    "$@" >"$evidence_directory/$action28x_remote_label.stdout" \
        2>"$evidence_directory/$action28x_remote_label.stderr" || action28x_remote_status=$?
    printf '%s_%s_%s_status=%s\n' "$prefix" "$mode" "$action28x_remote_label" \
        "$action28x_remote_status"
    emit_stream "${action28x_remote_label}_stdout" \
        "$evidence_directory/$action28x_remote_label.stdout" || return 97
    emit_stream "${action28x_remote_label}_stderr" \
        "$evidence_directory/$action28x_remote_label.stderr" || return 97
    [[ "$action28x_remote_status" -eq 0 ]]
}
check() {
    local action28x_remote_label=$1
    ...
    first_failure=$action28x_remote_label
    printf '%s_%s_check_%s=false\n' "$prefix" "$mode" "$action28x_remote_label" >&2
    return 1
}
address_count() {
    local action28x_remote_family=$1
    local action28x_remote_cidr=$2

    ip -o "-$action28x_remote_family" address show dev eth0 |
        awk -v expected="$action28x_remote_cidr" \
...
ownership_exact() {
    local action28x_remote_state=$1
    local action28x_remote_count=0
    local action28x_remote_dbus='(us) 1 "Backup"'

    case "$action28x_remote_state" in
        master)
            action28x_remote_count=1
            action28x_remote_dbus='(us) 2 "Master"'
            ;;
        backup) ;;
        absent)
    ...
    [[ "$(dbus_state "$ipv4_object")" = "$action28x_remote_dbus" ]] || return 1
    [[ "$(dbus_state "$ipv6_object")" = "$action28x_remote_dbus" ]] || return 1
    [[ "$(address_count 4 "$dns_ipv4_cidr")" -eq "$action28x_remote_count" ]] || return 1
    [[ "$(address_count 6 "$dns_ipv6_cidr")" -eq "$action28x_remote_count" ]] || return 1
    [[ "$(address_count 4 "$caddy_ipv4_cidr")" -eq "$action28x_remote_count" ]] || return 1
    [[ "$(address_count 6 "$caddy_ipv6_cidr")" -eq "$action28x_remote_count" ]]
}
stable_ownership() {
    local action28x_remote_sample

    for action28x_remote_sample in 1 2 3 4 5; do
        ownership_exact "$expected_state" || return 1
        printf '%s_%s_stable_sample_%s=true\n' \
            "$prefix" "$mode" "$action28x_remote_sample"
        sleep 1
    done
}
manifest_valid() {
    local action28x_remote_release=$1

    [[ -d "$action28x_remote_release" && ! -L "$action28x_remote_release" ]] || return 1
    [[ -f "$action28x_remote_release/release-manifest.json" ]] || return 1
    [[ -f "$action28x_remote_release/manifest.sha256" ]] || return 1
    (cd "$action28x_remote_release" && sha256sum --strict --check manifest.sha256 >/dev/null)
}
historical_quarantine_valid() {
    local action28x_remote_incoming=$1
    local action28x_remote_quarantine=$2
    local action28x_remote_release_manifest_sha256=$3
    local action28x_remote_payload_manifest_sha256=$4

    [[ ! -e "$action28x_remote_incoming" && ! -L "$action28x_remote_incoming" ]] || return 1
    manifest_valid "$action28x_remote_quarantine" || return 1
    [[ "$(file_hash "$action28x_remote_quarantine/release-manifest.json")" = "$action28x_remote_release_manifest_sha256" ]] || return 1
    [[ "$(file_hash "$action28x_remote_quarantine/manifest.sha256")" = "$action28x_remote_payload_manifest_sha256" ]] || return 1
    [[ "$(jq -r '.revision // empty' \
        "$action28x_remote_quarantine/release-manifest.json")" = "$historical_revision" ]] || return 1
    [[ "$(jq -r '.source_node // empty' \
        "$action28x_remote_quarantine/release-manifest.json")" = node-a ]] || return 1
    [[ -f "$action28x_remote_quarantine/.finalize-request" &&
        ! -L "$action28x_remote_quarantine/.finalize-request" &&
        ! -s "$action28x_remote_quarantine/.finalize-request" ]] || return 1
    [[ -f "$action28x_remote_quarantine/.complete" &&
        ! -L "$action28x_remote_quarantine/.complete" &&
        ! -s "$action28x_remote_quarantine/.complete" ]] || return 1
    [[ ! -e "$action28x_remote_quarantine/.complete.pending" &&
        ! -L "$action28x_remote_quarantine/.complete.pending" ]] || return 1
    [[ -z "$(find "$action28x_remote_quarantine" -type l -print -quit)" ]] || return 1
    [[ -z "$(find "$action28x_remote_quarantine" ! -type d ! -type f -print -quit)" ]] || return 1
    [[ -z "$(find "$action28x_remote_quarantine" -type f -links +1 -print -quit)" ]] || return 1
    [[ -z "$(find "$action28x_remote_quarantine" -type d ! -perm 0550 -print -quit)" ]] || return 1
    [[ -z "$(find "$action28x_remote_quarantine" -type f ! -perm 0440 -print -quit)" ]]
}
run_historical_quarantine_self_test() {
    local action28x_remote_fixture_incoming
    local action28x_remote_fixture_quarantine
    local action28x_remote_fixture_release_hash
    local action28x_remote_fixture_payload_hash

    quarantine_fixture_root=$(mktemp -d /tmp/action28x-quarantine.XXXXXX)
    trap 'chmod -R u+rwX -- "$quarantine_fixture_root" 2>/dev/null || :; rm -rf -- "$quarantine_fixture_root"' EXIT INT TERM
    action28x_remote_fixture_incoming=$quarantine_fixture_root/incoming/$historical_revision
    action28x_remote_fixture_quarantine=$quarantine_fixture_root/quarantine/node-a-$historical_revision
    mkdir -p "$action28x_remote_fixture_quarantine"
    printf '{"revision":"%s","parent_revision":"obsolete","source_node":"node-a"}\n' \
        "$historical_revision" >"$action28x_remote_fixture_quarantine/release-manifest.json"
    printf 'fixture\n' >"$action28x_remote_fixture_quarantine/Caddyfile"
    : >"$action28x_remote_fixture_quarantine/.finalize-request"
    : >"$action28x_remote_fixture_quarantine/.complete"
    (
        cd "$action28x_remote_fixture_quarantine"
        sha256sum ./Caddyfile ./release-manifest.json >manifest.sha256
    )
    chmod 0440 "$action28x_remote_fixture_quarantine"/* \
        "$action28x_remote_fixture_quarantine"/.finalize-request \
        "$action28x_remote_fixture_quarantine"/.complete
    chmod 0550 "$action28x_remote_fixture_quarantine"
    action28x_remote_fixture_release_hash=$(file_hash \
        "$action28x_remote_fixture_quarantine/release-manifest.json")
    action28x_remote_fixture_payload_hash=$(file_hash \
        "$action28x_remote_fixture_quarantine/manifest.sha256")
    historical_quarantine_valid "$action28x_remote_fixture_incoming" \
        "$action28x_remote_fixture_quarantine" \
        "$action28x_remote_fixture_release_hash" \
        "$action28x_remote_fixture_payload_hash"
    mkdir -p "$action28x_remote_fixture_incoming"
    if historical_quarantine_valid "$action28x_remote_fixture_incoming" \
        "$action28x_remote_fixture_quarantine" \
        "$action28x_remote_fixture_release_hash" \
        "$action28x_remote_fixture_payload_hash"; then
        return 1
    fi
    rmdir "$action28x_remote_fixture_incoming"
    chmod 0644 "$action28x_remote_fixture_quarantine/Caddyfile"
    if historical_quarantine_valid "$action28x_remote_fixture_incoming" \
        "$action28x_remote_fixture_quarantine" \
        "$action28x_remote_fixture_release_hash" \
        "$action28x_remote_fixture_payload_hash"; then
...
configure_role() {
    ...
    backup_directory=/var/backups/caddy-ha/action28x-$role-go-live
    evidence_directory=/tmp/caddy-action28x/$run_id/$role
...
install_atomic() {
    local action28x_remote_source=$1
    local action28x_remote_target=$2
    local action28x_remote_mode=$3
    local action28x_remote_directory=${action28x_remote_target%/*}
    local action28x_remote_stage

    action28x_remote_stage=$(mktemp "$action28x_remote_directory/.action28x.XXXXXX")
    install -o root -g root -m "$action28x_remote_mode" \
        "$action28x_remote_source" "$action28x_remote_stage"
    mv -fT -- "$action28x_remote_stage" "$action28x_remote_target"
}
promote_release() {
    local action28x_remote_revision=$1
    local action28x_remote_source=/var/lib/caddy-sync/outbound/$action28x_remote_revision
    local action28x_remote_destination=/etc/caddy/releases/$action28x_remote_revision

    if [[ "$role" = node-b && "$action28x_remote_revision" = "$retained_revision" ]]; then
        action28x_remote_source=/var/lib/caddy-sync/incoming/node-a/$action28x_remote_revision
    fi
    manifest_valid "$action28x_remote_source" || return 1
    [[ "$(jq -r '.revision // empty' "$action28x_remote_source/release-manifest.json")" = "$action28x_remote_revision" ]] || return 1
    if [[ ! -d "$action28x_remote_destination" ]]; then
        cp -a -- "$action28x_remote_source" "$action28x_remote_destination"
    fi
    chown -R root:caddy-tls "$action28x_remote_destination"
    find "$action28x_remote_destination" -type d -exec chmod 0550 {} +
    find "$action28x_remote_destination" -type f -exec chmod 0440 {} +
    set -a
    # shellcheck disable=SC1091
    source /etc/default/caddy-ha
    set +a
    env CADDY_CONFIG_ROOT="$action28x_remote_destination" \
        caddy validate --config "$action28x_remote_destination/Caddyfile" \
        --adapter caddyfile >/dev/null
    ln -sfn "$action28x_remote_destination" /etc/caddy/current.new
    mv -Tf /etc/caddy/current.new /etc/caddy/current
    systemctl reload caddy.service
}
rollback_node() {
    local action28x_remote_before
    ...
    action28x_remote_before=$(<"$backup_directory/current.before")
    ln -sfn "$action28x_remote_before" /etc/caddy/current.new
    mv -Tf /etc/caddy/current.new /etc/caddy/current
...
install -d -o root -g root -m 0700 /tmp/caddy-action28x "$evidence_directory"
cd /
...
case "$mode" in
    ...
    commit)
        ...
        check transient_stage_absent test \
            -z "$(find /run -maxdepth 1 -name 'caddy-action28x-bundle.*' \
