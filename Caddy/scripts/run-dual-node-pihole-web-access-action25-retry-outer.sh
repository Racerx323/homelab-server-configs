...
readonly prefix=action_25_retry_outer
script_directory=$(cd -- "$(dirname -- "${BASH_SOURCE[0]}")" && pwd)
readonly script_directory
readonly caddy_root=${script_directory%/scripts}
readonly inspector=$script_directory/inspect-dual-node-pihole-web-access-action25-retry.sh
readonly regression=$caddy_root/tests/action25-retry-observed-response-regression.sh
...
validate_observed_endpoint() {
    local action25_outer_observed_role=$1
    local action25_outer_observed_token=$2
    local action25_outer_observed_transcript=$3
    local action25_outer_observed_label=$4
    local action25_outer_observed_fqdn=$5
    local action25_outer_observed_remote=$6
    local action25_outer_observed_prefix="action_25_retry_${action25_outer_observed_token}_observed_${action25_outer_observed_label}"

    validation_check "$action25_outer_observed_role" "${action25_outer_observed_label}_observed_command_status" \
        grep -Fqx "${action25_outer_observed_prefix}_command_status=0" \
        "$action25_outer_observed_transcript" || return 1
    validation_check "$action25_outer_observed_role" "${action25_outer_observed_label}_observed_output_classification" \
        grep -Fqx "${action25_outer_observed_prefix}_output_classification=bounded_safe" \
        "$action25_outer_observed_transcript" || return 1
    validation_check "$action25_outer_observed_role" "${action25_outer_observed_label}_observed_output_begin" \
        grep -Fqx "${action25_outer_observed_prefix}_output_begin" \
        "$action25_outer_observed_transcript" || return 1
    validation_check "$action25_outer_observed_role" "${action25_outer_observed_label}_observed_output_end" \
        grep -Fqx "${action25_outer_observed_prefix}_output_end" \
        "$action25_outer_observed_transcript" || return 1
    validation_check "$action25_outer_observed_role" "${action25_outer_observed_label}_observed_http_code" \
        grep -Fqx "${action25_outer_observed_prefix}_http_code=200" \
        "$action25_outer_observed_transcript" || return 1
    validation_check "$action25_outer_observed_role" "${action25_outer_observed_label}_observed_effective_url" \
        grep -Fqx "${action25_outer_observed_prefix}_effective_url=https://${action25_outer_observed_fqdn}/admin/" \
        "$action25_outer_observed_transcript" || return 1
    validation_check "$action25_outer_observed_role" "${action25_outer_observed_label}_observed_remote_ip" \
        grep -Fqx "${action25_outer_observed_prefix}_remote_ip=${action25_outer_observed_remote}" \
        "$action25_outer_observed_transcript" || return 1
    validation_check "$action25_outer_observed_role" "${action25_outer_observed_label}_observed_body_bytes" \
        grep -Eq "^${action25_outer_observed_prefix}_body_bytes=[1-9][0-9]*$" \
        "$action25_outer_observed_transcript" || return 1
    validation_check "$action25_outer_observed_role" "${action25_outer_observed_label}_observed_body_sha256" \
        grep -Eq "^${action25_outer_observed_prefix}_body_sha256=[0-9a-f]{64}$" \
        "$action25_outer_observed_transcript" || return 1
    validation_check "$action25_outer_observed_role" "${action25_outer_observed_label}_observed_body_classification" \
        grep -Fqx "${action25_outer_observed_prefix}_body_classification=bounded_safe" \
        "$action25_outer_observed_transcript" || return 1
    validation_check "$action25_outer_observed_role" "${action25_outer_observed_label}_observed_body_marker" \
        grep -Fqx "${action25_outer_observed_prefix}_body_pihole_marker=true" \
        "$action25_outer_observed_transcript" || return 1
}
validate_node_transcript() {
    ...
    action25_outer_actual_text=$(sed -n \
        "s/^action_25_retry_${action25_outer_token}_check_\([^=]*\)=true$/\1/p" \
    ...
    action25_outer_check_lines=$(grep -Ec "^action_25_retry_${action25_outer_token}_check_.*=(true|false)$" \
    ...
    validation_check "$action25_outer_role" declared_check_count \
        grep -Fqx "action_25_retry_${action25_outer_token}_check_count=$action25_outer_expected_count" \
        "$action25_outer_transcript" || return 1
    validation_check "$action25_outer_role" endpoint_count \
        grep -Fqx "action_25_retry_${action25_outer_token}_endpoint_count=6" \
        "$action25_outer_transcript" || return 1
    validation_check "$action25_outer_role" filesystem_mutation_false \
        grep -Fqx "action_25_retry_${action25_outer_token}_filesystem_mutation=false" \
        "$action25_outer_transcript" || return 1
    validation_check "$action25_outer_role" service_mutation_false \
        grep -Fqx "action_25_retry_${action25_outer_token}_service_mutation=false" \
        "$action25_outer_transcript" || return 1
    validation_check "$action25_outer_role" dns_mutation_false \
        grep -Fqx "action_25_retry_${action25_outer_token}_dns_mutation=false" \
        "$action25_outer_transcript" || return 1
    validation_check "$action25_outer_role" peer_ssh_false \
        grep -Fqx "action_25_retry_${action25_outer_token}_peer_ssh=false" \
        "$action25_outer_transcript" || return 1
    validation_check "$action25_outer_role" remote_complete \
        grep -Fqx "action_25_retry_${action25_outer_token}_remote_complete=true" \
        "$action25_outer_transcript" || return 1
    validation_check "$action25_outer_role" local_zone_hash \
        grep -Fqx "action_25_retry_${action25_outer_token}_value_local_zone_sha256=fa9f4850386ab1328f323c7c88bd9fa9ad0d5a84994b3066b6874deb5beb569c" \
        "$action25_outer_transcript" || return 1
    ...
    validation_check "$action25_outer_role" vrrp_state \
        grep -Fqx "action_25_retry_${action25_outer_token}_value_vrrp_state=$action25_outer_expected_vrrp" \
        "$action25_outer_transcript" || return 1
    while IFS='|' read -r action25_outer_observed_label action25_outer_observed_fqdn action25_outer_observed_remote; do
        validate_observed_endpoint "$action25_outer_role" "$action25_outer_token" \
            "$action25_outer_transcript" "$action25_outer_observed_label" \
            "$action25_outer_observed_fqdn" "$action25_outer_observed_remote" || return 1
    done <<'EOF'
...
EOF
    action25_outer_http_count=$(grep -Ec \
        "^action_25_retry_${action25_outer_token}_value_.*_http_code=200$" \
        "$action25_outer_transcript" || true) || return 1
    ...
    action25_outer_body_hash_count=$(grep -Ec \
        "^action_25_retry_${action25_outer_token}_value_.*_body_sha256=[0-9a-f]+$" \
        "$action25_outer_transcript" || true) || return 1
    ...
    action25_outer_false_count=$(grep -Ec \
        "^action_25_retry_${action25_outer_token}_check_.*=false$" \
        "$action25_outer_transcript" || true) || return 1
...
run_node() {
    ...
    "${CADDY_ACTION25_RETRY_SSH_BIN:-ssh}" \
...
run_action() {
    ...
    run_local_gates "${CADDY_ACTION25_RETRY_SKIP_REGRESSION:-false}" || return 1
...
case "${1:-}" in
    ...
    --validate-transcript)
        ...
        ;;
    --self-test)
        CADDY_ACTION25_RETRY_SKIP_REGRESSION=true run_local_gates true
        ;;
    '') run_action ;;
    *) exit 64 ;;
esac
