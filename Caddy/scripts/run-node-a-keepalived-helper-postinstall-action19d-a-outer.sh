...
readonly regression_sha256=55def1e05bdb4e9f84a991c90ac4bd8b4bcbef9da36981f55ec30033c0c475f8
...
readonly derivation="$script_directory/derive-node-a-keepalived-helper-postinstall-action19d-a.sh"
readonly regression="$caddy_root/tests/action19d-a-node-a-keepalived-helper-postinstall-regression.sh"
...
render_stage() {
    ...
    local staged_inspector=$stage_scripts/inspect-node-a-keepalived-helper-postinstall-action19d-a.sh
    local staged_runner=$stage_scripts/run-node-a-keepalived-helper-postinstall-action19d-a.sh
...
case "${1:-}" in
    --self-test | --source-test | --contract-test)
        [[ $# -eq 1 ]] || exit 64
        test_root=$(mktemp -d /tmp/caddy-action19d-a-outer-test.XXXXXX)
        readonly test_root
...
work_directory=$(mktemp -d /tmp/caddy-action19d-a-outer.XXXXXX)
