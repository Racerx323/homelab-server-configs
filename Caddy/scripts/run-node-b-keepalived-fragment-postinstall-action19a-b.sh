...
readonly ssh_binary=${CADDY_ACTION19AB_SSH_BINARY:-/usr/bin/ssh}
...
validate_transcript() {
    ...
    contract_root=$(mktemp -d /tmp/caddy-action19a-b-contract.XXXXXX) || return 97
    ...
    require_one "${prefix}_helper_execution=false" "$output_path" || return 97
...
case "${1:-}" in
    ...
    --contract-test)
        [[ $# -eq 1 ]]
        verify_sources
        contract_root=$(mktemp -d /tmp/caddy-action19a-b-runner-contract.XXXXXX)
...
work_directory=$(mktemp -d /tmp/caddy-action19a-b-runner.XXXXXX)
...
remote_status=0
if [[ "${CADDY_ACTION19AB_INTERCEPTED_TEST:-}" = 1 ]]; then
