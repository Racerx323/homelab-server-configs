...
readonly rollback_directory="$rollback_root/action17q-retry-node-b-protocol-v2.TEhT7k"
readonly rollback_authorization="$rollback_directory/authorized_keys.before"
readonly rollback_authorization_hash_record="$rollback_directory/authorized_keys.before.sha256"
...
record_assertion() {
    ...
    printf 'action_17q_b_assertion_%s=%s\n' \
...
stable_state_snapshot() {
    stat -c '%n|%U:%G:%a:%s:%i' \
        ...
        "$rollback_authorization_hash_record"
    printf 'receiver_v1_sha256=%s\n' "$(file_hash "$receiver_v1")"
    printf 'receiver_v2_sha256=%s\n' "$(file_hash "$receiver_v2")"
    printf 'finalizer_v2_sha256=%s\n' "$(file_hash "$finalizer_v2")"
    printf 'authorization_sha256=%s\n' "$(file_hash "$authorized_keys")"
    printf 'rollback_authorization_sha256=%s\n' \
        "$(file_hash "$rollback_authorization")"
    printf 'rollback_hash_record_sha256=%s\n' \
        "$(file_hash "$rollback_authorization_hash_record")"
...
if [[ "${1:-}" == --self-test && $# -eq 1 ]]; then
    ...
    printf 'action_17q_b_inspector_self_test_complete=true\n'
    exit 0
elif (($#)); then
...
work_directory=$(mktemp -d /tmp/caddy-action17q-b.XXXXXX)
...
record_command receiver_v2_metadata \
    test "$(stat -c '%U:%G:%a' "$receiver_v2" 2>/dev/null || true)" = \
    root:root:755
record_command receiver_v2_hash_exact \
    test "$(file_hash "$receiver_v2" 2>/dev/null || true)" = \
    "$expected_receiver_v2_sha256"
record_command receiver_v2_syntax bash -n "$receiver_v2"
record_command finalizer_v2_regular test -f "$finalizer_v2"
record_command finalizer_v2_not_symlink test ! -L "$finalizer_v2"
record_command finalizer_v2_metadata \
    test "$(stat -c '%U:%G:%a' "$finalizer_v2" 2>/dev/null || true)" = \
    root:root:755
record_command finalizer_v2_hash_exact \
    test "$(file_hash "$finalizer_v2" 2>/dev/null || true)" = \
    "$expected_finalizer_v2_sha256"
record_command finalizer_v2_syntax bash -n "$finalizer_v2"
...
record_command authorized_keys_hash_exact \
    ...
    "$expected_authorization_sha256"
record_command authorized_keys_single_line \
    test "$(wc -l <"$authorized_keys" 2>/dev/null || true)" -eq 1
record_command authorized_keys_node_a_fingerprint \
    test "$(authorization_fingerprint "$authorized_keys" \
        2>/dev/null || true)" = "$expected_node_a_fingerprint"
...
action17q_retry_backup_count=$(
    find "$rollback_root" -mindepth 1 -maxdepth 1 \
        -name 'action17q-retry-node-b-protocol-v2.*' -printf '.' 2>/dev/null |
        wc -c
)
readonly action17q_retry_backup_count
action17q_retry_stage_count=$(
    find /run -mindepth 1 -maxdepth 1 \
        -name 'caddy-action17q-retry-stage.*' -printf '.' 2>/dev/null |
        wc -c
)
readonly action17q_retry_stage_count
record_command action17q_backup_count_zero \
    test "$action17q_backup_count" -eq 0
record_command rollback_directory_regular test -d "$rollback_directory"
record_command rollback_directory_not_symlink test ! -L "$rollback_directory"
record_command rollback_directory_metadata \
    test "$(stat -c '%U:%G:%a' "$rollback_directory" \
        2>/dev/null || true)" = root:root:700
record_command rollback_authorization_regular test -f "$rollback_authorization"
record_command rollback_authorization_not_symlink \
    test ! -L "$rollback_authorization"
record_command rollback_authorization_metadata \
    test "$(stat -c '%U:%G:%a' "$rollback_authorization" \
        2>/dev/null || true)" = root:root:600
record_command rollback_authorization_hash_exact \
    test "$(file_hash "$rollback_authorization" \
        2>/dev/null || true)" = "$expected_old_authorization_sha256"
record_command rollback_hash_record_regular \
    test -f "$rollback_authorization_hash_record"
record_command rollback_hash_record_not_symlink \
    test ! -L "$rollback_authorization_hash_record"
record_command rollback_hash_record_metadata \
    test "$(stat -c '%U:%G:%a' "$rollback_authorization_hash_record" \
        2>/dev/null || true)" = root:root:600
record_command rollback_hash_record_content_exact \
    test "$(tr -d '\n' <"$rollback_authorization_hash_record" \
        2>/dev/null || true)" = "$expected_old_authorization_sha256"
record_command action17q_retry_backup_count_one \
    test "$action17q_retry_backup_count" -eq 1
record_command action17q_retry_stage_count_zero \
    test "$action17q_retry_stage_count" -eq 0
...
printf 'action_17q_b_value_reconcile_path_property_count=%s\n' \
    "$property_count"
printf 'action_17q_b_value_reconcile_path_property_names_sha256=%s\n' \
    "$property_names_sha256"
printf 'action_17q_b_value_reconcile_path_load_state=%s\n' \
    "$path_load_state"
printf 'action_17q_b_value_reconcile_path_active_state=%s\n' \
    "$path_active_state"
printf 'action_17q_b_value_reconcile_path_sub_state=%s\n' "$path_sub_state"
printf 'action_17q_b_value_reconcile_path_unit_file_state=%s\n' \
    "$path_unit_file_state"
printf 'action_17q_b_value_reconcile_path_fragment_path=%s\n' \
    "$path_fragment_path"
printf 'action_17q_b_value_reconcile_path_mainpid_present=%s\n' \
    "$path_mainpid_present"
printf 'action_17q_b_value_reconcile_path_nrestarts_present=%s\n' \
    "$path_nrestarts_present"
printf 'action_17q_b_value_action17q_backup_count=%s\n' \
    "$action17q_backup_count"
printf 'action_17q_b_value_action17q_retry_backup_count=%s\n' \
    "$action17q_retry_backup_count"
printf 'action_17q_b_value_action17q_retry_stage_count=%s\n' \
    "$action17q_retry_stage_count"
printf 'action_17q_b_value_rollback_directory=%s\n' "$rollback_directory"
printf 'action_17q_b_value_before_state_sha256=%s\n' \
    "$before_state_sha256"
printf 'action_17q_b_value_after_state_sha256=%s\n' \
    "$after_state_sha256"
printf 'action_17q_b_value_conclusion=%s\n' "$conclusion"
printf 'action_17q_b_assertion_count=%s\n' "$assertion_count"
printf 'action_17q_b_failed_assertion_count=%s\n' \
    "$failed_assertion_count"
printf 'action_17q_b_first_failure=%s\n' "$first_failure"
printf 'action_17q_b_helper_execution=false\n'
printf 'action_17q_b_release_mutation=false\n'
printf 'action_17q_b_authorization_mutation=false\n'
printf 'action_17q_b_lsyncd_reconciliation_activation=false\n'
printf 'action_17q_b_service_mutations=false\n'
printf 'action_17q_b_persistent_mutations=false\n'
printf 'action_17q_b_remote_complete=true\n'
