...
readonly prefix=action_20a_retry_probe
...
case "${1:-}" in
    ...
    --self-test)
        [[ $# -eq 1 ]] || exit 64
        self_test_root=$(mktemp -d /tmp/caddy-action20a-retry-probe-self.XXXXXX)
...
work_directory=$(mktemp -d /tmp/caddy-action20a-retry-probe.XXXXXX)
