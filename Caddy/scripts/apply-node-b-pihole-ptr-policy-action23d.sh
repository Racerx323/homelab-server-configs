...
readonly backup_dir=/var/backups/caddy-ha/action23d-node-b-pihole-ptr-policy
readonly transaction_file=/etc/pihole/.pihole-FTL.conf.action23d.new
...
address_count() {
    local action23d_family=$1
    local action23d_cidr=$2

    ip -o "-$action23d_family" address show dev "$interface" |
        awk -v wanted="$action23d_cidr" '$4 == wanted { count++ } END { print count + 0 }'
}
record_check() {
    local action23d_check_label=$1

    shift
    if [[ -n "${seen_checks[$action23d_check_label]+set}" ]]; then
        printf '%s_duplicate_check=%s\n' "$prefix" "$action23d_check_label" >&2
        return 1
    fi
    seen_checks[$action23d_check_label]=true
    if "$@"; then
        printf '%s_check_%s=true\n' "$prefix" "$action23d_check_label"
        return 0
    fi
...
run_dns_query() (
    ...
    local action23d_server=$1
    local action23d_port=$2
    local action23d_name=$3
    local action23d_type=$4

    if [[ "$action23d_type" == PTR ]]; then
        timeout 4 dig +time=2 +tries=1 +short -p "$action23d_port" \
            "@$action23d_server" -x "$action23d_name" 2>/dev/null
    else
        timeout 4 dig +time=2 +tries=1 +short -p "$action23d_port" \
            "@$action23d_server" "$action23d_name" "$action23d_type" 2>/dev/null
    fi
)
query_exact() {
    local action23d_server=$1
    ...
    local action23d_name=$3
    local action23d_type=$4
    local action23d_expected=$5
    local action23d_answer=

    action23d_answer=$(run_dns_query "$action23d_server" "$action23d_port" \
        "$action23d_name" "$action23d_type") || return 1
    action23d_answer=$(printf '%s\n' "$action23d_answer" | sed '/^$/d' | sort -u)
    [[ "$action23d_answer" == "$action23d_expected" ]]
}
https_probe() {
    local action23d_name=$1
    local action23d_address=$2

    curl -kfsS -o /dev/null --connect-timeout 3 --max-time 5 \
        --resolve "${action23d_name}:443:${action23d_address}" \
        "https://${action23d_name}/"
}
capture_pihole_version() {
    local action23d_version_output=$1

    "$pihole_cli" -v >"$action23d_version_output" 2>&1
}
validate_ftl_configuration() {
    "$ftl_binary" --test >/dev/null 2>&1
}
validate_candidate_delta() {
    local action23d_candidate=$1
    local action23d_reconstructed=$2

    [[ "$(grep -Fxc "$new_ptr_line" "$action23d_candidate" || true)" -eq 1 ]] || return 1
    [[ "$(grep -Fxc "$old_ptr_line" "$action23d_candidate" || true)" -eq 0 ]] || return 1
    awk -v old="$old_ptr_line" -v new="$new_ptr_line" '
        $0 == new { print old; changed++; next }
        { print }
        END { if (changed != 1) exit 42 }
    ' "$action23d_candidate" >"$action23d_reconstructed" || return 1
    [[ "$(file_hash "$action23d_reconstructed")" == "$before_ftl_sha256" ]]
}
validate_all_records() {
    local action23d_path=$1
    local action23d_server=$2
    local action23d_port=$3

    record_check "${action23d_path}_pihole_a" query_exact "$action23d_server" "$action23d_port" \
        pihole.local.theama.co A 10.1.0.55 || return 1
    record_check "${action23d_path}_pihole_aaaa" query_exact "$action23d_server" "$action23d_port" \
        pihole.local.theama.co AAAA fd36:5aa8:6971:1::55 || return 1
    record_check "${action23d_path}_node_a_a" query_exact "$action23d_server" "$action23d_port" \
        pihole0.local.theama.co A 10.1.0.53 || return 1
    record_check "${action23d_path}_node_a_aaaa" query_exact "$action23d_server" "$action23d_port" \
        pihole0.local.theama.co AAAA fd36:5aa8:6971:1::53 || return 1
    record_check "${action23d_path}_node_b_a" query_exact "$action23d_server" "$action23d_port" \
        pihole00.local.theama.co A 10.1.0.54 || return 1
    record_check "${action23d_path}_node_b_aaaa" query_exact "$action23d_server" "$action23d_port" \
        pihole00.local.theama.co AAAA fd36:5aa8:6971:1::54 || return 1
    record_check "${action23d_path}_pihole_ptr4" query_exact "$action23d_server" "$action23d_port" \
        10.1.0.55 PTR pihole.local.theama.co. || return 1
    record_check "${action23d_path}_pihole_ptr6" query_exact "$action23d_server" "$action23d_port" \
        fd36:5aa8:6971:1::55 PTR pihole.local.theama.co. || return 1
    record_check "${action23d_path}_node_a_ptr4" query_exact "$action23d_server" "$action23d_port" \
        10.1.0.53 PTR pihole0.local.theama.co. || return 1
    record_check "${action23d_path}_node_a_ptr6" query_exact "$action23d_server" "$action23d_port" \
        fd36:5aa8:6971:1::53 PTR pihole0.local.theama.co. || return 1
    record_check "${action23d_path}_node_b_ptr4" query_exact "$action23d_server" "$action23d_port" \
        10.1.0.54 PTR pihole00.local.theama.co. || return 1
    record_check "${action23d_path}_node_b_ptr6" query_exact "$action23d_server" "$action23d_port" \
        fd36:5aa8:6971:1::54 PTR pihole00.local.theama.co. || return 1
}
wait_for_ptr_passthrough() {
    local action23d_attempt

    for action23d_attempt in 1 2 3 4 5; do
        if query_exact 127.0.0.1 53 10.1.0.55 PTR pihole.local.theama.co.; then
            printf '%s_value_ptr_readiness_attempt=%s\n' "$prefix" "$action23d_attempt"
            return 0
        fi
        sleep 1
    done
...
emit_expected_checks() {
    local action23d_expected_command

    for action23d_expected_command in "${required_commands[@]}"; do
        printf 'command_%s_available\n' "${action23d_expected_command//-/_}"
    done
    printf '%s\n' \
...
emit_contract_transcript() {
    local action23d_contract_label
    local action23d_contract_count=0
    local action23d_before_hash=aaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaa
    local action23d_candidate_hash=bbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbb

    while IFS= read -r action23d_contract_label; do
        printf '%s_check_%s=true\n' "$prefix" "$action23d_contract_label"
        action23d_contract_count=$((action23d_contract_count + 1))
    done < <(emit_expected_checks)
    printf '%s_value_before_ftl_sha256=%s\n' "$prefix" "$action23d_before_hash"
    printf '%s_value_candidate_ftl_sha256=%s\n' "$prefix" "$action23d_candidate_hash"
    ...
    printf '%s_check_count=%s\n' "$prefix" "$action23d_contract_count"
...
self_test() {
    local action23d_test_root
    local action23d_test_live
    local action23d_test_candidate
    local action23d_test_reconstructed

    action23d_test_root=$(mktemp -d /tmp/caddy-action23d-self-test.XXXXXX) || return 1
    trap 'rm -rf -- "$action23d_test_root"' RETURN
    action23d_test_live=$action23d_test_root/live.conf
    action23d_test_candidate=$action23d_test_root/candidate.conf
    action23d_test_reconstructed=$action23d_test_root/reconstructed.conf
    printf 'alpha=true\n%s\nomega=true\n' "$old_ptr_line" >"$action23d_test_live"
    before_ftl_sha256=$(file_hash "$action23d_test_live")
    awk -v old="$old_ptr_line" -v new="$new_ptr_line" '
        $0 == old { print new; changed++; next }
        { print }
        END { if (changed != 1) exit 42 }
    ' "$action23d_test_live" >"$action23d_test_candidate" || return 1
    validate_candidate_delta "$action23d_test_candidate" "$action23d_test_reconstructed" || return 1
    [[ "$(emit_expected_checks | wc -l)" -eq "$(emit_expected_checks | LC_ALL=C sort -u | wc -l)" ]] || return 1
    printf '%s_self_test_complete=true\n' "$prefix"
}
rollback() {
    local action23d_original_status=$?
    local action23d_rollback_failed=false
    ...
    if [[ "$transaction_complete" == true ]]; then
        ...
        exit "$action23d_original_status"
    fi
    ...
    if [[ "$mutation_started" == true ]]; then
        if ! install -o pihole -g root -m 0664 "$backup_dir/pihole-FTL.conf.before" "$transaction_file"; then
            action23d_rollback_failed=true
        fi
    ...
    [[ -z "$stage_dir" || ! -d "$stage_dir" ]] || rm -rf -- "$stage_dir"
    set -e
    if [[ "$action23d_rollback_failed" == true ]]; then
...
trap rollback EXIT
for action23d_required_command in "${required_commands[@]}"; do
    record_check "command_${action23d_required_command//-/_}_available" \
        command -v "$action23d_required_command" || exit 1
done
...
pihole_version_file=$(mktemp /tmp/caddy-action23d-version.XXXXXX)
...
stage_dir=$(mktemp -d /run/caddy-action23d.XXXXXX)
