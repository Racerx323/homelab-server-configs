...
readonly prefix=action_20e_a_probe
...
expected_assertions() {
    printf '%s\n' \
        ...
        dry_run_status_zero dry_run_stdout_safe dry_run_stderr_safe \
...
capture_root=$(mktemp -d /run/caddy-action20e-a-probe.XXXXXX)
...
trap cleanup EXIT
readonly dry_stdout=$capture_root/tmpfiles-dry-run.stdout
readonly dry_stderr=$capture_root/tmpfiles-dry-run.stderr
touch "$dry_stdout" "$dry_stderr"
chmod 0600 "$dry_stdout" "$dry_stderr"
...
dry_status=0
systemd-tmpfiles --dry-run --create "$installed_config" >"$dry_stdout" 2>"$dry_stderr" || dry_status=$?
readonly dry_status
run_assertion dry_run_status_zero test "$dry_status" -eq 0
run_assertion dry_run_stdout_safe safe_stream "$dry_stdout"
run_assertion dry_run_stderr_safe safe_stream "$dry_stderr"
...
emit_status=0
emit_stream dry_run_stdout "$dry_stdout" || emit_status=$?
emit_stream dry_run_stderr "$dry_stderr" || emit_status=$?
