...
readonly driver="$script_dir/apply-node-b-dns-nss-correction-action17m.sh"
...
self_test() {
    ...
    printf 'action_17m_runner_self_test_complete=true\n'
}

contract_test() {
    grep -Fq "readonly host_alias=pihole00.local.theama.co" "$0"
    grep -Fq "readonly host_address=10.1.0.54" "$0"
    grep -Fq "sudo -n /bin/bash -c 'cd / &&" "$0"
    grep -Fq 'action_17m_acceptance=true' "$driver"
    grep -Fq 'action_17m_resolv_conf_mutation=false' "$driver"
    if grep -Eq '(^|[[:space:]])(rsync|scp|sftp)([[:space:]]|$)' "$driver"; then
        return 1
    fi
    printf 'action_17m_runner_contract_test_complete=true\n'
}
...
case "${1:-}" in
    ...
    --source-test)
        [[ $# -eq 1 ]]
        verify_source
        printf 'action_17m_runner_source_test_complete=true\n'
        exit 0
        ;;
    --contract-test)
        [[ $# -eq 1 ]]
...
archive="$work_dir/action17m.tar"
...
tar -C "$work_dir" -cf "$archive" \
    -C "$script_dir" apply-node-b-dns-nss-correction-action17m.sh \
...
remote_stage=\$(mktemp -d /run/caddy-action17m.XXXXXX) && \
trap '\\''rm -rf -- \"\$remote_stage\"'\\'' EXIT && \
tar -C \"\$remote_stage\" -xf - && \
chown root:root \"\$remote_stage\"/* && \
chmod 0700 \"\$remote_stage\" && \
chmod 0700 \"\$remote_stage/apply-node-b-dns-nss-correction-action17m.sh\" && \
chmod 0600 \"\$remote_stage/pihole-local-zone.conf\" && \
/bin/bash \"\$remote_stage/apply-node-b-dns-nss-correction-action17m.sh\" \
...
failed_check_count=$(
    grep -Ec '^action_17m_check_[A-Za-z0-9_]+=false$' "$transcript" || true
)
readonly failed_check_count
if [[ "$ssh_status" -eq 0 ]] &&
    grep -Fqx 'action_17m_acceptance=true' "$transcript" &&
    grep -Fqx 'action_17m_resolv_conf_mutation=false' "$transcript" &&
    [[ "$failed_check_count" -eq 0 ]]; then
    printf 'action_17m_ssh_status=0\n'
    printf 'action_17m_runner_acceptance=true\n'
    exit 0
fi

sed -n '1,240p' "$transcript" >&2
printf 'action_17m_ssh_status=%s\n' "$ssh_status" >&2
if grep -Fqx 'action_17m_rollback_complete=true' "$transcript"; then
    printf 'action_17m_runner_rollback_complete=true\n' >&2
elif grep -Fqx 'manual_intervention_required=true' "$transcript"; then
    printf 'action_17m_runner_manual_intervention_required=true\n' >&2
    exit 125
fi
printf 'action_17m_runner_acceptance=false\n' >&2
exit 1
