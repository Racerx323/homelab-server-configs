...
readonly prefix=action_28j_transaction
...
record_check() {
    local action28j_tx_check_label=$1
    shift
    if "$@" >/dev/null 2>&1; then
        printf '%s_check_%s=true\n' "$prefix" "$action28j_tx_check_label"
    else
        printf '%s_check_%s=false\n' "$prefix" "$action28j_tx_check_label"
        failed_check_count=$((failed_check_count + 1))
        [[ "$first_failure" != none ]] || first_failure=$action28j_tx_check_label
    fi
    return 0
}
safe_stream() {
    local action28j_tx_stream=$1
    [[ "$(wc -c <"$action28j_tx_stream")" -le 131072 ]] || return 1
    [[ "$(line_count "$action28j_tx_stream")" -le 1024 ]] || return 1
    ! LC_ALL=C grep -n '[^[:print:][:space:]]' "$action28j_tx_stream" >/dev/null || return 1
}
emit_stream() {
    local action28j_tx_label=$1
    local action28j_tx_stream=$2
    printf '%s_value_%s_bytes=%s\n' "$prefix" "$action28j_tx_label" "$(wc -c <"$action28j_tx_stream")"
    printf '%s_value_%s_lines=%s\n' "$prefix" "$action28j_tx_label" "$(line_count "$action28j_tx_stream")"
    printf '%s_value_%s_sha256=%s\n' "$prefix" "$action28j_tx_label" "$(file_hash "$action28j_tx_stream")"
    if [[ -s "$action28j_tx_stream" ]]; then
        printf '%s_value_%s_content_begin\n' "$prefix" "$action28j_tx_label"
        sed "s/^/${prefix}_value_${action28j_tx_label}_content=/" "$action28j_tx_stream"
...
address_count() {
    local action28j_tx_family=$1
    local action28j_tx_cidr=$2
    address_query "$action28j_tx_family" | awk -v expected="$action28j_tx_cidr" '$4 == expected { count++ } END { print count + 0 }'
}
ready() {
...
capture_directory=$(mktemp -d /run/caddy-action28j-transaction.XXXXXX)
