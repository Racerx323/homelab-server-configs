...
case "${1:-}" in
    ...
    --contract-test)
        [[ $# -eq 1 ]]
        verify_sources
        contract_directory=$(mktemp -d /tmp/caddy-action18c-contract.XXXXXX)
        trap 'rm -rf -- "$contract_directory"' EXIT
        write_fixture "$contract_directory/valid" action_18c_node_b preflight
        validate_transcript valid action_18c_node_b \
            "$contract_directory/valid" preflight 0 >/dev/null
        [[ "$checks_failed" -eq 0 ]]
        printf 'action_18c_false_negative_valid_transcript_accepted=true\n'
        ...
        validate_transcript false_check action_18c_node_b \
        ...
        printf 'action_18c_node_b_value_phase=preflight\n' \
            >>"$contract_directory/duplicate"
        checks_total=0 checks_passed=0 checks_failed=0 first_failure=none
        validate_transcript duplicate_value action_18c_node_b \
...
work_directory=$(mktemp -d /tmp/caddy-action18c-runner.XXXXXX)
readonly work_directory
cleanup() { rm -rf -- "$work_directory"; }
trap cleanup EXIT
for stream_path in node-a-pre.out node-a-pre.err node-b-pre.out node-b-pre.err \
    node-b-transfer.out node-b-transfer.err node-a-complete.out node-a-complete.err; do
    : >"$work_directory/$stream_path"
    chmod 0600 "$work_directory/$stream_path"
done

node_a_pre_status=0
run_remote "$node_a_alias" "$node_a_target" --preflight "$inspector" \
...
validate_transcript node_a_preflight action_18c_node_a \
    "$work_directory/node-a-pre.out" preflight "$node_a_pre_status"

node_b_pre_status=not_run
if [[ "$checks_failed" -eq 0 ]]; then
    node_b_pre_status=0
    run_remote "$node_b_alias" "$node_b_target" --preflight "$driver" \
    ...
    validate_transcript node_b_preflight action_18c_node_b \
        "$work_directory/node-b-pre.out" preflight "$node_b_pre_status"
fi

node_b_transfer_status=not_run
node_a_complete_status=not_run
revision=unavailable
parent_revision=unavailable
manifest_sha256=unavailable
if [[ "$checks_failed" -eq 0 ]]; then
    node_b_transfer_status=0
    run_remote "$node_b_alias" "$node_b_target" --transfer "$driver" \
        "$work_directory/node-b-transfer.out" "$work_directory/node-b-transfer.err" \
        node_b_transfer_status
    emit_stream node_b_transfer_stdout "$work_directory/node-b-transfer.out"
    emit_stream node_b_transfer_stderr "$work_directory/node-b-transfer.err"
    validate_transcript node_b_transfer action_18c_node_b \
        "$work_directory/node-b-transfer.out" transfer "$node_b_transfer_status"
    revision=$(value_for action_18c_node_b_value_revision \
        "$work_directory/node-b-transfer.out") || revision=unavailable
    parent_revision=$(value_for action_18c_node_b_value_parent_revision \
        "$work_directory/node-b-transfer.out") || parent_revision=unavailable
    manifest_sha256=$(value_for action_18c_node_b_value_manifest_sha256 \
        "$work_directory/node-b-transfer.out") || manifest_sha256=unavailable
...
if [[ "$checks_failed" -eq 0 ]]; then
    ...
    run_remote "$node_a_alias" "$node_a_target" \
        "--complete $revision $parent_revision $manifest_sha256" "$inspector" \
        "$work_directory/node-a-complete.out" "$work_directory/node-a-complete.err" \
        node_a_complete_status
    emit_stream node_a_complete_stdout "$work_directory/node-a-complete.out"
    emit_stream node_a_complete_stderr "$work_directory/node-a-complete.err"
    validate_transcript node_a_complete action_18c_node_a \
        "$work_directory/node-a-complete.out" complete "$node_a_complete_status"
fi
