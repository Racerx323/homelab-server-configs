...
readonly prefix=action_28_outer
...
readonly runner="$script_directory/run-node-a-to-node-b-release-transfer-action28.sh"
readonly publisher="$script_directory/publish-release-v2.sh"
readonly regression="$caddy_root/tests/action28-node-a-to-node-b-release-transfer-regression.sh"
...
run_local_gates
work_directory=$(mktemp -d /tmp/caddy-action28-outer.XXXXXX)
