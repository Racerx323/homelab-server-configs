...
validate_fragment_parser() {
    ...
    parser_root=$(mktemp -d /tmp/caddy-action19a-b-parser.XXXXXX) || return 1
...
case "${1:-}" in
    ...
    --self-test)
        [[ $# -eq 1 ]]
        expected_root=$(mktemp -d /tmp/caddy-action19a-b-expected.XXXXXX)
...
work_directory=$(mktemp -d /tmp/caddy-action19a-b-inspector.XXXXXX)
