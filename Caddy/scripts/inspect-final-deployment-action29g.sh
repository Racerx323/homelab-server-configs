...
readonly prefix=action_29g_remote
readonly schema_version=action29g-node-snapshot-v2
...
first_failure=none
action29g_remote_versions=
action29g_remote_web_body=
action29g_remote_manifest_stdout=
action29g_remote_manifest_stderr=
action29g_remote_manifest_status=0
...
action29b_backup_valid() {
    local action29g_remote_backup=${1:-$expected_action29b_backup}
    local action29g_remote_owner=${2:-root}
    local action29g_remote_group=${3:-root}
    local action29g_remote_before_ftl=${4:-$action29b_before_ftl_sha256}
    local action29g_remote_before_domain=${5:-$action29b_before_domain_sha256}
    local action29g_remote_expected_manifest

    [[ -d "$action29g_remote_backup" && ! -L "$action29g_remote_backup" ]] || return 1
    [[ "$(stat -c '%U:%G:%a' "$action29g_remote_backup")" = "$action29g_remote_owner:$action29g_remote_group:700" ]] || return 1
    metadata_owner_exact "$action29g_remote_backup/manifest" \
        "$action29g_remote_owner" "$action29g_remote_group" 600 || return 1
    metadata_owner_exact "$action29g_remote_backup/pihole-FTL.conf.before" \
        "$action29g_remote_owner" "$action29g_remote_group" 600 || return 1
    metadata_owner_exact "$action29g_remote_backup/local.theama.co.conf.before" \
        "$action29g_remote_owner" "$action29g_remote_group" 600 || return 1
    metadata_owner_exact "$action29g_remote_backup/transaction.complete" \
        "$action29g_remote_owner" "$action29g_remote_group" 600 || return 1
    [[ "$(file_hash "$action29g_remote_backup/pihole-FTL.conf.before")" = "$action29g_remote_before_ftl" ]] || return 1
    [[ "$(file_hash "$action29g_remote_backup/local.theama.co.conf.before")" = "$action29g_remote_before_domain" ]] || return 1
    [[ "$(<"$action29g_remote_backup/transaction.complete")" = committed ]] || return 1
    action29g_remote_expected_manifest=$(printf '%s\n' \
        'action=29b' "role=${node_token//_/-}" \
        "before_ftl_sha256=$action29g_remote_before_ftl" \
        "before_domain_sha256=$action29g_remote_before_domain" \
        "target_ftl_sha256=$pihole_ftl_target_sha256" \
        "target_domain_sha256=$pihole_domain_target_sha256") || return 1
    [[ "$(<"$action29g_remote_backup/manifest")" = "$action29g_remote_expected_manifest" ]]
}
pihole_ftl_v5_semantics_exact() {
    local action29g_remote_file=${1:-$pihole_ftl}
    local action29g_remote_observed
    local action29g_remote_expected

    action29g_remote_observed=$(grep -Ev '^[[:space:]]*(#|$)' "$action29g_remote_file") || return 1
    action29g_remote_expected=$'PRIVACYLEVEL=0\nRATE_LIMIT=1000/60\nRESOLVE_IPV6=YES\nPIHOLE_PTR=NONE'
    [[ "$action29g_remote_observed" = "$action29g_remote_expected" ]]
}
pihole_domain_v5_semantics_exact() {
    local action29g_remote_file=${1:-$pihole_domain}
    local action29g_remote_observed
    local action29g_remote_expected

    action29g_remote_observed=$(grep -Ev '^[[:space:]]*(#|$)' "$action29g_remote_file") || return 1
    action29g_remote_expected=$'domain=local.theama.co\nserver=/local.theama.co/127.0.0.1#5335\nserver=/local.theama.co/::1#5335\nserver=/0.1.10.in-addr.arpa/127.0.0.1#5335\nserver=/0.1.10.in-addr.arpa/::1#5335\nserver=/1.1.10.in-addr.arpa/127.0.0.1#5335\nserver=/1.1.10.in-addr.arpa/::1#5335\nserver=/2.1.10.in-addr.arpa/127.0.0.1#5335\nserver=/2.1.10.in-addr.arpa/::1#5335\nserver=/3.1.10.in-addr.arpa/127.0.0.1#5335\nserver=/3.1.10.in-addr.arpa/::1#5335\nserver=/1.0.0.0.1.7.9.6.8.a.a.5.6.3.d.f.ip6.arpa/127.0.0.1#5335\nserver=/1.0.0.0.1.7.9.6.8.a.a.5.6.3.d.f.ip6.arpa/::1#5335'
    [[ "$action29g_remote_observed" = "$action29g_remote_expected" ]]
}
endpoint_specs() {
    printf '%s\n' \
...
probe_web_endpoint() {
    local action29g_remote_label=$1
    local action29g_remote_fqdn=$2
    local action29g_remote_resolve=$3
    local action29g_remote_expected_ip=$4
    local action29g_remote_status=0
    local action29g_remote_metadata
    local action29g_remote_http_code
    local action29g_remote_effective_url
    local action29g_remote_remote_ip
    local action29g_remote_redirects
    local action29g_remote_body_bytes
    local action29g_remote_body_sha256

    : >"$action29g_remote_web_body" || return 1
    action29g_remote_metadata=$(/usr/bin/curl --noproxy '*' --insecure --silent --show-error \
        --http1.1 --location --max-redirs 3 --proto-redir '=https' \
        --connect-timeout 3 --max-time 10 \
        --resolve "${action29g_remote_fqdn}:443:${action29g_remote_resolve}" \
        --output "$action29g_remote_web_body" \
        --write-out '%{http_code}|%{url_effective}|%{remote_ip}|%{num_redirects}' \
        "https://${action29g_remote_fqdn}/admin/") || action29g_remote_status=$?
    IFS='|' read -r action29g_remote_http_code action29g_remote_effective_url \
        action29g_remote_remote_ip action29g_remote_redirects <<<"$action29g_remote_metadata"
    action29g_remote_body_bytes=$(wc -c <"$action29g_remote_web_body") || return 1
    action29g_remote_body_sha256=$(file_hash "$action29g_remote_web_body") || return 1
    printf '%s_%s_observed_web_%s_status=%s\n' "$prefix" "$node_token" "$action29g_remote_label" "$action29g_remote_status"
    printf '%s_%s_observed_web_%s_http_code=%s\n' "$prefix" "$node_token" "$action29g_remote_label" "$action29g_remote_http_code"
    printf '%s_%s_observed_web_%s_effective_url=%s\n' "$prefix" "$node_token" "$action29g_remote_label" "$action29g_remote_effective_url"
    printf '%s_%s_observed_web_%s_remote_ip=%s\n' "$prefix" "$node_token" "$action29g_remote_label" "$action29g_remote_remote_ip"
    printf '%s_%s_observed_web_%s_redirects=%s\n' "$prefix" "$node_token" "$action29g_remote_label" "$action29g_remote_redirects"
    printf '%s_%s_observed_web_%s_body_bytes=%s\n' "$prefix" "$node_token" "$action29g_remote_label" "$action29g_remote_body_bytes"
    printf '%s_%s_observed_web_%s_body_sha256=%s\n' "$prefix" "$node_token" "$action29g_remote_label" "$action29g_remote_body_sha256"
    check "web_${action29g_remote_label}_status" test "$action29g_remote_status" -eq 0 || return 1
    check "web_${action29g_remote_label}_http_200" test "$action29g_remote_http_code" = 200 || return 1
    check "web_${action29g_remote_label}_effective_url" test "$action29g_remote_effective_url" = \
        "https://${action29g_remote_fqdn}/admin/login.php" || return 1
    check "web_${action29g_remote_label}_remote_ip" test "$action29g_remote_remote_ip" = \
        "$action29g_remote_expected_ip" || return 1
    check "web_${action29g_remote_label}_redirect_once" test "$action29g_remote_redirects" = 1 || return 1
    check "web_${action29g_remote_label}_body_nonempty" test "$action29g_remote_body_bytes" -gt 0 || return 1
    check "web_${action29g_remote_label}_body_bounded" test "$action29g_remote_body_bytes" -le \
        "$maximum_web_body_bytes" || return 1
    check "web_${action29g_remote_label}_body_pihole" grep -Fqi Pi-hole \
        "$action29g_remote_web_body" || return 1
}
manifest_paths_safe() {
    local action29g_manifest_path=$1
    ...
    awk '
        ...
        END { exit bad ? 1 : 0 }
    ' "$action29g_manifest_path"
}
manifest_file_set_matches() {
    local action29g_release_path=$1

    cmp -s \
        <(awk '{ print substr($0, 67) }' "$action29g_release_path/manifest.sha256" | LC_ALL=C sort -u) \
        <(
            cd -- "$action29g_release_path" || exit 1
            find . -type f \
...
safe_manifest_stream() {
    local action29g_manifest_stream=$1

    [[ "$(wc -c <"$action29g_manifest_stream")" -le "$maximum_manifest_stream_bytes" ]] || return 1
    [[ "$(awk 'END { print NR }' "$action29g_manifest_stream")" -le "$maximum_manifest_stream_lines" ]] || return 1
    iconv -f UTF-8 -t UTF-8 "$action29g_manifest_stream" >/dev/null 2>&1 || return 1
    ! LC_ALL=C.UTF-8 grep -n '[^[:print:][:space:]]' "$action29g_manifest_stream" >/dev/null || return 1
    ! grep -Eqi 'BEGIN [A-Z ]*PRIVATE KEY|Authorization:[[:space:]]*Bearer|WEBPASSWORD' \
        "$action29g_manifest_stream"
}
emit_manifest_stream() {
    local action29g_manifest_label=$1
    local action29g_manifest_stream=$2

    printf '%s_%s_manifest_%s_bytes=%s\n' "$prefix" "$node_token" \
        "$action29g_manifest_label" "$(wc -c <"$action29g_manifest_stream")"
    printf '%s_%s_manifest_%s_lines=%s\n' "$prefix" "$node_token" \
        "$action29g_manifest_label" "$(awk 'END { print NR }' "$action29g_manifest_stream")"
    printf '%s_%s_manifest_%s_sha256=%s\n' "$prefix" "$node_token" \
        "$action29g_manifest_label" "$(file_hash "$action29g_manifest_stream")"
    if ! safe_manifest_stream "$action29g_manifest_stream"; then
        printf '%s_%s_manifest_%s_classification=unsafe_retained\n' \
            "$prefix" "$node_token" "$action29g_manifest_label" >&2
        return 97
    fi
    printf '%s_%s_manifest_%s_classification=bounded_safe\n' \
        "$prefix" "$node_token" "$action29g_manifest_label"
    if [[ -s "$action29g_manifest_stream" ]]; then
        printf '%s_%s_manifest_%s_begin\n' "$prefix" "$node_token" "$action29g_manifest_label"
        cat -- "$action29g_manifest_stream"
...
capture_manifest_hash_check() {
    ...
    : >"$action29g_remote_manifest_stdout" || return 1
    : >"$action29g_remote_manifest_stderr" || return 1
    action29g_remote_manifest_status=0
    (
        cd -- "$action29g_release_path" || exit 1
        sha256sum --strict --check manifest.sha256
    ) >"$action29g_remote_manifest_stdout" \
        2>"$action29g_remote_manifest_stderr" || action29g_remote_manifest_status=$?
    printf '%s_%s_manifest_hash_check_status=%s\n' "$prefix" "$node_token" \
        "$action29g_remote_manifest_status"
    emit_manifest_stream hash_check_stdout "$action29g_remote_manifest_stdout" || return $?
    emit_manifest_stream hash_check_stderr "$action29g_remote_manifest_stderr" || return $?
}
...
expected_checks() {
    ...
    while IFS='|' read -r action29g_remote_endpoint _ _ _; do
        printf '%s\n' \
            "web_${action29g_remote_endpoint}_status" \
            "web_${action29g_remote_endpoint}_http_200" \
            "web_${action29g_remote_endpoint}_effective_url" \
            "web_${action29g_remote_endpoint}_remote_ip" \
            "web_${action29g_remote_endpoint}_redirect_once" \
            "web_${action29g_remote_endpoint}_body_nonempty" \
            "web_${action29g_remote_endpoint}_body_bounded" \
            "web_${action29g_remote_endpoint}_body_pihole"
    done < <(endpoint_specs)
}
state_snapshot() {
...
cleanup() {
    local action29g_remote_cleanup_status=$?

    trap - EXIT INT TERM
    if [[ -n ${action29g_remote_versions:-} ]]; then
        rm -f -- "$action29g_remote_versions"
    fi
    if [[ -n ${action29g_remote_web_body:-} ]]; then
        rm -f -- "$action29g_remote_web_body"
    fi
    if [[ -n ${action29g_remote_manifest_stdout:-} ]]; then
        rm -f -- "$action29g_remote_manifest_stdout"
    fi
    if [[ -n ${action29g_remote_manifest_stderr:-} ]]; then
        rm -f -- "$action29g_remote_manifest_stderr"
    fi
    exit "$action29g_remote_cleanup_status"
}
run_inspection() {
    ...
    local action29g_remote_observed_ftl_sha256
    local action29g_remote_observed_domain_sha256
    local action29g_remote_observed_manifest_sha256
    local action29g_remote_manifest_paths_status=0
    local action29g_remote_manifest_file_set_status=0
    local action29g_remote_endpoint
    local action29g_remote_fqdn
    local action29g_remote_resolve
    local action29g_remote_expected_ip

    trap cleanup EXIT INT TERM
    action29_remote_started_ns=$(date +%s%N) || return 1
    action29g_remote_versions=$(mktemp /tmp/action29g-versions.XXXXXX) || return 1
    action29g_remote_web_body=$(mktemp /tmp/action29g-web-body.XXXXXX) || return 1
    action29g_remote_manifest_stdout=$(mktemp /tmp/action29g-manifest-stdout.XXXXXX) || return 1
    action29g_remote_manifest_stderr=$(mktemp /tmp/action29g-manifest-stderr.XXXXXX) || return 1
    ...
    action29g_remote_observed_ftl_sha256=$(file_hash "$pihole_ftl") || return 1
    printf '%s_%s_expected_pihole_ftl_sha256=%s\n' "$prefix" "$node_token" "$pihole_ftl_target_sha256"
    printf '%s_%s_observed_pihole_ftl_sha256=%s\n' "$prefix" "$node_token" "$action29g_remote_observed_ftl_sha256"
    check pihole_ftl_hash_exact test "$action29g_remote_observed_ftl_sha256" = "$pihole_ftl_target_sha256" || return 1
    ...
    action29g_remote_observed_domain_sha256=$(file_hash "$pihole_domain") || return 1
    printf '%s_%s_expected_pihole_domain_sha256=%s\n' "$prefix" "$node_token" "$pihole_domain_target_sha256"
    printf '%s_%s_observed_pihole_domain_sha256=%s\n' "$prefix" "$node_token" "$action29g_remote_observed_domain_sha256"
    check pihole_domain_hash_exact test "$action29g_remote_observed_domain_sha256" = "$pihole_domain_target_sha256" || return 1
    ...
    if [[ "$action29g_remote_manifest_paths_status" -eq 0 ]]; then
        ...
        emit_manifest_stream hash_check_stdout "$action29g_remote_manifest_stdout" || return $?
        emit_manifest_stream hash_check_stderr "$action29g_remote_manifest_stderr" || return $?
        action29g_remote_manifest_status=97
    fi
    check current_manifest_identity_exact test "$action29g_remote_observed_manifest_sha256" = \
        "$expected_payload_sha256" || return 1
    check current_manifest_paths_safe test "$action29g_remote_manifest_paths_status" -eq 0 || return 1
    check current_manifest_file_set_exact test "$action29g_remote_manifest_file_set_status" -eq 0 || return 1
    check current_manifest_hashes_valid test "$action29g_remote_manifest_status" -eq 0 || return 1
    ...
    while IFS='|' read -r action29g_remote_endpoint action29g_remote_fqdn \
        action29g_remote_resolve action29g_remote_expected_ip; do
        probe_web_endpoint "$action29g_remote_endpoint" "$action29g_remote_fqdn" \
            "$action29g_remote_resolve" "$action29g_remote_expected_ip" || return 1
    done < <(endpoint_specs)
    version_inventory >"$action29g_remote_versions" 2>&1 || action29_remote_version_status=$?
    check version_inventory_status test "$action29_remote_version_status" -eq 0 || return 1
    check version_inventory_nonempty test -s "$action29g_remote_versions" || return 1
    check version_inventory_safe safe_version_inventory "$action29g_remote_versions" || return 1
    ...
    sed "s/^/${prefix}_${node_token}_observed_version=/" "$action29g_remote_versions"
...
case "${1:-}" in
    ...
    --capture-manifest-fixture)
        configure_node "${2:-}" || exit $?
        action29g_remote_manifest_stdout=$(mktemp /tmp/action29g-fixture-stdout.XXXXXX) || exit 1
        action29g_remote_manifest_stderr=$(mktemp /tmp/action29g-fixture-stderr.XXXXXX) || exit 1
        trap cleanup EXIT INT TERM
