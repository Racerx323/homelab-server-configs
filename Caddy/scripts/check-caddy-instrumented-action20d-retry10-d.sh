...
case "${1:-}" in
    --self-test)
        [[ $# -eq 1 ]] || exit 64
        command -v date logger sha256sum systemctl caddy curl >/dev/null
        printf 'caddy_ha_health_instrumentation_self_test_complete=true\n'
        exit 0
        ;;
    --regression)
        [[ $# -eq 2 ]] || exit 64
...
if ! health_run_stage validation caddy validate \
