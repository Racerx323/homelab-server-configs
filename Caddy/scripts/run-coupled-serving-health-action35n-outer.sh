...
readonly prefix=action_35_n_outer
readonly transaction_sha256=1a3ef5c67aae2a57bcd20cce26c1fa8be0e97cc230fee3caef72c2cd2f2da2f8
...
safe_capture() {
    local action35n_path=$1

    regular_file "$action35n_path"
    [[ "$(stat -c '%s' "$action35n_path")" -le "$max_stream_bytes" ]]
    iconv -f UTF-8 -t UTF-8 "$action35n_path" >/dev/null
    ! LC_ALL=C grep -Paq '[\x00-\x08\x0b\x0c\x0e-\x1f\x7f]' "$action35n_path"
}

capture() {
    local action35n_label=$1
    shift
    local action35n_stdout=$workstation_evidence/$action35n_label.stdout
    local action35n_stderr=$workstation_evidence/$action35n_label.stderr
    local action35n_status=$workstation_evidence/$action35n_label.status
    ...
    safe_capture "$action35n_stdout"
    safe_capture "$action35n_stderr"
    return "$action35n_rc"
}

build_payload() {
    local action35n_repository action35n_source action35n_target action35n_mode
    local action35n_hash action35n_lifecycle action35n_source_path action35n_destination
    ...
    while IFS=$'\t' read -r action35n_repository action35n_source action35n_target \
        action35n_mode action35n_hash action35n_lifecycle; do
        [[ "$action35n_repository" = '# repository' ]] && continue
        [[ "$action35n_mode" =~ ^0[0-7]{3}$ && "$action35n_lifecycle" = production-candidate ]]
        [[ "$action35n_target" = /* ]]
        action35n_source_path=${workspace_root}/$action35n_repository/$action35n_source
        regular_file "$action35n_source_path"
        [[ "$(sha256sum "$action35n_source_path" | awk '{ print $1 }')" = "$action35n_hash" ]]
        action35n_destination=$payload_stage/repositories/$action35n_repository/$action35n_source
        install -d -m 0700 "$(dirname -- "$action35n_destination")"
        install -m 0600 "$action35n_source_path" "$action35n_destination"
...
readonly archive=$2
case "$remote_root" in /tmp/caddy-action35n-*) ;; *) exit 64 ;; esac
case "$archive" in /tmp/caddy-action35n-*.tar) ;; *) exit 64 ;; esac
...
case "$evidence_root" in /tmp/caddy-action35n-*) ;; *) exit 64 ;; esac
...
ssh_stream() {
    local action35n_host=$1
    local action35n_program=$2
    shift 2
    "$ssh_command" "$action35n_host" \
        "cd / && sudo -n /bin/bash -s --$(printf ' %q' "$@")" <"$action35n_program"
}

upload_payload() {
    local action35n_role=$1
    local action35n_host=$2
    local action35n_remote_root=$3
    local action35n_remote_archive=$4
    local action35n_hash

    action35n_hash=$(awk '{ print $1 }' "$workstation_evidence/payload.sha256")
    capture "$action35n_role-upload-prepare" ssh_stream "$action35n_host" \
        "$prepare_program" "$action35n_remote_root" "$action35n_remote_archive"
    capture "$action35n_role-upload-copy" "$scp_command" -p -- \
        "$payload_archive" "$action35n_host:$action35n_remote_archive"
    capture "$action35n_role-upload-accept" ssh_stream "$action35n_host" \
        "$accept_program" "$action35n_remote_root" "$action35n_remote_archive" \
        "$action35n_hash"
}
...
remote_transaction() {
    ...
    capture "$action35n_label" ssh_stream "$action35n_host" "$transaction" "$@"
}

readback() {
    local action35n_role=$1
    local action35n_host=$2
    local action35n_remote_evidence=$3

    capture "$action35n_role-readback" ssh_stream "$action35n_host" \
        "$readback_program" "$action35n_remote_evidence"
}
...
cleanup_remote() {
    ...
    capture "$action35n_role-disposition" ssh_stream "$action35n_host" \
        "$disposition_program" "$action35n_remote_root" "$action35n_remote_archive"
}

run_live() {
    local action35n_node_b_mutated=false
    local action35n_node_a_mutated=false
    local action35n_failure=0
    local action35n_phase_status=0
    ...
    if upload_payload node-b "$node_b_host" "$node_b_payload" "$node_b_archive" &&
        ...
        return "$action35n_phase_status"
    fi

    action35n_node_b_mutated=true
    ...
    if [[ "$action35n_failure" -eq 0 ]]; then
    ...
    if ! remote_transaction node-a-journal-capture "$node_a_host" journal-capture node-a \
        "$node_a_payload" "$node_a_evidence"; then
        [[ "$action35n_failure" -ne 0 ]] || action35n_failure=1
    fi
    ...
    if [[ "$action35n_failure" -ne 0 ]]; then
        if [[ "$action35n_node_a_mutated" = true ]]; then
            remote_transaction node-a-rollback "$node_a_host" rollback node-a \
                "$node_a_payload" "$node_a_evidence" || exit 125
        fi
        if [[ "$action35n_node_b_mutated" = true ]]; then
...
production_path_test() {
    ...
    local action35n_remote_base action35n_raw action35n_decision action35n_observed
    local action35n_status action35n_role action35n_remote_evidence action35n_test_id
    ...
    action35n_remote_base=$(mktemp -d /tmp/caddy-action35n-production-path.XXXXXX)
    action35n_test_id=${action35n_remote_base##*.}
    test_remote_base=$action35n_remote_base
    export ACTION35N_TEST_REMOTE_BASE=$test_remote_base
...
[[ "$1" = -n ]]
shift
export ACTION35N_PRODUCTION_PATH_TEST=1
exec "$@"
SUDO
...
set -Eeuo pipefail
[[ "$1" = -u && "$3" = -- ]]
printf '%s\n' "$2" >>"$ACTION35N_TEST_REMOTE_BASE/runuser.identities"
shift 3
exec "$@"
RUNUSER
...
#!/usr/bin/env bash
set -Eeuo pipefail
printf '%s\n' "$*" >>"$ACTION35N_TEST_REMOTE_BASE/systemctl.calls"
case "$1" in
    is-active) [[ "$2" = --quiet ]] ;;
    start | stop) [[ $# -eq 2 ]] ;;
...
set -Eeuo pipefail
case " $* " in
    *' --show-cursor '*) printf '%s\n' '-- cursor: s=action35n-production-path' ;;
    *' --after-cursor '*) printf '%s\n' 'test-host caddy.service: action35n production-path event' ;;
    *) exit 64 ;;
esac
JOURNALCTL
    cat >"$ssh_command" <<'SSH'
...
shift
command="$*"
printf '%s\t%s\n' "$host" "$command" >>"$ACTION35N_TEST_REMOTE_BASE/ssh.calls"
PATH="$ACTION35N_TEST_REMOTE_BASE/bin:/usr/bin:/bin" /bin/bash -c "$command"
SSH
    cat >"$scp_command" <<'SCP'
#!/usr/bin/env bash
set -Eeuo pipefail
...
source=$3
target=${4#*:}
printf '%s\t%s\n' "$source" "$target" >>"$ACTION35N_TEST_REMOTE_BASE/scp.calls"
...
SCP
    ...
    node_a_payload=/tmp/caddy-action35n-test-$action35n_test_id-node-a
    node_b_payload=/tmp/caddy-action35n-test-$action35n_test_id-node-b
    ...
    install -d -m 0700 "$retained_candidate" "$retained_root/incoming/node-b" \
    ...
    : >"$retained_candidate/.complete"
    : >"$retained_candidate/.finalize-request"
    ...
    ACTION35N_INCOMING_ROOT=$retained_root/incoming \
        ACTION35N_OUTGOING_ROOT=$retained_root/outgoing \
        ACTION35N_QUARANTINE_ROOT=$retained_root/quarantine \
        ACTION35N_RELEASES_ROOT=$retained_root/releases \
        ACTION35N_RETAINED_RELEASE_MANIFEST_SHA256=$retained_release_hash \
        ACTION35N_RETAINED_PAYLOAD_MANIFEST_SHA256=$retained_payload_hash \
    ...
    ACTION35N_INCOMING_ROOT=$retained_root/incoming \
        ...
        ACTION35N_RETAINED_RELEASE_MANIFEST_SHA256=$retained_release_hash \
        ACTION35N_RETAINED_PAYLOAD_MANIFEST_SHA256=$retained_payload_hash \
        ACTION35N_SYSTEMCTL_COMMAND=$test_remote_base/bin/systemctl \
        remote_transaction node-b-retained-disposition-producer "$node_b_host" \
        retained-disposition node-b "$node_b_payload" "$node_b_evidence"
    ACTION35N_INCOMING_ROOT=$retained_root/incoming \
        ACTION35N_OUTGOING_ROOT=$retained_root/outgoing \
        ACTION35N_QUARANTINE_ROOT=$retained_root/quarantine \
        ACTION35N_RELEASES_ROOT=$retained_root/releases \
        ...
        ACTION35N_RETAINED_PAYLOAD_MANIFEST_SHA256=$retained_payload_hash \
        ACTION35N_SYSTEMCTL_COMMAND=$test_remote_base/bin/systemctl \
        remote_transaction node-b-retained-rollback-producer "$node_b_host" retained-rollback \
        node-b "$node_b_payload" "$node_b_evidence"
    [[ -d "$retained_candidate" && ! -e "$node_b_evidence/retained-incoming" ]]
    ACTION35N_ENVIRONMENT_FILE=$test_remote_base/environment-node-a \
        ACTION35N_KEEPALIVED_COMMAND=$test_remote_base/bin/keepalived \
        ACTION35N_UNBOUND_CHECKCONF_COMMAND=$test_remote_base/bin/unbound-checkconf \
        ACTION35N_RUNUSER_COMMAND=$test_remote_base/bin/runuser \
        ACTION35N_SYSTEMCTL_COMMAND=$test_remote_base/bin/systemctl \
        ACTION35N_DNS_DIG_COMMAND=$test_remote_base/bin/dig \
        ACTION35N_CURL_COMMAND=$test_remote_base/bin/curl \
        ACTION35N_SS_COMMAND=$test_remote_base/bin/ss \
        remote_transaction node-a-candidate-producer "$node_a_host" candidate-check \
        node-a "$node_a_payload" "$node_a_evidence"
    ACTION35N_ENVIRONMENT_FILE=$test_remote_base/environment-node-b \
        ACTION35N_KEEPALIVED_COMMAND=$test_remote_base/bin/keepalived \
        ACTION35N_UNBOUND_CHECKCONF_COMMAND=$test_remote_base/bin/unbound-checkconf \
        ACTION35N_RUNUSER_COMMAND=$test_remote_base/bin/runuser \
        ACTION35N_SYSTEMCTL_COMMAND=$test_remote_base/bin/systemctl \
        ...
        node-b "$node_b_payload" "$node_b_evidence"
    ACTION35N_JOURNALCTL_COMMAND=$test_remote_base/bin/journalctl \
    ...
    ACTION35N_TARGET_ROOT=$test_remote_base/node-b-root \
        ACTION35N_SYSTEMCTL_COMMAND=$test_remote_base/bin/systemctl \
        ACTION35N_UNBOUND_CHECKCONF_COMMAND=$test_remote_base/bin/unbound-checkconf \
        remote_transaction node-b-install-producer "$node_b_host" install node-b \
        "$node_b_payload" "$node_b_evidence"
    ACTION35N_TARGET_ROOT=$test_remote_base/node-a-root \
        ACTION35N_SYSTEMCTL_COMMAND=$test_remote_base/bin/systemctl \
        ACTION35N_UNBOUND_CHECKCONF_COMMAND=$test_remote_base/bin/unbound-checkconf \
    ...
    action35n_raw=$action35n_test_root/raw/outer-preflight.txt
    action35n_decision=$action35n_test_root/decisions/outer-preflight.tsv
    cat "$test_remote_base/ssh.calls" "$test_remote_base/scp.calls" \
        ...
        >"$action35n_raw"
    action35n_observed=$(wc -l <"$action35n_raw")
    write_decision outer-preflight reach 0 "$action35n_observed" \
        "$action35n_observed" "$action35n_raw" "$action35n_decision"

    for action35n_role in node-a node-b; do
        if [[ "$action35n_role" = node-a ]]; then
            action35n_remote_evidence=$node_a_evidence
        else
            action35n_remote_evidence=$node_b_evidence
        fi
        action35n_raw=$action35n_test_root/raw/evidence-readback-$action35n_role-success.txt
        action35n_decision=$action35n_test_root/decisions/evidence-readback-$action35n_role-success.tsv
        ssh_stream "test-$action35n_role" "$readback_program" \
            "$action35n_remote_evidence" >"$action35n_raw"
        action35n_observed=$(sha256sum "$action35n_raw" | awk '{ print $1 }')
        write_decision "evidence-readback-$action35n_role-success" accept 0 \
            "$action35n_observed" "$action35n_observed" "$action35n_raw" "$action35n_decision"

        action35n_raw=$action35n_test_root/raw/evidence-readback-$action35n_role-failure.txt
        action35n_decision=$action35n_test_root/decisions/evidence-readback-$action35n_role-failure.tsv
        if ssh_stream "test-$action35n_role" "$readback_program" \
            "$action35n_remote_evidence-absent" >"$action35n_raw" 2>&1; then
            action35n_status=0
        else
            action35n_status=$?
        fi
        write_decision "evidence-readback-$action35n_role-failure" reject \
            "$action35n_status" present absent "$action35n_raw" "$action35n_decision"
    done
    cleanup_remote node-a "$node_a_host" "$node_a_payload" "$node_a_archive"
    cleanup_remote node-b "$node_b_host" "$node_b_payload" "$node_b_archive"
    chmod -R u+rwX -- "$action35n_remote_base"
    rm -rf -- "$action35n_remote_base"
...
transaction=$repository_root/Caddy/scripts/apply-coupled-serving-health-action35n.sh
...
if [[ "$invocation_mode" = --production-path-test ]]; then
    workstation_evidence=$(mktemp -d /tmp/caddy-action35n-outer-test.XXXXXX)
else
    workstation_evidence=$(mktemp -d /tmp/caddy-ssh-evidence-action35n.XXXXXX)
fi
readonly workstation_evidence
chmod 0700 "$workstation_evidence"
payload_stage=$workstation_evidence/payload
payload_archive=$workstation_evidence/action35n-payload.tar
...
readonly run_id
node_a_payload=/tmp/caddy-action35n-$run_id-node-a
node_b_payload=/tmp/caddy-action35n-$run_id-node-b
node_a_archive=$node_a_payload.tar
node_b_archive=$node_b_payload.tar
node_a_evidence=$node_a_payload/evidence
node_b_evidence=$node_b_payload/evidence
