...
readonly prefix=action_20a_retry
...
readonly probe_sha256=19d3fffb5519d143c4e7c27c384ba9d30020ffc0174fb320be5088ca90c51076
readonly maximum_stream_bytes=2097152
readonly maximum_stream_lines=8192

script_directory=$(cd -- "$(dirname -- "${BASH_SOURCE[0]}")" && pwd)
readonly script_directory
readonly node_a_baseline="$script_directory/run-node-a-caddy-health-postinstall-action20c-a-outer.sh"
readonly node_b_baseline="$script_directory/run-node-b-caddy-health-postinstall-action20b-a-outer.sh"
readonly node_a_producer="$script_directory/inspect-node-a-caddy-health-postinstall-action20c-a.sh"
readonly node_b_producer="$script_directory/inspect-node-b-caddy-health-postinstall-action20b-a.sh"
readonly probe="$script_directory/inspect-dual-node-caddy-vrrp-preactivation-action20a-retry.sh"
...
validate_baseline() {
    ...
    local baseline_producer
    local baseline_expected_labels
    local baseline_observed_labels
    local baseline_expected_count
    local baseline_observed_count
    local baseline_contract_directory

    [[ "$baseline_status" -eq 0 ]] || return 1
    [[ ! -s "$baseline_stderr" ]] || return 1
    case "$baseline_role" in
        node-a)
            baseline_prefix=action_20c_a
            baseline_producer=$node_a_producer
            ;;
        node-b)
            baseline_prefix=action_20b_a
            baseline_producer=$node_b_producer
            ;;
        *) return 1 ;;
    esac
    baseline_contract_directory=${baseline_stdout%/*}
    baseline_expected_labels=$(mktemp "$baseline_contract_directory/baseline-expected.XXXXXX") ||
        return 1
    baseline_observed_labels=$(mktemp "$baseline_contract_directory/baseline-observed.XXXXXX") ||
        return 1
    "$baseline_producer" --expected-assertions |
        LC_ALL=C sort >"$baseline_expected_labels" || return 1
    sed -n "s/^${baseline_prefix}_assertion_\([a-z0-9_]*\)=\(true\|false\)$/\1 \2/p" \
        "$baseline_stdout" | LC_ALL=C sort >"$baseline_observed_labels" ||
        return 1
    baseline_expected_count=$(wc -l <"$baseline_expected_labels") || return 1
    baseline_observed_count=$(wc -l <"$baseline_observed_labels") || return 1
    [[ "$baseline_expected_count" -eq 138 ]] || return 1
    [[ "$baseline_observed_count" -eq 138 ]] || return 1
    [[ "$(cut -d' ' -f1 "$baseline_expected_labels" | uniq -d | wc -l)" -eq 0 ]] ||
        return 1
    [[ "$(cut -d' ' -f1 "$baseline_observed_labels" | uniq -d | wc -l)" -eq 0 ]] ||
        return 1
    ! grep -q ' false$' "$baseline_observed_labels" || return 1
    cut -d' ' -f1 "$baseline_observed_labels" |
        cmp -s "$baseline_expected_labels" - || return 1
    require_one "${baseline_prefix}_assertion_count=138" "$baseline_stdout" ||
        return 1
    ...
    require_one "${baseline_prefix}_validation_status=0" "$baseline_stdout" ||
        return 1
    require_one "${baseline_prefix}_runner_status=0" "$baseline_stdout" || return 1
    require_one "${baseline_prefix}_outer_cleanup_complete=true" \
        "$baseline_stdout" || return 1
    require_one "${baseline_prefix}_helper_execution=true" "$baseline_stdout" ||
        return 1
    require_one "${baseline_prefix}_assertion_state_unchanged=true" \
        "$baseline_stdout" || return 1
    for baseline_marker in filesystem_mutations service_mutations \
        vrrp_mutations vip_mutations persistent_mutations; do
...
validate_probe() {
    ...
    while IFS= read -r expected_label; do
        require_one "action_20a_retry_probe_assertion_${expected_label}=true" \
            "$probe_stdout" || return 1
    done < <("$probe" --expected-assertions)
    [[ "$(grep -Ec '^action_20a_retry_probe_assertion_[a-z0-9_]+=(true|false)$' \
        "$probe_stdout")" -eq "$probe_expected_count" ]] || return 1
    [[ "$(grep '^action_20a_retry_probe_assertion_' "$probe_stdout" |
        cut -d= -f1 | LC_ALL=C sort | uniq -d | wc -l)" -eq 0 ]] || return 1
    require_one "action_20a_retry_probe_assertion_count=$probe_expected_count" \
        "$probe_stdout" || return 1
    require_one 'action_20a_retry_probe_failed_assertion_count=0' "$probe_stdout" ||
        return 1
    require_one 'action_20a_retry_probe_first_failure=none' "$probe_stdout" || return 1
    require_one "action_20a_retry_probe_value_node_role=$probe_role" \
        "$probe_stdout" || return 1
    require_one 'action_20a_retry_probe_notification_helper_invoked=false' \
        "$probe_stdout" || return 1
    for probe_marker in filesystem_mutations service_mutations vrrp_mutations \
        vip_mutations persistent_mutations; do
        require_one "action_20a_retry_probe_${probe_marker}=false" \
            "$probe_stdout" || return 1
    done
    require_one 'action_20a_retry_probe_remote_complete=true' "$probe_stdout"
}
verify_sources() {
    [[ -f "$node_a_baseline" && ! -L "$node_a_baseline" ]] || return 1
    [[ -f "$node_b_baseline" && ! -L "$node_b_baseline" ]] || return 1
    [[ -f "$probe" && ! -L "$probe" ]] || return 1
    [[ -f "$node_a_producer" && ! -L "$node_a_producer" ]] || return 1
    [[ -f "$node_b_producer" && ! -L "$node_b_producer" ]] || return 1
    [[ "$(file_hash "$node_a_baseline")" = "$node_a_baseline_sha256" ]] ||
        return 1
    [[ "$(file_hash "$node_b_baseline")" = "$node_b_baseline_sha256" ]] ||
        return 1
    [[ "$(file_hash "$node_a_producer")" = "$node_a_producer_sha256" ]] ||
        return 1
    [[ "$(file_hash "$node_b_producer")" = "$node_b_producer_sha256" ]] ||
        return 1
    [[ "$(file_hash "$probe")" = "$probe_sha256" ]] || return 1
    bash -n "$node_a_baseline" "$node_b_baseline" "$node_a_producer" \
        "$node_b_producer" "$probe" || return 1
...
write_probe_fixture() {
    ...
    {
        while IFS= read -r fixture_label; do
            printf 'action_20a_retry_probe_assertion_%s=true\n' "$fixture_label"
        done < <("$probe" --expected-assertions)
        printf '%s\n' \
            "action_20a_retry_probe_value_node_role=$fixture_role" \
            "action_20a_retry_probe_value_priority=$fixture_priority" \
            'action_20a_retry_probe_value_ipv4_vrid=110' \
            'action_20a_retry_probe_value_ipv6_vrid=111' \
            "action_20a_retry_probe_value_ipv4_source=$fixture_ipv4_source" \
            "action_20a_retry_probe_value_ipv4_peer=$fixture_ipv4_peer" \
            "action_20a_retry_probe_value_ipv6_source=$fixture_ipv6_source" \
            "action_20a_retry_probe_value_ipv6_peer=$fixture_ipv6_peer" \
            "action_20a_retry_probe_value_dns_ipv4_vip_count=$dns_count" \
            "action_20a_retry_probe_value_dns_ipv6_vip_count=$dns_count" \
            'action_20a_retry_probe_value_caddy_ipv4_vip_count=0' \
            'action_20a_retry_probe_value_caddy_ipv6_vip_count=0' \
            "action_20a_retry_probe_value_main_sha256=$fixture_state" \
            "action_20a_retry_probe_value_fragment_sha256=$fixture_fragment" \
            "action_20a_retry_probe_value_before_state_sha256=$fixture_state" \
            "action_20a_retry_probe_value_after_state_sha256=$fixture_state" \
            'action_20a_retry_probe_value_health_status=0' \
            'action_20a_retry_probe_value_health_stream_classification=true' \
            "action_20a_retry_probe_assertion_count=$("$probe" --expected-assertions | wc -l)" \
            'action_20a_retry_probe_failed_assertion_count=0' \
            'action_20a_retry_probe_first_failure=none' \
            'action_20a_retry_probe_notification_helper_invoked=false' \
            'action_20a_retry_probe_filesystem_mutations=false' \
            'action_20a_retry_probe_service_mutations=false' \
            'action_20a_retry_probe_vrrp_mutations=false' \
            'action_20a_retry_probe_vip_mutations=false' \
            'action_20a_retry_probe_persistent_mutations=false' \
            'action_20a_retry_probe_remote_complete=true'
    } >"$fixture_path"
}
run_contract_test() {
    local contract_root
    local contract_role_key

    contract_root=$(mktemp -d /tmp/caddy-action20a-retry-contract.XXXXXX) || return 1
    : >"$contract_root/empty"
    for contract_role in node-a node-b; do
        contract_role_key=${contract_role//-/_}
        if write_baseline_fixture "$contract_role" "$contract_root/baseline"; then
            printf '%s_contract_%s_baseline_fixture=true\n' "$prefix" \
                "$contract_role_key"
        else
            printf '%s_contract_%s_baseline_fixture=false\n' "$prefix" \
                "$contract_role_key" >&2
            rm -rf -- "$contract_root"
            return 1
        fi
        if validate_baseline "$contract_role" "$contract_root/baseline" \
            "$contract_root/empty" 0; then
            printf '%s_contract_%s_baseline_valid=true\n' "$prefix" \
                "$contract_role_key"
        else
            printf '%s_contract_%s_baseline_valid=false\n' "$prefix" \
                "$contract_role_key" >&2
            rm -rf -- "$contract_root"
            return 1
        fi
        if write_probe_fixture "$contract_role" 0 "$contract_root/probe"; then
            printf '%s_contract_%s_probe_fixture=true\n' "$prefix" \
                "$contract_role_key"
        else
            printf '%s_contract_%s_probe_fixture=false\n' "$prefix" \
                "$contract_role_key" >&2
            rm -rf -- "$contract_root"
            return 1
        fi
        if validate_probe "$contract_role" "$contract_root/probe" \
            "$contract_root/empty" 0; then
            printf '%s_contract_%s_probe_valid=true\n' "$prefix" \
                "$contract_role_key"
        else
            printf '%s_contract_%s_probe_valid=false\n' "$prefix" \
                "$contract_role_key" >&2
            rm -rf -- "$contract_root"
            return 1
        fi
        printf 'action_20a_retry_probe_assertion_identity_root=true\n' \
            >>"$contract_root/probe"
        if validate_probe "$contract_role" "$contract_root/probe" \
            "$contract_root/empty" 0; then
            printf '%s_contract_%s_duplicate_probe_rejected=false\n' "$prefix" \
                "$contract_role_key" >&2
            rm -rf -- "$contract_root"
            return 1
        fi
        printf '%s_contract_%s_duplicate_probe_rejected=true\n' "$prefix" \
            "$contract_role_key"
    done
    rm -rf -- "$contract_root"
}
...
verify_sources
if [[ "${CADDY_ACTION20ARETRY_INTERCEPTED_TEST:-}" = 1 ]]; then
    case "$PWD" in
...
work_directory=$(mktemp -d /tmp/caddy-action20a-retry.XXXXXX)
...
if [[ "${CADDY_ACTION20ARETRY_INTERCEPTED_TEST:-}" = 1 ]]; then
    node_a_baseline_command=${CADDY_ACTION20ARETRY_NODE_A_BASELINE:?}
    node_b_baseline_command=${CADDY_ACTION20ARETRY_NODE_B_BASELINE:?}
    ssh_binary=${CADDY_ACTION20ARETRY_SSH_BINARY:?}
else
...
node_a_health=$(extract_one action_20a_retry_probe_value_health_status "$node_a_probe_path" || true)
node_b_health=$(extract_one action_20a_retry_probe_value_health_status "$node_b_probe_path" || true)
node_a_before=$(extract_one action_20a_retry_probe_value_before_state_sha256 "$node_a_probe_path" || true)
node_a_after=$(extract_one action_20a_retry_probe_value_after_state_sha256 "$node_a_probe_path" || true)
node_b_before=$(extract_one action_20a_retry_probe_value_before_state_sha256 "$node_b_probe_path" || true)
node_b_after=$(extract_one action_20a_retry_probe_value_after_state_sha256 "$node_b_probe_path" || true)
node_a_dns4=$(extract_one action_20a_retry_probe_value_dns_ipv4_vip_count "$node_a_probe_path" || true)
node_a_dns6=$(extract_one action_20a_retry_probe_value_dns_ipv6_vip_count "$node_a_probe_path" || true)
node_b_dns4=$(extract_one action_20a_retry_probe_value_dns_ipv4_vip_count "$node_b_probe_path" || true)
node_b_dns6=$(extract_one action_20a_retry_probe_value_dns_ipv6_vip_count "$node_b_probe_path" || true)
node_a_caddy4=$(extract_one action_20a_retry_probe_value_caddy_ipv4_vip_count "$node_a_probe_path" || true)
node_a_caddy6=$(extract_one action_20a_retry_probe_value_caddy_ipv6_vip_count "$node_a_probe_path" || true)
node_b_caddy4=$(extract_one action_20a_retry_probe_value_caddy_ipv4_vip_count "$node_b_probe_path" || true)
node_b_caddy6=$(extract_one action_20a_retry_probe_value_caddy_ipv6_vip_count "$node_b_probe_path" || true)
...
run_assertion caddy_ipv6_absent_both test "$node_a_caddy6" = 0 -a \
    "$node_b_caddy6" = 0
node_a_priority=$(extract_one action_20a_retry_probe_value_priority "$node_a_probe_path" || true)
node_b_priority=$(extract_one action_20a_retry_probe_value_priority "$node_b_probe_path" || true)
...
run_assertion vrids_exact bash -c \
    ...
    _ \
    "$(extract_one action_20a_retry_probe_value_ipv4_vrid "$node_a_probe_path" || true)" \
    "$(extract_one action_20a_retry_probe_value_ipv6_vrid "$node_a_probe_path" || true)" \
    "$(extract_one action_20a_retry_probe_value_ipv4_vrid "$node_b_probe_path" || true)" \
    "$(extract_one action_20a_retry_probe_value_ipv6_vrid "$node_b_probe_path" || true)"
# shellcheck disable=SC2016
run_assertion ipv4_peers_reciprocal bash -c \
    '[[ "$1" = "$4" && "$2" = "$3" ]]' _ \
    "$(extract_one action_20a_retry_probe_value_ipv4_source "$node_a_probe_path" || true)" \
    "$(extract_one action_20a_retry_probe_value_ipv4_peer "$node_a_probe_path" || true)" \
    "$(extract_one action_20a_retry_probe_value_ipv4_source "$node_b_probe_path" || true)" \
    "$(extract_one action_20a_retry_probe_value_ipv4_peer "$node_b_probe_path" || true)"
# shellcheck disable=SC2016
run_assertion ipv6_peers_reciprocal bash -c \
    '[[ "$1" = "$4" && "$2" = "$3" ]]' _ \
    "$(extract_one action_20a_retry_probe_value_ipv6_source "$node_a_probe_path" || true)" \
    "$(extract_one action_20a_retry_probe_value_ipv6_peer "$node_a_probe_path" || true)" \
    "$(extract_one action_20a_retry_probe_value_ipv6_source "$node_b_probe_path" || true)" \
    "$(extract_one action_20a_retry_probe_value_ipv6_peer "$node_b_probe_path" || true)"
# shellcheck disable=SC2016
run_assertion main_inclusion_prerequisites bash -c \
    'for transcript in "$1" "$2"; do for label in main_configuration_excludes_fragment main_configuration_caddy_names_clear main_configuration_vrids_clear main_configuration_terminal_newline main_hash_matches_backup; do grep -Fqx "action_20a_retry_probe_assertion_${label}=true" "$transcript" || exit 1; done; done' \
    _ "$node_a_probe_path" "$node_b_probe_path"
# shellcheck disable=SC2016
run_assertion notification_helpers_not_invoked bash -c \
    'grep -Fqx action_20a_retry_probe_notification_helper_invoked=false "$1" && grep -Fqx action_20a_retry_probe_notification_helper_invoked=false "$2"' \
    _ "$node_a_probe_path" "$node_b_probe_path"
