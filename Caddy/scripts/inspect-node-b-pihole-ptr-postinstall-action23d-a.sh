...
record_check() {
    local action23da_check_label=$1

    shift
    if [[ -n "${seen_checks[$action23da_check_label]+set}" ]]; then
        printf '%s_duplicate_check=%s\n' "$prefix" "$action23da_check_label" >&2
        return 1
    fi
    seen_checks[$action23da_check_label]=true
    if "$@"; then
        printf '%s_check_%s=true\n' "$prefix" "$action23da_check_label"
        return 0
    fi
...
address_count() {
    local action23da_family=$1
    local action23da_cidr=$2

    ip -o "-$action23da_family" address show dev "$interface" |
        awk -v wanted="$action23da_cidr" '$4 == wanted { count++ } END { print count + 0 }'
}
https_probe() {
    local action23da_name=$1
    local action23da_address=$2

    curl -kfsS -o /dev/null --connect-timeout 3 --max-time 5 \
        --resolve "${action23da_name}:443:${action23da_address}" \
        "https://${action23da_name}/"
}
run_dns_query() (
    trap - ERR

    local action23da_server=$1
    local action23da_port=$2
    local action23da_name=$3
    local action23da_type=$4

    if [[ "$action23da_type" == PTR ]]; then
        timeout 4 dig +time=2 +tries=1 +short -p "$action23da_port" \
            "@$action23da_server" -x "$action23da_name" 2>/dev/null
    else
        timeout 4 dig +time=2 +tries=1 +short -p "$action23da_port" \
            "@$action23da_server" "$action23da_name" "$action23da_type" 2>/dev/null
    fi
)
query_and_emit() {
    local action23da_label=$1
    local action23da_server=$2
    local action23da_port=$3
    local action23da_name=$4
    local action23da_type=$5
    local action23da_expected=$6
    local action23da_answer=
    local action23da_status=0

    if action23da_answer=$(run_dns_query "$action23da_server" "$action23da_port" \
        "$action23da_name" "$action23da_type"); then
        action23da_status=0
    else
        action23da_status=$?
    fi
    action23da_answer=$(printf '%s\n' "$action23da_answer" | sed '/^$/d' | LC_ALL=C sort -u | paste -sd, -)
    action23da_answer=${action23da_answer:-none}
    record_check "${action23da_label}_status" test "$action23da_status" -eq 0 || return 1
    record_check "${action23da_label}_safe" safe_single_value "$action23da_answer" || return 1
    printf '%s_value_%s=%s\n' "$prefix" "$action23da_label" "$action23da_answer"
    if [[ "$action23da_expected" != classify ]]; then
        record_check "${action23da_label}_exact" test "$action23da_answer" = "$action23da_expected" || return 1
    fi
}
capture_safe() {
    local action23da_capture=$1

    [[ $(printf '%s' "$action23da_capture" | wc -c) -le "$maximum_capture_bytes" ]] || return 1
    [[ $(printf '%s\n' "$action23da_capture" | wc -l) -le "$maximum_capture_lines" ]] || return 1
    ! grep -Eqi 'WEBPASSWORD|PRIVATE KEY|Authorization:|api[_-]?key|token=' \
        <<<"$action23da_capture"
}
emit_capture() {
    local action23da_capture_label=$1
    local action23da_capture_text=$2
    local action23da_capture_sha256

    action23da_capture_sha256=$(printf '%s' "$action23da_capture_text" | sha256sum | awk '{ print $1 }')
    record_check "${action23da_capture_label}_safe" capture_safe "$action23da_capture_text" || return 1
    printf '%s_value_%s_sha256=%s\n' "$prefix" "$action23da_capture_label" "$action23da_capture_sha256"
    printf '%s_value_%s_begin\n' "$prefix" "$action23da_capture_label"
    if [[ -n "$action23da_capture_text" ]]; then
        printf '%s\n' "$action23da_capture_text"
    else
        printf 'none\n'
    fi
...
residue_count() {
    local action23da_residue_root=$1
    local action23da_residue_pattern=$2

    find "$action23da_residue_root" -maxdepth 1 -mindepth 1 \
        -name "$action23da_residue_pattern" -print | wc -l
}
state_snapshot() {
    printf 'ftl_hash=%s\n' "$(file_hash "$live_ftl")"
...
emit_expected_checks() {
    local action23da_expected_command

    for action23da_expected_command in "${required_commands[@]}"; do
        printf 'command_%s_available\n' "${action23da_expected_command//-/_}"
    done
    printf '%s\n' \
...
emit_contract_transcript() {
    local action23da_contract_label
    local action23da_contract_count=0
    local action23da_fixture_hash=aaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaa

    while IFS= read -r action23da_contract_label; do
        printf '%s_check_%s=true\n' "$prefix" "$action23da_contract_label"
        action23da_contract_count=$((action23da_contract_count + 1))
...
for action23da_required_command in "${required_commands[@]}"; do
    record_check "command_${action23da_required_command//-/_}_available" \
        command_available "$action23da_required_command" || exit 1
done
...
record_check backup_manifest_action_exact test "$(grep -Fxc 'action=23d' "$backup_manifest" || true)" -eq 1 || exit 1
record_check backup_manifest_node_exact test "$(grep -Fxc 'node=j1-svpihole00' "$backup_manifest" || true)" -eq 1 || exit 1
...
record_check run_stage_residue_absent test "$(residue_count /run 'caddy-action23d.*')" -eq 0 || exit 1
record_check version_residue_absent test "$(residue_count /tmp 'caddy-action23d-version.*')" -eq 0 || exit 1
...
record_check run_stage_residue_absent_after test "$(residue_count /run 'caddy-action23d.*')" -eq 0 || exit 1
record_check version_residue_absent_after test "$(residue_count /tmp 'caddy-action23d-version.*')" -eq 0 || exit 1
