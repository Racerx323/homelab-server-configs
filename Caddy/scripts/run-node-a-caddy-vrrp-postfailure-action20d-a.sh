...
readonly ssh_binary=${CADDY_ACTION20DA_SSH_BINARY:-/usr/bin/ssh}
...
validate_transcript() {
    ...
    contract_root=$(mktemp -d /tmp/caddy-action20d-a-contract.XXXXXX) || return 97
    ...
    require_one "${prefix}_candidate_validation_invoked=false" "$output_path" || return 97
...
write_contract_fixture() {
    ...
    printf '%s\n' \
        ...
        "${prefix}_candidate_validation_invoked=false" \
...
case "${1:-}" in
    ...
    --contract-test)
        [[ $# -eq 1 ]]
        verify_sources
        contract_root=$(mktemp -d /tmp/caddy-action20d-a-runner-contract.XXXXXX)
        ...
        sed '/^action_20d_a_assertion_main_hash_exact=/d' "$contract_root/valid.out" >"$contract_root/missing.out"
...
verify_sources
working_directory_approved
work_directory=$(mktemp -d /tmp/caddy-action20d-a-runner.XXXXXX)
...
remote_status=0
if [[ "${CADDY_ACTION20DA_INTERCEPTED_TEST:-}" = 1 ]]; then
