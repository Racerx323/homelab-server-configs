...
if [[ "${1:-}" == --self-test && $# -eq 1 ]]; then
    ...
    printf 'action_16ak_e_sync_ssh_postinstall_self_test_complete=true\n'
    exit 0
elif (($#)); then
...
printf 'action_16ak_e_remote_reached=true\n'
...
record_command libexec_directory test -d "$libexec"
record_command libexec_not_symlink test ! -L "$libexec"
record_equal libexec_meta \
    "$(stat -c '%U:%G:%a' "$libexec" 2>/dev/null || true)" root:root:755
record_equal libexec_symlink_count \
    "$(find "$libexec" -type l -print 2>/dev/null | wc -l)" 0
...
record_equal receiver_meta \
    "$(stat -c '%U:%G:%a' "$receiver" 2>/dev/null || true)" root:root:755
record_equal receiver_hash \
    "$(sha256sum "$receiver" 2>/dev/null | awk '{ print $1 }' || true)" \
    "$receiver_sha256"
record_equal setup_helper_meta \
    "$(stat -c '%U:%G:%a' "$setup_helper" 2>/dev/null || true)" root:root:755
record_equal setup_helper_hash \
    "$(sha256sum "$setup_helper" 2>/dev/null | awk '{ print $1 }' || true)" \
    "$setup_helper_sha256"
record_equal validator_meta \
    "$(stat -c '%U:%G:%a' "$validator" 2>/dev/null || true)" root:root:755
record_equal validator_hash \
    "$(sha256sum "$validator" 2>/dev/null | awk '{ print $1 }' || true)" \
    "$validator_sha256"
...
observed_public_key=$(<"$public_key")
derived_public_key=$(
    ssh-keygen -y -f "$private_key" 2>/dev/null |
        awk '{ print $1, $2 }'
)
record_equal public_key_exact "$observed_public_key" \
    "$expected_node_a_public_key"
record_equal private_public_key_match "$derived_public_key" \
    "$(awk '{ print $1, $2 }' "$public_key" 2>/dev/null)"
observed_node_a_fingerprint=$(
    ssh-keygen -lf "$public_key" -E sha256 2>/dev/null |
        awk '{ print $2 }'
)
record_equal node_a_fingerprint "$observed_node_a_fingerprint" \
...
record_equal known_hosts_exact \
    "$(<"$known_hosts")" "pihole00.local.theama.co $node_b_host_key"
record_equal known_hosts_line_count "$(wc -l <"$known_hosts")" 1
record_command known_hosts_lookup \
    ssh-keygen -F pihole00.local.theama.co -f "$known_hosts"
...
record_equal authorized_keys_line_count "$(wc -l <"$authorized_keys")" 1
record_absent lsyncd_configuration_absent "$lsyncd_config"
record_command independent_ssh_client_construction \
    ssh -G -F /dev/null -i "$private_key" \
    -o BatchMode=yes -o IdentitiesOnly=yes \
    -o StrictHostKeyChecking=yes \
    -o "UserKnownHostsFile=$known_hosts" \
    caddy-sync@pihole00.local.theama.co
...
if [[ "$mismatch_count" -eq 0 ]]; then
    printf 'action_16ak_e_postinstall_valid=true\n'
    printf 'action_16ak_e_read_only_inspection_complete=true\n'
    exit 0
fi
printf 'action_16ak_e_postinstall_valid=false\n'
printf 'action_16ak_e_read_only_inspection_complete=true\n'
exit 1
