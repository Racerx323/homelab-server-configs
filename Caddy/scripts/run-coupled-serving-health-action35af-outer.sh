...
readonly prefix=action_35_af_outer
readonly transaction_sha256=69e9ac737a8e89f25fbfbc21b0f222dd98c2fc455f8e974919f140fd4efa3366
...
safe_capture() {
    local action35af_path=$1

    regular_file "$action35af_path"
    [[ "$(stat -c '%s' "$action35af_path")" -le "$max_stream_bytes" ]]
    iconv -f UTF-8 -t UTF-8 "$action35af_path" >/dev/null
    ! LC_ALL=C grep -Paq '[\x00-\x08\x0b\x0c\x0e-\x1f\x7f]' "$action35af_path"
}

capture() {
    local action35af_label=$1
    shift
    local action35af_stdout=$workstation_evidence/$action35af_label.stdout
    local action35af_stderr=$workstation_evidence/$action35af_label.stderr
    local action35af_status=$workstation_evidence/$action35af_label.status
    ...
    safe_capture "$action35af_stdout"
    safe_capture "$action35af_stderr"
    return "$action35af_rc"
}

build_payload() {
    local action35af_repository action35af_source action35af_target action35af_mode
    local action35af_hash action35af_lifecycle action35af_source_path action35af_destination
    ...
    install -m 0600 "$repository_root/Caddy/manifests/action35af-node-b-quarantine.tsv" \
        "$payload_stage/manifests/action35af-node-b-quarantine.tsv"
    while IFS=$'\t' read -r action35af_repository action35af_source action35af_target \
        action35af_mode action35af_hash action35af_lifecycle; do
        [[ "$action35af_repository" = '# repository' ]] && continue
        [[ "$action35af_mode" =~ ^0[0-7]{3}$ && "$action35af_lifecycle" = production-candidate ]]
        [[ "$action35af_target" = /* ]]
        action35af_source_path=${workspace_root}/$action35af_repository/$action35af_source
        regular_file "$action35af_source_path"
        [[ "$(sha256sum "$action35af_source_path" | awk '{ print $1 }')" = "$action35af_hash" ]]
        action35af_destination=$payload_stage/repositories/$action35af_repository/$action35af_source
        install -d -m 0700 "$(dirname -- "$action35af_destination")"
        install -m 0600 "$action35af_source_path" "$action35af_destination"
...
readonly archive=$2
case "$remote_root" in /tmp/caddy-action35af-*) ;; *) exit 64 ;; esac
case "$archive" in /tmp/caddy-action35af-*.tar) ;; *) exit 64 ;; esac
...
case "$evidence_root" in /tmp/caddy-action35af-*) ;; *) exit 64 ;; esac
...
ssh_stream() {
    local action35af_host=$1
    local action35af_program=$2
    shift 2
    "$ssh_command" "$action35af_host" \
        "cd / && sudo -n /bin/bash -s --$(printf ' %q' "$@")" <"$action35af_program"
}

upload_payload() {
    local action35af_role=$1
    local action35af_host=$2
    local action35af_remote_root=$3
    local action35af_remote_archive=$4
    local action35af_hash

    action35af_hash=$(awk '{ print $1 }' "$workstation_evidence/payload.sha256")
    capture "$action35af_role-upload-prepare" ssh_stream "$action35af_host" \
        "$prepare_program" "$action35af_remote_root" "$action35af_remote_archive"
    capture "$action35af_role-upload-copy" "$scp_command" -p -- \
        "$payload_archive" "$action35af_host:$action35af_remote_archive"
    capture "$action35af_role-upload-accept" ssh_stream "$action35af_host" \
        "$accept_program" "$action35af_remote_root" "$action35af_remote_archive" \
        "$action35af_hash"
}
...
remote_transaction() {
    ...
    capture "$action35af_label" ssh_stream "$action35af_host" "$transaction" "$@"
}

readback() {
    local action35af_role=$1
    local action35af_host=$2
    local action35af_remote_evidence=$3

    capture "$action35af_role-readback" ssh_stream "$action35af_host" \
        "$readback_program" "$action35af_remote_evidence"
}
...
cleanup_remote() {
    ...
    capture "$action35af_role-disposition" ssh_stream "$action35af_host" \
        "$disposition_program" "$action35af_remote_root" "$action35af_remote_archive"
}

run_live() {
    local action35af_node_b_mutated=false
    local action35af_node_a_mutated=false
    local action35af_failure=0
    local action35af_phase_status=0
    local action35af_target_revision=
    ...
    if upload_payload node-b "$node_b_host" "$node_b_payload" "$node_b_archive" &&
        ...
        return "$action35af_phase_status"
    fi

    action35af_node_b_mutated=true
    ...
    if [[ "$action35af_failure" -eq 0 ]]; then
    ...
    if [[ "$action35af_failure" -eq 0 ]]; then
        action35af_node_a_mutated=true
    ...
    if [[ "$action35af_failure" -eq 0 ]]; then
        if remote_transaction node-a-publish "$node_a_host" publish node-a \
            "$node_a_payload" "$node_a_evidence"; then
            action35af_target_revision=$(sed -n \
            ...
            if [[ ! "$action35af_target_revision" =~ ^[A-Za-z0-9][A-Za-z0-9._-]*$ ||
    ...
    if [[ "$action35af_failure" -eq 0 ]]; then
        if remote_transaction node-a-record-target "$node_a_host" record-target node-a \
            "$node_a_payload" "$node_a_evidence" "$action35af_target_revision" &&
            remote_transaction node-b-record-target "$node_b_host" record-target node-b \
                "$node_b_payload" "$node_b_evidence" "$action35af_target_revision" &&
    ...
    if ! remote_transaction node-a-journal-capture "$node_a_host" journal-capture node-a \
        "$node_a_payload" "$node_a_evidence"; then
        [[ "$action35af_failure" -ne 0 ]] || action35af_failure=1
    fi
    ...
    if [[ "$action35af_failure" -ne 0 ]]; then
        if [[ "$action35af_node_a_mutated" = true ]]; then
            remote_transaction node-a-rollback "$node_a_host" rollback node-a \
                "$node_a_payload" "$node_a_evidence" || exit 125
        fi
        if [[ "$action35af_node_b_mutated" = true ]]; then
...
production_path_test() {
    ...
    local action35af_remote_base action35af_raw action35af_decision action35af_observed
    local action35af_status action35af_role action35af_remote_evidence action35af_test_id
    local action35af_tmpfiles_line action35af_keepalived_stop_line
    local action35af_keepalived_line
    local action35af_mode action35af_variant_root action35af_variant_evidence
    local retained_root retained_candidate retained_release_hash retained_payload_hash
    local action35af_transaction_test_evidence
    ...
    action35af_remote_base=$(mktemp -d /tmp/caddy-action35af-production-path.XXXXXX)
    action35af_test_id=${action35af_remote_base##*.}
    test_remote_base=$action35af_remote_base
    export ACTION35AF_TEST_REMOTE_BASE=$test_remote_base
...
[[ "$1" = -n ]]
shift
export ACTION35AF_PRODUCTION_PATH_TEST=1
exec "$@"
SUDO
...
printf '%s:%s\n' "$runuser_identity" "$runuser_group" \
    >>"$ACTION35AF_TEST_REMOTE_BASE/runuser.identities"
printf 'runuser %s:%s\n' "$runuser_identity" "$runuser_group" \
    >>"$ACTION35AF_TEST_REMOTE_BASE/ordering.calls"
if [[ "${1:-}" = env && "${2:-}" = -i ]]; then
    shift 2
    exec env -i ACTION35AF_TEST_REMOTE_BASE="$ACTION35AF_TEST_REMOTE_BASE" "$@"
fi
exec "$@"
RUNUSER
...
#!/usr/bin/env bash
set -Eeuo pipefail
printf '%s\n' "$*" >>"$ACTION35AF_TEST_REMOTE_BASE/prohibited-keepalived-parser.calls"
exit 97
KEEPALIVED
...
#!/usr/bin/env bash
set -Eeuo pipefail
printf '%s\n' "$*" >>"$ACTION35AF_TEST_REMOTE_BASE/systemctl.calls"
printf 'systemctl %s\n' "$*" >>"$ACTION35AF_TEST_REMOTE_BASE/ordering.calls"
state_root=${ACTION35AF_TARGET_ROOT:-$ACTION35AF_TEST_REMOTE_BASE/default-root}
state_file=$state_root/run/action35af-keepalived-stopped
case "$1" in
    is-active)
        [[ "$2" = --quiet ]]
    ...
    start)
        ...
        if [[ "$2" = keepalived.service ]]; then
            rm -f -- "$state_file"
            if [[ -x "$ACTION35AF_TARGET_ROOT/etc/scripts/check-dns.sh" &&
                -x "$ACTION35AF_TARGET_ROOT/usr/local/libexec/check-caddy.sh" ]]; then
                for sequence in 1 2 3; do
                    env DNS_CHECK_DIG_COMMAND="$ACTION35AF_TEST_REMOTE_BASE/bin/dig" \
                        DNS_CHECK_SYSTEMCTL_COMMAND="$ACTION35AF_TEST_REMOTE_BASE/bin/systemctl" \
                        "$ACTION35AF_TEST_REMOTE_BASE/bin/runuser" -u pi -- \
                        "$ACTION35AF_TARGET_ROOT/etc/scripts/check-dns.sh"
                    env CADDY_SERVING_HEALTH_ENVIRONMENT_FILE="$ACTION35AF_TARGET_ROOT/etc/default/caddy-ha" \
                        CADDY_SERVING_HEALTH_CURL_COMMAND="$ACTION35AF_TEST_REMOTE_BASE/bin/curl" \
                        CADDY_SERVING_HEALTH_SYSTEMCTL_COMMAND="$ACTION35AF_TEST_REMOTE_BASE/bin/systemctl" \
                        "$ACTION35AF_TEST_REMOTE_BASE/bin/runuser" -u keepalived_script \
                        -g caddy-tls -- "$ACTION35AF_TARGET_ROOT/usr/local/libexec/check-caddy.sh"
                done
            fi
            printf '%s\n' \
                ...
                >>"$ACTION35AF_TEST_REMOTE_BASE/keepalived.journal"
            if [[ "${ACTION35AF_TEST_DAEMON_MODE:-success}" = journal-failure ]]; then
                printf '%s\n' \
...
#!/usr/bin/env bash
set -Eeuo pipefail
[[ "$1" = --create && "$2" = "${ACTION35AF_TARGET_ROOT:?}/etc/tmpfiles.d/caddy-ha.conf" ]]
install -d -m 0755 "$ACTION35AF_TARGET_ROOT/run/caddy-serving-health"
install -d -m 0755 "$ACTION35AF_TARGET_ROOT/run/caddy-serving-health/keepalived"
printf '%s\n' "$*" >>"$ACTION35AF_TEST_REMOTE_BASE/tmpfiles.calls"
printf 'tmpfiles %s\n' "$*" >>"$ACTION35AF_TEST_REMOTE_BASE/ordering.calls"
TMPFILES
    cat >"$test_remote_base/bin/dig" <<'DIG'
#!/usr/bin/env bash
if [[ "${ACTION35AF_TEST_DIG_FAIL:-0}" = 1 ]]; then
    exit 9
fi
case " $* " in
    *' A '*) printf '10.1.0.55\n' ;;
...
#!/usr/bin/env bash
case "${ACTION35AF_TEST_CURL_MODE:-success}" in
    failure) exit 7 ;;
...
set -Eeuo pipefail
case " $* " in
    *' --show-cursor '*) printf '%s\n' '-- cursor: s=action35af-production-path' ;;
    *' --after-cursor '*)
        if [[ -s "$ACTION35AF_TEST_REMOTE_BASE/keepalived.journal" ]]; then
            cat "$ACTION35AF_TEST_REMOTE_BASE/keepalived.journal"
        else
            printf '%s\n' 'test-host caddy.service: action35af production-path event'
        fi
        ;;
    *) exit 64 ;;
esac
...
shift
command="$*"
printf '%s\t%s\n' "$host" "$command" >>"$ACTION35AF_TEST_REMOTE_BASE/ssh.calls"
PATH="$ACTION35AF_TEST_REMOTE_BASE/bin:/usr/bin:/bin" /bin/bash -c "$command"
SSH
    cat >"$scp_command" <<'SCP'
#!/usr/bin/env bash
set -Eeuo pipefail
...
source=$3
target=${4#*:}
printf '%s\t%s\n' "$source" "$target" >>"$ACTION35AF_TEST_REMOTE_BASE/scp.calls"
install -o "$(id -un)" -g "$(id -gn)" -m 0600 "$source" "$target"
SCP
    chmod 0700 "$ssh_command" "$scp_command" "$test_remote_base/bin/"*
    export ACTION35AF_SLEEP_COMMAND=$test_remote_base/bin/sleep
    ...
    node_a_payload=/tmp/caddy-action35af-test-$action35af_test_id-node-a
    node_b_payload=/tmp/caddy-action35af-test-$action35af_test_id-node-b
    ...
    ACTION35AF_INCOMING_ROOT=$retained_root/incoming \
        ACTION35AF_OUTGOING_ROOT=$retained_root/outgoing \
        ACTION35AF_QUARANTINE_ROOT=$retained_root/quarantine \
        ACTION35AF_RELEASES_ROOT=$retained_root/releases \
        ACTION35AF_RETAINED_RELEASE_MANIFEST_SHA256=$retained_release_hash \
        ACTION35AF_RETAINED_PAYLOAD_MANIFEST_SHA256=$retained_payload_hash \
    ...
    ACTION35AF_ENVIRONMENT_FILE=$test_remote_base/environment-node-b \
        ACTION35AF_UNBOUND_CHECKCONF_COMMAND=$test_remote_base/bin/unbound-checkconf \
        ACTION35AF_RUNUSER_COMMAND=$test_remote_base/bin/runuser \
        ACTION35AF_SYSTEMCTL_COMMAND=$test_remote_base/bin/systemctl \
        ACTION35AF_DNS_DIG_COMMAND=$test_remote_base/bin/dig \
        ACTION35AF_CURL_COMMAND=$test_remote_base/bin/curl \
        ACTION35AF_SS_COMMAND=$test_remote_base/bin/ss \
    ...
    ACTION35AF_JOURNALCTL_COMMAND=$test_remote_base/bin/journalctl \
    ...
    ACTION35AF_TARGET_ROOT=$test_remote_base/node-b-root \
        ACTION35AF_SYSTEMCTL_COMMAND=$test_remote_base/bin/systemctl \
        ACTION35AF_JOURNALCTL_COMMAND=$test_remote_base/bin/journalctl \
        ACTION35AF_SYSTEMD_TMPFILES_COMMAND=$test_remote_base/bin/systemd-tmpfiles \
        ACTION35AF_RUNUSER_COMMAND=$test_remote_base/bin/runuser \
        ACTION35AF_DNS_DIG_COMMAND=$test_remote_base/bin/dig \
        ACTION35AF_CURL_COMMAND=$test_remote_base/bin/curl \
        ACTION35AF_SS_COMMAND=$test_remote_base/bin/ss \
        ...
        "$node_b_payload" "$node_b_evidence"
    action35af_raw=$action35af_test_root/raw/keepalived-daemon-owned-acceptance.txt
    action35af_decision=$action35af_test_root/decisions/keepalived-daemon-owned-acceptance.tsv
    install -m 0600 "$test_remote_base/ordering.calls" "$action35af_raw"
    action35af_tmpfiles_line=$(grep -n '^tmpfiles --create ' "$action35af_raw" | cut -d: -f1)
    action35af_keepalived_stop_line=$(grep -n '^systemctl stop keepalived.service$' \
        "$action35af_raw" | cut -d: -f1)
    action35af_keepalived_line=$(grep -n '^systemctl start keepalived.service$' \
        "$action35af_raw" | cut -d: -f1)
    [[ "$action35af_keepalived_stop_line" -lt "$action35af_tmpfiles_line" &&
        "$action35af_tmpfiles_line" -lt "$action35af_keepalived_line" ]]
    [[ "$(grep -c '^runuser pi:default$' "$action35af_raw")" -eq 3 ]]
    [[ "$(grep -c '^runuser keepalived_script:caddy-tls$' "$action35af_raw")" -eq 3 ]]
    grep -Fxq 'action_35_af_check_keepalived_daemon_dns_success=true' \
        "$workstation_evidence/node-b-install-producer.stdout"
    grep -Fxq 'action_35_af_check_keepalived_daemon_proxy_success=true' \
        "$workstation_evidence/node-b-install-producer.stdout"
    ...
    action35af_transaction_test_evidence=$action35af_test_root/transaction-through-outer
    install -d -m 0700 "$action35af_transaction_test_evidence"
    CADDY_PRODUCTION_PATH_EVIDENCE_ROOT=$action35af_transaction_test_evidence \
        ACTION35AF_TEST_REPOSITORY_ROOT=$repository_root \
        remote_transaction node-a-post-promote-production-path "$node_a_host" \
        --production-path-test
    ACTION35AF_TARGET_ROOT=$test_remote_base/node-a-root \
        ACTION35AF_SYSTEMCTL_COMMAND=$test_remote_base/bin/systemctl \
        ACTION35AF_JOURNALCTL_COMMAND=$test_remote_base/bin/journalctl \
        ACTION35AF_SYSTEMD_TMPFILES_COMMAND=$test_remote_base/bin/systemd-tmpfiles \
        ACTION35AF_RUNUSER_COMMAND=$test_remote_base/bin/runuser \
    ...
    action35af_mode=journal-failure
    action35af_variant_root=$test_remote_base/rejection-$action35af_mode-root
    action35af_variant_evidence=$node_b_payload/evidence-$action35af_mode
    install -d -m 0700 "$action35af_variant_root" "$action35af_variant_evidence"
    install -d -m 0755 "$action35af_variant_root/etc/default" \
        "$action35af_variant_root/usr/local/libexec"
    install -m 0640 "$test_remote_base/environment-node-b" \
        "$action35af_variant_root/etc/default/caddy-ha"
    install -m 0755 "$repository_root/Caddy/scripts/prepare-lighttpd-config.sh" \
        "$action35af_variant_root/usr/local/libexec/prepare-lighttpd-config.sh"
    : >"$test_remote_base/keepalived.journal"
    if ACTION35AF_TEST_DAEMON_MODE=$action35af_mode \
        ACTION35AF_TARGET_ROOT=$action35af_variant_root \
        ACTION35AF_SYSTEMCTL_COMMAND=$test_remote_base/bin/systemctl \
        ACTION35AF_JOURNALCTL_COMMAND=$test_remote_base/bin/journalctl \
        ACTION35AF_SYSTEMD_TMPFILES_COMMAND=$test_remote_base/bin/systemd-tmpfiles \
        ACTION35AF_RUNUSER_COMMAND=$test_remote_base/bin/runuser \
        ...
        ACTION35AF_SS_COMMAND=$test_remote_base/bin/ss \
        ACTION35AF_UNBOUND_CHECKCONF_COMMAND=$test_remote_base/bin/unbound-checkconf \
        remote_transaction "daemon-$action35af_mode-rejection-producer" \
        "$node_b_host" install node-b "$node_b_payload" \
        "$action35af_variant_evidence"; then
        action35af_status=0
    else
        action35af_status=$?
    fi
    [[ "$action35af_status" -ne 0 ]]
    action35af_raw=$action35af_test_root/raw/daemon-$action35af_mode-rejection.txt
    action35af_decision=$action35af_test_root/decisions/daemon-$action35af_mode-rejection.tsv
    {
        cat "$workstation_evidence/daemon-$action35af_mode-rejection-producer.stdout"
        cat "$workstation_evidence/daemon-$action35af_mode-rejection-producer.stderr"
        cat "$test_remote_base/keepalived.journal"
    } >"$action35af_raw"
    write_decision "daemon-$action35af_mode-rejection" reject \
        "$action35af_status" accepted rejected "$action35af_raw" "$action35af_decision"
    ACTION35AF_TARGET_ROOT=$action35af_variant_root \
        ACTION35AF_SYSTEMCTL_COMMAND=$test_remote_base/bin/systemctl \
        ACTION35AF_UNBOUND_CHECKCONF_COMMAND=$test_remote_base/bin/unbound-checkconf \
        remote_transaction "daemon-$action35af_mode-rollback-producer" \
        "$node_b_host" rollback node-b "$node_b_payload" \
        "$action35af_variant_evidence"
    [[ "$(find "$action35af_variant_root" -type f -printf '%P\n' | LC_ALL=C sort)" = $'etc/default/caddy-ha\nusr/local/libexec/prepare-lighttpd-config.sh' ]]
    ...
    action35af_raw=$action35af_test_root/raw/outer-preflight.txt
    action35af_decision=$action35af_test_root/decisions/outer-preflight.tsv
    cat "$test_remote_base/ssh.calls" "$test_remote_base/scp.calls" \
        ...
        >"$action35af_raw"
    action35af_observed=$(wc -l <"$action35af_raw")
    write_decision outer-preflight reach 0 "$action35af_observed" \
        "$action35af_observed" "$action35af_raw" "$action35af_decision"

    for action35af_role in node-a node-b; do
        if [[ "$action35af_role" = node-a ]]; then
            action35af_remote_evidence=$node_a_evidence
        else
            action35af_remote_evidence=$node_b_evidence
        fi
        action35af_raw=$action35af_test_root/raw/evidence-readback-$action35af_role-success.txt
        action35af_decision=$action35af_test_root/decisions/evidence-readback-$action35af_role-success.tsv
        ssh_stream "test-$action35af_role" "$readback_program" \
            "$action35af_remote_evidence" >"$action35af_raw"
        action35af_observed=$(sha256sum "$action35af_raw" | awk '{ print $1 }')
        write_decision "evidence-readback-$action35af_role-success" accept 0 \
            "$action35af_observed" "$action35af_observed" "$action35af_raw" "$action35af_decision"

        action35af_raw=$action35af_test_root/raw/evidence-readback-$action35af_role-failure.txt
        action35af_decision=$action35af_test_root/decisions/evidence-readback-$action35af_role-failure.tsv
        if ssh_stream "test-$action35af_role" "$readback_program" \
            "$action35af_remote_evidence-absent" >"$action35af_raw" 2>&1; then
            action35af_status=0
        else
            action35af_status=$?
        fi
        printf 'exit_status=%s\n' "$action35af_status" >>"$action35af_raw"
        write_decision "evidence-readback-$action35af_role-failure" reject \
            "$action35af_status" present absent "$action35af_raw" "$action35af_decision"
    done
    cleanup_remote node-a "$node_a_host" "$node_a_payload" "$node_a_archive"
    cleanup_remote node-b "$node_b_host" "$node_b_payload" "$node_b_archive"
    chmod -R u+rwX -- "$action35af_remote_base"
    rm -rf -- "$action35af_remote_base"
...
transaction=$repository_root/Caddy/scripts/apply-coupled-serving-health-action35af.sh
...
if [[ "$invocation_mode" = --production-path-test ]]; then
    workstation_evidence=$(mktemp -d /tmp/caddy-action35af-outer-test.XXXXXX)
else
    workstation_evidence=$(mktemp -d /tmp/caddy-ssh-evidence-action35af.XXXXXX)
fi
readonly workstation_evidence
chmod 0700 "$workstation_evidence"
payload_stage=$workstation_evidence/payload
payload_archive=$workstation_evidence/action35af-payload.tar
...
readonly run_id
node_a_payload=/tmp/caddy-action35af-$run_id-node-a
node_b_payload=/tmp/caddy-action35af-$run_id-node-b
node_a_archive=$node_a_payload.tar
node_b_archive=$node_b_payload.tar
node_a_evidence=$node_a_payload/evidence
node_b_evidence=$node_b_payload/evidence
