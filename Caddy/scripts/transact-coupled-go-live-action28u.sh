...
readonly prefix=action_28u_remote
...
usage() {
    printf 'Usage: %s --mode MODE --role node-a|node-b --stage DIR --run-id ID [--revision ID] [--source node-a|node-b]\n' "${0##*/}" >&2
}
...
safe_stream() {
    local action28u_remote_stream=$1

    [[ "$(wc -c <"$action28u_remote_stream")" -le "$maximum_stream_bytes" ]] || return 1
    [[ "$(line_count "$action28u_remote_stream")" -le "$maximum_stream_lines" ]] || return 1
    ! LC_ALL=C grep -n '[^[:print:][:space:]]' "$action28u_remote_stream" >/dev/null || return 1
    ! grep -Eqi 'BEGIN [A-Z ]*PRIVATE KEY|Authorization:[[:space:]]*Bearer|WEBPASSWORD' \
        "$action28u_remote_stream"
}
emit_stream() {
    local action28u_remote_label=$1
    local action28u_remote_stream=$2

    printf '%s_%s_%s_bytes=%s\n' "$prefix" "$mode" "$action28u_remote_label" \
        "$(wc -c <"$action28u_remote_stream")"
    printf '%s_%s_%s_lines=%s\n' "$prefix" "$mode" "$action28u_remote_label" \
        "$(line_count "$action28u_remote_stream")"
    printf '%s_%s_%s_sha256=%s\n' "$prefix" "$mode" "$action28u_remote_label" \
        "$(file_hash "$action28u_remote_stream")"
    if ! safe_stream "$action28u_remote_stream"; then
        printf '%s_%s_%s_classification=unsafe_retained\n' \
            "$prefix" "$mode" "$action28u_remote_label" >&2
        return 97
    fi
    printf '%s_%s_%s_classification=bounded_safe\n' \
        "$prefix" "$mode" "$action28u_remote_label"
    if [[ -s "$action28u_remote_stream" ]]; then
        printf '%s_%s_%s_begin\n' "$prefix" "$mode" "$action28u_remote_label"
        sed "s/^/${prefix}_${mode}_${action28u_remote_label}_content=/" \
            "$action28u_remote_stream"
        printf '%s_%s_%s_end\n' "$prefix" "$mode" "$action28u_remote_label"
    else
...
run_captured() {
    local action28u_remote_label=$1
    local action28u_remote_status=0

    shift
    install -m 0600 /dev/null "$evidence_directory/$action28u_remote_label.stdout"
    install -m 0600 /dev/null "$evidence_directory/$action28u_remote_label.stderr"
    "$@" >"$evidence_directory/$action28u_remote_label.stdout" \
        2>"$evidence_directory/$action28u_remote_label.stderr" || action28u_remote_status=$?
    printf '%s_%s_%s_status=%s\n' "$prefix" "$mode" "$action28u_remote_label" \
        "$action28u_remote_status"
    emit_stream "${action28u_remote_label}_stdout" \
        "$evidence_directory/$action28u_remote_label.stdout" || return 97
    emit_stream "${action28u_remote_label}_stderr" \
        "$evidence_directory/$action28u_remote_label.stderr" || return 97
    [[ "$action28u_remote_status" -eq 0 ]]
}
check() {
    local action28u_remote_label=$1
    ...
    first_failure=$action28u_remote_label
    printf '%s_%s_check_%s=false\n' "$prefix" "$mode" "$action28u_remote_label" >&2
    return 1
}
address_count() {
    local action28u_remote_family=$1
    local action28u_remote_cidr=$2

    ip -o "-$action28u_remote_family" address show dev eth0 |
        awk -v expected="$action28u_remote_cidr" \
...
ownership_exact() {
    local action28u_remote_state=$1
    local action28u_remote_count=0
    local action28u_remote_dbus='(us) 1 "Backup"'

    case "$action28u_remote_state" in
        master)
            action28u_remote_count=1
            action28u_remote_dbus='(us) 2 "Master"'
            ;;
        backup) ;;
        absent)
    ...
    [[ "$(dbus_state "$ipv4_object")" = "$action28u_remote_dbus" ]] || return 1
    [[ "$(dbus_state "$ipv6_object")" = "$action28u_remote_dbus" ]] || return 1
    [[ "$(address_count 4 "$dns_ipv4_cidr")" -eq "$action28u_remote_count" ]] || return 1
    [[ "$(address_count 6 "$dns_ipv6_cidr")" -eq "$action28u_remote_count" ]] || return 1
    [[ "$(address_count 4 "$caddy_ipv4_cidr")" -eq "$action28u_remote_count" ]] || return 1
    [[ "$(address_count 6 "$caddy_ipv6_cidr")" -eq "$action28u_remote_count" ]]
}
stable_ownership() {
    local action28u_remote_sample

    for action28u_remote_sample in 1 2 3 4 5; do
        ownership_exact "$expected_state" || return 1
        printf '%s_%s_stable_sample_%s=true\n' \
            "$prefix" "$mode" "$action28u_remote_sample"
        sleep 1
    done
}
manifest_valid() {
    local action28u_remote_release=$1

    [[ -d "$action28u_remote_release" && ! -L "$action28u_remote_release" ]] || return 1
    [[ -f "$action28u_remote_release/release-manifest.json" ]] || return 1
    [[ -f "$action28u_remote_release/manifest.sha256" ]] || return 1
    (cd "$action28u_remote_release" && sha256sum --strict --check manifest.sha256 >/dev/null)
}
configure_role() {
    case "$role" in
        node-a)
    ...
    backup_directory=/var/backups/caddy-ha/action28u-$role-go-live
    evidence_directory=/tmp/caddy-action28u/$run_id/$role
...
install_atomic() {
    local action28u_remote_source=$1
    local action28u_remote_target=$2
    local action28u_remote_mode=$3
    local action28u_remote_directory=${action28u_remote_target%/*}
    local action28u_remote_stage

    action28u_remote_stage=$(mktemp "$action28u_remote_directory/.action28u.XXXXXX")
    install -o root -g root -m "$action28u_remote_mode" \
        "$action28u_remote_source" "$action28u_remote_stage"
    mv -fT -- "$action28u_remote_stage" "$action28u_remote_target"
}
promote_release() {
    local action28u_remote_revision=$1
    local action28u_remote_source=/var/lib/caddy-sync/outbound/$action28u_remote_revision
    local action28u_remote_destination=/etc/caddy/releases/$action28u_remote_revision

    if [[ "$role" = node-b && "$action28u_remote_revision" = "$retained_revision" ]]; then
        action28u_remote_source=/var/lib/caddy-sync/incoming/node-a/$action28u_remote_revision
    fi
    manifest_valid "$action28u_remote_source" || return 1
    [[ "$(jq -r '.revision // empty' "$action28u_remote_source/release-manifest.json")" = "$action28u_remote_revision" ]] || return 1
    if [[ ! -d "$action28u_remote_destination" ]]; then
        cp -a -- "$action28u_remote_source" "$action28u_remote_destination"
    fi
    chown -R root:caddy-tls "$action28u_remote_destination"
    find "$action28u_remote_destination" -type d -exec chmod 0550 {} +
    find "$action28u_remote_destination" -type f -exec chmod 0440 {} +
    set -a
    # shellcheck disable=SC1091
    source /etc/default/caddy-ha
    set +a
    env CADDY_CONFIG_ROOT="$action28u_remote_destination" \
        caddy validate --config "$action28u_remote_destination/Caddyfile" \
        --adapter caddyfile >/dev/null
    ln -sfn "$action28u_remote_destination" /etc/caddy/current.new
    mv -Tf /etc/caddy/current.new /etc/caddy/current
    systemctl reload caddy.service
}
rollback_node() {
    local action28u_remote_before
    ...
    action28u_remote_before=$(<"$backup_directory/current.before")
    ln -sfn "$action28u_remote_before" /etc/caddy/current.new
    mv -Tf /etc/caddy/current.new /etc/caddy/current
...
install -d -o root -g root -m 0700 /tmp/caddy-action28u "$evidence_directory"
cd /
...
case "$mode" in
    ...
    commit)
        ...
        check transient_stage_absent test \
            -z "$(find /run -maxdepth 1 -name 'caddy-action28u-bundle.*' \
