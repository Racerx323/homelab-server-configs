...
readonly prefix=action_34l_remote
readonly artifact_manifest_relative=Caddy/manifests/durable-apprise-action34l.tsv
...
if [[ "$production_path_test_mode" = true ]]; then
    [[ "$production_path_test_root" = /tmp/caddy-action34l-production-path.* ]] || exit 64
    ...
    node_evidence_root=/tmp/caddy-action34l
fi
...
if [[ "$production_path_test_mode" = true ]]; then
    ...
    backup_root=$backup_parent/action34l-$role-$run_token
fi
...
production_path_verify_ownership_rewrite() {
    local action34l_rewrite_record=$production_path_test_root/ownership-rewrite.json
    local action34l_rewrite_uid action34l_rewrite_gid
    action34l_rewrite_uid=$(id -u) || return 1
    action34l_rewrite_gid=$(id -g) || return 1
    jq -n '{retry:{attempt:1,next_attempt_epoch:1}}' >"$action34l_rewrite_record" || return 1
    chmod 0600 "$action34l_rewrite_record" || return 1
    force_record_eligible_at "$action34l_rewrite_record" 1700000100 \
        "$action34l_rewrite_uid" "$action34l_rewrite_gid" || return 1
    [[ "$(stat -c '%u:%g:%a' "$action34l_rewrite_record")" = "$action34l_rewrite_uid:$action34l_rewrite_gid:600" ]] || return 1
    [[ "$(jq -r '.retry.next_attempt_epoch' "$action34l_rewrite_record")" -eq 1700000100 ]] || return 1
    printf '%s_%s_production_ownership_rewrite=preserved\n' \
        "$prefix" "${role//-/_}"
}

production_path_write_record() {
    local action34l_record_path=$1
    local action34l_record_id=${action34l_record_path##*/}
    local action34l_record_source=$2
    local action34l_record_body=$3
    action34l_record_id=${action34l_record_id%.json}
    jq -n --arg id "$action34l_record_id" --arg source "$action34l_record_source" \
        --arg body "$action34l_record_body" '{
        ...
        }' >"$action34l_record_path" || return 1
    chmod 0600 "$action34l_record_path"
}

production_path_prepare_queue() {
    local action34l_case_root=$1
    local action34l_case=$2
    local action34l_case_queue=$action34l_case_root/queue
    local action34l_extra_name=aaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaa.json
    [[ "$action34l_case" != node-b-queue-absent &&
        "$action34l_case" != node-a-queue-absent ]] || return 0
    install -d -m 0700 "$action34l_case_queue" || return 1
    install -d -m 0700 "$action34l_case_queue/pending" \
        "$action34l_case_queue/inflight" "$action34l_case_queue/dead-letter" \
        "$action34l_case_queue/delivered" || return 1
    case "$action34l_case" in
        node-b-exact-two-records | node-b-extra-record | node-b-unsafe-metadata | \
            node-b-symlink | node-b-malformed-record)
            production_path_write_record \
                "$action34l_case_queue/pending/$action34g_retained_record_two" \
    ...
    case "$action34l_case" in
        node-b-exact-two-records | node-b-one-record | node-b-extra-record | \
            ...
            production_path_write_record \
                "$action34l_case_queue/pending/$action34g_retained_record_one" \
                caddy-sync 'Action 34g controlled enqueue test' || return 1
            ;;
        node-b-symlink)
            ln -s /dev/null \
                "$action34l_case_queue/pending/$action34g_retained_record_one" || return 1
            ;;
        node-b-malformed-record)
            printf 'not-json\n' \
                >"$action34l_case_queue/pending/$action34g_retained_record_one" || return 1
            chmod 0600 "$action34l_case_queue/pending/$action34g_retained_record_one" || return 1
            ;;
    esac
    if [[ "$action34l_case" = node-b-extra-record ]]; then
        production_path_write_record "$action34l_case_queue/pending/$action34l_extra_name" \
            caddy-sync 'unexpected extra record' || return 1
    elif [[ "$action34l_case" = node-b-unsafe-metadata ]]; then
        chmod 0755 "$action34l_case_queue" || return 1
    fi
}

production_path_run_case() {
    local action34l_case=$1
    local action34l_case_role=$2
    local action34l_expected_status=$3
    local action34l_case_root=$4/$action34l_case
    local action34l_case_stdout=$action34l_case_root.stdout
    local action34l_case_stderr=$action34l_case_root.stderr
    local action34l_case_status=0
    install -d -m 0700 "$action34l_case_root" || return 1
    production_path_prepare_queue "$action34l_case_root" "$action34l_case" || return 1
    (
        cd /
        /bin/bash "$transaction_path" --production-path-case \
            "$action34l_case" "$action34l_case_root" "$action34l_case_role"
    ) >"$action34l_case_stdout" 2>"$action34l_case_stderr" || action34l_case_status=$?
    [[ "$action34l_case_status" -eq "$action34l_expected_status" ]] || return 1
    if [[ "$action34l_expected_status" -eq 0 ]]; then
        grep -Fq 'production_payload_validation=reached' "$action34l_case_stdout" || return 1
        grep -Fq 'production_mutation_boundary=reached_without_mutation' \
            "$action34l_case_stdout" || return 1
        grep -Fq 'production_ownership_rewrite=preserved' "$action34l_case_stdout" || return 1
    fi
}

production_path_test() {
    local action34l_test_root
    action34l_test_root=$(mktemp -d /tmp/caddy-action34l-production-path.XXXXXX) || return 1
    # Expand the validated mktemp path now; the local variable is unavailable at EXIT.
    # shellcheck disable=SC2064
    trap "rm -rf -- '$action34l_test_root'" EXIT
    printf 'production_path_dispatch_entry=true\n'
    production_path_run_case node-b-queue-absent node-b 0 "$action34l_test_root" || return 1
    ...
    production_path_run_case node-b-exact-two-records node-b 0 "$action34l_test_root" || return 1
    printf 'production_path_node_b_exact_two_records=true\n'
    production_path_run_case node-b-one-record node-b 1 "$action34l_test_root" || return 1
    printf 'production_path_node_b_one_record_rejected=true\n'
    production_path_run_case node-b-extra-record node-b 1 "$action34l_test_root" || return 1
    printf 'production_path_node_b_extra_record_rejected=true\n'
    production_path_run_case node-b-unsafe-metadata node-b 1 "$action34l_test_root" || return 1
    printf 'production_path_node_b_unsafe_metadata_rejected=true\n'
    production_path_run_case node-b-symlink node-b 1 "$action34l_test_root" || return 1
    printf 'production_path_node_b_symlink_rejected=true\n'
    production_path_run_case node-b-malformed-record node-b 1 "$action34l_test_root" || return 1
    printf 'production_path_node_b_malformed_record_rejected=true\n'
    production_path_run_case node-a-queue-absent node-a 0 "$action34l_test_root" || return 1
    printf 'production_path_node_a_queue_absent=true\n'
    rm -rf -- "$action34l_test_root" || return 1
    trap - EXIT
}
...
apply_action() {
    ...
    if [[ "$production_path_test_mode" = true ]]; then
        ...
        action34h_stage=$(mktemp -d /run/caddy-action34l-stage.XXXXXX) || return 1
    fi
...
cleanup_unmutated_backup() {
    [[ "$backup_root" =~ ^/var/backups/caddy-ha/action34l-node-[ab]-[0-9]{10,20}-[0-9]+$ ]] || return 1
