...
readonly prefix=action_20o_a_outer
readonly target=pi@10.1.0.54
readonly remote_command='cd / && sudo -n /bin/bash -s --'
readonly inspector_sha256=637cdb573603044ab35a7bbc936dcda0ec2c16f41edb99d285f0094138bfba80
...
readonly inspector=$script_directory/inspect-node-b-keepalived-dbus-postrollback-action20o-a.sh
readonly shfmt_canonical=$caddy_root/tests/shfmt-canonical.sh
readonly ssh_binary=${CADDY_ACTION20OA_SSH_BIN:-ssh}
...
run_action() (
    ...
    action20oa_outer_root=$(mktemp -d /tmp/caddy-action20o-a-outer.XXXXXX) || return 1
