...
readonly runner_revision=action17u-b-retry
...
# Invoked indirectly by the EXIT trap installed before capture-file creation.
# shellcheck disable=SC2317
cleanup_runner_work_directory() {
    local cleanup_status=$?

    trap - EXIT
    if [[ "$retain_work_directory" == false &&
        -n "$work_directory" && -d "$work_directory" ]]; then
...
trap cleanup_runner_work_directory EXIT
remote_stdout=$(mktemp "$work_directory/remote.stdout.XXXXXX")
remote_stderr=$(mktemp "$work_directory/remote.stderr.XXXXXX")
if [[ "$(stat -c '%U:%G:%a' "$remote_stdout")" == aaron:aaron:600 &&
"$(stat -c '%U:%G:%a' "$remote_stderr")" == aaron:aaron:600 ]]; then
    printf '%s_capture_files_secure=true\n' "$prefix"
else
    printf '%s_capture_files_secure=false\n' "$prefix" >&2
    exit 97
fi
...
printf '%s_value_runner_revision=%s\n' "$prefix" "$runner_revision"
