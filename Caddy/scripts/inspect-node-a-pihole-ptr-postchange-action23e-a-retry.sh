...
readonly prefix=action_23e_a_retry
...
readonly local_dns_vip_ptr=j1-svpihole0.local.theama.co.
readonly rejected_node_b_ptr=j1-svpihole00.local.theama.co.
...
record_check() {
    local action23ear_check_label=$1

    shift
    if [[ -n "${seen_checks[$action23ear_check_label]+set}" ]]; then
        printf '%s_duplicate_check=%s\n' "$prefix" "$action23ear_check_label" >&2
        return 1
    fi
    seen_checks[$action23ear_check_label]=true
    if "$@"; then
        printf '%s_check_%s=true\n' "$prefix" "$action23ear_check_label"
        return 0
    fi
...
address_count() {
    local action23ear_family=$1
    local action23ear_cidr=$2

    ip -o "-$action23ear_family" address show dev "$interface" |
        awk -v wanted="$action23ear_cidr" '$4 == wanted { count++ } END { print count + 0 }'
}
https_probe() {
    local action23ear_name=$1
    local action23ear_address=$2

    curl -kfsS -o /dev/null --connect-timeout 3 --max-time 5 \
        --resolve "${action23ear_name}:443:${action23ear_address}" \
        "https://${action23ear_name}/"
}
run_dns_query() (
    trap - ERR

    local action23ear_server=$1
    local action23ear_port=$2
    local action23ear_name=$3
    local action23ear_type=$4

    if [[ "$action23ear_type" == PTR ]]; then
        timeout 4 dig +time=2 +tries=1 +short -p "$action23ear_port" \
            "@$action23ear_server" -x "$action23ear_name" 2>/dev/null
    else
        timeout 4 dig +time=2 +tries=1 +short -p "$action23ear_port" \
            "@$action23ear_server" "$action23ear_name" "$action23ear_type" 2>/dev/null
    fi
)
ptr_expectation_matches() {
    local action23ear_policy=$1
    local action23ear_answer=$2

    case "$action23ear_policy" in
        direct) [[ "$action23ear_answer" == "$direct_dns_vip_ptr" ]] ;;
        local) [[ "$action23ear_answer" == "$local_dns_vip_ptr" ]] ;;
        *) return 1 ;;
    esac
}
query_and_emit() {
    local action23ear_label=$1
    local action23ear_server=$2
    local action23ear_port=$3
    local action23ear_name=$4
    local action23ear_type=$5
    local action23ear_expected=$6
    local action23ear_answer=
    local action23ear_status=0

    if action23ear_answer=$(run_dns_query "$action23ear_server" "$action23ear_port" \
        "$action23ear_name" "$action23ear_type"); then
        action23ear_status=0
    else
        action23ear_status=$?
    fi
    action23ear_answer=$(printf '%s\n' "$action23ear_answer" | sed '/^$/d' | LC_ALL=C sort -u | paste -sd, -)
    action23ear_answer=${action23ear_answer:-none}
    record_check "${action23ear_label}_status" test "$action23ear_status" -eq 0 || return 1
    record_check "${action23ear_label}_safe" safe_single_value "$action23ear_answer" || return 1
    printf '%s_value_%s=%s\n' "$prefix" "$action23ear_label" "$action23ear_answer"
    if [[ "$action23ear_expected" == policy:* ]]; then
        record_check "${action23ear_label}_exact" ptr_expectation_matches \
            "${action23ear_expected#policy:}" "$action23ear_answer" || return 1
    elif [[ "$action23ear_expected" != classify ]]; then
        record_check "${action23ear_label}_exact" test "$action23ear_answer" = "$action23ear_expected" || return 1
    fi
}
capture_safe() {
    local action23ear_capture=$1

    [[ $(printf '%s' "$action23ear_capture" | wc -c) -le "$maximum_capture_bytes" ]] || return 1
    [[ $(printf '%s\n' "$action23ear_capture" | wc -l) -le "$maximum_capture_lines" ]] || return 1
    ! grep -Eqi 'WEBPASSWORD|PRIVATE KEY|Authorization:|api[_-]?key|token=' \
        <<<"$action23ear_capture"
}
emit_capture() {
    local action23ear_capture_label=$1
    local action23ear_capture_text=$2
    local action23ear_capture_sha256

    action23ear_capture_sha256=$(printf '%s' "$action23ear_capture_text" | sha256sum | awk '{ print $1 }')
    record_check "${action23ear_capture_label}_safe" capture_safe "$action23ear_capture_text" || return 1
    printf '%s_value_%s_sha256=%s\n' "$prefix" "$action23ear_capture_label" "$action23ear_capture_sha256"
    printf '%s_value_%s_begin\n' "$prefix" "$action23ear_capture_label"
    if [[ -n "$action23ear_capture_text" ]]; then
        printf '%s\n' "$action23ear_capture_text"
    else
        printf 'none\n'
    fi
...
residue_count() {
    local action23ear_residue_root=$1
    local action23ear_residue_pattern=$2

    find "$action23ear_residue_root" -maxdepth 1 -mindepth 1 \
        -name "$action23ear_residue_pattern" -print | wc -l
}
state_snapshot() {
    printf 'ftl_hash=%s\n' "$(file_hash "$live_ftl")"
...
emit_expected_checks() {
    local action23ear_expected_command

    for action23ear_expected_command in "${required_commands[@]}"; do
        printf 'command_%s_available\n' "${action23ear_expected_command//-/_}"
    done
    printf '%s\n' \
...
emit_contract_transcript() {
    local action23ear_contract_label
    local action23ear_contract_count=0
    local action23ear_fixture_hash=aaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaa

    while IFS= read -r action23ear_contract_label; do
        printf '%s_check_%s=true\n' "$prefix" "$action23ear_contract_label"
        action23ear_contract_count=$((action23ear_contract_count + 1))
    ...
    printf '%s_value_ftl_sha256=%s\n' "$prefix" "$action23ear_fixture_hash"
    ...
    printf '%s_check_count=%s\n' "$prefix" "$action23ear_contract_count"
...
self_test() {
    ...
    ptr_expectation_matches direct "$direct_dns_vip_ptr" || return 1
    ! ptr_expectation_matches direct "$local_dns_vip_ptr" || return 1
    ! ptr_expectation_matches direct "$rejected_node_b_ptr" || return 1
    ! ptr_expectation_matches direct "$rejected_generic_ptr" || return 1
    ptr_expectation_matches local "$local_dns_vip_ptr" || return 1
    ! ptr_expectation_matches local "$direct_dns_vip_ptr" || return 1
    ! ptr_expectation_matches local "$rejected_node_b_ptr" || return 1
    ! ptr_expectation_matches local "$rejected_generic_ptr" || return 1
...
for action23ear_required_command in "${required_commands[@]}"; do
    record_check "command_${action23ear_required_command//-/_}_available" \
        command_available "$action23ear_required_command" || exit 1
done
...
query_and_emit direct_pihole_ptr4 127.0.0.1 5335 10.1.0.55 PTR policy:direct || exit 1
query_and_emit direct_pihole_ptr6 127.0.0.1 5335 fd36:5aa8:6971:1::55 PTR policy:direct || exit 1
...
query_and_emit local_pihole_ptr4 127.0.0.1 53 10.1.0.55 PTR policy:local || exit 1
query_and_emit local_pihole_ptr6 127.0.0.1 53 fd36:5aa8:6971:1::55 PTR policy:local || exit 1
