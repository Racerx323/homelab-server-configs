...
readonly new_validate_block_sha256=be85e806dfefe0781806e8f95f00c2e1e1d4ef9393aca24efa2a382bad99ccde
...
expected_check_labels() {
    printf '%s\n' \
        ...
        candidate_redirect_single candidate_old_line_absent candidate_validate_block_hash \
        ...
        installed_redirect_single installed_old_line_absent installed_validate_block_hash \
        ...
        backup_retained backup_manifest_final_hash
}
...
expected_rollback_check_labels() {
    printf '%s\n' \
        ...
        manifest_hash_exact request_absent pending_absent complete_absent \
...
case "${1:-}" in
    --self-test)
        ...
        [[ "$(expected_rollback_check_labels | wc -l)" -eq 21 ]]
...
require_check candidate_redirect_single \
    test "$(grep -Fxc '        --adapter caddyfile >/dev/null' "$candidate")" -eq 1
require_check candidate_old_line_absent \
    test "$(grep -Fxc '        --adapter caddyfile' "$candidate")" -eq 0
...
require_check action17t_backup_count_zero \
    test "$(find "$rollback_root" -mindepth 1 -maxdepth 1 \
        -type d -name 'action17t-node-b-stdout-safe-finalizer.*' -print | wc -l)" -eq 0
...
capture_continuity_state
work_directory=$(mktemp -d /run/caddy-action17t-node-b.XXXXXX)
...
rollback_directory=$(
    mktemp -d "$rollback_root/action17t-node-b-stdout-safe-finalizer.XXXXXX"
)
...
require_check installed_redirect_single \
    test "$(grep -Fxc '        --adapter caddyfile >/dev/null' "$live_finalizer")" -eq 1
require_check installed_old_line_absent \
    test "$(grep -Fxc '        --adapter caddyfile' "$live_finalizer")" -eq 0
...
printf '%s_persistent_mutation_scope=stdout_safe_finalizer,rollback_backup\n' \
    "$prefix"

transaction_complete=true
trap - EXIT
rm -rf -- "$work_directory"
printf '%s_node_b_stdout_safe_finalizer_install_complete=true\n' "$prefix"
