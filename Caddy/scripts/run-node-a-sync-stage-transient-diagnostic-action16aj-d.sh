...
readonly driver_sha256=fbe55c4be2bd0880371df05b0b509eee8aa0300cc2005ab3dca631e24c1abbce
...
readonly script_dir
readonly driver="$script_dir/diagnose-node-a-sync-stage-transient-action16aj-d.sh"
...
if [[ "${1:-}" == --self-test && $# -eq 1 ]]; then
    ...
    printf 'action_16aj_d_transient_diagnostic_runner_self_test_complete=true\n'
    exit 0
...
work_dir=$(mktemp -d /tmp/caddy-action16aj-d.XXXXXX)
readonly work_dir
...
finish_local() {
    ...
    if [[ -e "$work_dir" || -L "$work_dir" ]]; then
        printf 'action_16aj_d_local_cleanup_complete=false\n' >&2
        exit 97
    fi
    printf 'action_16aj_d_local_cleanup_complete=true\n'
    exit "$status"
}
...
if grep -Fq 'manual_intervention_required=true' "$remote_error"; then
    finish_local 97
fi

failure_count=$(
    sed -n 's/^diagnostic_failure_count=//p' "$remote_output"
)
first_failure=$(
    sed -n 's/^first_failure=//p' "$remote_output"
)
if [[ ! "$failure_count" =~ ^[0-9]+$ ||
    ! "$first_failure" =~ ^[A-Za-z0-9_]+$ ]]; then
    printf 'Invalid diagnostic summary.\n' >&2
    finish_local 1
fi
if [[ "$failure_count" -eq 0 && "$first_failure" != none ]] ||
    [[ "$failure_count" -ne 0 && "$first_failure" == none ]]; then
    printf 'Inconsistent diagnostic summary.\n' >&2
    finish_local 1
fi
