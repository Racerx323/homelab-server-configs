...
record_check() {
    local action28e_e_label=$1

    shift
    if [[ -n "${seen_checks[$action28e_e_label]+set}" ]]; then
        printf '%s_duplicate_check=%s\n' "$prefix" "$action28e_e_label" >&2
        return 1
    fi
    seen_checks[$action28e_e_label]=true
    if "$@"; then
        printf '%s_check_%s=true\n' "$prefix" "$action28e_e_label"
        return 0
    fi
...
marker_state() {
    local action28e_e_marker=$1

    if [[ ! -e "$action28e_e_marker" && ! -L "$action28e_e_marker" ]]; then
        printf 'absent\n'
    elif [[ -f "$action28e_e_marker" && ! -L "$action28e_e_marker" &&
        ! -s "$action28e_e_marker" ]]; then
...
manifest_file_set_matches() {
    local action28e_e_release=$1
    local action28e_e_expected
    local action28e_e_observed

    action28e_e_expected=$(mktemp /tmp/action28e-e-expected.XXXXXX) || return 1
    action28e_e_observed=$(mktemp /tmp/action28e-e-observed.XXXXXX) || {
        rm -f -- "$action28e_e_expected"
        return 1
    }
    awk '{ print substr($0, 67) }' "$action28e_e_release/manifest.sha256" |
        LC_ALL=C sort -u >"$action28e_e_expected"
    (
        cd "$action28e_e_release"
        find . -type f \
            ...
            -print | LC_ALL=C sort
    ) >"$action28e_e_observed"
    local action28e_e_status=0
    cmp -s "$action28e_e_expected" "$action28e_e_observed" || action28e_e_status=$?
    rm -f -- "$action28e_e_expected" "$action28e_e_observed"
    return "$action28e_e_status"
}
manifest_hashes_valid() {
    local action28e_e_release=$1

    (cd "$action28e_e_release" && sha256sum --strict --check manifest.sha256 >/dev/null)
}
current_tree_accepted() {
    local action28e_e_observed_tree

    action28e_e_observed_tree=$(tree_digest "$(root_path /etc/caddy/current)") || return 1
    if [[ "$role" == node-a ]]; then
        [[ "$action28e_e_observed_tree" = "$accepted_current_tree_sha256" ]]
    else
        valid_sha256 "$action28e_e_observed_tree"
    fi
}
unit_state() {
    local action28e_e_unit=$1

    if [[ "$test_mode" == true ]]; then
        case "$action28e_e_unit" in
            caddy.service | lighttpd.service | keepalived.service) printf 'active\n' ;;
            *) printf 'inactive\n' ;;
        esac
    else
        systemctl show "$action28e_e_unit" --no-pager --property ActiveState --value
    fi
}
address_count() {
    local action28e_e_family=$1
    local action28e_e_cidr=$2

    if [[ "$test_mode" == true ]]; then
        if [[ "$action28e_e_cidr" == "$physical_ipv4_cidr" ||
            "$action28e_e_cidr" == "$physical_ipv6_cidr" ]]; then
            printf '1\n'
    ...
    case "$action28e_e_family" in
        4) ip -o -4 addr show dev "$interface" | awk -v cidr="$action28e_e_cidr" '$4 == cidr { count++ } END { print count + 0 }' ;;
        6) ip -o -6 addr show dev "$interface" | awk -v cidr="$action28e_e_cidr" '$4 == cidr { count++ } END { print count + 0 }' ;;
        *) return 1 ;;
    esac
}
vrrp_state() {
...
snapshot() {
    local action28e_e_unit
    local action28e_e_current

    action28e_e_current=$(root_path /etc/caddy/current)
    {
        printf 'hostname=%s\n' "$expected_hostname"
        printf 'vrrp=%s\n' "$(vrrp_state)"
        printf 'current_link=%s\n' "$(readlink -- "$action28e_e_current")"
        printf 'current_target=%s\n' "$(readlink -f -- "$action28e_e_current")"
        printf 'current_tree=%s\n' "$(tree_digest "$action28e_e_current")"
        for action28e_e_unit in "${continuity_units[@]}"; do
            printf 'unit=%s|%s\n' "$action28e_e_unit" "$(unit_state "$action28e_e_unit")"
        done
...
expected_checks() {
    printf '%s\n' \
        ...
        historical_release_manifest_regular historical_release_manifest_not_symlink \
        historical_payload_manifest_regular historical_payload_manifest_not_symlink \
        historical_release_manifest_hash_exact historical_payload_manifest_hash_exact \
        historical_tree_hash_role_exact hidden_stage_residue_absent candidate_count_bounded \
...
emit_historical_identity_and_check() {
    local action28e_e_identity_kind=$1
    local action28e_e_expected_identity=$2
    local action28e_e_observed_identity=$3
    local action28e_e_check_label=$4

    printf '%s_value_historical_%s_expected_sha256=%s\n' \
        "$prefix" "$action28e_e_identity_kind" "$action28e_e_expected_identity"
    printf '%s_value_historical_%s_observed_sha256=%s\n' \
        "$prefix" "$action28e_e_identity_kind" "$action28e_e_observed_identity"
    record_check "$action28e_e_check_label" test \
        "$action28e_e_observed_identity" = "$action28e_e_expected_identity"
}
...
candidate_role_marker_state() {
    ...
    if [[ "$role" == node-a ]]; then
        [[ ! -e "$action28e_e_release/.complete" ]]
    else
        [[ -f "$action28e_e_release/.complete" &&
            ! -L "$action28e_e_release/.complete" &&
            ! -s "$action28e_e_release/.complete" ]]
    fi
}
candidate_optional_check() {
    ...
    if [[ "$candidate_name" == absent ]]; then
        record_check "$action28e_e_label" true
    else
        record_check "$action28e_e_label" "$@"
    fi
}
...
case "${1:-}" in
    ...
    --test-historical-identity)
        [[ $# -eq 5 && "${CADDY_ACTION28E_E_TEST_MODE:-}" == 1 ]]
        emit_historical_identity_and_check "$2" "$3" "$4" "$5"
        exit 0
        ;;
    node-a | node-b)
        [[ $# -eq 1 ]]
        ...
        ;;
    --test-root)
        [[ $# -eq 3 && "${CADDY_ACTION28E_E_TEST_MODE:-}" == 1 ]]
        test_mode=true
        root_prefix=$2
        role=$3
        ;;
...
record_check historical_release_manifest_regular test -f "$historical_release/release-manifest.json"
record_check historical_release_manifest_not_symlink test ! -L "$historical_release/release-manifest.json"
record_check historical_payload_manifest_regular test -f "$historical_release/manifest.sha256"
record_check historical_payload_manifest_not_symlink test ! -L "$historical_release/manifest.sha256"
historical_release_manifest_observed=$(file_hash "$historical_release/release-manifest.json")
readonly historical_release_manifest_observed
emit_historical_identity_and_check release_manifest \
    "$historical_release_manifest_sha256" "$historical_release_manifest_observed" \
    historical_release_manifest_hash_exact
historical_payload_manifest_observed=$(file_hash "$historical_release/manifest.sha256")
readonly historical_payload_manifest_observed
emit_historical_identity_and_check payload_manifest \
    "$historical_payload_manifest_sha256" "$historical_payload_manifest_observed" \
    historical_payload_manifest_hash_exact
if [[ "$role" == node-a ]]; then
    historical_tree_observed=$(tree_digest "$historical_release")
    readonly historical_tree_observed
    emit_historical_identity_and_check tree "$historical_outbound_tree_sha256" \
        "$historical_tree_observed" historical_tree_hash_role_exact
else
    printf '%s_value_historical_tree_expected_sha256=not_applicable\n' "$prefix"
    printf '%s_value_historical_tree_observed_sha256=not_applicable\n' "$prefix"
    record_check historical_tree_hash_role_exact true
fi
