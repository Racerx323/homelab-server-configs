...
readonly prefix=action_20d_retry6_node
...
expected_checks() {
    printf '%s\n' \
        ...
        reload_check_config_absent sanitized_candidate_static_contract \
...
case "${1:-}" in
    ...
    --self-test)
        [[ $# -eq 1 ]] || exit 64
        [[ "$(expected_checks | wc -l)" -eq 110 ]]
        [[ "$(expected_checks | LC_ALL=C sort -u | wc -l)" -eq 110 ]]
    ...
    --inspect)
        ...
        configure_role
        [[ "$node_role" = node-a ]] || exit 64
        transaction_root=$(mktemp -d /run/caddy-action20d-retry6-inspect.XXXXXX)
    ...
    --rollback)
        ...
        configure_role
        [[ "$node_role" = node-a ]] || exit 64
        transaction_root=$(mktemp -d /run/caddy-action20d-retry6-rollback.XXXXXX)
        readonly transaction_root
...
transaction_root=$(mktemp -d /run/caddy-action20d-retry6-node.XXXXXX)
readonly transaction_root
...
record_check reload_check_config_absent test \
    "$(grep -Eic '^[[:space:]]*reload_check_config([[:space:]]|$)' \
        "$main_configuration" "$fragment" "$candidate_configuration" \
