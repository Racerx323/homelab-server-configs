...
readonly prefix=action_28z_outer
...
readonly transaction=$script_directory/transact-coupled-go-live-action28z.sh
...
readonly regression=$caddy_root/tests/action28z-coupled-go-live-regression.sh
readonly manifest=$caddy_root/manifests/caddy-coupled-go-live-action28z.yaml
readonly expected_transaction_sha256=cb773994f54126c24bc86b85a4e05d317f6dfbd2697abb6d5bc0aa395ff47b96
...
readonly expected_node_a_lsyncd_sha256=aed4a0a7b5c31d3668c55fe774575f3faede74196fa57f2a8c93b48d3945e421
readonly expected_node_b_lsyncd_sha256=2c715655a13df60df84665a435f80e4aa9071f5dd9a17676d940c7ecdf2d2b18
evidence_root=${CADDY_ACTION28Z_EVIDENCE_ROOT:-/tmp/caddy-ssh-evidence/action28z}
readonly evidence_root
ssh_binary=${CADDY_ACTION28Z_SSH_BIN:-/usr/bin/ssh}
readonly ssh_binary
run_id=
outer_root=
last_stdout=
...
valid_source() {
    local action28z_outer_expected=$1
    local action28z_outer_source=$2

    [[ -f "$action28z_outer_source" && ! -L "$action28z_outer_source" ]] || return 1
    [[ "$(file_hash "$action28z_outer_source")" = "$action28z_outer_expected" ]]
}
...
gate() {
    local action28z_outer_label=$1

    shift
    if "$@" >/dev/null 2>&1; then
        printf '%s_gate_%s=true\n' "$prefix" "$action28z_outer_label"
        return 0
    fi
    printf '%s_gate_%s=false\n' "$prefix" "$action28z_outer_label" >&2
    return 1
}
safe_stream() {
    local action28z_outer_stream=$1

    [[ "$(wc -c <"$action28z_outer_stream")" -le "$maximum_stream_bytes" ]] || return 1
    [[ "$(line_count "$action28z_outer_stream")" -le "$maximum_stream_lines" ]] || return 1
    iconv -f UTF-8 -t UTF-8 "$action28z_outer_stream" >/dev/null 2>&1 || return 1
    ! LC_ALL=C.UTF-8 grep -n '[^[:print:][:space:]]' \
        "$action28z_outer_stream" >/dev/null || return 1
    ! grep -Eqi 'BEGIN [A-Z ]*PRIVATE KEY|Authorization:[[:space:]]*Bearer|WEBPASSWORD' \
        "$action28z_outer_stream"
}
run_stream_classifier_self_test() {
    local action28z_outer_fixture

    classifier_fixture_root=$(mktemp -d /tmp/action28z-outer-stream.XXXXXX)
    ...
    for action28z_outer_fixture in control invalid secret bytes lines; do
        if safe_stream "$classifier_fixture_root/$action28z_outer_fixture"; then
            return 1
        fi
    done
...
emit_stream() {
    local action28z_outer_label=$1
    local action28z_outer_stream=$2

    printf '%s_%s_bytes=%s\n' "$prefix" "$action28z_outer_label" \
        "$(wc -c <"$action28z_outer_stream")"
    printf '%s_%s_lines=%s\n' "$prefix" "$action28z_outer_label" \
        "$(line_count "$action28z_outer_stream")"
    printf '%s_%s_sha256=%s\n' "$prefix" "$action28z_outer_label" \
        "$(file_hash "$action28z_outer_stream")"
    if ! safe_stream "$action28z_outer_stream"; then
        printf '%s_%s_classification=unsafe_retained\n' "$prefix" "$action28z_outer_label" >&2
        return 97
    fi
    printf '%s_%s_classification=bounded_safe\n' "$prefix" "$action28z_outer_label"
    if [[ -s "$action28z_outer_stream" ]]; then
        printf '%s_%s_begin\n' "$prefix" "$action28z_outer_label"
        cat "$action28z_outer_stream"
        printf '%s_%s_end\n' "$prefix" "$action28z_outer_label"
    else
...
prepare_capture_files() {
    local action28z_outer_capture

    for action28z_outer_capture in "$@"; do
        install -m 0600 /dev/null "$action28z_outer_capture"
        [[ -f "$action28z_outer_capture" && ! -L "$action28z_outer_capture" ]]
        [[ "$(stat -c '%a' "$action28z_outer_capture")" = 600 ]]
    done
}
validate_remote_program() {
    local action28z_outer_program=$1

    # These are exact generated-program literals, not expressions to expand.
    # shellcheck disable=SC2016
    grep -Fq '/bin/bash "$stage/transact-coupled-go-live-action28z.sh" --mode' \
        "$action28z_outer_program" || return 1
    # shellcheck disable=SC2016
    ! grep -Eq '^"\$stage/transact-coupled-go-live-action28z\.sh" --mode' \
        "$action28z_outer_program" || return 1
    # shellcheck disable=SC2016
    grep -Fq 'chmod 0700 "$stage/transact-coupled-go-live-action28z.sh"' \
        "$action28z_outer_program"
}
run_local_gates() {
...
write_remote() {
    local action28z_outer_role=$1
    local action28z_outer_mode=$2
    local action28z_outer_revision=$3
    local action28z_outer_source=$4
    local action28z_outer_output=$5
    local action28z_outer_config=$node_a_lsyncd
    local action28z_outer_archive=$outer_root/payload.tar

    [[ "$action28z_outer_role" = node-a ]] || action28z_outer_config=$node_b_lsyncd
    tar -cf "$action28z_outer_archive" -C "$script_directory" \
        transact-coupled-go-live-action28z.sh publish-release-v2.sh reconcile-release-v2.sh
    tar -rf "$action28z_outer_archive" -C "${action28z_outer_config%/*}" \
        --transform='s|.*|configs/caddy.lua|' "${action28z_outer_config##*/}"
    ...
    # shellcheck disable=SC2016
    {
        printf '%s\n' '#!/usr/bin/env bash' 'set -Eeuo pipefail' 'set +x' 'umask 077' \
            ...
            'export PATH' 'cd /' \
            'stage=$(mktemp -d /run/caddy-action28z-bundle.XXXXXX)' \
        ...
        base64 "$action28z_outer_archive"
        printf '%s\n' 'ACTION28Z_ARCHIVE' \
            'tar -xf "$stage/payload.tar" -C "$stage" --no-same-owner --no-same-permissions' \
            'mv "$stage/configs/caddy.lua" "$stage/caddy.lua"' \
            'chmod 0700 "$stage/transact-coupled-go-live-action28z.sh"' \
            'chmod 0600 "$stage/publish-release-v2.sh" "$stage/reconcile-release-v2.sh" "$stage/caddy.lua"'
        printf '/bin/bash "$stage/transact-coupled-go-live-action28z.sh" --mode %q --role %q --stage "$stage" --run-id %q' \
            "$action28z_outer_mode" "$action28z_outer_role" "$run_id"
        [[ -z "$action28z_outer_revision" ]] || printf ' --revision %q' "$action28z_outer_revision"
        [[ -z "$action28z_outer_source" ]] || printf ' --source %q' "$action28z_outer_source"
        printf '\n'
    } >"$action28z_outer_output"
    chmod 0600 "$action28z_outer_output"
    validate_remote_program "$action28z_outer_output"
}
run_phase() {
    local action28z_outer_label=$1
    local action28z_outer_role=$2
    local action28z_outer_mode=$3
    local action28z_outer_revision=${4:-}
    local action28z_outer_source=${5:-}
    local action28z_outer_target=$node_a_target
    local action28z_outer_remote=$outer_root/$action28z_outer_label.remote.sh
    local remote_stdout=$outer_root/$action28z_outer_label.stdout
    local remote_stderr=$outer_root/$action28z_outer_label.stderr
    local status_file=$outer_root/$action28z_outer_label.status
    local remote_status=0

    [[ "$action28z_outer_role" = node-a ]] || action28z_outer_target=$node_b_target
    write_remote "$action28z_outer_role" "$action28z_outer_mode" \
        "$action28z_outer_revision" "$action28z_outer_source" "$action28z_outer_remote"
    ...
    "$ssh_binary" -T -F /dev/null -o BatchMode=yes -o ClearAllForwardings=yes \
        ...
        -o StrictHostKeyChecking=yes "$action28z_outer_target" \
        'cd / && sudo -n /bin/bash -s --' <"$action28z_outer_remote" \
        >"$remote_stdout" 2>"$remote_stderr" || remote_status=$?
    printf '%s\n' "$remote_status" >"$status_file"
    printf '%s_phase_%s_status=%s\n' "$prefix" "$action28z_outer_label" "$remote_status"
    ...
    grep -Fxq "action_28z_remote_${action28z_outer_mode}_acceptance=true" \
        "$remote_stdout"
}
start_availability() {
    local action28z_outer_label=$1

    availability_stop=$outer_root/availability-$action28z_outer_label.stop
    availability_samples=$outer_root/availability-$action28z_outer_label.samples
    availability_stderr=$outer_root/availability-$action28z_outer_label.stderr
    install -m 0600 /dev/null "$availability_samples" "$availability_stderr"
    (
        local action28z_outer_dns4_status
        local action28z_outer_dns6_status
        local action28z_outer_https4_status
        local action28z_outer_https6_status

        while [[ ! -e "$availability_stop" ]]; do
            action28z_outer_dns4_status=0
            action28z_outer_dns6_status=0
            action28z_outer_https4_status=0
            action28z_outer_https6_status=0
            dig +time=2 +tries=1 +short @10.1.0.55 \
                pihole-admin.local.theama.co A >/dev/null \
                2>>"$availability_stderr" || action28z_outer_dns4_status=$?
            dig +time=2 +tries=1 +short @fd36:5aa8:6971:1::55 \
                pihole-admin.local.theama.co AAAA >/dev/null \
                2>>"$availability_stderr" || action28z_outer_dns6_status=$?
            curl --fail --silent --show-error --max-time 5 \
                --resolve pihole-admin.local.theama.co:443:10.1.0.56 \
                https://pihole-admin.local.theama.co/admin/login.php \
                >/dev/null 2>>"$availability_stderr" || action28z_outer_https4_status=$?
            curl --fail --silent --show-error --max-time 5 \
                --resolve 'pihole-admin.local.theama.co:443:[fd36:5aa8:6971:1::56]' \
                https://pihole-admin.local.theama.co/admin/login.php \
                >/dev/null 2>>"$availability_stderr" || action28z_outer_https6_status=$?
            printf 'epoch=%s dns4_status=%s dns6_status=%s https4_status=%s https6_status=%s\n' \
                "$(date +%s)" "$action28z_outer_dns4_status" \
                "$action28z_outer_dns6_status" "$action28z_outer_https4_status" \
                "$action28z_outer_https6_status" >>"$availability_samples"
            sleep 1
        done
    ) &
    availability_pid=$!
...
stop_availability() {
    ...
    emit_stream "availability_${action28z_outer_label}_samples" "$availability_samples" || return 97
    emit_stream "availability_${action28z_outer_label}_stderr" "$availability_stderr" || return 97
    gate "availability_${action28z_outer_label}_sampled" test -s "$availability_samples" || return 1
    gate "availability_${action28z_outer_label}_continuous" \
        test -z "$(grep -E 'status=[1-9][0-9]*' "$availability_samples" || true)"
    availability_pid=
}
recover() {
    local action28z_outer_recovery_ok=true
    ...
    if [[ "$node_a_relinquished" = true ]]; then
        run_phase recovery_restore_owner node-a restore-owner || action28z_outer_recovery_ok=false
    fi
    if [[ "$node_a_installed" = true ]]; then
        run_phase recovery_rollback_node_a node-a rollback || action28z_outer_recovery_ok=false
    fi
    if [[ "$node_b_installed" = true ]]; then
        run_phase recovery_rollback_node_b node-b rollback || action28z_outer_recovery_ok=false
    fi
    run_phase recovery_node_a_master node-a state '' master || action28z_outer_recovery_ok=false
    run_phase recovery_node_b_backup node-b state '' backup || action28z_outer_recovery_ok=false
    printf '%s_recovery_proven=%s\n' "$prefix" "$action28z_outer_recovery_ok"
    [[ "$action28z_outer_recovery_ok" = true ]]
}
must_phase() {
    if run_phase "$@"; then
        return 0
...
if [[ "${1:-}" = --producer-self-test && $# -eq 1 ]]; then
    action28z_outer_producer_root=$(mktemp -d /tmp/action28z-producer-test.XXXXXX)
    readonly action28z_outer_producer_root
    trap 'rm -rf -- "$action28z_outer_producer_root"' EXIT INT TERM
    outer_root=$action28z_outer_producer_root
    run_id=${outer_root##*/}
    action28z_outer_producer_remote=$outer_root/remote.sh
    write_remote node-b preflight '' '' "$action28z_outer_producer_remote"
    prepare_capture_files "$outer_root/stdout" "$outer_root/stderr" "$outer_root/status"
    validate_remote_program "$action28z_outer_producer_remote"
    action28z_outer_malformed=$outer_root/malformed.sh
    # This is an exact generated-program literal, not an expression to expand.
    # shellcheck disable=SC2016
    sed 's|/bin/bash "$stage/transact-coupled-go-live-action28z.sh"|"$stage/transact-coupled-go-live-action28z.sh"|' \
        "$action28z_outer_producer_remote" >"$action28z_outer_malformed"
    if validate_remote_program "$action28z_outer_malformed"; then
...
emergency_revision=$(sed -n \
    's/^action_28z_remote_publish_value_revision=\([A-Za-z0-9][A-Za-z0-9._-]*\)$/\1/p' \
    "$last_stdout")
