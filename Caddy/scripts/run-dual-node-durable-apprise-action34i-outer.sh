...
readonly prefix=action_34i_outer
...
readonly action_transaction=$script_directory/apply-durable-apprise-action34h.sh
readonly regression=$caddy_root/tests/durable-apprise-action34i-regression.sh
...
readonly action_manifest=$caddy_root/manifests/durable-apprise-action34i.yaml
readonly expected_upload_transaction_sha256=34d43af6e72bcb40826eb8510cfe0a2d11369f8b7ec94529772e47217505675f
readonly expected_action_transaction_sha256=a909614f5d806e9c0da1d283090e5c8aa332ba9bf00a558707439899c5dac55c
readonly expected_regression_sha256=d9c06d97ded16bbab221c527964df5ac771ae36a2499ed12905b832d18bce825
...
readonly evidence_root=/tmp/caddy-ssh-evidence/action34i
...
build_payload() {
    ...
    install -m 0600 "$artifact_manifest" \
        "$action34i_outer_payload_root/Caddy/manifests/durable-apprise-action34h.tsv" || return 1
    ...
    payload_archive=$work_root/caddy-action34i-payload.tar
...
run_local_gates() {
    ...
    gate shellcheck shellcheck "$upload_transaction" "$regression" "${BASH_SOURCE[0]}" || return 1
    gate canonical_format /bin/bash "$caddy_root/tests/shfmt-canonical.sh" --check \
        "$upload_transaction" "$regression" "${BASH_SOURCE[0]}" || return 1
