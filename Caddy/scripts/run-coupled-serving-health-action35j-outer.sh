...
readonly prefix=action_35_j_outer
readonly transaction_sha256=c3ae04896a4c84bdd67d5e8c6de13b161ae09f2e44e99c709d0c7a2ec9e28b24
...
safe_capture() {
    local action35j_path=$1

    regular_file "$action35j_path"
    [[ "$(stat -c '%s' "$action35j_path")" -le "$max_stream_bytes" ]]
    iconv -f UTF-8 -t UTF-8 "$action35j_path" >/dev/null
    ! LC_ALL=C grep -Paq '[\x00-\x08\x0b\x0c\x0e-\x1f\x7f]' "$action35j_path"
}

capture() {
    local action35j_label=$1
    shift
    local action35j_stdout=$workstation_evidence/$action35j_label.stdout
    local action35j_stderr=$workstation_evidence/$action35j_label.stderr
    local action35j_status=$workstation_evidence/$action35j_label.status
    ...
    safe_capture "$action35j_stdout"
    safe_capture "$action35j_stderr"
    return "$action35j_rc"
}

build_payload() {
    local action35j_repository action35j_source action35j_target action35j_mode
    local action35j_hash action35j_lifecycle action35j_source_path action35j_destination
    ...
    while IFS=$'\t' read -r action35j_repository action35j_source action35j_target \
        action35j_mode action35j_hash action35j_lifecycle; do
        [[ "$action35j_repository" = '# repository' ]] && continue
        [[ "$action35j_mode" =~ ^0[0-7]{3}$ && "$action35j_lifecycle" = production-candidate ]]
        [[ "$action35j_target" = /* ]]
        action35j_source_path=${workspace_root}/$action35j_repository/$action35j_source
        regular_file "$action35j_source_path"
        [[ "$(sha256sum "$action35j_source_path" | awk '{ print $1 }')" = "$action35j_hash" ]]
        action35j_destination=$payload_stage/repositories/$action35j_repository/$action35j_source
        install -d -m 0700 "$(dirname -- "$action35j_destination")"
        install -m 0600 "$action35j_source_path" "$action35j_destination"
...
readonly archive=$2
case "$remote_root" in /tmp/caddy-action35j-*) ;; *) exit 64 ;; esac
case "$archive" in /tmp/caddy-action35j-*.tar) ;; *) exit 64 ;; esac
...
case "$evidence_root" in /tmp/caddy-action35j-*) ;; *) exit 64 ;; esac
...
ssh_stream() {
    local action35j_host=$1
    local action35j_program=$2
    shift 2
    "$ssh_command" "$action35j_host" \
        "cd / && sudo -n /bin/bash -s --$(printf ' %q' "$@")" <"$action35j_program"
}

upload_payload() {
    local action35j_role=$1
    local action35j_host=$2
    local action35j_remote_root=$3
    local action35j_remote_archive=$4
    local action35j_hash

    action35j_hash=$(awk '{ print $1 }' "$workstation_evidence/payload.sha256")
    capture "$action35j_role-upload-prepare" ssh_stream "$action35j_host" \
        "$prepare_program" "$action35j_remote_root" "$action35j_remote_archive"
    capture "$action35j_role-upload-copy" "$scp_command" -p -- \
        "$payload_archive" "$action35j_host:$action35j_remote_archive"
    capture "$action35j_role-upload-accept" ssh_stream "$action35j_host" \
        "$accept_program" "$action35j_remote_root" "$action35j_remote_archive" \
        "$action35j_hash"
}
...
remote_transaction() {
    ...
    capture "$action35j_label" ssh_stream "$action35j_host" "$transaction" "$@"
}

readback() {
    local action35j_role=$1
    local action35j_host=$2
    local action35j_remote_evidence=$3

    capture "$action35j_role-readback" ssh_stream "$action35j_host" \
        "$readback_program" "$action35j_remote_evidence"
}
...
cleanup_remote() {
    ...
    capture "$action35j_role-disposition" ssh_stream "$action35j_host" \
        "$disposition_program" "$action35j_remote_root" "$action35j_remote_archive"
}

run_live() {
    local action35j_node_b_mutated=false
    local action35j_node_a_mutated=false
    local action35j_failure=0
    local action35j_phase_status=0
    ...
    action35j_node_b_mutated=true
    if [[ "$action35j_failure" -eq 0 ]]; then
    ...
    if ! remote_transaction node-a-journal-capture "$node_a_host" journal-capture node-a \
        "$node_a_payload" "$node_a_evidence"; then
        [[ "$action35j_failure" -ne 0 ]] || action35j_failure=1
    fi
    ...
    if [[ "$action35j_failure" -ne 0 ]]; then
        if [[ "$action35j_node_a_mutated" = true ]]; then
            remote_transaction node-a-rollback "$node_a_host" rollback node-a \
                "$node_a_payload" "$node_a_evidence" || exit 125
        fi
        if [[ "$action35j_node_b_mutated" = true ]]; then
...
production_path_test() {
    ...
    local action35j_remote_base action35j_raw action35j_decision action35j_observed
    local action35j_status action35j_role action35j_remote_evidence action35j_test_id
    ...
    action35j_remote_base=$(mktemp -d /tmp/caddy-action35j-production-path.XXXXXX)
    action35j_test_id=${action35j_remote_base##*.}
    test_remote_base=$action35j_remote_base
    export ACTION35J_TEST_REMOTE_BASE=$test_remote_base
...
[[ "$1" = -n ]]
shift
export ACTION35J_PRODUCTION_PATH_TEST=1
exec "$@"
SUDO
...
set -Eeuo pipefail
[[ "$1" = -u && "$3" = -- ]]
printf '%s\n' "$2" >>"$ACTION35J_TEST_REMOTE_BASE/runuser.identities"
shift 3
exec "$@"
RUNUSER
...
set -Eeuo pipefail
case " $* " in
    *' --show-cursor '*) printf '%s\n' '-- cursor: s=action35j-production-path' ;;
    *' --after-cursor '*) printf '%s\n' 'test-host caddy.service: action35j production-path event' ;;
    *) exit 64 ;;
esac
JOURNALCTL
    cat >"$ssh_command" <<'SSH'
...
shift
command="$*"
printf '%s\t%s\n' "$host" "$command" >>"$ACTION35J_TEST_REMOTE_BASE/ssh.calls"
PATH="$ACTION35J_TEST_REMOTE_BASE/bin:/usr/bin:/bin" /bin/bash -c "$command"
SSH
    cat >"$scp_command" <<'SCP'
#!/usr/bin/env bash
set -Eeuo pipefail
...
source=$3
target=${4#*:}
printf '%s\t%s\n' "$source" "$target" >>"$ACTION35J_TEST_REMOTE_BASE/scp.calls"
...
SCP
    ...
    node_a_payload=/tmp/caddy-action35j-test-$action35j_test_id-node-a
    node_b_payload=/tmp/caddy-action35j-test-$action35j_test_id-node-b
    ...
    ACTION35J_ENVIRONMENT_FILE=$test_remote_base/environment-node-a \
        ACTION35J_KEEPALIVED_COMMAND=$test_remote_base/bin/keepalived \
        ACTION35J_RUNUSER_COMMAND=$test_remote_base/bin/runuser \
        ACTION35J_SYSTEMCTL_COMMAND=$test_remote_base/bin/systemctl \
        ACTION35J_DNS_DIG_COMMAND=$test_remote_base/bin/dig \
        ACTION35J_CURL_COMMAND=$test_remote_base/bin/curl \
        ACTION35J_SS_COMMAND=$test_remote_base/bin/ss \
        remote_transaction node-a-candidate-producer "$node_a_host" candidate-check \
        node-a "$node_a_payload" "$node_a_evidence"
    ACTION35J_ENVIRONMENT_FILE=$test_remote_base/environment-node-b \
        ACTION35J_KEEPALIVED_COMMAND=$test_remote_base/bin/keepalived \
        ACTION35J_RUNUSER_COMMAND=$test_remote_base/bin/runuser \
        ACTION35J_SYSTEMCTL_COMMAND=$test_remote_base/bin/systemctl \
        ACTION35J_DNS_DIG_COMMAND=$test_remote_base/bin/dig \
    ...
    ACTION35J_JOURNALCTL_COMMAND=$test_remote_base/bin/journalctl \
    ...
    ACTION35J_TARGET_ROOT=$test_remote_base/node-b-root \
        ACTION35J_SYSTEMCTL_COMMAND=$test_remote_base/bin/systemctl \
        remote_transaction node-b-install-producer "$node_b_host" install node-b \
        "$node_b_payload" "$node_b_evidence"
    ACTION35J_TARGET_ROOT=$test_remote_base/node-a-root \
        ACTION35J_SYSTEMCTL_COMMAND=$test_remote_base/bin/systemctl \
    ...
    action35j_raw=$action35j_test_root/raw/outer-preflight.txt
    action35j_decision=$action35j_test_root/decisions/outer-preflight.tsv
    cat "$test_remote_base/ssh.calls" "$test_remote_base/scp.calls" \
        "$test_remote_base/runuser.identities" >"$action35j_raw"
    action35j_observed=$(wc -l <"$action35j_raw")
    write_decision outer-preflight reach 0 "$action35j_observed" \
        "$action35j_observed" "$action35j_raw" "$action35j_decision"

    for action35j_role in node-a node-b; do
        if [[ "$action35j_role" = node-a ]]; then
            action35j_remote_evidence=$node_a_evidence
        else
            action35j_remote_evidence=$node_b_evidence
        fi
        action35j_raw=$action35j_test_root/raw/evidence-readback-$action35j_role-success.txt
        action35j_decision=$action35j_test_root/decisions/evidence-readback-$action35j_role-success.tsv
        ssh_stream "test-$action35j_role" "$readback_program" \
            "$action35j_remote_evidence" >"$action35j_raw"
        action35j_observed=$(sha256sum "$action35j_raw" | awk '{ print $1 }')
        write_decision "evidence-readback-$action35j_role-success" accept 0 \
            "$action35j_observed" "$action35j_observed" "$action35j_raw" "$action35j_decision"

        action35j_raw=$action35j_test_root/raw/evidence-readback-$action35j_role-failure.txt
        action35j_decision=$action35j_test_root/decisions/evidence-readback-$action35j_role-failure.tsv
        if ssh_stream "test-$action35j_role" "$readback_program" \
            "$action35j_remote_evidence-absent" >"$action35j_raw" 2>&1; then
            action35j_status=0
        else
            action35j_status=$?
        fi
        write_decision "evidence-readback-$action35j_role-failure" reject \
            "$action35j_status" present absent "$action35j_raw" "$action35j_decision"
    done
    cleanup_remote node-a "$node_a_host" "$node_a_payload" "$node_a_archive"
    cleanup_remote node-b "$node_b_host" "$node_b_payload" "$node_b_archive"
    rm -rf -- "$action35j_remote_base"
...
transaction=$repository_root/Caddy/scripts/apply-coupled-serving-health-action35j.sh
...
if [[ "$invocation_mode" = --production-path-test ]]; then
    workstation_evidence=$(mktemp -d /tmp/caddy-action35j-outer-test.XXXXXX)
else
    workstation_evidence=$(mktemp -d /tmp/caddy-ssh-evidence-action35j.XXXXXX)
fi
readonly workstation_evidence
chmod 0700 "$workstation_evidence"
payload_stage=$workstation_evidence/payload
payload_archive=$workstation_evidence/action35j-payload.tar
...
readonly run_id
node_a_payload=/tmp/caddy-action35j-$run_id-node-a
node_b_payload=/tmp/caddy-action35j-$run_id-node-b
node_a_archive=$node_a_payload.tar
node_b_archive=$node_b_payload.tar
node_a_evidence=$node_a_payload/evidence
node_b_evidence=$node_b_payload/evidence
