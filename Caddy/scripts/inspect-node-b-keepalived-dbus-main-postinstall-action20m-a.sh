...
readonly prior_main_sha256=8b260315628888c937c6f7eaf21f63e73eb52cfd9cd6e9c0d9d90fa9cf9fbd3f
...
readonly backup_directory=/var/backups/caddy-ha/action20m-node-b-dbus-main.wHwzci
...
action20ma_failed_check_count=0
action20ma_first_failure=none

file_hash() { sha256sum "$1" | awk '{ print $1 }'; }
address_count() {
    local action20ma_address_family=$1
    local action20ma_expected_cidr=$2

    ip -o "-$action20ma_address_family" address show dev eth0 2>/dev/null |
        awk -v expected="$action20ma_expected_cidr" '$4 == expected { count++ }
            END { print count + 0 }'
}
matching_path_count() {
    local action20ma_parent=$1
    local action20ma_pattern=$2

    find "$action20ma_parent" -mindepth 1 -maxdepth 1 -name "$action20ma_pattern" \
...
expected_checks() {
    printf '%s\n' \
        ...
        keepalived_restarts_unchanged vrrp_state_backup_after \
...
record_check() {
    local action20ma_check_label=$1

    shift
    if "$@"; then
        printf '%s_check_%s=true\n' "$prefix" "$action20ma_check_label"
    else
        printf '%s_check_%s=false\n' "$prefix" "$action20ma_check_label"
        action20ma_failed_check_count=$((action20ma_failed_check_count + 1))
        if [[ "$action20ma_first_failure" = none ]]; then
            action20ma_first_failure=$action20ma_check_label
        fi
    fi
    return 0
}
global_defs_contains_enable_dbus() {
    local action20ma_configuration_path=$1
    ...
    ' "$action20ma_configuration_path"
}
runtime_residue_count() {
    matching_path_count /run 'caddy-action20m*'
}
install_residue_count() {
    matching_path_count /etc/keepalived '.keepalived.conf.action20m*'
}
state_snapshot() {
    printf 'main=%s|%s\n' \
...
self_test() {
    local action20ma_fixture_root
    local action20ma_fixture_main
    ...
    action20ma_fixture_root=$(mktemp -d /tmp/caddy-action20m-a-self.XXXXXX) || return 1
    trap 'rm -rf -- "$action20ma_fixture_root"' RETURN
    action20ma_fixture_main=$action20ma_fixture_root/keepalived.conf
    printf '%s\n' 'global_defs {' '    enable_dbus' '}' '' "$include_record" >"$action20ma_fixture_main"
    global_defs_contains_enable_dbus "$action20ma_fixture_main" || return 1
    [[ "$(tail -n 1 "$action20ma_fixture_main")" = "$include_record" ]] || return 1
    sed '/enable_dbus/d' "$action20ma_fixture_main" >"$action20ma_fixture_root/missing"
    if global_defs_contains_enable_dbus "$action20ma_fixture_root/missing"; then
        return 1
    fi
...
action20ma_keepalived_pid_before=$(systemctl show keepalived.service --property MainPID --value 2>/dev/null || true)
readonly action20ma_keepalived_pid_before
action20ma_keepalived_restarts_before=$(systemctl show keepalived.service --property NRestarts --value 2>/dev/null || true)
readonly action20ma_keepalived_restarts_before
action20ma_backup_manifest_sha256=$(file_hash "$backup_manifest" 2>/dev/null || true)
readonly action20ma_backup_manifest_sha256
action20ma_state_before=$(state_snapshot | sha256sum | awk '{ print $1 }')
readonly action20ma_state_before
...
record_check backup_manifest_action grep -Fqx action=20m "$backup_manifest"
record_check backup_manifest_node grep -Fqx node=node-b "$backup_manifest"
...
record_check backup_count_exact test "$(matching_path_count /var/backups/caddy-ha 'action20m-node-b-dbus-main.*')" -eq 1
...
record_check keepalived_pid_numeric_before test "$action20ma_keepalived_pid_before" -gt 0
record_check keepalived_restarts_numeric_before test "$action20ma_keepalived_restarts_before" -ge 0
record_check vrrp_state_backup_before test "$(sed -n '1p' /run/caddy-ha/vrrp-state 2>/dev/null || true)" = BACKUP
record_check caddy_ipv4_count_before test "$(address_count 4 "$vip_ipv4_cidr")" -eq 0
record_check caddy_ipv6_count_before test "$(address_count 6 "$vip_ipv6_cidr")" -eq 0
record_check dns_ipv4_count_before test "$(address_count 4 "$dns_ipv4_cidr")" -eq 0
record_check dns_ipv6_count_before test "$(address_count 6 "$dns_ipv6_cidr")" -eq 0
...
record_check keepalived_pid_unchanged test "$(systemctl show keepalived.service --property MainPID --value 2>/dev/null || true)" = "$action20ma_keepalived_pid_before"
record_check keepalived_restarts_unchanged test "$(systemctl show keepalived.service --property NRestarts --value 2>/dev/null || true)" = "$action20ma_keepalived_restarts_before"
record_check vrrp_state_backup_after test "$(sed -n '1p' /run/caddy-ha/vrrp-state 2>/dev/null || true)" = BACKUP
record_check caddy_ipv4_count_after test "$(address_count 4 "$vip_ipv4_cidr")" -eq 0
record_check caddy_ipv6_count_after test "$(address_count 6 "$vip_ipv6_cidr")" -eq 0
record_check dns_ipv4_count_after test "$(address_count 4 "$dns_ipv4_cidr")" -eq 0
record_check dns_ipv6_count_after test "$(address_count 6 "$dns_ipv6_cidr")" -eq 0
...
record_check backup_manifest_hash_unchanged test "$(file_hash "$backup_manifest" 2>/dev/null || true)" = "$action20ma_backup_manifest_sha256"
...
action20ma_state_after=$(state_snapshot | sha256sum | awk '{ print $1 }')
readonly action20ma_state_after
record_check state_snapshot_unchanged test "$action20ma_state_after" = "$action20ma_state_before"
...
printf '%s_value_before_state_sha256=%s\n' "$prefix" "$action20ma_state_before"
printf '%s_value_after_state_sha256=%s\n' "$prefix" "$action20ma_state_after"
printf '%s_check_count=%s\n' "$prefix" "$expected_check_count"
printf '%s_failed_check_count=%s\n' "$prefix" "$action20ma_failed_check_count"
printf '%s_first_failure=%s\n' "$prefix" "$action20ma_first_failure"
...
[[ "$action20ma_failed_check_count" -eq 0 ]]
