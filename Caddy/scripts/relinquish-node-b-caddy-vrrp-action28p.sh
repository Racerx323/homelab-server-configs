...
if [[ "${CADDY_ACTION28P_TEST_MODE:-}" = 1 ]]; then
    PATH=${CADDY_ACTION28P_TEST_PATH:-/usr/local/sbin:/usr/local/bin:/usr/sbin:/usr/bin:/sbin:/bin}
else
...
readonly prefix=action_28p
fixture_root=${CADDY_ACTION28P_FIXTURE_ROOT:-}
[[ -z "$fixture_root" || "${CADDY_ACTION28P_TEST_MODE:-}" = 1 ]] || exit 64
...
readonly backup_directory=$fixture_root/var/backups/caddy-ha/action28p-node-b-caddy-vrrp-relinquish
readonly legacy_action28n_backup_directory=$fixture_root/var/backups/caddy-ha/action28n-node-b-caddy-vrrp-relinquish
...
expected_checks() {
    printf '%s\n' \
        ...
        legacy_action_28n_backup_absent legacy_action_28n_residue_absent backup_absent \
...
file_metadata() {
    local action28p_metadata_path=$1

    if [[ -n "$fixture_root" ]]; then
        printf 'root:root:%s\n' "$(stat -c '%a' "$action28p_metadata_path")"
    else
        stat -c '%U:%G:%a' "$action28p_metadata_path"
    fi
}
address_query() { ip -o "-$1" address show dev "$interface"; }
address_count() {
    local action28p_family=$1
    local action28p_cidr=$2

    address_query "$action28p_family" |
        awk -v expected="$action28p_cidr" '$4 == expected { count++ } END { print count + 0 }'
}
safe_stream() {
    local action28p_stream=$1

    [[ "$(wc -c <"$action28p_stream")" -le "$maximum_stream_bytes" ]] || return 1
    [[ "$(line_count "$action28p_stream")" -le "$maximum_stream_lines" ]] || return 1
    ! LC_ALL=C grep -n '[^[:print:][:space:]]' "$action28p_stream" >/dev/null || return 1
    ! grep -Eqi 'BEGIN [A-Z ]*PRIVATE KEY|Authorization:[[:space:]]*Bearer|WEBPASSWORD' "$action28p_stream"
}
emit_stream() {
    local action28p_label=$1
    local action28p_stream=$2

    printf '%s_capture_%s_bytes=%s\n' "$prefix" "$action28p_label" "$(wc -c <"$action28p_stream")"
    printf '%s_capture_%s_lines=%s\n' "$prefix" "$action28p_label" "$(line_count "$action28p_stream")"
    printf '%s_capture_%s_sha256=%s\n' "$prefix" "$action28p_label" "$(file_hash "$action28p_stream")"
    if safe_stream "$action28p_stream"; then
        printf '%s_capture_%s_classification=bounded_safe\n' "$prefix" "$action28p_label"
        if [[ -s "$action28p_stream" ]]; then
            printf '%s_capture_%s_begin\n' "$prefix" "$action28p_label"
            sed "s/^/${prefix}_capture_${action28p_label}_content=/" "$action28p_stream"
...
require_check() {
    local action28p_required_label=$1

    shift
    check "$action28p_required_label" "$@" || {
        first_failure=$action28p_required_label
        return 1
    }
}
rollback_check() {
    local action28p_rollback_label=$1

    shift
    if "$@" >/dev/null 2>&1; then
        printf '%s_rollback_check_%s=true\n' "$prefix" "$action28p_rollback_label"
        return 0
    fi
...
require_rollback_check() {
    local action28p_rollback_required_label=$1

    shift
    rollback_check "$action28p_rollback_required_label" "$@" || {
        rollback_first_failure=$action28p_rollback_required_label
        return 1
    }
}
candidate_contract() {
    local action28p_candidate_file=$1

    [[ "$(file_hash "$action28p_candidate_file")" = "$retired_main_sha256" ]] || return 1
    [[ "$(grep -Fxc "$include_line" "$action28p_candidate_file" || true)" -eq 0 ]] || return 1
    [[ "$(grep -Fxc 'vrrp_instance PIHOLE_IPV4 {' "$action28p_candidate_file" || true)" -eq 1 ]] || return 1
    [[ "$(grep -Fxc 'vrrp_instance PIHOLE_IPV6 {' "$action28p_candidate_file" || true)" -eq 1 ]] || return 1
    [[ "$(grep -Fxc '        10.1.0.55/22 dev eth0' "$action28p_candidate_file" || true)" -eq 1 ]] || return 1
    [[ "$(grep -Fxc '        fd36:5aa8:6971:1::55/128 dev eth0 preferred_lft forever' "$action28p_candidate_file" || true)" -eq 1 ]] || return 1
    [[ "$(grep -Fxc '        10.1.0.56/22 dev eth0' "$action28p_candidate_file" || true)" -eq 0 ]] || return 1
    [[ "$(grep -Fxc '        fd36:5aa8:6971:1::56/128 dev eth0 preferred_lft forever' "$action28p_candidate_file" || true)" -eq 0 ]]
}
candidate_byte_boundary() {
    local action28p_source_file=$1
    local action28p_candidate_file=$2
    local action28p_rebuilt

    action28p_rebuilt=$(mktemp /tmp/caddy-action28p-boundary.XXXXXX) || return 1
    sed '$d' "$action28p_source_file" | sed '$d' >"$action28p_rebuilt" || {
        rm -f -- "$action28p_rebuilt"
        return 1
    }
    cmp -s "$action28p_rebuilt" "$action28p_candidate_file"
    local action28p_boundary_status=$?
    rm -f -- "$action28p_rebuilt"
    return "$action28p_boundary_status"
}
https_status() {
    local action28p_address=$1
    local action28p_status=0
    local action28p_code

    action28p_code=$(curl --noproxy '*' --insecure --silent --show-error --location \
        --max-redirs 3 --proto-redir '=https' --connect-timeout 3 --max-time 10 \
        --resolve "${node_b_fqdn}:443:${action28p_address}" --output /dev/null \
        --write-out '%{http_code}' "https://${node_b_fqdn}/admin/") || action28p_status=$?
    [[ "$action28p_status" -eq 0 && "$action28p_code" = 200 ]]
}
run_captured() {
    local action28p_capture_label=$1
    local action28p_capture_status=0

    shift
    install -m 0600 /dev/null "$capture_directory/${action28p_capture_label}.stdout"
    install -m 0600 /dev/null "$capture_directory/${action28p_capture_label}.stderr"
    "$@" >"$capture_directory/${action28p_capture_label}.stdout" \
        2>"$capture_directory/${action28p_capture_label}.stderr" || action28p_capture_status=$?
    emit_stream "${action28p_capture_label}_stdout" "$capture_directory/${action28p_capture_label}.stdout" || return 97
    emit_stream "${action28p_capture_label}_stderr" "$capture_directory/${action28p_capture_label}.stderr" || return 97
    printf '%s_capture_%s_status=%s\n' "$prefix" "$action28p_capture_label" "$action28p_capture_status"
    [[ "$action28p_capture_status" -eq 0 ]]
}
wait_relinquished() {
...
recovery_check() {
    local action28p_recovery_label=$1

    shift
    if "$@" >/dev/null 2>&1; then
        printf '%s_recovery_check_%s=true\n' "$prefix" "$action28p_recovery_label"
        return 0
    fi
...
rollback_live() {
    local action28p_rollback_status=0
    ...
    if [[ -f "$backup_directory/keepalived.conf" ]]; then
        install -o root -g root -m 0644 "$backup_directory/keepalived.conf" \
            "$main_configuration" || action28p_rollback_status=125
        if [[ "$action28p_rollback_status" -eq 0 ]]; then
            action28p_recovery_reload_status=0
            run_captured rollback_reload systemctl reload keepalived.service ||
                action28p_recovery_reload_status=$?
            recovery_check reload_status_zero test "$action28p_recovery_reload_status" -eq 0 ||
                action28p_rollback_status=125
            reload_count=$((reload_count + 1))
        fi
        if [[ "$action28p_rollback_status" -eq 0 ]]; then
            recovery_check caddy_vips_restored_within_bound wait_restored || action28p_rollback_status=125
        fi
    else
        action28p_rollback_status=125
    fi
    recovery_check main_restored test "$(file_hash "$main_configuration" 2>/dev/null || true)" = "$baseline_main_sha256" || action28p_rollback_status=125
    recovery_check keepalived_active systemctl is-active --quiet keepalived.service || action28p_rollback_status=125
    recovery_check caddy_active systemctl is-active --quiet caddy.service || action28p_rollback_status=125
    recovery_check dns_ipv4_absent test "$(address_count 4 "$dns_ipv4_cidr" 2>/dev/null || true)" -eq 0 || action28p_rollback_status=125
    recovery_check dns_ipv6_absent test "$(address_count 6 "$dns_ipv6_cidr" 2>/dev/null || true)" -eq 0 || action28p_rollback_status=125
    recovery_check caddy_ipv4_owned test "$(address_count 4 "$caddy_ipv4_cidr" 2>/dev/null || true)" -eq 1 || action28p_rollback_status=125
    recovery_check caddy_ipv6_owned test "$(address_count 6 "$caddy_ipv6_cidr" 2>/dev/null || true)" -eq 1 || action28p_rollback_status=125
    printf '%s_rollback_status=%s\n' "$prefix" "$action28p_rollback_status"
    return "$action28p_rollback_status"
}
cleanup() {
    local action28p_cleanup_status=$?

    trap - EXIT ERR HUP INT TERM
    if [[ "$action28p_cleanup_status" -ne 0 && "$mutation_started" = true && "$transaction_complete" != true ]]; then
        rollback_live || action28p_cleanup_status=125
    fi
    ...
    if [[ "$action28p_cleanup_status" -eq 0 ]]; then
        printf '%s_acceptance=true\n' "$prefix"
    else
        printf '%s_acceptance=false\n' "$prefix"
    fi
    exit "$action28p_cleanup_status"
}
external_rollback() {
    local action28p_rollback_stage_path

    first_failure=none
    rollback_first_failure=none
    transaction_root=$(mktemp -d "$run_root/caddy-action28p-rollback.XXXXXX")
    action28p_rollback_stage_path=$transaction_root
    ...
    local action28p_external_reload_status=0
    run_captured external_rollback_reload systemctl reload keepalived.service ||
        action28p_external_reload_status=$?
    require_rollback_check reload_status_zero test "$action28p_external_reload_status" -eq 0 || return 125
    ...
    local action28p_ipv4_status=0 action28p_ipv6_status=0
    address_query 4 >/dev/null 2>&1 || action28p_ipv4_status=$?
    address_query 6 >/dev/null 2>&1 || action28p_ipv6_status=$?
    require_rollback_check ipv4_query_status_zero test "$action28p_ipv4_status" -eq 0 || return 125
    require_rollback_check ipv6_query_status_zero test "$action28p_ipv6_status" -eq 0 || return 125
    ...
    transaction_root=
    require_rollback_check transaction_stage_removed test ! -e "$action28p_rollback_stage_path" || return 125
...
require_check legacy_action_28n_backup_absent test ! -e "$legacy_action28n_backup_directory"
require_check legacy_action_28n_residue_absent test -z \
    "$(find "$run_root" -maxdepth 1 -name 'caddy-action28n-*' -print -quit 2>/dev/null)"
require_check backup_absent test ! -e "$backup_directory"
require_check transaction_residue_absent test -z "$(find "$run_root" -maxdepth 1 -name 'caddy-action28p-*' -print -quit 2>/dev/null)"
...
transaction_root=$(mktemp -d "$run_root/caddy-action28p-stage.XXXXXX")
...
require_check backup_manifest_action grep -Fqx 'action=28p' "$backup_directory/manifest"
