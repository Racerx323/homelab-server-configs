...
expected_assertion_labels() {
    printf '%s\n' \
        ...
        action17s_work_directory_count_zero \
        current_link_exact \
        ...
        finalizer_require_check_passes_child_stdout \
        finalizer_validate_block_single \
        finalizer_validate_block_line_count_exact \
        finalizer_validate_block_hash_exact \
        finalizer_validate_invocation_single \
        finalizer_validate_stdout_redirect_absent \
        finalizer_validate_stderr_redirect_absent \
        release_state_unchanged \
        current_link_unchanged \
        current_target_unchanged \
        caddy_active_unchanged \
        lighttpd_active_unchanged \
        lsyncd_active_state_unchanged \
        lsyncd_unit_file_state_unchanged \
        caddy_lsyncd_active_state_unchanged \
        reconcile_path_active_state_unchanged \
...
work_directory=$(mktemp -d /tmp/caddy-action17s-a-node-b.XXXXXX)
...
record_command action17s_backup_count_zero \
    test "$(find "$rollback_root" -mindepth 1 -maxdepth 1 \
        -type d -name 'action17s-node-b-marker-migration.*' -print | wc -l)" -eq 0
record_command action17s_work_directory_count_zero \
    test "$(find /run -mindepth 1 -maxdepth 1 \
        -type d -name 'caddy-action17s-node-b.*' -print | wc -l)" -eq 0
...
record_command finalizer_require_check_passes_child_stdout \
    grep -Fq 'if "$@"; then' "$finalizer"
validate_block >"$validate_source"
record_command finalizer_validate_block_single \
    test "$(grep -Fc 'require_check caddy_configuration_valid' "$finalizer")" -eq 1
record_command finalizer_validate_block_line_count_exact \
    test "$(wc -l <"$validate_source")" -eq 7
record_command finalizer_validate_block_hash_exact \
    test "$(file_hash "$validate_source")" = "$expected_validate_block_sha256"
record_command finalizer_validate_invocation_single \
    test "$(grep -Ec '^[[:space:]]*caddy validate ' "$finalizer")" -eq 1
record_command finalizer_validate_stdout_redirect_absent \
    test "$(grep -Ec '(^|[[:space:]])(1?>|&>)[^&]' "$validate_source")" -eq 0
record_command finalizer_validate_stderr_redirect_absent \
    test "$(grep -Ec '(^|[[:space:]])2>' "$validate_source")" -eq 0
...
readonly before_caddy_active before_lighttpd_active before_lsyncd_active
readonly before_lsyncd_enabled before_caddy_lsyncd_active
readonly before_reconcile_path_active before_reconcile_service_active

release_snapshot >"$after_release"
record_command release_state_unchanged \
    test "$(file_hash "$after_release")" = "$before_release_sha256"
record_command current_link_unchanged \
    test "$(readlink /etc/caddy/current)" = "$before_current_link"
record_command current_target_unchanged \
    test "$(readlink -e /etc/caddy/current)" = "$before_current_target"
record_command caddy_active_unchanged \
    test "$(systemctl is-active caddy.service)" = "$before_caddy_active"
record_command lighttpd_active_unchanged \
    test "$(systemctl is-active lighttpd.service)" = "$before_lighttpd_active"
record_command lsyncd_active_state_unchanged \
    test "$(systemctl is-active lsyncd.service 2>/dev/null || true)" = \
    "$before_lsyncd_active"
record_command lsyncd_unit_file_state_unchanged \
    test "$(systemctl is-enabled lsyncd.service 2>/dev/null || true)" = \
    "$before_lsyncd_enabled"
record_command caddy_lsyncd_active_state_unchanged \
    test "$(systemctl is-active caddy-lsyncd.service 2>/dev/null || true)" = \
    "$before_caddy_lsyncd_active"
record_command reconcile_path_active_state_unchanged \
    test "$(systemctl is-active caddy-sync-reconcile.path 2>/dev/null || true)" = \
    "$before_reconcile_path_active"
record_command reconcile_service_active_state_unchanged \
...
printf '%s_value_validate_block_lines=7\n' "$prefix"
printf '%s_value_validate_block_sha256=%s\n' \
    "$prefix" "$expected_validate_block_sha256"
printf '%s_value_stdout_source_classification=unsuppressed_caddy_validate_success_path\n' \
    "$prefix"
printf '%s_prior_stdout_bytes_recoverable=false\n' "$prefix"
printf '%s_prior_stdout_lines_recoverable=false\n' "$prefix"
printf '%s_prior_stdout_sha256_recoverable=false\n' "$prefix"
printf '%s_finalizer_invoked=false\n' "$prefix"
printf '%s_raw_stdout_source_emitted=false\n' "$prefix"
