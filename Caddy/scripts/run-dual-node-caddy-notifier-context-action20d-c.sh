...
readonly ssh_binary=${CADDY_ACTION20DC_SSH_BINARY:-/usr/bin/ssh}
...
validate_probe() {
    ...
    validation_observed_count=$(grep -Ec '^action_20d_c_probe_assertion_[a-z0-9_]+=(true|false)$' "$validation_stdout") || return 1
    [[ "$validation_observed_count" -eq "$validation_expected_count" ]] || return 1
    [[ "$(grep '^action_20d_c_probe_assertion_' "$validation_stdout" | cut -d= -f1 | LC_ALL=C sort | uniq -d | wc -l)" -eq 0 ]] || return 1
    while IFS= read -r validation_label; do
        require_one "action_20d_c_probe_assertion_${validation_label}=true" "$validation_stdout" || return 1
    done < <("$probe" --expected-assertions)
    require_one "action_20d_c_probe_value_node_role=$validation_role" "$validation_stdout" || return 1
    require_one "action_20d_c_probe_assertion_count=$validation_expected_count" "$validation_stdout" || return 1
    require_one 'action_20d_c_probe_failed_assertion_count=0' "$validation_stdout" || return 1
    require_one 'action_20d_c_probe_first_failure=none' "$validation_stdout" || return 1
    require_one 'action_20d_c_probe_notification_helper_invoked=false' "$validation_stdout" || return 1
    for validation_marker in filesystem_mutations service_mutations keepalived_mutations \
        ...
        require_one "action_20d_c_probe_${validation_marker}=false" "$validation_stdout" || return 1
    done
    require_one 'action_20d_c_probe_remote_complete=true' "$validation_stdout"
}
run_probe() {
    local probe_role=$1
...
write_probe_fixture() {
    ...
    fixture_dedupe_metadata=pi:pi:750
    {
        ...
        printf '%s\n' \
            ...
            "action_20d_c_probe_value_state_directory_metadata=$fixture_state_metadata" \
            "action_20d_c_probe_value_dedupe_directory_metadata=$fixture_dedupe_metadata" \
...
run_self_test() {
    ...
    printf '%s\n' \
        'action_20d_c_probe_assertion_state_directory_writable_as_pi=false' \
        >>"$self_root/node-a"
...
node_a_user=$(extract_one action_20d_c_probe_value_inherited_execution_user "$node_a_stdout" 2>/dev/null || true)
node_b_user=$(extract_one action_20d_c_probe_value_inherited_execution_user "$node_b_stdout" 2>/dev/null || true)
...
node_a_state_metadata=$(extract_one action_20d_c_probe_value_state_directory_metadata "$node_a_stdout" 2>/dev/null || true)
node_b_state_metadata=$(extract_one action_20d_c_probe_value_state_directory_metadata "$node_b_stdout" 2>/dev/null || true)
node_a_dedupe_metadata=$(extract_one action_20d_c_probe_value_dedupe_directory_metadata "$node_a_stdout" 2>/dev/null || true)
node_b_dedupe_metadata=$(extract_one action_20d_c_probe_value_dedupe_directory_metadata "$node_b_stdout" 2>/dev/null || true)
...
run_assertion node_a_caddy_ipv4_vip_absent require_one \
    'action_20d_c_probe_value_caddy_ipv4_vip_count=0' "$node_a_stdout"
run_assertion node_a_caddy_ipv6_vip_absent require_one \
    'action_20d_c_probe_value_caddy_ipv6_vip_count=0' "$node_a_stdout"
run_assertion node_b_caddy_ipv4_vip_absent require_one \
    'action_20d_c_probe_value_caddy_ipv4_vip_count=0' "$node_b_stdout"
run_assertion node_b_caddy_ipv6_vip_absent require_one \
    'action_20d_c_probe_value_caddy_ipv6_vip_count=0' "$node_b_stdout"
run_assertion node_a_dns_ipv4_vip_owner require_one \
    'action_20d_c_probe_value_dns_ipv4_vip_count=1' "$node_a_stdout"
run_assertion node_a_dns_ipv6_vip_owner require_one \
    'action_20d_c_probe_value_dns_ipv6_vip_count=1' "$node_a_stdout"
run_assertion node_b_dns_ipv4_vip_absent require_one \
    'action_20d_c_probe_value_dns_ipv4_vip_count=0' "$node_b_stdout"
run_assertion node_b_dns_ipv6_vip_absent require_one \
    'action_20d_c_probe_value_dns_ipv6_vip_count=0' "$node_b_stdout"
run_assertion node_a_notifier_not_invoked require_one \
    'action_20d_c_probe_notification_helper_invoked=false' "$node_a_stdout"
run_assertion node_b_notifier_not_invoked require_one \
    'action_20d_c_probe_notification_helper_invoked=false' "$node_b_stdout"
node_a_before_hash=$(extract_one action_20d_c_probe_value_before_snapshot_sha256 "$node_a_stdout" 2>/dev/null || true)
node_a_after_hash=$(extract_one action_20d_c_probe_value_after_snapshot_sha256 "$node_a_stdout" 2>/dev/null || true)
node_b_before_hash=$(extract_one action_20d_c_probe_value_before_snapshot_sha256 "$node_b_stdout" 2>/dev/null || true)
node_b_after_hash=$(extract_one action_20d_c_probe_value_after_snapshot_sha256 "$node_b_stdout" 2>/dev/null || true)
