...
reset_caddy_if_failed() {
    ...
    caddy_active_state_supported "$active_state"
    if [[ "$active_state" == failed ]]; then
        systemctl reset-failed caddy.service
        [[ "$(systemctl show caddy.service \
            --property=ActiveState --value 2>/dev/null)" != failed ]]
        printf 'caddy_reset_%s=true\n' "$context"
    else
        printf 'caddy_reset_%s=false\n' "$context"
    fi
}
...
rollback() {
    ...
    trap - EXIT
    printf 'action_16ap_retry_failure_status=%s\n' "$triggering_status" >&2

    if [[ "$caddy_unmasked" == true ]]; then
        reset_caddy_if_failed during_rollback || rollback_ok=false
...
if [[ "${1:-}" == --self-test && $# -eq 1 ]]; then
    ...
    caddy_active_state_supported inactive
    caddy_active_state_supported active
    caddy_active_state_supported failed
    if caddy_active_state_supported masked; then
        exit 1
    fi
    if caddy_active_state_supported unknown; then
        exit 1
    fi
    printf 'action_16ap_retry_cutover_self_test_complete=true\n'
    success=true
    exit 0
elif (($#)); then
...
printf 'action_16ap_retry_remote_reached=true\n'
[[ $EUID -eq 0 ]]
for command_name in \
...
printf 'action_16ap_retry_preflight_complete=true\n'
printf 'action_16ap_retry_mutation_started=true\n'
...
reset_caddy_if_failed before_start
...
printf 'action_16ap_retry_cutover_complete=true\n'
success=true
