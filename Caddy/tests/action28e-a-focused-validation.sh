...
readonly prefix=action_28e_a_focused
...
readonly regression=$test_directory/action28e-a-dual-node-post-execution-regression.sh
readonly focused=$test_directory/action28e-a-focused-validation.sh
readonly manifest=$caddy_root/manifests/protocol-v2-post-action28e-a.yaml
...
readonly outer_sha256=01c7727f88ef214d2a0cb539e437d4d4ac916bd7f8f7b703cf265f5db49aea12
readonly regression_sha256=84b3c899bf8d20c354376161285fb6082ea86784376354a01835426f0f52c74f
readonly manifest_sha256=a974c20410f492cf09f1078a01ad580bf19e70516df3c0f4486dd5ddf5017452
readonly action28e_outer_sha256=d6b87c4991d89d67937c6024368dd745fa24160ebe42d79ce1cd55a4e15efb4f

file_hash() { sha256sum "$1" | awk '{ print $1 }'; }
record_check() {
    local action28e_a_focused_label=$1

    shift
    if "$@" >/dev/null; then
        printf '%s_check_%s=true\n' "$prefix" "$action28e_a_focused_label"
        return 0
    fi
    printf '%s_check_%s=false\n' "$prefix" "$action28e_a_focused_label" >&2
    return 1
}
yaml_check() {
    ...
    grep -Fqx 'action: 28e-a' "$manifest" || return 1
...
for action28e_a_focused_entrypoint in "$inspector" "$outer" "$regression" "$focused"; do
    record_check "executable_index_$(basename "$action28e_a_focused_entrypoint" | tr '.-' '__')" \
        test "$(git ls-files --stage -- \
            "Caddy/${action28e_a_focused_entrypoint#"$caddy_root/"}" | awk '{ print $1 }')" = 100755
done
