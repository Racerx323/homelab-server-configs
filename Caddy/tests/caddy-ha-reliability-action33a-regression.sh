...
readonly transaction=$caddy_root/scripts/transact-caddy-ha-reliability-action33a.sh
readonly outer=$caddy_root/scripts/run-dual-node-caddy-ha-reliability-action33a-outer.sh
...
CADDY_ACTION33A_SSH_BIN=/bin/false /bin/bash "$outer" --self-test >/tmp/action33a-self-test.stdout
grep -Fxq 'action_33a_outer_self_test_complete=true' /tmp/action33a-self-test.stdout
rm -f -- /tmp/action33a-self-test.stdout

require_literal 'cd / && sudo -n /bin/bash' "$outer"
require_literal '-s -- $mode $role $run_id $scenario $argument' "$outer"
require_literal 'capture_files "$action33a_outer_stdout" "$action33a_outer_stderr" "$action33a_outer_status_file"' "$outer"
...
require_literal 'systemd-run --unit "caddy-action33a-reboot-$run_id-$scenario"' "$transaction"
require_literal 'systemd-run --unit "$action33a_remote_unit" --on-active=45s' "$transaction"
require_literal '--publish-emergency' "$outer"
require_literal '--reject-normal' "$outer"
require_literal 'sleep 12' "$outer"
require_literal 'action33a-$run_id-' "$outer"
...
fixture_root=$(mktemp -d /tmp/action33a-regression.XXXXXX)
...
if assert_outbound_role_policy node-a "$fixture_outbound" \
    ...
    printf 'action33a regression accepted payload drift\n' >&2
    exit 1
fi
...
if assert_outbound_role_policy node-a "$fixture_outbound" \
    "$fixture_releases" "$fixture_current" >/dev/null 2>&1; then
    printf 'action33a regression accepted an outbound symlink\n' >&2
    exit 1
fi
...
printf 'caddy_ha_reliability_action33a_regression_complete=true\n'
