...
readonly prefix=action_23e_a_retry_focused_validation
readonly inspector_sha256=a4d0e3e060060c831898ccd6aac4ae01f22cb055700b0cac82057aa25d1a2e9a
readonly outer_sha256=fc677453ecfbafd4928e06ea0f682ede56ef103666540938dc7db79e56c5afad
readonly regression_sha256=fc9ffc9aac5c48a4f9b6191b03533b5012449a7bc3a6b5b05e3831d328f1b3b6
readonly manifest_sha256=6b32b9c09568973ce9e7014a7eefbc1169f0dac6638af6561132785b84f413ca
...
readonly regression=$test_directory/action23e-a-retry-node-a-postchange-regression.sh
readonly focused=$test_directory/action23e-a-retry-focused-validation.sh
readonly manifest=$caddy_root/manifests/dns-action23e-a-retry-postchange-acceptance.yaml

file_hash() { sha256sum "$1" | awk '{ print $1 }'; }
record_check() {
    local action23ear_focused_label=$1

    shift
    if "$@" >/dev/null; then
        printf '%s_check_%s=true\n' "$prefix" "$action23ear_focused_label"
        return 0
    fi
    printf '%s_check_%s=false\n' "$prefix" "$action23ear_focused_label" >&2
    return 1
}
yaml_check() {
    ...
    grep -Fqx 'action: 23e-a-retry' "$manifest" || return 1
...
for action23ear_focused_entrypoint in "$inspector" "$outer" "$regression" "$focused"; do
    record_check "index_mode_$(basename "$action23ear_focused_entrypoint" | tr '.-' '__')" \
        test "$(git -C "$repository_root" ls-files --stage -- \
            "Caddy/${action23ear_focused_entrypoint#"$caddy_root/"}" | awk '{ print $1 }')" = 100755
done
