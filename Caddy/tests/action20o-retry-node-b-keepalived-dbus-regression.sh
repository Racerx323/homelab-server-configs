...
readonly prefix=action_20o_retry_regression
...
source_delta_exact() {
    local action20o_retry_regression_derived=$1

    sed "s/$stale_source_sha256/$corrected_source_sha256/" \
        "$original_transaction" >"$action20o_retry_regression_derived" || return 1
    cmp -s "$action20o_retry_regression_derived" "$transaction"
}
...
readonly derived_transaction=$regression_root/derived-transaction.sh
...
record_check original_stale_source_once test "$(grep -Foc "$stale_source_sha256" "$original_transaction")" -eq 1
record_check original_corrected_source_absent test "$(grep -Foc "$corrected_source_sha256" "$original_transaction" || true)" -eq 0
record_check retry_stale_source_absent test "$(grep -Foc "$stale_source_sha256" "$transaction" || true)" -eq 0
record_check retry_corrected_source_once test "$(grep -Foc "$corrected_source_sha256" "$transaction")" -eq 1
record_check retry_exact_one_token_delta source_delta_exact "$derived_transaction"
...
record_check positive_complete grep -Fqx 'action_20o_retry_outer_complete=true' "$positive_stdout"
record_check positive_node_b_contact grep -Fqx 'action_20o_retry_outer_node_b_contacted=true' "$positive_stdout"
record_check positive_node_a_ssh_absent grep -Fqx 'action_20o_retry_outer_node_a_ssh_contacted=false' "$positive_stdout"
