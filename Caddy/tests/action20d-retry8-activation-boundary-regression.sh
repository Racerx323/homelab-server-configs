...
check_regression() {
    local retry8_regression_label=$1

    shift
    if "$@"; then
        printf 'action_20d_retry8_regression_check_%s=true\n' "$retry8_regression_label"
        return 0
    fi
    printf 'action_20d_retry8_regression_check_%s=false\n' \
        "$retry8_regression_label" >&2
    return 1
}

test_root=$(mktemp -d /tmp/caddy-action20d-retry8-regression.XXXXXX)
readonly test_root
...
make_gate() {
    ...
    # shellcheck disable=SC2016
    {
        ...
        printf '%s\n' 'printf "%s\n" "$fixture_label" >>"$ACTION20D_RETRY8_ORDER_LOG"'
        printf '%s\n' 'printf "fixture_%s_stdout=true\n" "$fixture_label"'
        printf '%s\n' 'printf "fixture_%s_stderr=true\n" "$fixture_label" >&2'
        printf '%s\n' 'if [[ "${ACTION20D_RETRY8_FAIL_GATE:-none}" = "$fixture_label" ]]; then'
...
make_gate "$test_root/validation" validation
make_gate "$test_root/readiness" readiness
make_gate "$test_root/activation" activation
/bin/bash "$collision" "$test_root/validation" "$test_root/readiness" \
    "$test_root/activation" >/dev/null
/bin/bash "$collision" "$outer" "$0" >/dev/null

: >"$test_root/success.order"
ACTION20D_RETRY8_ORDER_LOG=$test_root/success.order \
    /bin/bash "$outer" --production-path-test \
    "$test_root/validation" "$test_root/readiness" "$test_root/activation" \
...
check_regression success_order test \
    "$(paste -sd, "$test_root/success.order")" = validation,readiness,activation
for success_label in \
    action_20d_retry8_validation_accepted=true \
    action_20d_retry8_readiness_accepted=true \
    action_20d_retry8_activation_accepted=true \
    action_20d_retry8_boundary_cleanup_complete=true \
    action_20d_retry8_boundary_accepted=true \
    action_20d_retry8_readiness_stderr_classification=bounded_safe \
    action_20d_retry8_activation_stderr_classification=bounded_safe; do
...
printf 'action_20d_retry8_regression_success_order_and_evidence=true\n'
...
run_failure_case() {
    ...
    : >"$failure_order"
    ACTION20D_RETRY8_ORDER_LOG=$failure_order \
        ACTION20D_RETRY8_FAIL_GATE=$failed_gate_label \
        /bin/bash "$outer" --production-path-test \
        "$test_root/validation" "$test_root/readiness" "$test_root/activation" \
    ...
    check_regression "${failed_gate_label}_failure_not_accepted" \
        grep -Fxq action_20d_retry8_boundary_accepted=false "$failure_stdout"
    if [[ "$failed_gate_label" = validation ]]; then
        check_regression validation_failure_readiness_blocked \
            grep -Fxq action_20d_retry8_readiness_invoked=false "$failure_stdout"
        check_regression validation_failure_activation_blocked \
            grep -Fxq action_20d_retry8_activation_invoked=false "$failure_stdout"
...
run_failure_case validation validation
run_failure_case readiness validation,readiness
run_failure_case activation validation,readiness,activation
printf 'action_20d_retry8_regression_failure_ordering_and_rollback=true\n'
...
# The dollar-prefixed text is literal outer source.
# shellcheck disable=SC2016
check_regression validation_precedes_readiness_and_activation \
    grep -Fq 'run_boundary "$validation_outer" "$readiness_outer" "$activation_outer"' "$outer"
...
printf 'action_20d_retry8_regression_false_positive_and_false_negative_controls=true\n'
printf 'action_20d_retry8_regression_complete=true\n'
