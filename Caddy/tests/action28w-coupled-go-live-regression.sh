...
readonly transaction=$caddy_root/scripts/transact-coupled-go-live-action28w.sh
readonly outer=$caddy_root/scripts/run-dual-node-coupled-go-live-action28w-outer.sh
...
require_text() {
    local action28w_regression_file=$1
    local action28w_regression_text=$2

    grep -Fq -- "$action28w_regression_text" "$action28w_regression_file"
}
reject_text() {
    local action28w_regression_file=$1
    local action28w_regression_text=$2
...
line_number() {
    ...
    grep -Fn -- "$action28w_regression_text" "$action28w_regression_file" |
        awk -F: 'NR == 1 { print $1 }'
}
require_order() {
    local action28w_regression_file=$1
    local action28w_regression_previous=0
    local action28w_regression_text
    local action28w_regression_current

    shift
    for action28w_regression_text in "$@"; do
        action28w_regression_current=$(line_number \
            "$action28w_regression_file" "$action28w_regression_text")
        [[ -n "$action28w_regression_current" ]]
        [[ "$action28w_regression_current" -gt "$action28w_regression_previous" ]]
        action28w_regression_previous=$action28w_regression_current
    done
}
...
"$transaction" --self-test | grep -Fxq \
    'action_28w_remote_self_test_complete=true'
action28w_regression_remote_classifier_output=$(
    "$transaction" --stream-classifier-self-test
)
action28w_regression_outer_classifier_output=$(
    /bin/bash "$outer" --stream-classifier-self-test
)
for action28w_regression_classifier_suffix in \
    ...
    grep -Fxq \
        "action_28w_remote_stream_classifier_${action28w_regression_classifier_suffix}=true" \
        <<<"$action28w_regression_remote_classifier_output"
    grep -Fxq \
        "action_28w_outer_stream_classifier_${action28w_regression_classifier_suffix}=true" \
        <<<"$action28w_regression_outer_classifier_output"
done
action28w_regression_producer_output=$(/bin/bash "$outer" --producer-self-test)
grep -Fxq 'action_28w_outer_producer_remote_valid=true' \
    <<<"$action28w_regression_producer_output"
grep -Fxq 'action_28w_outer_producer_capture_files_valid=true' \
    <<<"$action28w_regression_producer_output"
grep -Fxq 'action_28w_outer_producer_malformed_rejected=true' \
    <<<"$action28w_regression_producer_output"
grep -Fxq 'action_28w_outer_producer_self_test_complete=true' \
    <<<"$action28w_regression_producer_output"
action28w_regression_recovery_output=$(
    /bin/bash "$outer" --pre-mutation-recovery-self-test
)
grep -Fxq 'action_28w_outer_recovery_started=true' \
    <<<"$action28w_regression_recovery_output"
grep -Fxq 'action_28w_outer_recovery_not_required=true' \
    <<<"$action28w_regression_recovery_output"
grep -Fxq 'action_28w_outer_recovery_proven=true' \
    <<<"$action28w_regression_recovery_output"
grep -Fxq 'action_28w_outer_pre_mutation_recovery_self_test_complete=true' \
    <<<"$action28w_regression_recovery_output"
if grep -Fq 'phase_recovery_' <<<"$action28w_regression_recovery_output"; then
    exit 1
fi
...
require_text "$outer" '/tmp/caddy-ssh-evidence/action28w'
require_text "$outer" "'cd / && sudo -n /bin/bash -s --'"
# These are exact source literals, not expressions to expand.
# shellcheck disable=SC2016
require_text "$outer" \
    'printf '\''/bin/bash "$stage/transact-coupled-go-live-action28w.sh" --mode %q'
# shellcheck disable=SC2016
require_text "$outer" \
...
for action28w_regression_config in "$node_a_lsyncd" "$node_b_lsyncd"; do
    require_text "$action28w_regression_config" 'delete = false'
    require_text "$action28w_regression_config" '"--exclude=.complete"'
    require_text "$action28w_regression_config" 'IdentitiesOnly = "yes"'
done

printf 'action_28w_regression_complete=true\n'
