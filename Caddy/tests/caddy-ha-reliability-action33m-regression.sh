...
readonly transaction=$caddy_root/scripts/transact-caddy-ha-reliability-action33m.sh
readonly outer=$caddy_root/scripts/run-dual-node-caddy-ha-reliability-action33m-outer.sh
...
fixture_root=$(mktemp -d /tmp/action33m-regression.XXXXXX)
...
/bin/bash -n "$outer"
CADDY_ACTION33M_SSH_BIN=/bin/false /bin/bash "$outer" --self-test \
    >"$fixture_root/self-test.stdout"
grep -Fxq 'action_33m_outer_self_test_complete=true' \
    "$fixture_root/self-test.stdout"
grep -Fxq 'action_33m_outer_gate_baseline_bundle_self_test=true' \
    "$fixture_root/self-test.stdout"
/bin/bash "$transaction" --reject-capture-self-test \
    >"$fixture_root/reject-capture-self-test.stdout"
grep -Fxq \
    'action_33m_remote__check_reject_capture_stdout_regular=true' \
    "$fixture_root/reject-capture-self-test.stdout"
grep -Fxq \
    'action_33m_remote__check_reject_capture_stderr_regular=true' \
    "$fixture_root/reject-capture-self-test.stdout"
grep -Fxq \
    'action_33m_remote__check_reject_capture_status_regular=true' \
    "$fixture_root/reject-capture-self-test.stdout"
grep -Fxq 'action_33m_remote_reject_capture_self_test_complete=true' \
    "$fixture_root/reject-capture-self-test.stdout"

require_literal 'cd / && sudo -n /bin/bash' "$outer"
require_literal '-s -- $mode $role $run_id $scenario $argument' "$outer"
require_literal 'capture_files "$action33m_outer_stdout" "$action33m_outer_stderr" "$action33m_outer_status_file"' "$outer"
...
require_literal '/run/caddy-action33m-baseline-$run_id-$scenario-$role' "$transaction"
require_literal '/tmp/caddy-action33m-baseline-$run_id-$scenario-$role.tar' "$transaction"
...
require_literal 'failed_action33k_normalized_revision=$failed_action33k_emergency_revision-normalized' \
    "$transaction"
require_literal 'failed_action33k_emergency_manifest_sha256=79cacd055fd5823443129b1721e591933317c3ac9ff8c4e00d7694f768a22230' \
    "$transaction"
require_literal 'failed_action33k_normalized_manifest_sha256=bf711fa44181d89654ea08530d5fd44a9dade7dc1a4cd5ea42858b1309d8f807' \
    "$transaction"
require_literal 'payload_manifest_matches' "$transaction"
require_literal 'test "$action33m_remote_source" = node-b' "$transaction"
require_literal 'test "$action33m_remote_source" = node-a' "$transaction"
...
require_literal 'action: 33m' "$manifest"
require_literal 'failed_predecessor: 33l' "$manifest"
...
require_literal 'release_manifest_sha256: 79cacd055fd5823443129b1721e591933317c3ac9ff8c4e00d7694f768a22230' \
    "$manifest"
require_literal 'validate_every_matching_path_before_cleanup: true' "$manifest"
require_literal 'consumed_action33l_evidence=/tmp/caddy-ssh-evidence/action33l/run.xQQgAG' "$outer"
require_literal 'consumed_action33l_evidence_sha256=d73c07fc7a1a47a300c5f1c3573fb49ebec20667abe60e903fcccce4259004aa' "$outer"
require_literal 'action33l_failed_before_mutation_status' "$outer"
require_literal 'action33l_no_recovery_mutation_evidence' "$outer"

/bin/bash "$transaction" --action33k-family-self-test \
    >"$fixture_root/action33k-family-self-test.stdout"
for expected in \
    action_33m_remote__check_action33k_emergency_family_accepted=true \
    action_33m_remote__check_action33k_normalized_family_accepted=true \
    action_33m_remote_action33k_wrong_source_rejected=true \
    action_33m_remote_action33k_wrong_payload_rejected=true \
    action_33m_remote_action33k_wrong_parent_rejected=true \
    action_33m_remote_action33k_wrong_normalized_hash_rejected=true \
    action_33m_remote_action33k_family_self_test_complete=true; do
...
grep -Fxq 'action_33m_outer_gate_consumed_rolling_failed_exact=true' \
    "$fixture_root/consumed-evidence.stdout"
grep -Fxq 'action_33m_outer_gate_consumed_rolling_recovery_window_bounded=true' \
    "$fixture_root/consumed-evidence.stdout"
grep -Fxq 'action_33m_outer_consumed_evidence_self_test_complete=true' \
    "$fixture_root/consumed-evidence.stdout"
/bin/bash "$outer" --availability-stop-self-test \
    >"$fixture_root/availability-stop.stdout"
grep -Fxq 'action_33m_outer_gate_stop-self-test_availability_grammar=true' \
    "$fixture_root/availability-stop.stdout"
grep -Fxq 'action_33m_outer_gate_stop_self_test_no_open_sample=true' \
    "$fixture_root/availability-stop.stdout"
grep -Fxq 'action_33m_outer_availability_stop_self_test_complete=true' \
    "$fixture_root/availability-stop.stdout"
write_sample() {
    local action33m_test_path=$1
    local action33m_test_timestamp=$2
    local action33m_test_state=$3
    printf '%s\t10.1.0.56\n%s\n' "$action33m_test_timestamp" \
        "$action33m_test_state" >>"$action33m_test_path"
}
run_acceptance() {
    local action33m_test_path=$1
    /bin/bash "$outer" --availability-self-test "$action33m_test_path" \
        >/dev/null
}
...
if run_acceptance "$outside"; then
    printf 'Action 33m accepted a failure outside the handoff window\n' >&2
    exit 1
fi
...
if run_acceptance "$multiple"; then
    printf 'Action 33m accepted multiple failures in one handoff\n' >&2
    exit 1
fi
...
if run_acceptance "$consecutive"; then
    printf 'Action 33m accepted consecutive failed samples\n' >&2
    exit 1
fi
...
if run_acceptance "$slow"; then
    printf 'Action 33m accepted an overlong failed-sample recovery\n' >&2
    exit 1
fi
...
if run_acceptance "$wide"; then
    printf 'Action 33m accepted an overlong handoff window\n' >&2
    exit 1
fi

printf 'caddy_ha_reliability_action33m_regression_complete=true\n'
