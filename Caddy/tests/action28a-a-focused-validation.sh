...
readonly prefix=action_28a_a_focused_validation
readonly inspector_sha256=d49d1ee1c9eccaf28338b3265358bc50fb45481f977df1a350146e2ba0301c76
readonly outer_sha256=4c6d4b7824d91b7f19036de6e957cab8c033d50521900b012752fe35bb450877
readonly regression_sha256=c5b23d66f0e6a06422f5ae1bbc60aad1649a9b689dac9092bf7e3bad0076756b
readonly manifest_sha256=b728effbf75b8d129fd164086dda1ed4c4d4f192f6053f9583706dbf21b183be
...
readonly regression=$test_directory/action28a-a-node-a-postfailure-regression.sh
readonly focused=$test_directory/action28a-a-focused-validation.sh
readonly manifest=$caddy_root/manifests/protocol-v2-node-a-publisher-postfailure-action28a-a.yaml

file_hash() { sha256sum "$1" | awk '{ print $1 }'; }
record_check() {
    local action28a_a_focused_label=$1

    shift
    if "$@" >/dev/null; then
        printf '%s_check_%s=true\n' "$prefix" "$action28a_a_focused_label"
        return 0
    fi
    printf '%s_check_%s=false\n' "$prefix" "$action28a_a_focused_label" >&2
    return 1
}
yaml_check() {
    ...
    grep -Fqx 'action: 28a-a' "$manifest" || return 1
    ...
    grep -Fqx '  action_28a_rerun_permitted: false' "$manifest" || return 1
...
for action28a_a_focused_entrypoint in "$inspector" "$outer" "$regression" "$focused"; do
    record_check "index_mode_$(basename "$action28a_a_focused_entrypoint" | tr '.-' '__')" \
        test "$(git -C "$repository_root" ls-files --stage -- \
            "Caddy/${action28a_a_focused_entrypoint#"$caddy_root/"}" | awk '{ print $1 }')" = 100755
done
