...
readonly runner=$caddy_root/scripts/run-dual-node-caddy-runtime-directories-action20e.sh
...
test_tmpfiles_lifecycle() {
    ...
    cp "$config" "$lifecycle_root/etc/tmpfiles.d/caddy-ha.conf"
    sed -i -e 's/ pi caddy-sync / 1000 1001 /' -e 's/ pi pi / 1000 1000 /' \
...
write_fake_ssh() {
    ...
    printf '%s\n' \
        ...
        'printf "%s\n" "$*" >>"$ACTION20E_CALL_LOG"' \
        'if [[ "$*" == *"--rollback"* ]]; then' \
        '    while IFS= read -r label; do printf "action_20e_node_check_%s=true\n" "$label"; done <"$ACTION20E_ROLLBACK_LABELS"' \
        '    printf "action_20e_node_rollback_complete=true\n"' \
        '    exit 0' \
        'fi' \
        'role=node-b' \
        '[[ "$*" == *"node-a"* ]] && role=node-a' \
        'if [[ "${ACTION20E_FAIL_NODE_A:-0}" = 1 && "$role" = node-a ]]; then' \
        '    printf "action_20e_node_check_identity_root=false\n"' \
        '    exit 1' \
        'fi' \
        'while IFS= read -r label; do' \
        '    [[ "${ACTION20E_OMIT_LABEL:-}" = "$label" ]] && continue' \
        '    printf "action_20e_node_check_%s=true\n" "$label"' \
        'done <"$ACTION20E_LABELS"' \
        'printf "action_20e_node_backup_path=/var/backups/caddy-ha/action20e-%s-runtime-directories.ABC123\n" "$role"' \
        'printf "%s\n" action_20e_node_keepalived_mutated=false action_20e_node_service_mutations=false action_20e_node_vrrp_mutated=false action_20e_node_vip_mutated=false action_20e_node_notifier_invoked=false action_20e_node_install_complete=true' \
        >"$fake_path"
...
test_intercepted_runner() {
    ...
    export ACTION20E_CALL_LOG=$intercept_root/calls
    export ACTION20E_LABELS=$intercept_root/labels
    export ACTION20E_ROLLBACK_LABELS=$intercept_root/rollback-labels
    /bin/bash "$copied_root/scripts/$(basename "$runner")" \
        >"$intercept_root/success.out"
    grep -Fxq 'action_20e_transaction_complete=true' "$intercept_root/success.out"
    [[ "$(wc -l <"$intercept_root/calls")" -eq 2 ]]
    : >"$intercept_root/calls"
    status_value=0
    ACTION20E_FAIL_NODE_A=1 \
        /bin/bash "$copied_root/scripts/$(basename "$runner")" \
        >"$intercept_root/failure.out" 2>"$intercept_root/failure.err" || status_value=$?
    [[ "$status_value" -eq 1 ]]
    [[ "$(wc -l <"$intercept_root/calls")" -eq 3 ]]
    grep -Fxq 'action_20e_node_b_rollback_accepted=true' "$intercept_root/failure.out"
    : >"$intercept_root/calls"
    status_value=0
    ACTION20E_OMIT_LABEL=identity_root \
        /bin/bash "$copied_root/scripts/$(basename "$runner")" \
        >"$intercept_root/missing.out" 2>"$intercept_root/missing.err" || status_value=$?
    [[ "$status_value" -eq 1 ]]
    [[ "$(wc -l <"$intercept_root/calls")" -eq 1 ]]
    grep -Fxq 'action_20e_node_a_contacted=false' "$intercept_root/missing.out"
}
write_fake_acceptance_ssh() {
    ...
    printf '%s\n' \
        ...
        'printf "%s\n" "$*" >>"$ACTION20EA_CALL_LOG"' \
        'while IFS= read -r label; do' \
        '    [[ "${ACTION20EA_OMIT_LABEL:-}" = "$label" ]] && continue' \
        '    printf "action_20e_a_probe_assertion_%s=true\n" "$label"' \
        'done <"$ACTION20EA_LABELS"' \
        'printf "%s\n" action_20e_a_probe_persistent_mutations=false action_20e_a_probe_inspection_complete=true' \
        >"$fake_path"
...
test_intercepted_acceptance() {
    ...
    export ACTION20EA_CALL_LOG=$acceptance_root/acceptance-calls
    export ACTION20EA_LABELS=$acceptance_root/acceptance-labels
    /bin/bash "$copied_runner" >"$acceptance_root/acceptance-success.out"
    grep -Fxq 'action_20e_a_acceptance_complete=true' "$acceptance_root/acceptance-success.out"
    [[ "$(wc -l <"$acceptance_root/acceptance-calls")" -eq 2 ]]
    : >"$acceptance_root/acceptance-calls"
    status_value=0
    ACTION20EA_OMIT_LABEL=identity_root /bin/bash "$copied_runner" \
    ...
    grep -Fxq 'action_20e_a_node_b_accepted=false' "$acceptance_root/acceptance-missing.err"
}
...
case "${1:-}" in
    --self-test)
        [[ $# -eq 1 ]] || exit 64
        verify_sources
        printf 'action_20e_regression_self_test_complete=true\n'
        exit 0
        ;;
    "") [[ $# -eq 0 ]] || exit 64 ;;
    *) exit 64 ;;
