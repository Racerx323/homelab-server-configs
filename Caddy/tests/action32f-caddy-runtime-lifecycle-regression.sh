...
readonly transaction=$caddy_root/scripts/apply-caddy-runtime-lifecycle-action32f.sh
readonly outer=$caddy_root/scripts/run-dual-node-caddy-runtime-lifecycle-action32f-outer.sh
...
require_literal() {
    local action32f_regression_literal=$1
    local action32f_regression_file=$2

    grep -Fq -- "$action32f_regression_literal" "$action32f_regression_file"
}
...
line_number() {
    ...
    awk -v wanted="$action32f_regression_literal" \
        'index($0, wanted) { print NR; found++ } END { if (found != 1) exit 1 }' \
        "$action32f_regression_file"
}

make_release() {
    local action32f_regression_path=$1
    local action32f_regression_revision=$2
    local action32f_regression_parent=${3:-parent}

    install -d -m 0700 "$action32f_regression_path"
    printf ':80 { respond "fixture" }\n' >"$action32f_regression_path/Caddyfile"
    printf '{"parent_revision":"%s","revision":"%s","source_node":"node-a"}\n' \
        "$action32f_regression_parent" "$action32f_regression_revision" \
        >"$action32f_regression_path/release-manifest.json"
    (
        cd "$action32f_regression_path"
        sha256sum ./Caddyfile ./release-manifest.json >manifest.sha256
    )
    : >"$action32f_regression_path/.finalize-request"
    : >"$action32f_regression_path/.complete"
    find "$action32f_regression_path" -type d -exec chmod 0550 {} +
    find "$action32f_regression_path" -type f -exec chmod 0440 {} +
}

fixture_root=$(mktemp -d /tmp/caddy-action32f-regression.XXXXXX)
...
if grep -Fq 'finalized_candidate_semantics inspect_finalized_candidate' \
    ...
    printf 'action32f_regression_live_classifier_ran_before_freeze=true\n' >&2
    exit 1
fi
...
if grep -Fq 'check no_finalized_candidate no_finalized_candidate' \
    "$fixture_root/capture-definition"; then
    printf 'action32f_regression_base_overrode_successor=true\n' >&2
    exit 1
fi
...
if inspect_finalized_candidate >/dev/null 2>&1; then
    printf 'action32f_regression_absent_node_environment_accepted=false\n' >&2
    exit 1
fi
...
if inspect_finalized_candidate >/dev/null 2>&1; then
    printf 'action32f_regression_divergent_candidate_accepted=false\n' >&2
    exit 1
fi
...
if inspect_finalized_candidate >/dev/null 2>&1; then
    printf 'action32f_regression_overpermissive_candidate_accepted=false\n' >&2
    exit 1
fi
chmod -R u+rwX "$successor_incoming_root/node-a/revision-current"
rm -rf -- "$successor_incoming_root/node-a/revision-current"
...
if inspect_finalized_candidate >/dev/null 2>&1; then
    printf 'action32f_regression_multiple_candidates_accepted=false\n' >&2
    exit 1
fi
...
if inspect_finalized_candidate >/dev/null 2>&1; then
    printf 'action32f_regression_installed_release_mismatch_accepted=false\n' >&2
    exit 1
fi
...
if inspect_finalized_candidate >/dev/null 2>&1; then
    printf 'action32f_regression_deployable_child_accepted=false\n' >&2
    exit 1
fi
...
if inspect_finalized_candidate >/dev/null 2>&1; then
    printf 'action32f_regression_divergent_installed_destination_accepted=false\n' >&2
    exit 1
fi
...
if inspect_finalized_candidate >/dev/null 2>&1; then
    printf 'action32f_regression_mutable_installed_destination_accepted=false\n' >&2
    exit 1
fi
...
if inspect_finalized_candidate >/dev/null 2>&1; then
    printf 'action32f_regression_unsafe_non_directory_source_accepted=false\n' >&2
    exit 1
fi
...
role=node-a
if inspect_finalized_candidate >/dev/null 2>&1; then
    printf 'action32f_regression_node_a_candidate_accepted=false\n' >&2
    exit 1
fi
...
require_literal 'action32f_disposition=consume_exact_active_replay' "$transaction"
require_literal 'action32f_disposition=discard_exact_installed_release_replay' "$transaction"
...
require_literal 'readonly manifest_relative=Caddy/manifests/caddy-runtime-lifecycle-action32f.tsv' "$transaction"
...
require_literal '--config "$action32f_candidate/Caddyfile" --adapter caddyfile' "$transaction"
...
require_literal 'successor_persist_base "$action32f_role"' "$transaction"
require_literal 'source "$successor_definitions"' "$transaction"
require_literal 'install_successor_overrides' "$transaction"
require_literal 'NODE_FQDN="$action32f_node_fqdn"' "$transaction"
require_literal 'NODE_IPV4="$action32f_node_ipv4"' "$transaction"
require_literal 'NODE_IPV6="$action32f_node_ipv6"' "$transaction"
...
if grep -Fq 'rm -rf -- "$successor_expected_candidate"' "$transaction"; then
    printf 'action32f_regression_transaction_direct_candidate_deletion=true\n' >&2
    exit 1
fi

printf 'action32f_regression_exact_replay_accepted=true\n'
printf 'action32f_regression_divergence_rejected=true\n'
printf 'action32f_regression_overpermissive_candidate_rejected=true\n'
printf 'action32f_regression_multiple_candidates_rejected=true\n'
printf 'action32f_regression_installed_release_mismatch_rejected=true\n'
printf 'action32f_regression_mutable_installed_release_rejected=true\n'
printf 'action32f_regression_historical_quarantine_ignored_and_preserved=true\n'
printf 'action32f_regression_non_finalized_entry_preserved=true\n'
printf 'action32f_regression_unsafe_source_entry_rejected=true\n'
printf 'action32f_regression_deployable_child_rejected=true\n'
printf 'action32f_regression_node_a_candidate_rejected=true\n'
printf 'action32f_regression_corrected_reconciler_precedes_drain=true\n'
printf 'action32f_regression_actual_payload_path_covered=true\n'
printf 'action32f_regression_runtime_source_order_covered=true\n'
printf 'action32f_regression_exact_environment_propagated=true\n'
printf 'action32f_regression_absent_environment_rejected=true\n'
printf 'action32f_regression_candidate_label_inventory_exact=true\n'
printf 'action32f_regression_complete=true\n'
