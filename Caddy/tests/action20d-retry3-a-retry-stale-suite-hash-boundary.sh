...
readonly prefix=action_20d_retry3_a_retry_boundary
readonly maximum_stream_bytes=1048576
readonly maximum_stream_lines=4096
readonly failed_boundary_sha256=4d9254e622ae7aeeb328ec457e6635ab3b702145df6747be04ee8a6c5efdb8fb
...
readonly current_complete_sha256=5d7131b06cf068595438232ec3e458b5e520a50dd2c1b66384c387eb4680e852
readonly current_integration_sha256=756edbcf24a44b4ab579c9bddc46bcece5e344d5eadc121194e5b34d1a757f01
...
readonly retry3_stdout_sha256=d5eaa9dbb719bd2daa68770bcd390ba9089839a2dc9dc443ae08751167c98904
...
record_assertion() {
    local retry_assertion_label=$1

    shift
    if "$@"; then
        printf '%s_%s=true\n' "$prefix" "$retry_assertion_label"
        return 0
    fi
    printf '%s_%s=false\n' "$prefix" "$retry_assertion_label" >&2
    return 1
}

safe_stream() {
    local retry_stream_path=$1

    [[ "$(wc -c <"$retry_stream_path")" -le "$maximum_stream_bytes" ]] || return 1
    [[ "$(line_count "$retry_stream_path")" -le "$maximum_stream_lines" ]] || return 1
    ! LC_ALL=C grep -n '[^[:print:][:space:]]' "$retry_stream_path" >/dev/null || return 1
    ! grep -Eqi \
...
emit_stream() {
    local retry_stream_label=$1
    local retry_stream_path=$2

    printf '%s_%s_bytes=%s\n' "$prefix" "$retry_stream_label" \
        "$(wc -c <"$retry_stream_path")"
    printf '%s_%s_lines=%s\n' "$prefix" "$retry_stream_label" \
        "$(line_count "$retry_stream_path")"
    printf '%s_%s_sha256=%s\n' "$prefix" "$retry_stream_label" \
        "$(file_hash "$retry_stream_path")"
    if ! safe_stream "$retry_stream_path"; then
        printf '%s_%s_classification=unsafe_retained\n' \
            "$prefix" "$retry_stream_label" >&2
        return 97
    fi
    printf '%s_%s_classification=bounded_safe\n' "$prefix" "$retry_stream_label"
    if [[ -s "$retry_stream_path" ]]; then
        printf '%s_%s_begin\n' "$prefix" "$retry_stream_label"
        cat "$retry_stream_path"
        printf '%s_%s_end\n' "$prefix" "$retry_stream_label"
    else
...
write_retry2_expected() {
    local retry2_expected_stdout_path=$1
    local retry2_expected_stderr_path=$2
    ...
    printf '%s\n' \
        ...
        >"$retry2_expected_stdout_path"
    printf '%s\n' \
        action_20d_retry2_outer_gate_complete_suite_hash=false \
        action_20d_retry2_focused_outer_self_test=false \
        >"$retry2_expected_stderr_path"
}

write_retry3_expected() {
    local retry3_expected_stdout_path=$1
    local retry3_expected_stderr_path=$2
    ...
    printf '%s\n' \
        ...
        >"$retry3_expected_stdout_path"
    printf '%s\n' action_20d_retry3_focused_complete_suite_exact=false \
        >"$retry3_expected_stderr_path"
}

run_validator() {
    local retry_validator_label=$1
    local retry_validator_path=$2
    local expected_validator_stdout_hash=$3
    local expected_validator_stderr_hash=$4
    local retry_capture_root=$5
    local validator_stdout_path=$retry_capture_root/$retry_validator_label.stdout
    local validator_stderr_path=$retry_capture_root/$retry_validator_label.stderr
    local expected_stdout_path=$retry_capture_root/$retry_validator_label.expected.stdout
    local expected_stderr_path=$retry_capture_root/$retry_validator_label.expected.stderr
    ...
    if [[ "$retry_validator_label" = retry2 ]]; then
        write_retry2_expected "$expected_stdout_path" "$expected_stderr_path"
    elif [[ "$retry_validator_label" = retry3 ]]; then
        write_retry3_expected "$expected_stdout_path" "$expected_stderr_path"
    else
        printf '%s_%s_label_valid=false\n' "$prefix" "$retry_validator_label" >&2
        return 64
    fi

    /bin/bash "$retry_validator_path" >"$validator_stdout_path" \
        2>"$validator_stderr_path" || validator_status=$?
    printf '%s_%s_status=%s\n' "$prefix" "$retry_validator_label" "$validator_status"
    emit_stream "${retry_validator_label}_stdout" "$validator_stdout_path" || return 97
    emit_stream "${retry_validator_label}_stderr" "$validator_stderr_path" || return 97

    record_assertion "${retry_validator_label}_status_expected" \
        same_value "$validator_status" 1 || validator_failure_count=$((validator_failure_count + 1))
    record_assertion "${retry_validator_label}_stdout_hash_exact" \
        same_value "$(file_hash "$validator_stdout_path")" "$expected_validator_stdout_hash" || validator_failure_count=$((validator_failure_count + 1))
    record_assertion "${retry_validator_label}_stderr_hash_exact" \
        same_value "$(file_hash "$validator_stderr_path")" "$expected_validator_stderr_hash" || validator_failure_count=$((validator_failure_count + 1))
    record_assertion "${retry_validator_label}_stdout_content_exact" \
        cmp -s "$validator_stdout_path" "$expected_stdout_path" || validator_failure_count=$((validator_failure_count + 1))
    record_assertion "${retry_validator_label}_stderr_content_exact" \
        cmp -s "$validator_stderr_path" "$expected_stderr_path" || validator_failure_count=$((validator_failure_count + 1))
    record_assertion "${retry_validator_label}_stdout_no_false" \
        test "$(grep -Ec '=false$' "$validator_stdout_path")" -eq 0 || validator_failure_count=$((validator_failure_count + 1))
    record_assertion "${retry_validator_label}_expected_failure_count" \
        test "$(grep -Ec '=false$' "$validator_stderr_path")" -eq "$([[ "$retry_validator_label" = retry2 ]] && printf 2 || printf 1)" || validator_failure_count=$((validator_failure_count + 1))
    printf '%s_%s_failed_assertions=%s\n' \
        "$prefix" "$retry_validator_label" "$validator_failure_count"
    [[ "$validator_failure_count" -eq 0 ]]
}

validate_sources() {
    local source_retry2_validator=$1
    local expected_retry2_validator_hash=$2
    local source_retry2_outer=$3
    local expected_retry2_outer_hash=$4
    local source_retry3_validator=$5
    local expected_retry3_validator_hash=$6
    local source_retry3_outer=$7
    local expected_retry3_outer_hash=$8
    local source_complete_suite=$9
    local expected_complete_hash=${10}
    local source_integration_suite=${11}
    local expected_integration_hash=${12}
    local source_failure_count=0

    record_assertion failed_boundary_immutable same_value "$(file_hash "$failed_boundary")" "$failed_boundary_sha256" || source_failure_count=$((source_failure_count + 1))
    record_assertion retry2_validator_regular regular_file "$source_retry2_validator" || source_failure_count=$((source_failure_count + 1))
    record_assertion retry2_validator_executable executable_file "$source_retry2_validator" || source_failure_count=$((source_failure_count + 1))
    record_assertion retry2_validator_hash_exact same_value "$(file_hash "$source_retry2_validator")" "$expected_retry2_validator_hash" || source_failure_count=$((source_failure_count + 1))
    record_assertion retry2_outer_hash_exact same_value "$(file_hash "$source_retry2_outer")" "$expected_retry2_outer_hash" || source_failure_count=$((source_failure_count + 1))
    record_assertion retry3_validator_regular regular_file "$source_retry3_validator" || source_failure_count=$((source_failure_count + 1))
    record_assertion retry3_validator_executable executable_file "$source_retry3_validator" || source_failure_count=$((source_failure_count + 1))
    record_assertion retry3_validator_hash_exact same_value "$(file_hash "$source_retry3_validator")" "$expected_retry3_validator_hash" || source_failure_count=$((source_failure_count + 1))
    record_assertion retry3_outer_hash_exact same_value "$(file_hash "$source_retry3_outer")" "$expected_retry3_outer_hash" || source_failure_count=$((source_failure_count + 1))
    record_assertion complete_suite_hash_exact same_value "$(file_hash "$source_complete_suite")" "$expected_complete_hash" || source_failure_count=$((source_failure_count + 1))
    record_assertion integration_suite_hash_exact same_value "$(file_hash "$source_integration_suite")" "$expected_integration_hash" || source_failure_count=$((source_failure_count + 1))
    record_assertion retry2_complete_pin_exact exact_line_once "readonly complete_suite_sha256=$retry2_stale_complete_sha256" "$source_retry2_outer" || source_failure_count=$((source_failure_count + 1))
    record_assertion retry2_integration_pin_exact exact_line_once "readonly integration_suite_sha256=$retry2_stale_integration_sha256" "$source_retry2_outer" || source_failure_count=$((source_failure_count + 1))
    record_assertion retry3_complete_pin_exact exact_line_once "readonly complete_suite_sha256=$retry3_stale_complete_sha256" "$source_retry3_outer" || source_failure_count=$((source_failure_count + 1))
    record_assertion retry3_integration_pin_exact exact_line_once "readonly integration_suite_sha256=$retry3_stale_integration_sha256" "$source_retry3_outer" || source_failure_count=$((source_failure_count + 1))
...
run_boundary() {
    ...
    boundary_capture_root=$(mktemp -d /tmp/caddy-action20d-retry3-a-retry.XXXXXX)
    ...
    printf '%s_failed_boundary_modified=false\n' "$prefix"
...
run_production_transcript_test() {
    local transcript_capture_root
    local cleanup_command
    local transcript_status=0

    transcript_capture_root=$(mktemp -d /tmp/caddy-action20d-retry3-a-production-transcript.XXXXXX)
    chmod 0700 "$transcript_capture_root"
    printf -v cleanup_command 'rm -rf -- %q' "$transcript_capture_root"
    # shellcheck disable=SC2064
    trap "$cleanup_command" EXIT
    run_validator retry3 "$retry3_validator" "$retry3_stdout_sha256" \
        "$retry3_stderr_sha256" "$transcript_capture_root" || transcript_status=$?
    printf '%s_production_transcript_status=%s\n' "$prefix" "$transcript_status"
    printf '%s_production_transcript_complete_suite_invoked=false\n' "$prefix"
    printf '%s_production_transcript_node_contact=false\n' "$prefix"
    rm -rf -- "$transcript_capture_root"
    trap - EXIT
    printf '%s_production_transcript_cleanup_complete=true\n' "$prefix"
    [[ "$transcript_status" -eq 0 ]]
}
...
case "${1:-}" in
    ...
    "")
        ...
        ;;
    *)
        printf 'Usage: %s [--self-test|--source-test|--contract-test|--production-transcript-test|--production-path-test RETRY2_VALIDATOR RETRY2_VALIDATOR_SHA256 RETRY2_OUTER RETRY2_OUTER_SHA256 RETRY3_VALIDATOR RETRY3_VALIDATOR_SHA256 RETRY3_OUTER RETRY3_OUTER_SHA256 COMPLETE COMPLETE_SHA256 INTEGRATION INTEGRATION_SHA256]\n' \
            "${0##*/}" >&2
        exit 64
        ;;
esac
