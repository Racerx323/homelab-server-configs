...
readonly prefix=action_20b_regression
script_directory=$(cd -- "$(dirname -- "${BASH_SOURCE[0]}")" && pwd)
readonly script_directory
readonly caddy_root=${script_directory%/tests}
readonly candidate="$caddy_root/scripts/check-caddy-action20b.sh"
readonly installer="$caddy_root/scripts/install-node-b-caddy-health-context-action20b.sh"
readonly runner="$caddy_root/scripts/run-node-b-caddy-health-context-correction-action20b.sh"
...
readonly installer_sha256=b3942a984e14dd37d2e31b1764102eacdbd33e321439fcd30c26ae011e6b3a4a
...
installer_contract() {
    grep -Fq 'keepalived_caddy_tls_group_exact' "$installer" &&
        ...
        grep -Fq '.check-caddy.action20b-validate.XXXXXX' "$installer" &&
        ...
        grep -Fq 'action20b-node-b-health-context.XXXXXX' "$installer" &&
...
ssh() {
    local intercepted_mode=${ACTION20B_INTERCEPT_MODE:-valid}
    local emitted_label
    local first_label=true

    cat >/dev/null
    printf 'ssh\n' >>"$ACTION20B_INTERCEPT_COUNT"
    while IFS= read -r emitted_label; do
        ...
        first_label=false
        printf 'action_20b_check_%s=true\n' "$emitted_label"
    done < <(/bin/bash "$ACTION20B_TEST_INSTALLER" --expected-checks)
    if [[ "$intercepted_mode" == duplicate ]]; then
        printf 'action_20b_check_identity_root=true\n'
    elif [[ "$intercepted_mode" == false ]]; then
        printf 'action_20b_check_identity_root=false\n'
    fi
    printf '%s\n' \
        'action_20b_preflight_complete=true' \
        'action_20b_mutation_started=true' \
        'action_20b_helper_invoked_for_validation=true' \
        'action_20b_fragment_mutated=false' \
        'action_20b_keepalived_mutated=false' \
        'action_20b_service_mutations=false' \
        'action_20b_vrrp_mutated=false' \
        'action_20b_vip_mutated=false' \
        'action_20b_backup_path=/var/backups/caddy-ha/action20b-node-b-health-context.ABC123' \
        'action_20b_persistent_mutation_scope=health_helper,rollback_backup' \
        'action_20b_install_complete=true'
}
export -f ssh
...
case "${1:-}" in
    --self-test)
        ...
        require_gate exact_check_count test \
            "$(/bin/bash "$installer" --expected-check-count)" -eq 65
        require_gate exact_check_labels_unique test \
            "$(/bin/bash "$installer" --expected-checks | LC_ALL=C sort -u | wc -l)" -eq 65
        ...
        require_gate host_suite_outer_signature grep -Fq \
            'run-node-b-caddy-health-context-correction-action20b-outer.sh' \
            "$host_suite"
        require_gate host_suite_regression_signature grep -Fq \
            'action20b-node-b-caddy-health-context-correction-regression.sh' \
            "$host_suite"
...
work_directory=$(mktemp -d /tmp/caddy-action20b-regression.XXXXXX)
readonly work_directory
trap 'rm -rf -- "$work_directory"' EXIT
export ACTION20B_TEST_INSTALLER=$installer
export ACTION20B_INTERCEPT_COUNT=$work_directory/ssh.count
: >"$ACTION20B_INTERCEPT_COUNT"

valid_runner_status=0
ACTION20B_INTERCEPT_MODE=valid /bin/bash "$runner" \
...
require_gate false_negative_valid_contract_accepted grep -Fxq \
    'action_20b_remote_status=0' "$work_directory/valid.stdout"
require_gate valid_stderr_empty test ! -s "$work_directory/valid.stderr"
require_gate intercepted_ssh_once test "$(wc -l <"$ACTION20B_INTERCEPT_COUNT")" -eq 1

for negative_mode in missing duplicate false; do
    if ACTION20B_INTERCEPT_MODE=$negative_mode /bin/bash "$runner" \
...
require_gate production_path_network_contact_false test \
    "$(wc -l <"$ACTION20B_INTERCEPT_COUNT")" -eq 4
