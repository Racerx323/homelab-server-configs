...
readonly prefix=action_29e_regression
test_directory=$(cd -- "$(dirname -- "${BASH_SOURCE[0]}")" && pwd)
readonly test_directory
readonly caddy_root=${test_directory%/tests}
readonly inspector=$caddy_root/scripts/inspect-final-deployment-action29e.sh
readonly outer=$caddy_root/scripts/run-final-deployment-acceptance-action29e-outer.sh
fixture_root=
...
run_regression() {
    ...
    local action29e_regression_backup
    local action29e_regression_owner
    local action29e_regression_group
    local action29e_regression_before_ftl
    local action29e_regression_before_domain
    local action29e_regression_release
    local action29e_regression_manifest_output
    ...
    for action29_regression_role in node-a node-b; do
        ...
        sed 's/^action_29e_remote_\(.*\)_observed_pihole_ftl_sha256=.*/action_29e_remote_\1_observed_pihole_ftl_sha256=aaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaa/' \
...
EOF
    ...
    action29e_regression_backup=$fixture_root/action29b-backup
    action29e_regression_owner=$(id -un) || return 1
    action29e_regression_group=$(id -gn) || return 1
    install -d -m 0700 "$action29e_regression_backup" || return 1
    printf '%s\n' before-ftl >"$action29e_regression_backup/pihole-FTL.conf.before" || return 1
    printf '%s\n' before-domain >"$action29e_regression_backup/local.theama.co.conf.before" || return 1
    printf '%s\n' committed >"$action29e_regression_backup/transaction.complete" || return 1
    chmod 0600 "$action29e_regression_backup/pihole-FTL.conf.before" \
        "$action29e_regression_backup/local.theama.co.conf.before" \
        "$action29e_regression_backup/transaction.complete" || return 1
    action29e_regression_before_ftl=$(sha256sum \
        "$action29e_regression_backup/pihole-FTL.conf.before" | awk '{ print $1 }') || return 1
    action29e_regression_before_domain=$(sha256sum \
        "$action29e_regression_backup/local.theama.co.conf.before" | awk '{ print $1 }') || return 1
    printf '%s\n' \
        'action=29b' 'role=node-a' \
        "before_ftl_sha256=$action29e_regression_before_ftl" \
        "before_domain_sha256=$action29e_regression_before_domain" \
        ...
        >"$action29e_regression_backup/manifest" || return 1
    chmod 0600 "$action29e_regression_backup/manifest" || return 1
    check action29b_backup_real_validator /bin/bash "$inspector" --validate-action29b-backup \
        node-a "$action29e_regression_backup" "$action29e_regression_owner" \
        "$action29e_regression_group" "$action29e_regression_before_ftl" \
        "$action29e_regression_before_domain" || return 1
    sed 's/^action=29b$/action=wrong/' "$action29e_regression_backup/manifest" \
        >"$fixture_root/altered-manifest" || return 1
    mv -fT "$fixture_root/altered-manifest" "$action29e_regression_backup/manifest" || return 1
    chmod 0600 "$action29e_regression_backup/manifest" || return 1
    check action29b_backup_altered_rejected reject_command /bin/bash "$inspector" \
        --validate-action29b-backup node-a "$action29e_regression_backup" \
        "$action29e_regression_owner" "$action29e_regression_group" \
        "$action29e_regression_before_ftl" "$action29e_regression_before_domain" || return 1
    action29e_regression_release=$fixture_root/release
    install -d -m 0700 "$action29e_regression_release" || return 1
    printf 'fixture\n' >"$action29e_regression_release/Caddyfile" || return 1
    (
        cd -- "$action29e_regression_release" || exit 1
        sha256sum ./Caddyfile >manifest.sha256
    ) || return 1
    check manifest_paths_real_producer /bin/bash "$inspector" --validate-manifest-paths \
        "$action29e_regression_release/manifest.sha256" || return 1
    check manifest_file_set_real_producer /bin/bash "$inspector" --validate-manifest-file-set \
        "$action29e_regression_release" || return 1
    action29e_regression_manifest_output=$fixture_root/manifest-valid.out
    /bin/bash "$inspector" --capture-manifest-fixture node-a \
        "$action29e_regression_release" >"$action29e_regression_manifest_output" || return 1
    check manifest_hash_real_producer_status grep -Fqx \
        'action_29e_remote_node_a_manifest_hash_check_status=0' \
        "$action29e_regression_manifest_output" || return 1
    check manifest_hash_real_producer_content grep -Fqx './Caddyfile: OK' \
        "$action29e_regression_manifest_output" || return 1
    printf 'drift\n' >>"$action29e_regression_release/Caddyfile" || return 1
    action29e_regression_manifest_output=$fixture_root/manifest-invalid.out
    /bin/bash "$inspector" --capture-manifest-fixture node-a \
        "$action29e_regression_release" >"$action29e_regression_manifest_output" || return 1
    check manifest_hash_drift_status grep -Fqx \
        'action_29e_remote_node_a_manifest_hash_check_status=1' \
        "$action29e_regression_manifest_output" || return 1
    check manifest_hash_drift_content grep -Fqx './Caddyfile: FAILED' \
        "$action29e_regression_manifest_output" || return 1
    printf '%064d  ../escape\n' 0 >"$action29e_regression_release/manifest.sha256" || return 1
    check manifest_unsafe_path_rejected reject_command /bin/bash "$inspector" \
        --validate-manifest-paths "$action29e_regression_release/manifest.sha256" || return 1
    check manifest_file_set_drift_rejected reject_command /bin/bash "$inspector" \
        --validate-manifest-file-set "$action29e_regression_release" || return 1
    # shellcheck disable=SC2016
    ...
    check production_manifest_path_gate grep -Fq \
        'check current_manifest_paths_safe test "$action29e_remote_manifest_paths_status" -eq 0' \
        "$inspector" || return 1
    # shellcheck disable=SC2016
    check production_manifest_file_set_gate grep -Fq \
        'check current_manifest_file_set_exact test "$action29e_remote_manifest_file_set_status" -eq 0' \
        "$inspector" || return 1
    # shellcheck disable=SC2016
    ...
    check cleanup_unset_safe grep -Fqx \
        'action_29e_remote_cleanup_unset_safe=true' "$action29_regression_cleanup" || return 1
