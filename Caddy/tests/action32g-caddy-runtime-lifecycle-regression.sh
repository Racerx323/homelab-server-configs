...
readonly transaction=$caddy_root/scripts/apply-caddy-runtime-lifecycle-action32g.sh
readonly outer=$caddy_root/scripts/run-dual-node-caddy-runtime-lifecycle-action32g-outer.sh
...
require_literal() {
    local action32g_regression_literal=$1
    local action32g_regression_file=$2

    grep -Fq -- "$action32g_regression_literal" "$action32g_regression_file"
}
...
line_number() {
    ...
    awk -v wanted="$action32g_regression_literal" \
        'index($0, wanted) { print NR; found++ } END { if (found != 1) exit 1 }' \
        "$action32g_regression_file"
}

make_release() {
    local action32g_regression_path=$1
    local action32g_regression_revision=$2
    local action32g_regression_parent=${3:-parent}

    install -d -m 0700 "$action32g_regression_path"
    printf ':80 { respond "fixture" }\n' >"$action32g_regression_path/Caddyfile"
    printf '{"parent_revision":"%s","revision":"%s","source_node":"node-a"}\n' \
        "$action32g_regression_parent" "$action32g_regression_revision" \
        >"$action32g_regression_path/release-manifest.json"
    (
        cd "$action32g_regression_path"
        sha256sum ./Caddyfile ./release-manifest.json >manifest.sha256
    )
    : >"$action32g_regression_path/.finalize-request"
    : >"$action32g_regression_path/.complete"
    find "$action32g_regression_path" -type d -exec chmod 0550 {} +
    find "$action32g_regression_path" -type f -exec chmod 0440 {} +
}

fixture_root=$(mktemp -d /tmp/caddy-action32g-regression.XXXXXX)
...
if grep -Fq 'finalized_candidate_semantics inspect_finalized_candidate' \
    ...
    printf 'action32g_regression_live_classifier_ran_before_freeze=true\n' >&2
    exit 1
fi
grep -Fq 'finalized_candidate_drained_by_corrected_reconciler=true' \
    "$fixture_root/drain-definition"
grep -Fq 'run_captured reconcile_candidate_drain' \
    "$fixture_root/drain-definition"
grep -Fq 'systemctl start caddy-sync-reconcile.service' \
    "$fixture_root/drain-definition"
...
if grep -Fq 'check no_finalized_candidate no_finalized_candidate' \
    "$fixture_root/capture-definition"; then
    printf 'action32g_regression_base_overrode_successor=true\n' >&2
    exit 1
fi
...
make_release "$fixture_root/releases/revision-old" revision-old
cp -a "$fixture_root/releases/revision-old" \
    "$successor_incoming_root/node-a/revision-old"
install -d -m 0750 "$successor_quarantine_root/historical-retained-name"
printf 'preserved historical evidence\n' \
    >"$successor_quarantine_root/historical-retained-name/evidence"
chmod 0440 "$successor_quarantine_root/historical-retained-name/evidence"
chmod 0550 "$successor_quarantine_root/historical-retained-name"
...
quarantine_hash_before=$(find "$successor_quarantine_root" \
    -printf '%P\t%y\t%m\n' | LC_ALL=C sort | sha256sum | awk '{ print $1 }')
...
grep -Fxq 'finalized_candidate_installed_replay_count=1' "$fixture_root/exact.stdout"
grep -Fxq 'finalized_candidate_disposition=consume-active-then-discard-exact-installed-replays' \
    "$fixture_root/exact.stdout"
grep -Fxq 'observed_candidate_1_disposition=consume_exact_active_replay' \
    "$fixture_root/exact.stdout"
grep -Fxq 'observed_candidate_2_disposition=discard_exact_installed_release_replay' \
    "$fixture_root/exact.stdout"
grep -Fq 'class=retained_non_finalized' "$fixture_root/exact.stdout"
grep -Fq 'installed_release_path='"$fixture_root/releases/revision-old" \
    "$fixture_root/exact.stdout"
retained_hash_after=$(find "$successor_incoming_root/node-a/historical-bootstrap" \
    -printf '%P\t%y\t%m\n' | LC_ALL=C sort | sha256sum | awk '{ print $1 }')
[[ "$retained_hash_before" = "$retained_hash_after" ]]
quarantine_hash_after=$(find "$successor_quarantine_root" \
    -printf '%P\t%y\t%m\n' | LC_ALL=C sort | sha256sum | awk '{ print $1 }')
...
if inspect_finalized_candidate >/dev/null 2>&1; then
    printf 'action32g_regression_absent_node_environment_accepted=false\n' >&2
    exit 1
fi
...
if inspect_finalized_candidate >/dev/null 2>&1; then
    printf 'action32g_regression_divergent_candidate_accepted=false\n' >&2
    exit 1
fi
...
if inspect_finalized_candidate >/dev/null 2>&1; then
    printf 'action32g_regression_overpermissive_candidate_accepted=false\n' >&2
    exit 1
fi
chmod -R u+rwX "$successor_incoming_root/node-a/revision-current"
rm -rf -- "$successor_incoming_root/node-a/revision-current"
...
if inspect_finalized_candidate >/dev/null 2>&1; then
    printf 'action32g_regression_multiple_candidates_accepted=false\n' >&2
    exit 1
fi
...
printf 'installed-release-drift\n' >>"$successor_incoming_root/node-a/revision-old/Caddyfile"
if inspect_finalized_candidate >/dev/null 2>&1; then
    printf 'action32g_regression_installed_release_mismatch_accepted=false\n' >&2
    exit 1
fi
...
if inspect_finalized_candidate >/dev/null 2>&1; then
    printf 'action32g_regression_deployable_child_accepted=false\n' >&2
    exit 1
fi
...
chmod u+w "$fixture_root/releases/revision-old/Caddyfile"
printf 'destination-drift\n' >>"$fixture_root/releases/revision-old/Caddyfile"
if inspect_finalized_candidate >/dev/null 2>&1; then
    printf 'action32g_regression_divergent_installed_destination_accepted=false\n' >&2
    exit 1
fi
chmod -R u+rwX "$fixture_root/releases/revision-old"
rm -rf -- "$fixture_root/releases/revision-old"
make_release "$fixture_root/releases/revision-old" revision-old
chmod 0644 "$fixture_root/releases/revision-old/Caddyfile"
if inspect_finalized_candidate >/dev/null 2>&1; then
    printf 'action32g_regression_mutable_installed_destination_accepted=false\n' >&2
    exit 1
fi
chmod 0440 "$fixture_root/releases/revision-old/Caddyfile"

printf 'unsafe\n' >"$successor_incoming_root/node-a/unexpected-file"
if inspect_finalized_candidate >/dev/null 2>&1; then
    printf 'action32g_regression_unsafe_non_directory_source_accepted=false\n' >&2
    exit 1
fi
...
role=node-a
if inspect_finalized_candidate >/dev/null 2>&1; then
    printf 'action32g_regression_node_a_candidate_accepted=false\n' >&2
    exit 1
fi
...
require_literal 'action32g_disposition=consume_exact_active_replay' "$transaction"
require_literal 'action32g_disposition=discard_exact_installed_release_replay' "$transaction"
require_literal 'find_exact_installed_release_payload_match' "$transaction"
...
require_literal 'readonly manifest_relative=Caddy/manifests/caddy-runtime-lifecycle-action32g.tsv' "$transaction"
require_literal 'readonly manifest_sha256=705c0e2e590e83da942048ca47c61fab545eb52c9f7b5fe866ba18524c7d1587' "$transaction"
require_literal 'candidate_check active_replay_count_one' "$transaction"
require_literal 'candidate_check all_nonactive_candidates_recognized_replays' "$transaction"
...
require_literal '--config "$action32g_candidate/Caddyfile" --adapter caddyfile' "$transaction"
...
require_literal 'run_captured reconcile_candidate_drain' "$transaction"
require_literal 'systemctl start caddy-sync-reconcile.service' "$transaction"
require_literal 'successor_persist_base "$action32g_role"' "$transaction"
require_literal 'source "$successor_definitions"' "$transaction"
require_literal 'install_successor_overrides' "$transaction"
require_literal 'NODE_FQDN="$action32g_node_fqdn"' "$transaction"
require_literal 'NODE_IPV4="$action32g_node_ipv4"' "$transaction"
require_literal 'NODE_IPV6="$action32g_node_ipv6"' "$transaction"
...
drain_start_line=$(line_number 'run_captured reconcile_candidate_drain' \
    "$fixture_root/drain-definition")
drain_wait_line=$(line_number 'for action32g_override_attempt in' \
    "$fixture_root/drain-definition")
[[ "$drain_start_line" -lt "$drain_wait_line" ]]

action32g_regression_drain_present=true
action32g_regression_runtime_order=$fixture_root/runtime-order
systemctl() {
    [[ "$*" = 'start caddy-sync-reconcile.service' ]] || return 1
    printf 'reconcile-service-started\n' >>"$action32g_regression_runtime_order"
    action32g_regression_drain_present=false
}
incoming_has_finalized_candidate() {
    printf 'candidate-state-probed\n' >>"$action32g_regression_runtime_order"
    [[ "$action32g_regression_drain_present" = true ]]
}
no_finalized_candidate >"$fixture_root/drain-runtime.stdout"
printf 'reconcile-service-started\ncandidate-state-probed\n' \
    >"$fixture_root/runtime-order.expected"
cmp -s "$fixture_root/runtime-order.expected" "$action32g_regression_runtime_order"
grep -Fxq 'finalized_candidate_drain_attempt=0' \
    "$fixture_root/drain-runtime.stdout"
grep -Fxq 'finalized_candidate_drained_by_corrected_reconciler=true' \
    "$fixture_root/drain-runtime.stdout"
if grep -Fq 'rm -rf -- "$successor_expected_candidate"' "$transaction"; then
    printf 'action32g_regression_transaction_direct_candidate_deletion=true\n' >&2
    exit 1
fi

printf 'action32g_regression_exact_replay_accepted=true\n'
printf 'action32g_regression_divergence_rejected=true\n'
printf 'action32g_regression_overpermissive_candidate_rejected=true\n'
printf 'action32g_regression_multiple_candidates_rejected=true\n'
printf 'action32g_regression_installed_release_mismatch_rejected=true\n'
printf 'action32g_regression_mutable_installed_release_rejected=true\n'
printf 'action32g_regression_historical_quarantine_ignored_and_preserved=true\n'
printf 'action32g_regression_non_finalized_entry_preserved=true\n'
printf 'action32g_regression_unsafe_source_entry_rejected=true\n'
printf 'action32g_regression_deployable_child_rejected=true\n'
printf 'action32g_regression_node_a_candidate_rejected=true\n'
printf 'action32g_regression_corrected_reconciler_precedes_drain=true\n'
printf 'action32g_regression_deterministic_reconcile_trigger_precedes_drain_wait=true\n'
printf 'action32g_regression_generated_runtime_trigger_precedes_drain_probe=true\n'
printf 'action32g_regression_actual_payload_path_covered=true\n'
printf 'action32g_regression_runtime_source_order_covered=true\n'
printf 'action32g_regression_exact_environment_propagated=true\n'
printf 'action32g_regression_absent_environment_rejected=true\n'
printf 'action32g_regression_candidate_label_inventory_exact=true\n'
printf 'action32g_regression_complete=true\n'
