...
readonly prefix=action_23e_regression
...
fixture_root=$(mktemp -d /tmp/caddy-action23e-regression.XXXXXX)
readonly fixture_root
readonly fake_ssh=$fixture_root/ssh

cleanup() {
    local action23e_regression_status=$?

    rm -rf -- "$fixture_root"
    exit "$action23e_regression_status"
}
trap cleanup EXIT

record_check() {
    local action23e_regression_label=$1

    shift
    if "$@" >/dev/null; then
        printf '%s_check_%s=true\n' "$prefix" "$action23e_regression_label"
        return 0
    fi
    printf '%s_check_%s=false\n' "$prefix" "$action23e_regression_label" >&2
    return 1
}
run_transport_case() {
    local action23e_regression_mode=$1
    local action23e_regression_expected_status=$2
    local action23e_regression_expected_acceptance=$3
    local action23e_regression_case_root
    local action23e_regression_stdout
    local action23e_regression_stderr
    local action23e_regression_status=0

    action23e_regression_case_root=$(mktemp -d "$fixture_root/case.XXXXXX")
    action23e_regression_stdout=$action23e_regression_case_root/stdout
    action23e_regression_stderr=$action23e_regression_case_root/stderr
    if CADDY_ACTION23E_TEST_MODE=1 \
        CADDY_ACTION23E_SSH_BINARY="$fake_ssh" \
        ACTION23E_FIXTURE_MODE="$action23e_regression_mode" \
        ACTION23E_FIXTURE_DRIVER="$driver" \
        ACTION23E_FIXTURE_LOG="$action23e_regression_case_root/ssh.log" \
        /bin/bash "$outer" --test-transport \
        >"$action23e_regression_stdout" 2>"$action23e_regression_stderr"; then
        action23e_regression_status=0
    else
        action23e_regression_status=$?
    fi
    [[ "$action23e_regression_status" -eq "$action23e_regression_expected_status" ]] || return 1
    grep -Fqx 'fixture_target_exact=true' "$action23e_regression_case_root/ssh.log" || return 1
    grep -Fqx 'fixture_root_cwd_exact=true' "$action23e_regression_case_root/ssh.log" || return 1
    grep -Fqx 'fixture_driver_exact=true' "$action23e_regression_case_root/ssh.log" || return 1
    if [[ "$action23e_regression_expected_acceptance" == true ]]; then
        grep -Fqx 'action_23e_outer_acceptance=true' "$action23e_regression_stdout" || return 1
        grep -Fqx 'action_23e_outer_node_a_contacted=false' "$action23e_regression_stdout" || return 1
        grep -Fqx 'action_23e_outer_node_b_contacted=false' "$action23e_regression_stdout" || return 1
        grep -Fqx 'action_23e_outer_action_23b_rerun=false' "$action23e_regression_stdout" || return 1
        grep -Fqx 'action_23e_outer_action_23d_rerun=false' "$action23e_regression_stdout" || return 1
        [[ ! -s "$action23e_regression_stderr" ]] || return 1
    else
        ! grep -Fqx 'action_23e_outer_acceptance=true' "$action23e_regression_stdout" || return 1
    fi
}
...
readonly fixture_mode=${ACTION23E_FIXTURE_MODE:?}
readonly fixture_driver=${ACTION23E_FIXTURE_DRIVER:?}
readonly fixture_log=${ACTION23E_FIXTURE_LOG:?}
received_driver=$(mktemp /tmp/caddy-action23e-received.XXXXXX)
...
printf 'fixture_driver_exact=%s\n' \
    ...
    >>"$fixture_log"
contract=$(mktemp /tmp/caddy-action23e-contract.XXXXXX)
readonly contract
trap 'rm -f -- "$received_driver" "$contract"' EXIT
/bin/bash "$fixture_driver" --contract-transcript >"$contract"
case "$fixture_mode" in
    success) cat "$contract" ;;
    missing_label) sed '/^action_23e_check_uid_root=true$/d' "$contract" ;;
    duplicate_label) cat "$contract"; printf 'action_23e_check_uid_root=true\n' ;;
    false_label) sed 's/^action_23e_check_uid_root=true$/action_23e_check_uid_root=false/' "$contract" ;;
    altered_domain) sed 's/^action_23e_value_domain_policy=local[.]theama[.]co$/action_23e_value_domain_policy=local.thema.co/' "$contract" ;;
    stale_ptr_policy) sed 's/^action_23e_value_new_ptr_policy=NONE$/action_23e_value_new_ptr_policy=HOSTNAMEFQDN/' "$contract" ;;
