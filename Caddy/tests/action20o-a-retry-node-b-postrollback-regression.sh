...
readonly historical_inspector=$caddy_root/scripts/inspect-node-b-keepalived-dbus-postrollback-action20o-a.sh
readonly historical_outer=$caddy_root/scripts/run-node-b-keepalived-dbus-postrollback-action20o-a-outer.sh
readonly historical_inspector_sha256=637cdb573603044ab35a7bbc936dcda0ec2c16f41edb99d285f0094138bfba80
readonly historical_outer_sha256=602bd70eafa21240eb7000f1f41fbb3d5bc7bdee88f755da52ea8e93b9824b20
...
fixture() {
    ...
    printf '%s\n' \
        ...
        'action_20o_a_value_dbus_snapshot_normalization=process_column_busctl_only' \
...
run_case() {
    ...
    action20oa_regression_root=$(mktemp -d /tmp/caddy-action20o-a-retry-regression.XXXXXX) || return 1
...
MOCK
    ...
    CADDY_ACTION20OA_RETRY_SSH_BIN=$action20oa_regression_root/ssh \
    ...
    if [[ "$action20oa_regression_expected_status" = 0 ]]; then
        ...
        grep -Fqx 'action_20o_a_retry_outer_complete=true' "$action20oa_regression_root/stdout" || return 1
    else
        [[ "$action20oa_regression_actual_status" -ne 0 ]] || return 1
    fi
}

[[ "$(sha256sum "$historical_inspector" | awk '{ print $1 }')" = "$historical_inspector_sha256" ]] || exit 1
[[ "$(sha256sum "$historical_outer" | awk '{ print $1 }')" = "$historical_outer_sha256" ]] || exit 1
awk '
    /# action20oa-dbus-normalization-begin/ { skip=1; next }
    /# action20oa-dbus-normalization-end/ { skip=0; next }
    !skip { print }
' "$inspector" | sed 's/normalized_dbus_hash/text_hash/' |
    cmp -s "$historical_inspector" - || exit 1
[[ "$(grep -Fc "printf '%s\\n' \"\$action20oa_dbus_text\" | awk '\$3 != \"busctl\"'" "$inspector" || true)" -eq 1 ]] || exit 1
grep -Fq 'org.keepalived.Vrrp1' "$inspector" || exit 1
grep -Fq '/org/keepalived/Vrrp1/Instance/eth0/110/IPv4' "$inspector" || exit 1

run_case success 0 || exit 1
run_case query_failure 1 || exit 1
run_case reordered 1 || exit 1
printf '%s\n' \
    action_20o_a_retry_regression_historical_artifacts_immutable=true \
    action_20o_a_retry_regression_exact_derivation=true \
    action_20o_a_retry_regression_keepalived_dbus_preserved=true \
    action_20o_a_retry_regression_positive_path=true \
    action_20o_a_retry_regression_query_failure_rejected=true \
    action_20o_a_retry_regression_reordered_labels_rejected=true \
    action_20o_a_retry_regression_complete=true
