...
readonly transaction=$caddy_root/scripts/transact-caddy-ha-reliability-action33j.sh
readonly outer=$caddy_root/scripts/run-dual-node-caddy-ha-reliability-action33j-outer.sh
...
fixture_root=$(mktemp -d /tmp/action33j-regression.XXXXXX)
...
/bin/bash -n "$outer"
CADDY_ACTION33J_SSH_BIN=/bin/false /bin/bash "$outer" --self-test \
    >"$fixture_root/self-test.stdout"
grep -Fxq 'action_33j_outer_self_test_complete=true' \
    "$fixture_root/self-test.stdout"
/bin/bash "$transaction" --reject-capture-self-test \
    >"$fixture_root/reject-capture-self-test.stdout"
grep -Fxq \
    'action_33j_remote__check_reject_capture_stdout_regular=true' \
    "$fixture_root/reject-capture-self-test.stdout"
grep -Fxq \
    'action_33j_remote__check_reject_capture_stderr_regular=true' \
    "$fixture_root/reject-capture-self-test.stdout"
grep -Fxq \
    'action_33j_remote__check_reject_capture_status_regular=true' \
    "$fixture_root/reject-capture-self-test.stdout"
grep -Fxq 'action_33j_remote_reject_capture_self_test_complete=true' \
    "$fixture_root/reject-capture-self-test.stdout"

require_literal 'cd / && sudo -n /bin/bash' "$outer"
require_literal '-s -- $mode $role $run_id $scenario $argument' "$outer"
require_literal 'capture_files "$action33j_outer_stdout" "$action33j_outer_stderr" "$action33j_outer_status_file"' "$outer"
...
require_literal 'begin_handoff "$action33j_outer_scenario-b-to-a"' "$outer"
...
require_literal 'first_live_case: node-a-controlled-outage' "$manifest"
...
grep -Fxq 'action_33j_outer_gate_consumed_rolling_failed_exact=true' \
    "$fixture_root/consumed-evidence.stdout"
grep -Fxq 'action_33j_outer_gate_consumed_rolling_recovery_window_bounded=true' \
    "$fixture_root/consumed-evidence.stdout"
grep -Fxq 'action_33j_outer_consumed_evidence_self_test_complete=true' \
    "$fixture_root/consumed-evidence.stdout"
write_sample() {
    local action33j_test_path=$1
    local action33j_test_timestamp=$2
    local action33j_test_state=$3
    printf '%s\t10.1.0.56\n%s\n' "$action33j_test_timestamp" \
        "$action33j_test_state" >>"$action33j_test_path"
}
run_acceptance() {
    local action33j_test_path=$1
    /bin/bash "$outer" --availability-self-test "$action33j_test_path" \
        >/dev/null
}
...
if run_acceptance "$outside"; then
    printf 'Action 33j accepted a failure outside the handoff window\n' >&2
    exit 1
fi
...
if run_acceptance "$multiple"; then
    printf 'Action 33j accepted multiple failures in one handoff\n' >&2
    exit 1
fi
...
if run_acceptance "$consecutive"; then
    printf 'Action 33j accepted consecutive failed samples\n' >&2
    exit 1
fi
...
if run_acceptance "$slow"; then
    printf 'Action 33j accepted an overlong failed-sample recovery\n' >&2
    exit 1
fi
...
if run_acceptance "$wide"; then
    printf 'Action 33j accepted an overlong handoff window\n' >&2
    exit 1
fi

printf 'caddy_ha_reliability_action33j_regression_complete=true\n'
