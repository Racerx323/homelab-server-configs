...
readonly caddy_root
readonly inspector="$caddy_root/scripts/inspect-node-b-protocol-v2-postinstall-action17q-b.sh"
readonly runner="$caddy_root/scripts/run-node-b-protocol-v2-postinstall-action17q-b.sh"
...
readonly runner_sha256=20822b4e02466727e57e6257aad03f13660cc9b1ec049146f482c42b6508eede
...
assert_inspector_contract_alignment() {
    local alignment_directory
    local alignment_failed=false
    local expected_labels_path
    local inspector_labels_path

    alignment_directory=$(mktemp -d /tmp/caddy-action17q-b-alignment.XXXXXX)
    expected_labels_path="$alignment_directory/expected"
    inspector_labels_path="$alignment_directory/inspector"

    printf '%s\n' "${expected_assertions[@]}" |
        LC_ALL=C sort >"$expected_labels_path"
    awk '
        /record_command\(\)/ { next }
        /record_command [a-z0-9_]+/ {
            line = $0
            sub(/^.*record_command /, "", line)
            sub(/[[:space:]].*$/, "", line)
            print line
            next
        }
        /record_command[[:space:]]*\\$/ {
            getline
            line = $0
            sub(/^[[:space:]]*/, "", line)
            sub(/[[:space:]].*$/, "", line)
    ...
    ' "$inspector" |
        LC_ALL=C sort >"$inspector_labels_path"

    cmp -s "$expected_labels_path" "$inspector_labels_path" ||
        alignment_failed=true
    [[ "$(wc -l <"$expected_labels_path")" -eq 81 ]] ||
        alignment_failed=true
    [[ "$(sort -u "$inspector_labels_path" | wc -l)" -eq 81 ]] ||
        alignment_failed=true
    rm -rf -- "$alignment_directory"
...
assert_static_policy() {
    if grep -Eq \
        ...
        printf 'Action 17q-b contains a service mutation.\n' >&2
        return 1
    fi
    if grep -Eq \
        '(^|[[:space:]])(rsync|scp|sftp)([[:space:]]|$)' \
        "$inspector" "$runner"; then
        printf 'Action 17q-b contains a transfer command.\n' >&2
        return 1
    fi
    if grep -Eq \
        'ACTION17QB_(FIXTURE|CAPTURED|SSH_ARGUMENTS|STATUS)' "$runner"; then
        printf 'Production Action 17q-b runner contains a fixture bypass.\n' >&2
        return 1
    fi
    grep -Fq \
    ...
    grep -Fq 'finalizer_v2_hash_exact' "$inspector"
    grep -Fq 'authorized_keys_node_a_fingerprint' "$inspector"
    grep -Fq \
        'action17q-retry-node-b-protocol-v2.TEhT7k' "$inspector"
    grep -Fq 'rollback_hash_record_content_exact' "$inspector"
    grep -Fq \
        'action_17q_b_helper_execution=false' "$inspector"
    grep -Fq \
        'action_17q_b_persistent_mutations=false' "$inspector"
    grep -Fq \
...
write_fake_ssh() {
    ...
    printf '%s\n' \
        '#!/usr/bin/env bash' \
        'set -Eeuo pipefail' \
        'cat >"$ACTION17QB_CAPTURED_INSPECTOR"' \
        'printf "%s\n" "$*" >"$ACTION17QB_SSH_ARGUMENTS"' \
        'cat "$ACTION17QB_FIXTURE"' \
        'exit "${ACTION17QB_STATUS:-0}"' \
        >"$fake_ssh_path"
...
run_case() {
    ...
    set +e
    ACTION17QB_CAPTURED_INSPECTOR="$case_root/$case_suffix.inspector" \
        ACTION17QB_SSH_ARGUMENTS="$case_root/$case_suffix.arguments" \
        ACTION17QB_FIXTURE="$case_fixture" \
        ACTION17QB_STATUS="$case_status" \
...
run_production_path_regression() {
    ...
    case_root=$(mktemp -d /tmp/caddy-action17q-b-regression.XXXXXX)
    ...
    case_runner="$case_root/Caddy/scripts/run-node-b-protocol-v2-postinstall-action17q-b.sh"
    write_fake_ssh "$case_bin/ssh"
    sed -i "s|^PATH=/usr/bin:/bin$|PATH=$case_bin:/usr/bin:/bin|" \
        "$case_runner"
    chmod 0755 \
        "$case_runner" \
        "$case_root/Caddy/scripts/inspect-node-b-protocol-v2-postinstall-action17q-b.sh" \
        "$case_root/Caddy/tests/check-shell-readonly-local-collisions.sh"
    ...
    grep -Fxq action_17q_b_runner_contract_valid=true \
        "$case_root/valid.out"
    grep -Fxq action_17q_b_runner_acceptance=true \
        "$case_root/valid.out"
    grep -Fxq action_17q_b_workstation_cleanup_complete=true \
        "$case_root/valid.out"
    ...
    sed \
        -e 's/action_17q_b_assertion_receiver_v2_hash_exact=true/action_17q_b_assertion_receiver_v2_hash_exact=false/' \
        -e 's/action_17q_b_failed_assertion_count=0/action_17q_b_failed_assertion_count=1/' \
        -e 's/action_17q_b_first_failure=none/action_17q_b_first_failure=receiver_v2_hash_exact/' \
    ...
    grep -Fxq action_17q_b_runner_acceptance=false \
        "$case_root/mismatch.out"

    malformed_fixture="$case_root/malformed.fixture"
    sed \
        's/action_17q_b_value_reconcile_path_mainpid_present=false/action_17q_b_value_reconcile_path_mainpid_present=true/' \
        "$valid_fixture" >"$malformed_fixture"
    run_case "$malformed_fixture" "$case_root" 0 malformed "$case_runner"
    [[ "$observed_status" -eq 97 ]]
    grep -Fq 'Action 17q-b transcript contract failed.' \
        "$case_root/malformed.err"

    cp -- "$valid_fixture" "$case_root/duplicate.fixture"
    printf 'action_17q_b_assertion_state_unchanged=true\n' \
    ...
    sed \
        's/action_17q_b_value_after_state_sha256=2222222222222222222222222222222222222222222222222222222222222222/action_17q_b_value_after_state_sha256=3333333333333333333333333333333333333333333333333333333333333333/' \
        "$valid_fixture" >"$case_root/drift.fixture"
    run_case "$case_root/drift.fixture" "$case_root" 0 drift "$case_runner"
    [[ "$observed_status" -eq 97 ]]

    printf 'action_17q_b_false_negative_valid_evidence_accepted=true\n'
    printf 'action_17q_b_false_negative_semantic_mismatch_preserved=true\n'
    printf 'action_17q_b_false_positive_contradiction_rejected=true\n'
    printf 'action_17q_b_false_positive_duplicate_rejected=true\n'
    printf 'action_17q_b_false_positive_state_drift_rejected=true\n'
    printf 'action_17q_b_production_path_network_contact=false\n'
}
...
if [[ "${CADDY_VALIDATION_CONTAINER:-}" == 1 ]]; then
    ...
    printf 'action_17q_b_container_projection_validated=true\n'
else
    run_production_path_regression
fi

printf 'action_17q_b_node_b_postinstall_regression_complete=true\n'
