...
readonly prefix=action_30b_focused

check() {
    local action30b_focused_label=$1

    shift
    if "$@" >/dev/null 2>&1; then
        printf '%s_check_%s=true\n' "$prefix" "$action30b_focused_label"
        return 0
    fi
    printf '%s_check_%s=false\n' "$prefix" "$action30b_focused_label" >&2
    return 1
}
manifest_valid() {
...
run_checks() {
    local action30b_focused_entry

    # conditional-validator-explicit-failures-begin
    for action30b_focused_entry in "$transaction" "$outer" "$regression" "$policy" "${BASH_SOURCE[0]}"; do
        check "$(basename "$action30b_focused_entry" | tr -c '[:alnum:]' _)_syntax" \
            /bin/bash -n "$action30b_focused_entry" || return 1
    done
    ...
    check outer_self_test env CADDY_ACTION30B_SKIP_REGRESSION=true \
        CADDY_ACTION30B_SKIP_REPEATED_POLICIES=true /bin/bash "$outer" --self-test || return 1
