...
require_true() {
    local action23m_regression_label=$1

    shift
    if "$@"; then
        printf 'action_23m_regression_%s=true\n' "$action23m_regression_label"
    else
        printf 'action_23m_regression_%s=false\n' "$action23m_regression_label" >&2
        return 1
    fi
}
...
run_transport_case() {
    local action23m_case_mode=$1
    local action23m_case_status_expected=$2
    local action23m_case_acceptance_expected=$3
    local action23m_case_dir
    local action23m_case_stdout
    local action23m_case_stderr
    local action23m_case_status

    action23m_case_dir=$(mktemp -d)
    action23m_case_stdout="$action23m_case_dir/stdout"
    action23m_case_stderr="$action23m_case_dir/stderr"
    set +e
    ACTION23M_TEST_SSH_BIN="$fake_ssh" \
        ACTION23M_FIXTURE_MODE="$action23m_case_mode" \
        ACTION23M_FIXTURE_DRIVER="$driver" \
        ACTION23M_FIXTURE_LOG="$action23m_case_dir/fake-ssh.log" \
        /bin/bash "$outer" --transport-test \
        >"$action23m_case_stdout" 2>"$action23m_case_stderr"
    action23m_case_status=$?
    set -e

    [[ "$action23m_case_status" -eq "$action23m_case_status_expected" ]]
    grep -Fqx 'fake_ssh_archive_candidate_hash=true' \
        "$action23m_case_dir/fake-ssh.log"
    grep -Fqx 'fake_ssh_remote_boundary=true' \
        "$action23m_case_dir/fake-ssh.log"
    if [[ "$action23m_case_acceptance_expected" == true ]]; then
        grep -Fqx 'action_23m_outer_acceptance=true' "$action23m_case_stdout"
        grep -Fqx 'action_23m_outer_stdout_classification=bounded_safe' \
            "$action23m_case_stdout"
        grep -Fqx 'action_23m_outer_stderr_classification=bounded_safe' \
            "$action23m_case_stdout"
        [[ ! -s "$action23m_case_stderr" ]]
    else
        if grep -Fqx 'action_23m_outer_acceptance=true' "$action23m_case_stdout"; then
            return 1
        fi
        grep -Fqx 'action_23m_outer_acceptance=false' "$action23m_case_stderr"
    fi
    rm -rf -- "$action23m_case_dir"
}
...
require_true readiness_probe_explicit_status grep -Fq \
    'if action23m_probe_output=$(run_dns_probe_command' "$driver"
...
require_true candidate_parser_redirect_internal grep -Fq \
    'unbound-checkconf "$action23m_parser_root" >/dev/null' "$driver"
...
require_true accepted_domain_hash_pinned grep -Fqx \
    ...
    "$driver"
require_true accepted_parent_hash_pinned grep -Fqx \
    'readonly accepted_local_zone_sha256=adee452bf547479bdcf3b38c214a75344d49207c4c88814f5670146f4804ddb9' \
    "$driver"
require_true candidate_srv_hash_pinned grep -Fqx \
    'readonly candidate_local_zone_sha256=fa9f4850386ab1328f323c7c88bd9fa9ad0d5a84994b3066b6874deb5beb569c' \
    "$driver"
require_true exact_https_srv_asserted grep -Fq \
    '_https._tcp.proxy.local.theama.co. 180 IN SRV 0 0 443 proxy.local.theama.co.' \
    "$driver"
require_true direct_srv_readiness grep -Fq '[direct_caddy_srv]=SRV' "$driver"
require_true local_srv_readiness grep -Fq '[local_caddy_srv]=SRV' "$driver"
require_true predecessor_driver_invocation_absent test \
    "$(grep -Ec 'action23(a|b|g|h|i|j|k|l)' \
        "$driver" || true)" -eq 0
...
readonly fixture_mode=${ACTION23M_FIXTURE_MODE:?}
readonly fixture_driver=${ACTION23M_FIXTURE_DRIVER:?}
readonly fixture_log=${ACTION23M_FIXTURE_LOG:?}
...
printf 'fake_ssh_archive_candidate_hash=%s\n' \
    "$(if [[ "$(sha256sum "$candidate" | awk '{ print $1 }')" == \
        fa9f4850386ab1328f323c7c88bd9fa9ad0d5a84994b3066b6874deb5beb569c ]]; then
        printf true
    else
        printf false
    fi)" >>"$fixture_log"
...
case "$fixture_mode" in
    success)
        ...
        ;;
    missing_label)
        sed '/^action_23m_check_uid_is_root=true$/d' "$contract"
        ;;
    duplicate_label)
        cat "$contract"
        printf 'action_23m_check_uid_is_root=true\n'
        ;;
    false_label)
        sed 's/^action_23m_check_uid_is_root=true$/action_23m_check_uid_is_root=false/' \
            "$contract"
        ;;
    wrong_ftl_hash)
        sed 's/^action_23m_pihole_ftl_sha256=.*/action_23m_pihole_ftl_sha256=aaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaa/' \
            "$contract"
        ;;
    wrong_domain_hash)
        sed 's/^action_23m_pihole_domain_sha256=.*/action_23m_pihole_domain_sha256=bbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbb/' \
            "$contract"
        ;;
    stderr)
        cat "$contract"
...
printf 'action_23m_regression_node_contact=false\n'
printf 'action_23m_regression_live_mutation=false\n'
printf 'action_23m_regression_complete=true\n'
