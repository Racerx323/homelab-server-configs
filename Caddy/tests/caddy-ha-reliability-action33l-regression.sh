...
readonly transaction=$caddy_root/scripts/transact-caddy-ha-reliability-action33l.sh
readonly outer=$caddy_root/scripts/run-dual-node-caddy-ha-reliability-action33l-outer.sh
...
fixture_root=$(mktemp -d /tmp/action33l-regression.XXXXXX)
...
/bin/bash -n "$outer"
CADDY_ACTION33L_SSH_BIN=/bin/false /bin/bash "$outer" --self-test \
    >"$fixture_root/self-test.stdout"
grep -Fxq 'action_33l_outer_self_test_complete=true' \
    "$fixture_root/self-test.stdout"
grep -Fxq 'action_33l_outer_gate_baseline_bundle_self_test=true' \
    "$fixture_root/self-test.stdout"
/bin/bash "$transaction" --reject-capture-self-test \
    >"$fixture_root/reject-capture-self-test.stdout"
grep -Fxq \
    'action_33l_remote__check_reject_capture_stdout_regular=true' \
    "$fixture_root/reject-capture-self-test.stdout"
grep -Fxq \
    'action_33l_remote__check_reject_capture_stderr_regular=true' \
    "$fixture_root/reject-capture-self-test.stdout"
grep -Fxq \
    'action_33l_remote__check_reject_capture_status_regular=true' \
    "$fixture_root/reject-capture-self-test.stdout"
grep -Fxq 'action_33l_remote_reject_capture_self_test_complete=true' \
    "$fixture_root/reject-capture-self-test.stdout"

require_literal 'cd / && sudo -n /bin/bash' "$outer"
require_literal '-s -- $mode $role $run_id $scenario $argument' "$outer"
require_literal 'capture_files "$action33l_outer_stdout" "$action33l_outer_stderr" "$action33l_outer_status_file"' "$outer"
...
require_literal '/run/caddy-action33l-baseline-$run_id-$scenario-$role' "$transaction"
require_literal '/tmp/caddy-action33l-baseline-$run_id-$scenario-$role.tar' "$transaction"
...
require_literal 'failed_action33k_run_id=20260813T000701Z-2499021' \
    "$transaction"
require_literal 'failed_action33k_manifest_sha256=bf711fa44181d89654ea08530d5fd44a9dade7dc1a4cd5ea42858b1309d8f807' \
    "$transaction"
...
grep -Fxq 'action_33l_outer_gate_consumed_rolling_failed_exact=true' \
    "$fixture_root/consumed-evidence.stdout"
grep -Fxq 'action_33l_outer_gate_consumed_rolling_recovery_window_bounded=true' \
    "$fixture_root/consumed-evidence.stdout"
grep -Fxq 'action_33l_outer_consumed_evidence_self_test_complete=true' \
    "$fixture_root/consumed-evidence.stdout"
/bin/bash "$outer" --availability-stop-self-test \
    >"$fixture_root/availability-stop.stdout"
grep -Fxq 'action_33l_outer_gate_stop-self-test_availability_grammar=true' \
    "$fixture_root/availability-stop.stdout"
grep -Fxq 'action_33l_outer_gate_stop_self_test_no_open_sample=true' \
    "$fixture_root/availability-stop.stdout"
grep -Fxq 'action_33l_outer_availability_stop_self_test_complete=true' \
    "$fixture_root/availability-stop.stdout"
write_sample() {
    local action33l_test_path=$1
    local action33l_test_timestamp=$2
    local action33l_test_state=$3
    printf '%s\t10.1.0.56\n%s\n' "$action33l_test_timestamp" \
        "$action33l_test_state" >>"$action33l_test_path"
}
run_acceptance() {
    local action33l_test_path=$1
    /bin/bash "$outer" --availability-self-test "$action33l_test_path" \
        >/dev/null
}
...
if run_acceptance "$outside"; then
    printf 'Action 33l accepted a failure outside the handoff window\n' >&2
    exit 1
fi
...
if run_acceptance "$multiple"; then
    printf 'Action 33l accepted multiple failures in one handoff\n' >&2
    exit 1
fi
...
if run_acceptance "$consecutive"; then
    printf 'Action 33l accepted consecutive failed samples\n' >&2
    exit 1
fi
...
if run_acceptance "$slow"; then
    printf 'Action 33l accepted an overlong failed-sample recovery\n' >&2
    exit 1
fi
...
if run_acceptance "$wide"; then
    printf 'Action 33l accepted an overlong handoff window\n' >&2
    exit 1
fi

printf 'caddy_ha_reliability_action33l_regression_complete=true\n'
