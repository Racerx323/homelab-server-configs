...
readonly prefix=action_23b_a_regression
...
write_mock_ssh() {
    ...
    printf '%s\n' \
        '#!/usr/bin/env bash' \
        'set -Eeuo pipefail' \
        'printf '\''%s\n'\'' "$@" >"$ACTION23BA_MOCK_ARGS"' \
        'cat >/dev/null' \
        'cat "$ACTION23BA_MOCK_STDOUT"' \
        'cat "$ACTION23BA_MOCK_STDERR" >&2' \
        'exit "$ACTION23BA_MOCK_STATUS"' >"$action23aa_regression_mock" || return 1
...
run_case() (
    ...
    CADDY_ACTION23BA_TEST_MODE=1 \
        CADDY_ACTION23BA_SSH_BINARY=$action23aa_regression_mock \
        ACTION23BA_MOCK_ARGS=$action23aa_regression_mock_args \
        ACTION23BA_MOCK_STDOUT=$action23aa_regression_transcript \
        ACTION23BA_MOCK_STDERR=$action23aa_regression_mock_stderr \
        ACTION23BA_MOCK_STATUS=$action23aa_regression_remote_status \
    ...
    if ! grep -Fqx 'pi@10.1.0.53' "$action23aa_regression_mock_args"; then
    ...
    if [[ "$action23aa_regression_expected_status" -eq 0 ]]; then
        grep -Fqx action_23b_a_outer_acceptance=true "$action23aa_regression_outer_stdout" || return 1
        grep -Fqx action_23b_a_outer_complete=true "$action23aa_regression_outer_stdout" || return 1
...
run_validation_case() (
    ...
    CADDY_ACTION23BA_TEST_MODE=1 /bin/bash "$outer" --test-validate \
...
run_regression() (
    ...
    action23aa_regression_root=$(mktemp -d /tmp/caddy-action23b-a-regression.XXXXXX) || return 1
    ...
    for action23aa_regression_case in false missing duplicate reordered changed_state changed_hash \
        invalid_classification missing_classification; do
        cp -- "$action23aa_regression_valid" \
            "$action23aa_regression_root/$action23aa_regression_case.transcript" || return 1
    done
    sed -i '0,/action_23b_a_check_local_zone_hash_restored=true/s//action_23b_a_check_local_zone_hash_restored=false/' \
        "$action23aa_regression_root/false.transcript" || return 1
    sed -i '0,/action_23b_a_check_local_zone_hash_restored=true/d' \
        "$action23aa_regression_root/missing.transcript" || return 1
    printf '%s\n' action_23b_a_check_local_zone_hash_restored=true \
    ...
    sed -i "s/action_23b_a_value_after_state_sha256=$fixture_state_sha256/action_23b_a_value_after_state_sha256=bbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbb/" \
        "$action23aa_regression_root/changed_state.transcript" || return 1
    sed -i 's/action_23b_a_value_local_zone_sha256=c70f709789223f91835f0f21c397f577d1e2bb24005a5defbf46055fb411dbb4/action_23b_a_value_local_zone_sha256=bbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbb/' \
        "$action23aa_regression_root/changed_hash.transcript" || return 1
    sed -i 's/action_23b_a_value_dns_ptr4_classification=local_hosts_override/action_23b_a_value_dns_ptr4_classification=invalid/' \
        "$action23aa_regression_root/invalid_classification.transcript" || return 1
    sed -i '/^action_23b_a_value_dns_ptr4_classification=/d' \
        "$action23aa_regression_root/missing_classification.transcript" || return 1
    for action23aa_regression_case in false missing duplicate reordered changed_state changed_hash \
        invalid_classification missing_classification; do
    ...
    record_gate dns_vip_ipv4_present grep -Fq 'readonly dns_ipv4=10.1.0.55' "$inspector" || return 1
    record_gate dns_vip_ipv6_present grep -Fq 'readonly dns_ipv6=fd36:5aa8:6971:1::55' "$inspector" || return 1
    record_gate node_ipv4_present grep -Fq 'readonly node_ipv4=10.1.0.53' "$inspector" || return 1
    record_gate node_ipv6_present grep -Fq 'readonly node_ipv6=fd36:5aa8:6971:1::53' "$inspector" || return 1
    record_gate dns_fqdn_present grep -Fq 'readonly dns_fqdn=pihole.local.theama.co' "$inspector" || return 1
    record_gate node_fqdn_present grep -Fq 'readonly node_fqdn=pihole0.local.theama.co' "$inspector" || return 1
    record_gate direct_unbound_path grep -Fq '[direct_unbound]=127.0.0.1' "$inspector" || return 1
    record_gate local_pihole_path grep -Fq '[local_pihole]=127.0.0.1' "$inspector" || return 1
    record_gate dns_vip_path grep -Fq "[dns_vip_ipv4]=\"\$dns_ipv4\"" "$inspector" || return 1
    record_gate node_physical_path grep -Fq "[node_ipv4]=\"\$node_ipv4\"" "$inspector" || return 1
