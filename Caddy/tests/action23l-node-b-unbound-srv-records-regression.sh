...
require_true() {
    local action23l_regression_label=$1

    shift
    if "$@"; then
        printf 'action_23l_regression_%s=true\n' "$action23l_regression_label"
    else
        printf 'action_23l_regression_%s=false\n' "$action23l_regression_label" >&2
        return 1
    fi
}
...
run_transport_case() {
    local action23l_case_mode=$1
    local action23l_case_status_expected=$2
    local action23l_case_acceptance_expected=$3
    local action23l_case_dir
    local action23l_case_stdout
    local action23l_case_stderr
    local action23l_case_status

    action23l_case_dir=$(mktemp -d)
    action23l_case_stdout="$action23l_case_dir/stdout"
    action23l_case_stderr="$action23l_case_dir/stderr"
    set +e
    ACTION23L_TEST_SSH_BIN="$fake_ssh" \
        ACTION23L_FIXTURE_MODE="$action23l_case_mode" \
        ACTION23L_FIXTURE_DRIVER="$driver" \
        ACTION23L_FIXTURE_LOG="$action23l_case_dir/fake-ssh.log" \
        /bin/bash "$outer" --transport-test \
        >"$action23l_case_stdout" 2>"$action23l_case_stderr"
    action23l_case_status=$?
    set -e

    [[ "$action23l_case_status" -eq "$action23l_case_status_expected" ]]
    grep -Fqx 'fake_ssh_archive_candidate_hash=true' \
        "$action23l_case_dir/fake-ssh.log"
    grep -Fqx 'fake_ssh_remote_boundary=true' \
        "$action23l_case_dir/fake-ssh.log"
    if [[ "$action23l_case_acceptance_expected" == true ]]; then
        grep -Fqx 'action_23l_outer_acceptance=true' "$action23l_case_stdout"
        grep -Fqx 'action_23l_outer_stdout_classification=bounded_safe' \
            "$action23l_case_stdout"
        grep -Fqx 'action_23l_outer_stderr_classification=bounded_safe' \
            "$action23l_case_stdout"
        [[ ! -s "$action23l_case_stderr" ]]
    else
        if grep -Fqx 'action_23l_outer_acceptance=true' "$action23l_case_stdout"; then
            return 1
        fi
        grep -Fqx 'action_23l_outer_acceptance=false' "$action23l_case_stderr"
    fi
    rm -rf -- "$action23l_case_dir"
}
...
require_true readiness_probe_explicit_status grep -Fq \
    'if action23l_probe_output=$(run_dns_probe_command' "$driver"
...
require_true candidate_parser_redirect_internal grep -Fq \
    'unbound-checkconf "$action23l_parser_root" >/dev/null' "$driver"
...
readonly fixture_mode=${ACTION23L_FIXTURE_MODE:?}
readonly fixture_driver=${ACTION23L_FIXTURE_DRIVER:?}
readonly fixture_log=${ACTION23L_FIXTURE_LOG:?}
...
case "$fixture_mode" in
    success)
        ...
        ;;
    missing_label)
        sed '/^action_23l_check_uid_is_root=true$/d' "$contract"
        ;;
    duplicate_label)
        cat "$contract"
        printf 'action_23l_check_uid_is_root=true\n'
        ;;
    false_label)
        sed 's/^action_23l_check_uid_is_root=true$/action_23l_check_uid_is_root=false/' \
            "$contract"
        ;;
    wrong_ftl_hash)
        sed 's/^action_23l_pihole_ftl_sha256=.*/action_23l_pihole_ftl_sha256=aaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaa/' \
            "$contract"
        ;;
    wrong_domain_hash)
        sed 's/^action_23l_pihole_domain_sha256=.*/action_23l_pihole_domain_sha256=bbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbb/' \
            "$contract"
        ;;
    stderr)
        cat "$contract"
...
printf 'action_23l_regression_node_contact=false\n'
printf 'action_23l_regression_live_mutation=false\n'
printf 'action_23l_regression_complete=true\n'
