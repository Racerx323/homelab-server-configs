...
readonly readiness_runner=$caddy_root/scripts/run-dual-node-caddy-readiness-action20d-retry10.sh
...
check_regression() {
    local retry10_regression_label=$1

    shift
    if "$@"; then
        printf 'action_20d_retry10_regression_check_%s=true\n' "$retry10_regression_label"
        return 0
    fi
    printf 'action_20d_retry10_regression_check_%s=false\n' \
        "$retry10_regression_label" >&2
    return 1
}

test_root=$(mktemp -d /tmp/caddy-action20d-retry10-regression.XXXXXX)
readonly test_root
...
make_gate() {
    ...
    # shellcheck disable=SC2016
    {
        ...
        printf '%s\n' 'printf "%s\n" "$fixture_label" >>"$ACTION20D_RETRY10_ORDER_LOG"'
        printf '%s\n' 'printf "fixture_%s_stdout=true\n" "$fixture_label"'
        printf '%s\n' 'printf "fixture_%s_stderr=true\n" "$fixture_label" >&2'
        printf '%s\n' 'if [[ "${ACTION20D_RETRY10_FAIL_GATE:-none}" = "$fixture_label" ]]; then'
...
check_regression readiness_runner_pins_probe \
    grep -Fq 'readonly probe_sha256=7f448b968df5c96feb11e5ca8d0d0cc738b1019edfa61a32ebde4c9c02ea98c0' \
    "$readiness_runner"

: >"$test_root/success.order"
ACTION20D_RETRY10_ORDER_LOG=$test_root/success.order \
...
for success_label in \
    action_20d_retry10_readiness_accepted=true \
    action_20d_retry10_activation_accepted=true \
    action_20d_retry10_boundary_cleanup_complete=true \
    action_20d_retry10_boundary_accepted=true \
    action_20d_retry10_readiness_stderr_classification=bounded_safe \
    action_20d_retry10_activation_stderr_classification=bounded_safe; do
...
printf 'action_20d_retry10_regression_success_order_and_evidence=true\n'
...
run_failure_case() {
    ...
    : >"$failure_order"
    ACTION20D_RETRY10_ORDER_LOG=$failure_order \
        ACTION20D_RETRY10_FAIL_GATE=$failed_gate_label \
        /bin/bash "$outer" --production-path-test \
        "$test_root/readiness" "$test_root/activation" \
    ...
    check_regression "${failed_gate_label}_failure_not_accepted" \
        grep -Fxq action_20d_retry10_boundary_accepted=false "$failure_stdout"
    if [[ "$failed_gate_label" = readiness ]]; then
        check_regression readiness_failure_activation_blocked \
            grep -Fxq action_20d_retry10_activation_invoked=false "$failure_stdout"
    else
...
printf 'action_20d_retry10_regression_failure_ordering_and_rollback=true\n'
...
check_regression node_a_production_candidate_hash_updated \
    grep -Fq 'expected_production_candidate_sha256=357eb09c628fa3b78efcaacf53278ab09a6574b991820632ee3ec2614265d8e2' \
    "$transaction"
candidate_contract_status=0
/bin/bash "$transaction" --candidate-contract-test \
    >"$test_root/candidate-contract.stdout" \
    2>"$test_root/candidate-contract.stderr" || candidate_contract_status=$?
check_regression candidate_contract_status_zero test \
    "$candidate_contract_status" -eq 0
check_regression candidate_contract_stderr_empty test ! -s \
    "$test_root/candidate-contract.stderr"
for candidate_contract_label in exact_bytes mode_exact \
    missing_blank_bytes_differ extra_blank_rejected \
    missing_terminal_newline_rejected reordered_include_rejected \
    embedded_fragment_rejected include_exactly_once line_count_exact; do
    check_regression "candidate_contract_${candidate_contract_label}" \
        test "$(grep -Fxc \
            "action_20d_retry10_node_candidate_contract_${candidate_contract_label}=true" \
            "$test_root/candidate-contract.stdout")" -eq 1
done
check_regression candidate_contract_complete test \
    "$(grep -Fxc 'action_20d_retry10_node_candidate_contract_complete=true' \
        "$test_root/candidate-contract.stdout")" -eq 1
check_regression live_path_uses_tested_builder test \
    "$(grep -Fxc "build_production_candidate \"\$main_configuration\" \"\$production_configuration\"" \
        "$transaction")" -eq 1
check_regression legacy_main_plus_fragment_hash_absent test \
    "$(grep -Fc '49ab7ef6d2b7c988f192b79e64175ac57f309c0fd639132e34858eda400cc460' \
        "$transaction" || true)" -eq 0
...
printf 'action_20d_retry10_regression_false_positive_and_false_negative_controls=true\n'
printf 'action_20d_retry10_regression_complete=true\n'
