...
readonly prefix=action_29k_regression
test_directory=$(cd -- "$(dirname -- "${BASH_SOURCE[0]}")" && pwd)
readonly test_directory
readonly caddy_root=${test_directory%/tests}
readonly inspector=$caddy_root/scripts/inspect-final-deployment-action29i.sh
readonly outer=$caddy_root/scripts/run-final-deployment-acceptance-action29k-outer.sh
readonly dns_probe=$caddy_root/scripts/run-dual-node-dns-record-families-action24-retry5-outer.sh
readonly protocol_probe=$caddy_root/scripts/run-workstation-caddy-http3-action26-h3-retry-outer.sh
fixture_root=
...
run_regression() {
    ...
    local action29i_regression_backup
    local action29i_regression_owner
    local action29i_regression_group
    local action29i_regression_before_ftl
    local action29i_regression_before_domain
    local action29i_regression_release
    local action29i_regression_manifest_output
    local action29i_regression_expected_labels
    local action29i_regression_dns_render
    local action29j_regression_dns_hashes
    local action29j_regression_dns_self_test_stdout
    local action29j_regression_dns_self_test_stderr

    fixture_root=$(mktemp -d /tmp/action29-regression.XXXXXX) || return 1
    trap 'rm -rf -- "$fixture_root"' EXIT INT TERM
    install -m 0600 /dev/null "$fixture_root/empty" || return 1
    action29i_regression_dns_render=$fixture_root/dns-render
    install -d -m 0700 "$action29i_regression_dns_render" || return 1
    /bin/bash "$dns_probe" --render-to "$action29i_regression_dns_render" || return 1
    action29j_regression_dns_hashes=$fixture_root/dns-hashes
    /bin/bash "$dns_probe" --render-hashes >"$action29j_regression_dns_hashes" || return 1
    check dns_rendered_inspector_hash_exact grep -Fqx \
        'rendered_inspector_sha256=7a5cd81e0b1aad8ed110ed6220319d47a3cce4ea4f7eea9b7bf5102723b6d018' \
        "$action29j_regression_dns_hashes" || return 1
    check dns_rendered_core_hash_exact grep -Fqx \
        'rendered_core_sha256=645e36c40dbdb47f1cdefbbcebc50eb948e26397c87ce5212fd616867d4b03c8' \
        "$action29j_regression_dns_hashes" || return 1
    check dns_stale_rendered_core_hash_absent reject_command grep -Fq \
        3afe9ff5ab8bbb8ec07151c4be074318eaf020d301e53be363e851002e2363e4 \
        "$dns_probe" || return 1
    check protocol_producer_prefix grep -Fqx \
        'readonly prefix=action_26_h3_retry_outer' "$protocol_probe" || return 1
    check protocol_producer_completion_emitter grep -Fq \
        "printf '%s_complete=true" "$protocol_probe" || return 1
    check protocol_consumer_marker_exact grep -Fq \
        "protocol) printf '%s\\n' action_26_h3_retry_outer_complete=true" \
        "$outer" || return 1
    check protocol_stale_consumer_marker_absent reject_command grep -Fq \
        "protocol) printf '%s\\n' action_26_h3_retry_complete=true" \
        "$outer" || return 1
    action29j_regression_dns_self_test_stdout=$fixture_root/dns-self-test.stdout
    action29j_regression_dns_self_test_stderr=$fixture_root/dns-self-test.stderr
    install -m 0600 /dev/null "$action29j_regression_dns_self_test_stdout" || return 1
    install -m 0600 /dev/null "$action29j_regression_dns_self_test_stderr" || return 1
    /bin/bash "$dns_probe" --self-test >"$action29j_regression_dns_self_test_stdout" \
        2>"$action29j_regression_dns_self_test_stderr" || return 1
    check dns_production_self_test_stderr_empty test ! -s \
        "$action29j_regression_dns_self_test_stderr" || return 1
    check dns_production_self_test_complete grep -Fqx \
        'action_24_retry_outer_self_test_complete=true' \
        "$action29j_regression_dns_self_test_stdout" || return 1
    check dns_production_ftl_hash grep -Fqx \
        ...
        "$action29i_regression_dns_render/inspector" || return 1
    ...
    check dns_stale_domain_hash_absent reject_command grep -Fq \
        ...
        "$action29i_regression_dns_render/inspector" || return 1
    check dns_stale_vrrp_exact_checks_absent reject_command grep -Eq \
        'vrrp_state_exact(_after)?' "$action29i_regression_dns_render/inspector" || return 1
    check dns_stale_vrrp_value_absent reject_command grep -Fq \
        value_vrrp_state "$action29i_regression_dns_render/inspector" || return 1
    check dns_stale_vrrp_consumer_absent reject_command grep -Fq \
        '_vrrp_state"' "$action29i_regression_dns_render/core" || return 1
    check dns_vrrp_file_continuity_retained grep -Fq \
        "printf 'vrrp=%s" "$action29i_regression_dns_render/inspector" || return 1
    action29i_regression_expected_labels=$fixture_root/expected-labels
    /bin/bash "$inspector" --expected-checks node-a \
        >"$action29i_regression_expected_labels" || return 1
    ...
    for action29_regression_role in node-a node-b; do
        ...
        sed 's/^action_29i_remote_\(.*\)_observed_pihole_ftl_sha256=.*/action_29i_remote_\1_observed_pihole_ftl_sha256=aaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaa/' \
    ...
    for action29_regression_component in dns protocol tls; do
        case "$action29_regression_component" in
            ...
            protocol) action29_regression_marker=action_26_h3_retry_outer_complete=true ;;
...
EOF
    ...
    action29i_regression_backup=$fixture_root/action29b-backup
    action29i_regression_owner=$(id -un) || return 1
    action29i_regression_group=$(id -gn) || return 1
    install -d -m 0700 "$action29i_regression_backup" || return 1
    printf '%s\n' before-ftl >"$action29i_regression_backup/pihole-FTL.conf.before" || return 1
    printf '%s\n' before-domain >"$action29i_regression_backup/local.theama.co.conf.before" || return 1
    printf '%s\n' committed >"$action29i_regression_backup/transaction.complete" || return 1
    chmod 0600 "$action29i_regression_backup/pihole-FTL.conf.before" \
        "$action29i_regression_backup/local.theama.co.conf.before" \
        "$action29i_regression_backup/transaction.complete" || return 1
    action29i_regression_before_ftl=$(sha256sum \
        "$action29i_regression_backup/pihole-FTL.conf.before" | awk '{ print $1 }') || return 1
    action29i_regression_before_domain=$(sha256sum \
        "$action29i_regression_backup/local.theama.co.conf.before" | awk '{ print $1 }') || return 1
    printf '%s\n' \
        'action=29b' 'role=node-a' \
        "before_ftl_sha256=$action29i_regression_before_ftl" \
        "before_domain_sha256=$action29i_regression_before_domain" \
        ...
        >"$action29i_regression_backup/manifest" || return 1
    chmod 0600 "$action29i_regression_backup/manifest" || return 1
    check action29b_backup_real_validator /bin/bash "$inspector" --validate-action29b-backup \
        node-a "$action29i_regression_backup" "$action29i_regression_owner" \
        "$action29i_regression_group" "$action29i_regression_before_ftl" \
        "$action29i_regression_before_domain" || return 1
    sed 's/^action=29b$/action=wrong/' "$action29i_regression_backup/manifest" \
        >"$fixture_root/altered-manifest" || return 1
    mv -fT "$fixture_root/altered-manifest" "$action29i_regression_backup/manifest" || return 1
    chmod 0600 "$action29i_regression_backup/manifest" || return 1
    check action29b_backup_altered_rejected reject_command /bin/bash "$inspector" \
        --validate-action29b-backup node-a "$action29i_regression_backup" \
        "$action29i_regression_owner" "$action29i_regression_group" \
        "$action29i_regression_before_ftl" "$action29i_regression_before_domain" || return 1
    action29i_regression_release=$fixture_root/release
    install -d -m 0700 "$action29i_regression_release" || return 1
    printf 'fixture\n' >"$action29i_regression_release/Caddyfile" || return 1
    (
        cd -- "$action29i_regression_release" || exit 1
        sha256sum ./Caddyfile >manifest.sha256
    ) || return 1
    check manifest_paths_real_producer /bin/bash "$inspector" --validate-manifest-paths \
        "$action29i_regression_release/manifest.sha256" || return 1
    check manifest_file_set_real_producer /bin/bash "$inspector" --validate-manifest-file-set \
        "$action29i_regression_release" || return 1
    action29i_regression_manifest_output=$fixture_root/manifest-valid.out
    /bin/bash "$inspector" --capture-manifest-fixture node-a \
        "$action29i_regression_release" >"$action29i_regression_manifest_output" || return 1
    check manifest_hash_real_producer_status grep -Fqx \
        'action_29i_remote_node_a_manifest_hash_check_status=0' \
        "$action29i_regression_manifest_output" || return 1
    check manifest_hash_real_producer_content grep -Fqx './Caddyfile: OK' \
        "$action29i_regression_manifest_output" || return 1
    printf 'drift\n' >>"$action29i_regression_release/Caddyfile" || return 1
    action29i_regression_manifest_output=$fixture_root/manifest-invalid.out
    /bin/bash "$inspector" --capture-manifest-fixture node-a \
        "$action29i_regression_release" >"$action29i_regression_manifest_output" || return 1
    check manifest_hash_drift_status grep -Fqx \
        'action_29i_remote_node_a_manifest_hash_check_status=1' \
        "$action29i_regression_manifest_output" || return 1
    check manifest_hash_drift_content grep -Fqx './Caddyfile: FAILED' \
        "$action29i_regression_manifest_output" || return 1
    printf '%064d  ../escape\n' 0 >"$action29i_regression_release/manifest.sha256" || return 1
    check manifest_unsafe_path_rejected reject_command /bin/bash "$inspector" \
        --validate-manifest-paths "$action29i_regression_release/manifest.sha256" || return 1
    check manifest_file_set_drift_rejected reject_command /bin/bash "$inspector" \
        --validate-manifest-file-set "$action29i_regression_release" || return 1
    # shellcheck disable=SC2016
    ...
    check production_manifest_path_gate grep -Fq \
        'check current_manifest_paths_safe test "$action29i_remote_manifest_paths_status" -eq 0' \
        "$inspector" || return 1
    # shellcheck disable=SC2016
    check production_manifest_file_set_gate grep -Fq \
        'check current_manifest_file_set_exact test "$action29i_remote_manifest_file_set_status" -eq 0' \
        "$inspector" || return 1
    # shellcheck disable=SC2016
    ...
    check cleanup_unset_safe grep -Fqx \
        'action_29i_remote_cleanup_unset_safe=true' "$action29_regression_cleanup" || return 1
