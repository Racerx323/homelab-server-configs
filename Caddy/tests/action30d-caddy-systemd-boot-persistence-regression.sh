...
readonly prefix=action_30d_regression
...
check() {
    local action30d_regression_label=$1

    shift
    if "$@"; then
        printf '%s_check_%s=true\n' "$prefix" "$action30d_regression_label"
        return 0
    fi
    printf '%s_check_%s=false\n' "$prefix" "$action30d_regression_label" >&2
    return 1
}
run_outer() {
    local action30d_regression_fail=${1:-}
    local action30d_regression_status=0

    ACTION30D_FAKE_CALLS=$fixture_root/calls \
        ACTION30D_FAKE_PAYLOADS=$fixture_root/payloads \
        ACTION30D_FAKE_FAIL=$action30d_regression_fail \
        CADDY_ACTION30D_SSH_BIN=$fixture_root/fake-ssh \
        CADDY_ACTION30D_EVIDENCE_ROOT=$fixture_root/evidence \
        CADDY_ACTION30D_SKIP_REGRESSION=true \
        CADDY_ACTION30D_SKIP_REPEATED_POLICIES=true \
        /bin/bash "$outer" >"$fixture_root/outer.stdout" \
        2>"$fixture_root/outer.stderr" || action30d_regression_status=$?
    printf '%s\n' "$action30d_regression_status" >"$fixture_root/outer.status"
}
run_regression() {
    local action30d_regression_transaction_sha256
    local action30d_regression_status_file
    local action30d_semantic_stdout

    fixture_root=$(mktemp -d /tmp/action30d-regression.XXXXXX) || return 1
    trap 'rm -rf -- "$fixture_root"' EXIT INT TERM
    action30d_regression_transaction_sha256=$(sha256sum "$transaction" | awk '{ print $1 }') || return 1
    cat >"$fixture_root/fake-ssh" <<'FAKE'
#!/usr/bin/env bash
set -Eeuo pipefail
command_line=${!#}
payload=$(mktemp "$ACTION30D_FAKE_PAYLOADS/payload.XXXXXX")
cat >"$payload"
printf '%s\n' "$command_line" >>"$ACTION30D_FAKE_CALLS"
printf '%s\n' "$(sha256sum "$payload" | awk '{ print $1 }')" >>"$ACTION30D_FAKE_PAYLOADS/hashes"
if [[ -n ${ACTION30D_FAKE_FAIL:-} && $command_line == *"$ACTION30D_FAKE_FAIL"* ]]; then
    exit 1
fi
case "$command_line" in
    *'--apply node-a') printf 'action_30d_remote_node_a_complete=true\n' ;;
    *'--apply node-b') printf 'action_30d_remote_node_b_complete=true\n' ;;
    *'--verify node-a') printf 'action_30d_remote_node_a_verify_complete=true\n' ;;
    *'--verify node-b') printf 'action_30d_remote_node_b_verify_complete=true\n' ;;
    *'--verify-continuity node-a') printf 'action_30d_remote_node_a_verify_continuity_complete=true\n' ;;
    *'--verify-continuity node-b') printf 'action_30d_remote_node_b_verify_continuity_complete=true\n' ;;
    *'--rollback node-a') printf 'action_30d_remote_node_a_rollback_complete=true\n' ;;
    *'--rollback node-b') printf 'action_30d_remote_node_b_rollback_complete=true\n' ;;
    *) exit 64 ;;
esac
FAKE
    ...
    action30d_regression_status_file=$fixture_root/lsyncd.status
    printf 'Lsyncd status report at Tue Aug 11 18:00:00 2026\nSync1 source=/var/lib/caddy-sync/outbound/\nThere are 0 delays\n' \
        >"$action30d_regression_status_file" || return 1
    ...
    action30d_semantic_stdout=$fixture_root/semantic.stdout
    /bin/bash "$transaction" --semantic-self-test >"$action30d_semantic_stdout" || return 1
    check multiple_node_b_revisions_accepted grep -Fqx \
        'action_30d_remote_role_based_multiple_node_b_revisions_quarantined=true' \
        "$action30d_semantic_stdout" || return 1
    check complete_invalid_inventory_emitted grep -Fqx \
        'action_30d_remote_hidden_symlink_special_complete_inventory_emitted=true' \
        "$action30d_semantic_stdout" || return 1
    check invalid_inventory_rejected grep -Fqx \
        'action_30d_remote_hidden_symlink_special_entries_rejected=true' \
        "$action30d_semantic_stdout" || return 1
    touch -d '2 days ago' "$action30d_regression_status_file" || return 1
    check old_parseable_snapshot_accepted /bin/bash "$health_worker" \
        --validate-status-file "$action30d_regression_status_file" || return 1
    printf 'not an lsyncd status snapshot\n' >"$action30d_regression_status_file" || return 1
    if /bin/bash "$health_worker" --validate-status-file "$action30d_regression_status_file"; then
        return 1
    fi
    ...
    check health_worker_source_matches_embedded /bin/bash -c '
        ...
        extracted=$3
        compare_worker() {
            awk '\''/^    cat >.*ACTION30D_HEALTH_WORKER/{capture=1; next} /^ACTION30D_HEALTH_WORKER$/{capture=0; exit} capture { print }'\'' \
    ...
    check node_b_all_candidates_quarantined grep -Fq \
        '[[ "$action30d_source" = node-b ]]' "$transaction" || return 1
    check node_a_exact_current_identity_compared grep -Fq \
        '"$(file_hash "$action30d_current_candidate/manifest.sha256")"' \
        "$transaction" || return 1
    ...
    check success_complete grep -Fqx 'action_30d_outer_complete=true' "$fixture_root/outer.stdout" || return 1
...
ORDER
    check exact_generated_payload awk -v expected="$action30d_regression_transaction_sha256" '
        $0 != expected { exit 1 }
        END { exit !(NR == 5) }
    ...
    check recovery_proven grep -Fqx 'action_30d_outer_recovery_proven=true' "$fixture_root/outer.stdout" || return 1
