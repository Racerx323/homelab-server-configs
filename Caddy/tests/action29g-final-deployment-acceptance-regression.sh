...
readonly prefix=action_29g_regression
test_directory=$(cd -- "$(dirname -- "${BASH_SOURCE[0]}")" && pwd)
readonly test_directory
readonly caddy_root=${test_directory%/tests}
readonly inspector=$caddy_root/scripts/inspect-final-deployment-action29g.sh
readonly outer=$caddy_root/scripts/run-final-deployment-acceptance-action29g-outer.sh
readonly dns_probe=$caddy_root/scripts/run-dual-node-dns-record-families-action24-retry2-outer.sh
fixture_root=
...
run_regression() {
    ...
    local action29g_regression_backup
    local action29g_regression_owner
    local action29g_regression_group
    local action29g_regression_before_ftl
    local action29g_regression_before_domain
    local action29g_regression_release
    local action29g_regression_manifest_output
    local action29g_regression_expected_labels
    local action29g_regression_dns_render

    fixture_root=$(mktemp -d /tmp/action29-regression.XXXXXX) || return 1
    trap 'rm -rf -- "$fixture_root"' EXIT INT TERM
    install -m 0600 /dev/null "$fixture_root/empty" || return 1
    action29g_regression_dns_render=$fixture_root/dns-render
    install -d -m 0700 "$action29g_regression_dns_render" || return 1
    /bin/bash "$dns_probe" --render-to "$action29g_regression_dns_render" || return 1
    check dns_production_ftl_hash grep -Fqx \
        ...
        "$action29g_regression_dns_render/inspector" || return 1
    ...
    check dns_stale_domain_hash_absent reject_command grep -Fq \
        ...
        "$action29g_regression_dns_render/inspector" || return 1
    action29g_regression_expected_labels=$fixture_root/expected-labels
    /bin/bash "$inspector" --expected-checks node-a \
        >"$action29g_regression_expected_labels" || return 1
    ...
    for action29_regression_role in node-a node-b; do
        ...
        sed 's/^action_29g_remote_\(.*\)_observed_pihole_ftl_sha256=.*/action_29g_remote_\1_observed_pihole_ftl_sha256=aaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaa/' \
...
EOF
    ...
    action29g_regression_backup=$fixture_root/action29b-backup
    action29g_regression_owner=$(id -un) || return 1
    action29g_regression_group=$(id -gn) || return 1
    install -d -m 0700 "$action29g_regression_backup" || return 1
    printf '%s\n' before-ftl >"$action29g_regression_backup/pihole-FTL.conf.before" || return 1
    printf '%s\n' before-domain >"$action29g_regression_backup/local.theama.co.conf.before" || return 1
    printf '%s\n' committed >"$action29g_regression_backup/transaction.complete" || return 1
    chmod 0600 "$action29g_regression_backup/pihole-FTL.conf.before" \
        "$action29g_regression_backup/local.theama.co.conf.before" \
        "$action29g_regression_backup/transaction.complete" || return 1
    action29g_regression_before_ftl=$(sha256sum \
        "$action29g_regression_backup/pihole-FTL.conf.before" | awk '{ print $1 }') || return 1
    action29g_regression_before_domain=$(sha256sum \
        "$action29g_regression_backup/local.theama.co.conf.before" | awk '{ print $1 }') || return 1
    printf '%s\n' \
        'action=29b' 'role=node-a' \
        "before_ftl_sha256=$action29g_regression_before_ftl" \
        "before_domain_sha256=$action29g_regression_before_domain" \
        ...
        >"$action29g_regression_backup/manifest" || return 1
    chmod 0600 "$action29g_regression_backup/manifest" || return 1
    check action29b_backup_real_validator /bin/bash "$inspector" --validate-action29b-backup \
        node-a "$action29g_regression_backup" "$action29g_regression_owner" \
        "$action29g_regression_group" "$action29g_regression_before_ftl" \
        "$action29g_regression_before_domain" || return 1
    sed 's/^action=29b$/action=wrong/' "$action29g_regression_backup/manifest" \
        >"$fixture_root/altered-manifest" || return 1
    mv -fT "$fixture_root/altered-manifest" "$action29g_regression_backup/manifest" || return 1
    chmod 0600 "$action29g_regression_backup/manifest" || return 1
    check action29b_backup_altered_rejected reject_command /bin/bash "$inspector" \
        --validate-action29b-backup node-a "$action29g_regression_backup" \
        "$action29g_regression_owner" "$action29g_regression_group" \
        "$action29g_regression_before_ftl" "$action29g_regression_before_domain" || return 1
    action29g_regression_release=$fixture_root/release
    install -d -m 0700 "$action29g_regression_release" || return 1
    printf 'fixture\n' >"$action29g_regression_release/Caddyfile" || return 1
    (
        cd -- "$action29g_regression_release" || exit 1
        sha256sum ./Caddyfile >manifest.sha256
    ) || return 1
    check manifest_paths_real_producer /bin/bash "$inspector" --validate-manifest-paths \
        "$action29g_regression_release/manifest.sha256" || return 1
    check manifest_file_set_real_producer /bin/bash "$inspector" --validate-manifest-file-set \
        "$action29g_regression_release" || return 1
    action29g_regression_manifest_output=$fixture_root/manifest-valid.out
    /bin/bash "$inspector" --capture-manifest-fixture node-a \
        "$action29g_regression_release" >"$action29g_regression_manifest_output" || return 1
    check manifest_hash_real_producer_status grep -Fqx \
        'action_29g_remote_node_a_manifest_hash_check_status=0' \
        "$action29g_regression_manifest_output" || return 1
    check manifest_hash_real_producer_content grep -Fqx './Caddyfile: OK' \
        "$action29g_regression_manifest_output" || return 1
    printf 'drift\n' >>"$action29g_regression_release/Caddyfile" || return 1
    action29g_regression_manifest_output=$fixture_root/manifest-invalid.out
    /bin/bash "$inspector" --capture-manifest-fixture node-a \
        "$action29g_regression_release" >"$action29g_regression_manifest_output" || return 1
    check manifest_hash_drift_status grep -Fqx \
        'action_29g_remote_node_a_manifest_hash_check_status=1' \
        "$action29g_regression_manifest_output" || return 1
    check manifest_hash_drift_content grep -Fqx './Caddyfile: FAILED' \
        "$action29g_regression_manifest_output" || return 1
    printf '%064d  ../escape\n' 0 >"$action29g_regression_release/manifest.sha256" || return 1
    check manifest_unsafe_path_rejected reject_command /bin/bash "$inspector" \
        --validate-manifest-paths "$action29g_regression_release/manifest.sha256" || return 1
    check manifest_file_set_drift_rejected reject_command /bin/bash "$inspector" \
        --validate-manifest-file-set "$action29g_regression_release" || return 1
    # shellcheck disable=SC2016
    ...
    check production_manifest_path_gate grep -Fq \
        'check current_manifest_paths_safe test "$action29g_remote_manifest_paths_status" -eq 0' \
        "$inspector" || return 1
    # shellcheck disable=SC2016
    check production_manifest_file_set_gate grep -Fq \
        'check current_manifest_file_set_exact test "$action29g_remote_manifest_file_set_status" -eq 0' \
        "$inspector" || return 1
    # shellcheck disable=SC2016
    ...
    check cleanup_unset_safe grep -Fqx \
        'action_29g_remote_cleanup_unset_safe=true' "$action29_regression_cleanup" || return 1
