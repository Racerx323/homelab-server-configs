...
readonly transaction=$caddy_root/scripts/transact-coupled-go-live-action28y.sh
readonly outer=$caddy_root/scripts/run-dual-node-coupled-go-live-action28y-outer.sh
...
require_text() {
    local action28y_regression_file=$1
    local action28y_regression_text=$2

    grep -Fq -- "$action28y_regression_text" "$action28y_regression_file"
}
reject_text() {
    local action28y_regression_file=$1
    local action28y_regression_text=$2
...
line_number() {
    ...
    grep -Fn -- "$action28y_regression_text" "$action28y_regression_file" |
        awk -F: 'NR == 1 { print $1 }'
}
require_order() {
    local action28y_regression_file=$1
    local action28y_regression_previous=0
    local action28y_regression_text
    local action28y_regression_current

    shift
    for action28y_regression_text in "$@"; do
        action28y_regression_current=$(line_number \
            "$action28y_regression_file" "$action28y_regression_text")
        [[ -n "$action28y_regression_current" ]]
        [[ "$action28y_regression_current" -gt "$action28y_regression_previous" ]]
        action28y_regression_previous=$action28y_regression_current
    done
}
...
"$transaction" --self-test | grep -Fxq \
    'action_28y_remote_self_test_complete=true'
action28y_regression_remote_classifier_output=$(
    "$transaction" --stream-classifier-self-test
)
action28y_regression_quarantine_output=$(
    "$transaction" --historical-quarantine-self-test
)
for action28y_regression_quarantine_suffix in \
    ...
    grep -Fxq \
        "action_28y_remote_${action28y_regression_quarantine_suffix}=true" \
        <<<"$action28y_regression_quarantine_output"
done
action28y_regression_outer_classifier_output=$(
    /bin/bash "$outer" --stream-classifier-self-test
)
for action28y_regression_classifier_suffix in \
    ...
    grep -Fxq \
        "action_28y_remote_stream_classifier_${action28y_regression_classifier_suffix}=true" \
        <<<"$action28y_regression_remote_classifier_output"
    grep -Fxq \
        "action_28y_outer_stream_classifier_${action28y_regression_classifier_suffix}=true" \
        <<<"$action28y_regression_outer_classifier_output"
done
action28y_regression_producer_output=$(/bin/bash "$outer" --producer-self-test)
grep -Fxq 'action_28y_outer_producer_remote_valid=true' \
    <<<"$action28y_regression_producer_output"
grep -Fxq 'action_28y_outer_producer_capture_files_valid=true' \
    <<<"$action28y_regression_producer_output"
grep -Fxq 'action_28y_outer_producer_malformed_rejected=true' \
    <<<"$action28y_regression_producer_output"
grep -Fxq 'action_28y_outer_producer_self_test_complete=true' \
    <<<"$action28y_regression_producer_output"
action28y_regression_recovery_output=$(
    /bin/bash "$outer" --pre-mutation-recovery-self-test
)
grep -Fxq 'action_28y_outer_recovery_started=true' \
    <<<"$action28y_regression_recovery_output"
grep -Fxq 'action_28y_outer_recovery_not_required=true' \
    <<<"$action28y_regression_recovery_output"
grep -Fxq 'action_28y_outer_recovery_proven=true' \
    <<<"$action28y_regression_recovery_output"
grep -Fxq 'action_28y_outer_pre_mutation_recovery_self_test_complete=true' \
    <<<"$action28y_regression_recovery_output"
if grep -Fq 'phase_recovery_' <<<"$action28y_regression_recovery_output"; then
    exit 1
fi
...
require_text "$outer" '/tmp/caddy-ssh-evidence/action28y'
require_text "$outer" "'cd / && sudo -n /bin/bash -s --'"
# These are exact source literals, not expressions to expand.
# shellcheck disable=SC2016
require_text "$outer" \
    'printf '\''/bin/bash "$stage/transact-coupled-go-live-action28y.sh" --mode %q'
# shellcheck disable=SC2016
require_text "$outer" \
...
# shellcheck disable=SC2016
require_text "$transaction" '"$evidence_directory/$action28y_remote_label.stdout"'
# shellcheck disable=SC2016
require_text "$transaction" '"$evidence_directory/$action28y_remote_label.stderr"'
for action28y_regression_lsyncd_capture in \
    lsyncd_journal_cursor enable_lsyncd lsyncd_stability lsyncd_show \
    lsyncd_status lsyncd_journal; do
    require_text "$transaction" "run_captured $action28y_regression_lsyncd_capture"
done
...
for action28y_regression_config in "$node_a_lsyncd" "$node_b_lsyncd"; do
    require_text "$action28y_regression_config" 'delete = false'
    require_text "$action28y_regression_config" '"--exclude=.complete"'
    require_text "$action28y_regression_config" 'IdentitiesOnly = "yes"'
done

printf 'action_28y_regression_complete=true\n'
