...
readonly prefix=action_20h_retry_focused
test_directory=$(cd -- "$(dirname -- "${BASH_SOURCE[0]}")" && pwd)
readonly test_directory
readonly caddy_root=${test_directory%/tests}
readonly candidate=$caddy_root/scripts/check-caddy-vrrp-action20h.sh
readonly installer=$caddy_root/scripts/install-node-a-caddy-health-helper-action20h-retry.sh
readonly stager=$caddy_root/scripts/stage-node-a-caddy-health-helper-action20h.sh
readonly runner=$caddy_root/scripts/run-node-a-caddy-health-helper-action20h-retry.sh
readonly outer=$caddy_root/scripts/run-node-a-caddy-health-helper-action20h-retry-outer.sh
readonly regression=$test_directory/action20h-retry-stale-path-regression.sh
...
readonly outer_sha256=e72b77ba02bdf7e0ae868e59bb695028419a4253ba777b0c1b66a0d6f29160ff
readonly regression_sha256=5ed35e5da3285e3dae1097a68b505da1caa7f1974febf52e3ef11a0f6268b0f4

file_hash() { sha256sum "$1" | awk '{ print $1 }'; }
gate() {
    local action20h_retry_focused_label=$1

    shift
    if "$@"; then
        printf '%s_check_%s=true\n' "$prefix" "$action20h_retry_focused_label"
        return 0
    fi
    printf '%s_check_%s=false\n' "$prefix" "$action20h_retry_focused_label" >&2
    return 1
}
source_hashes_exact() {
