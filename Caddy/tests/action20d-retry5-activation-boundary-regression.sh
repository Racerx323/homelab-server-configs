...
readonly outer=$caddy_root/scripts/run-dual-node-caddy-vrrp-activation-action20d-retry5-outer.sh
readonly collision=$script_directory/check-shell-readonly-local-collisions-v2.sh

check_regression() {
    local retry5_regression_label=$1

    shift
    if "$@"; then
        printf 'action_20d_retry5_regression_check_%s=true\n' "$retry5_regression_label"
        return 0
    fi
    printf 'action_20d_retry5_regression_check_%s=false\n' \
        "$retry5_regression_label" >&2
    return 1
}

test_root=$(mktemp -d /tmp/caddy-action20d-retry5-regression.XXXXXX)
readonly test_root
...
make_gate() {
    ...
    # shellcheck disable=SC2016
    {
        ...
        printf '%s\n' 'printf "%s\n" "$fixture_label" >>"$ACTION20D_RETRY5_ORDER_LOG"'
        printf '%s\n' 'printf "fixture_%s_stdout=true\n" "$fixture_label"'
        printf '%s\n' 'printf "fixture_%s_stderr=true\n" "$fixture_label" >&2'
        printf '%s\n' 'if [[ "${ACTION20D_RETRY5_FAIL_GATE:-none}" = "$fixture_label" ]]; then'
...
ACTION20D_RETRY5_ORDER_LOG=$test_root/success.order \
...
for success_label in \
    action_20d_retry5_readiness_accepted=true \
    action_20d_retry5_activation_accepted=true \
    action_20d_retry5_boundary_cleanup_complete=true \
    action_20d_retry5_boundary_accepted=true \
    action_20d_retry5_readiness_stderr_classification=bounded_safe \
    action_20d_retry5_activation_stderr_classification=bounded_safe; do
...
printf 'action_20d_retry5_regression_success_order_and_evidence=true\n'
...
run_failure_case() {
    ...
    : >"$failure_order"
    ACTION20D_RETRY5_ORDER_LOG=$failure_order \
        ACTION20D_RETRY5_FAIL_GATE=$failed_gate_label \
        /bin/bash "$outer" --production-path-test \
        "$test_root/readiness" "$test_root/activation" \
    ...
    check_regression "${failed_gate_label}_failure_not_accepted" \
        grep -Fxq action_20d_retry5_boundary_accepted=false "$failure_stdout"
    if [[ "$failed_gate_label" = readiness ]]; then
        check_regression readiness_failure_activation_blocked \
            grep -Fxq action_20d_retry5_activation_invoked=false "$failure_stdout"
    else
...
printf 'action_20d_retry5_regression_failure_ordering_and_rollback=true\n'
...
printf 'action_20d_retry5_regression_false_positive_and_false_negative_controls=true\n'
printf 'action_20d_retry5_regression_complete=true\n'
