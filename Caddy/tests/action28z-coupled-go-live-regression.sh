...
readonly transaction=$caddy_root/scripts/transact-coupled-go-live-action28z.sh
readonly outer=$caddy_root/scripts/run-dual-node-coupled-go-live-action28z-outer.sh
...
require_text() {
    local action28z_regression_file=$1
    local action28z_regression_text=$2

    grep -Fq -- "$action28z_regression_text" "$action28z_regression_file"
}
reject_text() {
    local action28z_regression_file=$1
    local action28z_regression_text=$2
...
line_number() {
    ...
    grep -Fn -- "$action28z_regression_text" "$action28z_regression_file" |
        awk -F: 'NR == 1 { print $1 }'
}
require_order() {
    local action28z_regression_file=$1
    local action28z_regression_previous=0
    local action28z_regression_text
    local action28z_regression_current

    shift
    for action28z_regression_text in "$@"; do
        action28z_regression_current=$(line_number \
            "$action28z_regression_file" "$action28z_regression_text")
        [[ -n "$action28z_regression_current" ]]
        [[ "$action28z_regression_current" -gt "$action28z_regression_previous" ]]
        action28z_regression_previous=$action28z_regression_current
    done
}
...
"$transaction" --self-test | grep -Fxq \
    'action_28z_remote_self_test_complete=true'
action28z_regression_remote_classifier_output=$(
    "$transaction" --stream-classifier-self-test
)
action28z_regression_quarantine_output=$(
    "$transaction" --historical-quarantine-self-test
)
for action28z_regression_quarantine_suffix in \
    ...
    grep -Fxq \
        "action_28z_remote_${action28z_regression_quarantine_suffix}=true" \
        <<<"$action28z_regression_quarantine_output"
done
action28z_regression_outer_classifier_output=$(
    /bin/bash "$outer" --stream-classifier-self-test
)
for action28z_regression_classifier_suffix in \
    ...
    grep -Fxq \
        "action_28z_remote_stream_classifier_${action28z_regression_classifier_suffix}=true" \
        <<<"$action28z_regression_remote_classifier_output"
    grep -Fxq \
        "action_28z_outer_stream_classifier_${action28z_regression_classifier_suffix}=true" \
        <<<"$action28z_regression_outer_classifier_output"
done
action28z_regression_producer_output=$(/bin/bash "$outer" --producer-self-test)
grep -Fxq 'action_28z_outer_producer_remote_valid=true' \
    <<<"$action28z_regression_producer_output"
grep -Fxq 'action_28z_outer_producer_capture_files_valid=true' \
    <<<"$action28z_regression_producer_output"
grep -Fxq 'action_28z_outer_producer_malformed_rejected=true' \
    <<<"$action28z_regression_producer_output"
grep -Fxq 'action_28z_outer_producer_self_test_complete=true' \
    <<<"$action28z_regression_producer_output"
action28z_regression_recovery_output=$(
    /bin/bash "$outer" --pre-mutation-recovery-self-test
)
grep -Fxq 'action_28z_outer_recovery_started=true' \
    <<<"$action28z_regression_recovery_output"
grep -Fxq 'action_28z_outer_recovery_not_required=true' \
    <<<"$action28z_regression_recovery_output"
grep -Fxq 'action_28z_outer_recovery_proven=true' \
    <<<"$action28z_regression_recovery_output"
grep -Fxq 'action_28z_outer_pre_mutation_recovery_self_test_complete=true' \
    <<<"$action28z_regression_recovery_output"
if grep -Fq 'phase_recovery_' <<<"$action28z_regression_recovery_output"; then
    exit 1
fi
...
require_text "$outer" '/tmp/caddy-ssh-evidence/action28z'
require_text "$outer" "'cd / && sudo -n /bin/bash -s --'"
# These are exact source literals, not expressions to expand.
# shellcheck disable=SC2016
require_text "$outer" \
    'printf '\''/bin/bash "$stage/transact-coupled-go-live-action28z.sh" --mode %q'
# shellcheck disable=SC2016
require_text "$outer" \
...
# shellcheck disable=SC2016
require_text "$transaction" '"$evidence_directory/$action28z_remote_label.stdout"'
# shellcheck disable=SC2016
require_text "$transaction" '"$evidence_directory/$action28z_remote_label.stderr"'
for action28z_regression_lsyncd_capture in \
    lsyncd_journal_cursor enable_lsyncd lsyncd_stability lsyncd_show \
    lsyncd_status lsyncd_journal; do
    require_text "$transaction" "run_captured $action28z_regression_lsyncd_capture"
done
...
for action28z_regression_config in "$node_a_lsyncd" "$node_b_lsyncd"; do
    reject_text "$action28z_regression_config" 'logfile ='
    require_text "$action28z_regression_config" 'statusFile = "/run/caddy-lsyncd/status"'
    require_text "$action28z_regression_config" 'delete = false'
    require_text "$action28z_regression_config" '"--exclude=.complete"'
    require_text "$action28z_regression_config" 'IdentitiesOnly = "yes"'
done
require_text "$caddy_root/systemd/caddy-lsyncd.service" \
    'ExecStart=/usr/bin/lsyncd -nodaemon /etc/lsyncd/caddy.lua'

printf 'action_28z_regression_complete=true\n'
