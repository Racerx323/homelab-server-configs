...
readonly caddy_root
readonly transaction="$caddy_root/scripts/migrate-node-b-retained-release-marker-action17s-retry.sh"
readonly runner="$caddy_root/scripts/run-node-b-retained-release-marker-migration-action17s-retry.sh"
...
readonly runner_sha256=f5319707d8943eb9f129da5c31b8724cf0ef608eee3847c10ac43e9122a96700
...
assert_static_policy() {
    ...
    grep -Fq 'action17s-retry-node-b-marker-migration.XXXXXX' "$transaction"
    grep -Fq 'persistent_mutation_scope=finalize_request,complete,rollback_metadata' \
        "$transaction"
    grep -Fq "readonly expected_finalizer_sha256=$expected_finalizer_sha256" \
        "$transaction"
    for evidence_token in bytes lines sha256 classification; do
        grep -Fq "%s_value_%s_${evidence_token}=%s" "$transaction"
        grep -Fq "%s_remote_%s_${evidence_token}=%s" "$runner"
    done
    ...
    grep -Fq 'emit_stream_evidence finalizer_stderr "$finalizer_error"' \
        "$transaction"
    # shellcheck disable=SC2016
    grep -Fq 'emit_stream_evidence stdout "$remote_output"' "$runner"
    # shellcheck disable=SC2016
    grep -Fq 'emit_stream_evidence stderr "$remote_error"' "$runner"
    ...
    if grep -Eq \
        ...
        printf 'Action 17s retry contains a service mutation.\n' >&2
        return 1
    fi
    if grep -Eq '(^|[[:space:]])(rsync|scp|sftp)([[:space:]]|$)' \
        "$transaction" "$runner"; then
        printf 'Action 17s retry contains a release transfer.\n' >&2
        return 1
    fi
    # shellcheck disable=SC2016
    if grep -Fq 'rm -rf -- "$release"' "$transaction"; then
        printf 'Action 17s retry rollback removes the retained release.\n' >&2
        return 1
    fi
    if grep -Eq 'ACTION17S_(FIXTURE|CAPTURED|SSH_ARGUMENTS|STATUS)' "$runner"; then
        printf 'Production Action 17s retry runner contains a fixture bypass.\n' >&2
        return 1
    fi
}
...
write_success_fixture() {
    ...
    fixture_count=$("$transaction" --expected-checks | wc -l)
    {
        ...
        printf '%s\n' \
            ...
            action_17s_retry_value_revision=action17p-node-a-to-node-b-bootstrap \
...
run_production_path_regression() {
    ...
    local empty_error
    local missing_stream_fixture
    local rollback_error
    local rollback_output
    local valid_fixture

    case_root=$(mktemp -d /tmp/caddy-action17s-retry-regression.XXXXXX)
    ...
    case_runner="$case_root/Caddy/scripts/run-node-b-retained-release-marker-migration-action17s-retry.sh"
    ...
    grep -Fxq action_17s_retry_runner_acceptance=true "$case_root/valid.out"
    grep -Fxq action_17s_retry_workstation_cleanup_complete=true "$case_root/valid.out"
    ...
    grep -Fq 'Action 17s retry success transcript contract failed.' \
        "$case_root/duplicate.err"

    missing_stream_fixture="$case_root/missing-stream.fixture"
    cp -- "$valid_fixture" "$missing_stream_fixture"
    sed -i '/action_17s_retry_value_finalizer_stdout_sha256=/d' \
        "$missing_stream_fixture"
    run_case \
        "$empty_error" "$missing_stream_fixture" "$case_root" "$case_runner" \
        0 missing-stream
    [[ "$observed_status" -eq 97 ]]
    grep -Fq 'Action 17s retry success transcript contract failed.' \
        "$case_root/missing-stream.err"

    rollback_output="$case_root/rollback.out.fixture"
    rollback_error="$case_root/rollback.err.fixture"
    printf '%s\n' \
        action_17s_retry_preflight_complete=true \
        action_17s_retry_mutation_started=true >"$rollback_output"
    {
    ...
    grep -Fxq action_17s_retry_runner_acceptance=false "$case_root/rollback.out"
    grep -Fxq action_17s_retry_workstation_cleanup_complete=true \
    ...
    [[ "$observed_status" -eq 97 ]]
    grep -Fq 'Action 17s retry rollback evidence is incomplete.' \
        "$case_root/incomplete.err"

    printf '%s\n' \
        action_17s_retry_false_negative_valid_success_accepted=true \
        action_17s_retry_false_negative_complete_rollback_preserved=true \
        action_17s_retry_false_positive_duplicate_assertion_rejected=true \
        action_17s_retry_false_positive_missing_stream_evidence_rejected=true \
        action_17s_retry_false_positive_incomplete_rollback_rejected=true \
        action_17s_retry_production_path_network_contact=false
}
...
printf 'action_17s_retry_regression_assertion_integrity_hashes=true\n'
bash -n "$transaction" "$runner"
shellcheck "$transaction" "$runner"
printf 'action_17s_retry_regression_assertion_shell_validation=true\n'
"$collision_checker" "$transaction" "$runner" "$0" >/dev/null
printf 'action_17s_retry_regression_assertion_readonly_local_collisions_absent=true\n'
"$transaction" --self-test >/dev/null
"$runner" --self-test >/dev/null
"$runner" --contract-test >/dev/null
printf 'action_17s_retry_regression_assertion_self_contract_tests=true\n'
"$source_context_policy" --runner "$runner" >/dev/null
printf 'action_17s_retry_regression_assertion_source_context=true\n'
[[ "$("$transaction" --expected-checks | wc -l)" -eq 144 ]]
[[ "$("$transaction" --expected-checks | LC_ALL=C sort -u | wc -l)" -eq 144 ]]
[[ "$("$transaction" --expected-rollback-checks | wc -l)" -eq 46 ]]
[[ "$("$transaction" --expected-rollback-checks | LC_ALL=C sort -u | wc -l)" -eq 46 ]]
printf 'action_17s_retry_regression_assertion_expected_labels_exact=true\n'
assert_static_policy
printf 'action_17s_retry_regression_assertion_static_policy=true\n'
if [[ "${CADDY_VALIDATION_CONTAINER:-}" == 1 ]]; then
    [[ "$caddy_root" = /workspace/homelab-server-configs/Caddy ]]
    printf 'action_17s_retry_container_projection_validated=true\n'
else
    run_production_path_regression
fi
printf 'action_17s_retry_regression_assertion_production_boundary=true\n'

printf 'action_17s_retry_node_b_marker_migration_regression_complete=true\n'
