...
readonly prefix=durable_apprise_action34b_regression
test_directory=$(cd -- "$(dirname -- "${BASH_SOURCE[0]}")" && pwd)
readonly test_directory
readonly caddy_root=${test_directory%/tests}
readonly repository_root=${caddy_root%/Caddy}
readonly transaction=$caddy_root/scripts/apply-durable-apprise-action34b.sh
readonly outer=$caddy_root/scripts/run-dual-node-durable-apprise-action34b-outer.sh
...
readonly predecessor_transaction=$caddy_root/scripts/apply-durable-apprise-action34a.sh
readonly predecessor_outer=$caddy_root/scripts/run-dual-node-durable-apprise-action34a-outer.sh
readonly predecessor_manifest=$caddy_root/manifests/durable-apprise-action34a.tsv
readonly predecessor_action_manifest=$caddy_root/manifests/durable-apprise-action34a.yaml
readonly predecessor_regression=$test_directory/durable-apprise-action34a-regression.sh
readonly expected_predecessor_transaction_sha256=723d249f9746317eab37779a4172006a4d06e0e5fbc8da906e0e1f6120a4e6d0
readonly expected_predecessor_outer_sha256=6dfd94c46a0c759aec4f0bc81f48ccab769e5b6f527a290241e45316bfba24f7
readonly expected_predecessor_manifest_sha256=6eb8b42edf4120ad60befbdbddf54cf71435a0f8ba52840c8d748cac176ae621
readonly expected_predecessor_action_manifest_sha256=baa04dc8a8f97beb1b7a8244639e0648e2b63ff3c4d87d4d38fb30959a64d593
readonly expected_predecessor_regression_sha256=a4f1bef36d9d168d91b4d45304e19be28022dc53488c7ba2c0f0b47d8edb4911
readonly failed_payload_sha256=15ff568ac2e0f66d6ba662d9d300470ec17ec37b6140782c8b47d2df3081dcd3

fixture_root=$(mktemp -d /tmp/caddy-action34b-regression.XXXXXX)
...
check() {
    local action34b_regression_label=$1
    shift
    if "$@" >/dev/null 2>&1; then
        printf '%s_check_%s=true\n' "$prefix" "$action34b_regression_label"
        return 0
    fi
    printf '%s_check_%s=false\n' "$prefix" "$action34b_regression_label" >&2
    return 1
}

candidate_sources_match() {
    local action34b_regression_source action34b_regression_target action34b_regression_mode
    local action34b_regression_baseline action34b_regression_candidate action34b_regression_source_path
    # conditional-validator-explicit-failures-begin
    while IFS=$'\t' read -r action34b_regression_source action34b_regression_target \
        action34b_regression_mode action34b_regression_baseline action34b_regression_candidate; do
        if [[ -z "$action34b_regression_source" || "$action34b_regression_source" = \#* ]]; then
            continue
        fi
        : "$action34b_regression_target" "$action34b_regression_mode" "$action34b_regression_baseline"
        case "$action34b_regression_source" in
            Caddy/*) action34b_regression_source_path=$repository_root/$action34b_regression_source ;;
            homelab-dns/*) action34b_regression_source_path=$repository_root/../$action34b_regression_source ;;
            *) return 1 ;;
        esac
        [[ -f "$action34b_regression_source_path" && ! -L "$action34b_regression_source_path" ]] || return 1
        [[ "$(file_hash "$action34b_regression_source_path")" = "$action34b_regression_candidate" ]] || return 1
    done <"$artifact_manifest"
    # conditional-validator-explicit-failures-end
}

function_order_is_safe() {
    local action34b_regression_backup_line action34b_regression_install_line
    local action34b_regression_marker_line action34b_regression_mutation_line
    # conditional-validator-explicit-failures-begin
    action34b_regression_backup_line=$(grep -nF "backup_artifacts \"\$action34b_stage\"" "$transaction" | cut -d: -f1) || return 1
    action34b_regression_install_line=$(grep -nF "install_artifacts \"\$action34b_stage\"" "$transaction" | cut -d: -f1) || return 1
    action34b_regression_marker_line=$(grep -nF "install -m 0600 /dev/null \"\$backup_root/mutation-started\"" "$transaction" | cut -d: -f1) || return 1
    action34b_regression_mutation_line=$(grep -nF 'mutation_started=true' "$transaction" | tail -n 1 | cut -d: -f1) || return 1
    [[ "$action34b_regression_backup_line" -lt "$action34b_regression_install_line" ]] || return 1
    [[ "$action34b_regression_marker_line" -lt "$action34b_regression_mutation_line" ]] || return 1
    # conditional-validator-explicit-failures-end
}
...
successor_does_not_rerun_predecessors() {
    ! grep -Eq 'apply-durable-apprise-action34a?\.sh|run-dual-node-durable-apprise-action34a?-outer\.sh' \
        "$transaction" "$outer"
}
