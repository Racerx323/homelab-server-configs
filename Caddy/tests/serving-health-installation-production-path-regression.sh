...
readonly transaction=$repository_root/Caddy/scripts/apply-coupled-serving-health-action35t.sh
readonly outer=$repository_root/Caddy/scripts/run-coupled-serving-health-action35t-outer.sh
...
grep -Fq 'action_35_t_production_path_test_complete=true' "$root/transaction.stdout"
grep -Fq 'action_35_t_expected_production_inventory_node_a_dns_health_helper=' \
    "$root/transaction.stdout"
grep -Fq 'action_35_t_observed_production_inventory_node_a_dns_health_helper=' \
    "$root/transaction.stdout"
grep -Fq 'action_35_t_outer_production_path_test_complete=true' "$root/outer.stdout"
grep -Fq 'action_35_t_outer_check_keepalived_parser_not_invoked=true' "$root/outer.stdout"
grep -Fq 'cd / && sudo -n /bin/bash -s --' "$root/outer/raw/outer-preflight.txt"
grep -Eq '/tmp/caddy-action35t-test-[A-Za-z0-9]+-node-a' "$root/outer/raw/outer-preflight.txt"
grep -Eq '/tmp/caddy-action35t-test-[A-Za-z0-9]+-node-b' "$root/outer/raw/outer-preflight.txt"
...
grep -Fq 'action_35_t_check_node_a_quarantine_top_level_exact=true' \
    "$root/transaction/raw/node-a-quarantine-baseline.txt"
grep -Fq 'action_35_t_check_node_a_quarantine_after_disposition_inventory=true' \
    "$root/transaction/raw/node-a-quarantine-disposition.txt"
grep -Fq 'action_35_t_check_node_a_quarantine_restored_top_level_exact=true' \
