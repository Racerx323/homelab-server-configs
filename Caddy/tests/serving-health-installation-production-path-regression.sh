...
readonly transaction=$repository_root/Caddy/scripts/apply-coupled-serving-health-action35k.sh
readonly outer=$repository_root/Caddy/scripts/run-coupled-serving-health-action35k-outer.sh
...
grep -Fq 'action_35_k_production_path_test_complete=true' "$root/transaction.stdout"
grep -Fq 'action_35_k_expected_production_inventory_node_a_dns_health_helper=' \
    "$root/transaction.stdout"
grep -Fq 'action_35_k_observed_production_inventory_node_a_dns_health_helper=' \
    "$root/transaction.stdout"
grep -Fq 'action_35_k_outer_production_path_test_complete=true' "$root/outer.stdout"
grep -Fq 'cd / && sudo -n /bin/bash -s --' "$root/outer/raw/outer-preflight.txt"
grep -Eq '/tmp/caddy-action35k-test-[A-Za-z0-9]+-node-a' "$root/outer/raw/outer-preflight.txt"
grep -Eq '/tmp/caddy-action35k-test-[A-Za-z0-9]+-node-b' "$root/outer/raw/outer-preflight.txt"
