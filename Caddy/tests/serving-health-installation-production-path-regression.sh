...
readonly transaction=$repository_root/Caddy/scripts/apply-coupled-serving-health-action35n.sh
readonly outer=$repository_root/Caddy/scripts/run-coupled-serving-health-action35n-outer.sh
...
grep -Fq 'action_35_n_production_path_test_complete=true' "$root/transaction.stdout"
grep -Fq 'action_35_n_expected_production_inventory_node_a_dns_health_helper=' \
    "$root/transaction.stdout"
grep -Fq 'action_35_n_observed_production_inventory_node_a_dns_health_helper=' \
    "$root/transaction.stdout"
grep -Fq 'action_35_n_outer_production_path_test_complete=true' "$root/outer.stdout"
grep -Fq 'cd / && sudo -n /bin/bash -s --' "$root/outer/raw/outer-preflight.txt"
grep -Eq '/tmp/caddy-action35n-test-[A-Za-z0-9]+-node-a' "$root/outer/raw/outer-preflight.txt"
grep -Eq '/tmp/caddy-action35n-test-[A-Za-z0-9]+-node-b' "$root/outer/raw/outer-preflight.txt"
...
node_b_disposition_line=$(grep -n ' retained-disposition node-b ' \
    "$root/outer/raw/outer-preflight.txt" | cut -d: -f1)
...
[[ "$node_b_disposition_line" -lt "$node_b_install_line" ]]
