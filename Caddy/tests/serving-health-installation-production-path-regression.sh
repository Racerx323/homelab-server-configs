...
readonly transaction=$repository_root/Caddy/scripts/apply-coupled-serving-health-action35v.sh
readonly outer=$repository_root/Caddy/scripts/run-coupled-serving-health-action35v-outer.sh
...
grep -Fq 'action_35_v_production_path_test_complete=true' "$root/transaction.stdout"
grep -Fq 'action_35_v_expected_production_inventory_node_a_dns_health_helper=' \
    "$root/transaction.stdout"
grep -Fq 'action_35_v_observed_production_inventory_node_a_dns_health_helper=' \
    "$root/transaction.stdout"
grep -Fq 'action_35_v_outer_production_path_test_complete=true' "$root/outer.stdout"
grep -Fq 'action_35_v_outer_check_keepalived_parser_not_invoked=true' "$root/outer.stdout"
grep -Fq 'cd / && sudo -n /bin/bash -s --' "$root/outer/raw/outer-preflight.txt"
grep -Eq '/tmp/caddy-action35v-test-[A-Za-z0-9]+-node-a' "$root/outer/raw/outer-preflight.txt"
grep -Eq '/tmp/caddy-action35v-test-[A-Za-z0-9]+-node-b' "$root/outer/raw/outer-preflight.txt"
...
grep -Fxq pi "$root/outer/transaction-through-outer/runuser.calls"
grep -Fxq keepalived_script "$root/outer/transaction-through-outer/runuser.calls"
...
grep -Fq 'current_after=20260817T160328Z-472d68b9-2bfb-40f1-8563-0754067182ca' \
    "$root/outer/transaction-through-outer/raw/post-promotion-sequence.txt"
grep -Fq 'action_35_v_check_local_candidate_selected=true' \
    "$root/outer/transaction-through-outer/raw/post-promotion-sequence.txt"
...
grep -Fq 'action_35_v_check_node_a_quarantine_top_level_exact=true' \
    "$root/transaction/raw/node-a-quarantine-baseline.txt"
grep -Fq 'action_35_v_check_node_a_quarantine_after_disposition_inventory=true' \
    "$root/transaction/raw/node-a-quarantine-disposition.txt"
grep -Fq 'action_35_v_check_node_a_quarantine_restored_top_level_exact=true' \
