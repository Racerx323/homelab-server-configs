...
readonly transaction=$caddy_root/scripts/transact-caddy-ha-reliability-action33d.sh
readonly outer=$caddy_root/scripts/run-dual-node-caddy-ha-reliability-action33d-outer.sh
...
CADDY_ACTION33D_SSH_BIN=/bin/false /bin/bash "$outer" --self-test >/tmp/action33d-self-test.stdout
grep -Fxq 'action_33d_outer_self_test_complete=true' /tmp/action33d-self-test.stdout
rm -f -- /tmp/action33d-self-test.stdout

require_literal 'cd / && sudo -n /bin/bash' "$outer"
require_literal '-s -- $mode $role $run_id $scenario $argument' "$outer"
require_literal 'capture_files "$action33d_outer_stdout" "$action33d_outer_stderr" "$action33d_outer_status_file"' "$outer"
...
require_literal 'systemd-run --unit "caddy-action33d-reboot-$run_id-$scenario"' "$transaction"
require_literal 'systemd-run --unit "$action33d_remote_unit" --on-active=45s' "$transaction"
require_literal '--publish-emergency' "$outer"
require_literal '--reject-normal' "$outer"
require_literal 'sleep 12' "$outer"
require_literal 'action33d-$run_id-' "$outer"
...
require_literal 'assert_replay_destination_absent "$action33d_remote_destination"' "$transaction"
require_literal 'suspend_sync_health_for_lsyncd_outage publish' "$transaction"
require_literal 'expect_invalid_transport_fail_closed' "$transaction"
require_literal 'invalid_transport_lsyncd_exec_status_255' "$transaction"
...
replay_absence_line=$(grep -nF 'assert_replay_destination_absent "$action33d_remote_destination"' "$transaction" | cut -d: -f1)
...
fixture_root=$(mktemp -d /tmp/action33d-regression.XXXXXX)
...
# Exercise the actual invalid-transport boundary. The real service start may
# return success before receiver rejection makes the managed process exit 255.
invalid_result=exit-code
systemctl() {
    printf '%s\n' "$*" >>"$health_trace"
    case "$1:$2" in
        is-active:caddy-sync-health.timer)
        ...
        start:caddy-lsyncd.service) return 0 ;;
        is-active:caddy-lsyncd.service)
            printf 'failed\n'
            return 3
            ;;
        show:caddy-lsyncd.service)
            case "$*" in
                *'-p Result --value'*) printf '%s\n' "$invalid_result" ;;
                *'-p ExecMainStatus --value'*) printf '255\n' ;;
...
grep -Fq 'check_invalid_transport_lsyncd_failed=true' \
    "$fixture_root/invalid-transport.stdout"
grep -Fq 'check_invalid_transport_lsyncd_result_exit_code=true' \
    "$fixture_root/invalid-transport.stdout"
grep -Fq 'check_invalid_transport_lsyncd_exec_status_255=true' \
    "$fixture_root/invalid-transport.stdout"
grep -Fq 'check_invalid_transport_lsyncd_main_pid_zero=true' \
    "$fixture_root/invalid-transport.stdout"
grep -Fq 'check_invalid_transport_health_timer_remains_suspended=true' \
    "$fixture_root/invalid-transport.stdout"
invalid_result=success
if expect_invalid_transport_fail_closed \
    >"$fixture_root/invalid-transport-wrong.stdout" \
    2>"$fixture_root/invalid-transport-wrong.stderr"; then
    printf 'action33d regression accepted a non-fail-closed invalid transport\n' >&2
    exit 1
fi
grep -Fq 'check_invalid_transport_lsyncd_result_exit_code=false' \
    "$fixture_root/invalid-transport-wrong.stderr"

stage_invalid_case=$(sed -n \
    '/^stage_invalid()/,/^assert_invalid_rejected()/p' "$transaction")
grep -Fq 'expect_invalid_transport_fail_closed' <<<"$stage_invalid_case"
if grep -Fq 'restore_and_accept_sync_health invalid_transport' \
    <<<"$stage_invalid_case"; then
    printf 'action33d regression found the consumed PID-stability contract\n' >&2
    exit 1
fi
...
if assert_replay_destination_absent "$replay_destination" \
    ...
    printf 'action33d regression accepted an existing replay destination\n' >&2
    exit 1
fi
...
if assert_outbound_role_policy node-a "$fixture_outbound" \
    ...
    printf 'action33d regression accepted payload drift\n' >&2
    exit 1
fi
...
if assert_outbound_role_policy node-a "$fixture_outbound" \
    "$fixture_releases" "$fixture_current" >/dev/null 2>&1; then
    printf 'action33d regression accepted an outbound symlink\n' >&2
    exit 1
fi
...
printf 'caddy_ha_reliability_action33d_regression_complete=true\n'
