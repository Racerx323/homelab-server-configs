...
readonly prefix=action_28r_regression
...
regression_root=$(mktemp -d /tmp/caddy-action28r-regression.XXXXXX)
...
cleanup() {
    if [[ "${CADDY_ACTION28R_KEEP_FIXTURE:-}" = 1 ]]; then
...
record_check() {
    local action28r_regression_label=$1
    local action28r_regression_stdout=$regression_root/check-$1.stdout
    local action28r_regression_stderr=$regression_root/check-$1.stderr

    shift
    if "$@" >"$action28r_regression_stdout" 2>"$action28r_regression_stderr"; then
        printf '%s_check_%s=true\n' "$prefix" "$action28r_regression_label"
        return 0
    fi
    if [[ -s "$action28r_regression_stdout" ]]; then
        sed "s/^/${prefix}_failure_${action28r_regression_label}_stdout=/" \
            "$action28r_regression_stdout" >&2
    fi
    if [[ -s "$action28r_regression_stderr" ]]; then
        sed "s/^/${prefix}_failure_${action28r_regression_label}_stderr=/" \
            "$action28r_regression_stderr" >&2
    fi
    printf '%s_check_%s=false\n' "$prefix" "$action28r_regression_label" >&2
    return 1
}
command_rejected() {
...
prepare_node_b() {
    local action28r_regression_backup=$node_b_root/var/backups/caddy-ha/action28p-node-b-caddy-vrrp-relinquish

    install -d -m 0755 "$node_b_root/etc/keepalived/conf.d" "$node_b_root/state" \
        "$node_b_root/run" "$node_b_root/var/backups/caddy-ha"
    install -d -m 0700 "$action28r_regression_backup"
    cp -- "$node_b_fixture" "$node_b_root/etc/keepalived/keepalived.conf"
    render_node_b_fragment
    install -m 0600 "$node_b_fixture" "$action28r_regression_backup/keepalived.conf"
    printf '\ninclude /etc/keepalived/conf.d/caddy-ha.conf\n' \
        >>"$action28r_regression_backup/keepalived.conf"
    install -m 0600 "$node_b_root/etc/keepalived/conf.d/caddy-ha.conf" \
        "$action28r_regression_backup/caddy-ha.conf"
    printf 'action=28p\nbaseline_main_sha256=%s\nfragment_sha256=%s\nretired_main_sha256=%s\n' \
        "$node_b_baseline_sha256" "$node_b_fragment_sha256" "$node_b_main_sha256" \
        >"$action28r_regression_backup/manifest"
    chmod 0600 "$action28r_regression_backup/manifest"
...
prepare_node_a() {
    local action28r_regression_backup=$node_a_root/var/backups/caddy-ha/action28m-node-a-caddy-service-restoration
...
write_fake_commands() {
    ...
    # The expression belongs to the generated fixture script.
    # shellcheck disable=SC2016
    printf '%s\n' '#!/usr/bin/env bash' \
        'printf "%s\\n" "${ACTION28R_FAKE_HOSTNAME:-j1-svpihole0}"' >"$fake_bin/hostname"
    ...
    printf '%s\n' '#!/usr/bin/env bash' \
        'set -Eeuo pipefail' \
        'case "${1:-}" in' \
        '    is-active) grep -Fqx "${3:-}=active" "$ACTION28R_FIXTURE_ROOT/state/services" ;;' \
        '    reload)' \
        '        [[ "${2:-}" = keepalived.service ]]' \
        '        if grep -Fqx "        10.1.0.56/22 dev eth0" "$ACTION28R_FIXTURE_ROOT/etc/keepalived/keepalived.conf"; then' \
        '            printf "%s\\n" "2: eth0 inet 10.1.0.55/22 scope global eth0" "2: eth0 inet 10.1.0.56/22 scope global eth0" >"$ACTION28R_FIXTURE_ROOT/state/ipv4"' \
        '            printf "%s\\n" "2: eth0 inet6 fd36:5aa8:6971:1::55/128 scope global" "2: eth0 inet6 fd36:5aa8:6971:1::56/128 scope global" >"$ACTION28R_FIXTURE_ROOT/state/ipv6"' \
        '        else' \
        '            printf "2: eth0 inet 10.1.0.55/22 scope global eth0\\n" >"$ACTION28R_FIXTURE_ROOT/state/ipv4"' \
        '            printf "2: eth0 inet6 fd36:5aa8:6971:1::55/128 scope global\\n" >"$ACTION28R_FIXTURE_ROOT/state/ipv6"' \
        '        fi' \
        '        ;;' \
        '    *) exit 64 ;;' \
    ...
    printf '%s\n' '#!/usr/bin/env bash' \
        'set -Eeuo pipefail' \
        'case " $* " in' \
        '    *" -4 "*) cat "$ACTION28R_FIXTURE_ROOT/state/ipv4" ;;' \
        '    *" -6 "*) cat "$ACTION28R_FIXTURE_ROOT/state/ipv6" ;;' \
        '    *) exit 64 ;;' \
        'esac' >"$fake_bin/ip"
    printf '%s\n' '#!/usr/bin/env bash' \
        'case " $* " in' \
    ...
    printf '%s\n' '#!/usr/bin/env bash' \
        ...
        '    counter_file=$ACTION28R_FIXTURE_ROOT/state/busctl-count' \
        '    count=0' \
        '    [[ ! -f "$counter_file" ]] || count=$(<"$counter_file")' \
        '    count=$((count + 1))' \
        '    printf "%s\\n" "$count" >"$counter_file"' \
        '    if [[ "${ACTION28R_FORCE_MASTER:-}" = 1 || "$count" -le 4 ]]; then' \
        "        printf '%s\\n' '(us) 1 \"Master\"'" \
        '    else' \
        "        printf '%s\\n' '(us) 1 \"Backup\"'" \
        '    fi' \
...
run_transaction_producer() {
    (
        cd /
        ACTION28R_FIXTURE_ROOT=$node_a_root \
            CADDY_ACTION28R_TEST_MODE=1 \
            CADDY_ACTION28R_FRAGMENT_SHA256=$node_a_fixture_fragment_sha256 \
            CADDY_ACTION28R_TEST_PATH="$fake_bin:/usr/bin:/bin" \
            CADDY_ACTION28R_FIXTURE_ROOT=$node_a_root \
            /bin/bash "$transaction" --execute
    )
}
run_node_b_convergence_producer() {
    local action28r_regression_force_master=${1:-0}

    (
        cd /
        rm -f -- "$node_b_root/state/busctl-count"
        ACTION28R_FIXTURE_ROOT=$node_b_root \
            ACTION28R_FAKE_HOSTNAME=j1-svpihole00 \
            ACTION28R_FORCE_MASTER=$action28r_regression_force_master \
            CADDY_ACTION28R_TEST_MODE=1 \
            CADDY_ACTION28R_TEST_PATH="$fake_bin:/usr/bin:/bin" \
            CADDY_ACTION28R_FIXTURE_ROOT=$node_b_root \
            CADDY_ACTION28R_NODE_A_MAIN_SHA256=$coupled_main_sha256 \
            CADDY_ACTION28R_NODE_A_FRAGMENT_SHA256=$node_a_fixture_fragment_sha256 \
            CADDY_ACTION28R_NODE_B_MAIN_SHA256=$node_b_main_sha256 \
            CADDY_ACTION28R_NODE_B_FRAGMENT_SHA256=$node_b_fragment_sha256 \
            /bin/bash "$convergence_inspector" --node-b 's=fixture;i=1'
    )
}
run_outer() {
    CADDY_ACTION28R_TEST_MODE=1 CADDY_ACTION28R_SKIP_LOCAL_GATES=true \
        CADDY_ACTION28R_FRAGMENT_SHA256=$node_a_fixture_fragment_sha256 \
        CADDY_ACTION28R_SSH_BIN=$fake_ssh \
        CADDY_ACTION28R_EVIDENCE_ROOT=$regression_root/evidence \
        ACTION28R_NODE_B_INSPECTOR_HASH="$(file_hash "$node_b_inspector")" \
        ACTION28R_NODE_A_INSPECTOR_HASH="$(file_hash "$node_a_inspector")" \
        ACTION28R_OBSERVATION_START_HASH="$(file_hash "$observation_start")" \
        ACTION28R_CONVERGENCE_INSPECTOR_HASH="$(file_hash "$convergence_inspector")" \
        ACTION28R_TRANSACTION_HASH="$(file_hash "$transaction")" \
        ACTION28R_NODE_B_ROOT=$node_b_root ACTION28R_NODE_A_ROOT=$node_a_root \
        ACTION28R_FAKE_BIN=$fake_bin \
        ACTION28R_NODE_A_MAIN_HASH=$baseline_main_sha256 \
        ACTION28R_NODE_A_BASELINE_HASH=$baseline_main_sha256 \
        ACTION28R_NODE_A_FRAGMENT_HASH=$node_a_fixture_fragment_sha256 \
        ACTION28R_NODE_B_MAIN_HASH=$node_b_main_sha256 \
        ACTION28R_NODE_B_FRAGMENT_HASH=$node_b_fragment_sha256 \
        /bin/bash "$outer"
}
run_transcript_test() {
    local action28r_regression_node_b_convergence=${1:-$regression_root/evidence-output/node_b_convergence.stdout}
    ...
    CADDY_ACTION28R_TEST_MODE=1 \
        ...
        "$regression_root/evidence-output/node_b_observation_start.stdout" \
        "$regression_root/empty" 0 \
        "$regression_root/evidence-output/node_a_transaction.stdout" \
        "$regression_root/empty" 0 \
        "$action28r_regression_node_b_convergence" "$regression_root/empty" 0 \
        "$regression_root/evidence-output/node_b_final_state.stdout" \
        "$regression_root/empty" 0 \
        "$regression_root/evidence-output/node_a_final_state.stdout" \
        "$regression_root/empty" 0
}
...
PATH=/usr/bin:/bin
export PATH
received=$(mktemp /tmp/caddy-action28r-received.XXXXXX)
trap 'rm -f -- "$received"' EXIT
cat >"$received"
[[ " $* " == *' cd / && sudo -n /bin/bash -s --'* ]] || exit 90
received_hash=$(sha256sum "$received" | awk '{ print $1 }')
if [[ " $* " == *' pi@10.1.0.54 '* && "$received_hash" = "$ACTION28R_OBSERVATION_START_HASH" ]]; then
    cd /
    ACTION28R_FAKE_HOSTNAME=j1-svpihole00 \
        CADDY_ACTION28R_TEST_MODE=1 \
        CADDY_ACTION28R_TEST_PATH="$ACTION28R_FAKE_BIN:/usr/bin:/bin" \
        /bin/bash "$received"
elif [[ " $* " == *' pi@10.1.0.54 '* && " $* " == *' --node-b '* && \
    "$received_hash" = "$ACTION28R_CONVERGENCE_INSPECTOR_HASH" ]]; then
    cd /
    ACTION28R_FIXTURE_ROOT=$ACTION28R_NODE_B_ROOT \
        ACTION28R_FAKE_HOSTNAME=j1-svpihole00 \
        CADDY_ACTION28R_TEST_MODE=1 \
        CADDY_ACTION28R_TEST_PATH="$ACTION28R_FAKE_BIN:/usr/bin:/bin" \
        CADDY_ACTION28R_FIXTURE_ROOT=$ACTION28R_NODE_B_ROOT \
        CADDY_ACTION28R_NODE_A_MAIN_SHA256=9e3dbf9760733f0ddb46ba51996cfdd9ad9af723d561ee2376de8c7c7d6ee3aa \
        CADDY_ACTION28R_NODE_A_FRAGMENT_SHA256=$ACTION28R_NODE_A_FRAGMENT_HASH \
        CADDY_ACTION28R_NODE_B_MAIN_SHA256=$ACTION28R_NODE_B_MAIN_HASH \
        CADDY_ACTION28R_NODE_B_FRAGMENT_SHA256=$ACTION28R_NODE_B_FRAGMENT_HASH \
        /bin/bash "$received" --node-b 's=fixture;i=1'
elif [[ " $* " == *' pi@10.1.0.53 '* && " $* " == *' --node-a '* && \
    "$received_hash" = "$ACTION28R_CONVERGENCE_INSPECTOR_HASH" ]]; then
    cd /
    ACTION28R_FIXTURE_ROOT=$ACTION28R_NODE_A_ROOT \
        ACTION28R_FAKE_HOSTNAME=j1-svpihole0 \
        CADDY_ACTION28R_TEST_MODE=1 \
        CADDY_ACTION28R_TEST_PATH="$ACTION28R_FAKE_BIN:/usr/bin:/bin" \
        CADDY_ACTION28R_FIXTURE_ROOT=$ACTION28R_NODE_A_ROOT \
        CADDY_ACTION28R_NODE_A_MAIN_SHA256=9e3dbf9760733f0ddb46ba51996cfdd9ad9af723d561ee2376de8c7c7d6ee3aa \
        CADDY_ACTION28R_NODE_A_FRAGMENT_SHA256=$ACTION28R_NODE_A_FRAGMENT_HASH \
        CADDY_ACTION28R_NODE_B_MAIN_SHA256=$ACTION28R_NODE_B_MAIN_HASH \
        CADDY_ACTION28R_NODE_B_FRAGMENT_SHA256=$ACTION28R_NODE_B_FRAGMENT_HASH \
        /bin/bash "$received" --node-a
elif [[ " $* " == *' pi@10.1.0.54 '* && "$received_hash" = "$ACTION28R_NODE_B_INSPECTOR_HASH" ]]; then
    cd /
    CADDY_ACTION28PA_TEST_MODE=1 \
        ...
        /bin/bash "$received" --fixture-root "$ACTION28R_NODE_B_ROOT"
elif [[ " $* " == *' pi@10.1.0.53 '* && "$received_hash" = "$ACTION28R_NODE_A_INSPECTOR_HASH" ]]; then
    cd /
    CADDY_ACTION28MB_TEST_MODE=1 \
        CADDY_ACTION28MB_RETIRED_MAIN_SHA256=$ACTION28R_NODE_A_MAIN_HASH \
        CADDY_ACTION28MB_BASELINE_MAIN_SHA256=$ACTION28R_NODE_A_BASELINE_HASH \
        CADDY_ACTION28MB_FRAGMENT_SHA256=$ACTION28R_NODE_A_FRAGMENT_HASH \
        /bin/bash "$received" --fixture-root "$ACTION28R_NODE_A_ROOT"
elif [[ " $* " == *' pi@10.1.0.53 '* && " $* " == *' --execute '* && "$received_hash" = "$ACTION28R_TRANSACTION_HASH" ]]; then
    cd /
    ACTION28R_FIXTURE_ROOT=$ACTION28R_NODE_A_ROOT \
        CADDY_ACTION28R_TEST_MODE=1 \
        CADDY_ACTION28R_FRAGMENT_SHA256=$ACTION28R_NODE_A_FRAGMENT_HASH \
        CADDY_ACTION28R_TEST_PATH="$ACTION28R_FAKE_BIN:/usr/bin:/bin" \
        CADDY_ACTION28R_FIXTURE_ROOT=$ACTION28R_NODE_A_ROOT \
        /bin/bash "$received" --execute
elif [[ " $* " == *' pi@10.1.0.53 '* && " $* " == *' --rollback '* && "$received_hash" = "$ACTION28R_TRANSACTION_HASH" ]]; then
    cd /
    ACTION28R_FIXTURE_ROOT=$ACTION28R_NODE_A_ROOT \
        CADDY_ACTION28R_TEST_MODE=1 \
        CADDY_ACTION28R_FRAGMENT_SHA256=$ACTION28R_NODE_A_FRAGMENT_HASH \
...
record_check node_b_transient_master_converges run_node_b_convergence_producer
record_check node_b_prolonged_master_rejected command_rejected \
    run_node_b_convergence_producer 1
...
evidence_directory=$(find "$regression_root/evidence" -mindepth 1 -maxdepth 1 \
    -type d -name 'run.*' -print -quit)
readonly evidence_directory
record_check evidence_directory_present test -n "$evidence_directory"
ln -s -- "$evidence_directory" "$regression_root/evidence-output"
record_check coupled_main_installed test \
    "$(file_hash "$node_a_root/etc/keepalived/keepalived.conf")" = "$coupled_main_sha256"
record_check action_28r_backup_manifest grep -Fqx 'action=28r' \
    "$node_a_root/var/backups/caddy-ha/action28r-node-a-coupled-vip-acquisition/manifest"
...
record_check transcript_valid run_transcript_test
cp -- "$regression_root/evidence-output/node_b_convergence.stdout" \
    "$regression_root/altered.stdout"
sed -i '/action_28r_convergence_check_sample_11_ipv4_state_backup=true/d' \
    "$regression_root/altered.stdout"
record_check missing_label_rejected command_rejected run_transcript_test \
    "$regression_root/altered.stdout"
cp -- "$regression_root/evidence-output/node_b_convergence.stdout" \
    "$regression_root/altered.stdout"
sed -i 's/action_28r_convergence_check_sample_11_ipv4_state_backup=true/action_28r_convergence_check_sample_11_ipv4_state_backup=false/' \
    "$regression_root/altered.stdout"
record_check false_label_rejected command_rejected run_transcript_test \
    "$regression_root/altered.stdout"
cp -- "$regression_root/evidence-output/node_b_convergence.stdout" \
    "$regression_root/altered.stdout"
printf 'action_28r_convergence_check_unexpected=true\n' >>"$regression_root/altered.stdout"
record_check extra_label_rejected command_rejected run_transcript_test \
    "$regression_root/altered.stdout"
cp -- "$regression_root/evidence-output/node_b_convergence.stdout" \
    "$regression_root/altered.stdout"
sed -i '/action_28r_convergence_check_sample_11_ipv4_state_backup=true/{h;d};/action_28r_convergence_check_sample_11_ipv6_state_backup=true/{p;x}' \
    "$regression_root/altered.stdout"
record_check reordered_label_rejected command_rejected run_transcript_test \
    "$regression_root/altered.stdout"
...
record_check exact_execute_transport grep -Fq \
    "'cd / && sudo -n /bin/bash -s -- --execute'" "$outer"
record_check consumed_action_28q_not_invoked command_rejected grep -Eq \
    'run_remote.*accepted_action_28q_a' "$outer"
record_check candidate_production_boundary "$transaction" --candidate-test \
    "$node_a_root/var/backups/caddy-ha/action28r-node-a-coupled-vip-acquisition/keepalived.conf" \
    "$coupled_fixture"
