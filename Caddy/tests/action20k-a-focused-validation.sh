...
readonly prefix=action_20k_a_focused_validation
...
readonly regression_sha256=e7ef71360ab3e9e6b48327ebb1331a6c8df563c2dab5fc009b9472749d676d7f
...
readonly regression=$test_directory/action20k-a-node-a-unicast-ttl-postinstall-regression.sh

file_hash() { sha256sum "$1" | awk '{ print $1 }'; }
record_check() {
    local action20ka_focused_label=$1

    shift
    if "$@" >/dev/null; then
        printf '%s_check_%s=true\n' "$prefix" "$action20ka_focused_label"
        return 0
    fi
    printf '%s_check_%s=false\n' "$prefix" "$action20ka_focused_label" >&2
    return 1
}
