...
readonly -a shell_files=(
    "$caddy_root/scripts/transact-coupled-go-live-action28ad.sh"
    "$caddy_root/scripts/run-dual-node-coupled-go-live-action28ad-outer.sh"
    ...
    "$script_directory/action28ad-coupled-go-live-regression.sh"
...
/bin/bash "$script_directory/ssh-stream-local-evidence-policy.sh" --check \
    "$caddy_root/scripts/run-dual-node-coupled-go-live-action28ad-outer.sh"
/bin/bash "$script_directory/action28ad-coupled-go-live-regression.sh"
...
if command -v yamllint >/dev/null 2>&1; then
    yamllint -s "$caddy_root/manifests/caddy-coupled-go-live-action28ad.yaml"
    printf 'action_28ad_focused_yaml_validation=passed\n'
else
    [[ "${CADDY_VALIDATION_CONTAINER:-}" = 1 ]]
    printf 'action_28ad_focused_yaml_validation=host_authoritative\n'
fi
printf 'action_28ad_focused_validation_complete=true\n'
