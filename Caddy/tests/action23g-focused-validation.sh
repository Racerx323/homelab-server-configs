...
readonly prefix=action_23g_focused_validation
readonly driver_sha256=a8c726ec744e3d609d4bd8b81b3b49711d425b15d3c1da06e962721cb844da2a
readonly outer_sha256=99e5ab1774bc6f2c4422b0ebc995d9cdda4a31bc3f242c9a3e2a0ca454064323
...
readonly regression="$test_directory/action23g-node-a-unbound-a-records-regression.sh"
readonly focused="$test_directory/action23g-focused-validation.sh"
...
record_check() {
    local action23g_focused_label=$1

    shift
    if "$@" >/dev/null; then
        printf '%s_check_%s=true\n' "$prefix" "$action23g_focused_label"
        return 0
    fi
    printf '%s_check_%s=false\n' "$prefix" "$action23g_focused_label" >&2
    return 1
}

native_parser_check() (
    local action23g_parser_dir
    local action23g_parser_candidate
    local action23g_parser_root
    ...
    action23g_parser_dir=$(mktemp -d)
    trap 'rm -rf -- "$action23g_parser_dir"' EXIT
    action23g_parser_candidate="$action23g_parser_dir/pihole-local-zone.conf"
    action23g_parser_root="$action23g_parser_dir/unbound.conf"
    awk '
    ...
    ' "$source_local_zone" >"$action23g_parser_candidate"
    [[ "$(file_hash "$action23g_parser_candidate")" == "$candidate_sha256" ]]
    {
...
yaml_check() {
    ...
    grep -Fqx 'action: 23g' "$action_manifest" || return 1
    grep -Fqx 'status: defined' "$action_manifest" || return 1
    grep -Fqx 'successor_of: 23b' "$action_manifest" || return 1
...
for action23g_focused_entrypoint in "$driver" "$outer" "$regression" "$focused"; do
    record_check "executable_index_$(basename "$action23g_focused_entrypoint" | tr '.-' '__')" \
        test "$(git ls-files --stage -- \
            "Caddy/${action23g_focused_entrypoint#"$caddy_root/"}" | awk '{ print $1 }')" = 100755
done
