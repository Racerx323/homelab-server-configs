...
readonly regression=$script_directory/action20d-retry5-activation-boundary-regression.sh
...
run_validation() {
    local retry5_focused_label=$1

    shift
    if "$@"; then
        printf 'action_20d_retry5_focused_%s=true\n' "$retry5_focused_label"
        return 0
    fi
    printf 'action_20d_retry5_focused_%s=false\n' "$retry5_focused_label" >&2
    return 1
}
...
run_validation regression_exact hash_exact \
    88ae997a24084e050a668f5005e76498cbd9d5c4cd9066d48ce05ae15b0b3ccf \
    "$regression"
run_validation outer_exact hash_exact \
    f4a0906113e30aa68f429920f369137a5ae80d9344be94c988c0f7974e7919e8 \
    "$outer"
...
if grep -Eq '^[[:space:]]*(podman|ssh|scp|rsync)([[:space:]]|$)' \
    ...
    printf 'Action 20d retry5 focused definition contains a direct prohibited command.\n' >&2
    exit 1
fi
printf 'action_20d_retry5_focused_complete_suite_invoked=false\n'
printf 'action_20d_retry5_focused_podman_invoked=false\n'
printf 'action_20d_retry5_focused_node_contact=false\n'
printf 'action_20d_retry5_focused_readiness_invoked=false\n'
printf 'action_20d_retry5_focused_activation_invoked=false\n'
printf 'action_20d_retry5_focused_validation_complete=true\n'
