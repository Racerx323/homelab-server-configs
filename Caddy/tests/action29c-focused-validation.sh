...
readonly regression=$test_directory/action29c-final-deployment-acceptance-regression.sh
readonly manifest=$caddy_root/manifests/caddy-final-deployment-acceptance-action29c.yaml
readonly dns_repo=${CADDY_ACTION29C_DNS_REPO:-/home/aaron/code/homelab-dns}
...
if [[ "${CADDY_VALIDATION_CONTAINER:-}" != 1 ]]; then
    ...
    printf 'action_29c_focused_pihole_v5_sources=passed\n'
else
    printf 'action_29c_focused_pihole_v5_sources=host_authoritative\n'
fi
if command -v yamllint >/dev/null 2>&1; then
    yamllint -s "$manifest"
    printf 'action_29c_focused_yaml_validation=passed\n'
else
    [[ "${CADDY_VALIDATION_CONTAINER:-}" = 1 ]]
    printf 'action_29c_focused_yaml_validation=host_authoritative\n'
fi
/bin/bash "$outer" --self-test
printf 'action_29c_focused_validation_complete=true\n'
