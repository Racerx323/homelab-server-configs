...
readonly prefix=action_28j_b_focused
...
readonly regression=$test_directory/action28j-b-durable-evidence-regression.sh
readonly focused=$test_directory/action28j-b-focused-validation.sh
readonly manifest=$caddy_root/manifests/caddy-failover-post-action28j-b.yaml
...
readonly outer_sha256=c28d4beea44eab0c175c7e75ad666f3e1fae09dabc40da2f2427a4eefb9a4b83
readonly regression_sha256=561f6b3791ca9f10e4449fc262395a11c09a752f060cd61fd42403d903a70fe3
readonly manifest_sha256=3bd05d5dc67a0023547d10498f98bb5ead1c477287e1ec459e7c244595fa5d5e

file_hash() { sha256sum -- "$1" | awk '{ print $1 }'; }
record_check() {
    local action28j_b_focused_label=$1

    shift
    if "$@" >/dev/null 2>&1; then
        printf '%s_check_%s=true\n' "$prefix" "$action28j_b_focused_label"
        return 0
    fi
    printf '%s_check_%s=false\n' "$prefix" "$action28j_b_focused_label" >&2
    return 1
}
yaml_check() {
    ...
    grep -Fqx 'action: 28j-b' "$manifest" || return 1
...
record_check durable_evidence_regression /bin/bash "$regression"
record_check plan_gate grep -Fq 'Action 28j-b' "$plan"

for action28j_b_focused_entrypoint in "$outer" "$regression" "$focused"; do
    record_check "executable_$(basename "$action28j_b_focused_entrypoint" | tr '.-' '__')" \
        test -x "$action28j_b_focused_entrypoint"
done
