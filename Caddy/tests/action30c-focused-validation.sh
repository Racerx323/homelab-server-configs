...
readonly prefix=action_30c_focused

check() {
    local action30c_focused_label=$1

    shift
    if "$@" >/dev/null 2>&1; then
        printf '%s_check_%s=true\n' "$prefix" "$action30c_focused_label"
        return 0
    fi
    printf '%s_check_%s=false\n' "$prefix" "$action30c_focused_label" >&2
    return 1
}
manifest_valid() {
...
run_checks() {
    local action30c_focused_entry

    # conditional-validator-explicit-failures-begin
    for action30c_focused_entry in "$transaction" "$outer" "$regression" "$policy" "${BASH_SOURCE[0]}"; do
        check "$(basename "$action30c_focused_entry" | tr -c '[:alnum:]' _)_syntax" \
            /bin/bash -n "$action30c_focused_entry" || return 1
    done
    check regression /bin/bash "$regression" || return 1
    check policy /bin/bash "$policy" --check || return 1
    check shellcheck shellcheck "$transaction" "$outer" "$regression" "$policy" "${BASH_SOURCE[0]}" || return 1
    check canonical /bin/bash "$caddy_root/tests/shfmt-canonical.sh" --check \
        "$transaction" "$outer" "$regression" "$policy" "${BASH_SOURCE[0]}" || return 1
    ...
    check outer_self_test env CADDY_ACTION30C_SKIP_REGRESSION=true \
        CADDY_ACTION30C_SKIP_REPEATED_POLICIES=true /bin/bash "$outer" --self-test || return 1
