...
readonly prefix=action_20f_a_regression
...
readonly transaction_outer_sha256=0baca52ef0891f7e511f4f92b486d449c703863805499fbe67123d69d4afbe23
...
readonly transaction_outer="$caddy_root/scripts/run-node-a-caddy-health-group-correction-action20f-outer.sh"
...
production_label_alignment() {
    ...
    alignment_root=$(mktemp -d /tmp/caddy-action20f-a-labels.XXXXXX)
...
static_read_only_policy() {
    ...
    grep -Fq 'readonly -a exact_context=(setpriv --reuid 993 --regid 991 --clear-groups --)' \
        "$inspector" || return 1
...
node_a_contract() {
    grep -Fq 'readonly expected_backup_path=/var/backups/caddy-ha/action19e-node-a-keepalived-fragment.JgYBbS' \
        ...
        grep -Fq 'readonly expected_health_backup_path=/var/backups/caddy-ha/action20c-node-a-health-context.DzQFvI' \
            "$inspector" "$runner" &&
        grep -Fq 'readonly expected_group_backup_path=/var/backups/caddy-ha/action20f-node-a-health-group.AuUZOk' \
            "$inspector" "$runner" &&
        grep -Fq 'readonly expected_fragment_sha256=6f5a58ca8fd39a1218de6fc964f54aef69f374351c0727bbb0fabf899a80ef39' \
            "$inspector" "$runner" &&
...
write_contract_from_producer() {
    ...
    assertion_count=$("$inspector" --expected-assertions | wc -l)
    {
        while IFS= read -r assertion_label; do
            printf 'action_20f_a_assertion_%s=true\n' "$assertion_label"
        done < <("$inspector" --expected-assertions)
        printf '%s\n' \
            "action_20f_a_value_expected_assertion_count=$assertion_count" \
            'action_20f_a_value_backup_path=/var/backups/caddy-ha/action19e-node-a-keepalived-fragment.JgYBbS' \
            'action_20f_a_value_backup_count=1' \
            'action_20f_a_value_health_backup_path=/var/backups/caddy-ha/action20c-node-a-health-context.DzQFvI' \
            'action_20f_a_value_health_backup_count=1' \
            'action_20f_a_value_group_backup_path=/var/backups/caddy-ha/action20f-node-a-health-group.AuUZOk' \
            'action_20f_a_value_group_backup_count=1' \
            'action_20f_a_value_health_sha256=9bf531ab7cbb047f8f09ce5956b597d5c8b6ede397c166f0b040e2e92fe8bbab' \
            'action_20f_a_value_main_sha256=cf4858888ae80772f1a50dda7c0ea120ff083eafae33a0ad4ca291d44755c1e2' \
            'action_20f_a_value_fragment_sha256=6f5a58ca8fd39a1218de6fc964f54aef69f374351c0727bbb0fabf899a80ef39' \
            "action_20f_a_value_before_state_sha256=$state_hash" \
            "action_20f_a_value_after_state_sha256=$state_hash" \
            "action_20f_a_assertion_count=$assertion_count" \
            'action_20f_a_failed_assertion_count=0' \
            'action_20f_a_first_failure=none' \
            'action_20f_a_helper_execution=true' \
            'action_20f_a_exact_context_uid=993' \
            'action_20f_a_exact_context_gid=991' \
            'action_20f_a_exact_context_supplementary_groups=cleared' \
            'action_20f_a_exact_context_validation=true' \
            'action_20f_a_filesystem_mutations=false' \
            'action_20f_a_service_mutations=false' \
            'action_20f_a_vrrp_mutations=false' \
            'action_20f_a_vip_mutations=false' \
            'action_20f_a_persistent_mutations=false' \
            'action_20f_a_remote_complete=true'
        for capture_label in \
            ...
            printf '%s\n' \
                "action_20f_a_capture_${capture_label}_bytes=0" \
                "action_20f_a_capture_${capture_label}_lines=0" \
                "action_20f_a_capture_${capture_label}_sha256=e3b0c44298fc1c149afbf4c8996fb92427ae41e4649b934ca495991b7852b855" \
                "action_20f_a_capture_${capture_label}_classification=bounded_safe" \
                "action_20f_a_capture_${capture_label}_content_secured=empty"
        done
    } >"$output_path"
}
...
#!/usr/bin/env bash
set -Eeuo pipefail
cat >"${ACTION20FA_STDIN_CAPTURE:?}"
printf '%s\n' "$*" >"${ACTION20FA_ARGS_CAPTURE:?}"
cat "${ACTION20FA_TRANSCRIPT:?}"
exit "${ACTION20FA_SSH_STATUS:?}"
FAKE_SSH
    chmod 0700 "$fake_ssh"
    (
        cd "$repository_root"
        CADDY_ACTION20FA_INTERCEPTED_TEST=1 \
            CADDY_ACTION20FA_SSH_BINARY="$fake_ssh" \
            ACTION20FA_STDIN_CAPTURE="$case_root/stdin" \
            ACTION20FA_ARGS_CAPTURE="$case_root/args" \
            ACTION20FA_TRANSCRIPT="$transcript_path" \
            ACTION20FA_SSH_STATUS="$expected_ssh_status" \
            "$runner"
    ...
    grep -Fq 'action_20f_a_remote_stream_classification=bounded_safe' "$case_root/stdout"
    if [[ "$expected_runner_status" -eq 97 ]]; then
        protected_evidence=$(sed -n \
            's/^action_20f_a_protected_evidence=//p' "$case_root/stderr")
        [[ "$protected_evidence" == /tmp/caddy-action20f-a-runner.* ]] || return 1
        [[ -d "$protected_evidence" && ! -L "$protected_evidence" ]] || return 1
        rm -rf -- "$protected_evidence"
    else
        grep -Fq 'action_20f_a_runner_cleanup_complete=true' "$case_root/stdout"
    fi
}
...
regression_root=$(mktemp -d /tmp/caddy-action20f-a-regression.XXXXXX)
...
sed -i \
    -e 's/action_20f_a_assertion_fragment_hash_exact=true/action_20f_a_assertion_fragment_hash_exact=false/' \
    -e 's/action_20f_a_failed_assertion_count=0/action_20f_a_failed_assertion_count=1/' \
    -e 's/action_20f_a_first_failure=none/action_20f_a_first_failure=fragment_hash_exact/' \
...
if [[ "${CADDY_VALIDATION_CONTAINER:-}" = 1 ]]; then
    ...
    sed -i '/^action_20f_a_assertion_health_script_hash_exact=/d' \
    ...
    printf '%s\n' 'action_20f_a_assertion_health_script_hash_exact=true' \
