...
readonly transaction=$caddy_root/scripts/transact-caddy-ha-reliability-action33n.sh
readonly outer=$caddy_root/scripts/run-dual-node-caddy-ha-reliability-action33n-outer.sh
...
fixture_root=$(mktemp -d /tmp/action33n-regression.XXXXXX)
...
/bin/bash -n "$outer"
CADDY_ACTION33N_SSH_BIN=/bin/false /bin/bash "$outer" --self-test \
    >"$fixture_root/self-test.stdout"
grep -Fxq 'action_33n_outer_self_test_complete=true' \
    "$fixture_root/self-test.stdout"
grep -Fxq 'action_33n_outer_gate_baseline_bundle_self_test=true' \
    "$fixture_root/self-test.stdout"
/bin/bash "$transaction" --reject-capture-self-test \
    >"$fixture_root/reject-capture-self-test.stdout"
grep -Fxq \
    'action_33n_remote__check_reject_capture_stdout_regular=true' \
    "$fixture_root/reject-capture-self-test.stdout"
grep -Fxq \
    'action_33n_remote__check_reject_capture_stderr_regular=true' \
    "$fixture_root/reject-capture-self-test.stdout"
grep -Fxq \
    'action_33n_remote__check_reject_capture_status_regular=true' \
    "$fixture_root/reject-capture-self-test.stdout"
grep -Fxq 'action_33n_remote_reject_capture_self_test_complete=true' \
    "$fixture_root/reject-capture-self-test.stdout"

require_literal 'cd / && sudo -n /bin/bash' "$outer"
require_literal '-s -- $mode $role $run_id $scenario $argument' "$outer"
require_literal 'capture_files "$action33n_outer_stdout" "$action33n_outer_stderr" "$action33n_outer_status_file"' "$outer"
...
require_literal '/run/caddy-action33n-baseline-$run_id-$scenario-$role' "$transaction"
require_literal '/tmp/caddy-action33n-baseline-$run_id-$scenario-$role.tar' "$transaction"
...
reject_literal 'action33n_remote_emergency_hash' "$transaction"
...
require_literal 'test "$action33n_remote_source" = node-b' "$transaction"
require_literal 'test "$action33n_remote_source" = node-a' "$transaction"
...
require_literal 'action: 33n' "$manifest"
require_literal 'failed_predecessor: 33m' "$manifest"
...
require_literal 'consumed_action33m_evidence=/tmp/caddy-ssh-evidence/action33m/run.E9TEiy' "$outer"
require_literal 'consumed_action33m_evidence_sha256=b517fc714d7c7442dc8cd68e3af04fc8e81892a7241393be01727f2952da4a7e' "$outer"
require_literal 'action33m_failed_before_mutation_status' "$outer"
require_literal 'action33m_no_recovery_mutation_evidence' "$outer"
reject_literal 'run-dual-node-caddy-ha-reliability-action33m-outer.sh' "$outer"

/bin/bash "$transaction" --action33k-family-self-test \
    >"$fixture_root/action33k-family-self-test.stdout"
for expected in \
    action_33n_remote__check_action33k_emergency_family_accepted=true \
    action_33n_remote__check_action33k_normalized_family_accepted=true \
    action_33n_remote_action33k_wrong_source_rejected=true \
    action_33n_remote_action33k_wrong_payload_rejected=true \
    action_33n_remote_action33k_wrong_parent_rejected=true \
    action_33n_remote_action33k_wrong_normalized_hash_rejected=true \
    action_33n_remote_action33k_family_self_test_complete=true; do
...
grep -Fxq 'action_33n_outer_gate_consumed_rolling_failed_exact=true' \
    "$fixture_root/consumed-evidence.stdout"
grep -Fxq 'action_33n_outer_gate_consumed_rolling_recovery_window_bounded=true' \
    "$fixture_root/consumed-evidence.stdout"
grep -Fxq 'action_33n_outer_consumed_evidence_self_test_complete=true' \
    "$fixture_root/consumed-evidence.stdout"
/bin/bash "$outer" --availability-stop-self-test \
    >"$fixture_root/availability-stop.stdout"
grep -Fxq 'action_33n_outer_gate_stop-self-test_availability_grammar=true' \
    "$fixture_root/availability-stop.stdout"
grep -Fxq 'action_33n_outer_gate_stop_self_test_no_open_sample=true' \
    "$fixture_root/availability-stop.stdout"
grep -Fxq 'action_33n_outer_availability_stop_self_test_complete=true' \
    "$fixture_root/availability-stop.stdout"
write_sample() {
    local action33n_test_path=$1
    local action33n_test_timestamp=$2
    local action33n_test_state=$3
    printf '%s\t10.1.0.56\n%s\n' "$action33n_test_timestamp" \
        "$action33n_test_state" >>"$action33n_test_path"
}
run_acceptance() {
    local action33n_test_path=$1
    /bin/bash "$outer" --availability-self-test "$action33n_test_path" \
        >/dev/null
}
...
if run_acceptance "$outside"; then
    printf 'Action 33n accepted a failure outside the handoff window\n' >&2
    exit 1
fi
...
if run_acceptance "$multiple"; then
    printf 'Action 33n accepted multiple failures in one handoff\n' >&2
    exit 1
fi
...
if run_acceptance "$consecutive"; then
    printf 'Action 33n accepted consecutive failed samples\n' >&2
    exit 1
fi
...
if run_acceptance "$slow"; then
    printf 'Action 33n accepted an overlong failed-sample recovery\n' >&2
    exit 1
fi
...
if run_acceptance "$wide"; then
    printf 'Action 33n accepted an overlong handoff window\n' >&2
    exit 1
fi

printf 'caddy_ha_reliability_action33n_regression_complete=true\n'
