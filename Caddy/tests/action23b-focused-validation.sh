...
readonly prefix=action_23b_focused_validation
readonly driver_sha256=23bc6578d1dca6c2b0e29f66cdffb7a1656ad5232c4e770d17e12cb8b377101d
readonly outer_sha256=4a4340908592044c01928ca0ede85a352ac58df90f31b9e7071e8d4e99729674
...
readonly regression="$test_directory/action23b-node-a-unbound-a-records-regression.sh"
readonly focused="$test_directory/action23b-focused-validation.sh"
...
record_check() {
    local action23b_focused_label=$1

    shift
    if "$@" >/dev/null; then
        printf '%s_check_%s=true\n' "$prefix" "$action23b_focused_label"
        return 0
    fi
    printf '%s_check_%s=false\n' "$prefix" "$action23b_focused_label" >&2
    return 1
}

native_parser_check() (
    local action23b_parser_dir
    local action23b_parser_candidate
    local action23b_parser_root
    ...
    action23b_parser_dir=$(mktemp -d)
    trap 'rm -rf -- "$action23b_parser_dir"' EXIT
    action23b_parser_candidate="$action23b_parser_dir/pihole-local-zone.conf"
    action23b_parser_root="$action23b_parser_dir/unbound.conf"
    awk '
    ...
    ' "$source_local_zone" >"$action23b_parser_candidate"
    [[ "$(file_hash "$action23b_parser_candidate")" == "$candidate_sha256" ]]
    {
...
yaml_check() {
    ...
    grep -Fqx 'action: 23b' "$action_manifest" || return 1
...
for action23b_focused_entrypoint in "$driver" "$outer" "$regression" "$focused"; do
    record_check "executable_index_$(basename "$action23b_focused_entrypoint" | tr '.-' '__')" \
        test "$(git ls-files --stage -- \
            "Caddy/${action23b_focused_entrypoint#"$caddy_root/"}" | awk '{ print $1 }')" = 100755
done
