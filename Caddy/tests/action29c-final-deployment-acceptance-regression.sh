...
readonly prefix=action_29c_regression
test_directory=$(cd -- "$(dirname -- "${BASH_SOURCE[0]}")" && pwd)
readonly test_directory
readonly caddy_root=${test_directory%/tests}
readonly inspector=$caddy_root/scripts/inspect-final-deployment-action29c.sh
readonly outer=$caddy_root/scripts/run-final-deployment-acceptance-action29c-outer.sh
fixture_root=
...
run_regression() {
    ...
    local action29c_regression_backup
    local action29c_regression_owner
    local action29c_regression_group
    local action29c_regression_before_ftl
    local action29c_regression_before_domain
    ...
    for action29_regression_role in node-a node-b; do
        ...
        sed 's/^action_29c_remote_\(.*\)_observed_pihole_ftl_sha256=.*/action_29c_remote_\1_observed_pihole_ftl_sha256=aaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaa/' \
...
EOF
    ...
    action29c_regression_backup=$fixture_root/action29b-backup
    action29c_regression_owner=$(id -un) || return 1
    action29c_regression_group=$(id -gn) || return 1
    install -d -m 0700 "$action29c_regression_backup" || return 1
    printf '%s\n' before-ftl >"$action29c_regression_backup/pihole-FTL.conf.before" || return 1
    printf '%s\n' before-domain >"$action29c_regression_backup/local.theama.co.conf.before" || return 1
    printf '%s\n' committed >"$action29c_regression_backup/transaction.complete" || return 1
    chmod 0600 "$action29c_regression_backup/pihole-FTL.conf.before" \
        "$action29c_regression_backup/local.theama.co.conf.before" \
        "$action29c_regression_backup/transaction.complete" || return 1
    action29c_regression_before_ftl=$(sha256sum \
        "$action29c_regression_backup/pihole-FTL.conf.before" | awk '{ print $1 }') || return 1
    action29c_regression_before_domain=$(sha256sum \
        "$action29c_regression_backup/local.theama.co.conf.before" | awk '{ print $1 }') || return 1
    printf '%s\n' \
        'action=29b' 'role=node-a' \
        "before_ftl_sha256=$action29c_regression_before_ftl" \
        "before_domain_sha256=$action29c_regression_before_domain" \
        ...
        >"$action29c_regression_backup/manifest" || return 1
    chmod 0600 "$action29c_regression_backup/manifest" || return 1
    check action29b_backup_real_validator /bin/bash "$inspector" --validate-action29b-backup \
        node-a "$action29c_regression_backup" "$action29c_regression_owner" \
        "$action29c_regression_group" "$action29c_regression_before_ftl" \
        "$action29c_regression_before_domain" || return 1
    sed 's/^action=29b$/action=wrong/' "$action29c_regression_backup/manifest" \
        >"$fixture_root/altered-manifest" || return 1
    mv -fT "$fixture_root/altered-manifest" "$action29c_regression_backup/manifest" || return 1
    chmod 0600 "$action29c_regression_backup/manifest" || return 1
    check action29b_backup_altered_rejected reject_command /bin/bash "$inspector" \
        --validate-action29b-backup node-a "$action29c_regression_backup" \
        "$action29c_regression_owner" "$action29c_regression_group" \
        "$action29c_regression_before_ftl" "$action29c_regression_before_domain" || return 1
    ...
    check cleanup_unset_safe grep -Fqx \
        'action_29c_remote_cleanup_unset_safe=true' "$action29_regression_cleanup" || return 1
