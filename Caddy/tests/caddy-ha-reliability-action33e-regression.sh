...
readonly transaction=$caddy_root/scripts/transact-caddy-ha-reliability-action33e.sh
readonly outer=$caddy_root/scripts/run-dual-node-caddy-ha-reliability-action33e-outer.sh
...
CADDY_ACTION33E_SSH_BIN=/bin/false /bin/bash "$outer" --self-test >/tmp/action33e-self-test.stdout
grep -Fxq 'action_33e_outer_self_test_complete=true' /tmp/action33e-self-test.stdout
rm -f -- /tmp/action33e-self-test.stdout

require_literal 'cd / && sudo -n /bin/bash' "$outer"
require_literal '-s -- $mode $role $run_id $scenario $argument' "$outer"
require_literal 'capture_files "$action33e_outer_stdout" "$action33e_outer_stderr" "$action33e_outer_status_file"' "$outer"
...
require_literal 'systemd-run --unit "caddy-action33e-reboot-$run_id-$scenario"' "$transaction"
require_literal 'systemd-run --unit "$action33e_remote_unit" --on-active=45s' "$transaction"
require_literal '--publish-emergency' "$outer"
require_literal '--reject-normal' "$outer"
require_literal 'sleep 12' "$outer"
require_literal 'action33e-$run_id-' "$outer"
...
require_literal 'assert_replay_destination_absent "$action33e_remote_destination"' "$transaction"
...
replay_absence_line=$(grep -nF 'assert_replay_destination_absent "$action33e_remote_destination"' "$transaction" | cut -d: -f1)
...
fixture_root=$(mktemp -d /tmp/action33e-regression.XXXXXX)
...
if accept_and_freeze_invalid_receiver_rejection \
    ...
    printf 'action33e regression accepted transport without receiver rejection\n' >&2
    exit 1
fi
...
if grep -Fq 'restore_and_accept_sync_health invalid_transport' \
    ...
    printf 'action33e regression found the consumed PID-stability contract\n' >&2
    exit 1
fi
...
if assert_replay_destination_absent "$replay_destination" \
    ...
    printf 'action33e regression accepted an existing replay destination\n' >&2
    exit 1
fi
...
if assert_outbound_role_policy node-a "$fixture_outbound" \
    ...
    printf 'action33e regression accepted payload drift\n' >&2
    exit 1
fi
...
if assert_outbound_role_policy node-a "$fixture_outbound" \
    "$fixture_releases" "$fixture_current" >/dev/null 2>&1; then
    printf 'action33e regression accepted an outbound symlink\n' >&2
    exit 1
fi
...
printf 'caddy_ha_reliability_action33e_regression_complete=true\n'
