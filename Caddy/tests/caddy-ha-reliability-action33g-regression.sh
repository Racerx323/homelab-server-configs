...
readonly transaction=$caddy_root/scripts/transact-caddy-ha-reliability-action33g.sh
readonly outer=$caddy_root/scripts/run-dual-node-caddy-ha-reliability-action33g-outer.sh
...
CADDY_ACTION33G_SSH_BIN=/bin/false /bin/bash "$outer" --self-test >/tmp/action33g-self-test.stdout
grep -Fxq 'action_33g_outer_self_test_complete=true' /tmp/action33g-self-test.stdout
rm -f -- /tmp/action33g-self-test.stdout

require_literal 'cd / && sudo -n /bin/bash' "$outer"
require_literal '-s -- $mode $role $run_id $scenario $argument' "$outer"
require_literal 'capture_files "$action33g_outer_stdout" "$action33g_outer_stderr" "$action33g_outer_status_file"' "$outer"
...
require_literal 'systemd-run --unit "caddy-action33g-reboot-$run_id-$scenario"' "$transaction"
require_literal 'systemd-run --unit "$action33g_remote_unit" --on-active=45s' "$transaction"
require_literal '--publish-emergency' "$outer"
require_literal '--reject-normal' "$outer"
require_literal 'sleep 12' "$outer"
require_literal 'action33g-$run_id-' "$outer"
require_literal 'action33f_recovered_baseline_consumed' "$outer"
require_literal 'action33e_completed_cases_consumed' "$outer"
reject_literal 'action33e_invalid_release_consumed' "$outer"
reject_literal 'action33e_interrupted_transfer_consumed' "$outer"
...
require_literal 'assert_replay_destination_absent "$action33g_remote_destination"' "$transaction"
...
replay_absence_line=$(grep -nF 'assert_replay_destination_absent "$action33g_remote_destination"' "$transaction" | cut -d: -f1)
...
require_literal 'build_candidate_manifest "$action33g_remote_destination"' \
    "$transaction"
require_literal 'rebuild_candidate_manifest "$action33g_remote_destination"' \
    "$transaction"
require_literal 'manifest_file_set_matches "$action33g_remote_destination"' \
    "$transaction"
reject_literal 'xargs -0 sha256sum >manifest.sha256.new' "$transaction"
...
fixture_root=$(mktemp -d /tmp/action33g-regression.XXXXXX)
...
# The sourced production definitions receive this same rewritten path above.
# Assign it explicitly so the regression contract is visible to static analysis.
outgoing_root=$fixture_root/outbound
run_id=fixture-run
...
# Exercise the production manifest builder against the receiver's exact file
# set contract. The replacement manifest is built outside the candidate tree,
# atomically installed, and independently accepted before transport.
manifest_candidate=$outgoing_root/action33g-$run_id-manifest-contract
manifest_broken=$outgoing_root/action33g-$run_id-manifest-broken
install -d -m 0700 "$outgoing_root" "$manifest_candidate" "$manifest_broken"
printf 'candidate payload\n' >"$manifest_candidate/Caddyfile"
printf '%s\n' \
    '{"revision":"manifest-contract","parent_revision":"fixture-parent","source_node":"node-a","created_at":"2026-08-12T00:00:00Z"}' \
    >"$manifest_candidate/release-manifest.json"
printf 'obsolete\n' >"$manifest_candidate/manifest.sha256"
rebuild_candidate_manifest "$manifest_candidate" manifest-contract \
    >"$fixture_root/manifest-build.stdout"
grep -Fq 'check_manifest_manifest-contract_file_set_exact=true' \
    "$fixture_root/manifest-build.stdout"
grep -Fq 'check_manifest_manifest-contract_hashes_valid=true' \
    "$fixture_root/manifest-build.stdout"
manifest_file_set_matches "$manifest_candidate"
(
    cd "$manifest_candidate"
    sha256sum --strict --check manifest.sha256 >/dev/null
)
test ! -e "$outgoing_root/action33g-$run_id-manifest-manifest-contract.tmp"

# Reproduce and reject Action 33f's exact self-inclusion defect: redirection
# creates manifest.sha256.new before find enumerates the candidate.
cp -a -- "$manifest_candidate/." "$manifest_broken/"
(
    cd "$manifest_broken"
...
if manifest_file_set_matches "$manifest_broken"; then
    printf 'action33g regression accepted candidate-local manifest staging\n' >&2
    exit 1
fi
chmod -R u+w "$manifest_candidate" "$manifest_broken"
rm -rf -- "$manifest_candidate" "$manifest_broken"
...
if accept_and_freeze_invalid_receiver_rejection \
    ...
    printf 'action33g regression accepted transport without receiver rejection\n' >&2
    exit 1
fi
...
if grep -Fq 'restore_and_accept_sync_health invalid_transport' \
    ...
    printf 'action33g regression found the consumed PID-stability contract\n' >&2
    exit 1
fi
...
if grep -Fq 'pid_stable' "$fixture_root/conflict-transport.stdout"; then
    printf 'action33g regression found PID stability inside conflict transport\n' >&2
    exit 1
fi
...
if assert_conflict_retained "$conflict_one+$conflict_two" \
    ...
    printf 'action33g regression accepted a conflict destination release\n' >&2
    exit 1
fi
...
if assert_replay_destination_absent "$replay_destination" \
    ...
    printf 'action33g regression accepted an existing replay destination\n' >&2
    exit 1
fi
...
if assert_outbound_role_policy node-a "$fixture_outbound" \
    ...
    printf 'action33g regression accepted payload drift\n' >&2
    exit 1
fi
...
if assert_outbound_role_policy node-a "$fixture_outbound" \
    "$fixture_releases" "$fixture_current" >/dev/null 2>&1; then
    printf 'action33g regression accepted an outbound symlink\n' >&2
    exit 1
fi
...
printf 'caddy_ha_reliability_action33g_regression_complete=true\n'
