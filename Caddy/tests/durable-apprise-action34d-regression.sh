...
readonly prefix=durable_apprise_action34d_regression
test_directory=$(cd -- "$(dirname -- "${BASH_SOURCE[0]}")" && pwd)
readonly test_directory
readonly caddy_root=${test_directory%/tests}
readonly repository_root=${caddy_root%/Caddy}
readonly transaction=$caddy_root/scripts/apply-durable-apprise-action34d.sh
readonly outer=$caddy_root/scripts/run-dual-node-durable-apprise-action34d-outer.sh
readonly artifact_manifest=$caddy_root/manifests/durable-apprise-action34d.tsv
...
readonly predecessor_transaction=$caddy_root/scripts/apply-durable-apprise-action34c.sh
readonly predecessor_outer=$caddy_root/scripts/run-dual-node-durable-apprise-action34c-outer.sh
readonly predecessor_manifest=$caddy_root/manifests/durable-apprise-action34c.tsv
readonly predecessor_action_manifest=$caddy_root/manifests/durable-apprise-action34c.yaml
readonly predecessor_regression=$test_directory/durable-apprise-action34c-regression.sh
readonly expected_predecessor_transaction_sha256=898aa7be094b8dc4f5398b3b0968d8d7a2106b2fe2d0b58d4b277b927d044274
readonly expected_predecessor_outer_sha256=866f5226030c286070d785e18dda756893e6d6509ebe4232b6663bfe41157e84
readonly expected_predecessor_manifest_sha256=ca4a5be9a19c55ebc9940a5a7a45efbccacfaeb1ca881f9b139df000b9880aec
readonly expected_predecessor_action_manifest_sha256=3b1680140aac4239b175b7922c271d19fb56aa4cf306f8186eabca00d7c97409
readonly expected_predecessor_regression_sha256=0da927e8a4f10f289b0aff4bec24a0b26e74b52ef59531068b6759bb75ef90e4
...
fixture_root=$(mktemp -d /tmp/caddy-action34d-regression.XXXXXX)
...
check() {
    local action34d_regression_label=$1
    shift
    if "$@" >/dev/null 2>&1; then
        printf '%s_check_%s=true\n' "$prefix" "$action34d_regression_label"
        return 0
    fi
    printf '%s_check_%s=false\n' "$prefix" "$action34d_regression_label" >&2
    return 1
}

candidate_sources_match() {
    local action34d_regression_source action34d_regression_target action34d_regression_mode
    local action34d_regression_baseline action34d_regression_candidate action34d_regression_source_path
    # conditional-validator-explicit-failures-begin
    while IFS=$'\t' read -r action34d_regression_source action34d_regression_target \
        action34d_regression_mode action34d_regression_baseline action34d_regression_candidate; do
        if [[ -z "$action34d_regression_source" || "$action34d_regression_source" = \#* ]]; then
            continue
        fi
        : "$action34d_regression_target" "$action34d_regression_mode" "$action34d_regression_baseline"
        case "$action34d_regression_source" in
            Caddy/*) action34d_regression_source_path=$repository_root/$action34d_regression_source ;;
            homelab-dns/*) action34d_regression_source_path=$repository_root/../$action34d_regression_source ;;
            *) return 1 ;;
        esac
        [[ -f "$action34d_regression_source_path" && ! -L "$action34d_regression_source_path" ]] || return 1
        [[ "$(file_hash "$action34d_regression_source_path")" = "$action34d_regression_candidate" ]] || return 1
    done <"$artifact_manifest"
    # conditional-validator-explicit-failures-end
}

function_order_is_safe() {
    local action34d_regression_backup_line action34d_regression_install_line
    local action34d_regression_marker_line action34d_regression_mutation_line
    # conditional-validator-explicit-failures-begin
    action34d_regression_backup_line=$(grep -nF "backup_artifacts \"\$action34d_stage\"" "$transaction" | cut -d: -f1) || return 1
    action34d_regression_install_line=$(grep -nF "install_artifacts \"\$action34d_stage\"" "$transaction" | cut -d: -f1) || return 1
    action34d_regression_marker_line=$(grep -nF "install -m 0600 /dev/null \"\$backup_root/mutation-started\"" "$transaction" | cut -d: -f1) || return 1
    action34d_regression_mutation_line=$(grep -nF 'mutation_started=true' "$transaction" | tail -n 1 | cut -d: -f1) || return 1
    [[ "$action34d_regression_backup_line" -lt "$action34d_regression_install_line" ]] || return 1
    [[ "$action34d_regression_marker_line" -lt "$action34d_regression_mutation_line" ]] || return 1
    # conditional-validator-explicit-failures-end
}
...
successor_does_not_rerun_predecessors() {
    ! grep -Eq 'apply-durable-apprise-action34(a|b|c)?\.sh|run-dual-node-durable-apprise-action34(a|b|c)?-outer\.sh' \
        "$transaction" "$outer"
}
...
notifier_prerequisite_is_exact() {
    local action34d_regression_source action34d_regression_target action34d_regression_mode
    local action34d_regression_baseline action34d_regression_candidate
    local action34d_regression_matches=0
    # conditional-validator-explicit-failures-begin
    while IFS=$'\t' read -r action34d_regression_source action34d_regression_target \
        action34d_regression_mode action34d_regression_baseline action34d_regression_candidate; do
        [[ "$action34d_regression_target" = /usr/local/bin/keepalived-notify.sh ]] || continue
        [[ "$action34d_regression_source" = homelab-dns/Keepalived/scripts/keepalived-notify.sh ]] || return 1
        [[ "$action34d_regression_mode" = 0755 ]] || return 1
        [[ "$action34d_regression_baseline" = "$observed_notifier_sha256" ]] || return 1
        [[ "$action34d_regression_candidate" = "$candidate_notifier_sha256" ]] || return 1
        action34d_regression_matches=$((action34d_regression_matches + 1))
    done <"$artifact_manifest"
    [[ "$action34d_regression_matches" -eq 1 ]] || return 1
    # conditional-validator-explicit-failures-end
}
