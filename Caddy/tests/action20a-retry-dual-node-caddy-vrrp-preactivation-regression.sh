...
readonly prefix=action_20a_retry_regression
readonly probe_sha256=19d3fffb5519d143c4e7c27c384ba9d30020ffc0174fb320be5088ca90c51076
readonly runner_sha256=cfac0ff96026ec1586ad3265e44d5962310cc9bfb18b0bdfcf883fb38ef0e536
...
readonly probe="$caddy_root/scripts/inspect-dual-node-caddy-vrrp-preactivation-action20a-retry.sh"
readonly runner="$caddy_root/scripts/run-dual-node-caddy-vrrp-preactivation-action20a-retry.sh"
...
run_intercepted_case() {
    ...
    case "$probe_corruption" in
        none) ;;
        node-a-missing)
            awk '!removed && /^action_20a_retry_probe_assertion_/ { removed=1; next } { print }' \
                "$case_root/node-a.probe" >"$case_root/node-a.probe.new"
            mv "$case_root/node-a.probe.new" "$case_root/node-a.probe"
            ;;
        node-b-duplicate)
            duplicate_line=$(grep -m1 '^action_20a_retry_probe_assertion_' \
...
FAKE_SSH
    ...
    (
        ...
        CADDY_ACTION20ARETRY_INTERCEPTED_TEST=1 \
    ...
    if [[ "$observed_status" -ne "$expected_status" ]]; then
        ...
        sed -n '/action_20a_retry_assertion_.*=false/p;/action_20a_retry_first_failure=/p' \
            "$case_root/stdout" >&2
    ...
    if ! grep -Fqx "action_20a_retry_readiness=$expected_readiness" \
...
regression_root=$(mktemp -d /tmp/caddy-action20a-retry-regression.XXXXXX)
