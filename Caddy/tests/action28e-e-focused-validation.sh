...
readonly prefix=action_28e_e_focused
...
readonly regression=$test_directory/action28e-e-dual-node-post-execution-regression.sh
readonly focused=$test_directory/action28e-e-focused-validation.sh
readonly manifest=$caddy_root/manifests/protocol-v2-post-action28e-e.yaml
...
readonly action28e_d_runner=$caddy_root/scripts/run-action28e-c-historical-identity-consumer-correction.sh
readonly inspector_sha256=96b159653883c5a67ae384b1129ce619f2e74f0b44c4846da1d44ae898cd96d9
readonly outer_sha256=f7e397e28cae312c57ff0d30b782ea3e3ce927715cf04ad0a891b0073618cb6c
readonly regression_sha256=100fce8cf9987895df8e1167c043489825cdce6d3c6e84ab57e9e0405b3cbd6c
readonly manifest_sha256=6ec3e315832683d50a6fe0990cde85483f9afaa524741ac5208e55e2747d4e07
...
record_check() {
    local action28e_e_focused_label=$1

    shift
    if "$@" >/dev/null; then
        printf '%s_check_%s=true\n' "$prefix" "$action28e_e_focused_label"
        return 0
    fi
    printf '%s_check_%s=false\n' "$prefix" "$action28e_e_focused_label" >&2
    return 1
}
yaml_check() {
    ...
    grep -Fqx 'action: 28e-e' "$manifest" || return 1
...
record_check action28e_d_immutable test "$(file_hash "$action28e_d_runner")" = "$action28e_d_runner_sha256"
...
for action28e_e_focused_entrypoint in "$inspector" "$outer" "$regression" "$focused"; do
    record_check "executable_index_$(basename "$action28e_e_focused_entrypoint" | tr '.-' '__')" \
        test "$(git ls-files --stage -- \
            "Caddy/${action28e_e_focused_entrypoint#"$caddy_root/"}" | awk '{ print $1 }')" = 100755
done
