...
- `prepare-startup.py`: offline rendering tied to the accepted bootstrap archive,
  with exact before/after hashes against installed artifacts. It refuses an unrelated active
...
and storage checks are assembled by `assemble-startup.py`; their execution and
results remain pending. The candidate refuses successful completion without
those live results. See [startup execution](STARTUP_EXECUTION.md).
...
copied into this procedure. Fresh deployed-policy verification remains required; accepted packet evidence
does not substitute for the actual application checks.

The reviewed stopped-volume preservation is archived. A fresh baseline and recovery
receipt are inputs to `assemble-startup.py`, which binds rendered artifacts,
concrete native/session/network/Job/resource/storage probes and expected fields.
The operation slot now defines application startup. The policy enables that
contract, but live execution still requires explicit approval of the exact bundle.
See [startup execution](STARTUP_EXECUTION.md) for mutation, acceptance, stop and
recovery boundaries. No startup was performed while preparing this contract.
