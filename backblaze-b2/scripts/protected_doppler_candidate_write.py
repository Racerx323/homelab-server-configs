#!/usr/bin/env python3
"""Store a one-time B2 candidate credential without exposing its values."""
...
PROJECT = "homelab-dev"
CONFIG = "prd_b2"
KEY_ID_NAME = "NAUTOBOT_RESTIC_B2_CANDIDATE_APPLICATION_KEY_ID"
KEY_VALUE_NAME = "NAUTOBOT_RESTIC_B2_CANDIDATE_APPLICATION_KEY"
CANDIDATE_NAMES = {KEY_ID_NAME, KEY_VALUE_NAME}
CANONICAL_NAMES = {
...
def set_argv(secret_name: str) -> tuple[str, ...]:
    if secret_name not in CANDIDATE_NAMES:
        raise ProtectedWriteBlocked("invalid_candidate_secret_name")
    return DOPPLER_BASE + (
        "secrets",
        "set",
        secret_name,
...
def store_from_fifos(
    ...
    try:
        ...
        if not CANDIDATE_NAMES <= names or not CANONICAL_NAMES <= names:
        ...
        raise
    return {
        "candidate_key_id_written": key_id_written,
        "candidate_key_value_written": key_value_written,
...
def write_evidence(
    ...
    document = {
        ...
        "candidate_values_or_identifiers_retained": False,
    ...
    if hasattr(os, "O_NOFOLLOW"):
        flags |= os.O_NOFOLLOW
    fd = os.open("candidate-write-result.json", flags, 0o600, dir_fd=root_fd)
    try:
