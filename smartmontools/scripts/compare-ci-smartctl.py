...
PATTERN = r'reset.*USB device|I/O error|Buffer I/O|EXT4-fs error|uas_eh|device offline|timing out command|blk_update_request|out of memory|oom-kill'
...
def query_argv(binary):
    return [binary, '-d', 'sntjmicron', '-r', 'nvmeioctl,2',
...
def main(bundle):
    ...
    if not any((p/'idVendor').exists() and (p/'idVendor').read_text().strip() == '152d'
               and (p/'idProduct').read_text().strip() == '0583' for p in ancestry):
    ...
    try:
        ...
        trial_cursor = cursor()
        for number, label in enumerate(['installed', 'candidate'] * 3, 1):
            token = cursor()
            prefix = f'{number:02d}-{label}'
            binary = '/usr/sbin/smartctl' if label == 'installed' else str(candidate)
            argv = query_argv(binary)
            row = {'number': number, 'binary': label, 'started_epoch': time.time(),
